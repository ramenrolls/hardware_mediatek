...
#define CFG_ENV_DATA_SIZE                                                         \
    (CFG_ENV_SIZE - sizeof(g_env[area].checksum) - sizeof(g_env[area].sig_head) - \
     sizeof(g_env[area].sig_tail))
#define CFG_ENV_DATA_OFFSET (sizeof(g_env[area].sig_head))
#define CFG_ENV_SIG_1_OFFSET \
    (CFG_ENV_SIZE - sizeof(g_env[area].checksum) - sizeof(g_env[area].sig_tail))
...
/* only used for user/kernel space sysenv coexist mechanism, removed if kernel module is phased out
 * - start*/
