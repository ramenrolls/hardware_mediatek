...
class DpVEncStream {
  public:
    ...
    DP_STATUS_ENUM queueSrcBuffer(void* pVA, uint32_t size);

    DP_STATUS_ENUM queueSrcBuffer(void* pVA, uint32_t MVA, uint32_t size);

    DP_STATUS_ENUM queueSrcBuffer(void** pVAList, uint32_t* pSizeList, int32_t planeNum);

    DP_STATUS_ENUM queueSrcBuffer(uint32_t* pMVAList, uint32_t* pSizeList, int32_t planeNum);

    DP_STATUS_ENUM queueSrcBuffer(void** pVAList, uint32_t* pMVAList, uint32_t* pSizeList,
                                  int32_t planeNum);

    // for ION file descriptor
    DP_STATUS_ENUM queueSrcBuffer(int32_t fileDesc, uint32_t* sizeList, uint32_t planeNum);
     ...
     */
    DP_STATUS_ENUM setSrcConfig(DpColorFormat srcFormat, int32_t srcWidth, int32_t srcHeight,
                                int32_t srcPitch, bool doFlush = true);
    ...
    DP_STATUS_ENUM setSrcConfig(int32_t width, int32_t height, int32_t YPitch, int32_t UVPitch,
                                ...
                                bool doFlush = true, DpSecure secure = DP_SECURE_NONE);
     ...
     *     else the API will return the error code.
     */
    DP_STATUS_ENUM setSrcCrop(int32_t XStart, int32_t XSubpixel, int32_t YStart, int32_t YSubpixel,
                              int32_t cropWidth, int32_t cropHeight);

    DP_STATUS_ENUM setSrcCrop(int32_t portIndex, int32_t XStart, int32_t XSubpixel, int32_t YStart,
                              int32_t YSubpixel, int32_t cropWidth, int32_t cropHeight);

    DP_STATUS_ENUM setSrcCrop(int32_t portIndex, int32_t XStart, int32_t XSubpixel, int32_t YStart,
                              int32_t YSubpixel, int32_t cropWidth, int32_t cropWidthSubpixel,
                              int32_t cropHeight, int32_t cropHeightSubpixel);

    DP_STATUS_ENUM queueDstBuffer(int32_t portIndex, void** pVAList, uint32_t* pSizeList,
                                  int32_t planeNum);

    DP_STATUS_ENUM queueDstBuffer(int32_t portIndex, uint32_t* pMVAList, uint32_t* pSizeList,
                                  int32_t planeNum);

    DP_STATUS_ENUM queueDstBuffer(int32_t portIndex, void** pVAList, uint32_t* pMVAList,
                                  uint32_t* pSizeList, int32_t planeNum);

    // for ION file descriptor
    DP_STATUS_ENUM queueDstBuffer(int32_t portIndex, int32_t fileDesc, uint32_t* pSizeList,
                                  int32_t planeNum);
     ...
     *     else the API will return the error code.
     */
    DP_STATUS_ENUM dequeueDstBuffer(int32_t portIndex, void** pVABase, bool waitBuf = true);
     ...
     *     else the API will return the error code.
     */
    DP_STATUS_ENUM setDstConfig(int32_t portIndex, DpColorFormat dstFormat, int32_t dstWidth,
                                int32_t dstHeight, int32_t dstPitch, bool doFlush = true);
     ...
     *     else the API will return the error code.
     */
    DP_STATUS_ENUM setRotation(int32_t portIndex, int32_t rotation);
     ...
     *     else the API will return the error code.
     */
    DP_STATUS_ENUM setFlipStatus(int32_t portIndex, bool flipStatus);
    ...
    DP_STATUS_ENUM setParameter(ISP_TPIPE_CONFIG_STRUCT& extraPara, ISP2MDP_STRUCT* isp2mdp,
                                uint32_t hint);

    DP_STATUS_ENUM setPortType(int32_t portIndex, uint32_t portType = 0,
                               DpVEnc_Config* VEnc_cfg = NULL);

    DP_STATUS_ENUM setSharpness(int32_t portIndex, int32_t gain) {
