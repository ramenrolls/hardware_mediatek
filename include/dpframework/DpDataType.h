...
class DpRect {
  public:
    ...
    inline DpRect(void)
        : x(eINITIAL_VALUE),
          sub_x(eINITIAL_VALUE),
          y(eINITIAL_VALUE),
          sub_y(eINITIAL_VALUE),
          w(eINITIAL_VALUE),
          sub_w(eINITIAL_VALUE),
          h(eINITIAL_VALUE),
          sub_h(eINITIAL_VALUE) {}

    inline DpRect(int32_t in_x, int32_t in_y, int32_t in_w, int32_t in_h, uint32_t in_sub_x = 0,
                  uint32_t in_sub_y = 0, uint32_t in_sub_w = 0, uint32_t in_sub_h = 0)
        : x(in_x),
          sub_x(in_sub_x),
          y(in_y),
          sub_y(in_sub_y),
...
class DpColorMatrix {
  ...
  public:
    ...
    inline DpColorMatrix(float in_c00, float in_c01, float in_c02, float in_c10, float in_c11,
                         float in_c12, float in_c20, float in_c21, float in_c22,
                         bool in_enable = true)
        : enable(in_enable),
          c00(in_c00),
          c01(in_c01),
    ...
    inline DpColorMatrix operator*(const DpColorMatrix& r) const {
        ...
        return DpColorMatrix(
                c00 * r.c00 + c01 * r.c10 + c02 * r.c20, c00 * r.c01 + c01 * r.c11 + c02 * r.c21,
                c00 * r.c02 + c01 * r.c12 + c02 * r.c22, c10 * r.c00 + c11 * r.c10 + c12 * r.c20,
                c10 * r.c01 + c11 * r.c11 + c12 * r.c21, c10 * r.c02 + c11 * r.c12 + c12 * r.c22,
                c20 * r.c00 + c21 * r.c10 + c22 * r.c20, c20 * r.c01 + c21 * r.c11 + c22 * r.c21,
                c20 * r.c02 + c21 * r.c12 + c22 * r.c22);
    }
    ...
    inline bool operator==(const DpColorMatrix& r) const {
        ...
        return (enable == r.enable) && (c00 == r.c00) && (c01 == r.c01) && (c02 == r.c02) &&
               (c10 == r.c10) && (c11 == r.c11) && (c12 == r.c12) && (c20 == r.c20) &&
               (c21 == r.c21) && (c22 == r.c22);
    }

    inline bool operator!=(const DpColorMatrix& r) const { return !(*this == r); }
};

inline DpColorMatrix operator*(const float l, const DpColorMatrix& r) {
    return DpColorMatrix(l * r.c00, l * r.c01, l * r.c02, l * r.c10, l * r.c11, l * r.c12,
                         l * r.c20, l * r.c21, l * r.c22, r.enable);
}
...
struct MDPSetting {
    ...
    MDPSetting() : buffer(NULL), size(0), offset(0) {}
};
...
struct DpDREParam {
    ...
    enum DRESRAM { SRAM00 = 0, SRAM01, SRAMDefault };
...
struct ClearZoomParam {
    ...
    ClearZoomParam() : captureShot(CAPTURE_SINGLE), p_customSetting(NULL) {}
};
...
struct DpHFGParam {
    ...
    uint32_t lowerISO;
    DpHFGParam()
        : p_slkParam(NULL), p_upperSetting(NULL), p_lowerSetting(NULL), upperISO(0), lowerISO(0) {}
};
...
struct DpPqParam {
    ...
    DpPqParam()
        : enable(0), scenario(MEDIA_UNKNOWN), srcGamut(DP_GAMUT_SRGB), dstGamut(DP_GAMUT_SRGB) {
        memset(&u, 0, sizeof(u));
    }
};
...
#define DP_COLORFMT_PACK(PACKED, LOOSE, VIDEO, PLANE, HFACTOR, VFACTOR, BITS, GROUP, SWAP_ENABLE, \
                         UNIQUEID)                                                                \
    (((PACKED) << 28) | ((LOOSE) << 27) | ((VIDEO) << 23) | ((PLANE) << 21) | ((HFACTOR) << 19) | \
...
#define DP_COLOR_IS_UV_COPLANE(color) \
    ((DP_COLOR_GET_PLANE_COUNT(color) == 2) && DP_COLOR_IS_YUV(color))
...
typedef enum DP_COLOR_ENUM {
    ...
    DP_COLOR_BAYER8_UNPAK =
            DP_COLORFMT_PACK(0, 0, 0, 1, 0, 0, 8, 2, 0, 22),  // fix 16 bits for pixel
    DP_COLOR_BAYER10_UNPAK =
            DP_COLORFMT_PACK(0, 0, 0, 1, 0, 0, 10, 2, 0, 22),  // fix 16 bits for pixel
    DP_COLOR_BAYER12_UNPAK =
            DP_COLORFMT_PACK(0, 0, 0, 1, 0, 0, 12, 2, 0, 22),  // fix 16 bits for pixel
    DP_COLOR_BAYER14_UNPAK =
            DP_COLORFMT_PACK(0, 0, 0, 1, 0, 0, 14, 2, 0, 22),  // fix 16 bits for pixel
    DP_COLOR_BAYER16_UNPAK =
            DP_COLORFMT_PACK(0, 0, 0, 1, 0, 0, 16, 2, 0, 22),  // fix 16 bits for pixel
...
enum DpSecure { DP_SECURE_NONE = 0, DP_SECURE = 1, DP_SECURE_PROTECTED = 2, DP_SECURE_SHIFT = 8 };
