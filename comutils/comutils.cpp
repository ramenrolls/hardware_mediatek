...
int32_t getEncoderInPutFormat() {
    ...
    if (property_get("persist.vendor.radio.vilte.enc.format", value, NULL)) {//resolution change
    interval format = atoi(value); VT_LOGI("[ID=%d]format
    %s",mMultiInstanceID,OmxFormatToString(format));
    }
    */
...
/*
default 0
ro_type : 0 -->rotate and resize with the same ratio  as the input buffer, fill the middle of the
output buffer with black edge on left-right ro_type : 1 -->crop input buffer with the same ratio as
output, rotate and resize to fully fill the output buffer, loose some input buffer content ro_type :
2 -->just rotate and resize the input buffer ,and fully fill the output buffer,distorte the picture
content ro_type : 3 -->ratate width changed W and H
*/
...
int vilte_ion_va_mva(unsigned int ion_fd, unsigned long va, unsigned int size, unsigned int* mva,
                     int* handleToBeFree) {
    // unsigned int ion_fd;
    int ion_user_handle;
    ...
    *mva = sys_data.get_phys_param.phy_addr;
    VT_LOGD("va 0x%lx, size %d,phy_addr 0x%x,mav 0x%x %d", va, size,
            sys_data.get_phys_param.phy_addr, *mva, ion_user_handle);
...
int64_t rotateBuffer(uint8_t* input, uint8_t* output, RotateInfo* info, int32_t* outFillLen) {
    ...
    ATRACE_CALL();
    VT_LOGD("input %p,s-w %d,s-h %d,s-F %s, t-w %d,t-h %d,t-bw %d,t-bh %d,t-F %s,output %p, degree "
            "%d,ro_type %d *outFillLen %d",
            input, srcWidth, srcHeight, OmxFormatToString(srcFormat), tarWidth, tarHeight,
            tarBufWidth, tarBufHeight, OmxFormatToString(tarFormat), output, degree, ro_type,
            *outFillLen);
    ...
    } else if (srcFormat == OMX_COLOR_Format32BitRGBA8888 ||
               srcFormat == OMX_COLOR_Format32bitARGB8888) {
        srcWStride = ROUND_16(srcWidth);
        srcHStride = ROUND_16(srcHeight);
    ...
    if (tarFormat == OMX_MTK_COLOR_FormatYV12) {
        ...
        // memset(dstYUVbuf_va + dstYUVbufSizeArray[0], 128,
        // dstYUVbufSizeArray[1]+dstYUVbufSizeArray[2]);
    ...
    *outFillLen = dstBufferSize;
    ...
    bliterr = blitStream.setSrcBuffer((void**)srcYUVbufArray, (unsigned int*)srcYUVbufSizeArray,
                                      srcNumPlanes);
#endif
    ...
    if (srcColorFormat == eARGB8888 || srcColorFormat == eRGBA8888) {
#if USING_MDP_BY_HIDL
        mpdParams.src_width = srcWidth;
        mpdParams.src_height = srcHeight;
        mpdParams.src_format = srcColorFormat;
        mpdParams.src_rect.x = srcRoi.x;
        mpdParams.src_rect.y = srcRoi.y;
        mpdParams.src_rect.w = srcRoi.w;
        mpdParams.src_rect.h = srcRoi.h;
#else
        bliterr = blitStream.setSrcConfig(srcWidth, srcHeight, srcColorFormat, eInterlace_None,
                                          &srcRoi);
#endif
    } else {
#if USING_MDP_BY_HIDL
        ...
        mpdParams.src_height = srcHeight;
        mpdParams.src_format = srcColorFormat;
        mpdParams.src_yPitch = yPitch;
        mpdParams.src_uvPitch = uvPitch;  // mpdParams.src_profile = 0;//DP_PROFILE_BT601
        mpdParams.src_rect.x = srcRoi.x;
        mpdParams.src_rect.y = srcRoi.y;
        mpdParams.src_rect.w = srcRoi.w;
        mpdParams.src_rect.h = srcRoi.h;
#else
        bliterr = blitStream.setSrcConfig(srcWidth, srcHeight, yPitch, uvPitch, srcColorFormat,
                                          DP_PROFILE_BT601, eInterlace_None, &srcRoi);
#endif
    }

    VT_LOGV("@ s-w %d  s-h%d srcConfig x %d y %d w %d h %d, yPitch%d,uvPitch%d,s-F %d", srcWidth,
            srcHeight, srcRoi.x, srcRoi.y, srcRoi.w, srcRoi.h, yPitch, uvPitch, srcColorFormat);
    ...
    VT_LOGV("@ t-w %d  t-h%d ,setDstBuffer+setDstConfig x %d y %d w %d h %d,rotate degree "
            "%d,yPitch%d,uvPitch%d,t-F %d",
            tarWidth, tarHeight, dstRoi.x, dstRoi.y, dstRoi.w, dstRoi.h, degree, yPitch, uvPitch,
            tarColorFormat);
    ...
    mpdParams.dst_width = resizeW;
    mpdParams.dst_height = resizeH;
    mpdParams.dst_format = tarColorFormat;
    mpdParams.dst_yPitch = yPitch;
    mpdParams.dst_uvPitch = uvPitch;  // mpdParams.dst_profile = 0;//DP_PROFILE_BT601
    mpdParams.dst_rect.x = dstRoi.x;
    mpdParams.dst_rect.y = dstRoi.y;
    mpdParams.dst_rect.w = dstRoi.w;
    mpdParams.dst_rect.h = dstRoi.h;
    ...
    ion_close(ion_fd);
#else
    bliterr = blitStream.setDstBuffer((void**)dstYUVbufArray, (unsigned int*)dstYUVbufSizeArray,
                                      tarNumPlanes);
    bliterr = blitStream.setDstConfig(resizeW, resizeH, yPitch, uvPitch, tarColorFormat,
                                      DP_PROFILE_BT601, eInterlace_None, &dstRoi);
    ...
    if (dumpYUV) {
        ...
        if (srcFormat == OMX_COLOR_Format32bitARGB8888 ||
            srcFormat == OMX_COLOR_Format32BitRGBA8888) {
...
#else
int64_t rotateBuffer(uint8_t* input, uint8_t* output, RotateInfo* info, int32_t* outFillLen) {
    VT_LOGE("should not called !!!:input %p,output %p, info %p *outFillLen %d", input, output, info,
            *outFillLen);
    return -1;
}
...
void MTKFindAVCSPSInfo(uint8_t* seqParamSet, size_t size, struct MtkSPSInfo* pSPSInfo) {
    ...
    pSPSInfo->height = (2 - frame_mbs_only_flag) * (pic_height_in_map_units_minus1 * 16 + 16);
    ...
    if (br.getBits(1)) {  // frame_cropping_flag
        ...
        ALOGV("frame_crop = (%u, %u, %u, %u), cropUnitX = %u, cropUnitY = %u",
              ...
              frame_crop_bottom_offset, cropUnitX, cropUnitY);

        pSPSInfo->width -= (frame_crop_left_offset + frame_crop_right_offset) * cropUnitX;
        pSPSInfo->height -= (frame_crop_top_offset + frame_crop_bottom_offset) * cropUnitY;
    }

    return;
}

sp<ABuffer> MakeAVCCodecSpecificData(const char* params, int32_t* profile, int32_t* level,
                                     int32_t* width, int32_t* height, int32_t* sarWidth,
                                     int32_t* sarHeight) {
    AString val(params);
    ...
    *level = 0;
    ...
    VT_LOGD("params : %s width=%d height=%d sarWidth=%d sarHeight=%d", val.c_str(), *width, *height,
            *sarWidth, *sarHeight);
    ...
    size_t csdSize = 1 + 3 + 1 + 1 /*configurationVersion +profilelevel +lengthSizes */
                     + 2 * numSeqParameterSets /*size field*/ + totalSeqParameterSetSize + 1 +
                     2 * numPicParameterSets /*size field*/ + totalPicParameterSetSize;
    ...
    for (size_t i = 0; i < numSeqParameterSets; ++i) {
        ...
        *out++ = nal->size() & 0xff;
        ...
        if (i == 0) {
            ...
            VT_LOGD("W %d H %d P %d L %d", spsInf.width, spsInf.height, spsInf.profile,
                    spsInf.level);
...
uint16_t U16_AT(const uint8_t* ptr) { return ptr[0] << 8 | ptr[1]; }
...
sp<ABuffer> MakeHEVCCodecSpecificData(const char* params, int32_t* width, int32_t* height) {
    ...
    *out++ = 0xfd;  // reserved(6bits,111111b)+chromaFormat(2)(0=monochrome, 1=4:2:0, 2=4:2:2,
    3=4:4:4)
    ...
    *out++ = 0xf8;  // reserved(5bits,11111b) + bitDepthChromaMinus8(3)
    ...
    // avgFrameRate (16bits,in units of frames/256 seconds,0 indicates an unspecified average frame
    rate) *out++ = avgFrameRate >> 8; *out++ = avgFrameRate & 0xff;

    // constantFrameRate(2bits,0=not be of constant frame
    rate),numTemporalLayers(3bits),temporalIdNested(1bits), *out++ = 0x03;
    // lengthSizeMinusOne(2bits)

    *out++ = numbOfArrays;  // numOfArrays

    if (videoParamSet != NULL) {
        *out++ = 0x3f & VPS_NAL_TYPE;  //
    array_completeness(1bit)+reserved(1bit,0)+NAL_unit_type(6bits)
    ...
    *height = pic_height_in_luma_samples;
    ...
    if (conformance_window_flag) {
        ...
        *height -= conf_win_top_offset + conf_win_bottom_offset * 2;
        ...
        ALOGI("frame_crop = (%u, %u, %u, %u)", conf_win_left_offset, conf_win_right_offset,
    ...
    ALOGI("bit_depth_luma_minus8 =%u, bit_depth_chroma_minus8 =%u", bit_depth_luma_minus8,
