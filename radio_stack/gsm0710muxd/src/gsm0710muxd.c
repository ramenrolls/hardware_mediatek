...
unsigned char test_channel_cmd[] = {
        GSM0710_CONTROL_TEST | GSM0710_CR, GSM0710_EA | (6 << 1), 'P', 'I', 'N', 'G', '\r', '\n',
};

/* MSC cmd pattern */
unsigned char msc_channel_cmd[] = {
        GSM0710_CONTROL_MSC | GSM0710_CR, /* byte#0: Type field */
        GSM0710_EA | (2 << 1), /* byte#1: Length field, its value=2 (2 parms in MSC: One byte is
                                  DLCI, the other is V.24 control signals) */
...
// static unsigned char psc_channel_cmd[] = { GSM0710_CONTROL_PSC | GSM0710_CR, GSM0710_EA | (0 <<
// 1), }; static unsigned char wakeup_sequence[] = { GSM0710_FRAME_FLAG, GSM0710_FRAME_FLAG, };

/* crc table from gsm0710 spec */
static const unsigned char r_crctable[] = {
...
#if (SIM_COUNT >= 2)
#if (SIM_COUNT >= 4)
#define MUXD_CH_NUM_ALL                                                            \
    (MUXD_CH_NUM_CCH + ((MUXD_CH_NUM_RILD + MUXD_CH_NUM_PDP + MUXD_CH_ATCI) * 4) + \
     MUXD_CH_NUM_VT + MUXD_CH_NUM_IMS + MUXD_CH_NUM_TEST + MUXD_CH_NUM_NW)
#elif (SIM_COUNT >= 3)
#define MUXD_CH_NUM_ALL                                                            \
    (MUXD_CH_NUM_CCH + ((MUXD_CH_NUM_RILD + MUXD_CH_NUM_PDP + MUXD_CH_ATCI) * 3) + \
     MUXD_CH_NUM_VT + MUXD_CH_NUM_IMS + MUXD_CH_NUM_TEST + MUXD_CH_NUM_NW)
#else
#define MUXD_CH_NUM_ALL                                                            \
    (MUXD_CH_NUM_CCH + ((MUXD_CH_NUM_RILD + MUXD_CH_NUM_PDP + MUXD_CH_ATCI) * 2) + \
     MUXD_CH_NUM_VT + MUXD_CH_NUM_IMS + MUXD_CH_NUM_TEST + MUXD_CH_NUM_NW)
#endif /*(SIM_COUNT >= 4)*/
#else  /*(SIM_COUNT >= 2) */
#define MUXD_CH_NUM_ALL                                                                         \
    (MUXD_CH_NUM_CCH + ((MUXD_CH_NUM_RILD + MUXD_CH_NUM_PDP + MUXD_CH_ATCI)) + MUXD_CH_NUM_VT + \
     MUXD_CH_NUM_IMS + MUXD_CH_NUM_TEST + MUXD_CH_NUM_EXT_AUD_SPEECH + MUXD_CH_NUM_NW)
...
// muxd contexts
Serial serial;
Channel channellist[GSM0710_MAX_CHANNELS];  // remember: [0] is not used acticly because it's the
                                            // control channel
...
static unsigned int pts_reopen =
        0; /*If != 0,  signals watchdog that one cahnnel needs to be reopened */
...
/*pthread */
/* Thread_ID:ser_read_thread -> Thread_Func: poll_thread_serial() -> Thread_Read_Ptr:
 * thread_serial_device_read() => Read data from /dev/ttyC0 and put data into serial->in_buf */
pthread_t ser_read_thread;
/* Thread_ID:frame_assembly_thread -> Thread_Func: assemble_frame_thread() -> Thread_Read_Ptr:
 * thread_serial_device_read() => Read data from serial->inbuf */
 ...
 */
static int baud_rates[] = {0, 9600, 19200, 38400, 57600, 115200, 230400, 460800, 921600};
...
static int baud_rate_index(int baud_rate) {
    ...
    for (i = 0; i < sizeof(baud_rates) / sizeof(*baud_rates); ++i)
        if (baud_rates[i] == baud_rate) return i;
    return -1;
}
...
unsigned char frame_calc_crc(const unsigned char* input, int length) {
    ...
    int i;
    for (i = 0; i < length; i++) fcs = r_crctable[fcs ^ input[i]];
...
static int syslogdump(const char* prefix, const unsigned char* ptr, unsigned int length) {
    ...
    for (unsigned int i = 0; i < length; i++) {
        ...
        if ((strlen(buffer) + strlen(temp) + 1) > 4096) {
            ...
            LOGMUX(LOG_DEBUG, "syslogdump ERROR: buffer size is low for %s %s", prefix, temp);
        } else {
...
static void dumpBuffer(GSM0710_Buffer* pBuf) {
    LOGMUX(LOG_DEBUG, "size %d, start %p, current %p, end %p", pBuf->datacount, pBuf->readp,
           pBuf->writep, pBuf->endp);
...
int write_frame(int channel, const unsigned char* input, int length, unsigned char type) {
    ...
    /* Default prefix_length = 1 (Frame flag) + 1 (Address field) + 1 (Control field) + 1 (Length
     * field) */
    int prefix_length = 4;
    int c;
...
//        LOGMUX(LOG_WARNING, "Didn't get frame-flag after wakeup");
...
#endif
#endif
    if (channel != MUXD_VT_CH_NUM) LOGMUX(LOG_DEBUG, "Sending frame to channel %d", channel);
    ...
    prefix[2] = type;
    if ((type == GSM0710_TYPE_UIH || type == GSM0710_TYPE_UI) && uih_pf_bit_received == 1 &&
        GSM0710_COMMAND_IS(input[0], GSM0710_CONTROL_MSC)) {
        prefix[2] = prefix[2] |
                    GSM0710_PF;   // Set the P/F bit in Response if Command from modem had it set
    ...
    if (!cmux_mode) {
        ...
        /* Using Address filed, Control field and Length field to calculate the FCS and store it in
         * postfix[0] */
        postfix[0] = frame_calc_crc(prefix + 1, prefix_length - 1);
        ...
        if (c != prefix_length) {
            LOGMUX(LOG_WARNING,
                   "Couldn't write the whole prefix to the serial port for the virtual port %d. "
                   "Wrote only %d bytes",
                   channel, c);
            ...
            /* But the current implementation, it will not record which index of next prefix data
             * shoud be written to the pty */
            /* Another potential problem: If it returns from here directly, original completed frame
             * maybe discontinuous with other subsystem's frame data */
            if (c < 0) {
                /* Check Error cause */
                if (writeErrno == ETXTBSY || writeErrno == ENODEV) {
                    LOGMUX(LOG_ERR, "write to MD get ETXTBSY/ENODEV (%d), drop data (%d)", errno,
                           length);
        ...
        if (length > 0) {
            ...
            if (length != c) {
                LOGMUX(LOG_WARNING,
                       "Couldn't write all data to the serial port from the virtual port %d. Wrote "
                       "only %d bytes",
                       channel, c);
                /* Note by LS: Potential Bug - When re-entering this function, it always sends from
                 * the prefix data again */
                /* If the remaining data exists due to previous write_frame(), it should not write
                 * the prefix data again */
                if (c < 0) {
                    /* Check Error Cause */
                    if (writeErrno == ETXTBSY || writeErrno == ENODEV) {
                        LOGMUX(LOG_ERR, "write to MD get ETXTBSY/ENODEV (%d), drop data (%d)",
                               errno, length);
                        mtk_property_set(PROPERTY_MODEM_EE, "1");
                        pthread_mutex_unlock(&write_frame_lock);
                        return length;
        ...
        if (c != 2) {
            LOGMUX(LOG_WARNING,
                   "Couldn't write the whole postfix to the serial port for the virtual port %d. "
                   "Wrote only %d bytes",
                   channel, c);
            /* Note by LS: Potential Bug - When re-entering this function, it always sends from the
             * prefix data again */
            /* If the remaining data exists due to previous write_frame(), it should not write the
             * prefix data again */
            /* Check Error Cause */
            if (writeErrno == ETXTBSY || writeErrno == ENODEV) {
                LOGMUX(LOG_ERR, "write to MD get ETXTBSY/ENODEV (%d), drop data (%d)", errno,
        ...
        offs += fill_adv_frame_buf(serial.adv_frame_buf + offs, prefix + 1,
                                   2); /* address, control */
        ...
        LOGMUX(LOG_DEBUG, "return from write()case=%d:wanted_written_len=%d,actual_written_len=%d",
               4, offs, c);
        if (c != offs) {
            LOGMUX(LOG_WARNING,
                   "Couldn't write the whole advanced packet to the serial port for the virtual "
                   "port %d. Wrote only %d bytes",
                   channel, c);
            pthread_mutex_unlock(&write_frame_lock);
            return 0;
        }
...
/* Add by LS */
static int rx_thread_write_frame(int channel, const unsigned char* input, int length,
                                 unsigned char type) {
#ifndef RX_THREAD_NONBLOCKING_WRITE_FRAME
    write_frame(channel, input, length, type);
#else
    /* Start a new thread to execute this BLOCKING write_frame: In this way, RX thread can handle
     * other channel's RX activity continuously */
...
static int handle_channel_data(unsigned char* buf, int len, int channel) {
    ...
    /* Note by LS: Bug - It should not always return 0 : It may reach the max write_retries to exit
     * the while loop */
    /* return 0; */
    return len - written;
}
...
static int logical_channel_close(Channel* channel) {
    ...
    if (channel->ptsname != NULL) free(channel->ptsname);
...
static int logical_channel_init(Channel* channel, int id) {
    ...
    /* Comment by LS:Because channel num#0 is used by Gsm0710Muxd locally and not exported to RILD
     * or network interface driver: */
...
static int logical_channel_establish(Channel* channel) {
    if (setup_pty_interface(channel) != 0) {
        ...
        // Set Contexts
        channel->v24_signals =
                GSM0710_SIGNAL_DV | GSM0710_SIGNAL_RTR | GSM0710_SIGNAL_RTC | GSM0710_EA;

        /* Only send SABM for each MUX's channel*/
        channel->sabm_ua_pending = 1;
        write_frame(channel->id, NULL, 0, GSM0710_TYPE_SABM | GSM0710_PF);

        LOGMUX(LOG_DEBUG, "Connecting %s to virtual channel %d on %s", channel->ptsname,
               channel->id, serial.devicename);
    }
    return GSM0710MUXD_SUCCESS;
}

/*
 * Purpose: Add this function for c_alloc_channel() and pty_restart_interface() to open a pty
 * channel and send the SABM command for it Input: Return:
 */
...
static int setup_pty_interface(Channel* channel) {
    ...
    // LOGMUX(LOG_DEBUG, "Found free channel %d fd %d on %s", channel->id, channel->fd,
    // channel->devicename);
    /* Comment by LS: Only channel num is larger than zero is necessary to create a pair of ptys */
    SYSCHECK(channel->fd = open(
                     channel->devicename,
                     O_RDWR | O_NONBLOCK));  // open pseudo terminal devices from /dev/ptmx master
    LOGMUX(LOG_VERBOSE,
           "setup_pty_interface SYSCHECK(channel->fd = open(channel->devicename, O_RDWR | "
           "O_NONBLOCK));");
    ...
    if (!strcmp(channel->devicename, "/dev/ptmx")) {
        ...
        if (channel->ptslink != NULL) {
            ...
            LOGMUX(LOG_INFO, "symlink done,fd %d: %s->%s", channel->fd, channel->ptsname,
                   channel->ptslink);
        }
#if 0
...
int pseudo_device_read(void* vargp) {
    ...
    /* The buffer size of N_TTY is defined as 4096 in tty.h: The maximum bytes of newly incoming
     * data maybe 4096 */
    /* Change the buf[4096] to buf[1024]: It should use 4 times read() to achieve the original
     * procedure */
    unsigned char buf[1024];
    int len = 0, read_action_count = 0;
    /* information from virtual port */
    /* channel->remaining records the num of bytes of data not written to the pty in the previous
     * write_frame() */
    /* And these remaining data are stored at the memory address pointed by channel->tmp and not
     * copied to the local buf yet */
    /* So, this time's newly read data should be stored to the local buf from the position buf +
     * channel->remaining */
    ...
    if (mux_setup_chnl_complete == 0) {
        LOGMUX(LOG_ERR, "access channel:%s too early, opened:%d", channel->devicename,
               channel->opened);
    ...
    if (!channel->opened && !LackofAtciChannel) {
        /* Note by LS: In our implementation: each non-control channel must be opened at the
    ...
    for (read_action_count = 0; read_action_count < MUXD_PTY_READ_ROUND; read_action_count++) {
        ...
        if (channel->tx_fc_off) {
            /* If tx_fc is off, it should block here and wait for tx_fc_on siganl to allow to send
             * data! */
            LOGMUX(LOG_VERBOSE, "Wait for FC On siganl");
            pthread_cond_wait(&channel->tx_fc_on_signal, &channel->tx_fc_lock);
            /* It receives the MSC cmd with FC On again: tx_fc_off should be reset as zero when
             * recving FC ON again */
            // channel->tx_fc_off = 0;
        ...
        if ((len = read(channel->fd, buf, sizeof(buf))) >= 0) {
            ...
            LOGMUX(LOG_DEBUG,
                   "Data from channel %d, wanted_read=%d, actual_read=%d bytes with "
                   "read_action_count=%d",
                   channel->id, sizeof(buf), len, read_action_count);
            ...
            if (channel->id == DEFAULT_MUX_UT_LB_TEST_CHNL_NUM) {
                ...
                if (len > 0) {
                    ...
                    while (lb_accumulated_write_len < len) {
                        lb_this_write_len = write(channel->fd, buf + lb_accumulated_write_len,
                                                  len - lb_accumulated_write_len);
                        if (lb_this_write_len >= 0) {
                            LOGMUX(LOG_DEBUG,
                                   "LB this time write back data len=%d, total read len=%d",
                                   lb_this_write_len, len);
                            ...
                            switch (errno) {
                                ...
                                case EAGAIN:
                                    LOGMUX(LOG_ERR,
                                           "Interrupt signal EAGAIN(i.e.,Nonblocing mode read but "
                                           "no data in buf now) caught");
                                    break;
#if 0
                            case EBADF:
                    ...
                    /* Simulate this time's read data are all written into the PTY for modem: It
...
#endif
                if (len > 0) remaining = handle_channel_data(buf, len, channel->id);

                /* copy remaining bytes from last packet into tmp */
                if (remaining > 0) {
                    /* Modify by LS: Because the serial.in_buf is opened as BLOCKING mode, the value
                     * returned from handle_channel_data() must be zero! */
            ...
            switch (errno) {
                ...
                case EAGAIN:
                    LOGMUX(LOG_ERR,
                           "Interrupt signal EAGAIN(i.e.,Nonblocing mode read but no data in buf "
                           "now) caught");
                    if (read_action_count == 0) {
                        /* pseudo_device_read() is invoked due to FDISSET, it should not no data in
                         * the buffer when 1st read() is invoked */
                        // skip the below assert because RILD will reopen channels due to SIM switch
                        // feature. Gsm0710Muxd_Assert(GSM0710MUXD_PTY_READ_ERR);
                    } else {
                        /* Add by LS: It alread read data from the buffer due to FDISSET, it can
                         * return now due to no data in the buffer now */
                        // read_action_count--;
                        /* Because read_action_count is not zero: It means at least one read action
                         * is performed!*/
                        /* If there is no data in the pty buffer, it can exit this for-loop then
                         * waits for next read_fd enabled via select() */
                        /* read_action_count is 3, after break statement: read_action_count is
                         * updated to 4 then exit this for-loop */
    ...
    if (read_action_count != MUXD_PTY_READ_ROUND) Gsm0710Muxd_Assert(GSM0710MUXD_PTY_READ_ERR);

    /* Total 4 read() action and each time try to read maximum size is 1024 bytes */
    /* Because N_TTY BUF SIZE is 4096: If each time read with buf's size 1024, it should execute 4
     * times read action */
    /* PTY's peer side may write 4096 bytes at one write action into the PTY Buffer: 4096 bytes data
     * to be processed due to FD_READ_SET is on */
    LOGMUX(LOG_VERBOSE, "End of Execute 4 read actions for this time's read_fd is select");
    return 0;

close_pty_channel:
    LOGMUX(LOG_ERR, "Appl. dropped connection, device %s shutting down. Set to be reopened",
           channel->ptsname);
...
/*
 * Purpose:  Allocate a channel and corresponding virtual port and a start a reading thread on that
 * port Input:      origin - string to define origin of allocation Return:    1 if fail, 0 if
 * success
 */
 ...
 * Return:    -
 */
static void gsm0710_buffer_destroy(GSM0710_Buffer* buf) { free(buf); }
 ...
 * Return:    number of characters written
 */
int gsm0710_buffer_write(GSM0710_Buffer* buf, const unsigned char* input, int length) {
    LOGMUX(LOG_VERBOSE, "Enter");
    LOGMUX(LOG_VERBOSE, "GSM0710 buffer (up-to-date): free %d, stored %d", gsm0710_buffer_free(buf),
           gsm0710_buffer_length(buf));
    int c = buf->endp - buf->writep;
    length = min(length, (int)gsm0710_buffer_free(buf));
    /* If buf->readp is located between the buf->writep and buf->endp: the free space will be less
     * than c */
    ...
    if (length > c) {
        ...
        /* After data is written to the end of buf, it will be wrapped around to the start of the
         * buf */
        ...
        buf->writep += length;
        if (buf->writep == buf->endp) buf->writep = buf->data;
    }

    pthread_mutex_lock(&buf->datacount_lock);
    /* After copying the data to the serial->in_buf, it is time to update datacount to avoid read
     * thread to get invalid data */
    buf->datacount += length; /*updating the data-not-yet-read counter*/
    LOGMUX(LOG_DEBUG, "GSM0710 buffer (up-to-date): written %d, free %d, stored %d", length,
           gsm0710_buffer_free(buf), gsm0710_buffer_length(buf));
    pthread_mutex_unlock(&buf->datacount_lock);

    pthread_mutex_lock(&buf->newdataready_lock);
    buf->newdataready = 1; /*signal assemble_frame_thread that new buffer data is ready and stored
                              in serial->in_buf */
...
void destroy_frame(GSM0710_Frame* frame) {
    if (frame->length > 0) {
        LOGMUX(LOG_VERBOSE, "frame_data ptr=0x%02X, frame_len=%d", (unsigned int)frame->data,
               frame->length);
...
static GSM0710_Frame* gsm0710_base_buffer_get_frame(GSM0710_Buffer* buf, int passed_depth_level) {
    int end = 0;
    int depth_level = 0;
    unsigned int length_needed =
            5;  // channel(Address Field), type(Control Field), length(Lengthe Field), fcs, flag
    unsigned char fcs = 0xFF;
    ...
    if (!buf->flag_found) {  // no frame started
        ...
        /* Add by LS: Once out of sync- After it checks all invalid bytes in buffer, it should
         * signal another thread to put data into buffer again */
        /* In order to find the Start Flag then sync again! */

        pthread_mutex_lock(&buf->bufferready_lock);
        if (buf->input_sleeping == 1) {
            /* Because the serial->inbuf's data are copied to the frame->data, it is time to signal
             * the thread_serial_device_read */
            // pthread_cond_signal(&newdataready_signal);
            /* [Bug] Note by LS: It should signal the thread waits for the bufferready_signal
             * instead of newdataready_signal */
    ...
    /*Make local copy of buffer pointer and data counter. They are shared between 2 threads, so we
     * want to update them only after a frame extraction */
    ...
    if (local_datacount >= length_needed) { /* enough data stored for 0710 frame header+footer? */
        if ((frame = (GSM0710_Frame*)malloc_r(sizeof(GSM0710_Frame))) != NULL) {
            /* Add by LS */
            /* It it does not set zero: when it encounters an error before allocating mem for
             * frame->data */
            ...
            frame->channel =
                    GSM0710_DECODE_ADDRESS(*local_readp); /*frame header address-byte read*/
            ...
            if (frame->channel >= (MAX_NON_GEMINI_CHNL_NUM - 2 + MUX_TEST_CHNL_OFFSET)) {
                /* Modify the received channel number from 28-30 (Modem MUX defined IT channels) to
                 * channel num values (used for channelist[]) 9-11 */
...
/* [Q] Why does it not parse the length field? In this way, it can throw the whole frame data by
 * moving ptrs */
/* [A] Because once it enters this function again, it will use a while-loop to find the 1st F9: In
 * this way, The previous frame's data can be regarded as thrown correctly */
#if 0
            ...
            LOGMUX(LOG_VERBOSE, "UIH Length Filed Byte#1=0x%02X,frame_length=%d", *local_readp,
                   frame->length);
            fcs = r_crctable[fcs ^ *local_readp];
            /* Note that: After check the length field value, length_needed and local_datacount are
             * not discounted by one */
            /*length_needed : 1 length byte + 1 fcs byte + 1 end frame flag */
        }

        /* Note by LS: Check if the maximum Length field occupies two bytes or not - According to
         * Spec: The Max Length field is 2-bytes */
        ...
        if ((*local_readp & 1) == 0) {
             ...
             */
            /* Only local_datacount needs to discount by one then buf->readp will be pointed to the
             * 1st byte of the Information field */
            /* Now, local_datacount = 1 length byte + payload + 1 fcs byte + 1 end frame flag ; it
             * should be equal to length_needed */
            gsm0710_buffer_inc(local_readp, local_datacount);
            /* 2nd byte: Its valus should shift left 7 bit and then add with 1st length field's
             * value => original value + 2nd byte-value x128 */
            frame->length += (*local_readp * 128); /*Frame header 2nd length-byte read*/
            LOGMUX(LOG_DEBUG, "UIH Length Filed Byte#2=0x%02X,frame_length=%d", *local_readp,
                   frame->length);
            fcs = r_crctable[fcs ^ *local_readp];
        }

        LOGMUX(LOG_VERBOSE, "Before: frame_length=%d", frame->length);
        length_needed += frame->length; /*length_needed : 1 length byte + payload + 1 fcs byte + 1
                                           end frame flag */
        LOGMUX(LOG_VERBOSE, "length_needed: %d, available in local_datacount: %d, frame_len=%d",
               length_needed, local_datacount, frame->length);

/* [Bug] Note by LS: It should check each channel's negotiated_N1 value instead of cmux_N1 */
#ifndef __MUX_UT__
        if (frame->length >
            channellist[frame->channel]
                    .negotiated_N1 /*|| frame->channel > vir_ports*/) /* Field Sanity check if
                                                                         payload is bigger than the
                                                                         max size negotiated in
                                                                         +CMUX */
#else                                                                 /* __MUX_UT__ */
        if ((frame->length > channellist[frame->channel].negotiated_N1 &&
             frame->channel <= MAX_NON_GEMINI_NON_DATA_CHNL_NUM) ||
            (frame->length > get_mux_ut_channel_n1() &&
             frame->channel > MAX_NON_GEMINI_NON_DATA_CHNL_NUM))
/* Case#1:Normal cmd & urc chnl: Frame size must be less than negotiated N1 ; Case#2: Test chnl:
 * Frame size must be less than setting value */
#endif                                                                /* __MUX_UT__ */
        {
#ifndef __MUX_UT__
            LOGMUX(LOG_WARNING,
                   "Dropping frame: Chnl_Num=%d,Corrupt! Length field indicated=%d. Max=%d allowed",
                   frame->channel, frame->length, channellist[frame->channel].negotiated_N1);
#else /* __MUX_UT__ */
            if (frame->channel <= MAX_NON_GEMINI_NON_DATA_CHNL_NUM)
                LOGMUX(LOG_WARNING,
                       "Dropping frame: Chnl_Num=%d,Corrupt! Length field indicated=%d. Max=%d "
                       "allowed",
                       frame->channel, frame->length, channellist[frame->channel].negotiated_N1);
            else
                LOGMUX(LOG_WARNING,
                       "Dropping frame: Chnl_Num=%d, Corrupt! Length field indicated=%d. MAX UT "
                       "N1=%d allowed",
                       frame->channel, frame->length, get_mux_ut_channel_n1());
...
#endif
            destroy_frame(frame);
            /* Note by LS: The following byte of FCS field may be F9 (It may only one F9 or two F9
             * F9) */
            /* Because it resets buf->flag_found as zero, it should parse this F9 at next time to
             * enter gsm0710_base_buffer_get_frame() */
            buf->flag_found = 0;
            buf->dropped_count++;
            goto update_buffer_dropping_frame; /* throw whole frame away, up until and incl.
                                                  local_readp */
        }

        if (!(local_datacount >= length_needed)) {
            destroy_frame(frame);
            /* Comment by LS: buf->readp will not be updated and still pointed to the Address field;
             * buf->flag_found is still remainded as 1 */
            /* At this time, there is no any frame data written to the pty because no a completed
             * frame can be obtained from the serial->in_buf */
            LOGMUX(LOG_DEBUG,
                   "Leave, frame extraction cancelled. Frame not completely stored in re-assembly "
                   "buffer yet");
            /* Becuase it does not have enough data, this thread will be notified again while data
             * is read from the modem side */
            return NULL;
        }
        ...
        if (frame->length > 0) {
            /* Now, local_readp is pointed to the 1st byte of the Information field and all data for
             * this completed frame are stored in buf */
            if ((frame->data = malloc_r(sizeof(char) * frame->length)) != NULL) {
                end = buf->endp - local_readp;
                if (frame->length > end) { /*wrap-around necessary*/
                    /* In this case: the buf->writep is not possible located between the buf->readp
                     * and buf->end */
        ...
        if (r_crctable[fcs ^ (*local_readp)] != 0xCF) {
            ...
            gsm0710_buffer_inc(local_readp, local_datacount);
            if (*local_readp != GSM0710_FRAME_FLAG) { /* the FCS didn't match, but the next byte may
                                                         not even be an end-frame-flag*/
                LOGMUX(LOG_WARNING,
                       "Dropping frame: Corrupt! End flag not present and FCS mismatch.");
                destroy_frame(frame);
                buf->flag_found = 0;
                buf->dropped_count++;
                goto update_buffer_dropping_frame; /* throw whole frame away, up until and incl.
            ...
            if (*local_readp != GSM0710_FRAME_FLAG) {
                LOGMUX(LOG_WARNING, "Dropping frame: End flag not present. Instead: %d",
                       *local_readp);
                destroy_frame(frame);
                buf->flag_found = 0;
                buf->dropped_count++;
            ...
            gsm0710_buffer_inc(local_readp,
                               local_datacount); /* prepare readp for next frame extraction */
        }
    } else {
    ...
    buf->datacount -=
            (local_datacount_backup - local_datacount); /* subtract whatever we analyzed */
    pthread_mutex_unlock(&buf->datacount_lock);
...
update_buffer_dropping_frame:
    ...
    return gsm0710_base_buffer_get_frame(buf,
                                         depth_level); /*continue extracting more frames if any*/
}
    ...
    while (gsm0710_buffer_length(buf) > 0) {
        if (!buf->adv_found_esc &&
            GSM0710_FRAME_ADV_FLAG ==
                    *(buf->readp)) { /* Whole frame parsed for escape chars, closing flag found */
            GSM0710_Frame* frame = NULL;
            ...
            if ((frame = (GSM0710_Frame*)calloc(1, sizeof(GSM0710_Frame))) !=
                NULL) {                                  /* frame is sane, allocate memory for it */
                ...
                frame->length = buf->adv_length - 3; /* the frame length field (total - address
                                                        field - type field - fcs field) */
            } else {
            ...
            if (frame->length > 0) {
                if ((frame->data = (unsigned char*)calloc(1, sizeof(char) * frame->length))) {
                    memcpy(frame->data, data + 2,
                           frame->length); /*copy data from first payload field*/
                    if (GSM0710_FRAME_IS(GSM0710_TYPE_UI, frame)) {
                        int i;
        ...
        if (buf->adv_length >=
            sizeof(buf->adv_data)) { /* frame data too much for buffer.. increase buffer size? */
        ...
        if (buf->adv_found_esc) { /* Treat found escape char (throw it away and complement 6th bit
                                     in next field */
            buf->adv_data[buf->adv_length] = *(buf->readp) ^ GSM0710_FRAME_ADV_ESC_COPML;
            buf->adv_length++;
            buf->adv_found_esc = 0;
        } else if (GSM0710_FRAME_ADV_ESC ==
                   *(buf->readp)) { /* no untreated escape char. if current field is escape char,
                                       note it down */
            buf->adv_found_esc = 1;
...
static int poll_modem_ready(int serial_device_fd, int to) {
    /* Note by LS: buf[] is used to store the +EIND: 128; it may not use so large buf-size (e.g.,
     * 1024) */
    unsigned char buf[1024];
    int sel;
    fd_set rfds;
        ...
        if (sel < 0) {
            ...
            if (FD_ISSET(serial_device_fd, &rfds)) {
                ...
                if (memstr((char*)buf_for_bootup->data, buf_for_bootup->datacount, "OK")) {
                    ...
                    pthread_mutex_unlock(&buf_for_bootup->datacount_lock);
                    return 0;
                } else if (memstr((char*)buf_for_bootup->data, buf_for_bootup->datacount,
                                  "ERROR")) {
...
static int handle_command(GSM0710_Frame* frame) {
    ...
    if (frame->length > 0) {
        ...
        /* Note by LS: If it assumes that Message_Type is 1 byte, the following codes should be
         * skipped */
        ...
        if ((type & GSM0710_CR) == GSM0710_CR) {
            ...
            while (frame->length > i) {
                /* length (declare as int: max 4 bytes): number of bytes of Length field =
                 * ((frame->data[i] & 11111110)>>1): LSB bit of frame->data[i] is E/A bit  */
            ...
            switch ((type & ~GSM0710_CR)) {
                ...
                // LOGMUX(LOG_DEBUG, "Frame->data = %s / frame->length = %d", frame->data + i,
                // frame->length - i); break;
                case GSM0710_CONTROL_TEST:
                // LOGMUX(LOG_DEBUG, "Test command: ");
                // LOGMUX(LOG_DEBUG, "Frame->data = %s / frame->length = %d", frame->data + i,
                // frame->length - i); serial->ping_number = 0; break;
...
#ifdef __MUXD_FLOWCONTROL__
                case GSM0710_CONTROL_MSC:
                    /* For MSC command, it has at least two bytea of data follow it (One byte is
                     * DLCI and the other byte is V.24 signal) */
                    if (i + 1 < frame->length) {
                        Channel* channel;
                        int fc_switched = 0;

                        /* DLCI only occupies the bit#2-#7; bit#0 is E/A and bit#1 is always set as
                         * 1 */
                        ...
                        if ((signals & GSM0710_SIGNAL_FC) == GSM0710_SIGNAL_FC) {
                            ...
                            // Choke Peer
                            if (fc_switched) _fc_chokePty(channel);
                        } else {
                            // op.arg |= USSP_CTS;
                            LOGMUX(LOG_DEBUG, "Frames allowed, channel id=%d,tx_fc_off=%d",
                                   channel->id, channel->tx_fc_off);
                            ...
                            if (channel->tx_fc_off == 1) {
                                ...
                                /* Add by LS: pseudo_device_read() thread may not have chance to
                                 * check this variable even FC is on again */
                                /* It should reset tx_fc_off as zero: In this way, when
                                 * pseudo_device_read() thread to check (and actually FC is on) */
                                /* pseudo_device_read() will read data from pty channel and write
                                 * into pty again */
                                /* without blocking to wait for cond signal's notification */
                                channel->tx_fc_off = 0;
                                /* This cond signal may be not affected: */
                                /* due to pseudo_device_read thread still has no chance to execute
                                 * the code "check tx_fc_off variable" */
                            ...
                            // Release Pty
                            if (fc_switched) _fc_releasePty(channel);
                        }
                    ...
                    if ((signals & GSM0710_SIGNAL_DV) == GSM0710_SIGNAL_DV)
                        ...
                        LOGMUX(LOG_ERR,
                               "Modem status command, but no info. i: %d, len: %d, data-len: %d", i,
                               length, frame->length);
                    }
                    break;
#endif /* __MUXD_FLOWCONTROL__ */
                ...
                default:
                    ...
                    if ((response = calloc(1, sizeof(char) * (2 + type_length))) != NULL) {
                        ...
                        while (type_length--) {
                            /* Value field: Copy the received Message type field of request (i.e.,
                             * from the byte of frame->data[0]) */
...
int extract_frames(GSM0710_Buffer* buf) {
    ...
    while ((frame = cmux_mode ? gsm0710_advanced_buffer_get_frame(buf)
                              : gsm0710_base_buffer_get_frame(buf, 0))) {
        /* cmux_mode=0: basic mode; frame = gsm0710_base_buffer_get_frame(buf) */
        /* If it can't obtain a completed frame(e.g.,f9,......,f9) in
         * gsm0710_base_buffer_get_frame(), NULL will be returned */
        frames_extracted++;
        /*Okay, go ahead and signal ser_read_thread to wake up if it is sleeping because reassembly
         * buffer was full before */
        pthread_mutex_lock(&buf->bufferready_lock);
        if (buf->input_sleeping == 1) {
            /* Because the serial->inbuf's data are copied to the frame->data, it is time to signal
             * the thread_serial_device_read */
        ...
        if ((GSM0710_FRAME_IS(GSM0710_TYPE_UI, frame) ||
             GSM0710_FRAME_IS(GSM0710_TYPE_UIH, frame))) {
            LOGMUX(LOG_VERBOSE, "Frame is UI or UIH");
            if (frame->control & GSM0710_PF) uih_pf_bit_received = 1;

            if (frame->channel > 0) {
                channel = &channellist[frame->channel];
                // if (!channel->opened) TODO

                /* frame->channel is the DLCI#i; in this case: it is the non-control channel (i.e.,
                 * DLCI#0) */
                if (frame->channel != MUXD_VT_CH_NUM) {
                ...
                if (frame != NULL) {
                    ...
                    while (1) {
                        if ((write_result = write(channel->fd, frame->data, frame->length)) >= 0) {
                            ...
                            /* Ref Linux Man Page: fsync() transfers ("flushes") all modified
                            ...
                            if (fsync_result < 0) {
                                LOGMUX(LOG_VERBOSE, "fsync() failed (%d, %d - %s)", fsync_result,
                            ...
                            if ((frame->length - write_result) > 0) {
                                ...
                                /* Start another thread to retry the action - writte data into the
                                 * channel*/
                            ...
                            switch (errno) {
                                ...
                                case EAGAIN:
                                    ...
                                    if (frame->channel != MUXD_VT_CH_NUM) {
                                        ...
                                        /* Start another thread to retry the action - writte data
                                         * into the channel*/
                                        start_retry_write_thread(channel);
                                    } else {
...
#endif
                                case EIO:
                                    // When channel fd has some problem and before reopen flag
                                    // raised, the flow will go into the assert flow. Since
                                    // "close_pty_channel" flow will raise the reopen flag soon, we
                                    // add a catch EIO here and make a delay. Next time, it will go
                                    // into the write reopen flow.
                                    LOGMUX(LOG_ERR, "Interrupt signal EIO caught");
                                    sleep(1);
                                    break;
                                default:
                                    ...
                                    if (channel->reopen) {
                                        LOGMUX(LOG_ERR, "channel%d needs to be reopened\n",
                                               channel->id);
                                        // MTK-START [mtk06800] retry after some intervals to
                                        // prevent too much logs
                                        sleep(1);
                                        // MTK-END [mtk06800] retry after some intervals to prevent
                                        // too much logs
                                        watchdog(&serial);
                                    } else {
                                        LOGMUX(LOG_ERR,
                                               "Unknown interrupt signal errno=%d caught from "
                                               "write()\n",
                                               backupErr);
                ...
                // control channel command (i.e., UIH Frame with the control command sent on
                // frame->channel#0)
            ...
            switch ((frame->control & ~GSM0710_PF)) {
                case GSM0710_TYPE_UA:
                    ...
                    /* [Bug] Note by LS: If Muxd does not send the DISC previously, it should
                     * ignores this unexpected UA instead of closing the pty */
                    if (channellist[frame->channel].opened) {
                        /* Note by LS: It seems not to send DISC from Gsm0710Muxd! */
                        /* pseudo_device_read() just reads data (i.e., UIH payload; but not control
                         * channel cmd) from master-pty and writes to the modem via serial */
                        int check_ua_iterator = 0;

                        pthread_mutex_lock(&ua_rsp_for_disc_lock);
                        /* After all disc are sent to the modem side, it is allowed to wait for the
                         * UA Rsp to check */
                        if (channellist[frame->channel].disc_ua_pending == 1) {
                            /* Note by LS: Check if the channel num is larger than zero */
                            if (frame->channel == 0) {
                                /* AP side does not send DLCI on chnl#0 to close down the MUX: It
                                 * should not receive UA for chnl#0 */
                                // assert(0);
                            ...
                            /* Note by LS: origin, fd ptsname, v24_signals and symbolic link are
                             * setup in c_alloc_channel => reset in logical_channel_close() */
                            SYSCHECK(logical_channel_close(channellist + frame->channel));
                            channellist[frame->channel].disc_ua_pending = 0;
                        } else {
                            LOGMUX(LOG_DEBUG, "Channel Opened:ignore this UA frame for channel=%d",
                                   frame->channel);
                        }

                        /* Note by LS:Only UA Rsp is triggered by close_devices(): It needs to do
                         * the following check */
                        if (close_down_send_disc_for_all_chnl == 1) {
                            /* Add by LS: Check if all UA Rsp for non-control channel's DISC are
                             * received or not */
                            for (check_ua_iterator = 1; check_ua_iterator < GSM0710_MAX_CHANNELS;
                                 check_ua_iterator++)
                                if (channellist[check_ua_iterator].disc_ua_pending == 1) break;

                            /* If the variable close_down_send_disc_for_all_chnl is not 1: It means
                             * that this UA for DISC is not triggered by close_devices() -> Not
                             * necessary to signal */
                            if (check_ua_iterator == GSM0710_MAX_CHANNELS)
                                /* It means that all UA Rsp for non-control channel's DISC are
                                 * received */
                        ...
                        if (channellist[frame->channel].sabm_ua_pending == 1) {
                            /* Note by LS: Should it check if this UA is triggered due to previous
                             * SABM first? */
                            ...
                            if (frame->channel == 0) {
                                ...
                                // static unsigned char version_test[] =
                                // "\x23\x21\x04TEMUXVERSION2\0"; write_frame(0, version_test,
                                // sizeof(version_test), GSM0710_TYPE_UIH);
                            } else {
...
// If all channels are established, set flag and send signal, as well
                            ...
                            LOGMUX(LOG_INFO, "UA to acknowledgde DISC on channel %d received",
                                   frame->channel);
                            channellist[frame->channel].disc_ua_pending = 0;
                        } else {
                            LOGMUX(LOG_INFO, "Channel Closed:ignore this UA frame for channel=%d",
                                   frame->channel);
                        }
                    }
                    break;
                case GSM0710_TYPE_DM:
                    if (channellist[frame->channel].opened) {
                        ...
                        if (frame->channel == 0) {
                            /* Note by LS: It seems that DM is triggered due to previous SABM on
                             * control channel */
                            LOGMUX(LOG_INFO, "Couldn't open control channel.\n->Terminating");
                            serial.state = MUX_STATE_CLOSING;
                            // close channels
                        } else {
                            LOGMUX(LOG_INFO, "Logical channel %d couldn't be opened",
                                   frame->channel);
                ...
                case GSM0710_TYPE_DISC:
                    if (channellist[frame->channel].opened) {
                        ...
                        rx_thread_write_frame(frame->channel, NULL, 0,
                                              GSM0710_TYPE_UA | GSM0710_PF);
                        if (frame->channel == 0) {
                            /* Note by LS */
                            /* 0710Spec: Section 5.3.4:DISC command sent at DLCI 0 have the same
        ...
        /* Memory allocation from frame and frame->data may be done in
         * gsm0710_base_buffer_get_frame() */
...
void* assemble_frame_thread(void* vargp) {
    ...
    while (buf != NULL) {
        pthread_mutex_lock(&buf->newdataready_lock);
        while (!(buf->datacount > 0) ||
               !buf->newdataready) { /* true if no data available in buffer or no new data written
                                        since last extract_frames() call */
            if (buf->input_sleeping !=
                1) { /* when is sleeping, thread_serial_device_read is wait */
                LOGMUX(LOG_VERBOSE,
                       "assemble_frame_thread put to sleep. GSM0710 buffer stored %d, "
                       "newdataready: %d",
                       buf->datacount, buf->newdataready);
                /* It should be signaled via pthread_cond_signal(&newdataready_signal); via
                 * thread_serial_device_read() -> gsm0710_buffer_write() */
                pthread_cond_wait(&buf->newdataready_signal,
                                  &buf->newdataready_lock); /* sleep until signalled by
                                                               thread_serial_device_read() */
                LOGMUX(LOG_VERBOSE,
                       "assemble_frame_thread awoken. GSM0710 buffer stored %d, newdataready: %d",
                       buf->datacount, buf->newdataready);
            } else {
        ...
        buf->newdataready =
                0; /*reset newdataready since buffer will be processed in extract_frames()*/
        pthread_mutex_unlock(&buf->newdataready_lock);
    ...
    for (i = 1; i < GSM0710_MAX_CHANNELS; i++) {
        if (channellist[i].fd >= 0) {
            if (channellist[i].opened)
                 ...
                 * It will think all DLCI are sent from the peer side: For any opened channel: Just
                 * close and reset its context
                 */
    ...
    switch (param) {
        ...
        case SIGTERM:
            ...
            if (g_set_force_assert_flag == 1 && g_set_alarm_flag == 1) {
                ...
                g_set_force_assert_flag = 0;
                LOGMUX(LOG_ERR,
                       "Recv SIGTERM before main process executes force assert and report");
                exit(0);
            } else
...
#ifdef __PRODUCTION_RELEASE__
        case SIGALRM:
            LOGMUX(LOG_ERR, "Recv SIGALRM: force_assert_flag=%d,alarm_flag=%d",
                   g_set_force_assert_flag, g_set_alarm_flag);
            if (g_set_force_assert_flag == 1) {
                g_set_force_assert_flag = 0;
                g_set_alarm_flag = 0;

                /* Re-try report */
                /* Plug-in to report abnormal case2 by Android property_set() to launch another
                 * process */
    ...
    LOGMUX(LOG_VERBOSE, "Enter");
    {
        ...
        switch (serial->state) {
            case MUX_STATE_MUXING: {
                /* Note by LS: It may not declare a local array with so large size; it can sync with
                 * CCCI TTY driver buf's size */
                ...
                if ((length = gsm0710_buffer_free(buf)) >
                    0) { /*available space in buffer (not locked since we want to utilize all
                            available space)*/
                    ...
                    /* Okay, internal buffer is full. we need to wait for the assembly thread to
                     * deliver a frame to the app(s). and free-up space */
                    pthread_mutex_lock(&buf->bufferready_lock);
                    LOGMUX(LOG_WARNING,
                           "Internal re-assembly buffer is full, waiting for flush to appl!");
                    buf->input_sleeping = 1; /* set sleeping flag - must be inside lock*/
                    while (!((gsm0710_buffer_free(buf)) > 0)) {
                        LOGMUX(LOG_VERBOSE,
                               "ser_read_thread put to sleep. GSM0710 buffer has %d bytes free",
                               gsm0710_buffer_free(buf));
                        /* Comment by LS: Once the data is removed from the serial->inbuf to
                         * frame->data in extract_frames(): it will invoke
                         * pthread_cond_wait(&bufferready_signal,&bufferready_lock) */
                        pthread_cond_wait(&buf->bufferready_signal,
                                          &buf->bufferready_lock); /* sleep until signalled by
                                                                      assembly thread() */
                        LOGMUX(LOG_VERBOSE, "ser_read_thread awoken");
                    }
                    buf->input_sleeping = 0; /* unset sleeping flag - must be inside lock*/
                    LOGMUX(LOG_VERBOSE,
                           "Internal re-assembly buffer partly flushed, free space: %d",
                           gsm0710_buffer_free(buf));
                    pthread_mutex_unlock(&buf->bufferready_lock);
                }
...
int open_serial_device(Serial* serial) {
    LOGMUX(LOG_VERBOSE, "Enter");
    unsigned int i;
    for (i = 0; i < GSM0710_MAX_CHANNELS; i++) SYSCHECK(logical_channel_init(channellist + i, i));
    ...
    if (!strcmp(serial->devicename, "/dev/socket/qemud")) {
        ...
        SYSCHECK(serial->fd = socket_local_client("qemud", ANDROID_SOCKET_NAMESPACE_RESERVED,
                                                  SOCK_STREAM));
        if (serial->fd >= 0) {
            char answer[2];

            if (write(serial->fd, "gsm", 3) != 3 || read(serial->fd, answer, 2) != 2 ||
        ...
        /* Comment by LS: The O_NOCTTY flag tells UNIX that this program doesn't want to be the
         * "controlling terminal" for that port */
        /* If you don't specify this then any input (such as keyboard abort signals and so forth)
        ...
        LOGMUX(LOG_DEBUG,
               "open_serial_device SYSCHECK(serial->fd = open(serial->devicename, O_RDWR | "
               "O_NOCTTY | O_NONBLOCK));");

        if (serial->fd < 0) {
            /* There is no such device! Try to handle it as socket */
            SYSCHECK(serial->fd = socket_local_client(
                             serial->devicename, ANDROID_SOCKET_NAMESPACE_FILESYSTEM, SOCK_STREAM));
            ...
            /* Note by LS: Why does it open the serial device with NON-BLOCK in open() then disable
             * this flag via fcntl() later? */
...
/* ALPS00574862 */
            ...
            uart_cfg_opt.c_iflag &=
                    ~(IXON | IXOFF | IXANY); /*clear flags for software flow control*/
    ...
    SYSCHECK(serial->fd = open(serial->devicename, O_RDWR | O_NOCTTY | O_NONBLOCK));
    LOGMUX(LOG_DEBUG,
           "open_serial_device 1 SYSCHECK(serial->fd = open(serial->devicename, O_RDWR | O_NOCTTY "
           "| O_NONBLOCK));");
...
int start_muxer(Serial* serial) {
    ...
    /* The timeout will affect the bootup time on Rachael35 because the first +EIND: 128 will always
     * lost. */
    int timeout = 10;
    if (isEngLoad) {
    ...
    if (cmux_mode) {
        ...
        /* Basic mode */
        SYSCHECK(snprintf(gsm_command, sizeof(gsm_command), "AT+CMUX=%d,%d,%d,%d", cmux_mode,
                          cmux_subset, cmux_port_speed, cmux_N1));
    }
...
/*
 * Purpose:  Close all devices, send mux termination frames (Close Down Procedure is triggered by AP
 * side) Input:    - Return:   0
 */
static int close_devices() {
    ...
    pthread_mutex_lock(&ua_rsp_for_disc_lock);
    /* close_down_send_disc_for_all_chnl should be wrapped by mutex and unlock this mutex after all
     * DISC comds are sent! */
    close_down_send_disc_for_all_chnl = 1;
    for (i = 1; i < GSM0710_MAX_CHANNELS; i++) {
        // terminate command given. Close all non-control channels one by one and finaly close the
        // control channel(i.e., channel num#0) the mux mode
        if (channellist[i].fd >= 0) {
            if (channellist[i].opened) {
                ...
                if (cmux_mode) {
                    ...
                    /* [Bug] Note by LS: It should use DISC frame to disconnect each non-control
                     * channel first */
#if 0
                    ...
                    /* In logical_channel_close(): it will set channel->opened = 0; In
                     * extract_frames(): After receiving the UA rsp frame of this DISC, it will
                     * invoke the logical_channel_close() in extract_frames() to handle this UA
                     */
                }
#if 0
    ...
    if (serial.fd >= 0) {
        /* Note by LS: It should wait for all UA for each non-contol channel are received then send
         * CLD on contorl channel */
        if (num_disc_sent > 0) {
            int rc = 0;
            struct timespec to;
             ...
             * But it must get the current time first then add ACK_T1_TIMEOUT seconds to obtain the
             * absolute time in the future
             */
            to.tv_sec = time(NULL) + ACK_T1_TIMEOUT;
            to.tv_nsec = 0;

            /* Because the peer side may not send UA Rsp for each DISC: it should set a timer to
             * guarantee to send the CLD command */
            /* Note by LS: 3rd parm of the pthread_cond_timedwait(): It will be the absolute time in
             * the future */
            /* It still wakes up from the blocking when the timer is expired or signal by the other
             * thread */

            rc = pthread_cond_timedwait(&ua_rsp_for_disc_signal, &ua_rsp_for_disc_lock, &to);
            close_down_send_disc_for_all_chnl = 0;
            if (rc == ETIMEDOUT) LOGMUX(LOG_DEBUG, "Fail to wait for all UA Rsp for DISC");
        }
        pthread_mutex_unlock(&ua_rsp_for_disc_lock);
        ...
        if (cmux_mode)
            ...
            /* Note by LS: Should this CLD cmd be sent after receiving all UA resposes for each DLC
    ...
    serial.state = MUX_STATE_OFF;
    /* Note by LS: If it stops the rild process by property_set("ctl.stop","ril-daemon"), it should
     * also set rild_started as 0 */
#if 0
    mtk_property_set("ctl.stop", "ril-daemon");
    rild_started = 0;
#endif
...
int watchdog(Serial* serial) {
    ...
    switch (serial->state) {
        ...
        case MUX_STATE_INITILIZING:
            ...
            /*If the returned value of start_muxer() is equal to zero: It means that serial->state
             * has been switched to MUX_STATE_MUXING */
            /* Create thread for assemble of frames from data in GSM0710 mux buffer */
            if (create_thread(&frame_assembly_thread, assemble_frame_thread,
                              (void*)serial->in_buf) != 0) {
                LOGMUX(LOG_ERR, "Could not create thread for frame-assmbly");
                pthread_mutex_unlock(&watch_lock);
                return GSM0710MUXD_CREATE_THREAD_ERR;
            }

            /* Create thread for polling on serial device (mux input) and writing to GSM0710 mux
             * buffer */
            Poll_Thread_Arg* poll_thread_arg = (Poll_Thread_Arg*)calloc(
                    1, sizeof(Poll_Thread_Arg));  // iniitialize pointer to thread args ;
            if (poll_thread_arg == NULL) {
                LOGMUX(LOG_ERR, "OOM");
                return GSM0710MUXD_OUT_OF_MEMORY;
            }
            ...
            if (create_thread(&ser_read_thread, poll_thread_serial, (void*)poll_thread_arg) !=
                0) {  // create thread for reading input from serial device
                LOGMUX(LOG_ERR, "Could not create thread for listening on %s", serial->devicename);
                pthread_mutex_unlock(&watch_lock);
                return GSM0710MUXD_CREATE_THREAD_ERR;
            }
            LOGMUX(LOG_DEBUG, "Thread is running and listening on %s",
                   serial->devicename);  // listening on serial port
             ...
             *     When AP Muxd finish setting flag for the MSC, try to write tx_msc_reaponse to
             * modem faile as the write_frame_lock is kept by step 2.
             * 4. Step 3 is executing in MD rx thread, it is waiting at write_frame_lock unlock, but
             * at the same time, MD tx thread is also waiting cmux buffer free with
             * write_frame_lock. Finally, it causes a dead lock beetween write frame and read frame.
             * We need to create a new thread to send tx_msc_response to avoid the dead lock.
             */
...
#ifdef __MUX_UT__
            /* Create thread for assemble of frames from data in GSM0710 mux buffer */
            if (create_thread(&ut_frame_assembly_thread, assemble_frame_thread,
                              (void*)serial->ut_in_buf) != 0) {
            ...
            /* Create thread for polling on serial device (mux input) and writing to GSM0710 mux
             * buffer */
            Poll_Thread_Arg* ut_poll_thread_arg = (Poll_Thread_Arg*)calloc(
                    1, sizeof(Poll_Thread_Arg));  // iniitialize pointer to thread args ;
            ...
            if (create_thread(&ut_ser_read_thread, ut_poll_thread_serial,
                              (void*)ut_poll_thread_arg) !=
                0) {  // create thread for reading input from serial device
                LOGMUX(LOG_ERR, "Could not create ut simulate thread for listening on %s",
                       "ut_simulated_serail_device");
                pthread_mutex_unlock(&watch_lock);
                return GSM0710MUXD_CREATE_THREAD_ERR;
            }
#endif /* __MUX_UT__ */
            ...
            if (rild_started == 0) {
                /* In this case, serial->state has been switched to MUX_STATE_MUXING when returning
                 * from start_muxer() successfully */
                rild_started++;
        ...
        case MUX_STATE_MUXING: {
            ...
            /* Comment by LS: If something wrong happens in pseudo_device_read()[e.g., read
             * failure]: pts_reopen will be set as 1 */
            pthread_mutex_lock(&pts_reopen_lock);
            pts_reopen = 0;
            i = 1;
            i < GSM0710_MAX_CHANNELS;
                ...
                for (i = 1; i < GSM0710_MAX_CHANNELS; i++) {
                    if (channellist[i].reopen == 1) {
                        if (GSM0710MUXD_SUCCESS != setup_pty_interface(&channellist[i])) {
                            ...
                            LOGMUX(LOG_INFO, "Restarted thread listening on %s",
                                   channellist[i].ptsname);
            ...
            if (use_ping) {
                if (serial->ping_number > use_ping) {
                    LOGMUX(LOG_DEBUG, "no ping reply for %d times, switch to CLOSING state",
                           serial->ping_number);
                    serial->state = MUX_STATE_CLOSING;
                    LOGMUX(LOG_DEBUG, "Switched Mux state to %d ", serial->state);
                } else {
        ...
        case MUX_STATE_CLOSING:
            /* In close_devices(): it will send the GSM0710_TYPE_DISC to each non-control DLC
             * channel + GSM0710_CONTROL_DISC to control channel; then serial->state will be set as
             * MUX_STATE_OFF */
            close_devices();
...
static int usage(char* _name) {
    ...
    fprintf(stderr,
            "\t-p <number>: use ping and reset modem after this number of unanswered pings [%d]\n",
            use_ping);
    ...
    fprintf(stderr,
            "\t-n <number of ports>: Number of virtual ports to create, must be in range 1-31 "
            "[%d]\n",
            vir_ports);
...
void set_main_exit_signal(int signal) {
    ...
    if (main_exit_signal == SIGTERM && signal == SIGUSR2) {
        ...
        /* To avoid the main proccess exit while-loop due to SIGTERM but executes force assert due
         * to invalid flag */
#ifdef __PRODUCTION_RELEASE__
        g_set_force_assert_flag = 0;
#endif
...
void* poll_thread(void* vargp) {
     ...
     */
    /* If the application does not write data anymore, these remaining data will not be sent due to
     * blocking in select() */
    while (1) {
        fd_set fdr, fdw;
        FD_ZERO(&fdr);
        ...
        if (select((poll_thread_arg->fd) + 1, &fdr, &fdw, NULL, NULL) > 0) {
            ...
            switch (errno) {
                ...
                case EBADF:
                    if (stop_muxd == 1) {
                        LOGMUX(LOG_ERR,
                               "Interrupt signal EBADF caught because devices is already shutdown "
                               "by self");
                        goto terminate;
                    } else {
                        LOGMUX(LOG_ERR, "Interrupt signal EBADF caught");
...
int main(int argc, char* argv[]) {
    ...
    /* For example: If the unmask value is 022(unmask is presented as ): the newly-created file's
     * permission is (~022)=664; directory's permission is (~022)=775 */
    umask(0);
    // signals treatment
    ...
    LOGMUX(LOG_NOTICE, "\t-s <serial port name>: Serial port device to connect to [%s]",
           serial.devicename);
    LOGMUX(LOG_NOTICE, "\t-t <timeout>: reset modem after this number of seconds of silence [%d]",
           use_timeout);
    ...
    LOGMUX(LOG_NOTICE, "\t-m <modem>: Mode (basic, advanced) [%s]",
           cmux_mode ? "advanced" : "basic");
    LOGMUX(LOG_NOTICE, "\t-f <framsize>: Frame size [%d]", cmux_N1);
    LOGMUX(LOG_NOTICE,
           "\t-n <number of ports>: Number of virtual ports to create, must be in range 1-31 [%d]",
           vir_ports);
    LOGMUX(LOG_NOTICE, "\t-o <output log to file>: Output log to /tmp/gsm0710muxd.log [%s]",
           logtofile ? "yes" : "no");
    ...
    while ((main_exit_signal == 0) && ((rc = watchdog(&serial)) == 0)) {
        ...
        LOGMUX(LOG_DEBUG, "Frames received/dropped: %lu/%lu", serial.in_buf->received_count,
    ...
    // finalize everything: serial.in_buf and serial_adv_frame_buf are allocated previously - It's
    // time to free
    /* Note by LS: If the main process exists the while-loop, it means that something is wrong */
    /* Two types of error causes returned from watchdog(): GSM0710MUXD_OPEN_SERIAL_DEV_ERR and
    ...
    if (main_exit_signal != 0) {
        ...
        /* It should return the error code from the watchdog exactly - Because some error is failed
         * to oper serial device, some is failed to create the thread */
    ...
    if (logtofile) fclose(muxlogfile);

#ifdef MUX_ANDROID
#ifdef __PRODUCTION_RELEASE__
    if (g_set_force_assert_flag == 1) {
        /* It means that this main_exit_signal triggered by Gsm0710Muxd_Assert() instead of recv a
         * signal actually */
        ...
        /* Set a 5 seconds alarm to check if the force assert for logging procedure is executed or
         * not */
        g_set_alarm_flag = 1;
        alarm(5);
