...
void ThroughputMonitor::run() {
    ...
    mRunning = 1;
    ALOGI("ThroughputMonitor is running, thread id = %d, threashold = %llu!", gettid(),
          lowpower_threshold);
    while (mRunning) {
        ...
        if (current >= last) {
            if ((current - last) > lowpower_threshold) {
                if ((PerfController::get_lowpower_perfhandle() == -1) &&
                    (PerfController::get_tether_perfhandle() == -1)) {
                    PerfController::enter_little_cpu();
#ifdef ACK_REDUCTION
                    PerfController::set_ack_reduction(ack_setting);
#endif

                    ALOGI("throughput reaches %llu bps and exceeds limitation",
                          (current - last) * 8L);
                }
            } else {
                if (PerfController::get_lowpower_perfhandle() != -1) {
                    ...
                    PerfController::restore_ack_reduction();
#endif
                    ALOGI("throughput reaches %llu bps and belows limitation",
                          (current - last) * 8L);
                }
            }
