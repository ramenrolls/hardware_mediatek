...
VectorImpl::VectorImpl(const VectorImpl& rhs)
    : mStorage(rhs.mStorage), mCount(rhs.mCount), mFlags(rhs.mFlags), mItemSize(rhs.mItemSize) {
    if (mStorage) {
...
ssize_t VectorImpl::insertAt(size_t index, size_t numItems) { return insertAt(0, index, numItems); }
...
status_t VectorImpl::sort(VectorImpl::compar_t cmp) { return sort(sortProxy, (void*)cmp); }
...
void VectorImpl::pop() {
    if (size()) removeItemsAt(size() - 1, 1);
}

void VectorImpl::push() { push(0); }

void VectorImpl::push(const void* item) { insertAt(item, size()); }

ssize_t VectorImpl::add() { return add(0); }

ssize_t VectorImpl::add(const void* item) { return insertAt(item, size()); }

ssize_t VectorImpl::replaceAt(size_t index) { return replaceAt(0, index); }

ssize_t VectorImpl::replaceAt(const void* prototype, size_t index) {
    LOG_ALWAYS_FATAL_IF(!(index < size()), "[%p] replace: index=%d, size=%d", this, (int)index,
                        (int)size());
...
ssize_t VectorImpl::removeItemsAt(size_t index, size_t count) {
    LOG_ALWAYS_FATAL_IF(!((index + count) <= size()), "[%p] remove: index=%d, count=%d, size=%d",
                        this, (int)index, (int)count, (int)size());

    if ((index + count) > size()) return BAD_VALUE;
    _shrink(index, count);
    return index;
}
...
void* VectorImpl::editItemLocation(size_t index) {
    LOG_ALWAYS_FATAL_IF(!(index < capacity()),
                        "[%p] editItemLocation: index=%d, capacity=%d, count=%d", this, (int)index,
                        (int)capacity(), (int)mCount);
...
const void* VectorImpl::itemLocation(size_t index) const {
    LOG_ALWAYS_FATAL_IF(!(index < capacity()), "[%p] itemLocation: index=%d, capacity=%d, count=%d",
...
void* VectorImpl::_grow(size_t where, size_t amount) {
    ...
    LOG_ALWAYS_FATAL_IF(!(where <= mCount), "[%p] _grow: where=%d, amount=%d, count=%d", this,
                        (int)where, (int)amount, (int)mCount);  // caller already checked
...
void VectorImpl::_shrink(size_t where, size_t amount) {
    ...
    if (new_size < (capacity() / 2)) {
        ...
        if ((where == new_size) && (mFlags & HAS_TRIVIAL_COPY) && (mFlags & HAS_TRIVIAL_DTOR)) {
            const SharedBuffer* cur_sb = SharedBuffer::bufferFromData(mStorage);
...
size_t VectorImpl::itemSize() const { return mItemSize; }
...
SortedVectorImpl::SortedVectorImpl(size_t itemSize, uint32_t flags) : VectorImpl(itemSize, flags) {}
