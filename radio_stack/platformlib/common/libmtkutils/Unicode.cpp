...
static const char32_t kFirstByteMark[] = {0x00000000, 0x00000000, 0x000000C0, 0x000000E0,
                                          0x000000F0};
...
static inline int32_t utf32_at_internal(const char* cur, size_t* num_read) {
    ...
    for (num_to_read = 1, mask = 0x40, to_ignore_mask = 0xFFFFFF80; (first_char & mask);
...
size_t utf8_to_utf32_length(const char* src, size_t src_len) {
    ...
    const char* end;
    size_t num_to_skip;
    for (cur = src, end = src + src_len, num_to_skip = 1; cur < end; cur += num_to_skip, ret++) {
        const char first_char = *cur;
        num_to_skip = 1;
        if ((first_char & 0x80) == 0) {  // ASCII
            continue;
