...
String16::String16(StaticLinkage) : mString(0) {
    // this constructor is used when we can't rely on the static-initializers
    // having run. In this case we always allocate an empty string. It's less
    // efficient than using getEmptyString(), but we assume it's uncommon.

    char16_t* data = static_cast<char16_t*>(SharedBuffer::alloc(sizeof(char16_t))->data());
    data[0] = 0;
    mString = data;
}
...
String16::~String16() { SharedBuffer::bufferFromData(mString)->release(); }

size_t String16::size() const { return SharedBuffer::sizeFromData(mString) / sizeof(char16_t) - 1; }
...
status_t String16::setTo(const char16_t* other, size_t len) {
    SharedBuffer* buf =
            SharedBuffer::bufferFromData(mString)->editResize((len + 1) * sizeof(char16_t));
    if (buf) {
        char16_t* str = (char16_t*)buf->data();
...
status_t String16::insert(size_t pos, const char16_t* chrs, size_t len) {
    ...
    SharedBuffer* buf =
            SharedBuffer::bufferFromData(mString)->editResize((myLen + len + 1) * sizeof(char16_t));
    if (buf) {
        char16_t* str = (char16_t*)buf->data();
        if (pos < myLen) {
...
bool String16::contains(const char16_t* chrs) const { return strstr16(mString, chrs) != nullptr; }
...
status_t String16::remove(size_t len, size_t begin) {
    ...
    if (begin > 0) {
        SharedBuffer* buf =
                SharedBuffer::bufferFromData(mString)->editResize((N + 1) * sizeof(char16_t));
        if (!buf) {
            return NO_MEMORY;
        }
        char16_t* str = (char16_t*)buf->data();
