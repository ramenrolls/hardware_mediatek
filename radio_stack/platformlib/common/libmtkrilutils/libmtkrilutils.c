...
static const char* sAtCmdWithoutContent[] = {
        ...
        "AT+EC2KCNMA", "AT+CGLA", "+CGLA", "+ESLOTSINFO", "AT+ECLCK=",
        // External SIM [Start]
        "+ERSAIND", "AT+ERSA", "+ERSAAUTH", "AT+ERSIMATO",
        // External SIM [End]
        "+CPBR", "AT+EPBW", "AT+EPBUM", "+EPBUM",
        "AT+CPIN=",  // Verify PIN
        "AT+CPIN2=", "AT+EPIN1=", "AT+EPIN2=", "AT+CPUK=", "AT+CPUK2=", "AT+ESLBLOB=3,",
        "AT+CPWD=", "AT+CLCK=",
        "AT+EAUTH",     // SIM AUTH
        "AT+ESIMAUTH",  // SIM AUTH
        "+CRSM",        // Read EFiccid
        "+ESIMIND: 3",  // SIM cache file
        "+CNUM", "+STKPCI: 1", "+STKCTRL:", "AT+STKTR=\"81030123", "AT+EGMR",
        /// M: CC: sensitive info hidden settings -start
        "ATDE", "ATD", "+CCWA", "+CDIP", "+CLIP", "+COLP", "+CNAP", "+ECPI", "+EAIC", "+EICPGU",
        "+CSSU", "+CLCC", "ATDER",
        /// M: CC: sensitive info hidden settings -end
        /// M: Ims CC -start
        "AT+EDCONF", "AT+ECONF", "+EDCONF", "+CDU", "+ECONF", "+ECALLPULL", "+ECT", "+EIMSEVTPKG",
        /// M: Ims CC -end
        /// M: SS sensitive info hidden settings -start
        "AT+ECUSD",  // SS
        "+CCFC",     // CF
        "+ECFU",     // CFU from MD
        "+EIMSXUI", "AT+EGBA", "+EGBA", "AT+EIUSD", "+ECMCCSS",
        /// M: SS -end
        "+REDIRNUM", "+CEXTD", "+CFNM", "AT+ECRLA", "+ECRLA", "AT+EAPNSET", "AT+EIAAPN", "AT+CLCC",
        "AT+VTS", "AT+EVTS", "AT+CFSH",
        ...
        "+CORG",  // Unused URC, but contains sensitive data, so need to filter
                  // MWIS [Start]
        "AT+EIMSGEO", "+EIMSGEO", "AT+EWIFIADDR", "AT+EWIFIASC", "AT+EWIFINATT", "+EWOKEEPALIVE",
        // MWIS [End]
        "+EDMFAPP", "+EIMSCMODE",
        // Data - Start
        "AT+EPDN", "+EPDN", "+CGCONTRDP:", "+CGDCONT:", "+CGPADDR:", "+CGPRCO:", "+EGCONTRDP:",
        // Data - End
        // NW [Start]
...
static const char* s_atcmd_log_reduction[] = {
        "AT+ESRVSTATE", "+ECBDINFO", "AT+ECBDINFO", "AT+EDEFROAM",
        "+EDEFROAM",    "AT+EIPRL",  "+EIPRL",
};
...
int getSimCount() {
    ...
    if (strcmp(featurevalue.value, "ss") == 0) {
        simCount = 1;
    } else if (strcmp(featurevalue.value, "dsds") == 0 || strcmp(featurevalue.value, "dsda") == 0) {
        simCount = 2;
    ...
    if (strcmp(property_value, "ss") == 0) {
        simCount = 1;
    } else if (strcmp(property_value, "dsds") == 0 || strcmp(property_value, "dsda") == 0) {
        simCount = 2;
...
int isEmulatorRunning() {
    ...
    if (strcmp(qemu, "1") == 0) isEmulatorRunning = 1;
...
#define MIN(a, b) ((a) < (b) ? (a) : (b))
    ...
    for (i = 0; i < MAX_PHONE_NUM; i++) {
        if (i == phoneId) {
            // use new value
            strncat(newPropertyValue, pUpdateValue,
                    MTK_PROPERTY_VALUE_MAX - strlen(newPropertyValue) - 1);
        } else {
            getMSimProperty(i, pPropertyName, oldItemValue);
            strncat(newPropertyValue, oldItemValue,
                    MTK_PROPERTY_VALUE_MAX - strlen(newPropertyValue) - 1);
        }
...
const char* mtkRequestToString(int request) {
    switch (request) {
        ...
        case RIL_LOCAL_REQUEST_SET_SVLTE_RAT_MODE:
            return "LOCAL_REQUEST_SET_SVLTE_RAT_MODE";
        /// M: [C2K 6M][NW] Set the SVLTE RAT mode@}
        /// M: [C2K 6M][NW]  @{
        ...
        case RIL_LOCAL_REQUEST_ABORT_AVAILABLE_NETWORK:
            return "LOCAL_ABORT_AVAILABLE_NETWORKS";
        case RIL_REQUEST_SIGNAL_STRENGTH_WITH_WCDMA_ECIO:
            return "GET_SIGNAL_STRENGTH_WITH_WCDMA_ECIO";
        case RIL_UNSOL_SIGNAL_STRENGTH_WITH_WCDMA_ECIO:
            return "UNSOL_SIGNAL_STRENGTH_WITH_WCDMA_ECIO";
        ...
        case RIL_LOCAL_GSM_REQUEST_SET_ACTIVE_PS_SLOT:
            return "RIL_LOCAL_GSM_REQUEST_SET_ACTIVE_PS_SLOT";
        case RIL_LOCAL_GSM_REQUEST_CONFIRM_INTER_3GPP_IRAT_CHANGE:
            return "RIL_LOCAL_GSM_REQUEST_CONFIRM_INTER_3GPP_IRAT_CHANGE";
        ...
        case RIL_LOCAL_REQUEST_CDMA_SMS_SPECIFIC_TO_GSM:
            return "CDMA_SMS_SPECIFIC_TO_GSM";
        case RIL_LOCAL_GSM_UNSOL_CDMA_SMS_SPECIFIC_FROM_GSM:
            return "CDMA_SMS_SPECIFIC_FROM_GSM";
        ...
        case RIL_UNSOL_VIRTUAL_SIM_STATUS_CHANGED:
            return "RIL_UNSOL_VIRTUAL_SIM_STATUS_CHANGED";
        ...
        case RIL_LOCAL_C2K_REQUEST_CONFIRM_INTER_3GPP_IRAT_CHANGE:
            return "RIL_LOCAL_C2K_REQUEST_CONFIRM_INTER_3GPP_IRAT_CHANGE";
        ...
        case RIL_LOCAL_REQUEST_EMBMS_AT_CMD:
            return "RIL_LOCAL_REQUEST_EMBMS_AT_CMD";
        case RIL_LOCAL_REQUEST_EMBMS_ENABLE:
            return "RIL_LOCAL_REQUEST_EMBMS_ENABLE";
        case RIL_LOCAL_REQUEST_EMBMS_DISABLE:
            return "RIL_LOCAL_REQUEST_EMBMS_DISABLE";
        case RIL_LOCAL_REQUEST_EMBMS_START_SESSION:
            return "RIL_LOCAL_REQUEST_EMBMS_START_SESSION";
        case RIL_LOCAL_REQUEST_EMBMS_STOP_SESSION:
            return "RIL_LOCAL_REQUEST_EMBMS_STOP_SESSION";
        case RIL_LOCAL_REQUEST_EMBMS_GET_TIME:
            return "RIL_LOCAL_REQUEST_EMBMS_GET_TIME";
        ...
        case RIL_LOCAL_REQUEST_EMBMS_SET_E911:
            return "RIL_LOCAL_REQUEST_EMBMS_SET_E911";
        case RIL_LOCAL_REQUEST_EMBMS_TRIGGER_CELL_INFO_NOTIFY:
            return "RIL_LOCAL_REQUEST_EMBMS_TRIGGER_CELL_INFO_NOTIFY";
        case RIL_LOCAL_REQUEST_SETUP_DATA_CALL_ALT:
            return "RIL_LOCAL_REQUEST_SETUP_DATA_CALL_ALT";
        case RIL_LOCAL_REQUEST_LAST_DATA_CALL_FAIL_CAUSE_ALT:
            return "RIL_LOCAL_REQUEST_LAST_DATA_CALL_FAIL_CAUSE_ALT";
        case RIL_LOCAL_REQUEST_PCSCF_DISCOVERY_PCO_ALT:
            return "RIL_LOCAL_REQUEST_PCSCF_DISCOVERY_PCO_ALT";
        ...
        case RIL_LOCAL_GSM_UNSOL_EMBMS_CELL_INFO_NOTIFICATION:
            return "RIL_LOCAL_GSM_UNSOL_EMBMS_CELL_INFO_NOTIFICATION";
        case RIL_LOCAL_GSM_UNSOL_EMBMS_COVERAGE_STATE:
            return "RIL_LOCAL_GSM_UNSOL_EMBMS_COVERAGE_STATE";
        case RIL_LOCAL_GSM_UNSOL_EMBMS_ACTIVE_SESSION:
            return "RIL_LOCAL_GSM_UNSOL_EMBMS_ACTIVE_SESSION";
        case RIL_LOCAL_GSM_UNSOL_EMBMS_AVAILABLE_SESSION:
            return "RIL_LOCAL_GSM_UNSOL_EMBMS_AVAILABLE_SESSION";
        case RIL_LOCAL_GSM_UNSOL_EMBMS_SAI_LIST_NOTIFICATION:
            return "RIL_LOCAL_GSM_UNSOL_EMBMS_SAI_LIST_NOTIFICATION";
        case RIL_LOCAL_GSM_UNSOL_EMBMS_OOS_NOTIFICATION:
            return "RIL_LOCAL_GSM_UNSOL_EMBMS_OOS_NOTIFICATION";
        /// M: eMBMS end
        ...
        case RIL_LOCAL_REQUEST_EMERGENCY_REDIAL:
            return "RIL_LOCAL_REQUEST_EMERGENCY_REDIAL";
        case RIL_LOCAL_REQUEST_EMERGENCY_SESSION_BEGIN:
            return "RIL_LOCAL_REQUEST_EMERGENCY_SESSION_BEGIN";
        case RIL_LOCAL_REQUEST_EMERGENCY_SESSION_END:
            return "RIL_LOCAL_REQUEST_EMERGENCY_SESSION_END";
        // Verizon hVoLTE/E911 interface
        case RIL_LOCAL_REQUEST_SET_HVOLTE_MODE:
            return "RIL_LOCAL_REQUEST_SET_HVOLTE_MODE";
        ...
        case RIL_UNSOL_RESET_ATTACH_APN:
            return "RIL_UNSOL_RESET_ATTACH_APN";
        ...
        // SIM ATT RSU
        case RIL_UNSOL_ATT_SIM_LOCK_NOTIFICATION:
            return "RIL_UNSOL_ATT_SIM_LOCK_NOTIFICATION";
        // MTK-END
        // M: MPS feature
        case RIL_LOCAL_REQUEST_DEACTIVATE_ALL_DATA_CALL:
            return "RIL_LOCAL_REQUEST_DEACTIVATE_ALL_DATA_CALL";
        case RIL_LOCAL_C2K_REQUEST_DEACTIVATE_ALL_DATA_CALL:
            return "RIL_LOCAL_C2K_REQUEST_DEACTIVATE_ALL_DATA_CALL";
        ...
        case RIL_UNSOL_CDMA_CARD_INITIAL_ESN_OR_MEID:
            return "CDMA_CARD_INITIAL_ESN_OR_MEID";
        case RIL_LOCAL_REQUEST_AT_COMMAND_WITH_PROXY:
            return "RIL_LOCAL_REQUEST_AT_COMMAND_WITH_PROXY";
        // / M: BIP {
        ...
        case RIL_LOCAL_REQUEST_MAL_PS_RGEGISTRATION_STATE:
            return "RIL_LOCAL_REQUEST_MAL_PS_RGEGISTRATION_STATE";
        /// @}
        // / M: OTASP {
        ...
        case RIL_LOCAL_REQUEST_HANDOVER_IND:
            return "RIL_LOCAL_REQUEST_HANDOVER_IND";
        case RIL_LOCAL_REQUEST_WIFI_DISCONNECT_IND:
            return "RIL_LOCAL_REQUEST_WIFI_DISCONNECT_IND";
        case RIL_LOCAL_REQUEST_PCSCF_DISCOVERY_PCO:
            return "RIL_REQUEST_PCSCF_DISCOVERY_PCO";
        case RIL_LOCAL_REQUEST_WIFI_CONNECT_IND:
            return "RIL_REQUEST_WIFI_CONNECT_IND";
        case RIL_LOCAL_GSM_UNSOL_DEDICATE_BEARER_ACTIVATED:
            return "RIL_LOCAL_GSM_UNSOL_DEDICATE_BEARER_ACTIVATED";
        case RIL_LOCAL_GSM_UNSOL_DEDICATE_BEARER_MODIFIED:
            return "RIL_LOCAL_GSM_UNSOL_DEDICATE_BEARER_MODIFIED";
        case RIL_LOCAL_GSM_UNSOL_DEDICATE_BEARER_DEACTIVATED:
            return "RIL_LOCAL_GSM_UNSOL_DEDICATE_BEARER_DEACTIVATED";
        case RIL_LOCAL_GSM_UNSOL_MAL_DATA_CALL_LIST_CHANGED:
        ...
        case RIL_LOCAL_C2K_REQUEST_SWITCH_HPF:
            return "RIL_LOCAL_C2K_REQUEST_SWITCH_HPF";
        /// M: CC: Switch antenna
        case RIL_LOCAL_REQUEST_SWITCH_ANTENNA:
            return "RIL_LOCAL_REQUEST_SWITCH_ANTENNA";
        /// M: CC: Normal/Emergency call redial @{
        case RIL_LOCAL_C2K_REQUEST_REDIAL:
            return "RIL_LOCAL_C2K_REQUEST_REDIAL";
        case RIL_LOCAL_C2K_REQUEST_EMERGENCY_REDIAL:
            return "RIL_LOCAL_C2K_REQUEST_EMERGENCY_REDIAL";
        /// @}
        /// M: [Network] Femtocell URC log print. @{
        case RIL_UNSOL_FEMTOCELL_INFO:
            return "UNSOL_FEMTOCELL_INFO";
        /// @}
        /// M: [Network][C2K] Sprint roaming control @{
        case RIL_REQUEST_SET_ROAMING_ENABLE:
            return "SET_ROAMING_ENABLE";
        case RIL_REQUEST_GET_ROAMING_ENABLE:
            return "GET_ROAMING_ENABLE";
        /// @}
        // M: AGPSD command
        case RIL_LOCAL_C2K_REQUEST_AGPS_TCP_CONNIND:
            return "RIL_REQUEST_AGPS_TCP_CONNIND";
        case RIL_LOCAL_C2K_UNSOL_VIA_GPS_EVENT:
            return "RIL_UNSOL_VIA_GPS_EVENT";
        // External SIM [START]
        ...
        case RIL_LOCAL_REQUEST_SET_MODEM_THERMAL:
            return "RIL_LOCAL_REQUEST_SET_MODEM_THERMAL";
        case RIL_REQUEST_CONFIG_MODEM_STATUS:
            return "RIL_REQUEST_CONFIG_MODEM_STATUS";
        ...
        case RIL_LOCAL_REQUEST_MODEM_POWEROFF_SYNC_C2K:
            return "RIL_LOCAL_REQUEST_MODEM_POWEROFF_SYNC_C2K";
        case RIL_LOCAL_GSM_UNSOL_ENHANCED_MODEM_POWER:
            return "RIL_LOCAL_GSM_UNSOL_ENHANCED_MODEM_POWER";
        ...
        case RIL_LOCAL_REQUEST_RESUME_WORLD_MODE:
            return "RIL_LOCAL_REQUEST_RESUME_WORLD_MODE";
        ...
        case RIL_LOCAL_REQUEST_SIM_GET_EFDIR:
            return "RIL_LOCAL_REQUEST_SIM_GET_EFDIR";
        case RIL_LOCAL_GSM_UNSOL_ESIMIND_APPLIST:
            return "RIL_LOCAL_GSM_UNSOL_ESIMIND_APPLIST";
        ...
        case RIL_LOCAL_REQUEST_RESET_SUSPEND:
            return "RIL_LOCAL_REQUEST_RESET_SUSPEND";
        /// @}
        ...
        case RIL_LOCAL_C2K_REQUEST_SET_UTK_MODE:
            return "RIL_LOCAL_C2K_REQUEST_SET_UTK_MODE";
        default:
...
int isEpdgSupport() { return isWfcSupport(); }
...
int isOp17MccMnc(char* mccmnc) { return (strcmp(mccmnc, "44010") == 0); }

int isOp129MccMnc(char* mccmnc) {
    return (strcmp(mccmnc, "44051") == 0 || strcmp(mccmnc, "44054") == 0);
}
...
int getClassType(char* apn, int supportedApnTypesBitmask) {
    ...
    if ((supportedApnTypesBitmask & RIL_APN_TYPE_EMERGENCY) ||
        strcasecmp(VZW_EMERGENCY_NI, apn) == 0) {
...
int isExternalSimOnlySlot(RIL_SOCKET_ID rid) {
    ...
    mtkLogI(LOG_TAG, "[isExternalSimOnlySlot] vsimOnlySlots:%d, supported:%d", atoi(property_value),
            supported);
