...
static int Ipv4GetMacByIoctl(const char* ipStr, const char* ifname,
    ...
    if (!ioctl(sock_fd, SIOCGARP, &arp_query)) {
        if (arp_query.arp_flags & ATF_COM) {  // NUD_STATE == NUD_VALID
            ...
            ALOGI("ioctl success, ipStr = %s, MAC address = %s", ipStr,
                  SocketAddr_ntop(WlanSocketAddr, buf));
            ret = 0;
        } else {
        ...
        ret = -errno;
        if (ret == -ENXIO) {
            ALOGI("ioctl fail, no neighbour, trigger arp, ifname = %s, err = %s (%d)", ifname,
                  strerror(errno), errno);
        } else {
            ALOGE("ioctl exception, ipStr = %s, ifname = %s, err = %s (%d)", ipStr, ifname,
                  strerror(errno), errno);
        }
        goto ioctl_out;
    }
...
static void WlanMacAddr_ntop(const struct wlan0_hw_addr* WlanMacAddr, char* buf) {
    sprintf(buf, "%02X:%02X:%02X:%02X:%02X:%02X", WlanMacAddr->wlan0_mac_addr[0],
            WlanMacAddr->wlan0_mac_addr[1], WlanMacAddr->wlan0_mac_addr[2],
            WlanMacAddr->wlan0_mac_addr[3], WlanMacAddr->wlan0_mac_addr[4],
            WlanMacAddr->wlan0_mac_addr[5]);
}
...
static int SendNewNeighborMessage(const char* ipStr, int ifIndex, bool is_ipv6,
    ...
    if (!is_ipv6)
        addattr_l(&NeighRequest.hdr, sizeof(NeighRequest), NDA_DST, &ip_addr,
                  sizeof(struct in_addr));
    else
        addattr_l(&NeighRequest.hdr, sizeof(NeighRequest), NDA_DST, &ip6_addr,
                  sizeof(struct in6_addr));

    // create the MAC attribute
    addattr_l(&NeighRequest.hdr, sizeof(NeighRequest), NDA_LLADDR, WlanSocketAddr->sa_data,
              MACSIZE);
...
int addattr_l(struct nlmsghdr* n, int maxlen, int type, const void* data, int alen) {
    ...
    if (NLMSG_ALIGN(n->nlmsg_len) + RTA_ALIGN(len) > maxlen) {
        ALOGD("addattr_l ERROR: message exceeded bound of %d, nlmsg_len:%d, len:%d\n", maxlen,
              NLMSG_ALIGN(n->nlmsg_len), RTA_ALIGN(len));
        return -1;
    }
    rta = NLMSG_TAIL(n);
    rta->rta_type = type;
...
static int read_neighbor_nlmsg(int fd, const char* ipStr, struct sockaddr* WlanSocketAddr) {
    ...
    for (nh = (struct nlmsghdr*)buff; NLMSG_OK(nh, read_size); nh = NLMSG_NEXT(nh, read_size)) {
...
static int neigh_mac_process(struct nlmsghdr* nh, const char* ipStr,
    ...
    if (ndmsg->ndm_family & (AF_INET | AF_INET6)) {
        ...
        if (tb[NDA_DST]) {
            inet_ntop(ndmsg->ndm_family, RTA_DATA(tb[NDA_DST]), listen_ip, sizeof(listen_ip));
            ...
            if (!strncmp(ipStr, listen_ip, strlen(ipStr))) {
                if (ndmsg->ndm_state & NUD_VALID) {
                    if (tb[NDA_LLADDR]) {
                        memcpy((char*)WlanSocketAddr->sa_data, (char*)RTA_DATA(tb[NDA_LLADDR]),
                               MACSIZE);
                        WlanSocketAddr->sa_family = ndmsg->ndm_family;
                        ALOGI("listen ip: %s , MAC addr: %s , sa_family = %d, ndmsg->ndm_state = "
                              "%d",
                    ...
                    ALOGE("3 times no arp/na, ndmsg->ndm_state = %d", ndmsg->ndm_state, ipStr,
                          listen_ip);
...
static const inline nsecs_t seconds_to_nanoseconds(nsecs_t secs) { return secs * 1000000000; }

static const inline nsecs_t nanoseconds_to_milliseconds(nsecs_t secs) { return secs / 1000000; }
...
int Get_Mac_Addr(const char* ipStr, const char* ifname, struct sockaddr* WlanSocketAddr) {
    ...
    if (!ret) {
        ALOGI("[Get_Mac_Addr] get success, ip = %s, MAC = %s", ipStr,
              SocketAddr_ntop(WlanSocketAddr, buf));
        goto out;
        ...
        if ((ret = SendNewNeighborMessage(ipStr, wlan_index, is_ipv6, WlanSocketAddr)) < 0) {
            ALOGE("failed to sendNewNeighborMessage, err = %s (%d)", strerror(errno), errno);
            goto clean_up;
        }
