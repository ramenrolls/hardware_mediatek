...
typedef enum { NETAGENT_IO_MTU = 0, NETAGENT_IO_MTU_MAX = 0xFFFFFFFF } netagent_io_mtu_e;
        ...
        if (netagent_io_get_cmd_type(netagent_io_cmd_obj, &cmd) == NETAGENT_IO_RET_SUCCESS) {
            switch (cmd) {
                case NETAGENT_IO_CMD_IFUP :
                    if (netagent_io_get_cmd_type(netagent_io_cmd_obj, &type) ==
NETAGENT_IO_RET_SUCCESS) { switch (type) { case NETAGENT_IO_ADDR_TYPE_IPv4 : if
(netagent_io_get_addr_v4(netagent_io_cmd_obj, &addr_v4) == NETAGENT_IO_RET_SUCCESS) { Dbg("get v4
address success"); } else { Err("get v4 address fail");
                                }
                                break;
                            case NETAGENT_IO_ADDR_TYPE_IPv6 :
                                if (netagent_io_get_addr_v6(netagent_io_cmd_obj, &addr_v6) ==
NETAGENT_IO_RET_SUCCESS) { Dbg("get v6 address success"); } else { Err("get v6 address fail");
                                }
                                break;
                            case NETAGENT_IO_ADDR_TYPE_IPv4v6;
                                if (netagent_io_get_addr_v4(netagent_io_cmd_obj, &addr_v4) ==
NETAGENT_IO_RET_SUCCESS) { Dbg("get v4 address success"); } else { Err("get v4 address fail");
                                }
                                if (netagent_io_get_addr_v6(netagent_io_cmd_obj, &addr_v6) ==
NETAGENT_IO_RET_SUCCESS) { Dbg("get v6 address success"); } else { Err("get v6 address fail");
                                }
...
Example 4 : send ifst (up with ipv4 address);
    ...
    netagent_io_cmd_obj = netagent_io_cmd_ifst_alloc(interface_id, NETAGENT_IO_IFST_UP,
