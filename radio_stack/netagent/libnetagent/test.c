...
void send_if_chg(void* netagent_io_obj, unsigned int interface_id, unsigned int addr_type,
                 unsigned char* addr) {
    void* netagent_io_cmd_obj = 0;
    netagent_io_cmd_obj =
            netagent_io_cmd_ipupdate_alloc(interface_id, addr_type, (unsigned int*)addr, -1);
    if (netagent_io_cmd_obj == NULL) {
        return;
    }
    netagent_io_send(netagent_io_obj, netagent_io_cmd_obj);
...
static void* task_stub(void* arg) {
            ...
            if (netagent_io_get_cmd_type(netagent_io_cmd_obj, &cmd) == NETAGENT_IO_RET_SUCCESS) {
                switch (cmd) {
                    case NETAGENT_IO_CMD_IFUP: {
                        if (netagent_io_get_addr_type(netagent_io_cmd_obj, &type) ==
                            ...
                            switch (type) {
                                case NETAGENT_IO_ADDR_TYPE_IPv4:
                                    if (netagent_io_get_addr_v4(netagent_io_cmd_obj, &addr_v4) ==
                                        NETAGENT_IO_RET_SUCCESS) {
                                        unsigned char* ptr = (unsigned char*)&addr_v4;
                                        Dbg("### get ifup event with v4 address = %d.%d.%d.%d, if "
                                            "= %d, reason = %s",
                                            *ptr, *(ptr + 1), *(ptr + 2), *(ptr + 3), if_id,
                                            reason);
                                    } else {
                                ...
                                case NETAGENT_IO_ADDR_TYPE_IPv6:
                                    if (netagent_io_get_addr_v6(netagent_io_cmd_obj, addr_v6) ==
                                        NETAGENT_IO_RET_SUCCESS) {
                                        unsigned char* ptr = (unsigned char*)&addr_v6;
                                        Dbg("### get ifup event with v6 address "
                                            "%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%"
                                            "02X:%02X:%02X:%02X:%02X:%02X, if = %d, reason = %s",
                                            *ptr, *(ptr + 1), *(ptr + 2), *(ptr + 3), *(ptr + 4),
                                            *(ptr + 5), *(ptr + 6), *(ptr + 7), *(ptr + 8),
                                            *(ptr + 9), *(ptr + 10), *(ptr + 11), *(ptr + 12),
                                            *(ptr + 13), *(ptr + 14), *(ptr + 15), if_id, reason);
                                    } else {
                        ...
                        if (netagent_io_get_addr_type(netagent_io_cmd_obj, &type) ==
                            ...
                            if (type & NETAGENT_IO_ADDR_TYPE_IPv4) {
                                if (netagent_io_get_addr_v4(netagent_io_cmd_obj, &addr_v4) ==
                                    NETAGENT_IO_RET_SUCCESS) {
                                    unsigned char* ptr = (unsigned char*)&addr_v4;
                                    Dbg("### get ifchg event with v4 address = %d.%d.%d.%d, if = "
                                        "%d, reason = %s",
                                        *ptr, *(ptr + 1), *(ptr + 2), *(ptr + 3), if_id, reason);
                                } else {
                                    Err("get v4 address fail");
                                }
                                {
                                    unsigned char addr_v6[4] = {0xAA, 0xBB, 0xCC, 0xDD};
                                    send_if_chg(netagent_io_obj, if_id, NETAGENT_IO_ADDR_TYPE_IPv4,
                                                addr_v6);
                                }
                            }
                            ...
                            if (type & NETAGENT_IO_ADDR_TYPE_IPv6) {
                                if (netagent_io_get_addr_v6(netagent_io_cmd_obj, addr_v6) ==
                                    NETAGENT_IO_RET_SUCCESS) {
                                    unsigned char* ptr = (unsigned char*)&addr_v6;
                                    Dbg("### get ifchg event with v6 address "
                                        "%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%"
                                        "02X:%02X:%02X:%02X:%02X, if = %d, reason = %s",
                                        *ptr, *(ptr + 1), *(ptr + 2), *(ptr + 3), *(ptr + 4),
                                        *(ptr + 5), *(ptr + 6), *(ptr + 7), *(ptr + 8), *(ptr + 9),
                                        *(ptr + 10), *(ptr + 11), *(ptr + 12), *(ptr + 13),
                                        *(ptr + 14), *(ptr + 15), if_id, reason);
                                } else {
                                    Err("get v6 address fail");
                                }
                                {
                                    unsigned char addr_v6[16] = {0xFE, 0x80, 0x12, 0x34, 0x56, 0x78,
                                                                 0xAB, 0xCD, 0xEF, 0x12, 0x34, 0x56,
                                                                 0x78, 0x90, 0xAB, 0xCD};
                                    send_if_chg(netagent_io_obj, if_id, NETAGENT_IO_ADDR_TYPE_IPv6,
                                                addr_v6);
                                }
                            }
