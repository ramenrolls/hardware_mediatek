...
RFX_REGISTER_DATA_TO_REQUEST_ID(RfxVoidData, RfxPhbMemStorageData,
...
RFX_REGISTER_DATA_TO_REQUEST_ID(RfxIntsData, RfxPhbEntriesExtData,
                                RFX_MSG_REQUEST_READ_PHB_ENTRY_EXT);
RFX_REGISTER_DATA_TO_REQUEST_ID(RfxPhbEntryExtData, RfxVoidData,
...
RmcPhbRequestHandler::RmcPhbRequestHandler(int slot_id, int channel_id)
    ...
    const int request1[] = {
            ...
            RFX_MSG_REQUEST_READ_UPB_AAS_LIST,      RFX_MSG_REQUEST_PHB_CURRENT_STORAGE_RESET,
...
void RmcPhbRequestHandler::onHandleRequest(const sp<RfxMclMessage>& msg) {
    ...
    if (cardType <= 0 && request != RFX_MSG_REQUEST_PHB_CURRENT_STORAGE_RESET) {
        logE(RFX_LOG_TAG, "onHandleRequest No SIM inserted, cardType: %d, request: %d", cardType,
             request);
...
#endif
        {0x0040, 0x0000},  //@
        {0x00A3, 0x0001}, {0x0024, 0x0002}, {0x00A5, 0x0003}, {0x00E8, 0x0004},
        {0x00E9, 0x0005}, {0x00F9, 0x0006}, {0x00EC, 0x0007}, {0x00F2, 0x0008},
        {0x00C7, 0x0009}, {0x0020, 0x0020}, {0x00D8, 0x000B}, {0x00F8, 0x000C},
        {0x0020, 0x0020}, {0x00C5, 0x000E}, {0x00E5, 0x000F}, {0x0394, 0x0010},
        {0x005F, 0x0011}, {0x03A6, 0x0012}, {0x0393, 0x0013}, {0x039B, 0x0014},
        {0x03A9, 0x0015}, {0x03A0, 0x0016}, {0x03A8, 0x0017}, {0x03A3, 0x0018},
        {0x0398, 0x0019}, {0x039E, 0x001A}, {0x00C6, 0x001C}, {0x00E6, 0x001D},
        {0x00DF, 0x001E}, {0x00C9, 0x001F}, {0x00A4, 0x0024}, {0x00A1, 0x0040},
        {0x00C4, 0x005B}, {0x00D6, 0x005C}, {0x00D1, 0x005D}, {0x00DC, 0x005E},
        {0x00A7, 0x005F}, {0x00BF, 0x0060}, {0x00E4, 0x007B}, {0x00F6, 0x007C},
        {0x00F1, 0x007D}, {0x00FC, 0x007E}, {0x00E0, 0x007F},
#ifdef ESC_SUPPORT
        {0x0020, 0x0020}, {0x005E, 0x1B14}, {0x007B, 0x1B28}, {0x007D, 0x1B29},
        {0x005C, 0x1B2F}, {0x005B, 0x1B3C}, {0x007E, 0x1B3D}, {0x005D, 0x1B3E},
        {0x007C, 0x1B40}, {0x20AC, 0x1B65}
#endif
};
...
int RmcPhbRequestHandler::isMatchGsm7bit(char* src, int maxLen) {
    ...
    DlogD(RFX_LOG_TAG, "isMatchGsm7bit source = %c%c%c%c", src[j], src[j + 1], src[j + 2],
          src[j + 3]);
...
int RmcPhbRequestHandler::isGsm7bitExtension(char* src, int maxLen) {
    ...
    DlogD(RFX_LOG_TAG, "isGsm7bitExtension source = %c%c%c%c", src[j], src[j + 1], src[j + 2],
          src[j + 3]);
...
int RmcPhbRequestHandler::encodeUCS2_0x81(char* src, char* des, int maxLen) {
...
#ifndef ESC_SUPPORT
                if (tmpAlphaId[i] == 0x0020 || tmpAlphaId[i] == 0x005E || tmpAlphaId[i] == 0x007B ||
                    tmpAlphaId[i] == 0x007D || tmpAlphaId[i] == 0x005B || tmpAlphaId[i] == 0x007E ||
                    tmpAlphaId[i] == 0x005D || tmpAlphaId[i] == 0x005C || tmpAlphaId[i] == 0x007C)
                    return 0;
                else
#endif
                {
    ...
    if (realLen > len) {
        *(pLen + 2) =
                (char)((realLen / 16) > 9 ? ((realLen / 16) - 10 + 'A') : (realLen / 16) + '0');
        *(pLen + 3) =
                (char)((realLen % 16) > 9 ? ((realLen % 16) - 10 + 'A') : (realLen % 16) + '0');
    }
    *des = '\0';
...
int RmcPhbRequestHandler::encodeUCS2_0x82(char* src, char* des, int maxLen) {
    ...
    for (i = 0; i < len; i++) {
        ...
        if (needSpecialEncoding == 0) {
            if ((tmpAlphaId[i] < 0x0080) && (tmpAlphaId[i] != 0x0060)) continue;
...
void RmcPhbRequestHandler::requestQueryPhbInfo(const sp<RfxMclMessage>& msg) {
    ...
    if (selectPhbStorage(type)) {
        ...
        logD(RFX_LOG_TAG, "PhbQueryInformation: %d %d %d %d", query_info[0], query_info[1],
             query_info[2], query_info[3]);
    ...
    if (selectPhbStorage(entry->type)) {
        if (entry->alphaId != NULL || entry->number != NULL) {
            ...
            if (len == 0 && strlen(number) == 0) {
                logE(RFX_LOG_TAG,
                     "The number and alphaid can't be empty String at the same time: %s %s", line,
                     number);
                goto error;
            }
            ...
            if (i != len || asciiNotSupported) {
                ...
                if (encodeUCS2_0x81(line, alphaId, sizeof(alphaId))) {  // try UCS2_0x81 coding
                    ...
                    logD(RFX_LOG_TAG, "alphaId length=%zu, line length=%zu", sizeof(alphaId),
                         strlen(line));
                    memcpy(alphaId, line,
            ...
            cmd.append(String8::format("AT+EPBW=%d, \"%s\", %d, \"%s\", 0, %d", entry->index,
                                       number, entry->ton, alphaId, encodeMethod));
        } else {
            // delete
...
void RmcPhbRequestHandler::requestReadPhbEntry(const sp<RfxMclMessage>& msg) {
    ...
    if (selectPhbStorage(type)) {
        ...
        for (i = 0, p_cur = p_response->getIntermediates(); p_cur != NULL;
...
void RmcPhbRequestHandler::requestQueryUPBCapability(const sp<RfxMclMessage>& msg) {
    ...
    logD(RFX_LOG_TAG, "requestQueryUPBCapability: %d, %d, %d, %d, %d, %d, %d, %d", upbCap[0],
         upbCap[1], upbCap[2], upbCap[3], upbCap[4], upbCap[5], upbCap[6], upbCap[7]);

    response = RfxMclMessage::obtainResponse(msg->getId(), RIL_E_SUCCESS, RfxIntsData(upbCap, 8),
                                             msg, false);
    responseToTelCore(response);
    return;
error:
...
void RmcPhbRequestHandler::requestEditUPBEntry(const sp<RfxMclMessage>& msg) {
    ...
    if (0 == strcmp(params[0], "4") || 0 == strcmp(params[0], "3") || 0 == strcmp(params[0], "2")) {
        char temp[RIL_MAX_PHB_NAME_LEN * 4 + 4 + 1] = {0};
        char str[RIL_MAX_PHB_NAME_LEN * 4 + 4 + 1] = {
                0};  // If using UCS2, each character use 4 bytes,
        ...
        cmd.append(String8::format("AT+EPBUM=2,%s,%s,%s,\"%s\",%d", params[0], params[2], params[1],
                                   str, encodeMethod));
    } else if (0 == strcmp(params[0], "1")) {  // Email
        char temp[RIL_MAX_PHB_EMAIL_LEN * 4 + 4 + 1] = {0};
        char str[RIL_MAX_PHB_EMAIL_LEN * 4 + 4 + 1] = {
                0};  // If using UCS2, each character use 4 bytes,
                     // if using 0x82 coding, we need extra 4 bytes, we also need '\0'
        line = params[3];
        if (line == NULL) {
        ...
        cmd.append(String8::format("AT+EPBUM=2,%s,%s,%s,\"%s\"", params[0], params[2], params[1],
                                   str));
...
void RmcPhbRequestHandler::requestReadGasList(const sp<RfxMclMessage>& msg) {
    ...
    response = RfxMclMessage::obtainResponse(
            msg->getId(), ret, RfxStringsData((char**)responses, (params[1] - params[0] + 1)), msg,
            false);
    responseToTelCore(response);
    free(responses);
    return;
...
void RmcPhbRequestHandler::requestReadUpbGrpEntry(const sp<RfxMclMessage>& msg) {
    ...
    if (paramCount < 1) {
        logE(RFX_LOG_TAG, "requestReadUpbGrpEntry param is not enough. paramCount is %d",
             paramCount);
        goto error;
    }
...
void RmcPhbRequestHandler::requestWriteUpbGrpEntry(const sp<RfxMclMessage>& msg) {
    ...
    int datalen = msg->getData()->getDataLength();
    int paramCount = datalen / sizeof(int);
    logD(RFX_LOG_TAG, "requestWriteUpbGrpEntry . datalen is %d, size of int is %zu", datalen,
         sizeof(int));
    if (paramCount < 1) {
        logE(RFX_LOG_TAG, "requestWriteUpbGrpEntry param is not enough. paramCount is %d",
             paramCount);
        goto error;
    }
...
void RmcPhbRequestHandler::requestGetPhoneBookStringsLength(const sp<RfxMclMessage>& msg) {
    ...
    line->atTokStart(&err);
    if (err < 0) goto error;
    // logD(RFX_LOG_TAG, "requestGetPhoneBookStringsLength: strlen of response is %d",
    // strlen(line)); +CPBR: (<bIndex>-<eIndex>), <max_num_len>, <max_alpha_len>
    ...
    line = p_response->getIntermediates();
    line->atTokStart(&err);
    // logD(RFX_LOG_TAG, "requestGetPhoneBookStringsLength: epbum strlen of response is %d",
    // strlen(line)); +CPBR: <n_anr>, <n_email>, <n_sne>, <n_aas>, <l_aas>, <n_gas>, <l_gas>,
    // <n_grp>
    ...
    res = RfxMclMessage::obtainResponse(msg->getId(), RIL_E_SUCCESS, RfxIntsData(response, 5), msg,
                                        false);
...
void RmcPhbRequestHandler::requestSetPhoneBookMemStorage(const sp<RfxMclMessage>& msg) {
    ...
    if (err < 0 || p_response == NULL) {
        ...
        int type = -1;
        if (0 == strcmp("SM", storage)) type = RIL_PHB_ADN;
        if (0 == strcmp("FD", storage)) type = RIL_PHB_FDN;
        if (0 == strcmp("ON", storage)) type = RIL_PHB_MSISDN;
        if (0 == strcmp("EN", storage)) type = RIL_PHB_ECC;
        current_phb_storage = type;
    }
    if (p_response == NULL || p_response->getSuccess() == 0) {
        ret = RIL_E_GENERIC_FAILURE;
...
void RmcPhbRequestHandler::loadUPBCapability() {
    ...
    logD(RFX_LOG_TAG, "loadUPBCapability: %d, %d, %d, %d, %d, %d, %d, %d", upbCap[0], upbCap[1],
         upbCap[2], upbCap[3], upbCap[4], upbCap[5], upbCap[6], upbCap[7]);
error:
    logE(RFX_LOG_TAG, "loadUPBCapability: Error!");
}
...
void RmcPhbRequestHandler::requestReadPhoneBookEntryExt(const sp<RfxMclMessage>& msg) {
    ...
    for (i = 0, p_cur = p_response->getIntermediates(); p_cur != NULL;
        ...
        entry->hidden = 0;
        DlogD(RFX_LOG_TAG, "PhbEntry %d: %s, %d, %s", entry->index, entry->number, entry->type,
              entry->text);
        // ANR
        if (maxAnrNum > 0 && (strlen(entry->text) > 0 || strlen(entry->number) > 0)) {
            ...
            if (p_res != NULL && p_res->getSuccess() != 0) {
                ...
                DlogD(RFX_LOG_TAG, "PhbEntry anr %d: %s, %d", entry->index, entry->adnumber,
                      entry->adtype);
            }
        }
        // email
...
void RmcPhbRequestHandler::requestWritePhoneBookEntryExt(const sp<RfxMclMessage>& msg) {
    ...
    char *line, *number;
    String8 cmd("");
    char alphaId[RIL_MAX_PHB_NAME_LEN * 4 + 4 + 1] = {
            0,
    };  // If using UCS2, each character use 4 bytes,
        // if using 0x82 coding, we need extra 4 bytes,
        // we also need '\0'
    ...
    if (entry != NULL && (entry->text != NULL || entry->number != NULL)) {
        ...
        requestClearPhbEntry(entry->index);
        cmd.append(String8::format("AT+EPBW=%d, \"%s\", %d, \"%s\", 0, %d", entry->index, number,
                                   entry->type, alphaId, encodeMethod));
    } else {
        // delete
        cmd.append(String8::format("AT+EPBW=%d", entry->index));
    }
    ...
    if (ret == RIL_E_SUCCESS && maxAnrNum > 0) {
        ...
        if (bIsTc1() == true) {
            if (entry->adnumber != NULL && strlen(entry->adnumber) > 0) {
                cmd.append(String8::format("AT+EPBUM=2,0,%d,%d,\"%s\",%d,0", entry->index, 1,
    ...
    if (ret == RIL_E_SUCCESS && maxEmailNum > 0) {
        ...
        if (bIsTc1() == true) {
            if (entry->email != NULL && strlen(entry->email) > 0) {
                ...
                cmd.append(String8::format("AT+EPBUM=2,1,%d,%d,\"%s\"", entry->index, 1,
                                           entry->email));
            } else {
            ...
            if (entry->email != NULL && strlen(entry->email) > 0) {
                // at+epbum=<op>,<type>,<adn_index>,<ef_index>,<email>
                cmd.append(String8::format("AT+EPBUM=2,1,%d,%d,\"%s\"", entry->index, maxEmailNum,
                                           entry->email));
            } else {
...
void RmcPhbRequestHandler::requestReadUPBEmail(const sp<RfxMclMessage>& msg) {
    ...
    if (maxEmailNum > 0) {
        ...
        res = RfxMclMessage::obtainResponse(msg->getId(), ret, RfxStringData(response, strLen), msg,
                                            false);
        responseToTelCore(res);
        return;
    }
error:
    ...
    res = RfxMclMessage::obtainResponse(msg->getId(), ret, RfxVoidData(), msg, false);
    responseToTelCore(res);
}
...
void RmcPhbRequestHandler::requestReadUPBAnr(const sp<RfxMclMessage>& msg) {
    ...
    if (maxAnrNum > 0) {
        ...
        response = RfxMclMessage::obtainResponse(msg->getId(), ret, RfxPhbEntriesData(entries, 1),
                                                 msg, false);
        responseToTelCore(response);
        if (entries[0] != NULL) {
            free(entries[0]);
...
void RmcPhbRequestHandler::requestReadAasList(const sp<RfxMclMessage>& msg) {
    ...
    for (i = params[0]; i <= params[1]; i++) {
        ...
        logD(RFX_LOG_TAG, "requestReadAasList:  [%d], %s", entryIndex - 1,
             responses[entryIndex - 1]);
    }
