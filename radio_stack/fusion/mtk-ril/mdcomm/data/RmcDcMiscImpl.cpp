...
void RmcDcMiscImpl::requestStartLce(const sp<RfxMclMessage>& msg) {
    ...
    p_response = atSendCommandSingleline(String8::format("AT+ELCE=%d,%u", lceMode, desiredInterval),
                                         "+ELCE:");

    if (p_response == NULL) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] fail to get p_response!", mSlotId, __FUNCTION__);
        goto error;
    }

    if (p_response->isAtResponseFail()) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] AT+ELCE=%d,%u responce ERROR:%d", mSlotId, __FUNCTION__,
                  lceMode, desiredInterval, p_response->atGetCmeError());
        goto error;
    }
    ...
    if (pLine == NULL) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] fail to get intermediate results!", mSlotId, __FUNCTION__);
        goto error;
    }
    ...
    if (err < 0) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing lce status", mSlotId,
                  __FUNCTION__);
        goto error;
    }
    ...
    if (pLine->atTokHasmore()) {
        ...
        if (err < 0) {
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing actual interval", mSlotId,
                      __FUNCTION__);
            goto error;
        }
    }
    ...
    RFX_LOG_I(RFX_LOG_TAG, "[%d][%s] lceStatus=%d, actualInterval=%u", mSlotId, __FUNCTION__,
              lceStatus, actualInterval);
    response->lce_status = lceStatus;
    response->actual_interval_ms = actualInterval;
    responseMsg = RfxMclMessage::obtainResponse(
            RIL_E_SUCCESS, RfxLceStatusResponseData(response, sizeof(RIL_LceStatusInfo)), msg);
...
void RmcDcMiscImpl::requestStopLce(const sp<RfxMclMessage>& msg) {
    ...
    if (p_response->isAtResponseFail()) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] AT+ELCE=0 responce ERROR:%d", mSlotId, __FUNCTION__,
...
void RmcDcMiscImpl::requestPullLceData(const sp<RfxMclMessage>& msg) {
    ...
    sp<RfxAtResponse> p_response;
    sp<RfxMclMessage> responseMsg;
    int lceMode = 0;  // LCE service mode : stop mode = 0; start with PUSH mode = 1; start with PULL
                      // mode = 2.
    int err = 0;
    ...
    if (err < 0) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing lce mode", mSlotId,
                  __FUNCTION__);
        goto error;
    }
    ...
    if (pLine->atTokHasmore()) {
        ...
        if (err < 0) {
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing last hop capacity", mSlotId,
                      __FUNCTION__);
            goto error;
        }
    ...
    RFX_LOG_I(RFX_LOG_TAG,
              "[%d][%s] last_hop_capacity_kbps=%u, confidence_level=%u, "
              "lce_suspended=%u",
              mSlotId, __FUNCTION__, response->last_hop_capacity_kbps, response->confidence_level,
              response->lce_suspended);
    responseMsg = RfxMclMessage::obtainResponse(
            RIL_E_SUCCESS, RfxLceDataResponseData(response, sizeof(RIL_LceDataInfo)), msg);
    responseToTelCore(responseMsg);
    FREEIF(response);
    return;
...
void RmcDcMiscImpl::setFdMode(const sp<RfxMclMessage>& msg) {
    ...
    if (pReqInt == NULL) {
        ...
        responseToTcl = RfxMclMessage::obtainResponse(msg->getId(), RIL_E_SUCCESS, RfxVoidData(),
                                                      msg, true);
    ...
    if (argsNum == 1) {
        ...
        responseFromModem = atSendCommand(String8::format("AT+EFD=%d,%d", pReqInt[1], pReqInt[2]));
    } else if (argsNum == 3) {
        ...
        responseFromModem = atSendCommand(
                String8::format("AT+EFD=%d,%d,%d", pReqInt[1], pReqInt[2], pReqInt[3]));
    } else {
...
void RmcDcMiscImpl::onLinkCapacityEstimate(const sp<RfxMclMessage>& msg) {
    ...
    if (err < 0) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing downlinkCapacityKbps", mSlotId,
                  __FUNCTION__);
        goto error;
    }
    ...
    if (err < 0) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing uplinkCapacityKbps", mSlotId,
                  __FUNCTION__);
        goto error;
    }

    urcMsg = RfxMclMessage::obtainUrc(
            RFX_MSG_URC_LINK_CAPACITY_ESTIMATE, mSlotId,
...
void RmcDcMiscImpl::disableLegacyFastDormancy() {
    ...
    if (err < 0) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing mode", mSlotId, __FUNCTION__);
        return;
    }
    ...
    if (pLine->atTokHasmore()) {
        ...
        if (err < 0) {
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing parameter", mSlotId,
                      __FUNCTION__);
            return;
        }
    } else {
    ...
    atSendCommand(String8::format("AT+EPCT=%d,%d", mode,
