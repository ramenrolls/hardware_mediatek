...
RmcDcImsDataChannel2ReqHandler::RmcDcImsDataChannel2ReqHandler(int slot_id, int channel_id)
    ...
    RFX_LOG_D(RFX_LOG_TAG, "RmcDcImsDataChannel2ReqHandler Ctor, slot_id=%d, channel_id=%d",
              slot_id, channel_id);
    ...
    sendEvent(RFX_MSG_EVENT_IMS_REQUEST_HANDLER_REGISTER_DONE, RfxVoidData(), RIL_CMD_PROXY_URC,
              m_slot_id);
}
...
void RmcDcImsDataChannel2ReqHandler::notifyImsBearerRequest(
        int requestId, RIL_IMS_BearerNotification* notification, bool isUpdateState) {
    RFX_LOG_D(RFX_LOG_TAG, "%s,isEnableBearerNotification=%d", __FUNCTION__,
              isEnableBearerNotification);
    ...
    if (isEnableBearerNotification == 1 || isEnableBearerNotification == 2) {
        sp<RfxMclMessage> urc_to_tel_core = RfxMclMessage::obtainUrc(
                requestId, m_slot_id,
                RfxImsBearerNotifyData((void*)notification, sizeof(RIL_IMS_BearerNotification)));
        responseToTelCore(urc_to_tel_core);
        if (isUpdateState) {
            if (action == 1) {
                if (strncmp(notification->type, "ims", 3) == 0) {
                    getMclStatusManager(m_slot_id)->setIntValue(RFX_STATUS_KEY_IMS_PDN_STATE,
                                                                pdnState);
                    getMclStatusManager()->setIntValue(RFX_STATUS_KEY_IMS_PDN_ACT_AID,
                                                       notification->aid);
                } else if (strncmp(notification->type, "emergency", 9) == 0) {
                    getMclStatusManager(m_slot_id)->setIntValue(RFX_STATUS_KEY_EIMS_PDN_STATE,
                                                                pdnState);
                    getMclStatusManager()->setIntValue(RFX_STATUS_KEY_EIMS_PDN_ACT_AID,
                                                       notification->aid);
                }
            } else if (action == 0) {
                if (strncmp(notification->type, "ims", 3) == 0) {
                    getMclStatusManager(m_slot_id)->setIntValue(RFX_STATUS_KEY_IMS_PDN_STATE,
                                                                pdnState);
                    getMclStatusManager()->setIntValue(RFX_STATUS_KEY_IMS_PDN_DEACT_AID,
                                                       notification->aid);
                } else if (strncmp(notification->type, "emergency", 9) == 0) {
                    getMclStatusManager(m_slot_id)->setIntValue(RFX_STATUS_KEY_EIMS_PDN_STATE,
                                                                pdnState);
                    getMclStatusManager()->setIntValue(RFX_STATUS_KEY_EIMS_PDN_DEACT_AID,
                                                       notification->aid);
                }
            }
        }
    ...
    if (isEnableBearerNotification == 0 || isEnableBearerNotification == 2) {
        ...
        RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] send %s, isEnableBearerNotification=%d ", rid,
                  __FUNCTION__, cmd.string(), isEnableBearerNotification);
    ...
    if (isEnableBearerNotification == 3) {
        ...
        if (action == 1) {
            if (strncmp(notification->type, "ims", 3) == 0) {
                sp<RfxMclMessage> urc_to_tel_core = RfxMclMessage::obtainUrc(
                        requestId, m_slot_id,
                        RfxImsBearerNotifyData((void*)notification,
                                               sizeof(RIL_IMS_BearerNotification)));
                responseToTelCore(urc_to_tel_core);
                if (isUpdateState) {
                    getMclStatusManager()->setIntValue(RFX_STATUS_KEY_IMS_PDN_ACT_AID,
                                                       notification->aid);
...
void RmcDcImsDataChannel2ReqHandler::notifyImsDataInfo(RIL_IMS_DataInfoNotify* notification) {
    ...
    sp<RfxMclMessage> urc_to_tel_core = RfxMclMessage::obtainUrc(
            RFX_MSG_URC_IMS_DATA_INFO_NOTIFY, m_slot_id,
            RfxImsDataInfoNotifyData((void*)notification, sizeof(RIL_IMS_DataInfoNotify)));
    responseToTelCore(urc_to_tel_core);
}
...
void RmcDcImsDataChannel2ReqHandler::handleImsBearerConfirm(const sp<RfxMclMessage>& msg) {
    ...
    p_response = atSendCommand(cmd);
    if (p_response->isAtResponseFail()) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s], %s returns ERROR: %d", rid, __FUNCTION__, cmd.string(),
                  p_response->getError());
    }
...
void RmcDcImsDataChannel2ReqHandler::handleImsBearerNotify(const sp<RfxMclMessage>& msg) {
    ...
    //+EIMSPDN: "notify", <aid>, <state>, <type>, <if_id>[, <dnsv4_num>, <dnsv4_addr> [,<dnsv6_num>,
    //<dnsv6_addr>]]
    ...
    if (strncmp("notify", cmdFormat, strlen("notify")) == 0) {
        ...
        if (err < 0) {
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing state", rid, __FUNCTION__);
            return;
        }
        ...
        RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] IMS notification phone=%d, aid=%d, type=%s, action = %d",
                  rid, __FUNCTION__, notification->phone, notification->aid, notification->type,
                  action);
        ...
        if (action == ACTION_IMS_BEARER_ACTIVATION) {
            ...
            if (err >= 0) {
                ...
                RFX_LOG_I(RFX_LOG_TAG,
                          "[%d][%s] activate pdn type is %s, slotid is %d, interfaceID is %d", rid,
                          __FUNCTION__, type, rid, interfaceId);
                    ...
                    if (begin_ != std::string::npos) {
                        ...
                        if (end_ != std::string::npos) {
                            ...
                            RFX_LOG_E(RFX_LOG_TAG,
                                      "[%d][%s] End index is not found, no need to clean property",
                                      rid, __FUNCTION__);
                        }
                    } else {
                        RFX_LOG_E(RFX_LOG_TAG,
                                  "[%d][%s] Begin index is not found, no need to clean property",
                                  rid, __FUNCTION__);
                    }
                    RFX_LOG_I(RFX_LOG_TAG, "[%d][%s] current vendor.ims.eims.pdn.info= %s", rid,
                              __FUNCTION__, currProp);
                    strncat(currProp + strlen(currProp), tempProp,
                            MTK_PROPERTY_VALUE_MAX - strlen(currProp) - 1);
                }
                rfx_property_set(IMS_EIMS_PDN_INFO, currProp);
                RFX_LOG_I(RFX_LOG_TAG, "[%d][%s] updated vendor.ims.eims.pdn.info= %s", rid,
                          __FUNCTION__, currProp);
            } else {
            ...
            char begin_Index[MAX_PROFILEID_LENGTH + 2] = {0};
            snprintf(begin_Index, sizeof(begin_Index), "%s_%d", type, rid);
            RFX_LOG_I(RFX_LOG_TAG, "[%d][%s] deactivate pdn type is %s, slotid is %d", rid,
                      __FUNCTION__, type, rid);
            if (strcmp(currProp, "") != 0) {
                ...
                if (begin_ != std::string::npos) {
                    ...
                    if (end_ != std::string::npos) {
                        ...
                        RFX_LOG_E(RFX_LOG_TAG,
                                  "[%d][%s] End index is not found, no need to clean property", rid,
                                  __FUNCTION__);
                    }
                } else {
                    RFX_LOG_E(RFX_LOG_TAG,
                              "[%d][%s] Begin index is not found, no need to clean property", rid,
                              __FUNCTION__);
                }
                RFX_LOG_I(RFX_LOG_TAG, "[%d][%s] updated vendor.ims.eims.pdn.info= %s", rid,
                          __FUNCTION__, currProp);
            } else {
                RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] Property is null, no need to clean property", rid,
                          __FUNCTION__);
            }  // LinkMaster
        ...
        RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] IMS notification phone=%d bearer initial...", rid,
                  __FUNCTION__, m_slot_id);
        sp<RfxMclMessage> urc_to_tel_core =
                RfxMclMessage::obtainUrc(RFX_MSG_URC_IMS_BEARER_INIT, m_slot_id,
        ...
        if (err < 0) {
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing cause", rid, __FUNCTION__);
            return;
        }
...
void RmcDcImsDataChannel2ReqHandler::handleImsBearerNotificationSet(const sp<RfxMclMessage>& msg) {
    ...
    RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] isEnableBearerNotification: %d", rid, __FUNCTION__,
              isEnableBearerNotification);
    responseMsg = RfxMclMessage::obtainResponse(RIL_E_SUCCESS, RfxVoidData(), msg);
    responseToTelCore(responseMsg);
}
...
void RmcDcImsDataChannel2ReqHandler::handlePcscfAddressDiscovery(const sp<RfxMclMessage>& msg) {
    ...
    if (err < 0) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing tranid", rid, __FUNCTION__);
        goto error;
    }

    em_ind = pLine->atTokNextint(&err);
    if (err < 0) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing em_ind", rid, __FUNCTION__);
        goto error;
    }

    method = pLine->atTokNextint(&err);
    if (err < 0) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing method", rid, __FUNCTION__);
        goto error;
    }

    interfaceId = pLine->atTokNextstr(&err);
    if (err < 0) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing interfaceId", rid, __FUNCTION__);
        goto error;
    }
    ...
    // RFX_LOG_D(RFX_LOG_TAG, "[%d][%s], rid, __FUNCTION__, send %s", rid, __FUNCTION__,
    // pcscfCmd.string());
...
void RmcDcImsDataChannel2ReqHandler::checkImsServiceRestartAfterSystemCrash() {
    ...
    RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] vendor.ril.zygote.state=%s", m_slot_id, __FUNCTION__,
              zygote_value);

    if (strcmp(zygote_value, "stopped") == 0 || strcmp(zygote_value, "restarting") == 0) {
        // trigger TRM to handle unsynchronized scenario
        RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] trigger TRM after zygote restarting", m_slot_id,
                  __FUNCTION__);
...
void RmcDcImsDataChannel2ReqHandler::notifyUnsyncImsBearerRequest() {
    ...
    imsPdnState =
            getMclStatusManager()->getIntValue(RFX_STATUS_KEY_IMS_PDN_STATE, IMS_OR_EIMS_PDN_INIT);
    ...
    if (imsPdnState == IMS_OR_EIMS_PDN_ACTIVATED) {
        ...
        if (aid != RFX_NO_ONGOING_IMS_PDN_AID) {
            ...
            RFX_LOG_D(
                    RFX_LOG_TAG,
                    "[%d][%s] notifyUnsyncImsBearerRequest phone=%d, aid=%d, type=%s, action = %d",
                    m_slot_id, __FUNCTION__, notification->phone, notification->aid,
                    notification->type, notification->action);

            notifyImsBearerRequest(RFX_MSG_URC_IMS_BEARER_STATE_NOTIFY, notification.get(), false);
        } else {
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] no ongoing IMS PDN ACT aid", m_slot_id, __FUNCTION__);
        }
        ...
        if (aid != RFX_NO_ONGOING_IMS_PDN_AID) {
            ...
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] no ongoing IMS PDN DEACT aid", m_slot_id,
                      __FUNCTION__);
        }
    } else {
        RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] no ongoing ACTIVATED or DEACTIVATED event", m_slot_id,
                  __FUNCTION__);
    }
}
