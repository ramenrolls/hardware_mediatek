...
int RmcDcCommonReqHandler::s_dataSetting_resend[MAX_SIM_COUNT][DATA_SETTING_NUMBERS] = {
        {SKIP_DATA_SETTINGS}};

RmcDcCommonReqHandler::RmcDcCommonReqHandler(int slot_id, int channel_id,
                                             RmcDcPdnManager* pdnManager)
    : RfxBaseHandler(slot_id, channel_id),
      m_pPdnManager(pdnManager),
...
int RmcDcCommonReqHandler::activatePdn(const char* reqApn, const char* profileType,
                                       Vector<int>* vAidList, int isEmergency,
                                       MTK_RIL_Data_Call_Response_v11* response, int cmdIndex) {
    String8 cmd("");
    int err = 0;
    ...
    RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] isEmergency: %d", rid, __FUNCTION__, isEmergency);

    if (response == NULL) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] response is null!", rid, __FUNCTION__);
        return PDN_FAILED;
    }
    ...
    for (p_cur = p_response->getIntermediates(); p_cur != NULL; p_cur = p_cur->getNext()) {
        ...
        if (err < 0) {
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] Error when token start", rid, __FUNCTION__);
            goto error;
        }
        ...
        if (err < 0) {
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] Error when parsing aid", rid, __FUNCTION__);
            goto error;
        }
        ...
        if (err < 0) {
            RFX_LOG_E(RFX_LOG_TAG,
                      "[%d][%s] ERROR occurs when converting aid to binary"
                      ", err = %d",
        ...
        RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] default bearer activated [aid=%d]", rid, __FUNCTION__,
                  aid);

        if (p_cur->atTokHasmore()) {
            reason = p_cur->atTokNextint(&err);
            if (err < 0) {
                RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] Error when parsing reason for aid%d", rid,
                          __FUNCTION__, aid);
                goto error;
            }

            RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] default bearer activated [aid=%d, reason=%d]", rid,
                      __FUNCTION__, aid, reason);

            if (p_cur->atTokHasmore()) {
                otherAid = p_cur->atTokNextint(&err);
                if (err < 0) {
                    RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] Error when parsing otherAid for aid%d", rid,
                              __FUNCTION__, aid);
                    goto error;
                }

                RFX_LOG_D(RFX_LOG_TAG,
                          "[%d][%s] default bearer activated [aid=%d, reason=%d,"
                          " otherAid=%d]",
                          rid, __FUNCTION__, aid, reason, otherAid);
            }
        }
    ...
    RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] %zu PDN activated", rid, __FUNCTION__, vAidList->size());

    if (p_response->isAtResponseFail() || (vAidList->size() == 0)) {
        if (p_response->getSuccess() > 0) {
            RFX_LOG_E(RFX_LOG_TAG,
                      "[%d][%s] weird case, can not get fail cause due to final"
        ...
        if (cmeError == CME_UNKNOWN || cmeError == CME_ERROR_NON_CME) {
            ...
            if (m_nGprsFailureCause == PDP_FAIL_NONE) {
                RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] NO CAUSE(0) but AT+EAPNACT failed", rid,
                          __FUNCTION__);
                m_nGprsFailureCause = PDP_FAIL_ERROR_UNSPECIFIED;
            } else {
                RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] AT+EAPNACT CAUSE: %X", rid, __FUNCTION__,
                          m_nGprsFailureCause);
            }
        }
        goto cme_error;
...
int RmcDcCommonReqHandler::getModemSuggestedRetryTime(const char* apnName) {
    ...
    if (m_nGprsFailureCause == PDP_FAIL_DATA_NOT_ALLOW) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] m_nGprsFailureCause is PDP_FAIL_DATA_NOT_ALLOW", rid,
                  __FUNCTION__);
    ...
    if (p_response->isATCmdRspErr()) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] AT+EDRETRY query command returns ERROR", rid,
                  __FUNCTION__);
        return -1;  // NO suggested retry time
    } else {
        err = p_response->getError();
        if (err < 0) {
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] fail to get error!", rid, __FUNCTION__);
            return -1;
        }
        ...
        if (line == NULL) {
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] fail to get intermediate results!", rid, __FUNCTION__);
            return -1;
        }
        ...
        if (suggestType == RETRY_TYPE_NO_SUGGEST) {
            ...
            if (line->atTokHasmore()) {
                ...
                if (err < 0) return -1;
                RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] Modem suggest time:[%d]", rid, __FUNCTION__,
                          suggestTime);
...
int RmcDcCommonReqHandler::updatePdnInformation(const int activatedAid, int protocol,
    ...
    if (p_response->isAtResponseFail()) {
        if (p_response->getSuccess() > 0) {
            ...
            RFX_LOG_E(RFX_LOG_TAG,
                      "[%d][%s] AT+EPDN(ifst) for activatedAid%d returns cme cause : %d", rid,
                      __FUNCTION__, activatedAid, cause);
        }
        return PDN_FAILED;
    }
    ...
    if (err < 0 || (tempAid != activatedAid)) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] fail to get aid or it's different to MD assigned", rid,
                  __FUNCTION__);
        return PDN_FAILED;
    }
    ...
    if (strncmp(DC_ERROR, cmdFormat, strlen(DC_ERROR)) == 0) {
        if (line->atTokHasmore()) {
            ...
            if (err < 0) {
                RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing err cause", rid,
                          __FUNCTION__);
            } else {
                RFX_LOG_E(RFX_LOG_TAG,
                          "[%d][%s] AT+EPDN(ifst) for activatedAid%d returns err cause : %d", rid,
                          __FUNCTION__, activatedAid, cause);
            }
        }
        return PDN_FAILED;
        ...
        if (err < 0) {
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing rat type", rid,
                      __FUNCTION__);
            return PDN_FAILED;
        }
        ...
        if (err < 0) {
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing transIntfId", rid,
                      __FUNCTION__);
            return PDN_FAILED;
        }
        RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] push transIntfId %d to pdn table [%d]", rid, __FUNCTION__,
                  transIntfId, activatedAid);
        updateTransIntfId(activatedAid, transIntfId);

        mtu = line->atTokNextint(&err);
        if (err < 0) {
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing mtu", rid, __FUNCTION__);
            return PDN_FAILED;
        }
        ...
        if (err < 0) {
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing addrType", rid,
                      __FUNCTION__);
            return PDN_FAILED;
        }
        ...
        if (line->atTokHasmore()) {
            ...
            if (err < 0) {
                RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing addr2", rid,
                          __FUNCTION__);
                return PDN_FAILED;
            }
        }
        ...
        if (PDN_SUCCESS != updateIpAddress(pdnType, activatedAid, addr1, addr2)) {
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] updateIpAddress failed for activatedAid%d", rid,
                      __FUNCTION__, activatedAid);
            return PDN_FAILED;
        }
        ...
        if (getTransIntfId(activatedAid) != transIntfId) {
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] transIntfId%d is different to MD assigned %d", rid,
                      __FUNCTION__, getTransIntfId(activatedAid), transIntfId);
            return PDN_FAILED;
        }
        ...
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] no matched command format (%s)", rid, __FUNCTION__,
                  cmdFormat);
        return PDN_FAILED;
    }
...
int RmcDcCommonReqHandler::updateIpAddress(int pdnType, int aid, char* addr1, char* addr2) {
    ...
    switch (pdnType) {
        ...
        default:
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] no matched pdn type (%d)", m_slot_id, __FUNCTION__,
                      pdnType);
            return PDN_FAILED;
    }
    return PDN_SUCCESS;
...
void RmcDcCommonReqHandler::requestSetupDataCall(const sp<RfxMclMessage>& msg) {
    ...
    if (pReqData[15] == NULL) {
        RFX_LOG_E(RFX_LOG_TAG,
                  "[%d][%s] pReqData[15] = NULL, suggest to check why ril version"
                  " is lower than V1.2",
                  rid, __FUNCTION__);
    } else {
        reason = atoi(pReqData[15]);
    }
    RFX_LOG_I(RFX_LOG_TAG, "[%d][%s] profileId=%d, reason=%d", rid, __FUNCTION__, profileId,
              reason);
    ...
    RFX_LOG_V(RFX_LOG_TAG, "[%d][%s] apn=%s, profile=%s, protocol=%d", rid, __FUNCTION__, pReqApn,
              pProfileType, protocol);
    ...
    if (!isDataAllowed(pReqApn)) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] isDataAllowed() return false!", rid, __FUNCTION__);
    ...
    if ((strncmp(pProfileType, "ims", 3) == 0) &&
        (getMclStatusManager(rid)->getIntValue(RFX_STATUS_KEY_IMS_PDN_STATE) ==
         IMS_OR_EIMS_PDN_DEACTIVATED)) {
        RFX_LOG_I(RFX_LOG_TAG, "[%d] %s pdn is already deactivated at MD, shouldn't be activated",
                  rid, pProfileType);
        goto error;
    } else if ((strncmp(pProfileType, "emergency", 9) == 0) &&
               (getMclStatusManager(rid)->getIntValue(RFX_STATUS_KEY_EIMS_PDN_STATE) ==
                IMS_OR_EIMS_PDN_DEACTIVATED)) {
        RFX_LOG_I(RFX_LOG_TAG, "[%d] %s pdn is already deactivated at MD, shouldn't be activated",
                  rid, pProfileType);
        goto error;
    ...
    // Activate PDN.
    if (PDN_SUCCESS !=
        activatePdn(pReqApn, pProfileType, &vAidList, isEmergency, response, cmdIndex)) {
        RFX_LOG_E(RFX_LOG_TAG,
                  "[%d][%s] activatePdn failed for apn [name=\"%s\",type=\"%s\",cmdIndex=%d]", rid,
                  __FUNCTION__, pReqApn, pProfileType, cmdIndex);
        goto error;
    }

    for (it = vAidList.begin(); it != vAidList.end(); it++) {
        int activatedAid = *it;
        // Wait for MD binding NW interface id and updating rat, interfaceId, mtu and address if
        // needed.
        if (PDN_SUCCESS != updatePdnInformation(activatedAid, IPV4V6, NETAGENT_IFST_UP)) {
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] updatePdnInformation failed for activatedAid%d", rid,
                      __FUNCTION__, activatedAid);
            response->status = m_nGprsFailureCause = PDP_FAIL_ERROR_UNSPECIFIED;
            goto error;
            ...
            RFX_LOG_E(RFX_LOG_TAG,
                      "[%d][%s] Error occurs when same apn has two different transIntfIds!", rid,
                      __FUNCTION__);
            response->status = m_nGprsFailureCause = PDP_FAIL_ERROR_UNSPECIFIED;
            goto error;
        }
    ...
    if (!isAllAidActive(&vAidList)) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] Error occurs when isAllAidActive!", rid, __FUNCTION__);
    ...
    sendEvent(RFX_MSG_EVENT_REQ_DATA_CONTEXT_IDS, RfxVoidData(), RIL_CMD_PROXY_5, m_slot_id, -1, -1,
              100e6);
    setPdnInfoProperty();
...
void RmcDcCommonReqHandler::onNwPdnAct(const sp<RfxMclMessage>& msg) {
    ...
    if (err < 0) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when converting aid to binary, err = %d", rid,
                  __FUNCTION__, err);
        AT_LINE_FREE(pLine);
        return;
    }

    // For the current design, we don't expect to use any default bearers which are established by
    // NW. However, if operator's requirements appear in the future, then the design should be
    // rethinking.
    if (confirmPdnUsage(aid, false) != CME_SUCCESS) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] confirmPdnUsage failed for aid%d", rid, __FUNCTION__, aid);
    }
...
void RmcDcCommonReqHandler::onNwPdnDeact(const sp<RfxMclMessage>& msg) {
    ...
    if (PDN_SUCCESS != updatePdnDeactInfo(aid)) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] updatePdnDeactInfo failed for aid%d", rid, __FUNCTION__,
                  aid);
    }
    ...
    for (int i = 0; i < nPdnInfoSize; i++) {
        if (isDedicateBearer(i) && getPrimaryAid(i) == aid) {
            RFX_LOG_D(RFX_LOG_TAG,
                      "[%d][%s] clean dedicate pdn aid%d info "
                      "due to default pdn aid%d deactivated",
                      rid, __FUNCTION__, getAid(i), aid);
            clearPdnInfo(i);
        }
    }
...
void RmcDcCommonReqHandler::onNwModify(const sp<RfxMclMessage>& msg) {
    ...
    if (err < 0) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing reason", rid, __FUNCTION__);
        goto error;
    }
    ...
    if (defaultAid == aid) {
        setIsDedicateBearer(aid, 0);
        RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] not to update active status [aid=%d, active=%d]", rid,
                  __FUNCTION__, aid, getPdnActiveStatus(aid));
    } else if (INVALID_AID != defaultAid) {
        RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] a dedicate bearer is modified [aid=%d]", rid, __FUNCTION__,
                  aid);
    ...
    if ((INVALID_AID != defaultAid) && (DATA_STATE_ACTIVE == getPdnActiveStatus(defaultAid))) {
        if (defaultAid == aid) {
            ...
            RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] not to send URC of dedicate bearer %d activation", rid,
                      __FUNCTION__, aid);
        }
    } else {
        RFX_LOG_E(RFX_LOG_TAG,
                  "[%d][%s] default bearer %d is not activated, "
                  "not to send URC of dedicate bearer activation",
...
void RmcDcCommonReqHandler::onNwReact(const sp<RfxMclMessage>& msg) {
    ...
    tempParam = pLine->atTokNextstr(&err);
    if (err < 0) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing defaultAid", rid, __FUNCTION__);
        goto error;
    }
    ...
    if (tempParam != NULL && strlen(tempParam) > 0) {
        ...
        if (pLine->atTokHasmore()) {
            ...
            if (err < 0) {
                RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing pdp address", rid,
                          __FUNCTION__);
                goto error;
            }
        }
        ...
        if (pLine->atTokHasmore()) {
            ...
            if (aid < 0) {
                RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs due to invalid aid=%d", rid,
                          __FUNCTION__, aid);
                goto error;
            }
            ...
            if (isDedicatedBearer) {
                RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] aid%d is a dedicate bearer", rid, __FUNCTION__,
                          aid);
            } else {
                sendDataCallListResponse(msg);
            }
        ...
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs on null/empty token string", rid,
                  __FUNCTION__);
    }
...
void RmcDcCommonReqHandler::onMePdnAct(const sp<RfxMclMessage>& msg) {
    ...
    if (err < 0) {
        RFX_LOG_E(RFX_LOG_TAG,
                  "[%d][%s] ERROR occurs when converting activatedAid to binary"
                  ", err = %d",
                  rid, __FUNCTION__, err);
        goto error;
    }
    ...
    if (pLine->atTokHasmore()) {
        reason = pLine->atTokNextint(&err);
        if (err < 0) {
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing reason for aid%d", rid,
                      __FUNCTION__, activatedAid);
            reason = NO_CAUSE;
        }
    }
    ...
    if (pLine->atTokHasmore()) {
        ...
        if (err < 0) {
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing otherAid for aid%d", rid,
                      __FUNCTION__, activatedAid);
    ...
    if (pLine->atTokHasmore()) {
        ...
        if (err < 0) {
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing wlanOffload for aid%d", rid,
                      __FUNCTION__, activatedAid);
    ...
    if (pLine->atTokHasmore()) {
        ...
        if (err < 0) {
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing sscType for aid%d", rid,
                      __FUNCTION__, activatedAid);
            sscMode = SSC_UNKNOWN;
        }
    }
    ...
    if (pLine->atTokHasmore()) {
        ...
        if (err < 0) {
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing oldAid for aid%d", rid,
                      __FUNCTION__, activatedAid);
            oldAid = INVALID_AID;
        }
    }

    RFX_LOG_I(RFX_LOG_TAG, "[%d][%s] aid%d is activated and current state is %d", rid, __FUNCTION__,
              activatedAid, getPdnActiveStatus(activatedAid));
    ...
    if (newerAid != INVALID_AID) {
        if ((getPdnActiveStatus(olderAid) != DATA_STATE_ACTIVE) ||
            ...
            if (confirmPdnUsage(newerAid, false, sscMode) != CME_SUCCESS) {
                RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] confirmPdnUsage failed for newerAid%d", rid,
                          __FUNCTION__, newerAid);
            }
        } else {
            ...
            if (PDN_SUCCESS != updatePdnInformation(newerAid, IPV4V6, NETAGENT_IFST_NONE)) {
                RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] updatePdnInformation failed for newerAid%d", rid,
                          __FUNCTION__, newerAid);
                goto error;
            }
            ...
            if (DATA_STATE_INACTIVE == getPdnActiveStatus(newerAid)) {
                RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] newerAid%d is inactive", rid, __FUNCTION__,
                          newerAid);
                goto error;
            }
...
void RmcDcCommonReqHandler::onMePdnDeact(const sp<RfxMclMessage>& msg) {
    ...
    if (err < 0) {
        RFX_LOG_E(RFX_LOG_TAG,
                  "[%d][%s] ERROR occurs when converting aid "
                  "to binary, err = %d",
                  rid, __FUNCTION__, err);
        AT_LINE_FREE(pLine);
        return;
...
void RmcDcCommonReqHandler::onPdnChange(const sp<RfxMclMessage>& msg) {
    ...
    if (err < 0) {
        if (pLine->atTokHasmore()) {
            RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] Has command data, need parsing more", rid,
                      __FUNCTION__);
    ...
    if (!needCheckmore && getTransIntfId(aid) == INVALID_TRANS_INTF_ID) {
        RFX_LOG_E(RFX_LOG_TAG,
                  "[%d][%s] ERROR occurs due to original"
                  " transIntfId removed",
                  rid, __FUNCTION__);
        goto error;
    }
    ...
    if (strncmp(DC_CHANGE, cmdFormat, strlen(DC_CHANGE)) == 0) {
        ...
        if (err < 0) {
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing event", rid, __FUNCTION__);
        }

        if (needCheckmore && event != DC_EVENT_DATA_RESET_NOTIFY) {
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] Have checked more but event is NOT 129", rid,
                      __FUNCTION__);
            goto error;
        }
        ...
        switch (event) {
            ...
            case DC_EVENT_HANDOVER_NORMAL_CHANGE:
                ...
                break;
            default:
                RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] no matched event (%d)", rid, __FUNCTION__, event);
                break;
        }
    }
...
void RmcDcCommonReqHandler::onMePdnPropertyChange(const sp<RfxMclMessage>& msg, const int aid) {
    ...
    if (PDN_SUCCESS != updatePdnAddress(aid)) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] updatePdnAddress failed for aid%d", rid, __FUNCTION__,
                  aid);
        return;
    }

    if (PDN_SUCCESS != updateDefaultBearerInfo(aid)) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] updateDefaultBearerInfo failed for aid%d", rid,
                  __FUNCTION__, aid);
        return;
    }
...
void RmcDcCommonReqHandler::onQualifiedNetworkTypeChanged(const sp<RfxMclMessage>& msg) {
    ...
    //+EIWLPL:<cmd>,<type>,<priority for setup>,<priority for cellular>,<priority for
    // wifi>,<description>
    //   <cmd> : 'notify'
    ...
    if (strncmp("notify", cmdFormat, strlen("notify")) == 0) {
        ...
        if (err < 0) {
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing type", rid, __FUNCTION__);
            return;
        }
        priorityListForSetup = pLine->atTokNextint(&err);
        if (err < 0) {
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing priorityListForSetup", rid,
                      __FUNCTION__);
            return;
        }
        ...
        if (err < 0) {
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing priorityListForWifi", rid,
                      __FUNCTION__);
            return;
        }
        descrption = pLine->atTokNextstr(&err);
        if (err < 0) {
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing descrption", rid,
                      __FUNCTION__);
            return;
        }
        ...
        for (int i = 0; i < mPdnTableSize; i++) {
            ...
            if (currentAid != INVALID_AID) {
                ...
                RFX_LOG_I(RFX_LOG_TAG, "[%d][%s] pdnInfo.profileId=%d, pdn_info.active=%d", rid,
                          __FUNCTION__, pdnInfo.profileId, pdnInfo.active);
        ...
        if (needToNotify) {
            if (matchApnType > 0) {
                ...
                int urc1Type[3] = {1, matchApnType | RIL_APN_TYPE_MMS | RIL_APN_TYPE_XCAP,
                                   ACCESS_NW_TYPE_UNKNOWN};
                int urc2Types[4] = {1, matchApnType | RIL_APN_TYPE_MMS | RIL_APN_TYPE_XCAP,
                                    ACCESS_NW_TYPE_UNKNOWN, ACCESS_NW_TYPE_UNKNOWN};
                ...
                switch (priorityListForSetup) {
                    ...
                    case PRIORITY_WIFI_PREFER:
                        ...
                        RFX_LOG_I(RFX_LOG_TAG, "[%d][%s] supportNetworkTypes=%d", rid, __FUNCTION__,
                                  supportNetworkTypes);
                        if (supportNetworkTypes == SUPPORT_TRANSPORT_MODE_CELLULAR_WIFI) {
                            ...
                            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] supportNetworkTypes unknown: %d", rid,
                                      __FUNCTION__, supportNetworkTypes);
                        }
                        break;

                    default:
                        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] priority list for setup unknown: %d", rid,
                                  __FUNCTION__, priorityListForSetup);
                        return;
                }

                if (urcWith2Types) {
                    RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] urcWith2Types: %d", rid, __FUNCTION__,
                              urc1Type[2]);
                    urcMsg = RfxMclMessage::obtainUrc(RFX_MSG_URC_QUALIFIED_NETWORK_TYPES_CHANGED,
                                                      m_slot_id, RfxIntsData(urc1Type, 3));
                    responseToTelCore(urcMsg);
                } else {
                    RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] urcWith3Types: %d", rid, __FUNCTION__,
                              urc2Types[2]);
                    urcMsg = RfxMclMessage::obtainUrc(RFX_MSG_URC_QUALIFIED_NETWORK_TYPES_CHANGED,
                ...
                RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] apn=%s not support qualified network update!", rid,
                          __FUNCTION__, type);
            }
        } else {
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] apn state is active skip to notify", rid,
                      __FUNCTION__);
        }
    } else {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] unknown <cmd> for QualifiedNetworkTypeChanged!", rid,
                  __FUNCTION__);
    }
    return;
}
...
void RmcDcCommonReqHandler::requestDeactivateDataCall(const sp<RfxMclMessage>& msg) {
    ...
    RFX_LOG_I(RFX_LOG_TAG, "[%d][%s] transIntfId=%d, reason: %d", rid, __FUNCTION__, transIntfId,
              reason);
...
int RmcDcCommonReqHandler::deactivateDataCall(const int aid, const int reason) {
    ...
    if (!validateAid(aid)) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] aid=%d is out of bound", rid, __FUNCTION__, aid);
    ...
    if (p_response->isAtResponseFail()) {
        if (p_response->getSuccess() == 0) {
            ...
            RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] CME CAUSE : %d", rid, __FUNCTION__, cause);
            return cause;
        } else {
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] CME CAUSE : CME_UNKNOWN", rid, __FUNCTION__);
            return CME_UNKNOWN;
        }
    }
    ...
    if (respAid != aid) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] aid%d is different to MD assigned aid%d", rid,
                  __FUNCTION__, aid, respAid);
        return CME_ERROR_NON_CME;
    }
...
void RmcDcCommonReqHandler::requestSyncApnTable(const sp<RfxMclMessage>& msg) {
    ...
    if (s_LastApnTable[m_slot_id] != NULL) {
        ...
        if (bSameApnTable) {
            RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] ignore same apn table sync", rid, __FUNCTION__);
            ...
            RFX_LOG_I(RFX_LOG_TAG, "[%d][%s] Sending event: RFX_MSG_EVENT_QUERY_SELF_IA", rid,
                      __FUNCTION__);
            sendEvent(RFX_MSG_EVENT_QUERY_SELF_IA, RfxVoidData(), RIL_CMD_PROXY_5, m_slot_id);
            responseToTelCore(response);
    ...
    if (p_response == NULL) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] 1. fail to get p_response!", rid, __FUNCTION__);
        goto error;
    }

    if (p_response->isAtResponseFail()) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] AT+EAPNLOCK=1 returns ERROR", rid, __FUNCTION__);
        goto error;
    }
    ...
    if (p_response == NULL) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] 2. fail to get p_response!", rid, __FUNCTION__);
        goto error;
    }

    if (p_response->isAtResponseFail()) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] AT+EAPNSET clear apn table ERROR", rid, __FUNCTION__);
        goto error;
    }
    ...
    for (int i = 0; i < nReqNum; i++) {
        ...
        cmd.clear();
        cmd.append(String8::format("AT+EAPNSET=\"%s\",%d,\"%s\",\"%s\"", apn, cmdIndex, user,
                                   password));
        p_response = atSendCommand(cmd);

        if (p_response == NULL) {
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] 3. fail to get p_response!", rid, __FUNCTION__);
            goto error;
        }

        if (p_response->isAtResponseFail()) {
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] AT+EAPNSET update user name and password ERROR", rid,
                      __FUNCTION__);
            goto error;
        }
        ...
        if (supportedTypesBitmask == RIL_APN_TYPE_ALL ||
            ...
            RFX_LOG_W(RFX_LOG_TAG, "[%d][%s] Unexpected to configure apn to support all types", rid,
                      __FUNCTION__);
        ...
        snprintf(tmpStrParam, MAX_AT_CMD_LENGTH,
                 "protocol=%s;roaming_protocol=%s;"
                 ...
                 strlen(dataProfilePtrs[i]->roamingProtocol) == 0
                         ? "IP"
                         : dataProfilePtrs[i]->roamingProtocol,
        ...
        if (p_response == NULL) {
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] 4. fail to get p_response!", rid, __FUNCTION__);
            goto error;
        }

        if (p_response->getIntermediates() != NULL) {
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] AT+EAPNSET update other parameters ERROR", rid,
                      __FUNCTION__);
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] %s", rid, __FUNCTION__,
                      p_response->getIntermediates()->getLine());
            goto error;
        }
    }
    ...
    if (RmcDcUtility::isSupportDefaultImsApnSettings()) {
        ...
        if (hasImsApn == -1) {
            ...
            cmd.append(String8::format(
                    "AT+EAPNSET=\"ims\",2,\"type=ims;protocol=IPV4V6;"
    ...
    if (p_response == NULL) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] 5. fail to get p_response!", rid, __FUNCTION__);
        goto error;
    }

    if (p_response->isAtResponseFail()) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] AT+EAPNLOCK=0 returns ERROR", rid, __FUNCTION__);
        response = RfxMclMessage::obtainResponse(msg->getId(), RIL_E_GENERIC_FAILURE, RfxVoidData(),
                                                 msg, false);
    } else {
...
error:
    ...
    if (p_response == NULL) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] 6. fail to get p_response!", rid, __FUNCTION__);
...
void RmcDcCommonReqHandler::requestSyncDataSettingsToMd(const sp<RfxMclMessage>& msg) {
    ...
    RFX_LOG_D(RFX_LOG_TAG,
              ...
              rid, __FUNCTION__, s_dataSetting[m_slot_id][MOBILE_DATA],
              s_dataSetting[m_slot_id][ROAMING_DATA], setDefaultSim,
    ...
    if (s_dataSetting[m_slot_id][DEFAULT_DATA_SIM] != SKIP_DATA_SETTINGS &&
        ...
        p_response = atSendCommand(String8::format("AT+EDALLOW=2,%d", setDefaultSim));
        ...
        if (p_response == NULL) {
            ...
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] AT+EDALLOW command returns ERROR", rid, __FUNCTION__);
            ret = RIL_E_GENERIC_FAILURE;
        }
    }
    ...
    if (RmcDcUtility::isOp20Support() == 1) {
        if (s_dataSetting[m_slot_id][DOMESTIC_ROAMING_DATA] == SKIP_DATA_SETTINGS &&
            ...
            p_response = atSendCommand(String8::format(
                    "AT+ECNCFG=%d,,,%d", s_dataSetting[m_slot_id][MOBILE_DATA], imsTestMode));
        } else if (s_dataSetting[m_slot_id][DOMESTIC_ROAMING_DATA] != SKIP_DATA_SETTINGS &&
                   s_dataSetting[m_slot_id][INTERNATIONAL_ROAMING_DATA] != SKIP_DATA_SETTINGS) {
            p_response = atSendCommand(String8::format(
                    "AT+ECNCFG=%d,,,%d,%d,%d", s_dataSetting[m_slot_id][MOBILE_DATA], imsTestMode,
                    s_dataSetting[m_slot_id][DOMESTIC_ROAMING_DATA],
                    s_dataSetting[m_slot_id][INTERNATIONAL_ROAMING_DATA]));
        } else {
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] invalid domestic/international roaming value", rid,
                      __FUNCTION__);
        ...
        if (s_dataSetting[m_slot_id][ROAMING_DATA] == SKIP_DATA_SETTINGS) {
            ...
            p_response = atSendCommand(
                    String8::format("AT+ECNCFG=%d,%d,,%d", s_dataSetting[m_slot_id][MOBILE_DATA],
...
void RmcDcCommonReqHandler::requestResendSyncDataSettingsToMd(const sp<RfxMclMessage>& msg) {
    ...
    RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] Start, dataOn:%d", rid, __FUNCTION__,
    ...
    if (RmcDcUtility::isOp20Support() == 1) {
        if (s_dataSetting_resend[m_slot_id][DOMESTIC_ROAMING_DATA] == SKIP_DATA_SETTINGS &&
            ...
            p_response = atSendCommand(String8::format("AT+ECNCFG=%d,,,%d",
                   ...
                   s_dataSetting_resend[m_slot_id][INTERNATIONAL_ROAMING_DATA] !=
                           SKIP_DATA_SETTINGS) {
            p_response = atSendCommand(String8::format(
                    "AT+ECNCFG=%d,,,%d,%d,%d", s_dataSetting_resend[m_slot_id][MOBILE_DATA],
        ...
        if (s_dataSetting_resend[m_slot_id][ROAMING_DATA] == SKIP_DATA_SETTINGS) {
            ...
            p_response = atSendCommand(String8::format(
                    "AT+ECNCFG=%d,%d,,%d", s_dataSetting_resend[m_slot_id][MOBILE_DATA],
...
RIL_DataCallFailCause RmcDcCommonReqHandler::convertFailCauseToRilStandard(int cause) {
    ...
    if (cause > SM_CAUSE_BASE && cause < SM_CAUSE_END) {
        ...
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ESM CAUSE: %X", rid, __FUNCTION__, cause);
        cause -= ESM_CAUSE_BASE;
    } else if (cause > PAM_CAUSE_BASE && cause < PAM_CAUSE_END) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] PAM CAUSE: %X", rid, __FUNCTION__, cause);
    ...
    switch (cause) {
        ...
        case 0x08:
            return PDP_FAIL_OPERATOR_BARRED;
        case 0x0E:
            return PDP_FAIL_NAS_SIGNALLING;
        case 0x18:
            return PDP_FAIL_MBMS_CAPABILITIES_INSUFFICIENT;
        case 0x19:
            return PDP_FAIL_LLC_SNDCP;
        case 0x1A:
        ...
        case 0x25:
            return PDP_FAIL_QOS_NOT_ACCEPTED;
        case 0x26:
            return PDP_FAIL_NETWORK_FAILURE;
        case 0x27:
            return PDP_FAIL_UMTS_REACTIVATION_REQ;
        case 0x28:
            return PDP_FAIL_FEATURE_NOT_SUPP;
        case 0x29:
            return PDP_FAIL_TFT_SEMANTIC_ERROR;
        case 0x2A:
            return PDP_FAIL_TFT_SYTAX_ERROR;
        case 0x2B:
            return PDP_FAIL_UNKNOWN_PDP_CONTEXT;
        case 0x2C:
            return PDP_FAIL_FILTER_SEMANTIC_ERROR;
        case 0x2D:
            return PDP_FAIL_FILTER_SYTAX_ERROR;
        case 0x2E:
            return PDP_FAIL_PDP_WITHOUT_ACTIVE_TFT;
        case 0x2F:
            return PDP_FAIL_MULTICAST_GROUP_MEMBERSHIP_TIMEOUT;
        case 0x30:
            return PDP_FAIL_BCM_VIOLATION;
        case 0x31:
            return PDP_FAIL_LAST_PDN_DISC_NOT_ALLOWED;
        case 0x32:
        ...
        case 0x35:
            return PDP_FAIL_ESM_INFO_NOT_RECEIVED;
        case 0x36:
            return PDP_FAIL_PDN_CONN_DOES_NOT_EXIST;
        case 0x37:
            return PDP_FAIL_MULTI_CONN_TO_SAME_PDN_NOT_ALLOWED;
        case 0x38:
            return PDP_FAIL_COLLISION_WITH_NW_INITIATED_REQUEST;
        case 0x3B:
            return PDP_FAIL_ESM_UNSUPPORTED_QCI_VALUE;
        case 0x3C:
            return PDP_FAIL_BEARER_HANDLING_NOT_SUPPORT;
        case 0x41:
            return PDP_FAIL_MAX_ACTIVE_PDP_CONTEXT_REACHED;
        case 0x42:
            return PDP_FAIL_UNSUPPORTED_APN_IN_CURRENT_PLMN;
        case 0x51:
            return PDP_FAIL_INVALID_TRANSACTION_ID;
        case 0x5F:
            return PDP_FAIL_MESSAGE_INCORRECT_SEMANTIC;
        case 0x60:
            return PDP_FAIL_INVALID_MANDATORY_INFO;
        case 0x61:
            return PDP_FAIL_MESSAGE_TYPE_UNSUPPORTED;
        case 0x62:
            return PDP_FAIL_MSG_TYPE_NONCOMPATIBLE_STATE;
        case 0x63:
            return PDP_FAIL_UNKNOWN_INFO_ELEMENT;
        case 0x64:
            return PDP_FAIL_CONDITIONAL_IE_ERROR;
        case 0x65:
            return PDP_FAIL_MSG_AND_PROTOCOL_STATE_UNCOMPATIBLE;
        case 0x6F:
            return PDP_FAIL_PROTOCOL_ERRORS;
        case 0x70:
            return PDP_FAIL_APN_TYPE_CONFLICT;
        case 0x71:
            return PDP_FAIL_INVALID_PCSCF_ADDR;
        case 0x72:
            return PDP_FAIL_INTERNAL_CALL_PREEMPT_BY_HIGH_PRIO_APN;
        case 0x73:
            return PDP_FAIL_EMM_ACCESS_BARRED;
        case 0x74:
            return PDP_FAIL_EMERGENCY_IFACE_ONLY;
        case 0x75:
            return PDP_FAIL_IFACE_MISMATCH;
        case 0x76:
            return PDP_FAIL_COMPANION_IFACE_IN_USE;
        case 0x77:
            return PDP_FAIL_IP_ADDRESS_MISMATCH;
        case 0x78:
            return PDP_FAIL_IFACE_AND_POL_FAMILY_MISMATCH;
        case 0x79:
            return PDP_FAIL_EMM_ACCESS_BARRED_INFINITE_RETRY;
        case 0x7A:
            return PDP_FAIL_AUTH_FAILURE_ON_EMERGENCY_CALL;
        case 0x0E0F:
            return PDP_FAIL_LOCAL_REJECT_ACT_REQ_DUE_TO_REACH_RETRY_COUNTER;
        case 0x0F47:
            return PDP_FAIL_TCM_ESM_TIMER_TIMEOUT;
        case 0x1402:
            return PDP_FAIL_PAM_ATT_PDN_ACCESS_REJECT_IMS_PDN_BLOCK_TEMP;
        case 0x1671:
            return PDP_FAIL_DATA_NOT_ALLOW;
        case -1:
...
int RmcDcCommonReqHandler::convertDeactCauseToMdStandard(int reason) {
    switch (reason) {
        case DEACT_REASON_AOSP_SHUTDOWN:
            return ATCMD_DEACT_CAUSE_EGACT_CAUSE_AOSP_SHUTDOWN;
        case DEACT_REASON_AOSP_HANDOVER:
            return ATCMD_DEACT_CAUSE_EGACT_CAUSE_AOSP_HANDOVER;
        case DEACT_REASON_APN_CHANGED:
            return ATCMD_DEACT_CAUSE_EGACT_CAUSE_APN_CHANGED;
        default:
            return ATCMD_DEACT_CAUSE_EGACT_CAUSE_DEACT_NORMAL;
    }
}
...
int RmcDcCommonReqHandler::isAllAidActive(Vector<int>* vAidList) {
    ...
    for (Vector<int>::iterator it = vAidList->begin(); it != vAidList->end(); it++) {
        int aid = *it;
        RFX_LOG_V(RFX_LOG_TAG, "[%d][%s] pdn_info[%d], state: %d", rid, __FUNCTION__, aid,
                  getPdnActiveStatus(aid));
        if (DATA_STATE_INACTIVE == getPdnActiveStatus(aid)) {
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] aid%d is inactive", rid, __FUNCTION__, aid);
            return 0;
        }
...
void RmcDcCommonReqHandler::requestOrSendDataCallList(const sp<RfxMclMessage>& msg) {
    ...
    for (int i = 0; i < nPdnInfoSize; i++) {
        if ((getPdnActiveStatus(i) == DATA_STATE_ACTIVE) &&
            ...
            (std::find(vTransIntfIdList.begin(), vTransIntfIdList.end(), getTransIntfId(i)) ==
             vTransIntfIdList.end())) {
            vTransIntfIdList.push_back(getTransIntfId(i));
        }
    }

    if (vTransIntfIdList.size() > 0) {
        RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] active trans-interface number is %zu", rid, __FUNCTION__,
                  vTransIntfIdList.size());

        responsesOfActive = (MTK_RIL_Data_Call_Response_v11*)calloc(
                1, vTransIntfIdList.size() * sizeof(MTK_RIL_Data_Call_Response_v11));

        if (responsesOfActive == NULL) {
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] fail to allocate responsesOfActive!", rid,
                      __FUNCTION__);
            RFX_ASSERT(false);
        }
        ...
        if (msg->getType() == RFX_MESSAGE_TYPE::REQUEST) {
            ...
            responseMsg = RfxMclMessage::obtainUrc(
                    RFX_MSG_URC_DATA_CALL_LIST_CHANGED, rid,
...
error:
    if (msg->getType() == RFX_MESSAGE_TYPE::REQUEST) {
        ...
        responseMsg =
                RfxMclMessage::obtainUrc(RFX_MSG_URC_DATA_CALL_LIST_CHANGED, rid, RfxVoidData());
    }
    responseToTelCore(responseMsg);
}
...
void RmcDcCommonReqHandler::requestOrSendDataCallList(const sp<RfxMclMessage>& msg,
    ...
    for (it = vAidList->begin(); it != vAidList->end(); it++) {
        int aid = *it;
        if (PDN_SUCCESS != updatePdnAddress(aid)) {
            // Not to check error since we expect to query all PDN connections even if error
            // occurred
            RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] updatePdnAddress failed for aid%d", rid, __FUNCTION__,
                      aid);
        }
...
void RmcDcCommonReqHandler::sendDataCallListResponse(const sp<RfxMclMessage>& msg,
                                                     int deactivatedAid, int sscMode) {
    int rid = m_slot_id;
    MTK_RIL_Data_Call_Response_v11* responsesOfActive = NULL;
    Vector<int> vTransIntfIdList;
    Vector<int>::iterator it;
    ...
    if (vTransIntfIdList.size() > 0) {
        ...
        for (it = vTransIntfIdList.begin(); it != vTransIntfIdList.end(); it++) {
            ...
            if (deactivatedAid != INVALID_AID && notifyDeactReasonIfNeeded(deactivatedAid) &&
            ...
            if (lifetime != INVALID_LIFE_TIME && secondAid != INVALID_AID &&
                getTransIntfId(secondAid) == transIntfId) {
                RFX_LOG_I(RFX_LOG_TAG,
                          "[%d][%s] ssc3 pdu info [type/aid/transIntfId/lifetime(s)],"
                          " first pdu [%s/%d/%d/%d], second pdu [%s/%d/%d]",
                          rid, __FUNCTION__, RmcDcUtility::getProfileType(getProfileId(firstAid)),
                          firstAid, getTransIntfId(firstAid), lifetime,
...
void RmcDcCommonReqHandler::updateActiveStatus() {
    ...
    RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] PDP capability [%d], PdnTableSize [%d]", rid, __FUNCTION__,
              maxAidSupported, getPdnTableSize());
    ...
    for (p_cur = p_response->getIntermediates(); p_cur != NULL; p_cur = p_cur->getNext()) {
        ...
        if (active > 0) {
            ...
            if (getTransIntfId(responseAid) == INVALID_TRANS_INTF_ID) {
                ...
                RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] aid%d is linkdown", rid, __FUNCTION__,
                          responseAid);
            } else {
    ...
    if (activeAidString.str().length() > 0) {
        RFX_LOG_I(RFX_LOG_TAG, "[%d][%s] active list [aid:primaryAid] = [%s]", rid, __FUNCTION__,
                  activeAidString.str().c_str());
    }
    if (inactiveAidString.str().length() > 0) {
        RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] inactive list [aid:primaryAid] = [%s]", rid, __FUNCTION__,
                  inactiveAidString.str().c_str());
    }
}
...
int RmcDcCommonReqHandler::updateDefaultBearerInfo() {
    ...
    //+CGCONTRDP response a list of [+CGCONTRDP: <aid>,<bearer_id>,<apn>[,<local_addr and
    // subnet_mask>
    ...
    if (p_response->isAtResponseFail()) {
        if (p_response->getSuccess() > 0) {
            ...
            int cause = p_response->atGetCmeError();
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] AT+CGCONTRDP returns cme cause : %d", rid,
                      __FUNCTION__, cause);
        }
        return PDN_FAILED;
    }
    ...
    for (p_cur = p_response->getIntermediates(); p_cur != NULL; p_cur = p_cur->getNext()) {
        ...
        if (err < 0) {
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing bearer id", rid,
                      __FUNCTION__);
            goto error;
        }
        ...
        if (err < 0) {
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing apn", rid, __FUNCTION__);
            goto error;
        }
        ...
        if (p_cur->atTokHasmore()) {
            ...
            if (err < 0) {
                RFX_LOG_E(RFX_LOG_TAG,
                          "[%d][%s] ERROR occurs when parsing local addr and subnet mask", rid,
                          __FUNCTION__);
                goto error;
            }
            out = NULL;
        ...
        if (p_cur->atTokHasmore()) {
            ...
            if (err < 0) {
                RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing gw addr", rid,
                          __FUNCTION__);
                goto error;
            }
            out = NULL;
        ...
        if (p_cur->atTokHasmore()) {
            ...
            if (err < 0) {
                RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing DNS prim addr", rid,
                          __FUNCTION__);
                goto error;
            }
            ...
            if (out != NULL && strlen(out) > 0 && strcmp(NULL_IPV4_ADDRESS, out) != 0) {
                ...
                if (RmcDcUtility::getAddressType(out) == IPV6) {
                    ...
                    RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] DNS%d V6 for aid%d is %s", rid, __FUNCTION__,
                              v6Count[responseAid], responseAid, getIpv6Dns(responseAid, index));
                } else if (RmcDcUtility::getAddressType(out) == IPV4) {
                    index = v4Count[responseAid]++;
                    updateIpv4Dns(responseAid, index, out);
                    RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] DNS%d V4 for aid%d is %s", rid, __FUNCTION__,
                              v4Count[responseAid], responseAid, getIpv4Dns(responseAid, index));
                } else {
                    RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] DNS for aid%d is NOT V4 or V6 type", rid,
                              __FUNCTION__, responseAid);
                    goto error;
                }
            }
            out = NULL;
        ...
        if (p_cur->atTokHasmore()) {
            ...
            if (err < 0) {
                RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing DNS sec addr", rid,
                          __FUNCTION__);
                goto error;
            }
        ...
        if (p_cur->atTokHasmore()) {
            ...
            if (err < 0) {
                RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing P-CSCF prim addr", rid,
                          __FUNCTION__);
                goto error;
            }
            if (out != NULL && strlen(out) > 0 && strcmp(NULL_IPV4_ADDRESS, out) != 0) {
                ...
                if (RmcDcUtility::getAddressType(out) == IPV6) {
                    ...
                    RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] P-CSCF is %s", rid, __FUNCTION__, pcscfV6);
                } else if (RmcDcUtility::getAddressType(out) == IPV4) {
                    updatePcscfAddress(responseAid, pcscfCount[responseAid]++, out);
                    RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] P-CSCF is %s", rid, __FUNCTION__, out);
                } else {
                    RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] P-CSCF for aid%d is NOT V4 or V6 type", rid,
                              __FUNCTION__, responseAid);
                    goto error;
                }
            }
        ...
        if (p_cur->atTokHasmore()) {
            ...
            if (err < 0) {
                RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing P-CSCF sec addr", rid,
                          __FUNCTION__);
                goto error;
            }
            if (out != NULL && strlen(out) > 0 && strcmp(NULL_IPV4_ADDRESS, out) != 0) {
        ...
        if (p_cur->atTokHasmore()) {
            ...
            if (err < 0) {
                ...
                RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] signalingFlag = %d for aid%d", rid, __FUNCTION__,
                          getSignalingFlag(responseAid), responseAid);
            }
        }
        ...
        if (p_cur->atTokHasmore()) {
            ...
            if (err < 0) {
                ...
                RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] LIPA_indication = %d for aid%d", rid, __FUNCTION__,
                          lipaIndication, responseAid);
            }
        }
        ...
        if (p_cur->atTokHasmore()) {
            ...
            if (err < 0) {
                ...
                RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] mtu = %d for aid%d", rid, __FUNCTION__,
                          getMtu(responseAid), responseAid);
            }
        }

        if (!cmd.isEmpty()) {
            RFX_LOG_D(RFX_LOG_TAG, "[%d][%s]%s for aid%d", rid, __FUNCTION__, cmd.string(),
                      responseAid);
        }
    }
...
int RmcDcCommonReqHandler::updateDefaultBearerInfo(int aid) {
    ...
    if (p_response->isAtResponseFail()) {
        if (p_response->getSuccess() > 0) {
            ...
            int cause = p_response->atGetCmeError();
            RFX_LOG_E(RFX_LOG_TAG,
                      "[%d][%s] AT+CGCONTRDP for activatedAid%d returns cme cause : %d", rid,
                      __FUNCTION__, aid, cause);
        }
        return PDN_FAILED;
    }
        ...
        if (p_cur->atTokHasmore()) {
            ...
            if (out != NULL && strlen(out) > 0 && strcmp(NULL_IPV4_ADDRESS, out) != 0) {
                ...
                if (RmcDcUtility::getAddressType(out) == IPV6) {
                    ...
                    RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] DNS%d V6 for aid%d is %s", rid, __FUNCTION__,
                              v6Count, aid, getIpv6Dns(aid, index));
                } else if (RmcDcUtility::getAddressType(out) == IPV4) {
                    index = v4Count++;
                    updateIpv4Dns(aid, index, out);
                    RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] DNS%d V4 for aid%d is %s", rid, __FUNCTION__,
                              v4Count, aid, getIpv4Dns(aid, index));
                } else {
                    RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] DNS for aid%d is NOT V4 or V6 type", rid,
                              __FUNCTION__, aid);
                    return PDN_FAILED;
    ...
    for (int i = 0; i < pdnInfoSize; i++) {
        if (DATA_STATE_INACTIVE != getPdnActiveStatus(i)) {
            if (PDN_SUCCESS != updatePdnAddress(i)) {
                RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] Fail to updatePdnAddress for aid%d", rid,
                          __FUNCTION__, i);
            }
        }
    }
...
int RmcDcCommonReqHandler::updatePdnAddress(int aid) {
    ...
    if (isDedicateBearer(aid)) {
        RFX_LOG_I(RFX_LOG_TAG, "[%d][%s] NO need to update dedicate bearer address [aid%d]", rid,
                  __FUNCTION__, aid);
        return PDN_SUCCESS;
    }
    ...
    /* Use AT+EPDN=<aid>,"addr" to query the ip address assigned to this PDP context indicated via
     * this aid */
    ...
    if (p_response == NULL) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] fail to get p_response for aid%d", rid, __FUNCTION__, aid);
        return PDN_FAILED;
    }
    ...
    if (p_response->isAtResponseFail()) {
        if (p_response->getSuccess() > 0) {
            RFX_LOG_E(RFX_LOG_TAG,
                      "[%d][%s] weird case, can not get fail cause for aid% "
                      "due to final response indicates success",
                      rid, __FUNCTION__, aid);
        }
        return PDN_FAILED;
    }
    ...
    for (p_cur = p_response->getIntermediates(); p_cur != NULL; p_cur = p_cur->getNext()) {
        ...
        responseAid = p_cur->atTokNextint(&err);
        if (err < 0) {
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing aid%d for %d's row", rid,
                      __FUNCTION__, aid, row);
            return PDN_FAILED;
        }

        if (responseAid != aid) {
            RFX_LOG_E(RFX_LOG_TAG,
                      "[%d][%s] ERROR aid%d is different to MD assigned aid%d for %d's row", rid,
                      __FUNCTION__, aid, responseAid, row);
            return PDN_FAILED;
        }
        ...
        if (err < 0) {
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR parsing cmd with aid%d for %d's row", rid,
                      __FUNCTION__, aid, row);
            return PDN_FAILED;
        }
        ...
        if (strncmp(DC_ERROR, cmdFormat, strlen(DC_ERROR)) == 0) {
            ...
            if (p_cur->atTokHasmore()) {
                ...
                if (err < 0) {
                    RFX_LOG_E(RFX_LOG_TAG,
                              "[%d][%s] ERROR occurs when parsing err cause for aid%d for %d's row",
                              rid, __FUNCTION__, aid, row);
                } else {
                    RFX_LOG_E(
                            RFX_LOG_TAG,
                            "[%d][%s] AT+EPDN(addr) for aid%d returns err cause : %d for %d's row",
            ...
            if (p_cur->atTokHasmore()) {
                ...
                if (err < 0) {
                    RFX_LOG_E(RFX_LOG_TAG,
                              "[%d][%s] ERROR parsing address type with aid%d for %d's row", rid,
                              __FUNCTION__, aid, row);
                    return PDN_FAILED;
                }
            }
            ...
            if (p_cur->atTokHasmore()) {
                ...
                if (addrType == ADDRESS_IPV4) {
                    ...
                    RFX_LOG_E(RFX_LOG_TAG,
                              "[%d][%s] no matched address type (%s) with aid%d for %d's row", rid,
                              __FUNCTION__, RmcDcUtility::addrTypeToString(addrType), aid, row);
                }
            }
        } else {
            RFX_LOG_E(RFX_LOG_TAG,
                      "[%d][%s] no matched command format (%s) with aid%d for %d's row", rid,
                      __FUNCTION__, cmdFormat, aid, row);
            return PDN_FAILED;
        }
        row++;
    }

    RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] Response IP address [aid%d, IPv4Addr=%s, IPv6Addr=%s]", rid,
              __FUNCTION__, aid, getIpv4Address(aid), ipv6AddressToString(aid).string());

    return PDN_SUCCESS;
}

void RmcDcCommonReqHandler::initDataCallResponse(MTK_RIL_Data_Call_Response_v11* responses,
                                                 int length) {
...
void RmcDcCommonReqHandler::createDataResponse(int transIntfId, int protocol,
    ...
    for (int i = 0; i < nPdnInfoSize; i++) {
        ...
        if ((interfaceId != INVALID_INTERFACE_ID) && (pdn_info.transIntfId == transIntfId) &&
            !pdn_info.isDedicateBearer && (pdn_info.active == DATA_STATE_ACTIVE)) {
            ...
            asprintf(&response->ifname, "%s%d", NetAgentService::getCcmniInterfaceName(rid),
                     interfaceId);
            ...
            for (int j = 0; j < MAX_NUM_IPV6_ADDRESS_NUMBER; j++) {
                if (strlen(pdnInfo->addressV6[j]) > 0 && (protocol == IPV6 || protocol == IPV4V6)) {
                    ...
                    if (j != 0 || addressV4Length != 0) ++addressV6Length;  // add one space
                }
            }
            ...
            for (int j = 0; j < MAX_NUM_DNS_ADDRESS_NUMBER; j++) {
                ...
                if ((strlen(pdnInfo->dnsV4[j]) > 0) && (addressV4Length > 0)) {
                    ...
                    if (j != 0 || v6DnsLength != 0) ++v4DnsLength;  // add one space
                }
                if ((strlen(pdnInfo->dnsV6[j]) > 0) && (addressV6Length > 0)) {
                    ...
                    if (j != 0 || v4DnsLength != 0) ++v6DnsLength;  // add one space
                }
            }
            ...
            for (int j = 0; j < MAX_PCSCF_NUMBER; j++) {
                if (strlen(pdnInfo->pcscf[j]) > 0) {
                    ...
                    if (j != 0) ++pcscfLength;  // add one space
                }
            }
            ...
            RFX_LOG_E(RFX_LOG_TAG,
                      "[%d][%s] not need to create pdn_info[%d] for transIntfId=%d"
...
String8 RmcDcCommonReqHandler::responsesToString(MTK_RIL_Data_Call_Response_v11* responses,
                                                 int num) {
    int rid = m_slot_id;
    String8 cmd("");
    ...
    if (num == 0 || responses == NULL) {
        RFX_LOG_E(RFX_LOG_TAG,
                  "[%d][%s] the number of data call responses is %d "
    ...
    for (int i = 0; i < num; i++) {
        cmd.append(String8::format(
                " {status=%d, suggestedRetryTime=%d, transIntfId=%d, active=%d,"
                " type=%s, ifname=%s, addresses=%s, dnses=%s, gateways=%s, pcscf=%s, mtu=%d, "
                "rat=%d}",
                (&responses[i])->status, (&responses[i])->suggestedRetryTime, (&responses[i])->cid,
                (&responses[i])->active, (&responses[i])->type, (&responses[i])->ifname,
                (&responses[i])->addresses, (&responses[i])->dnses, (&responses[i])->gateways,
                (&responses[i])->pcscf, (&responses[i])->mtu, (&responses[i])->rat));
    }
    return cmd;
}
...
void RmcDcCommonReqHandler::freeDataResponse(MTK_RIL_Data_Call_Response_v11* response) {
    if (response->ifname != NULL) {
        RFX_LOG_D(RFX_LOG_TAG,
                  "[%d][%s] type=%s, ifname=%s, addresses=%s, dnses=%s, gateways=%s, pcscf=%s",
                  m_slot_id, __FUNCTION__, response->type, response->ifname, response->addresses,
...
int RmcDcCommonReqHandler::convertIpv6Address(char* output, char* input, int isLinkLocal) {
    ...
    while (p) {
        ...
        p = strtok(NULL, ".");
        if (p == NULL) {
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] The format of IP address is illegal", rid,
                      __FUNCTION__);
            return -1;
        }
        memset(tmpOutput, 0, MAX_IPV6_ADDRESS_LENGTH);
...
bool RmcDcCommonReqHandler::isIpv6Global(char* ipv6Addr) {
    ...
    if (address != NULL) {
        ...
        if (ret <= 0) {
            RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] inet_pton ret: %d -> %s", rid, __FUNCTION__, ret,
                      cmd.string());
            return false;
        }
        ...
        if (IN6_IS_ADDR_MULTICAST(&sa.sin6_addr)) {
            ...
            if (IN6_IS_ADDR_MC_GLOBAL(&sa.sin6_addr)) {
                ...
                RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] not global -> %s", rid, __FUNCTION__,
                          cmd.string());
            }
        } else {
            if (IN6_IS_ADDR_LINKLOCAL(&sa.sin6_addr)) {
                RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] link-local -> %s", rid, __FUNCTION__,
                          cmd.string());
            } else if (IN6_IS_ADDR_SITELOCAL(&sa.sin6_addr)) {
                RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] site-local -> %s", rid, __FUNCTION__,
                          cmd.string());
                ...
                RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] unspecified -> %s", rid, __FUNCTION__,
                          cmd.string());
            } else if (_IN6_IS_ULA(&sa.sin6_addr)) {
                RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] take uni-local as global -> %s", rid, __FUNCTION__,
                          cmd.string());
                return true;
            } else {
                RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] global -> %s", rid, __FUNCTION__, cmd.string());
...
void RmcDcCommonReqHandler::getPco(int apnidx, const char* apn, const char* option,
                                   Vector<String8>& vContent) {
...
void RmcDcCommonReqHandler::getPco(PDN_INITIATOR who, int idx, const char* apn, const char* option,
                                   Vector<String8>& vContent) {
    String8 cmd("");
    sp<RfxAtResponse> p_response;
    RfxAtLine* p_cur = NULL;
    ...
    if (option == NULL) {
        ...
        if (who == PDN_AP_INITIATED) {
            ...
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] requested PDN format error!", rid, __FUNCTION__);
            return;
        }
    } else {
    ...
    for (p_cur = p_response->getIntermediates(); p_cur != NULL; p_cur = p_cur->getNext()) {
        ...
        if (err < 0) {
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] fail to get idx", rid, __FUNCTION__);
            return;
        }
        ...
        if (err < 0) {
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] fail to get content", rid, __FUNCTION__);
            return;
        }
...
bool RmcDcCommonReqHandler::isDataAllowed(const char* pReqApn) {
    ...
    RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] nWfcState = %d, nPsState = %d, roaming data = %d", m_slot_id,
              __FUNCTION__, nWfcState, nPsState, s_dataSetting[m_slot_id][ROAMING_DATA]);

    if (1 == nWfcState) {
        for (int i = 0; i < s_nLastReqNum[m_slot_id]; i++) {
            if (0 == strcmp(pReqApn, s_LastApnTable[m_slot_id][i].apn)) {
                bearer_bitmask = s_LastApnTable[m_slot_id][i].bearerBitmask;
                RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] bearer_bitmask = %d", m_slot_id, __FUNCTION__,
                          bearer_bitmask);
                if (0 == bearer_bitmask ||
...
void RmcDcCommonReqHandler::requestQueryPco(int idx, int ia, const char* apn, const char* iptype) {
    ...
    if (!RmcDcUtility::isOp12MccMnc(simMccMnc)) {
        RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] query PCO return: [%s] not OP12 SIM", m_slot_id,
                  __FUNCTION__, simMccMnc);
        return;
    }
    if (getPdnActiveStatus(idx) == DATA_STATE_INACTIVE) {
        RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] no need to check inactive aid %d", m_slot_id, __FUNCTION__,
                  idx);
        return;
    }
    ...
    if (contents.size() == 0) {
        RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] query PCO return: no contents", m_slot_id, __FUNCTION__);
        return;
    }
    ...
    if (delimiterA < 0) {
        RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] query PCO return: content's format missing ','", m_slot_id,
                  __FUNCTION__);
        return;
    }
    ...
    RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] idx:%d has PCO, content: %s, mccmnc: %s, val: %s,", m_slot_id,
              __FUNCTION__, idx, pcoContent.c_str(), pcoMccMnc.c_str(), pcoValue.c_str());
    ...
    if (ia) {
        ...
        urcMsg = RfxMclMessage::obtainUrc(RFX_MSG_UNSOL_PCO_DATA_AFTER_ATTACHED, m_slot_id,
                                          RfxPcoIaData(&pcoDataIa, sizeof(pcoDataIa)));
        ...
        for (int i = 0; i < getPdnTableSize(); i++) {
            if (tid == getTransIntfId(i)) {
                ...
                protoBitMask |= (ipv4.size() > 0 && ipv6.size() == 0)   ? 1 << IPV4
                                ...
                                : (ipv4.size() > 0 && ipv6.size() > 0)  ? 1 << IPV4V6
                                                                        : 1 << IPV4;
            }
        }
        std::string proto;
        ...
        RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] PCO cid %d, proto %s, pcoid %d, contents %s", m_slot_id,
                  __FUNCTION__, pcoData.cid, pcoData.bearer_proto, pcoData.pco_id,
                  pcoData.contents);
    }
...
int RmcDcCommonReqHandler::confirmPdnUsage(const int aid, const bool bUsed, int sscMode) {
    ...
    // <state>: 2 - NOT to use this PDN and NOT to receive "+CGEV" URCs for the corresponding
    // bearers; <state>: 3 - use this PDN and to receive "+CGEV" URCs for the corresponding bearers;
    // <state>: 4 - NOT to use this SSC mode3 PDU and NOT to receive "+CGEV" URCs for the
    // corresponding bearers; <state>: 5 - use this SSC mode3 PDU and to receive "+CGEV" URCs for
...
int RmcDcCommonReqHandler::updatePdnDeactInfo(const int aid) {
    ...
    //    +EPDN:<aid>,"err",<err>
    p_response =
            atSendCommandSingleline(String8::format("AT+EPDN=%d,\"deact_info\"", aid), "+EPDN:");
    ...
    if (strncmp(DC_ERROR, cmdFormat, strlen(DC_ERROR)) == 0) {
        if (pLine->atTokHasmore()) {
            ...
            if (err < 0) {
                ...
                RFX_LOG_E(RFX_LOG_TAG,
                          "[%d][%s] AT+EPDN(deact_info) for aid%d returns err cause : %d", rid,
                          __FUNCTION__, aid, cause);
            }
        }
        return PDN_FAILED;
        ...
        if (reason == NO_REASON) {
            RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] no reason for pdn deact aid%d", rid, __FUNCTION__,
                      aid);
        } else {
...
bool RmcDcCommonReqHandler::isFallbackPdn(const int aid) {
    ...
    if (getPdnActiveStatus(aid) != DATA_STATE_ACTIVE) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] no need to check non-active aid %d", rid, __FUNCTION__,
                  aid);
        return false;
    }
...
bool RmcDcCommonReqHandler::notifyDeactReasonIfNeeded(const int deactivatedAid) {
    ...
    if (!validateAid(deactivatedAid)) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] deactivatedAid=%d is out of bound", rid, __FUNCTION__,
                  deactivatedAid);
        return false;
    }

    // Update the reason of why pdn was deactivated to FWK for checking if needs to do pdn retrying
    // except that there has no existed reason or another of fallback pdn of the same apn is still
    // alive.
...
int RmcDcCommonReqHandler::getSupportNetworkTypes(int apnTypeId) {
    ...
    RFX_LOG_I(RFX_LOG_TAG, "[%d][%s] bSupportWifiBearer=%d bSupportCellularBearer=%d", m_slot_id,
              __FUNCTION__, bSupportWifiBearer, bSupportCellularBearer);
...
bool RmcDcCommonReqHandler::isSupportWifiBearer(int bearerBitmask) {
    RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] bearerBitmask = %d", m_slot_id, __FUNCTION__, bearerBitmask);
...
int RmcDcCommonReqHandler::getCmdIndexFromApnTable(const int slot_id, ApnTableReq* pApnTableReq) {
    ...
    RFX_LOG_V(
            RFX_LOG_TAG,
            "[%d][%s] pApnTableReq apn=%s, user=%s, pwd=%s, apnTypeId=%d, protocol=%s, authtype=%d",
            slot_id, __FUNCTION__, pApnTableReq->apn, pApnTableReq->username,
            pApnTableReq->password, pApnTableReq->apnTypeId, pApnTableReq->protocol,
            pApnTableReq->authtype);
    ...
    for (int i = 0; i < s_nLastReqNum[slot_id]; i++) {
        RFX_LOG_V(RFX_LOG_TAG,
                  "[%d][%s] s_nLastReqNum[%d][%d] apn=%s, user=%s, pwd=%s, "
                  "supportedTypesBitmask=%d, protocol=%s, authtype=%d",
                  slot_id, __FUNCTION__, slot_id, i, s_LastApnTable[slot_id][i].apn,
                  s_LastApnTable[slot_id][i].user, s_LastApnTable[slot_id][i].password,
                  s_LastApnTable[slot_id][i].supportedTypesBitmask,
                  s_LastApnTable[slot_id][i].protocol, s_LastApnTable[slot_id][i].authType);

        if (pApnTableReq->apn &&
            0 != strcasecmp(pApnTableReq->apn, s_LastApnTable[slot_id][i].apn)) {
            RFX_LOG_V(RFX_LOG_TAG, "[%d][%s] apn is not the same: %s", slot_id, __FUNCTION__,
                      s_LastApnTable[slot_id][i].apn);
            continue;
        }

        if (pApnTableReq->username &&
            0 != strcasecmp(pApnTableReq->username, s_LastApnTable[slot_id][i].user)) {
            RFX_LOG_V(RFX_LOG_TAG, "[%d][%s] user is not the same: %s", slot_id, __FUNCTION__,
                      s_LastApnTable[slot_id][i].user);
            continue;
        }

        if (pApnTableReq->password &&
            0 != strcasecmp(pApnTableReq->password, s_LastApnTable[slot_id][i].password)) {
            RFX_LOG_V(RFX_LOG_TAG, "[%d][%s] pwd is not the same: %s", slot_id, __FUNCTION__,
                      s_LastApnTable[slot_id][i].password);
            continue;
        }

        if (pApnTableReq->apnTypeId != APN_TYPE_INVALID) {
            if ((s_LastApnTable[slot_id][i].supportedTypesBitmask & pApnTableReq->apnTypeId) !=
        ...
        if (pApnTableReq->protocol &&
            0 != strcasecmp(pApnTableReq->protocol, s_LastApnTable[slot_id][i].protocol)) {
            RFX_LOG_V(RFX_LOG_TAG, "[%d][%s] protocol is not the same: %s", slot_id, __FUNCTION__,
                      s_LastApnTable[slot_id][i].protocol);
            continue;
        }
        if (pApnTableReq->authtype != s_LastApnTable[slot_id][i].authType) {
            RFX_LOG_V(RFX_LOG_TAG, "[%d][%s] authtype is not the same: %d", slot_id, __FUNCTION__,
                      s_LastApnTable[slot_id][i].authType);
            continue;
        }
...
void RmcDcCommonReqHandler::requestSetPreferredDataModem(const sp<RfxMclMessage>& msg) {
    ...
    RFX_LOG_I(RFX_LOG_TAG, "[%d][%s] requestSetPreferredDataModem, preferredMd=%d", rid,
              __FUNCTION__, preferredMd);
        ...
        if (match) {
            ...
            for (int i = 0; i < count; i++) {
                if (compareApn(dataProfilePtrs[i], dataProfilePtrs[index + count])) {
                    RFX_LOG_I(RFX_LOG_TAG, "[%d][%s] find same apn index = %d", m_slot_id,
                              __FUNCTION__, i);
            ...
            if (insert) {
                index++;
                RFX_LOG_I(RFX_LOG_TAG, "[%d][%s] insert count = %d", m_slot_id, __FUNCTION__,
                          index);
...
void RmcDcCommonReqHandler::setApnParameter(RIL_MtkDataProfileInfo* profile, std::string name,
                                            std::string value) {
...
bool RmcDcCommonReqHandler::getMccmnc(char* mccmnc) {
    ...
    if (m_slot_id == 0) {
        ...
        snprintf(key, MAX_LENGTH_PROPERTY_NAME - 1, "%s%s%d", PROPERTY_MCC_MNC_CDMA, ".",
                 m_slot_id);
    }
    rfx_property_get(key, mccmnc, "0");
    if (strlen(mccmnc) > 3) {
...
void RmcDcCommonReqHandler::updateMdDefaultEmergencyApn(RIL_MtkDataProfileInfo** dataProfilePtrs,
    ...
    RFX_LOG_I(RFX_LOG_TAG, "[%d][%s] emergency apn index %d, %d", m_slot_id, __FUNCTION__,
              nonEmptyApn, emptyApn);
    ...
    if (nonEmptyApn != -1 && emptyApn == -1 &&
        ...
        memset(strParam, 0, MAX_AT_CMD_LENGTH);
        snprintf(strParam, MAX_AT_CMD_LENGTH,
                 "type=emergency;protocol=%s;"
                 ...
                 dataProfilePtrs[nonEmptyApn]->roamingProtocol, ALL_BEARER_EXCLUDE_IWLAN_BITMASK);
...
void RmcDcCommonReqHandler::requestGetDataContextIds(const sp<RfxMclMessage>& msg) {
    ...
    RFX_LOG_I(RFX_LOG_TAG, "[%d][%s] the number of context ids = %d", m_slot_id, __FUNCTION__,
              (int)vTransIntfIdList.size());

    sendEvent(RFX_MSG_EVENT_RSP_DATA_CONTEXT_IDS,
              RfxIntsData((int*)vTransIntfIdList.array(), vTransIntfIdList.size()), RIL_CMD_PROXY_3,
              m_slot_id);
}
...
void RmcDcCommonReqHandler::requestSendQualifiedNetworkTypesChanged(const sp<RfxMclMessage>& msg) {
    ...
    data[0] =
            1;  // this element is 'mode' to indicate the parsing rule of the remain array elements.
    data[1] = apnType;
    data[2] = rat;

    urcMsg = RfxMclMessage::obtainUrc(RFX_MSG_URC_QUALIFIED_NETWORK_TYPES_CHANGED, m_slot_id,
                                      RfxIntsData(data, 3));
    responseToTelCore(urcMsg);

    RFX_LOG_I(RFX_LOG_TAG, "[%d][%s] mode=%d, apnType=%d, rat=%d", m_slot_id, __FUNCTION__, data[0],
              data[1], data[2]);
}
...
void RmcDcCommonReqHandler::sendQualifiedNetworkTypesChanged(const int aid) {
    ...
    if (p_response->isAtResponseFail()) {
        if (p_response->getSuccess() > 0) {
            ...
            cause = p_response->atGetCmeError();
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] AT+EPDN=%d,\"ran\" returns cme cause : %d", m_slot_id,
                      __FUNCTION__, aid, cause);
        }
        return;
    }
    ...
    if (line == NULL) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] fail to get intermediate results!", m_slot_id,
                  __FUNCTION__);
        return;
    }
    ...
    if (err < 0 || strncmp(DC_RAN, cmdFormat, strlen(DC_RAN)) != 0) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing cmd", m_slot_id, __FUNCTION__);
        return;
    }
    ...
    if (!pdn_info.isDedicateBearer && pdn_info.active == DATA_STATE_ACTIVE) {
        data[0] = 1;  // this element is 'mode' to indicate the parsing rule of the remain array
                      // elements.
        ...
        urcMsg = RfxMclMessage::obtainUrc(RFX_MSG_URC_QUALIFIED_NETWORK_TYPES_CHANGED, m_slot_id,
                                          RfxIntsData(data, 3));
        responseToTelCore(urcMsg);
        RFX_LOG_I(RFX_LOG_TAG, "[%d][%s] mode=%d, apnType=%d, rat=%d", m_slot_id, __FUNCTION__,
...
void RmcDcCommonReqHandler::setPdnInfoProperty() {
    ...
    for (int i = 0; i < nPdnInfoSize; i++) {
        if ((getPdnActiveStatus(i) == DATA_STATE_ACTIVE) &&
            ...
            if (pdn_info.profileId != RIL_DATA_PROFILE_IMS &&
                ...
                valuePdnInfo.append(
                        String8::format("%s_%d,", RmcDcUtility::getProfileType(pdn_info.profileId),
    ...
    RFX_LOG_I(RFX_LOG_TAG, "[%d][%s] %s = %s", m_slot_id, __FUNCTION__, keyPdnInfo.string(),
              valuePdnInfo.string());
}
...
void RmcDcCommonReqHandler::mobileDataUsageNotify(const sp<RfxMclMessage>& msg) {
    ...
    if (pLine == NULL) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] fail to new pLine!", m_slot_id, __FUNCTION__);
        goto error;
    }
    ...
    if (err < 0) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing tx_bytes", m_slot_id,
                  __FUNCTION__);
        goto error;
    }
    ...
    if (err < 0) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing tx_packets", m_slot_id,
                  __FUNCTION__);
        goto error;
    }
    ...
    if (err < 0) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing rx_bytes", m_slot_id,
                  __FUNCTION__);
        goto error;
    }
    ...
    if (err < 0) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing rx_packets", m_slot_id,
                  __FUNCTION__);
        goto error;
    }
    RFX_LOG_E(RFX_LOG_TAG, "urc parsing");
    urcMsg = RfxMclMessage::obtainUrc(RFX_MSG_URC_MOBILE_DATA_USAGE, m_slot_id,
                                      RfxIntsData(response, 4 * sizeof(int)));
    responseToTelCore(urcMsg);
...
void RmcDcCommonReqHandler::adjustBearerBitmask(RIL_MtkDataProfileInfo** dataProfilePtrs,
    ...
    for (int i = 0; i < count; i++) {
        if (dataProfilePtrs[i]->bearerBitmask == 0 &&
            ...
            RFX_LOG_I(RFX_LOG_TAG, "[%d][%s] %s find %d apn that support wifi only", m_slot_id,
                      __FUNCTION__, dataProfilePtrs[i]->apn, find);
...
bool RmcDcCommonReqHandler::isSupportWifiType(int typesBitmask) {
    if (typesBitmask & RIL_APN_TYPE_IMS || typesBitmask & RIL_APN_TYPE_MMS ||
        typesBitmask & RIL_APN_TYPE_XCAP || typesBitmask & RIL_APN_TYPE_EMERGENCY) {
        return true;
    }
    return false;
}
...
int RmcDcCommonReqHandler::getAddressLifeTime(const int aid, int* pLifeTime) {
    ...
    p_response =
            atSendCommandSingleline(String8::format("AT+EGPCORDP=%d,\"001E\"", aid), "+EGPCORDP:");
    ...
    if (p_response->isAtResponseFail()) {
        if (p_response->getSuccess() > 0) {
            ...
            cause = p_response->atGetCmeError();
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] AT+EGPCORDP for aid%d returns cme cause : %d", rid,
                      __FUNCTION__, aid, cause);
        }
        ...
        // is disconnected by modem default time configuration, operator requirement or even
        // network.
    ...
    if (strncmp("001E", cmdFormat, strlen("001E")) == 0) {
        if (pLine->atTokHasmore()) {
            *pLifeTime = pLine->atTokNexthexint(&err);
            if (err < 0) {
                RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing life time", rid,
                          __FUNCTION__);
                return PDN_FAILED;
            }
        }
...
void RmcDcCommonReqHandler::requestStartKeepalive(const sp<RfxMclMessage>& msg) {
    ...
    if (pReq->type == NATT_IPV4) {
        ...
        snprintf(sourceAddress, MAX_IPV6_ADDRESS_LENGTH,
                 "%02X%02X:%02X%02X:"
        ...
        snprintf(destinationAddress, MAX_IPV6_ADDRESS_LENGTH,
                 "%02X%02X:%02X%02X:"
                 "%02X%02X:%02X%02X:%02X%02X:%02X%02X:%02X%02X:%02X%02X",
                 pReq->destinationAddress[0] & 0xff, pReq->destinationAddress[1] & 0xff,
                 pReq->destinationAddress[2] & 0xff, pReq->destinationAddress[3] & 0xff,
    ...
    cmd.append(String8::format("AT+EKALIVE=1,%d,%d,\"%s\",\"%s\",\"%s\",\"%s\",%d", type,
                               getInterfaceId(pReq->cid), sourceAddress, destinationAddress,
                               srcPort, dstPort, pReq->maxKeepaliveIntervalMillis / 1000));
    ...
    if (p_response->isAtResponseFail()) {
        if (p_response->getSuccess() > 0) {
            ...
            RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] AT+EKALIVE=1,x returns cme cause : %d", m_slot_id,
                      __FUNCTION__, p_response->atGetCmeError());
        }
        goto finish;
    }
...
finish:
    ...
    responseMsg = RfxMclMessage::obtainResponse(
            status, RfxKeepaliveStatusData(&keepaliveStatus, sizeof(keepaliveStatus)), msg);
    responseToTelCore(responseMsg);
}
...
void RmcDcCommonReqHandler::requestStopKeepalive(const sp<RfxMclMessage>& msg) {
    ...
    if (p_response == NULL || p_response->isAtResponseFail()) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] AT+EKALIVE=0,x returns ERROR", m_slot_id, __FUNCTION__);
...
void RmcDcCommonReqHandler::updateKeepaliveStatus(const sp<RfxMclMessage>& msg) {
    ...
    if (err < 0) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing conn_id", m_slot_id,
                  __FUNCTION__);
        goto error;
    }
    keepaliveStatus.sessionHandle = (uint32_t)ret;
    ...
    ret = pLine->atTokNextint(&err);
    if (err < 0) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing md_satus", m_slot_id,
                  __FUNCTION__);
        goto error;
    }
    keepaliveStatus.code = (RIL_KeepaliveStatusCode)ret;
    ...
    RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] sessionHandle=%d, code=%d", m_slot_id, __FUNCTION__,
              keepaliveStatus.sessionHandle, keepaliveStatus.code);
    urcMsg = RfxMclMessage::obtainUrc(
            RFX_MSG_URC_KEEPALIVE_STATUS, m_slot_id,
...
void RmcDcCommonReqHandler::onNwLimitNotify(const sp<RfxMclMessage>& msg) {
    ...
    response[0] = pLine->atTokNextint(&err);
    if (err < 0) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] ERROR occurs when parsing aid", m_slot_id, __FUNCTION__);
        goto error;
    }
