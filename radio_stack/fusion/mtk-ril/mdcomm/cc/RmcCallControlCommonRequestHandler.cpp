...
RmcCallControlCommonRequestHandler::RmcCallControlCommonRequestHandler(int slot_id, int channel_id)
...
void RmcCallControlCommonRequestHandler::requestGetCurrentCalls(const sp<RfxMclMessage>& msg) {
    // GWSD
    if (RfxGwsdUtils::getCallControlHandler() != NULL) {
        if (RfxGwsdUtils::getCallControlHandler()->fakeGetCurrentCalls(
                    this, msg, RmcCallControlUrcHandler::getSpeechCodec())) {
            return;
        }
    }
...
void RmcCallControlCommonRequestHandler::requestDial(const sp<RfxMclMessage>& msg, bool isEcc,
                                                     bool isVt, bool isImsDial) {
    RIL_Dial* pDial = (RIL_Dial*)(msg->getData()->getData());
    String8 cmd;
    int clirMode = pDial->clir;
    int request = msg->getId();

    if (request == RFX_MSG_REQUEST_IMS_DIAL || request == RFX_MSG_REQUEST_IMS_VT_DIAL ||
...
void RmcCallControlCommonRequestHandler::requestQueryTtyMode(const sp<RfxMclMessage>& msg) {
    ...
    if (p_response->getError() != 0 || p_response->getSuccess() != 1) {
        ...
        sp<RfxMclMessage> response = RfxMclMessage::obtainResponse(
                msg->getId(), RIL_E_SUCCESS, RfxIntsData(&mode, 1), msg, false);
        responseToTelCore(response);
        return;
    }
    ...
    if (isCdmaRat()) {
        if (callFailCause.cause_code == 1) {  // Phone is locked
            ...
            callFailCause.cause_code = CALL_FAIL_FDN_BLOCKED;
                   ...
                   callFailCause.cause_code ==
                           101 ||  // After a MS initiates a call, NW fails to respond
        ...
        if (callFailCause.cause_code == 10)
            ...
            callFailCause.cause_code = CALL_FAIL_ACCESS_CLASS_BLOCKED;
                 ...
                 (callFailCause.cause_code > 127 && callFailCause.cause_code != 2165 &&
                  callFailCause.cause_code != 380 && callFailCause.cause_code != 999))
            callFailCause.cause_code = CALL_FAIL_ERROR_UNSPECIFIED;
    }
    logD(RFX_LOG_TAG, "RIL fail cause_code = %d, vendor_cause = %s", callFailCause.cause_code,
         callFailCause.vendor_cause);
...
void RmcCallControlCommonRequestHandler::requestSetCallIndication(const sp<RfxMclMessage>& msg) {
    ...
    if (pInt[0] == 1) {
        ...
        logD(RFX_LOG_TAG, "RIL call indication cause %d, causeToModem=%d", pInt[3], causeToModem);
    }
...
/// C2K specific start
void RmcCallControlCommonRequestHandler::requestSetPreferredVoicePrivacyMode(
        const sp<RfxMclMessage>& msg) {
    RIL_Errno err;
    int* pInt = (int*)msg->getData()->getData();
...
void RmcCallControlCommonRequestHandler::requestQueryPreferredVoicePrivacyMode(
        const sp<RfxMclMessage>& msg) {
    RIL_Errno err;
    int result = 0;
    ...
    sp<RfxMclMessage> response = RfxMclMessage::obtainResponse(
            msg->getId(), RIL_E_SUCCESS, RfxIntsData((void*)&result, sizeof(int)), msg);
    responseToTelCore(response);
}
...
void RmcCallControlCommonRequestHandler::requestFlash(const sp<RfxMclMessage>& msg) {
    ...
    if (number == NULL || strlen(number) == 0) {
        ...
        if (strlen(origNumber.string()) < 16) {
            ...
            logD(RFX_LOG_TAG, "AT> AT+CFSH=%s", RfxRilUtils::pii(RFX_LOG_TAG, origNumber.string()));
        } else {
...
void RmcCallControlCommonRequestHandler::requestExitEmergencyCallbackMode(
        const sp<RfxMclMessage>& msg) {
...
void RmcCallControlCommonRequestHandler::requestEmergencyRedial(const sp<RfxMclMessage>& msg) {
    ...
    String8 cmd =
            String8::format("%s=\"%s%s\",%d", "ATDER", clir, dialString.string(), pRedial->call_id);
    logD(RFX_LOG_TAG, "AT> ATDER=\"%s%s\",%d", clir,
         RfxRilUtils::pii(RFX_LOG_TAG, dialString.string()), pRedial->call_id);
...
void RmcCallControlCommonRequestHandler::requestNotifyEmergencySession(const sp<RfxMclMessage>& msg,
                                                                       bool isStarted) {
...
void RmcCallControlCommonRequestHandler::queryEmciSupport() {
    ...
    if (support == 1) {
        rfx_property_set("vendor.ril.call.emci_support", String8::format("%d", support).string());
    } else {
        rfx_property_set("vendor.ril.call.emci_support", String8::format("%d", 0).string());
    }
}
