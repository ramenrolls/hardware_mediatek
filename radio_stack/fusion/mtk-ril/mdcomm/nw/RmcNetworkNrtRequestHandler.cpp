...
static const int request[] = {RFX_MSG_REQUEST_QUERY_AVAILABLE_NETWORKS,
                              RFX_MSG_REQUEST_QUERY_AVAILABLE_NETWORKS_WITH_ACT,
                              RFX_MSG_RIL_REQUEST_START_NETWORK_SCAN};

static const int events[] = {RFX_MSG_EVENT_SIMULATE_NETWORK_SCAN};
...
RFX_REGISTER_DATA_TO_REQUEST_ID(RfxVoidData, RfxStringsData,
                                RFX_MSG_REQUEST_QUERY_AVAILABLE_NETWORKS_WITH_ACT);
RFX_REGISTER_DATA_TO_REQUEST_ID(RfxNetworkScanData, RfxVoidData,
                                RFX_MSG_RIL_REQUEST_START_NETWORK_SCAN);
...
RmcNetworkNrtRequestHandler::RmcNetworkNrtRequestHandler(int slot_id, int channel_id)
    : RmcNetworkHandler(slot_id, channel_id), mPlmn_list_format(0) {
    int err;
...
void RmcNetworkNrtRequestHandler::requestQueryAvailableNetworks(const sp<RfxMclMessage>& msg) {
    ...
    for (i = 0, j = 0; i < num; i++, j += 4) {
        ...
        if (len == 5 || len == 6) {
            if (isOp08Support()) {
                ...
                err = getOperatorNamesFromNumericCode(response[j + 2], lac, response[j + 0],
                                                      response[j + 1], MAX_OPER_NAME_LENGTH);
            }
            if (err < 0) goto error;
        } else {
    ...
    logD(LOG_TAG,
         "requestQueryAvailableNetworks sucess, clear plmnListOngoing and plmnListAbort flag");
    mPlmnListOngoing = 0;
    ...
    resp = RfxMclMessage::obtainResponse(msg->getId(), RIL_E_SUCCESS,
                                         RfxStringsData(response_filter, num_filter * 4), msg,
                                         false);
...
error:
    ...
    logE(LOG_TAG,
         "requestQueryAvailableNetworks must never return error when radio is on, plmnListAbort=%d",
         mPlmnListAbort);
    if (mPlmnListAbort == 1) {
        resp = RfxMclMessage::obtainResponse(msg->getId(), RIL_E_CANCELLED, RfxVoidData(), msg,
                                             false);
    } else {
        resp = RfxMclMessage::obtainResponse(msg->getId(), RIL_E_MODEM_ERR, RfxVoidData(), msg,
                                             false);
    }
    mPlmnListOngoing = 0;
    getMclStatusManager()->setBoolValue(RFX_STATUS_KEY_NETWORK_SCAN_ONGOING, false);
...
void RmcNetworkNrtRequestHandler::requestQueryAvailableNetworksWithAct(
    ...
    logD(LOG_TAG,
         "requestQueryAvailableNetworksWithAct sucess, clear plmnListOngoing and plmnListAbort "
         "flag");
    mPlmnListOngoing = 0;
    getMclStatusManager()->setBoolValue(RFX_STATUS_KEY_NETWORK_SCAN_ONGOING, false);
    mPlmnListAbort = 0; /* always clear here to prevent race condition scenario */
...
error:
    ...
    if (lacStr) free(lacStr);
    logV(LOG_TAG,
         "requestQueryAvailableNetworksWithAct must never return error when radio is on, "
         "plmnListAbort=%d",
         mPlmnListAbort);
    if (mPlmnListAbort == 1) {
        resp = RfxMclMessage::obtainResponse(msg->getId(), RIL_E_CANCELLED, RfxVoidData(), msg,
        ...
        resp = RfxMclMessage::obtainResponse(msg->getId(), RIL_E_GENERIC_FAILURE, RfxVoidData(),
                                             msg, false);
    }
    mPlmnListOngoing = 0;
    getMclStatusManager()->setBoolValue(RFX_STATUS_KEY_NETWORK_SCAN_ONGOING, false);
...
void RmcNetworkNrtRequestHandler::requestStartNetworkScan(const sp<RfxMclMessage>& msg) {
    ...
    for (size_t i = 0; i < (p_args->specifiers_length); i++) {
        logV(LOG_TAG, "requestStartNetworkScan p_args->specifiers[%zu]->radio_access_network=%d", i,
             p_args->specifiers[i].radio_access_network);
        logV(LOG_TAG, "requestStartNetworkScan p_args->specifiers[%zu]->radio_access_network=%d", i,
             p_args->specifiers[i].bands_length);
        for (size_t j = 0; j < (p_args->specifiers[i].bands_length); j++) {
            if (p_args->specifiers[i].radio_access_network == 0x01) {
                logV(LOG_TAG,
                     "requestStartNetworkScan p_args->specifiers[%zu].bands.geran_bands[%zu]=%d", i,
                     j, p_args->specifiers[i].bands.geran_bands[j]);
            } else if (p_args->specifiers[i].radio_access_network == 0x02) {
                logV(LOG_TAG,
                     "requestStartNetworkScan p_args->specifiers[%zu].bands.utran_bands[%zu]=%d", i,
                     j, p_args->specifiers[i].bands.utran_bands[j]);
            } else if (p_args->specifiers[i].radio_access_network == 0x03) {
                logV(LOG_TAG,
                     "requestStartNetworkScan p_args->specifiers[%zu].bands.eutran_bands[%zu]=%d",
                     i, j, p_args->specifiers[i].bands.eutran_bands[j]);
            } else {
                logV(LOG_TAG, "Should not be here");
            }
        }
        logV(LOG_TAG, "requestStartNetworkScan p_args->specifiers[%zu]->channels_length=%d", i,
             p_args->specifiers[i].channels_length);
        for (size_t j = 0; j < (p_args->specifiers[i].channels_length); j++) {
            logV(LOG_TAG, "requestStartNetworkScan p_args->specifiers[%zu]->channels[%zu]=%d", i, j,
                 p_args->specifiers[i].channels[j]);
        }
    }
    ...
    if (p_args->maxSearchTime < 70 || p_args->maxSearchTime > 3600) {
        if (p_args->maxSearchTime >= 60 && p_args->maxSearchTime < 70)
            ril_err = RIL_E_REQUEST_NOT_SUPPORTED;
        goto error;
    }
    logV(LOG_TAG, "requestStartNetworkScan incrementalResults=%d", p_args->incrementalResults);
    logV(LOG_TAG, "requestStartNetworkScan incrementalResultsPeriodicity=%d",
         p_args->incrementalResultsPeriodicity);
    if (p_args->incrementalResultsPeriodicity < 1 || p_args->incrementalResultsPeriodicity > 10)
        goto error;
    ...
    sendEvent(RFX_MSG_EVENT_SIMULATE_NETWORK_SCAN, RfxVoidData(), RIL_CMD_PROXY_8, m_slot_id, -1,
              -1, ms2ns(200), MTK_RIL_REQUEST_PRIORITY_MEDIUM);
    return;
...
void RmcNetworkNrtRequestHandler::handleSimulatedNetworkScan() {
    ...
    for (int i = 0; i < num; i++) {
        ...
        switch (result->network_infos[i].cellInfoType) {
            case RIL_CELL_INFO_TYPE_GSM:
                ...
                getOperatorNamesFromNumericCode(mccmnc, lac, longname, shortname,
                                                MAX_OPER_NAME_LENGTH);
                asprintf(
                        &(result->network_infos[i].CellInfo.gsm.cellIdentityGsm.operName.long_name),
                        "%s", longname);
                asprintf(&(result->network_infos[i]
            ...
            case RIL_CELL_INFO_TYPE_LTE:
                ...
                getOperatorNamesFromNumericCode(mccmnc, lac, longname, shortname,
                                                MAX_OPER_NAME_LENGTH);
                asprintf(
                        &(result->network_infos[i].CellInfo.lte.cellIdentityLte.operName.long_name),
                        "%s", longname);
                asprintf(&(result->network_infos[i]
    ...
    urc = RfxMclMessage::obtainUrc(
            ...
            RfxNetworkScanResultData((void*)result, sizeof(RIL_NetworkScanResult)));
    // response to TeleCore
