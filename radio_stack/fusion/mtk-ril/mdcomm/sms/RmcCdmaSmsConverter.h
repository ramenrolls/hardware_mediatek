...
class BitwiseOutputStream {
  ...
  public:
    ...
    //
    // RETURNS: void
    void write(int bits,      // [IN] the amount of data to write (gte 0, lte 8)
    ...
    //
    // RETURNS: void
    void writeByteArray(int bits,                         // [IN] the amount of data to write
    ...
    int getBitLen() const { return m_pos; }
  ...
  public:
    ...
    BitwiseOutputStream(int startLen  // [IN] the start buffer size of the stream
    );
...
class BitwiseInputStream {
  ...
  public:
    ...
    unsigned char read(int bits  // [IN] bits the amount of data to read (gte 0, lte 8)
    );

    // Read data in bulk into a byte array and increment the current position.
    //
    // RETURNS: None
    void readByteArray(int bits,                   // [IN] number of bits to read
    ...
    //
    // RETURNS: None
    void skip(int bits  // [IN] the amount by which to increment the position
    );
...
class CdmaSmsMessage : public RfxObject {
  ...
  public:
    ...
    String8 toHexHeader() { return String8::format("%02x", m_type); }
  ...
  public:
    ...
    CdmaSmsMessage(const int type  // [IN] Construct the message by type
    );

    // Constructor for construct the message by hexPdu from modem
    CdmaSmsMessage(const char* hexPdu,  // [IN] type part of the hexPdu from modem
...
class RmcTlvNode : public RfxObject {
  ...
  public:
    ...
    String8 toHexHeader() { return String8::format("%02x%02x", m_tag, m_length); }
  ...
  public:
    ...
    RmcTlvNode(int tag,    // [IN] the tag of the TLV node
               int length  // [IN] the length of the TLV node
    );

    // Constructor for construct the node by the hexPdu from modem
    RmcTlvNode(const char* hexPdu,  // [IN] A part of the hexPdu which corresponse to a TLV mode
...
class TeleServiceId : public RmcTlvNode {
  ...
  public:
    ...
    bool isCtWapPush() { return (getTeleServiceId() == CT_WAP_PUSH_TID); }
  ...
  public:
    ...
    TeleServiceId(int id  // [IN] the teleservice id from RILJ
    );

    // Constructor for construct the teleservice if by the hexPdu from modem
    TeleServiceId(const char* hexPdu,  // [IN] the teleservice part of the hexPdu from modem
                  int len              // [IN] the length of the hexPdu
    );
...
class CdmaSmsAddress : public RmcTlvNode {
  ...
  public:
    ...
    bool getAddress(RIL_CDMA_SMS_Address& addr  // [OUT] the address
    );

    // Get the string format address
    //
    // return true if get the address successfully
    bool getAddress(String8& addr  // [OUT] the address
    );
  ...
  public:
    ...
    CdmaSmsAddress(RIL_CDMA_SMS_Address& addr,  // [IN] the address
                   bool isMo                    // [IN] MO or MT
    );

    // Constructor for construct the address by the hexPdu from modem
    CdmaSmsAddress(const char* hexPdu,  // the address part of the hexPdu from the modem
...
class CdmaSmsSubAddress : public RmcTlvNode {
  ...
  public:
    ...
    bool getSubAddr(RIL_CDMA_SMS_Subaddress& subAddr  // [OUT] the sub address
    );
  ...
  public:
    ...
    CdmaSmsSubAddress(const char* hexPdu,  // [IN] the sub address part of the hexPdu from the modem
                      int len              // [IN] the length of the hexPdu
    );
...
class BearerReplyOpt : public RmcTlvNode {
  ...
  public:
    ...
    BearerReplyOpt(const char* hexPdu,  // [IN] Bearer reply option part of the hexPdu from modem
                   int len              // [IN] The length of the hexPdu
    );
...
class CauseCode : public RmcTlvNode {
  ...
  public:
    ...
    CauseCode(RIL_CDMA_SMS_Ack& ack,  // [IN] the ack message from RILJ
    ...
    CauseCode(const char* hexPdu,  // [IN] the cause code part of hexPdu from modem
              int len              // [IN] the length of the hexPdu
    );
...
class UserData {
  ...
  public:
    UserData() : msgEncoding(-1), msgType(-1), numFields(-1) {}
...
class MessageIdentifer {
  public:
    MessageIdentifer() : messageType(-1), messageId(-1), hasUserDataHeader(false) {}
...
class BearerData : public RmcTlvNode {
  ...
  public:
    ...
    bool getBearerData(bool isCtWap,               // [IN] if WAP push message of CT
    ...
    int getMessageId() const { return m_msgIdentifier.messageId; }

    // Get the message type
    //
    // RETURNS: the message type
    int getMessageType() const { return m_msgIdentifier.messageType; }

    // Check if status report
    //
    // RETURNS: true if statue report
    bool isStatusReport() { return m_msgIdentifier.messageType == MESSAGE_TYPE_DELIVERY_ACK; }
    ...
    bool isNeedStatusReport() { return m_needStatusReport; }

    // Check if having user data header
    //
    // RETURNS: true if having user data header
    bool hasUserDataHeader() { return m_msgIdentifier.hasUserDataHeader; }

    // Get the reference of user data payload.
    //
    // RETURNS: the reference of user data payload
    const Vector<unsigned char>& getUserDataPayload() { return m_UserData.userDataPayload; }

    // Get the message encoding
    //
    // RETURNS: message encoding
    int getMsgEncoding() const { return m_UserData.msgEncoding; }

    // Get the field number
    //
    // RETURNS: field number
    int getNumFields() const { return m_UserData.numFields; }

    // Constructor / Desctructor
  public:
    // Default constructor
    BearerData() : RmcTlvNode(BEARER_DATA, -1), m_needStatusReport(0) {}

    // Constructor for construct the Bearer Data from the structure of RILJ
    BearerData(unsigned char* data,  // [IN] the bearer data
    ...
    BearerData(const char* hexPdu,  // [IN] the bearer data part of hex Pdu from modem
               int len              // [IN] the length of the hexPdu
    );
...
class RmcCdmaSmsConverter {
  ...
  public:
     ...
     */
    static bool toHexPdu(RIL_CDMA_SMS_Message& message,  // [IN] The normal SMS from RILJ
     ...
     * RETURNS: true if successfully convert, otherwise false
     */
    static bool toHexPdu(RIL_CDMA_SMS_Ack* pAck, int replySeqNo, RIL_CDMA_SMS_Address* pAddress,
                         String8& hexPdu);
     ...
     * RETURNS: true if successfully convert, otherwise false
     */
    static bool toMessage(RIL_CDMA_SMS_Message* pMessage,  // [OUT] The message to RILJ
     ...
     */
    static bool isStatusReport(RIL_CDMA_SMS_Message* pMessage,  // [IN] The SMS message
     ...
     */
    static bool isNeedStatusReport(RIL_CDMA_SMS_Message* pMessage  // [IN] The SMS message
    );

    /*
     * Get the needed inbound sms info.
     */
    static void getInboundSmsInfo(RIL_CDMA_SMS_Message* pMessage,  // [IN] The MT SMS message
