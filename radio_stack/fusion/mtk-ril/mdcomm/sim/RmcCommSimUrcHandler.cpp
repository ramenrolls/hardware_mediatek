...
static const char* commUrcList[] = {
        "+EUSIM:", "+ETESTSIM:",   "+ESIMS:",      "+ECPIN:",      "+ESIMIND:",
        "+ICCID",  "+ESMLSTATUS:", "+ESLOTSINFO:", "+ESMLRSUEVT:", "+ETMOEVT:",
};
...
RmcCommSimUrcHandler::RmcCommSimUrcHandler(int slot_id, int channel_id)
    ...
    if (isSimSlotLockSupport()) {
        ...
        getNonSlotMclStatusManager()->setIntValue(RFX_STATUS_KEY_SML_SLOT_LOCK_POLICY,
                                                  SML_SLOT_LOCK_POLICY_UNKNOWN, true);
        getNonSlotMclStatusManager()->setIntValue(RFX_STATUS_KEY_SML_SLOT_LOCK_POLICY_LOCK_STATE,
                                                  SML_SLOT_LOCK_POLICY_LOCK_STATE_UNKNOWN, true);
        getMclStatusManager()->setIntValue(RFX_STATUS_KEY_SML_SLOT_LOCK_POLICY_SERVICE_CAPABILITY,
                                           SML_SLOT_LOCK_POLICY_SERVICE_CAPABILITY_UNKNOWN, true);
        getMclStatusManager()->setIntValue(RFX_STATUS_KEY_SML_SLOT_LOCK_POLICY_VALID_CARD,
...
RmcSimBaseHandler::SIM_HANDLE_RESULT RmcCommSimUrcHandler::needHandle(
    ...
    if (strStartsWith(ss, "+EUSIM:") || strStartsWith(ss, "+ESIMS: 0,0") ||
        strStartsWith(ss, "+ESIMS: 0,5") || strStartsWith(ss, "+ESIMS: 0,10") ||
        strStartsWith(ss, "+ESIMS: 0,11") || strStartsWith(ss, "+ESIMS: 0,13") ||
        strStartsWith(ss, "+ESIMS: 0,15") || strStartsWith(ss, "+ESIMS: 0,19") ||
        // MTK-START: AOSP SIM PLUG IN/OUT
        strStartsWith(ss, "+ESIMS: 0,26") ||
        // MTK-EDN
        strStartsWith(ss, "+ESIMS: 1,2") || strStartsWith(ss, "+ESIMS: 1,6") ||
        strStartsWith(ss, "+ESIMS: 1,9") || strStartsWith(ss, "+ESIMS: 1,12") ||
        strStartsWith(ss, "+ESIMS: 1,14") || strStartsWith(ss, "+ECPIN:") ||
        strStartsWith(ss, "+ESIMIND:") || strStartsWith(ss, "+ICCID:") ||
        strStartsWith(ss, "+ESMLSTATUS:") || strStartsWith(ss, "+ESIMS: 0,28") ||
        strStartsWith(ss, "+ESIMS: 1,29") || strStartsWith(ss, "+ESLOTSINFO:") ||
        strStartsWith(ss, "+ETESTSIM:") || strStartsWith(ss, "+ESMLRSUEVT:") ||
...
void RmcCommSimUrcHandler::handleEcpin(const sp<RfxMclMessage>& msg, RfxAtLine* urc) {
    ...
    vsim_init = getMclStatusManager()->getIntValue(RFX_STATUS_KEY_ECPIN_STATE, RFX_ECPIN_DONE);
    ...
    if ((cpin_status == 1) && (vsim_init == RFX_WAIT_FOR_ECPIN)) {
        ...
        unsol = RfxMclMessage::obtainUrc(RFX_MSG_URC_RESPONSE_SIM_STATUS_CHANGED, m_slot_id,
                                         RfxVoidData());
...
void RmcCommSimUrcHandler::handleCardType(const sp<RfxMclMessage>& msg, RfxAtLine* urc) {
    ...
    bool isMdSupportIccid =
            getMclStatusManager()->getBoolValue(RFX_STATUS_KEY_MD_ICCID_SUPPORT, false);
...
void RmcCommSimUrcHandler::handleSimStateChanged(const sp<RfxMclMessage>& msg, char* urc) {
    ...
    if (strStartsWith(urc, "+ESIMS: 0,0") || strStartsWith(urc, "+ESIMS: 0,10") ||
        ...
        if (strStartsWith(urc, "+ESIMS: 0,11") && RfxRilUtils::rfxGetSimCount() >= 2 &&
        ...
        if (strStartsWith(urc, "+ESIMS: 0,11") || strStartsWith(urc, "+ESIMS: 0,13") ||
            ...
            if (getMclStatusManager()->getBoolValue(RFX_STATUS_KEY_MODEM_SIMONOFF_SUPPORT)) {
                ...
                rfx_property_set(simOnOff.string(),
                                 String8::format("%d", SIM_POWER_STATE_SIM_ON).string());
                ...
                sp<RfxMclMessage> unsol = RfxMclMessage::obtainUrc(
                        RFX_MSG_URC_SIM_POWER_CHANGED, m_slot_id, RfxIntsData(&simPower, 1));
                responseToTelCore(unsol);
            }
            ...
            if (common_slot_no_changed == true) {
                ...
                if (RfxRilUtils::isVsimEnabled()) {
                    logD(mTag,
                         "Ingore no changed event during vsim enabled on common slot project.");
                } else {
                    // External SIM [End]
                ...
                if ((isCommontSlotSupport() == true) &&
                    (getVsimPlugInOutEvent(m_slot_id) == VSIM_TRIGGER_PLUG_OUT)) {
                    ...
                    // External SIM [End]
                    unsol = RfxMclMessage::obtainUrc(RFX_MSG_URC_SIM_PLUG_OUT, m_slot_id,
                                                     RfxVoidData());
            ...
            unsol = RfxMclMessage::obtainUrc(RFX_MSG_URC_SIM_IMEI_LOCK, m_slot_id, RfxVoidData());
            responseToTelCore(unsol);
        } else {
            unsol = RfxMclMessage::obtainUrc(RFX_MSG_URC_RESPONSE_SIM_STATUS_CHANGED, m_slot_id,
                                             RfxVoidData());
        ...
        responseToTelCore(unsol);
        // MTK-END
    } else if (strStartsWith(urc, "+ESIMS: 1,9") || strStartsWith(urc, "+ESIMS: 1,14") ||
               strStartsWith(urc, "+ESIMS: 1,12")) {
        ...
        if (RfxRilUtils::rfxGetSimCount() >= 2) {
            if (isCommontSlotSupport() == true && (strStartsWith(urc, "+ESIMS: 1,12"))) {
                // External SIM [Start]
                if ((isCommontSlotSupport() == true) &&
                    (getVsimPlugInOutEvent(m_slot_id) == VSIM_TRIGGER_PLUG_IN)) {
                    logD(mTag, "vsim trigger tray plug in on common slot project.");
                    // setVsimPlugInOutEvent(m_slot_id, VSIM_TRIGGER_RESET);
                } else if ((isCommontSlotSupport() == true) && RfxRilUtils::isVsimEnabled()) {
                    logD(mTag,
                         "Ingore tray plug in event during vsim enabled on common slot project.");
                } else {
                    // External SIM [End]

                    // In this feature, when we receive "ESIMS: 1, 12", it does not mean SIM card
                    // plug, but means slot plug in. That is, it might be no SIM card in this slot.
                    // Thus, we need to query SIM state when detect SIM missing and update flag at
                    // that time.
                    logD(mTag,
                         "Receive plug in in common slot project so do not set sim inserted status "
                         "here");

                    int simCount = RfxRilUtils::rfxGetSimCount();
                    logD(mTag, "mTrayPluginCount: %d (slot %d)", mTrayPluginCount, m_slot_id);
                    ...
                    if (mTrayPluginCount == 0) {
                        ...
                        for (int i = 0; i < simCount; i++) {
                            getMclStatusManager(i)->setBoolValue(
                                    RFX_STATUS_KEY_MODEM_SIM_TASK_READY, false);
                        }
                    } else {
                        mTrayPluginCount--;
                    }

                    unsol = RfxMclMessage::obtainUrc(RFX_MSG_URC_TRAY_PLUG_IN, m_slot_id,
                                                     RfxVoidData());
                    responseToTelCore(unsol);
                }
            }
        ...
        unsol = RfxMclMessage::obtainUrc(RFX_MSG_URC_RESPONSE_SIM_STATUS_CHANGED, m_slot_id,
                                         RfxVoidData());
        responseToTelCore(unsol);
        unsol = RfxMclMessage::obtainUrc(RFX_MSG_URC_SIM_IMSI_REFRESH_DONE, m_slot_id,
                                         RfxVoidData());
        responseToTelCore(unsol);
...
void RmcCommSimUrcHandler::handleSimIndication(const sp<RfxMclMessage>& msg, RfxAtLine* urc) {
    ...
    switch (indEvent) {
        ...
        case 2:
            ...
            // uicc_app_list = is_csim_exist | is_usim_exist | is_isim_exist (currently isim always
            // 0) is_usim_exist:2 is_csim_exist:4 (is_csim_exist | is_usim_exist): 6 For icc card
            // uicc_app_list:0
...
void RmcCommSimUrcHandler::handleIccidIndication(const sp<RfxMclMessage>& msg, RfxAtLine* urc) {
    ...
    if (state == 1 && atLine->atTokHasmore()) {
        ...
        unsol = RfxMclMessage::obtainUrc(RFX_MSG_URC_CARD_DETECTED_IND, m_slot_id, RfxVoidData());
        responseToTelCore(unsol);
    } else {
...
void RmcCommSimUrcHandler::handleSimSlotLockStatusChanged(RfxAtLine* urc) {
    ...
    if (!getNonSlotMclStatusManager()->getBoolValue(RFX_STATUS_KEY_SML_SLOT_LOCK_MD_SUPPORTED,
                                                    false)) {
        getNonSlotMclStatusManager()->setBoolValue(RFX_STATUS_KEY_SML_SLOT_LOCK_MD_SUPPORTED, true,
                                                   false);
    }
    ...
    logD(mTag,
         "handleSimSlotLockStatusChanged: rule: %d, sub_rule: %d, lock_state: %d, \
            service: %d, valid_card: %d, slot %d",
         rule, sub_rule, lock_state, service, valid_card, m_slot_id);

    // Parse and update lock policy rule into sml_status[0], currently MD only uses 4 bits of
    // sub_rule
    ...
    if (SML_NO_RULE == rule) {
        ...
        if ((SML_SLOT_LOCK_USE_SAME_CONTEXT == slot_lock_rule) &&
            (SML_INVALID_SIM_FULL_SERVICE == capability_rule)) {
            sml_status[0] = SML_SLOT_LOCK_POLICY_LK_SLOTA_RESTRICT_INVALID_ECC_FOR_VALID_NO_SERVICE;
        }
    ...
    old_valid_info =
            getMclStatusManager()->getIntValue(RFX_STATUS_KEY_SML_SLOT_LOCK_POLICY_VALID_CARD);
    ...
    getNonSlotMclStatusManager()->setIntValue(RFX_STATUS_KEY_SML_SLOT_LOCK_POLICY, sml_status[0],
                                              true);
    getNonSlotMclStatusManager()->setIntValue(RFX_STATUS_KEY_SML_SLOT_LOCK_POLICY_LOCK_STATE,
                                              sml_status[1], true);
    getMclStatusManager()->setIntValue(RFX_STATUS_KEY_SML_SLOT_LOCK_POLICY_SERVICE_CAPABILITY,
    ...
    logD(mTag, "handleSimSlotLockStatusChanged sml_status: %d, %d, %d, %d, slot %d", sml_status[0],
         sml_status[1], sml_status[2], sml_status[3], m_slot_id);
    response = RfxMclMessage::obtainUrc(RFX_MSG_URC_SIM_SLOT_LOCK_POLICY_NOTIFY, m_slot_id,
                                        RfxIntsData(sml_status, 4));
...
void RmcCommSimUrcHandler::onSmlConfigureCheck() {
    ...
    if (!smlConfigureChecked) {
        getNonSlotMclStatusManager()->setBoolValue(RFX_STATUS_KEY_SML_SLOT_LOCK_CONFIG_CHECKED,
                                                   true, false);
        if (getNonSlotMclStatusManager()->getBoolValue(RFX_STATUS_KEY_SML_SLOT_LOCK_MD_SUPPORTED,
                                                       false)) {
            if (!isSimSlotLockSupport()) {
...
error:
    if (decodeerr == RIL_E_INVALID_STATE) {
        ...
        sendEvent(RFX_MSG_EVENT_GET_SLOT_STATUS, RfxStringData(ss), RIL_CMD_PROXY_7, m_slot_id, -1,
                  -1, 0.5 * 1000000000);
    }
...
int RmcCommSimUrcHandler::getVsimPlugInOutEvent(int slotId) { return mVsimPlugInOut[slotId]; }

void RmcCommSimUrcHandler::setMdWaitingResponse(int slotId, int waiting) {
    mVsimMdWaiting[slotId] = waiting;
}

int RmcCommSimUrcHandler::getMdWaitingResponse(int slotId) { return mVsimMdWaiting[slotId]; }
// External SIM [End]
...
void RmcCommSimUrcHandler::handleRsuEvent(const sp<RfxMclMessage>& msg, RfxAtLine* urc) {
    ...
    if (pLine->atTokHasmore()) {
        ...
        sp<RfxMclMessage> p_urc = RfxMclMessage::obtainUrc(RFX_MSG_UNSOL_SML_RSU_EVENT, m_slot_id,
...
void RmcCommSimUrcHandler::handleTmoRsuEvent(const sp<RfxMclMessage>& msg, RfxAtLine* urc) {
    ...
    p_urc = RfxMclMessage::obtainUrc(RFX_MSG_UNSOL_SML_RSU_EVENT, m_slot_id,
                                     RfxStringsData(event, 2));
    responseToTelCore(p_urc);
    if (event[0] != NULL) {
        free(event[0]);
