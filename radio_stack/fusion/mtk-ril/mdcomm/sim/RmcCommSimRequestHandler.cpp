...
static const int ch1ReqList[] = {RFX_MSG_REQUEST_ENTER_SIM_PIN,
...
static const int ch3ReqList[] = {RFX_MSG_REQUEST_SIM_AUTHENTICATION};

static const int ch1EventList[] = {
        RFX_MSG_EVENT_SIM_QUERY_ICCID,        RFX_MSG_EVENT_SIM_DETECT_SIM,
        RFX_MSG_EVENT_SIM_SIM_AUTHENTICATION, RFX_MSG_EVENT_SIM_USIM_AUTHENTICATION,
        RFX_MSG_EVENT_GET_SLOT_STATUS,
};

// Real-time
static const int ch7ReqList[] = {
        RFX_MSG_REQUEST_GET_SIM_STATUS,       RFX_MSG_REQUEST_SET_SIM_POWER,
        RFX_MSG_REQUEST_GET_SLOT_STATUS,      RFX_MSG_REQUEST_ACTIVATE_UICC_CARD,
...
RFX_REGISTER_DATA_TO_REQUEST_ID(RfxRawData, RfxRawData, RFX_MSG_REQUEST_SIM_SAP_CONNECT);
RFX_REGISTER_DATA_TO_REQUEST_ID(RfxRawData, RfxRawData, RFX_MSG_REQUEST_SIM_SAP_DISCONNECT);
RFX_REGISTER_DATA_TO_REQUEST_ID(RfxRawData, RfxRawData, RFX_MSG_REQUEST_SIM_SAP_APDU);
RFX_REGISTER_DATA_TO_REQUEST_ID(RfxRawData, RfxRawData, RFX_MSG_REQUEST_SIM_SAP_TRANSFER_ATR);
RFX_REGISTER_DATA_TO_REQUEST_ID(RfxRawData, RfxRawData, RFX_MSG_REQUEST_SIM_SAP_POWER);
RFX_REGISTER_DATA_TO_REQUEST_ID(RfxRawData, RfxRawData, RFX_MSG_REQUEST_SIM_SAP_RESET_SIM);
RFX_REGISTER_DATA_TO_REQUEST_ID(RfxRawData, RfxRawData,
                                RFX_MSG_REQUEST_SIM_SAP_SET_TRANSFER_PROTOCOL);
RFX_REGISTER_DATA_TO_REQUEST_ID(RfxVoidData, RfxRawData, RFX_MSG_REQUEST_SIM_SAP_ERROR_RESP);
RFX_REGISTER_DATA_TO_REQUEST_ID(RfxStringsData, RfxIntsData,
                                RFX_MSG_REQUEST_SIM_ENTER_DEVICE_NETWORK_DEPERSONALIZATION);
...
RFX_REGISTER_DATA_TO_REQUEST_ID(RfxVoidData, RfxIntsData, RFX_MSG_REQUEST_ACTIVATE_UICC_CARD);
RFX_REGISTER_DATA_TO_REQUEST_ID(RfxVoidData, RfxIntsData, RFX_MSG_REQUEST_DEACTIVATE_UICC_CARD);
    ...
    switch (request) {
        ...
        case RFX_MSG_REQUEST_SET_FACILITY_LOCK: {
            ...
            if (facility == NULL || strlen(facility) <= 0) {
                ...
                response = RfxMclMessage::obtainResponse(msg->getId(), RIL_E_INVALID_ARGUMENTS,
...
bool RmcCommSimRequestHandler::getSimAppInfo(int app_id, RIL_AppStatus* p_app_info,
    ...
    if (p_response != NULL && p_response->getSuccess() > 0) {
        ...
        String8 logStr;
        logStr = String8::format("app_id %d, app_type %d, app_state %d, AID %s, label %s, ", app_id,
        ...
        logStr += String8::format("pin1 %d, pin2 %d, (%d, %d, %d, %d)", p_app_info->pin1,
                                  p_app_info->pin2, retry->pin1, retry->pin2, retry->puk1,
                                  retry->puk2);
...
void RmcCommSimRequestHandler::handleGetSimStatus(const sp<RfxMclMessage>& msg) {
    ...
    // External SIM [Start]
    if (mEvsimSent == false && RfxRilUtils::isExternalSimSupport() == 1 &&
        ...
        if (vsimMgrExisted == false && clientId == -1 &&
            ...
            int majorRadioState =
                    getMclStatusManager(majorSlotId)
    ...
    if (msg->getTimeStamp() <= tSimSwitch) {
        ...
        ril_error = RIL_E_INVALID_STATE;
               ...
               (((cardType == RFX_CARD_TYPE_SIM) && ((cdma3GNT == AP_TRIGGER_SWITCH_RUIM) ||
                                                     (cdma3GNT == GMSS_TRIGGER_SWITCH_RUIM))) ||
        ...
        do {
            ...
            if (ret == UICC_BUSY) {
                ...
                if ((count == MAX_RETRY_COUNT) || RfxRilUtils::isVsimEnabledBySlot(m_slot_id)) {
                    ret = UICC_NOT_READY;
                    getMclStatusManager()->setIntValue(RFX_STATUS_KEY_ECPIN_STATE,
                                                       RFX_WAIT_FOR_ECPIN);
                } else {
                    sleepMsec(200);
                }
                ...
                if (err < 0 || p_response->getSuccess() == 0) {
                    ...
                    testSimProp.append((m_slot_id == 0) ? ""
                                                        : String8::format(".%d", (m_slot_id + 1)));
                    ...
                    for (int i = 0; i < RfxRilUtils::rfxGetSimCount(); i++) {
                        if (i != m_slot_id && pinDefer[i] == 1) {
                            ...
                            sp<RfxMclMessage> unsol = RfxMclMessage::obtainUrc(
                                    RFX_MSG_URC_RESPONSE_SIM_STATUS_CHANGED, i, RfxVoidData());
                ...
                if (isRjioRsuSupport()) {
                    ...
                    for (int i = 0; i < RfxRilUtils::rfxGetSimCount(); i++) {
                        if (i != m_slot_id && pinDefer[i] == 1) {
                            ...
                            responseToTelCore(unsol);
                            logD(mTag,
                                 "UICC_NP ,PIN , PUK Send SIM status changed due to SIM PIN DEFER");
                            pinDefer[i] = 0;
                        }
                    }
                }
        ...
        if (ret == UICC_NOT_READY || !getIccId(p_card_status->iccId)) {
            ...
            if (cardType & RFX_CARD_TYPE_USIM) {
                // Query USIM app info
                result =
                        getSimAppInfo(UICC_APP_USIM,
                                      &p_card_status->applications[p_card_status->num_applications],
                                      (UICC_Status)ret);
            } else if (cardType & RFX_CARD_TYPE_SIM) {
                // Query SIM app info
                result = getSimAppInfo(
                        UICC_APP_SIM, &p_card_status->applications[p_card_status->num_applications],
                        (UICC_Status)ret);
            }
            ...
            if (cardType & RFX_CARD_TYPE_CSIM) {
                // Query CSIM app info
                result =
                        getSimAppInfo(UICC_APP_CSIM,
                                      &p_card_status->applications[p_card_status->num_applications],
                                      (UICC_Status)ret);
            } else if (cardType & RFX_CARD_TYPE_RUIM) {
                // Query RUIM app info
                result =
                        getSimAppInfo(UICC_APP_RUIM,
                                      &p_card_status->applications[p_card_status->num_applications],
                                      (UICC_Status)ret);
            }
        ...
        if (!getIccId(p_card_status->iccId) ||
            (ret == UICC_NOT_READY && (count >= MAX_RETRY_COUNT))) {
            ril_error = RIL_E_SIM_ERR;
    ...
    if (ril_error == RIL_E_SUCCESS) {
        ...
        response = RfxMclMessage::obtainResponse(msg->getId(), ril_error, RfxSimStatusData(NULL, 0),
                                                 msg, false);
    }
    responseToTelCore(response);
...
void RmcCommSimRequestHandler::handleIsimAuthentication(const sp<RfxMclMessage>& msg) {
    ...
    if (RIL_E_SUCCESS == result) {
        response = RfxMclMessage::obtainResponse(
                msg->getId(), result, RfxStringData((void*)auth_rsp, strlen(auth_rsp)), msg, false);
    } else {
        response = RfxMclMessage::obtainResponse(msg->getId(), result, RfxStringData(), msg, false);
    }
    responseToTelCore(response);
    if (NULL != auth_rsp) {
        free(auth_rsp);
...
void RmcCommSimRequestHandler::handleGeneralSimAuthentication(const sp<RfxMclMessage>& msg) {
        ...
        if (pSimAuthData->mode == 0) {
            ...
            if (pSimAuthData->sessionId == 0) {
                ...
                if (pSimAuthData->param2 != NULL && strlen(pSimAuthData->param2) > 0) {
                    ...
                    cmd.append(String8::format("AT+EAUTH=\"%s\",\"%s\"", pSimAuthData->param1,
                                               pSimAuthData->param2));
                } else {
                ...
                cmd.append(String8::format("AT+ESIMAUTH=%d,%d,\"%s%s\"", pSimAuthData->sessionId,
                                           pSimAuthData->mode, pSimAuthData->param1,
                                           pSimAuthData->param2));
            }
        } else if (pSimAuthData->mode == 1) {
            // mode 1 is represent GBA authentication mode
            cmd.append(String8::format("AT+ESIMAUTH=%d,%d,\"%02x%s%s\"", pSimAuthData->sessionId,
                                       pSimAuthData->mode, pSimAuthData->tag, pSimAuthData->param1,
                                       pSimAuthData->param2));
        }
            ...
            if (err < 0 || p_response->getSuccess() == 0) {
                ...
                if (line->atTokHasmore()) {
                    ...
                    logI(mTag, "requestGeneralSimAuth response len = %zu %02x, %02x",
                         strlen(sim_auth_response.simResponse), sim_auth_response.sw1,
    ...
    if (result == RIL_E_SUCCESS) {
        response = RfxMclMessage::obtainResponse(
                msg->getId(), result,
                RfxSimIoRspData((void*)&sim_auth_response, sizeof(RIL_SIM_IO_Response)), msg,
                false);
    } else {
        response = RfxMclMessage::obtainResponse(msg->getId(), result, RfxSimIoRspData(NULL, 0),
                                                 msg, false);
    }
    responseToTelCore(response);
}
void RmcCommSimRequestHandler::handleSimAuthentication(const sp<RfxMclMessage>& msg) {
    ...
    do {
        ...
        if (out_put != NULL) {
            if ((appTypeId == UICC_APP_USIM) || (appTypeId == UICC_APP_ISIM)) {
                ...
                cmd.append(
                        String8::format("AT+CGLA=%d,%d,\"%02x%02x%02x%02x%02x%s00\"", channelId,
                                        (unsigned int)(12 + strlen((char*)out_put)),
                                        ((channelId <= 3) ? channelId : (4 * 16 + channelId - 4)),
                                        0x88, 0, pSimAuthData->authContext,
                                        (unsigned int)(strlen((char*)out_put) / 2), out_put));
            } else {
                cmd.append(String8::format("AT+CGLA=%d,%d,\"%02x%02x%02x%02x%s0C\"", channelId,
                                           (unsigned int)(10 + strlen((char*)out_put)), 0xA0, 0x88,
                                           0x00, 0x00, out_put));
            }
        ...
        if (p_response != NULL && p_response->getSuccess() > 0) {
            ...
            logI(mTag, "SIM authentication payload = %s %02x, %02x", sim_auth_response->simResponse,
    ...
    sp<RfxMclMessage> response = RfxMclMessage::obtainResponse(
            ...
            RfxSimIoRspData((void*)sim_auth_response,
                            ((sim_auth_response != NULL) ? sizeof(RIL_SIM_IO_Response) : 0)),
            msg, false);
...
int RmcCommSimRequestHandler::openChannel(const sp<RfxMclMessage>& msg, RIL_SIM_IO_Response* sr,
                                          int* len) {
    ...
    cmd.append(String8::format("AT+CGLA=%d,%d,\"%02x%02x%02x%02x%02x\"", 0, 10, 0x00, 0x70, 0x00,
                               0x00, 0x01));
    ...
    if (err < 0 || p_response->getSuccess() == 0) {
        ...
        if (p_response->getFinalResponse() != NULL) {
            ...
            } else if (!strcmp(p_response->getFinalResponse()->getLine(),
                               "+CME ERROR: NOT FOUND")) {
    ...
    if (sr->sw1 != 0x90 && sr->sw1 != 0x91) {  // wrong sw1, sw2
        if ((sr->sw1 == 0x6A && sr->sw2 == 0x82) || (sr->sw1 == 0x69 && sr->sw2 == 0x85) ||
...
int RmcCommSimRequestHandler::selectAid(RIL_SIM_IO_Response* sr, int* len, int channel, int p2,
                                        char* pAid) {
    sp<RfxAtResponse> p_response;
    String8 cmd("");
    int err, ril_error = RIL_E_SIM_ERR;
    ...
    *len = 0;
    ...
    if (aid != NULL) {
        if (ins == 0xA4 && mP2 == 0x04) {
            cmd.append(String8::format("AT+CGLA=%d,%d,\"%02x%02x%02x%02x%02x%s00\"", channel,
                                       length + 2, cla, ins, p1, mP2, p3, aid));
        } else {
            cmd.append(String8::format("AT+CGLA=%d,%d,\"%02x%02x%02x%02x%02x%s\"", channel, length,
                                       cla, ins, p1, mP2, p3, aid));
        }
    } else {
        if (ins == 0xA4 && mP2 == 0x04) {
            cmd.append(String8::format("AT+CGLA=%d,%d,\"%02x%02x%02x%02x%02x00\"", channel,
                                       length + 2, cla, ins, p1, mP2, p3));
        } else {
            cmd.append(String8::format("AT+CGLA=%d,%d,\"%02x%02x%02x%02x%02x\"", channel, length,
                                       cla, ins, p1, mP2, p3));
        }
    }
    ...
    if (sr->sw1 == 0x61) {  // GET RESPONSE setting p3 = sr.sw2
        ...
        if ((sr->sw1 == 0x6A && sr->sw2 == 0x82) || (sr->sw1 == 0x6A && sr->sw2 == 0x81) ||
            (sr->sw1 == 0x69 && sr->sw2 == 0x85) || (sr->sw1 == 0x69 && sr->sw2 == 0x99)) {
            ril_error = RIL_E_NO_SUCH_ELEMENT;
        } else if ((sr->sw1 == 0x6A && sr->sw2 == 0x84) || (sr->sw1 == 0x68 && sr->sw2 == 0x81)) {
            logE(mTag, "Not enough memory space in the file");
            ril_error = RIL_E_MISSING_RESOURCE;
        }
...
void RmcCommSimRequestHandler::handleSimOpenChannel(const sp<RfxMclMessage>& msg) {
    ...
    response = RfxMclMessage::obtainResponse(msg->getId(), (RIL_Errno)ril_error,
                                             RfxIntsData((void*)finalRsp, (i + 3) * sizeof(int)),
                                             msg, false);
...
error_select_aid:
    ...
    // <command> = 00 70 80 0X
    // X = channel
    logI(mTag, "handleSimOpenChannel: close logical channel %d due to fail to select AID", channel);
    cmd.append(String8::format("AT+CGLA=0,%d,\"%02x%02x%02x%02x\"", 8, 0x00, 0x70, 0x80, channel));
    p_response = atSendCommandSingleline(cmd, "+CGLA:");
    if (p_response == NULL) {
        goto error;
    }
...
error:
    logE(mTag, "handleSimOpenChannel, error %d, sw1 %x, sw2 %x, sr.simresponse %s", ril_error,
...
error:
    response =
            RfxMclMessage::obtainResponse(msg->getId(), (RIL_Errno)err, RfxVoidData(), msg, false);
...
void RmcCommSimRequestHandler::handleSimTransmitBasic(const sp<RfxMclMessage>& msg) {
    ...
    if ((p_args->data == NULL) || (strlen(p_args->data) == 0)) {
        ...
        if (p_args->p3 < 0) {
            cmd.append(String8::format("AT+CGLA=0,%d,\"%02x%02x%02x%02x\"", 8, p_args->cla,
                                       p_args->instruction, p_args->p1, p_args->p2));
        } else {
            cmd.append(String8::format("AT+CGLA=0,%d,\"%02x%02x%02x%02x%02x\"", 10, p_args->cla,
                                       p_args->instruction, p_args->p1, p_args->p2, p_args->p3));
        }
    } else {
        cmd.append(String8::format("AT+CGLA=0,%d,\"%02x%02x%02x%02x%02x%s\"",
                                   (unsigned int)(10 + strlen(p_args->data)), p_args->cla,
                                   p_args->instruction, p_args->p1, p_args->p2, p_args->p3,
                                   p_args->data));
    }
    p_response = atSendCommandSingleline(cmd, "+CGLA:");
...
error:
    response = RfxMclMessage::obtainResponse(msg->getId(), ril_error, RfxSimIoRspData(NULL, 0), msg,
                                             false);
...
void RmcCommSimRequestHandler::handleDetectSim(const sp<RfxMclMessage>& msg) {
    ...
    switch (cardPresent) {
        case 1:
            ...
            if (cardType & RFX_CARD_TYPE_SIM || cardType & RFX_CARD_TYPE_USIM) {
                ...
                if ((isCommontSlotSupport() == true) &&
                    (RmcCommSimUrcHandler::getVsimPlugInOutEvent(m_slot_id) ==
                ...
                // Wait for +EUSIM
                sendEvent(RFX_MSG_EVENT_SIM_DETECT_SIM, RfxStringData(urc), RIL_CMD_PROXY_1,
                          m_slot_id);
        ...
        case 0: {
            ...
            logD(mTag, "Event plug in was happened, but there is no SIM inserted actually.(%d)",
                 cardType);
            if (cardType != 0) {
            ...
            if (isCommontSlotSupport()) {
                ...
                if (getMclStatusManager()->getIntValue(RFX_STATUS_KEY_SIM_ONOFF_STATE) !=
                    ...
                    getMclStatusManager()->setString8Value(RFX_STATUS_KEY_SIM_ICCID,
                                                           String8("N/A"));
                }

                // External SIM [Start]
                if ((RmcCommSimUrcHandler::getVsimPlugInOutEvent(m_slot_id) ==
                     VSIM_TRIGGER_PLUG_IN)) {
                    logD(mTag, "vsim trigger no change on common slot project.");
                    RmcCommSimUrcHandler::setVsimPlugInOutEvent(m_slot_id, VSIM_TRIGGER_RESET);
                } else if (RfxRilUtils::isVsimEnabled()) {
                    logD(mTag,
                         "Ingore no change event during vsim enabled on common slot project.");
                } else {
                    // External SIM [End]
...
void RmcCommSimRequestHandler::handleLocalRequestUsimAuthentication(const sp<RfxMclMessage>& msg) {
    ...
    response = RfxMclMessage::obtainResponse(msg->getId(), RIL_E_SUCCESS,
                                             RfxStringData((void*)auth_rsp, strlen(auth_rsp)), msg,
                                             false);
...
void RmcCommSimRequestHandler::handleEnterNetworkDepersonalization(const sp<RfxMclMessage>& msg) {
    ...
    do {
        ...
        if (UICC_NP == sim_status || UICC_NSP == sim_status || UICC_SP == sim_status ||
            UICC_CP == sim_status || UICC_SIMP == sim_status || UICC_NS_SP == sim_status ||
            ...
            if (p_response->getSuccess() == 0) {
                switch (p_response->atGetCmeError()) {
                    ...
                    case CME_PH_FSIM_PUK_REQUIRED:
                        if (UICC_NP == sim_status || UICC_NSP == sim_status ||
                            UICC_SP == sim_status || UICC_CP == sim_status) {
                            result = RIL_E_SUCCESS;
                    ...
                    case CME_LINK_SIM_C_PIN_REQUIRED:
                        if (UICC_NP == sim_status || UICC_NSP == sim_status ||
                            UICC_SP == sim_status || UICC_CP == sim_status ||
                            UICC_SIMP == sim_status || UICC_NS_SP == sim_status) {
                            result = RIL_E_SUCCESS;
    ...
    sp<RfxMclMessage> response =
            RfxMclMessage::obtainResponse(msg->getId(), result, RfxIntsData(), msg, false);
...
void RmcCommSimRequestHandler::handleSimTransmitWithExtendedAPDU(const sp<RfxMclMessage>& msg,
    ...
    logI(mTag, "handleSimTransmitWithExtendedAPDU session:%d, cla:%d, ins:%d, p1:%d, p2:%d, p3:%d",
         p_args->sessionid, p_args->cla, p_args->instruction, p_args->p1, p_args->p2, p_args->p3);
    ...
    if ((p_args->data == NULL) || (strlen(p_args->data) == 0)) {
        if (p_args->p3 < 0) {
            cmd.append(String8::format("AT+EXCGLA=%d,%d,%d,%d,\"%02x%02x%02x%02x\"", data_offset,
                                       p_args->sessionid, 8, 8, p_args->cla, p_args->instruction,
                                       p_args->p1, p_args->p2));
        } else {
            cmd.append(String8::format("AT+EXCGLA=%d,%d,%d,%d,\"%02x%02x%02x%02x%02x\"",
                                       data_offset, p_args->sessionid, 10, 10, p_args->cla,
                                       p_args->instruction, p_args->p1, p_args->p2, p_args->p3));
        }
        ...
        do {
            ...
            if (data_offset == 0) {
                cmd.append(String8::format("AT+EXCGLA=%d,%d,%d,%d,\"%02x%02x%02x%02x%02x%s\"",
                                           data_offset, p_args->sessionid, data_len + 10,
                                           temp_cmd_data_len, p_args->cla, p_args->instruction,
                                           p_args->p1, p_args->p2, p_args->p3, temp_data));
            } else {
                cmd.append(String8::format("AT+EXCGLA=%d,%d,%d,%d,\"%s\"", data_offset + 10,
                                           p_args->sessionid, data_len + 10, temp_cmd_data_len,
                                           temp_data));
            }
...
void RmcCommSimRequestHandler::handleEnterDepersonalization(const sp<RfxMclMessage>& msg) {
    ...
    do {
        ...
        logE(mTag, "handleEnterDepersonalization netpin : %s, cat : %s, status: %d", strings[0],
             strings[1], sim_status);

        if ((UICC_NP == sim_status && (cat == 0)) || (UICC_NSP == sim_status && (cat == 1)) ||
            (UICC_SP == sim_status && (cat == 2)) || (UICC_CP == sim_status && (cat == 3)) ||
        ...
        if (isAOSPPropSupport()) {
            ...
            logD(mTag, "handleEnterDepersonalization lockCount: %s, value: %s", lockCount.string(),
                 value.string());
        }
...
error:
    response = RfxMclMessage::obtainResponse(
            msg->getId(), ret, RfxIntsData((void*)&retryCount, sizeof(retryCount)), msg, false);
    responseToTelCore(response);
}
...
void RmcCommSimRequestHandler::handleSimTransmitChannel(const sp<RfxMclMessage>& msg) {
    ...
    if ((p_args->data == NULL) || (strlen(p_args->data) == 0)) {
        ...
        if (p_args->p3 < 0) {
            cmd.append(String8::format("AT+CGLA=%d,%d,\"%02x%02x%02x%02x\"", p_args->sessionid, 8,
                                       p_args->cla, p_args->instruction, p_args->p1, p_args->p2));
        } else {
            cmd.append(String8::format("AT+CGLA=%d,%d,\"%02x%02x%02x%02x%02x\"", p_args->sessionid,
                                       10, p_args->cla, p_args->instruction, p_args->p1, p_args->p2,
                                       p_args->p3));
        }
    } else {
        cmd.append(String8::format("AT+CGLA=%d,%d,\"%02x%02x%02x%02x%02x%s\"", p_args->sessionid,
                                   (unsigned int)(10 + strlen(p_args->data)), p_args->cla,
                                   p_args->instruction, p_args->p1, p_args->p2, p_args->p3,
                                   p_args->data));
    }
...
void RmcCommSimRequestHandler::handleEsimTransmitChannel(const sp<RfxMclMessage>& msg) {
    ...
    if ((p_args->data == NULL) || (strlen(p_args->data) == 0)) {
        ...
        if (p_args->p3 < 0) {
            cmd.append(String8::format("AT+ECGLA=%d,%d,\"%02x%02x%02x%02x\"", p_args->sessionid, 8,
                                       p_args->cla, p_args->instruction, p_args->p1, p_args->p2));
        } else {
            cmd.append(String8::format("AT+ECGLA=%d,%d,\"%02x%02x%02x%02x%02x\"", p_args->sessionid,
                                       10, p_args->cla, p_args->instruction, p_args->p1, p_args->p2,
                                       p_args->p3));
        }
    } else {
        cmd.append(String8::format("AT+ECGLA=%d,%d,\"%02x%02x%02x%02x%02x%s\"", p_args->sessionid,
                                   (unsigned int)(10 + strlen(p_args->data)), p_args->cla,
                                   p_args->instruction, p_args->p1, p_args->p2, p_args->p3,
                                   p_args->data));
    }
...
error:
    ...
    response = RfxMclMessage::obtainResponse(msg->getId(), ril_error, RfxStringData(), msg, false);
...
void RmcCommSimRequestHandler::handleSimGetIccid(const sp<RfxMclMessage>& msg) {
    ...
    if (!iccid.isEmpty()) {
        response = RfxMclMessage::obtainResponse(
                msg->getId(), RIL_E_SUCCESS, RfxStringData((void*)iccid.string(), iccid.length()),
                msg, false);
    } else {
        response =
                RfxMclMessage::obtainResponse(msg->getId(), ril_error, RfxStringData(), msg, false);
    }

    responseToTelCore(response);
}

void RmcCommSimRequestHandler::handleSecurityOperation(const sp<RfxMclMessage>& msg,
    ...
    do {
        switch (op) {
            ...
            case CHANGE_PIN2:
                /*  UI shall handle CHANGE PIN only operated on PIN which is enabled and nonblocking
                 * state. */
                if (dataLen == 3 * sizeof(char*)) {
                    if (CHANGE_PIN1 == op) {
                        ...
                        if (sim_status == UICC_PIN) {
                            op = ENTER_PIN1;
                            /* Solve CR - ALPS00260076 PIN can not be changed by using SS string
                               when PIN is dismissed Modem does not support multiple AT commands, so
                               input PIN1 first */
                            cmd.clear();
                            cmd.append(String8::format("AT+CPIN=\"%s\"", strings[0]));
                            bIgnoreCmd = true;
                            ...
                            if (p_response->getSuccess() > 0) {
                                bIgnoreCmd = false;
                                cmd.clear();
                                cmd.append(String8::format("AT+CPWD=\"SC\",\"%s\",\"%s\"",
                                                           strings[0], strings[1]));
                            ...
                            cmd.clear();
                            cmd.append(String8::format("AT+CPWD=\"SC\",\"%s\",\"%s\"", strings[0],
                                                       strings[1]));
                        }
                    } else {
                        cmd.clear();
                        cmd.append(String8::format("AT+CPWD=\"P2\",\"%s\",\"%s\"", strings[0],
                                                   strings[1]));
                    }
                } else {
                    err = -1;
        ...
        if (p_response->getSuccess() == 0) {
            ...
            if (atoi(property_value) == 2) {
                // 1. Since modem need to init after unlock SIM PIN, we try to query SIM state to
                // check
                ...
                do {
                    ...
                    if (UICC_BUSY == sim_status) {
                        sleepMsec(200);
                        count++;  // to avoid block; if the busy time is too long; need to check
                                  // modem.
                        if (count == 30) {
                            break;
                        }
    ...
    sp<RfxMclMessage> response = RfxMclMessage::obtainResponse(
            msg->getId(), result, RfxIntsData((void*)&opRetryCount, sizeof(opRetryCount)), msg,
            false);
...
// BTSAP - Start
void RmcCommSimRequestHandler::decodeBtSapPayload(int msgId, void* src, size_t srclen, void* dst) {
    pb_istream_t stream;
...
int RmcCommSimRequestHandler::hexStringToByteArrayEx(unsigned char* hexString, int hexStringLen,
                                                     unsigned char** byte) {
...
void RmcCommSimRequestHandler::handleBtSapConnect(const sp<RfxMclMessage>& msg) {
    ...
    RfxAtLine* line = NULL;
    RIL_Errno ret = RIL_E_GENERIC_FAILURE;
    RIL_SIM_SAP_STATUS_IND_Status unsolMsg =
            RIL_SIM_SAP_STATUS_IND_Status_RIL_SIM_STATUS_UNKNOWN_ERROR;
    ...
    if (p_response->getSuccess() == 0) {
        ...
        logD(mTag, "[BTSAP] requestLocalSapConnect, cur_type: %d, supp_type: %d, size: %d",
             ...
             getMclStatusManager()->getIntValue(RFX_STATUS_KEY_BTSAP_SUPPORT_PROTOCOL, -1), atrLen);
...
void RmcCommSimRequestHandler::handleBtSapTransferAtr(const sp<RfxMclMessage>& msg) {
    ...
    if (getMclStatusManager()->getIntValue(RFX_STATUS_KEY_CARD_TYPE, -1) <= 0) {
        ...
        rsp.atr = NULL;
    } else if (status == BT_SAP_POWER_ON || status == BT_SAP_CONNECTION_SETUP ||
        ...
        int reslen = hexStringToByteArrayEx((unsigned char*)atrStr, atrLen, (unsigned char**)&pRes);
...
void RmcCommSimRequestHandler::handleSetSimCardPower(const sp<RfxMclMessage>& msg) {
    ...
    if (RfxRilUtils::isExternalSimSupport() == 1 &&
        ...
        if (vsimMgrExisted == false && clientId == -1 &&
            isMtkFwkAddonNotExisted(m_slot_id) == true) {
            logW(mTag, "Set sim card power(%d), vsimMgr:%d, clientId:%d", *mode, vsimMgrExisted,
                 clientId);
            if (*mode == 0) {
...
void RmcCommSimRequestHandler::handleEnterDeviceNetworkDepersonalization(
    ...
    sp<RfxMclMessage> response = RfxMclMessage::obtainResponse(
            msg->getId(), ret, RfxIntsData((void*)&op_remain_count, sizeof(op_remain_count)), msg,
            false);
    responseToTelCore(response);
}
...
void RmcCommSimRequestHandler::querySimOnOffActivation() {
    ...
    if (support == 1) {
        ...
        rfx_property_set(simOnOff.string(), String8::format("%d", SIM_POWER_STATE_SIM_ON).string());
...
void RmcCommSimRequestHandler::handleEventGetSlotStatus(const sp<RfxMclMessage>& msg) {
    ...
    for (int i = 0; i < slotNum; i++) {
        ...
        logD(mTag, "handleEventGetSlotStatus prop:%s, iccid:%s, state:%d, comp:%d", prop.string(),
             iccid, pp_slot_status[i]->card_state, strcmp(iccid, "N/A"));
    ...
    unsol = RfxMclMessage::obtainUrc(
            RFX_MSG_URC_ICC_SLOT_STATUS, m_slot_id,
...
error:
    response = RfxMclMessage::obtainResponse(msg->getId(), ril_error, RfxSimSlotStatusData(NULL, 0),
                                             msg, false);
    responseToTelCore(response);
...
void RmcCommSimRequestHandler::handleSetAllowedCarriers(const sp<RfxMclMessage>& msg) {
    ...
    int err = -1;
    String8 cmd("");
    RIL_CarrierRestrictionsWithPriority* crp =
            (RIL_CarrierRestrictionsWithPriority*)(msg->getData()->getData());
    ...
    cmd.append(String8::format("AT+ECRRST=9,\"12345678\",%d,%d", crp->simLockMultiSimPolicy,
                               crp->allowedCarriersPrioritized));
    p_response = atSendCommand(cmd);
    err = p_response->getError();
    if (err < 0 || p_response->getSuccess() == 0) {
    ...
    for (int i = 0; i < crp->len_allowed_carriers; i++) {
        ...
        if (strcmp(allowed->mcc, "") == 0 && strcmp(allowed->mnc, "") == 0) {
            ...
            cmd.append(String8::format("AT+ECRRST=2,,0,%d,\"%s%s\"", allowed->match_type,
                                       allowed->mcc, allowed->mnc));
        } else {
            cmd.append(String8::format("AT+ECRRST=2,,0,%d,\"%s%s\",\"%s\"", allowed->match_type,
                                       allowed->mcc, allowed->mnc, allowed->match_data));
        }
        p_response = atSendCommand(cmd);
        err = p_response->getError();
        if (err < 0 || p_response->getSuccess() == 0) {
    ...
    for (int i = 0; i < crp->len_excluded_carriers; i++) {
        ...
        if (excluded->match_type == RIL_MATCH_ALL) {
            cmd.append(String8::format("AT+ECRRST=2,,1,%d,\"%s%s\"", excluded->match_type,
                                       excluded->mcc, excluded->mnc));
        } else {
            cmd.append(String8::format("AT+ECRRST=2,,1,%d,\"%s%s\",\"%s\"", excluded->match_type,
                                       excluded->mcc, excluded->mnc, excluded->match_data));
        }
        p_response = atSendCommand(cmd);
        err = p_response->getError();
        if (err < 0 || p_response->getSuccess() == 0) {
    ...
    response = RfxMclMessage::obtainResponse(msg->getId(), RIL_E_SUCCESS,
                                             RfxIntsData((void*)&allowedCount, sizeof(int)), msg,
                                             false);
    responseToTelCore(response);
    return;
...
void RmcCommSimRequestHandler::handleGetAllowedCarriers(const sp<RfxMclMessage>& msg) {
    ...
    result = (RIL_CarrierRestrictionsWithPriority*)calloc(
    ...
    result->len_allowed_carriers = total_allowed_len;
    result->allowed_carriers = allowed =
            (RIL_Carrier*)calloc(total_allowed_len, sizeof(RIL_Carrier));
    for (i = 0; i < RIL_MATCH_MAX; i++) {
        for (j = 0; j < len[i]; j++) {
    ...
    result->excluded_carriers = excluded =
            (RIL_Carrier*)calloc(total_excluded_len, sizeof(RIL_Carrier));
    for (i = 0; i < RIL_MATCH_MAX; i++) {
        for (j = 0; j < len[i]; j++) {
            logI(mTag, "i:%d, j:%d", i, j);
            RIL_Carrier* p_cr = excluded_tmp[i] + j;
    ...
    for (int i = 0; i < result->len_allowed_carriers; i++) {
        ...
        logI(mTag, "allowed mcc %s, mnc %s, match_type %d, match_data %s", allowed->mcc,
             allowed->mnc, allowed->match_type, allowed->match_data);
    }
    for (int i = 0; i < result->len_excluded_carriers; i++) {
        excluded = result->excluded_carriers + i;
        logI(mTag, "excluded mcc %s, mnc %s, match_type %d, match_data %s", excluded->mcc,
             excluded->mnc, excluded->match_type, excluded->match_data);
    }

    response = RfxMclMessage::obtainResponse(
            msg->getId(), RIL_E_SUCCESS,
            RfxSimCarrRstData((void*)result, sizeof(RIL_CarrierRestrictionsWithPriority)), msg,
            false);
    responseToTelCore(response);
...
void RmcCommSimRequestHandler::handleRsuRequest(const sp<RfxMclMessage>& msg) {
    ...
    logD(mTag, "[RSU-SIMLOCK] handleRsuRequest opId = %d, requestId = %d", request->opId,
         request->requestId);
...
void RmcCommSimRequestHandler::handleKDDIRsuRequest(const sp<RfxMclMessage>& msg) {
    ...
    sp<RfxMclMessage> mclResponse = RfxMclMessage::obtainResponse(
            msg->getId(), RIL_E_REQUEST_NOT_SUPPORTED, RfxVoidData(), msg);
    responseToTelCore(mclResponse);
}
...
void RmcCommSimRequestHandler::handleKDDIRsuSetSimLockData(const sp<RfxMclMessage>& msg) {
    ...
    logD(mTag,
         "[RSU-SIMLOCK] handleKDDIRsuSetSimLockData opId = %d, requestId = %d, data_size = %d",
    ...
    do {
        ...
        logD(mTag, "[RSU-SIMLOCK] requestId %d, length %d, offset %d", requestId, data_size,
             offset);
        ...
        cmd.clear();
        cmd.append(String8::format("AT+ESMLRSU=%d,%d,%d,%d,\"%s\"", request->opId, requestId,
                                   offset, length, lock_data));
    ...
    mclResponse = RfxMclMessage::obtainResponse(
            msg->getId(), RIL_E_SUCCESS, RfxRsuResponseData(response, sizeof(RIL_RsuResponse)),
            msg);
    responseToTelCore(mclResponse);
    free(response);
    return;
...
void RmcCommSimRequestHandler::handleKDDIRsuGetLockData(const sp<RfxMclMessage>& msg,
    ...
    cmd.append(String8::format("AT+ESMLRSU=%d,%d,%d", request->opId,
                               RSU_MODEM_REQUEST_GET_LOCK_DATA, sim_lock_group));
    ...
    for (p_cur = atResponse->getIntermediates(); p_cur != NULL; p_cur = p_cur->getNext()) {
        p_cur->atTokStart(&err);
        if (err < 0) {
        ...
        for (int i = 0; i < triplet_length; i++) {
            ...
            if (tmp_buffer1 == NULL) {
                ...
                tmp_buffer2 = (char*)calloc(
                        1, ((strlen(tmp_buffer1) + strlen(temp_data)) * sizeof(char)) + 2);
...
void RmcCommSimRequestHandler::handleVzwRsuInitiate(const sp<RfxMclMessage>& msg) {
    ...
    if (err < 0 || opId != request->opId) {
        logE(mTag,
             "[RSU-SIMLOCK] handleVzwRsuInitiate err < 0 or opId = %d"
             " not equal ot request opId = %d",
             opId, request->opId);
        goto error;
    }
    ...
    if (err < 0 || requestId != RSU_MODEM_REQUEST_INIT_REQUEST) {
        logE(mTag,
             "[RSU-SIMLOCK] handleVzwRsuInitiate err < 0 or requestId = %d"
             " not equal ot request requestId = %d",
...
void RmcCommSimRequestHandler::handleVzwRsuGetSharedKey(const sp<RfxMclMessage>& msg) {
    ...
    if (err < 0 || opId != request->opId) {
        logE(mTag,
             "[RSU-SIMLOCK] handleVzwRsuGetSharedKey err < 0 or opId = %d"
             " not equal ot request opId = %d",
             opId, request->opId);
        goto error;
    }
    ...
    if (err < 0 || requestId != RSU_MODEM_REQUEST_GET_SHARED_KEY) {
        logE(mTag,
             "[RSU-SIMLOCK] handleVzwRsuGetSharedKey err < 0 or requestId = %d"
             " not equal ot request requestId = %d",
             requestId, request->requestId);
        goto error;
    }
...
void RmcCommSimRequestHandler::handleVzwRsuUpdateLockData(const sp<RfxMclMessage>& msg) {
    ...
    cmd.append(String8::format("AT+ESMLRSU=%d,%d,\"%s\"", request->opId,
                               RSU_MODEM_REQUEST_UPDATE_LOCK_DATA, request->data));
    atResponse = atSendCommandSingleline(cmd, "+ESMLRSU:");
    if (atResponse == NULL) {
    ...
    if (err < 0 || opId != request->opId) {
        logE(mTag,
             "[RSU-SIMLOCK] handleVzwRsuUpdateLockData err < 0 or opId = %d"
             " not equal ot request opId = %d",
             opId, request->opId);
        goto error;
    }
    ...
    if (err < 0 || requestId != RSU_MODEM_REQUEST_UPDATE_LOCK_DATA) {
        logE(mTag,
             "[RSU-SIMLOCK] handleVzwRsuUpdateLockData err < 0 or requestId = %d"
             " not equal ot request requestId = %d",
             requestId, request->requestId);
        goto error;
    }
...
void RmcCommSimRequestHandler::handleVzwRsuGetVersion(const sp<RfxMclMessage>& msg) {
    ...
    cmd.append(
            String8::format("AT+ESMLRSU=%d,%d", request->opId, RSU_MODEM_REQUEST_GET_LOCK_VERSION));
    atResponse = atSendCommandSingleline(cmd, "+ESMLRSU:");
    if (atResponse == NULL) {
    ...
    if (err < 0 || opId != request->opId) {
        logE(mTag,
             "[RSU-SIMLOCK] handleVzwRsuGetVersion err < 0 or opId = %d"
             " not equal ot request opId = %d",
             opId, request->opId);
        goto error;
    }
    ...
    if (err < 0 || requestId != RSU_MODEM_REQUEST_GET_LOCK_VERSION) {
        logE(mTag,
             "[RSU-SIMLOCK] handleVzwRsuGetVersion err < 0 or requestId = %d"
             " not equal ot request requestId = %d",
             requestId, request->requestId);
        goto error;
    }
...
void RmcCommSimRequestHandler::handleVzwRsuGetStatus(const sp<RfxMclMessage>& msg) {
    ...
    cmd.append(
            String8::format("AT+ESMLRSU=%d,%d", request->opId, RSU_MODEM_REQUEST_GET_LOCK_STATUS));
    atResponse = atSendCommandSingleline(cmd, "+ESMLRSU:");
    if (atResponse == NULL) {
    ...
    if (err < 0 || opId != request->opId) {
        logE(mTag,
             "[RSU-SIMLOCK] handleVzwRsuGetStatus err < 0 or opId = %d"
             " not equal ot request opId = %d",
             opId, request->opId);
        goto error;
    }
    ...
    if (err < 0 || requestId != RSU_MODEM_REQUEST_GET_LOCK_STATUS) {
        logE(mTag,
             "[RSU-SIMLOCK] handleVzwRsuGetStatus err < 0 or requestId = %d"
             " not equal ot request requestId = %d",
             requestId, request->requestId);
        goto error;
    }
...
void RmcCommSimRequestHandler::handleVzwRsuUnlockTimer(const sp<RfxMclMessage>& msg) {
    ...
    cmd.append(String8::format("AT+ESMLRSU=%d,%d,%d", request->opId, RSU_MODEM_REQUEST_UNLOCK_TIMER,
                               (request->requestType == 2 ? 0 : 1)));
    atResponse = atSendCommandSingleline(cmd, "+ESMLRSU:");
    if (atResponse == NULL) {
    ...
    if (err < 0 || opId != request->opId) {
        logE(mTag,
             "[RSU-SIMLOCK] handleVzwRsuUnlockTimer er < 0 or opId = %d"
             " not equal ot request opId = %d",
             opId, request->opId);
        goto error;
    }
    ...
    if (err < 0 || requestId != RSU_MODEM_REQUEST_UNLOCK_TIMER) {
        logE(mTag,
             "[RSU-SIMLOCK] handleVzwRsuUnlockTimer err < 0 or requestId = %d"
             " not equal ot request requestId = %d",
             requestId, request->requestId);
        goto error;
    }
...
void RmcCommSimRequestHandler::handleTmoRsuGetRegisterRequest(const sp<RfxMclMessage>& msg) {
    ...
    if (err < 0 || opId != request->opId) {
        logE(mTag,
             "[RSU-SIMLOCK] handleTmoRsuSmlGetRegisterRequest err < 0 or opId = %d"
             " not equal ot request opId = %d",
             opId, request->opId);
        goto error;
    }
    ...
    if (err < 0 || requestId != RSU_MODEM_REQUEST_GET_SHARED_KEY) {
        logE(mTag,
             "[RSU-SIMLOCK] handleTmoRsuSmlGetRegisterRequest err < 0 or requestId = %d"
             " not equal ot request requestId = %d",
             requestId, request->requestId);
        goto error;
    }
...
void RmcCommSimRequestHandler::handleTmoRsuCreateUnlockRequest(const sp<RfxMclMessage>& msg) {
    ...
    if (err < 0 || opId != request->opId) {
        logE(mTag,
             "[RSU-SIMLOCK] handleTmoRsuSmlCreateUnlockRequest err < 0 or opId = %d"
             " not equal ot request opId = %d",
             opId, request->opId);
        goto error;
    }
    ...
    if (err < 0 || requestId != RSU_MODEM_REQUEST_INIT_REQUEST) {
        logE(mTag,
             "[RSU-SIMLOCK] handleTmoRsuSmlCreateUnlockRequest err < 0 or requestId = %d"
             " not equal ot request requestId = %d",
             requestId, request->requestId);
        goto error;
    }
    ...
    if (err < 0 || generateRequest == NULL) {
        logE(mTag,
             "[RSU-SIMLOCK] handleTmoRsuSmlCreateUnlockRequest err<0 or"
...
void RmcCommSimRequestHandler::handleTmoRsuProcessMessage(const sp<RfxMclMessage>& msg) {
    ...
    if (request->data != NULL) {
        cmd.append(String8::format("AT+ESMLRSU=%d,%d,\"%s\",%d,\"%s\"", request->opId,
                                   ...
                                   (int)strlen(request->data), request->data));
    } else {
        cmd.append(String8::format("AT+ESMLRSU=%d,%d,\"%s\",%d,\"%s\"", request->opId,
    ...
    if (err < 0 || opId != request->opId) {
        logE(mTag,
             "[RSU-SIMLOCK] handleTmoRsuSmlProcessMessage err < 0 or opId = %d"
             " not equal ot request opId = %d",
             opId, request->opId);
        goto error;
    }
    ...
    if (err < 0 || requestId != RSU_MODEM_REQUEST_UPDATE_LOCK_DATA) {
        logE(mTag,
             "[RSU-SIMLOCK] handleTmoRsuSmlProcessMessage err < 0 or requestId = %d"
             " not equal ot request requestId = %d",
             requestId, request->requestId);
        goto error;
    }
...
void RmcCommSimRequestHandler::handleTmoRsuGetAdapterConfig(const sp<RfxMclMessage>& msg) {
    ...
    if (err < 0 || opId != request->opId) {
        logE(mTag,
             "[RSU-SIMLOCK] handleTmoRsuSmlGetAdapterConfig err < 0 or opId = %d"
             " not equal ot request opId = %d",
             opId, request->opId);
        goto error;
    }
    ...
    if (err < 0 || requestId != RSU_MODEM_REQUEST_GET_LOCK_VERSION) {
        logE(mTag,
             "[RSU-SIMLOCK] handleTmoRsuSmlGetAdapterConfig err < 0 or requestId = %d"
             " not equal ot request requestId = %d",
             requestId, request->requestId);
        goto error;
    }
...
void RmcCommSimRequestHandler::handleTmoRsuGetSimlockStatus(const sp<RfxMclMessage>& msg) {
    ...
    if (err < 0 || opId != request->opId) {
        logE(mTag,
             "[RSU-SIMLOCK] handleTmoRsuSmlGetSimlockStatus err < 0 or opId = %d"
             " not equal ot request opId = %d",
             opId, request->opId);
        goto error;
    }
    ...
    if (err < 0 || requestId != RSU_MODEM_REQUEST_GET_LOCK_STATUS) {
        logE(mTag,
             "[RSU-SIMLOCK] handleTmoRsuSmlGetSimlockStatus err < 0 or requestId = %d"
             " not equal ot request requestId = %d",
             requestId, request->requestId);
        goto error;
    }
