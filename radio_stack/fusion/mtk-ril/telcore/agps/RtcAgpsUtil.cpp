...
RtcAgpsMessage::~RtcAgpsMessage() {
    RFX_LOG_V(AGPS_TAG, "[RtcAgpsMessage]destroyMessage %d, 0x%zu", m_id, (size_t)m_parcel);
    if (m_parcel != NULL) {
        delete (m_parcel);
        m_parcel = NULL;
    }
...
sp<RtcAgpsMessage> RtcAgpsMessage::obtainMessage(int32_t id, Parcel* parcel) {
    ...
    RFX_LOG_V(AGPS_TAG, "[RtcAgpsMessage]obtainMessage %d, 0x%zu", id, (size_t)parcel);
    new_msg->m_id = id;
 ...
 *****************************************************************************/
void RtcAgpsHandler::sendMessage(sp<Looper> looper) { looper->sendMessage(this, m_dummyMsg); }
