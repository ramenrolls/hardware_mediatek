...
static const char PROPERTY_ICCID_SIM[4][25] = {"vendor.ril.iccid.sim1", "vendor.ril.iccid.sim2",
                                               "vendor.ril.iccid.sim3", "vendor.ril.iccid.sim4"};

static const char* DISABLE_CDMA_RAT_MCCMNC[] = {"51009", "51028", "45006"};
...
void RtcRatSwitchController::onInit() {
    ...
    const int request_id_list[] = {
            RFX_MSG_REQUEST_SET_PREFERRED_NETWORK_TYPE, RFX_MSG_REQUEST_GET_PREFERRED_NETWORK_TYPE,
            RFX_MSG_REQUEST_VOICE_RADIO_TECH, RFX_MSG_REQUEST_GET_GMSS_RAT_MODE,
            RFX_MSG_REQUEST_LOCAL_ABORT_AVAILABLE_NETWORK};

    const int atci_request_id_list[] = {RIL_LOCAL_REQUEST_OEM_HOOK_ATCI_INTERNAL};

    const int urc_id_list[] = {RFX_MSG_URC_GMSS_RAT_CHANGED};
    ...
    for (int slotId = RFX_SLOT_ID_0; slotId < RFX_SLOT_COUNT; slotId++) {
        getStatusManager(slotId)->registerStatusChangedEx(
                ...
                RfxStatusChangeCallbackEx(this,
                                          &RtcRatSwitchController::onApVoiceCallCountChanged));
    }
    ...
    if (isHvolteSupport()) {
        getStatusManager()->registerStatusChanged(
                ...
                RfxStatusChangeCallback(this,
                                        &RtcRatSwitchController::onImsRegistrationStatusChanged));
    }
#endif
    getStatusManager()->registerStatusChanged(
...
void RtcRatSwitchController::setPreferredNetworkType(const sp<RfxMessage>& message) {
    ...
    int cardType = getStatusManager(m_slot_id)->getIntValue(RFX_STATUS_KEY_CARD_TYPE, 0);
    if (cardType == CARD_TYPE_NONE) {
        logD(RAT_CTRL_TAG, "setPreferredNetworkType: return directly because no sim.");
        sp<RfxMessage> resToRilj = RfxMessage::obtainResponse(RIL_E_INTERNAL_ERR, message, false);
        responseToRilj(resToRilj);
        return;
    }

    int nwType = ((int*)message->getData()->getData())[0];
    // No matter switch success or fail, always record the rilj prefer network type in system
    // property. This is consistent with the UI behavior.
...
void RtcRatSwitchController::setPreferredNetworkType(RatSwitchInfo ratSwtichInfo) {
    logV(RAT_CTRL_TAG,
         "[setPreferredNetworkType] ratSwtichInfo.card_type :%d, "
         "ratSwtichInfo.card_state: %d, ratSwtichInfo.rat_mode: %d, ratSwtichInfo.isCt3GDualMode: "
         "%s, ratSwtichInfo.ct3gStatus : %d. ",
    ...
    // Only card not plug in/out and not occur sim switch, the card state will be
    // card_state_no_changed
    int defaultNetworkType = calculateDefaultNetworkType(ratSwtichInfo);
    if (ratSwtichInfo.card_state == CARD_STATE_NO_CHANGED &&
        defaultNetworkType == mDefaultNetworkType) {
    ...
    if (ratSwtichInfo.card_type == 0 &&
        ...
        int slotCapability =
                getStatusManager(m_slot_id)->getIntValue(RFX_STATUS_KEY_SLOT_CAPABILITY, 0);
    ...
    if (targetRaf == 0) {
        logD(RAT_CTRL_TAG,
             "[setPreferredNetworkType] Raf filter result is 0, "
             "so use defaultRaf as targetRaf");
        targetRaf = defaultRaf;
    }

    if (ratSwtichInfo.isCt3GDualMode && ratSwtichInfo.card_state == CARD_STATE_CARD_TYPE_CHANGED &&
    ...
    logD(RAT_CTRL_TAG,
         "[setPreferredNetworkType] rafFromRiljNetworkType: %d, targetRaf: %d, targetNetworkType: "
         "%d, "
...
void RtcRatSwitchController::setPreferredNetworkType(const int prefNwType,
    ...
    if (prefNwType == -1) {
        ...
        if (mPendingInitRatSwitchRecord.prefNwType != -1) {
            ...
            switchNwRat(mPendingInitRatSwitchRecord.prefNwType, RAT_SWITCH_INIT, action, NULL);
            ...
            int targetPrefNwType =
                    filterPrefNwTypeFromRilj(mPendingNormalRatSwitchRecord.prefNwType);
            logD(RAT_CTRL_TAG, "[setRestrictedNetworkMode] Norm pending record in queue");
            mPendingRestrictedRatSwitchRecord.prefNwType = -1;
            switchNwRat(targetPrefNwType, RAT_SWITCH_NORMAL, action,
        ...
        logD(RAT_CTRL_TAG, "[setRestrictedNetworkMode] entering restricted mode: %d", prefNwType);
        if (mPendingInitRatSwitchRecord.prefNwType == -1 &&
...
void RtcRatSwitchController::switchNwRat(int prefNwType, const RatSwitchCaller ratSwitchCaller,
    ...
    logV(RAT_CTRL_TAG,
         "[switchNwRat] CurPreferedNwType: %d, prefNwType: %d, NW scan:%d,"
    ...
    if (sIsInSwitching) {
        ...
        if (ratSwitchCaller == RAT_SWITCH_NORMAL) {
            ...
            if (targetPrefNwType == -1) {
                ...
                if (message != NULL) {
                    sp<RfxMessage> resToRilj =
                            RfxMessage::obtainResponse(RIL_E_GENERIC_FAILURE, message);
                    responseToRilj(resToRilj);
                }
                processPendingRatSwitchRecord();
                return;
        ...
        if (targetPrefNwType == mCurPreferedNetWorkType && ratSwitchCaller != RAT_SWITCH_INIT) {
            logV(RAT_CTRL_TAG, "[switchNwRat] Already in desired mode:%d, switch not executed",
                 targetPrefNwType);
            if (action != NULL) {
                action->act();
            }
        ...
        if (isNetworkScanOngoing() != -1) {
            ...
            sp<RfxMessage> reqToRild = RfxMessage::obtainRequest(
                    isNetworkScanOngoing(), RFX_MSG_REQUEST_LOCAL_ABORT_AVAILABLE_NETWORK,
                    RfxVoidData());
            requestToMcl(reqToRild);
            return;
        }
        logD(RAT_CTRL_TAG,
             "[switchNwRat] ratSwitchCaller: %s, mCurPreferedNetWorkType: %d, "
             "mDefaultNetworkType: %d, prefNwTypeFromRilj: %d, "
             "targetPrefNwType: %d, GsmOnlySim: %d",
             switchCallerToString(ratSwitchCaller), mCurPreferedNetWorkType, mDefaultNetworkType,
        ...
        if (mRatSettings.ratSwitchCaller == RAT_SWITCH_INIT && isRafContainsCdma(targetRaf) &&
            isRafContainsGsm(targetRaf)) {
            getGmssRatMode();
            ...
            if (isRafContainsCdma(targetRaf) && isRafContainsGsm(targetRaf)) {
                ...
                configRatMode();
                    ...
                    mRatSettings.ratSwitchCaller == RAT_SWITCH_NORMAL) &&
                   isGsmOnlySim() &&
                   (!isRafContainsCdma(targetRaf) && isRafContainsCdma(currentRaf))) {
...
void RtcRatSwitchController::configRatMode() {
    sp<RfxMessage> reqToRild =
            RfxMessage::obtainRequest(m_slot_id, RFX_MSG_REQUEST_SET_PREFERRED_NETWORK_TYPE,
                                      RfxIntsData(&mRatSettings.prefNwType, 1));
    requestToMcl(reqToRild);
}
...
void RtcRatSwitchController::queueRatSwitchRecord(int prefNwType,
    ...
    logV(RAT_CTRL_TAG, "queueRatSwitchRecord(), ratSwitchCaller:%d prefNwType:%d", ratSwitchCaller,
         prefNwType);
    if (ratSwitchCaller == RAT_SWITCH_RESTRICT) {
        ...
        if (mPendingNormalRatSwitchRecord.prefNwType != -1 &&
            ...
            logV(RAT_CTRL_TAG,
                 "queueRatSwitchRecord(), set prefer network type is pending, "
...
void RtcRatSwitchController::processPendingRatSwitchRecord() {
    ...
    int prefNwType = -1;
    if (mPendingRestrictedRatSwitchRecord.prefNwType != -1 && mInRestrictedMode != true) {
        logD(RAT_CTRL_TAG,
             "[processPendingRestrictedRatSwitchRecord] "
        ...
        mPendingRestrictedRatSwitchRecord.prefNwType = -1;
        switchNwRat(prefNwType, mPendingRestrictedRatSwitchRecord.ratSwitchCaller,
        ...
        logD(RAT_CTRL_TAG,
             "[processPendingInitRatSwitchRecord] "
             "prefNwType: %d, ratSwitchCaller: %s",
        ...
        switchNwRat(prefNwType, mPendingInitRatSwitchRecord.ratSwitchCaller,
                    mPendingInitRatSwitchRecord.action, mPendingInitRatSwitchRecord.message);
    } else if (mPendingNormalRatSwitchRecord.prefNwType != -1) {
        ...
        mPendingNormalRatSwitchRecord.prefNwType = -1;
        switchNwRat(prefNwType, mPendingNormalRatSwitchRecord.ratSwitchCaller,
                    mPendingNormalRatSwitchRecord.action, mPendingNormalRatSwitchRecord.message);
    } else {
...
int RtcRatSwitchController::calculateDefaultNetworkType(RatSwitchInfo ratSwtichInfo) {
    ...
    if (isRafContainsCdma(defaultRaf)) {
        if ((((ratSwtichInfo.card_type & CARD_TYPE_RUIM) > 0) ||
             ((ratSwtichInfo.card_type & CARD_TYPE_CSIM) > 0)) &&
    ...
    int defaultNetworkType = RtcCapabilitySwitchUtil::getNetworkTypeFromRaf(defaultRaf);

    logD(RAT_CTRL_TAG,
         "[calculateDefaultNetworkType] card_type: %d, "
         "card_state: %d, rat_mode: %d, isCt3GDualMode: %s, ct3gStatus: %d. "
         " slotCap: %d, slotNwtype: %d, defaultRaf: %d, defNwType: %d.",
         ratSwtichInfo.card_type, ratSwtichInfo.card_state, ratSwtichInfo.rat_mode,
         ratSwtichInfo.isCt3GDualMode ? "true" : "false", ratSwtichInfo.ct3gStatus, slotCapability,
         RtcCapabilitySwitchUtil::getNetworkTypeFromRaf(slotCapability), defaultRaf,
         defaultNetworkType);
...
void RtcRatSwitchController::getPreferredNetworkType(const sp<RfxMessage>& message) {
    ...
    if ((mRestrictedCount > 0 || isAPInCall() == true) &&
        ...
        logD(RAT_CTRL_TAG, "[handleGetPreferredNwType] in restricted or call mode:%d",
             mPendingNormalRatSwitchRecord.prefNwType);
        resToRilj = RfxMessage::obtainResponse(
                m_slot_id, RFX_MSG_REQUEST_GET_PREFERRED_NETWORK_TYPE, RIL_E_SUCCESS,
                RfxIntsData(&mPendingNormalRatSwitchRecord.prefNwType, 1), message);
        ...
        resToRilj = RfxMessage::obtainResponse(
                m_slot_id, RFX_MSG_REQUEST_GET_PREFERRED_NETWORK_TYPE, RIL_E_SUCCESS,
                RfxIntsData(&mRatSettings.prefNwType, 1), message);
        responseToRilj(resToRilj);
        return;
        ...
        resToRilj = RfxMessage::obtainResponse(
                m_slot_id, RFX_MSG_REQUEST_GET_PREFERRED_NETWORK_TYPE, RIL_E_SUCCESS,
                RfxIntsData(&mRatSettings.prefNwTypeFromRilj, 1), message);
        responseToRilj(resToRilj);
    } else {
        ...
        logD(RAT_CTRL_TAG, "[handleGetPreferredNwType] preferredNetworkTypeFromProp :%d",
             preferredNetworkTypeFromProp);
        resToRilj = RfxMessage::obtainResponse(
                m_slot_id, RFX_MSG_REQUEST_GET_PREFERRED_NETWORK_TYPE, RIL_E_SUCCESS,
                RfxIntsData(&preferredNetworkTypeFromProp, 1), message);
        responseToRilj(resToRilj);
    }
}
...
void RtcRatSwitchController::requestVoiceRadioTech(const sp<RfxMessage>& message) {
    sp<RfxMessage> resToRilj;
    resToRilj = RfxMessage::obtainResponse(m_slot_id, RFX_MSG_REQUEST_VOICE_RADIO_TECH,
                                           RIL_E_SUCCESS, RfxIntsData(&mPhoneMode, 1), message);
    responseToRilj(resToRilj);
}
...
void RtcRatSwitchController::responseSetPreferredNetworkType(const sp<RfxMessage>& response) {
    ...
    if (error == RIL_E_SUCCESS) {
        ...
        if (mRatSettings.message != NULL) {
            sp<RfxMessage> resToRilj =
                    RfxMessage::obtainResponse(error, mRatSettings.message, false);
            responseToRilj(resToRilj);
        }
        ...
        if (error == RIL_E_OPERATION_NOT_ALLOWED) {
            ...
            if (mRatSettings.ratSwitchCaller == RAT_SWITCH_NORMAL &&
                ...
                if (mRatSettings.message != NULL) {
                    sp<RfxMessage> resToRilj =
                            RfxMessage::obtainResponse(RIL_E_SUCCESS, mRatSettings.message);
                    responseToRilj(resToRilj);
...
bool RtcRatSwitchController::onPreviewMessage(const sp<RfxMessage>& message) {
    if (sIsInSwitching == true && message->getId() == RFX_MSG_REQUEST_VOICE_RADIO_TECH) {
...
void RtcRatSwitchController::onApVoiceCallCountChanged(int slotId, RfxStatusKeyEnum key,
    ...
    logD(RAT_CTRL_TAG, "%s, slotId:%d, key:%d oldMode:%d, mode:%d", __FUNCTION__, slotId, key,
         oldMode, mode);
    if (mode == 1 && oldMode == 0 && isNetworkScanOngoing() != -1) {
        sp<RfxMessage> reqToRild = RfxMessage::obtainRequest(
                isNetworkScanOngoing(), RFX_MSG_REQUEST_LOCAL_ABORT_AVAILABLE_NETWORK,
                RfxVoidData());
        requestToMcl(reqToRild);
        ...
        if (mRestrictedCount == 0 && isAPInCall() == false &&
            (mPendingInitRatSwitchRecord.prefNwType != -1 ||
             mPendingNormalRatSwitchRecord.prefNwType != -1) &&
            mInRestrictedMode == false) {
            logV(RAT_CTRL_TAG, "%s, slotId:%d, key:%d oldMode:%d, mode:%d", __FUNCTION__, slotId,
                 key, oldMode, mode);
            processPendingRatSwitchRecord();
        }
    }
...
void RtcRatSwitchController::onImsRegistrationStatusChanged(RfxStatusKeyEnum key,
    ...
    if (oldMode != mode && isNetworkTypeContainLteCdma()) {
        sp<RfxMessage> message = RfxMessage::obtainRequest(
                m_slot_id, RFX_MSG_REQUEST_GET_GMSS_RAT_MODE, RfxVoidData());
...
bool RtcRatSwitchController::isNoSimInserted() {
    for (int i = 0; i < RFX_SLOT_COUNT; i++) {
        int cardType = getStatusManager(i)->getIntValue(RFX_STATUS_KEY_CARD_TYPE, 0);
...
bool RtcRatSwitchController::isGsmOnlySim() {
    ...
    if ((nCardType == RFX_CARD_TYPE_SIM || nCardType == RFX_CARD_TYPE_USIM ||
...
void RtcRatSwitchController::setNwTypeCacheByIndex(int index, int type) {
    logV(RAT_CTRL_TAG, "setNwTypeCacheByIndex::index:%d type:%d cache_nwtype:%d", index, type,
         mNetworkTypeCache);
...
PsRatFamily RtcRatSwitchController::getPsRatFamily(int radioTechnology) {
    if (radioTechnology == RADIO_TECH_GPRS || radioTechnology == RADIO_TECH_EDGE ||
        radioTechnology == RADIO_TECH_UMTS || radioTechnology == RADIO_TECH_HSDPA ||
        radioTechnology == RADIO_TECH_HSUPA || radioTechnology == RADIO_TECH_HSPA ||
        radioTechnology == RADIO_TECH_LTE || radioTechnology == RADIO_TECH_HSPAP ||
        radioTechnology == RADIO_TECH_GSM || radioTechnology == RADIO_TECH_TD_SCDMA ||
        radioTechnology == RADIO_TECH_LTE_CA) {
        return PS_RAT_FAMILY_GSM;
    } else if (radioTechnology == RADIO_TECH_IS95A || radioTechnology == RADIO_TECH_IS95B ||
               radioTechnology == RADIO_TECH_1xRTT || radioTechnology == RADIO_TECH_EVDO_0 ||
               radioTechnology == RADIO_TECH_EVDO_A || radioTechnology == RADIO_TECH_EVDO_B ||
...
void RtcRatSwitchController::getGmssRatMode() {
    sp<RfxMessage> reqToRild =
            RfxMessage::obtainRequest(m_slot_id, RFX_MSG_REQUEST_GET_GMSS_RAT_MODE, RfxVoidData());
    requestToMcl(reqToRild);
}
...
void RtcRatSwitchController::handleGmssRat(const sp<RfxMessage>& message) {
    ...
    if (gmssInfo.rat == GMSS_RAT_INVALID && gmssInfo.mcc == 0 &&
        gmssInfo.status == GMSS_STATUS_SELECT && gmssInfo.mspl == MSPL_RAT_NONE &&
    ...
    if (gmssInfo.status == GMSS_STATUS_ECC) {
        if (gmssInfo.rat == GMSS_RAT_3GPP2 || gmssInfo.rat == GMSS_RAT_C2K1X ||
            gmssInfo.rat == GMSS_RAT_C2KHRPD) {
            mNwsMode = NWS_MODE_CDMALTE;
        } else if (gmssInfo.rat == GMSS_RAT_GSM || gmssInfo.rat == GMSS_RAT_WCDMA) {
            mNwsMode = NWS_MODE_CSFB;
        } else {
            if (gmssInfo.ishome == false || isCdmaDualModeSimCard() == false) {
                mNwsMode = NWS_MODE_CSFB;
            } else {
                mNwsMode = NWS_MODE_CDMALTE;
            }
        ...
        if (RFX_FLAG_HAS(gmssInfo.mspl, MSPL_RAT_C2K) &&
            ...
            if (gmssInfo.status == GMSS_STATUS_ATTACHED) {
                if (GMSS_RAT_3GPP2 == gmssInfo.rat || GMSS_RAT_C2K1X == gmssInfo.rat ||
                    GMSS_RAT_C2KHRPD == gmssInfo.rat) {
                    mNwsMode = NWS_MODE_CDMALTE;
                } else if (GMSS_RAT_GSM == gmssInfo.rat || GMSS_RAT_WCDMA == gmssInfo.rat) {
                    mNwsMode = NWS_MODE_CSFB;
                } else {
                    // EUTRAN (LTE)
            ...
            if (isHvolteSupport() && isNetworkTypeContainLteCdma()) {
                volteRegistration =
                        getStatusManager()->getIntValue(RFX_STATUS_KEY_IMS_REGISTRATION_STATE, 0);
                if ((volteRegistration & VOLTE_REGISTERATION_STATE) == VOLTE_REGISTERATION_STATE) {
                    mNwsMode = NWS_MODE_CSFB;
                } else {
                    mNwsMode = NWS_MODE_CDMALTE;
                }
    ...
    getStatusManager()->setIntValue(RFX_STATUS_KEY_NWS_MODE, mNwsMode);
    logD(RAT_CTRL_TAG,
         "[handleGmssRat] rat:%d, mcc:%d, status:%d, mspl:%d, ishome:%d, NWS mode:%d,"
         "volteRegistration: %d.",
...
void RtcRatSwitchController::onVolteStateChanged(RfxStatusKeyEnum key, RfxVariant old_value,
    ...
    if (old_mode != new_mode && new_mode == 1 && isNetworkTypeContainLteCdma() && isCT4GCard()) {
...
bool RtcRatSwitchController::needDisableCdmaRat() {
    ...
    return (!mccmnc_gsm.isEmpty() &&
            needDisableCdmaRatByMccMnc((const char*)mccmnc_gsm.string())) ||
           (!mccmnc_cdma.isEmpty() &&
