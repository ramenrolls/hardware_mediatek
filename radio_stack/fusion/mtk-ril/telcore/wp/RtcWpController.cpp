    ...
    for (int i = 0; i < RfxRilUtils::rfxGetSimCount(); i++) {
        ...
        getStatusManager(i)->registerStatusChangedEx(
                ...
                RfxStatusChangeCallbackEx(this,
                                          &RtcWpController::onTelephonyAssistantStatusChanged));
    }

    closeRadioCount = 0;
    worldModeBlockedNum = 0;
    getNonSlotScopeStatusManager()->setIntValue(RFX_STATUS_KEY_WORLD_MODE_STATE, WP_SWITCHING_END);
    getNonSlotScopeStatusManager()->setIntValue(RFX_STATUS_KEY_WORLD_MODE_BLOCKED_STATE,
                                                NO_BLOCK_WP_CHANGED);
    getNonSlotScopeStatusManager()->setIntValue(RFX_STATUS_KEY_WORLD_MODE_BLOCKED_CHANGING_STATE,
                                                NO_BLOCKING_CHANGING);
    ...
    if (msgId == RFX_MSG_URC_WORLD_MODE_CHANGED) {
        ...
        if (state == 0) {
            ...
            if (blockedState == NO_BLOCK_WP_CHANGED) {
                ...
                } else if (blockStatusKeyValue ==
                           WP_LOCK_BY_WP) {  // world mode not end, modem send URC again
                    // when request response back, to resume world mode change
                    if ((modeState == WP_SWITCHING_START) ||
                        (modeState == WP_SWITCHING_SECOND_URC)) {
                        getNonSlotScopeStatusManager()->setIntValue(RFX_STATUS_KEY_WORLD_MODE_STATE,
                                                                    WP_SWITCHING_SECOND_URC);
                    }
                }
            } else {
...
int RtcWpController::resumeBlockedWorldModeChanged() {
    worldModeBlockedNum--;
    logD(WP_LOG_TAG, "resumeBlockedWorldModeChanged(), worldModeBlockedNum=%d",
         worldModeBlockedNum);
    if (0 == worldModeBlockedNum) {
        ...
        int blockedChangingState = getNonSlotScopeStatusManager()->getIntValue(
                RFX_STATUS_KEY_WORLD_MODE_BLOCKED_CHANGING_STATE, NO_BLOCKING_CHANGING);
        logD(WP_LOG_TAG, "resumeBlockedWorldModeChanged(), blockedChangingState=%d",
             blockedChangingState);
...
void RtcWpController::createNewMessageToRmc(int messageId, int slotId, int value) {
    int reserved[1];
    reserved[0] = value;
    sp<RfxMessage> request = RfxMessage::obtainRequest(slotId, messageId, RfxIntsData(reserved, 1));
...
void RtcWpController::resumeWorldModeChange() {
    ...
    sp<RfxMessage> request = RfxMessage::obtainRequest(
            mainSlotId, RFX_MSG_REQUEST_WORLD_MODE_RESUMING, RfxIntsData(reserved, 1));
    requestToMcl(request);
}
...
void RtcWpController::requestRadioOn() {
    for (int slotId = RFX_SLOT_ID_0; slotId < RfxRilUtils::rfxGetSimCount(); slotId++) {
        ...
        sp<RfxAction> action0 =
                new RfxAction1<int>(this, &RtcWpController::onRequestRadioOnDone, slotId);
...
void RtcWpController::requestRadioOff() {
    ...
    for (int slotId = RFX_SLOT_ID_0; slotId < RfxRilUtils::rfxGetSimCount(); slotId++) {
        backupRadioPower[slotId] =
                getStatusManager(slotId)->getBoolValue(RFX_STATUS_KEY_REQUEST_RADIO_POWER, false);
        logD(WP_LOG_TAG, "backupRadioPower slotid=%d %d", slotId, backupRadioPower[slotId]);
        sp<RfxAction> action0 =
                new RfxAction1<int>(this, &RtcWpController::onRequestRadioOffDone, slotId);
        RtcRadioController* radioController =
                (RtcRadioController*)findController(slotId, RFX_OBJ_CLASS_INFO(RtcRadioController));
...
void RtcWpController::resumeWorldModeChangeWithRadioOff() {
    ...
    getNonSlotScopeStatusManager()->setIntValue(RFX_STATUS_KEY_WORLD_MODE_STATE,
                                                WP_SWITCHING_START);
    sp<RfxMessage> startUrc = RfxMessage::obtainUrc(mainSlotId, RFX_MSG_URC_WORLD_MODE_CHANGED,
                                                    RfxIntsData(status, 1));
    responseToRilj(startUrc);
    requestRadioOff();
}

void RtcWpController::onCallCountChanged(int slotId, RfxStatusKeyEnum key, RfxVariant old_value,
                                         RfxVariant value) {
    RFX_UNUSED(key);
...
void RtcWpController::onRetryResumeTimeOut() {
    ...
    if (mRetryResumeCount < WP_COMMAND_MAX_RETRY_COUNT) {
        mRetryResumeCount++;
        mWpTimerHandle =
                RfxTimer::start(RfxCallback0(this, &RtcWpController::resumeWorldModeChange),
                                ms2ns(WP_COMMAND_RETRY_TIME));
    }
}
...
void RtcWpController::processWorldModeResumingResponse(const sp<RfxMessage>& message) {
    ...
    mRetryResumeCount = 0;
    modeChangingState = getNonSlotScopeStatusManager()->getIntValue(RFX_STATUS_KEY_WORLD_MODE_STATE,
                                                                    WP_SWITCHING_END);
    rfx_property_get("persist.vendor.radio.wm_state", worldModeState, "1");
    logD(WP_LOG_TAG,
         "ResumingResponse, state=%d, status=%d, mode=%d, modeChangingState=%d, worldModeState=%s",
...
bool RtcWpController::onCheckIfRejectMessage(const sp<RfxMessage>& message, bool isModemPowerOff,
                                             int radioState) {
    int msgId = message->getId();
    int slotId = message->getSlotId();
...
bool RtcWpController::isWorldModeSwitching() {
    ...
    modeState = getNonSlotScopeStatusManager()->getIntValue(RFX_STATUS_KEY_WORLD_MODE_STATE,
                                                            WP_SWITCHING_END);
...
int RtcWpController::getBlockedWorldModeStatusKeyValue(bool log_flag) {
    int modemOffState = getNonSlotScopeStatusManager()->getIntValue(RFX_STATUS_KEY_MODEM_OFF_STATE,
                                                                    MODEM_OFF_IN_IDLE);
    RadioPowerLock radioLock = (RadioPowerLock)getNonSlotScopeStatusManager()->getIntValue(
...
int RtcWpController::setBlockedWorldModeStatusKeyValue(int value) {
    ...
    if (WP_LOCK_IDLE == value) {
        ...
        getNonSlotScopeStatusManager()->setIntValue(RFX_STATUS_KEY_MODEM_OFF_STATE,
                                                    MODEM_OFF_BY_WORLD_PHONE);
        for (int slotId = RFX_SLOT_ID_0; slotId < RfxRilUtils::rfxGetSimCount(); slotId++) {
            getStatusManager(slotId)->setIntValue(RFX_STATUS_KEY_RADIO_LOCK,
...
void RtcWpController::onTelephonyAssistantStatusChanged(int slotId, RfxStatusKeyEnum key,
    ...
    reserved[0] = id;
    mainWorldModeId = id;
    sp<RfxMessage> request = RfxMessage::obtainRequest(slotId, RFX_MSG_REQUEST_WORLD_MODE_RELOAD_ID,
                                                       RfxIntsData(reserved, 1));
    requestToMcl(request);
}
