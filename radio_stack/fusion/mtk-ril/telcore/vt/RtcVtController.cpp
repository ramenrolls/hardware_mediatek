...
RtcVtController::RtcVtController() { mCurrentOpid = DEFAULT_OP_ID; }
...
void RtcVtController::onInit() {
    ...
    // enable ANBR URC
    int enable = 1;
    sp<RfxMessage> msg = RfxMessage::obtainRequest(m_slot_id, RFX_MSG_REQUEST_ENABLE_ANBR,
                                                   RfxIntsData(&enable, 1));
...
bool RtcVtController::onHandleRequest(const sp<RfxMessage>& message) {
    logD(RFX_LOG_TAG, "[Handle REQ] token = %d, request = %s", message->getPToken(),
...
bool RtcVtController::onHandleResponse(const sp<RfxMessage>& message) {
    logD(RFX_LOG_TAG, "[Handle RSP] token = %d, response = %s", message->getPToken(),
         RFX_ID_TO_STR(message->getId()));
...
void RtcVtController::handleGetInfoRequest(const sp<RfxMessage>& request) {
    const RIL_VT_CALL_STATUS_UPDATE* pRspData =
            (const RIL_VT_CALL_STATUS_UPDATE*)request->getData()->getData();
    ...
    logD(RFX_LOG_TAG, "[GET  INFO ] token = %d, requestId = %d, phone = %d, call id = %d",
         request->getPToken(), request->getId(), request->getSlotId(), call_id);
    /*
    sp<RfxMessage> message = RfxMessage::obtainRequest(request->getSlotId(), request->getId(),
    request, true); requestToMcl(message);
    */
}

void RtcVtController::handleGetInfoResponse(const sp<RfxMessage>& response) {
    logD(RFX_LOG_TAG, "[GET  INFO ] response->getError() = %d, getSlot() = %d",
         response->getError(), response->getSlotId());
...
void RtcVtController::handleUpdateOpidResponse(const sp<RfxMessage>& response) {
    logD(RFX_LOG_TAG, "[updateOpId] response->getError() = %d, getSlot() = %d",
         response->getError(), response->getSlotId());
    ...
    if (RIL_E_REQUEST_NOT_SUPPORTED == error) {
        ...
        sp<RfxMessage> msg =
                RfxMessage::obtainRequest(0, RFX_MSG_REQUEST_UPDATE_OPID, RfxIntsData(data, 2));
...
void RtcVtController::onCallStatusChanged(RfxStatusKeyEnum key, RfxVariant old_value,
...
void RtcVtController::onUiccMccMncChanged(RfxStatusKeyEnum key, RfxVariant old_value,
                                          RfxVariant value) {
    RFX_UNUSED(key);
    RFX_UNUSED(old_value);
...
void RtcVtController::onCardTypeChanged(RfxStatusKeyEnum key, RfxVariant oldValue,
                                        RfxVariant newValue) {
    RFX_UNUSED(key);
