...
static const char PROPERTY_LAST_ICCID_SIM[4][29] = {
        "persist.vendor.radio.ls1icid", "persist.vendor.radio.ls2icid",
        "persist.vendor.radio.ls3icid", "persist.vendor.radio.ls4icid"};
...
void RtcImsController::requestImsDisable(int slotId, const sp<RfxAction>& action) {
    ...
    sp<RfxMessage> msg = RfxMessage::obtainRequest(getSlotId(), RFX_MSG_REQUEST_SET_IMSCFG,
                                                   RfxIntsData(cfgparams, 6));
...
void RtcImsController::requestImsResume(int slotId, const sp<RfxAction>& action) {
    ...
    if (imscfgcache[slotId][5] != -1) {
        sp<RfxMessage> msg = RfxMessage::obtainRequest(getSlotId(), RFX_MSG_REQUEST_SET_IMSCFG,
                                                       RfxIntsData(imscfgcache[slotId], 6));
        requestToMcl(msg);
    }
}
...
void RtcImsController::handleXui(const sp<RfxMessage>& message) {
    ...
    for (int i = 0; i < (int)xuis.size(); ++i) {
        ...
        RFX_LOG_D(RFX_LOG_TAG, "handleXui setValue: %s", RfxRilUtils::pii(RFX_LOG_TAG, c.data()));
    }
    getStatusManager(m_slot_id)->setString8VectorValue(RFX_STATUS_KEY_XUI_INFO, xuis);

    Vector<String8> xuis_ =
            getStatusManager(m_slot_id)->getString8VectorValue(RFX_STATUS_KEY_XUI_INFO);
    for (int i = 0; i < (int)xuis_.size(); ++i) {
        string c = xuis_[i].string();
        RFX_LOG_D(RFX_LOG_TAG, "handleXui KEY_XUI_INFO: %s",
                  RfxRilUtils::pii(RFX_LOG_TAG, c.data()));
    }
...
void RtcImsController::onRadioStateChanged(RfxStatusKeyEnum key, RfxVariant old_value,
                                           RfxVariant value) {
    RFX_UNUSED(key);
...
bool RtcImsController::onCheckIfRejectMessage(const sp<RfxMessage>& message, bool isModemPowerOff,
                                              int radioState) {
    bool isWfcSupport = RfxRilUtils::isWfcSupport();

    // logD(RFX_LOG_TAG, "onCheckIfRejectMessage, id = %d, isModemPowerOff = %d, radioState = %d,
    // msg = %s", message->getId(), isModemPowerOff, radioState, idToString(message->getId()));

    // The RIL Request should be bypass anyway
    if (isWfcSupport && !isModemPowerOff) {
        // default dont need to log false case since this value will not cause this msg to be
        // rejected logD(RFX_LOG_TAG, "onCheckIfRejectMessage ret=false: isWfcSupport=%d,
        // isModemPowerOff=%d", isWfcSupport, isModemPowerOff);
        return false;
    }

    int msgId = message->getId();
    if ((radioState == (int)RADIO_STATE_UNAVAILABLE || radioState == (int)RADIO_STATE_OFF) &&
        (msgId == RFX_MSG_REQUEST_SET_IMSCFG || msgId == RFX_MSG_REQUEST_IMS_BEARER_STATE_CONFIRM ||
         msgId == RFX_MSG_REQUEST_RUN_GBA || msgId == RFX_MSG_REQUEST_SET_IMS_BEARER_NOTIFICATION ||
         msgId == RFX_MSG_REQUEST_SET_WFC_PROFILE ||
         msgId == RFX_MSG_REQUEST_IMS_REGISTRATION_STATE)) {
        // default dont need to log false case since this value will not cause this msg to be
        // rejected logD(RFX_LOG_TAG, "onCheckIfRejectMessage ret=false: radioState=%d, msgId=%d",
        // radioState, msgId);
        return false;
    }
...
void RtcImsController::onMainCapabilitySlotChanged(RfxStatusKeyEnum key, RfxVariant old_value,
    ...
    logD(RFX_LOG_TAG, "onMainCapabilitySlotChanged()  cap %d->%d, mMainSlotId:%d", oldType, newType,
         mMainSlotId);
...
void RtcImsController::onCardTypeChanged(RfxStatusKeyEnum key, RfxVariant old_value,
                                         RfxVariant value) {
    RFX_UNUSED(key);
    ...
    if (old_type == IMS_CARD_TYPE_INVALID || old_type == IMS_CARD_TYPE_NONE) {
        if (new_type == IMS_CARD_TYPE_NONE) {
            ...
            mIsSimSwitch = false;
            mCachedXuiUrc = NULL;
            getStatusManager(m_slot_id)->setString8VectorValue(RFX_STATUS_KEY_XUI_INFO,
                                                               Vector<String8>());
        } else if (new_type > 0 && new_type != IMS_CARD_TYPE_INVALID) {
            char newIccIdStr[RFX_PROPERTY_VALUE_MAX] = {0};
            rfx_property_get(PROPERTY_ICCID_SIM[curSlotId], newIccIdStr, "");
            logD(RFX_LOG_TAG, "onCardTypeChanged, newIccIdStr = %s", givePrintableStr(newIccIdStr));
            int waitCount = 1;
            if (sInitDone == false) {
                char cryptoState[RFX_PROPERTY_VALUE_MAX] = {0};
                rfx_property_get("ro.crypto.state", cryptoState, "");
...
void RtcImsController::onNoIccIdTimeout() {
    ...
    if (strlen(newIccIdStr) == 0 || strcmp("N/A", newIccIdStr) == 0) {
        ...
        if (mNoIccidRetryCount < READ_ICCID_MAX_RETRY_COUNT && mNoIccidTimerHandle == NULL) {
            mNoIccidRetryCount++;
            mNoIccidTimerHandle =
                    RfxTimer::start(RfxCallback0(this, &RtcImsController::onNoIccIdTimeout),
                                    ms2ns(READ_ICCID_RETRY_TIME));
        }
    } else {
        int newType = getStatusManager(slotId)->getIntValue(RFX_STATUS_KEY_CDMA_CARD_TYPE,
                                                            IMS_CARD_TYPE_NONE);
        if (newType <= 255) {
...
void RtcImsController::setDefaultVolteState(int slot_id, char new_iccid[], int card_type) {
    ...
    if (card_type == CT_3G_UIM_CARD || card_type == CT_UIM_SIM_CARD) {
    ...
    if (RfxRilUtils::isMultipleImsSupport() == 0) {
        if (strlen(oldIccIdStr) == 0 || strcmp("null", oldIccIdStr) == 0 ||
            strcmp(new_iccid, oldIccIdStr) != 0 || mIsSimSwitch || mIsBootUp) {
            rfx_property_set(PROPERTY_LAST_ICCID_SIM[slot_id], new_iccid);
            if (mIsBootUp) {
                for (int slot = 0; slot < RFX_SLOT_COUNT; slot++) {
                    if (strcmp(new_iccid, sLastBootIccId[slot]) == 0) {
                        ...
                        logD(RFX_LOG_TAG,
                             "setDefaultVolteState, change sim slot reboot, last= %d, %d", state,
                             setValue);
                        break;
                    }
                }
        ...
        if (strlen(oldIccIdStr) == 0 || strcmp("null", oldIccIdStr) == 0 ||
            strcmp(new_iccid, oldIccIdStr) != 0 || mIsBootUp) {
            rfx_property_set(PROPERTY_LAST_ICCID_SIM[slot_id], new_iccid);
            if (mIsBootUp) {
                for (int slot = 0; slot < RFX_SLOT_COUNT; slot++) {
                    if (strcmp(new_iccid, sLastBootIccId[slot]) == 0) {
...
void RtcImsController::setVolteStateProperty(int slot_id, bool isEnable) {
    ...
    logD(RFX_LOG_TAG, "setVolteStateProperty, new volte_state = %d, %s", stateValue, temp);
}
...
bool RtcImsController::isOp09SimCard(int slot_id, char icc_id[], int card_type) {
    ...
    if (strncmp(icc_id, "898603", 6) == 0 || strncmp(icc_id, "898611", 6) == 0 ||
        strncmp(icc_id, "8985302", 7) == 0 || strncmp(icc_id, "8985307", 7) == 0 ||
...
void RtcImsController::sendDefaultVolteStateUrc(int slot_id, int value) {
    ...
    response[1] = slot_id;
    sp<RfxMessage> msg =
            RfxMessage::obtainUrc(slot_id, RIL_MSG_UNSOL_VOLTE_SETTING, RfxIntsData(response, 2));
    responseToRilj(msg);
...
void RtcImsController::setVolteSettingStatus(const sp<RfxMessage>& message) {
    ...
    int sim_state = getStatusManager(slot_id)->getIntValue(RFX_STATUS_KEY_SIM_STATE);
...
void RtcImsController::updateImsRegStatus() {
    sp<RfxMessage> msg = RfxMessage::obtainRequest(
            getSlotId(), RFX_MSG_REQUEST_SET_IMS_REGISTRATION_REPORT, RfxVoidData());
    requestToMcl(msg);
}
    ...
    switch (msg_id) {
        case RFX_MSG_REQUEST_SET_IMSCFG:
            ...
            if (mIsImsDisabling != false) {
                logD(RFX_LOG_TAG, "onPreviewMessage - %s, mIsImsDisabling -  %d",
                     RFX_ID_TO_STR(msg_id), mIsImsDisabling);
            }
...
void RtcImsController::handleImsRegInfoInd(const sp<RfxMessage>& message) {
    ...
    logD(RFX_LOG_TAG, "[%d]handleImsRegInfoInd, (%d, %d, %d, %d, %s)", m_slot_id,
         params->report_type, params->account_id, params->expire_time, params->error_code,
         params->error_msg);
}
...
void RtcImsController::sendCachedImsRegInfo() {
    ...
    if (response.report_type >= 0) {
        ...
        response.uri = (char*)getStatusManager()
                               ->getString8Value(RFX_STATUS_KEY_IMS_REG_IND_URI, String8(""))
                               .string();
        response.error_msg =
                (char*)getStatusManager()
                        ->getString8Value(RFX_STATUS_KEY_IMS_REG_IND_ERROR_MSG, String8(""))
                        .string();

        logD(RFX_LOG_TAG, "[%d]sendCachedImsRegInfo, (%d, %d, %d, %d, %s)", m_slot_id,
             response.report_type, response.account_id, response.expire_time, response.error_code,
             response.error_msg);
