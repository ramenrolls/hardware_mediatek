...
RFX_REGISTER_DATA_TO_REQUEST_ID(RfxIntsData, RfxVoidData, RFX_MSG_REQUEST_STK_EVENT_NOTIFY);
RFX_REGISTER_DATA_TO_REQUEST_ID(RfxVoidData, RfxIntsData, RFX_MSG_REQUEST_STK_QUERY_CPIN_STATE);
// BIP @{
RFX_REGISTER_DATA_TO_REQUEST_ID(RfxIntsData, RfxVoidData, RFX_MSG_REQUEST_BIP_SEND_CONFIRM_INFO);
// BIP @}
// BTSAP @{
...
// BTSAP @}
// End
RFX_REGISTER_DATA_TO_REQUEST_ID(RfxIntsData, RfxVoidData, RFX_MSG_REQUEST_USSD_DOMAIN_INFO_ACK);
...
RtcCatController::RtcCatController() : mCardType(-1) {
    mCPinRetry = 0;
    mTimeoutHandle = NULL;
...
void RtcCatController::onInit() {
    ...
    const int request_id_list[] = {
            ...
            RFX_MSG_REQUEST_REPORT_STK_SERVICE_IS_RUNNING,
            RFX_MSG_REQUEST_STK_SEND_ENVELOPE_WITH_STATUS,
            RFX_MSG_REQUEST_STK_SEND_RESPONSE_BY_CMDTYPE, RFX_MSG_REQUEST_STK_EVENT_NOTIFY,
    ...
    const int urc_id_list[] = {
            RFX_MSG_URC_STK_SESSION_END,      RFX_MSG_URC_STK_PROACTIVE_COMMAND,
            RFX_MSG_URC_STK_EVENT_NOTIFY,     RFX_MSG_URC_STK_CALL_SETUP,
            RFX_MSG_URC_SIM_REFRESH,          RFX_MSG_URC_STK_CC_ALPHA_NOTIFY,
            RFX_MSG_URC_STK_SETUP_MENU_RESET, RFX_MSG_URC_STK_CALL_CTRL,
            RFX_MSG_URC_STK_TRIGGER_OTASP,    RFX_MSG_URC_STK_CPIN_STATE,
    };
...
void RtcCatController::onCardTypeChanged(RfxStatusKeyEnum key, RfxVariant oldValue,
                                         RfxVariant newValue) {
    RFX_UNUSED(key);
    ...
    if ((oldState > 0) && (newState <= 0)) {
        logD(RFX_LOG_TAG, "onCardTypeChanged  cmdType 0x%02X, cmdQual %d", mProCmdRec.cmdType,
             mProCmdRec.cmdQualifier);
...
void RtcCatController::onRadioStateChanged(RfxStatusKeyEnum key, RfxVariant old_value,
                                           RfxVariant value) {
    int oldState = -1, newState = -1;
    ...
    switch (msg_id) {
        case RFX_MSG_REQUEST_STK_SEND_ENVELOPE_COMMAND:
            ...
            if (isIgnoredCmd) {
                ...
                sp<RfxMessage> rsp = RfxMessage::obtainResponse(RIL_E_SUCCESS, message, false);
                responseToRilj(rsp);
                break;
            }
            ...
            if (0 != mProCmdRec.cmdType && mProCmdRec.cmdType == cmdType &&
                ...
                if (resCode >= 0) {
                    ...
                    sp<RfxMessage> msg = RfxMessage::obtainRequest(
                            m_slot_id, RFX_MSG_REQUEST_BIP_SEND_CONFIRM_INFO,
                            RfxIntsData(bip_Data, 2));
                    requestToMcl(msg);
...
char* RtcCatController::decodeStkRefreshFileChange(char* str, int** cmd, int* cmd_length) {
    ...
    if (str[offset] <=
        '7') {  // file list length: if length < 7F it will use 2 bytes else it will use 4 bytes
        offset += 2;
    } else {
        offset += 4;
    }
...
void RtcCatController::handleSimRefresh(char* urc) {
    ...
    switch (urc[9]) {
        ...
        case '4':
            ...
            msg = RfxMessage::obtainRequest(m_slot_id, RFX_MSG_REQUEST_STK_QUERY_CPIN_STATE,
                                            RfxVoidData());
            requestToMcl(msg);
            return;
    ...
    if (NULL != efId_str && 0 < strlen(efId_str)) {
        for (i = 0; i < files_num; i++) {
            ...
            logD(RFX_LOG_TAG, "onSimRefresh: efId = %X, file numbers = %d", simRefreshRspV7.ef_id,
                 files_num);

            // Todo: Add the data structure for RIL_SimRefreshResponse_v7
            sp<RfxMessage> refresh_urc = RfxMessage::obtainUrc(
                    m_slot_id, RFX_MSG_URC_SIM_REFRESH,
    ...
    if (CMD_TYPE_PROACTIVE == cmdClass) {
        ...
        switch (cmdType) {
            ...
            case CMD_SETUP_CALL:
                ...
                if (cmd_not_understood == 0) {
                    ...
                    notify_info[1] = 50;
                    msg = RfxMessage::obtainRequest(m_slot_id, RFX_MSG_REQUEST_STK_EVENT_NOTIFY,
                                                    RfxIntsData(notify_info, 2));
                    requestToMcl(msg);
                }
                break;
            case CMD_SEND_SMS:
                ...
                if (((mCardType & RFX_CARD_TYPE_RUIM) || (mCardType & RFX_CARD_TYPE_CSIM)) &&
                ...
                if (cmd_not_understood == 0) {
                    ...
                    if (feature_version == 1) {
                        ...
                        msg = RfxMessage::obtainRequest(
                                m_slot_id, RFX_MSG_REQUEST_USSD_DOMAIN_INFO_REQ, RfxVoidData());
        ...
        if (CMD_OPEN_CHAN == cmdType) {
            sp<RfxMessage> urc = RfxMessage::obtainUrc(
                    getSlotId(), RFX_MSG_URC_STK_PROACTIVE_COMMAND, RfxStringData(cmd));
            responseToRilj(urc);
            return;
        }
        // BIP @}
        ...
        if (CMD_SETUP_CALL == cmdType) {
            sp<RfxMessage> setup_call_urc = RfxMessage::obtainUrc(
                    getSlotId(), RFX_MSG_URC_STK_CALL_SETUP, RfxIntsData(duration, 1));
...
void RtcCatController::handleSimRefreshAfterQueryCpin() {
    ...
    if (pEfIdString) {
        ...
        for (i = 0; i < files_num; i++) {
            ...
            sp<RfxMessage> refresh_urc = RfxMessage::obtainUrc(
                    m_slot_id, RFX_MSG_URC_SIM_REFRESH,
                    RfxSimRefreshData((void*)(simRefreshResponse->simRefreshRsp),
                                      sizeof(RIL_SimRefreshResponse_v7)));
            responseToRilj(refresh_urc);
            readIdx += 4;  // go to next EFID
        }
    } else {
        sp<RfxMessage> refresh_urc =
                RfxMessage::obtainUrc(m_slot_id, RFX_MSG_URC_SIM_REFRESH,
                                      RfxSimRefreshData((void*)(simRefreshResponse->simRefreshRsp),
                                                        sizeof(RIL_SimRefreshResponse_v7)));
        responseToRilj(refresh_urc);
    }
...
void RtcCatController::sendStkQueuedCmd(int flag) {
    ...
    if (true == isProaCmdQueued) {
        ...
        if (flag == 0) {
            ...
            if (NULL != cachedMenu) {
                sp<RfxMessage> urc = RfxMessage::obtainUrc(
                        getSlotId(), RFX_MSG_URC_STK_PROACTIVE_COMMAND, RfxStringData(cachedMenu));
                responseToRilj(urc);
            }
        }
    }
    ...
    switch (id) {
        case RFX_MSG_REQUEST_STK_QUERY_CPIN_STATE:
            mTimeoutHandle = RfxTimer::start(
                    RfxCallback0(this, &RtcCatController::retryQueryCpinState), ms2ns(delayTime));
            break;
        default:
...
void RtcCatController::sendResponseForCardMissing(int cmdType, int cmdQual) {
    ...
    sp<RfxMessage> msg = RfxMessage::obtainRequest(
            m_slot_id, RFX_MSG_REQUEST_STK_SEND_RESPONSE_BY_CMDTYPE, RfxIntsData(cmdInfo, 2));
    requestToMcl(msg);
}
...
bool RtcCatController::onCheckIfRejectMessage(const sp<RfxMessage>& message, bool isModemPowerOff,
                                              int radioState) {
