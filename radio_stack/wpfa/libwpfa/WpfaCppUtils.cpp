...
extern "C" void deinitialRuleContainer() { mRuleContainer = NULL; }
...
extern "C" void readRingBuffer() {
    ...
    if (mWpfaRingBuffer == NULL) {
        ...
        while (1) {
            ...
            // mWpfaRingBuffer->readDataFromRingBuffer(tmpStr,
            // mRegion->read_idx,mRegion->data_size);
            /*************** blocking call ***************/
...
int m2aVersionNotify(void* data) {
    wifiproxy_ap_md_filter_ver_t* msg = (wifiproxy_ap_md_filter_ver_t*)data;
    mtkLogD(CPP_UTIL_TAG, "m2aVersionNotify: md ver:%d", msg->md_filter_ver);
    if (msg->md_filter_ver != CURRENT_AP_FILTER_VERSION) {
        mtkLogD(CPP_UTIL_TAG, "m2aVersionNotify: error md ver:%d", msg->md_filter_ver);
        return -1;
    }
...
// Call back function for DRV
int m2aExecuteFilterReg(void* data) {
    wifiproxy_m2a_reg_dl_filter_t* filterPtr = (wifiproxy_m2a_reg_dl_filter_t*)data;
