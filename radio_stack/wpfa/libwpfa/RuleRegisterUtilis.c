...
int icmpMergeArgs(wifiProxy_filter_ip_ver_e ipType, WPFA_filter_reg_t filter, int operation) {
    ...
    char* args[] = {iptablePathArray, WAIT_FLAG,         operationArray,   WPFA_CHAIN, "-p",
                    icmpTypeArray,    icmptype,          typeAndCode,      "-j",       "NFQUEUE",
                    "--queue-num",    NFQUEUE_QNUM_ICMP, "--queue-bypass", NULL};
    ...
    if (checkFilterConfig(mConfig, WIFIPROXY_FILTER_TYPE_PROTOCOL)) {
        ...
        switch (filter.protocol) {
            case WIFIPROXY_FILTER_PROTOCOL_ICMP: {
                if (ipType != WIFIPROXY_FILTER_IP_VER_IPV4) {
                    mtkLogD(RR_LOG_TAG,
                            "failed in commonMergeArgs: PROTOCOL=WIFIPROXY_FILTER_PROTOCOL_ICMP, "
                            "but ipType = %d",
                            ipType);
                    return MERGE_IPTABLE_ARG_FAILED;
                }
            ...
            case WIFIPROXY_FILTER_PROTOCOL_ICMPV6: {
                if (ipType != WIFIPROXY_FILTER_IP_VER_IPV6) {
                    mtkLogD(RR_LOG_TAG,
                            "failed in commonMergeArgs: PROTOCOL=WIFIPROXY_FILTER_PROTOCOL_ICMPV6, "
                            "but ipType = %d",
                            ipType);
                    return MERGE_IPTABLE_ARG_FAILED;
                }
            ...
            case WIFIPROXY_FILTER_PROTOCOL_IPV4: {
                if (ipType != WIFIPROXY_FILTER_IP_VER_IPV4) {
                    mtkLogD(RR_LOG_TAG,
                            "failed in commonMergeArgs: PROTOCOL=WIFIPROXY_FILTER_PROTOCOL_IPV4, "
                            "but ipType = %d",
                            ipType);
                    return MERGE_IPTABLE_ARG_FAILED;
                }
            ...
            case WIFIPROXY_FILTER_PROTOCOL_IPV6: {
                if (ipType != WIFIPROXY_FILTER_IP_VER_IPV6) {
                    mtkLogD(RR_LOG_TAG,
                            "failed in commonMergeArgs: PROTOCOL=WIFIPROXY_FILTER_PROTOCOL_IPV6, "
                            "but ipType = %d",
                            ipType);
                    return MERGE_IPTABLE_ARG_FAILED;
                }
