...
static CUSTOM_VT_VENC_QUALITY_CTRL_TABLE_T _rVtVEncQualityCtrlTable[] = {
        ...
        {/* Upper table begins. */
         {CUSTOM_VENC_RESOLUTION_176x144, CUSTOM_VT_SENSOR_AUTO_MODE, CUSTOM_VENC_QUALITY_LOW,
          CUSTOM_VENC_CODEC_MPEG4},
         /* Lower table begins. */
         {120, 18, 1, 31, 40, 3, 48000, 1, 0, 0}},

        {/* Upper table begins. */
         {CUSTOM_VENC_RESOLUTION_176x144, CUSTOM_VT_SENSOR_AUTO_MODE, CUSTOM_VENC_QUALITY_NORMAL,
          CUSTOM_VENC_CODEC_MPEG4},
         /* Lower table begins. */
         {100, 18, 1, 31, 40, 3, 48000, 1, 0, 0}},

        {/* Upper table begins. */
         {CUSTOM_VENC_RESOLUTION_176x144, CUSTOM_VT_SENSOR_AUTO_MODE, CUSTOM_VENC_QUALITY_GOOD,
          CUSTOM_VENC_CODEC_MPEG4},
         /* Lower table begins. */
        ...
        {/* Upper table begins. */
         {CUSTOM_VENC_RESOLUTION_176x144, CUSTOM_VT_SENSOR_NIGHT_MODE, CUSTOM_VENC_QUALITY_LOW,
          CUSTOM_VENC_CODEC_MPEG4},
         /* Lower table begins. */
         {120, 18, 1, 31, 40, 3, 48000, 1, 0, 0}},

        {/* Upper table begins. */
         {CUSTOM_VENC_RESOLUTION_176x144, CUSTOM_VT_SENSOR_NIGHT_MODE, CUSTOM_VENC_QUALITY_NORMAL,
          CUSTOM_VENC_CODEC_MPEG4},
         /* Lower table begins. */
         {100, 18, 1, 31, 40, 3, 48000, 1, 0, 0}},

        {/* Upper table begins. */
         {CUSTOM_VENC_RESOLUTION_176x144, CUSTOM_VT_SENSOR_NIGHT_MODE, CUSTOM_VENC_QUALITY_GOOD,
          CUSTOM_VENC_CODEC_MPEG4},
         /* Lower table begins. */
         {75, 18, 1, 31, 40, 3, 48000, 1, 0, 0}},
...
bool _Custom_VT_GetFrameRates(int iCodec, int iSensorMode, void* pFrameRates) {
    ...
    for (int i = 0; i < iEntry; i++) {
        if ((_rVtVEncQualityCtrlTable[i].tVT_Scenario_T.eCodec == iCodec) &&
            (_rVtVEncQualityCtrlTable[i].tVT_Scenario_T.eSensorMode == iSensorMode)) {
            if (_rVtVEncQualityCtrlTable[i].tVT_Scenario_T.eQuality == CUSTOM_VENC_QUALITY_GOOD) {
                pTempFrameR->iGoodQualityFrameRate =
                        (_rVtVEncQualityCtrlTable[i].tVT_VEnc_Param_T.iFrameRate) / 10.0;
            } else if (_rVtVEncQualityCtrlTable[i].tVT_Scenario_T.eQuality ==
                       CUSTOM_VENC_QUALITY_NORMAL) {
                pTempFrameR->iNormalQualityFrameRate =
                        (_rVtVEncQualityCtrlTable[i].tVT_VEnc_Param_T.iFrameRate) / 10.0;
            } else if (_rVtVEncQualityCtrlTable[i].tVT_Scenario_T.eQuality ==
                       CUSTOM_VENC_QUALITY_LOW) {
                pTempFrameR->iLowQualityFrameRate =
                        (_rVtVEncQualityCtrlTable[i].tVT_VEnc_Param_T.iFrameRate) / 10.0;
            }
        }
    }
...
signed short* _Custom_VT_GetDynamicRangeTable() { return variation_vt; }
...
bool _Custom_VT_GetCodecParam(int iCodec, int iSensorMode, int iEncLevel,
    ...
    for (int i = 0; i < iEntry; i++) {
        if ((_rVtVEncQualityCtrlTable[i].tVT_Scenario_T.eCodec == iCodec) &&
            (_rVtVEncQualityCtrlTable[i].tVT_Scenario_T.eSensorMode == iSensorMode) &&
            (_rVtVEncQualityCtrlTable[i].tVT_Scenario_T.eQuality == iEncLevel)) {
            if (_rVtVEncQualityCtrlTable[i].tVT_VEnc_Param_T.iIsCustomerSetTable == 1) {
                _rVtVEncQualityCtrlTable[i].tVT_VEnc_Param_T.pDynamicRangeTable =
                        variation_vt;  //_Custom_VT_GetDynamicRangeTable();
            } else {
