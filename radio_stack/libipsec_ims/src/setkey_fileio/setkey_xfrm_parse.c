...
const struct typeent xfrmproto_types[] = {{"esp", IPPROTO_ESP},
                                          {"ah", IPPROTO_AH},
                                          {"comp", IPPROTO_COMP},
                                          {"route2", IPPROTO_ROUTING},
                                          {"hao", IPPROTO_DSTOPTS},
...
__u8 xfrm_xfrmproto_getbyname(char* name) {
    ...
    for (i = 0;; i++) {
        ...
        if (!t->t_name || t->t_type == -1) break;

        if (strcmp(t->t_name, name) == 0) return t->t_type;
    }

    return 0;
}

int xfrm_id_parse(xfrm_address_t* saddr_xfrm, struct xfrm_id* id, __u16* family, char* src,
    ...
    if (family) *family = src_prefix.family;
...
void xfrm_selector_parse(struct xfrm_selector* sel, char* src, char* dst,
    ...
    if (sel->family == AF_INET)
        ALOGD("xfrm_selector_parse "
              "family:%u,prefix_d:%u,prefix_s:%u,daddr:0x%x,saddr:0x%x,sel->sport:%d,sel->dport:%d,"
              "proto:%u,user:%u\n",
              sel->family, sel->prefixlen_d, sel->prefixlen_s, sel->daddr.a4, sel->daddr.a4,
              sel->sport, sel->dport, sel->proto, sel->user);
    else
        ALOGD("xfrm_selector_parse family:%u,prefix_d:%u,prefix_s:%u,daddr:0x%x %x %x "
              "%x,saddr:0x%x %x %x %x ,sel->sport:%d,sel->dport:%d,proto:%u,user:%u\n",
              sel->family, sel->prefixlen_d, sel->prefixlen_s, sel->daddr.a6[0], sel->daddr.a6[1],
              sel->daddr.a6[2], sel->daddr.a6[3], sel->saddr.a6[0], sel->saddr.a6[1],
              sel->saddr.a6[2], sel->saddr.a6[3], sel->sport, sel->dport, sel->proto, sel->user);
#endif
}
