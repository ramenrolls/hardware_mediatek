...
int setkey_deleteSP_xfrm(char* src, char* dst, enum PROTOCOL_TYPE protocol, char* src_port,
    ...
    groups |= (nl_mgrp_xfrm(XFRMNLGRP_ACQUIRE) | nl_mgrp_xfrm(XFRMNLGRP_EXPIRE) |
               nl_mgrp_xfrm(XFRMNLGRP_SA) | nl_mgrp_xfrm(XFRMNLGRP_POLICY));
    ...
    ALOGD("setkey_deleteSP_xfrm successed fd:%d --spddelete %s[%s] %s[%s] %d -P %s;\n", rth.fd, src,
          src_port, dst, dst_port, protocol, direction);
#endif
    rtnl_close_xfrm(&rth);
    return 0;
}
...
int setkey_deleteSA_xfrm(char* src, char* dst, char* ipsec_type, char* spi_src) {
    ...
    addattr_l(&req.n, sizeof(req.buf), XFRMA_SRCADDR, (void*)&saddr_xfrm, sizeof(saddr_xfrm));
...
int setkey_setSA_xfrm(int cmd, char* ip_src, char* ip_dst, char* ipsec_type, char* spi_src,
                      char* mode, char* encrp_algo_src, char* encrp_key_src,
                      char* intergrity_algo_src, char* intergrity_key_src, int u_id) {
    ...
    /*sepearte src addr and src port\dst addr and dst port, state's format often is add xx[xx]
     * xx[xx] esp xx ....*/
    ...
    if (xfrm_id_parse(&req.xsinfo.saddr, &req.xsinfo.id, &req.xsinfo.family, src_arr, dst_arr,
                      ipsec_type) < 0) {
#ifdef INIT_ENG_BUILD
        ALOGD("setkey_SA_xfrm->xfrm_id_parse failed ip_src:%s,ip_dst:%s,ipsec_type:%s", ip_src,
              ip_dst, ipsec_type);
#endif
        return -1;
    }
    ...
    if (strcmp(encrp_algo_arr, "not-supported") == 0) return -1;
    ...
    } else if (xfrm_algo_parse((void*)&alg, encrp_algo_arr, encrp_key_src, buf, sizeof(alg.buf)) ==
               -1) {
#ifdef INIT_ENG_BUILD
        ALOGD("setkey_SA_xfrm->xfrm_algo_parse failed encrp_algo:%s,encrp_key:%s,", encrp_algo_src,
              encrp_key_src);
#endif
        return -1;
    }
    ...
    if (strcmp(intergrity_algo_arr, "not-supported") == 0) return -1;
    if ((intergrity_algo_src == NULL) || (strcmp(intergrity_algo_src, "null") == 0)) {
        alg.u.alg.alg_key_len = 0;
        memcpy(alg.u.alg.alg_name, "digest_null", strlen("digest_null"));
    } else if (xfrm_algo_parse((void*)&alg, intergrity_algo_arr, intergrity_key_src, buf,
                               sizeof(alg.buf)) < -1) {
#ifdef INIT_ENG_BUILD
        ALOGD("setkey_SA_xfrm->xfrm_algo_parse failed intergrity crp_algo:%s,intergrity _key:%s,",
...
#ifdef INIT_ENG_BUILD
    ALOGD("setkey_SA_xfrm successed fd:%d ---add %s %s %s %s  -m %s -E %s %s  -A %s %s -u %d; "
          "spi:%d\n",
          rth.fd, ip_src, ip_dst, ipsec_type, spi_src, mode, encrp_algo_src, encrp_key_src,
          intergrity_algo_src, intergrity_key_src, u_id, req.xsinfo.id.spi);
#endif
    rtnl_close_xfrm(&rth);
    return 0;
}

int setkey_SP_xfrm(int cmd, char* src_range, char* dst_range, enum PROTOCOL_TYPE protocol,
                   char* port_src, char* port_dst, char* src_tunnel, char* dst_tunnel,
                   char* ipsec_type, char* mode, char* direction, int u_id) {
    char src_arr[128] = {0};
    char dst_arr[128] = {0};
    ...
    if (src_range) strncpy(src_arr, src_range, strlen(src_range) + 1);
    if (dst_range) strncpy(dst_arr, dst_range, strlen(dst_range) + 1);

    if (src_tunnel) strncpy(src_tunnel_arr, src_tunnel, strlen(src_tunnel) + 1);
    if (dst_tunnel) strncpy(dst_tunnel_arr, dst_tunnel, strlen(dst_tunnel) + 1);
    ...
    if (tmpl->mode == XFRM_MODE_TUNNEL) {
        xfrm_id_parse(&tmpl->saddr, &tmpl->id, &tmpl->family, src_tunnel_arr, dst_tunnel_arr,
                      ipsec_type);
    }
...
#ifdef INIT_ENG_BUILD
    ALOGD("setkey_SP_xfrm successed fd:%d --spdadd %s[%s] %s[%s] %d -P %s ipsec "
          "%s/%s//unique:%d;\n",
          rth.fd, src_range, port_src, dst_range, port_dst, protocol, direction, ipsec_type, mode,
          u_id);
#endif
    rtnl_close_xfrm(&rth);
    return 0;
...
int setkey_SP_2layer_xfrm(int cmd, char* src_range, char* dst_range, enum PROTOCOL_TYPE protocol,
                          char* port_src, char* port_dst, char* src_tunnel, char* dst_tunnel,
...
#ifdef INIT_ENG_BUILD
    ALOGD("setkey_SP_2layer_xfrm successed fd:%d --spdupdate %s[%s] %s[%s] %d -P %s prio 1000 "
          "ipsec %s/%s//unique:%d %s/%s/%s-%s/unique:%d;\n",
          rth.fd, src_range, port_src, dst_range, port_dst, protocol, direction, ipsec_type1, mode1,
          u_id1, ipsec_type2, mode2, src_tunnel, dst_tunnel, u_id2);
#endif
    rtnl_close_xfrm(&rth);
    return 0;
}
...
int xfrm_policy_process_delete_exist(struct rtnl_handle_xfrm* rth, struct nlmsghdr* n,
                                     pid_t volte_pid) {
    ...
    if (req.xpid.sel.family == AF_INET)
        ALOGD("xfrm_policy_process_delete_exist successed fd:%d --spddelete 0x%x[%d] 0x%x[%d] %d "
              "-P %d;\n",
              rth->fd, req.xpid.sel.saddr.a4, req.xpid.sel.sport, req.xpid.sel.daddr.a4,
              req.xpid.sel.dport, req.xpid.sel.proto, req.xpid.dir);
    else
        ALOGD("xfrm_policy_process_delete_exist successed fd:%d --spddelete 0x%x %x %x %x[%d] 0x%x "
              "%x %x %x[%d] %d -P %d;\n",
              rth->fd, req.xpid.sel.saddr.a6[0], req.xpid.sel.saddr.a6[1], req.xpid.sel.saddr.a6[2],
              req.xpid.sel.saddr.a6[3], req.xpid.sel.sport, req.xpid.sel.daddr.a6[0],
              req.xpid.sel.daddr.a6[1], req.xpid.sel.daddr.a6[2], req.xpid.sel.daddr.a6[3],
              req.xpid.sel.dport, req.xpid.sel.proto, req.xpid.dir);
#endif
    return 0;
}

int xfrm_state_process_delete_exist(struct rtnl_handle_xfrm* rth, struct nlmsghdr* n,
                                    pid_t volte_pid) {
    ...
    addattr_l(&req.n, sizeof(req.buf), XFRMA_SRCADDR, (void*)&saddr, sizeof(saddr));
    if (send(rth->fd, (void*)&req, sizeof(req), 0) < 0) {
    ...
    if (xsinfo->sel.family == AF_INET)
        ALOGD("xfrm_state_process_delete_exist successed fd:%d --delete 0x%x 0x%x %d;\n", rth->fd,
              xsinfo->sel.saddr.a4, xsinfo->sel.daddr.a4, xsinfo->id.spi);
    else
        ALOGD("xfrm_state_process_delete_exist successed fd:%d --delete 0x%x %x %x %x 0x%x %x %x "
              "%x %d ;\n",
              rth->fd, xsinfo->sel.saddr.a6[0], xsinfo->sel.saddr.a6[1], xsinfo->sel.saddr.a6[2],
              xsinfo->sel.saddr.a6[3], xsinfo->sel.daddr.a6[0], xsinfo->sel.daddr.a6[1],
              xsinfo->sel.daddr.a6[2], xsinfo->sel.daddr.a6[3], xsinfo->id.spi);
#endif
    return 0;
}
