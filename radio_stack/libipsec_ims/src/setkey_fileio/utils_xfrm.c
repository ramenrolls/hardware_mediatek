...
int mask2bits(__u32 netmask_xfrm) {
    ...
    if ((host_xfrm & (host_xfrm + 1)) != 0) return -1;

    for (; mask_xfrm; mask_xfrm <<= 1) ++bits_xfrm;
    return bits_xfrm;
}
...
int get_netmask(unsigned* val_xfrm, const char* arg_xfrm, int base_xfrm) {
    ...
    if (!get_unsigned(val_xfrm, arg_xfrm, base_xfrm)) return 0;
...
int get_unsigned(unsigned* val_xfrm, const char* arg_xfrm, int base_xfrm) {
    ...
    if (!arg_xfrm || !*arg_xfrm) return -1;
    res_xfrm = strtoul(arg_xfrm, &ptr_xfrm, base_xfrm);
    if (!ptr_xfrm || ptr_xfrm == arg_xfrm || *ptr_xfrm || res_xfrm > UINT_MAX) return -1;
    *val_xfrm = res_xfrm;
    return 0;
}
...
int get_u32(__u32* val_xfrm, const char* arg_xfrm, int base_xfrm) {
    ...
    if (!arg_xfrm || !*arg_xfrm) return -1;
    res_xfrm = strtoul(arg_xfrm, &ptr_xfrm, base_xfrm);
    if (!ptr_xfrm || ptr_xfrm == arg_xfrm || *ptr_xfrm || res_xfrm > 0xFFFFFFFFUL) return -1;
    *val_xfrm = res_xfrm;
    return 0;
}
...
int get_u8(__u8* val_xfrm, const char* arg_xfrm, int base_xfrm) {
    ...
    if (!arg_xfrm || !*arg_xfrm) return -1;
    res_xfrm = strtoul(arg_xfrm, &ptr_xfrm, base_xfrm);
    if (!ptr_xfrm || ptr_xfrm == arg_xfrm || *ptr_xfrm || res_xfrm > 0xFF) return -1;
    *val_xfrm = res_xfrm;
    return 0;
}
...
int get_addr_ipv4(__u8* ap_xfrm, const char* cp_xfrm) {
    ...
    for (i_xfrm = 0; i_xfrm < 4; i_xfrm++) {
        ...
        if (n_xfrm > 255) return -1; /* bogus network value */
        ...
        if (*endp_xfrm == '\0') break;

        if (i_xfrm == 3 || *endp_xfrm != '.') return -1; /* extra characters */
        cp_xfrm = endp_xfrm + 1;
    }
...
int get_addr_1(inet_prefix* addr_xfrm, const char* name_xfrm, int family) {
    ...
    if (strcmp(name_xfrm, "default") == 0 || strcmp(name_xfrm, "all") == 0 ||
        strcmp(name_xfrm, "any") == 0) {
        if (family == AF_DECnet) return -1;
    ...
    if (strchr(name_xfrm, ':')) {
        ...
        if (inet_pton(AF_INET6, name_xfrm, addr_xfrm->data) <= 0) return -1;
    ...
    if (get_addr_ipv4((__u8*)addr_xfrm->data, name_xfrm) <= 0) return -1;
...
int get_prefix(inet_prefix* dst_xfrm, char* arg_xfrm, int family) {
    ...
    if (strcmp(arg_xfrm, "default") == 0 || strcmp(arg_xfrm, "any") == 0 ||
    ...
    if (err == 0) {
        ...
        if (slash) {
            if (get_netmask(&plen, slash + 1, 0) || plen > dst_xfrm->bitlen) {
                err = -1;
                goto done;
            }
