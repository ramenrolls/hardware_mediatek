...
static int get_address_index(const struct in_addr* addr, const struct apattern* sortlist,
                             int nsort);
static int get6_address_index(const struct ares_in6_addr* addr, const struct apattern* sortlist,
                              int nsort);
...
void ares_gethostbyname(ares_channel channel, const char* name, int family,
    ...
    if (fake_hostent(name, family, callback, arg)) return;
...
static void host_callback(void* arg, int status, int timeouts, unsigned char* abuf, int alen) {
    ...
    if (status == ARES_SUCCESS) {
        if (hquery->sent_family == AF_INET) {
            ...
            if (host && channel->nsort) sort_addresses(host, channel->sortlist, channel->nsort);
            ...
            if (host && channel->nsort) sort6_addresses(host, channel->sortlist, channel->nsort);
        }
        end_hquery(hquery, status, host);
    } else if ((status == ARES_ENODATA || status == ARES_EBADRESP || status == ARES_ETIMEOUT) &&
               (hquery->sent_family == AF_INET6 && hquery->want_family == AF_UNSPEC)) {
...
static int fake_hostent(const char* name, int family, ares_host_callback callback, void* arg) {
    ...
    if (family == AF_INET6) result = (ares_inet_pton(AF_INET6, name, &in6) < 1 ? 0 : 1);
...
static int file_lookup(const char* name, int family, struct hostent** host) {
    ...
    while ((status = ares__get_hostent(fp, family, host)) == ARES_SUCCESS) {
        if (strcasecmp((*host)->h_name, name) == 0) break;
        for (alias = (*host)->h_aliases; *alias; alias++) {
            if (strcasecmp(*alias, name) == 0) break;
        }
        if (*alias) break;
...
static int get_address_index(const struct in_addr* addr, const struct apattern* sortlist,
    ...
    for (i = 0; i < nsort; i++) {
        if (sortlist[i].family != AF_INET) continue;
        if (sortlist[i].type == PATTERN_MASK) {
            if ((addr->s_addr & sortlist[i].mask.addr4.s_addr) == sortlist[i].addrV4.s_addr) break;
        } else {
            if (!ares_bitncmp(&addr->s_addr, &sortlist[i].addrV4.s_addr, sortlist[i].mask.bits))
                break;
        }
    }
    return i;
...
static int get6_address_index(const struct ares_in6_addr* addr, const struct apattern* sortlist,
    ...
    for (i = 0; i < nsort; i++) {
        if (sortlist[i].family != AF_INET6) continue;
        if (!ares_bitncmp(addr, &sortlist[i].addrV6, sortlist[i].mask.bits)) break;
    }
    return i;
}
