...
static void search_callback(void* arg, int status, int timeouts, unsigned char* abuf, int alen) {
    ...
    if (status != ARES_ENODATA && status != ARES_ESERVFAIL && status != ARES_ENOTFOUND)
        ...
        if (squery->trying_as_is) squery->status_as_is = status;
         ...
         */
        if (status == ARES_ENODATA) squery->ever_got_nodata = 1;
        ...
        if (squery->next_domain < channel->ndomains) {
            ...
            squery->trying_as_is = 1;
            ares_query(channel, squery->name, squery->dnsclass, squery->type, search_callback,
                       squery);
        } else {
        ...
        if (hostaliases) {
            ...
            if (fp) {
                while ((status = ares__read_line(fp, &line, &linesize)) == ARES_SUCCESS) {
                    if (strncasecmp(line, name, len) != 0 || !ISSPACE(line[len])) continue;
                    p = line + len;
                    while (ISSPACE(*p)) p++;
                    if (*p) {
                        q = p + 1;
                ...
                free(line);
                fclose(fp);
                if (status != ARES_SUCCESS && status != ARES_EOF) return status;
            } else {
                error = ERRNO;
                switch (error) {
                    case ENOENT:
