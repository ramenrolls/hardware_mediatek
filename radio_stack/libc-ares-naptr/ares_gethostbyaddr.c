...
void ares_gethostbyaddr(ares_channel channel, const void* addr, int addrlen, int family,
...
static void addr_callback(void* arg, int status, int timeouts, unsigned char* abuf, int alen) {
    ...
    if (status == ARES_SUCCESS) {
        if (aquery->addr.family == AF_INET) {
            ...
            status = ares_parse_ptr_reply(abuf, alen, &aquery->addr.addrV4, (int)addrlen, AF_INET,
                                          &host);
        } else {
            addrlen = sizeof(aquery->addr.addrV6);
            status = ares_parse_ptr_reply(abuf, alen, &aquery->addr.addrV6, (int)addrlen, AF_INET6,
                                          &host);
        }
        end_aquery(aquery, status, host);
...
static int file_lookup(struct ares_addr* addr, struct hostent** host) {
    ...
    if (platform == WIN_NT) {
        ...
        if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, WIN_NS_NT_KEY, 0, KEY_READ, &hkeyHosts) ==
            ...
            RegQueryValueEx(hkeyHosts, DATABASEPATH, NULL, NULL, (LPBYTE)tmp, &dwLength);
    ...
    if (!PATH_HOSTS) return ARES_ENOTFOUND;
#endif
    ...
    while ((status = ares__get_hostent(fp, addr->family, host)) == ARES_SUCCESS) {
        ...
        if (addr->family == AF_INET) {
            if (memcmp((*host)->h_addr, &addr->addrV4, sizeof(addr->addrV4)) == 0) break;
        } else if (addr->family == AF_INET6) {
            if (memcmp((*host)->h_addr, &addr->addrV6, sizeof(addr->addrV6)) == 0) break;
        }
        ares_free_hostent(*host);
    }
    fclose(fp);
    if (status == ARES_EOF) status = ARES_ENOTFOUND;
...
static void ptr_rr_name(char* name, const struct ares_addr* addr) {
    if (addr->family == AF_INET) {
        ...
        sprintf(name, "%x.%x.%x.%x.%x.%x.%x.%x.%x.%x.%x.%x.%x.%x.%x.%x.", bytes[15] & 0xf,
                bytes[15] >> 4, bytes[14] & 0xf, bytes[14] >> 4, bytes[13] & 0xf, bytes[13] >> 4,
                bytes[12] & 0xf, bytes[12] >> 4, bytes[11] & 0xf, bytes[11] >> 4, bytes[10] & 0xf,
                bytes[10] >> 4, bytes[9] & 0xf, bytes[9] >> 4, bytes[8] & 0xf, bytes[8] >> 4);
        sprintf(name + strlen(name), "%x.%x.%x.%x.%x.%x.%x.%x.%x.%x.%x.%x.%x.%x.%x.%x.ip6.arpa",
                bytes[7] & 0xf, bytes[7] >> 4, bytes[6] & 0xf, bytes[6] >> 4, bytes[5] & 0xf,
                bytes[5] >> 4, bytes[4] & 0xf, bytes[4] >> 4, bytes[3] & 0xf, bytes[3] >> 4,
                bytes[2] & 0xf, bytes[2] >> 4, bytes[1] & 0xf, bytes[1] >> 4, bytes[0] & 0xf,
                bytes[0] >> 4);
    }
}
