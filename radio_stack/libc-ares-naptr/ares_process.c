...
static const struct nv flags[] = {
        {"usevc", ARES_FLAG_USEVC},       {"primary", ARES_FLAG_PRIMARY},
        {"igntc", ARES_FLAG_IGNTC},       {"norecurse", ARES_FLAG_NORECURSE},
        {"stayopen", ARES_FLAG_STAYOPEN}, {"noaliases", ARES_FLAG_NOALIASES}};
static const int nflags = sizeof(flags) / sizeof(flags[0]);

static const struct nv classes[] = {{"IN", C_IN}, {"CHAOS", C_CHAOS}, {"HS", C_HS}, {"ANY", C_ANY}};
static const int nclasses = sizeof(classes) / sizeof(classes[0]);

static const struct nv types[] = {{"A", T_A},
                                  {"NS", T_NS},
                                  {"MD", T_MD},
                                  {"MF", T_MF},
                                  {"CNAME", T_CNAME},
...
static void write_tcp_data(ares_channel channel, fd_set* write_fds, ares_socket_t write_fd,
                           struct timeval* now);
static void read_tcp_data(ares_channel channel, fd_set* read_fds, ares_socket_t read_fd,
                          struct timeval* now);
static void read_udp_packets(ares_channel channel, fd_set* read_fds, ares_socket_t read_fd,
                             struct timeval* now);
static void advance_tcp_send_queue(ares_channel channel, int whichserver, ssize_t num_bytes);
static void process_timeouts(ares_channel channel, struct timeval* now);
static void process_broken_connections(ares_channel channel, struct timeval* now);
static void process_answer(ares_channel channel, unsigned char* abuf, int alen, int whichserver,
                           int tcp, struct timeval* now);
...
static void end_query(ares_channel channel, struct query* query, int status, unsigned char* abuf,
                      int alen);
...
int ares__timedout(struct timeval* now, struct timeval* check) {
    ...
    if (secs > 0) return 1; /* yes, timed out */
    if (secs < 0) return 0; /* nope, not timed out */
...
int ares__timeadd(struct timeval* now, int millisecs) {
...
long ares__timeoffset(struct timeval* now, struct timeval* check) {
    return (check->tv_sec - now->tv_sec) * 1000 + (check->tv_usec - now->tv_usec) / 1000;
}

/*
 * generic process function
 */
static void processfds(ares_channel channel, fd_set* read_fds, ares_socket_t read_fd,
 ...
 * See what's up and respond accordingly.
 */
void ares_process_fd(ares_channel channel, ares_socket_t read_fd, /* use ARES_SOCKET_BAD or valid
                                                                     file descriptors */
...
static void write_tcp_data(ares_channel channel, fd_set* write_fds, ares_socket_t write_fd,
    ...
    if (!write_fds && (write_fd == ARES_SOCKET_BAD)) /* no possible action */
        return;
    ...
    for (i = 0; i < channel->nservers; i++) {
        ...
        if (!server->qhead || server->tcp_socket == ARES_SOCKET_BAD || server->is_broken) continue;

        if (write_fds) {
            if (!FD_ISSET(server->tcp_socket, write_fds)) continue;
        } else {
            if (server->tcp_socket != write_fd) continue;
        }
        ...
        n = 0;
        for (sendreq = server->qhead; sendreq; sendreq = sendreq->next) n++;
        ...
        if (vec) {
            ...
            free(vec);
            if (wcount < 0) {
                if (!try_again(SOCKERRNO)) handle_error(channel, i, now);
                continue;
            }
...
static void advance_tcp_send_queue(ares_channel channel, int whichserver, ssize_t num_bytes) {
    ...
    while (num_bytes > 0) {
        ...
        if ((size_t)num_bytes >= sendreq->len) {
            ...
            if (sendreq->data_storage) free(sendreq->data_storage);
            free(sendreq);
...
static void read_tcp_data(ares_channel channel, fd_set* read_fds, ares_socket_t read_fd,
    ...
    if (!read_fds && (read_fd == ARES_SOCKET_BAD)) /* no possible action */
        return;

    for (i = 0; i < channel->nservers; i++) {
        /* Make sure the server has a socket and is selected in read_fds. */
        server = &channel->servers[i];
        if (server->tcp_socket == ARES_SOCKET_BAD || server->is_broken) continue;

        if (read_fds) {
            if (!FD_ISSET(server->tcp_socket, read_fds)) continue;
        } else {
            if (server->tcp_socket != read_fd) continue;
        }
        ...
        if (server->tcp_lenbuf_pos != 2) {
             ...
             */
            count = sread(server->tcp_socket, server->tcp_lenbuf + server->tcp_lenbuf_pos,
                          2 - server->tcp_lenbuf_pos);
            if (count <= 0) {
                if (!(count == -1 && try_again(SOCKERRNO))) handle_error(channel, i, now);
                continue;
            }
            ...
            if (server->tcp_lenbuf_pos == 2) {
                ...
                if (!server->tcp_buffer) handle_error(channel, i, now);
                server->tcp_buffer_pos = 0;
            }
        } else {
            /* Read data into the allocated buffer. */
            count = sread(server->tcp_socket, server->tcp_buffer + server->tcp_buffer_pos,
            ...
            if (server->tcp_buffer_pos == server->tcp_length) {
                ...
                if (server->tcp_buffer) free(server->tcp_buffer);
...
static void read_udp_packets(ares_channel channel, fd_set* read_fds, ares_socket_t read_fd,
    ...
    for (i = 0; i < channel->nservers; i++) {
        ...
        if (server->udp_socket == ARES_SOCKET_BAD || server->is_broken) continue;

        if (read_fds) {
            if (!FD_ISSET(server->udp_socket, read_fds)) continue;
        } else {
            if (server->udp_socket != read_fd) continue;
        }
        ...
        do {
            ...
            count = (ssize_t)recvfrom(server->udp_socket, (void*)buf, sizeof(buf), 0, &from.sa,
                                      &fromlen);
#else
...
static void process_answer(ares_channel channel, unsigned char* abuf, int alen, int whichserver,
     ...
     */
    if (alen > PACKETSZ && !tcp) alen = PACKETSZ;
    ...
    if (!(channel->flags & ARES_FLAG_NOCHECKRESP)) {
        if (rcode == SERVFAIL || rcode == NOTIMP || rcode == REFUSED) {
            ...
            if (query->server == whichserver) next_server(channel, query, now);
            return;
        }
    }
...
static void next_server(ares_channel channel, struct query* query, struct timeval* now) {
    ...
    while (++(query->try_count) < (channel->nservers * channel->tries)) {
         ...
         */
        // ALOGD("%s server state:%d %d
        // ",__func__,server->is_broken,query->server_info[query->server].skip_server);
        if (!server->is_broken && !query->server_info[query->server].skip_server &&
            !(query->using_tcp && (query->server_info[query->server].tcp_connection_generation ==
...
void ares__send_query(ares_channel channel, struct query* query, struct timeval* now) {
    ...
    // printf("%s
    // tiemout:%d,timeplus:%d,query->try_count:%d,channel->nservers:%d\n",__func__,channel->timeout,timeplus,query->try_count,channel->nservers);
    query->timeout = *now;
    ares__timeadd(&query->timeout, timeplus);
    ...
    ares__insert_in_list(
            ...
            &(channel->queries_by_timeout[query->timeout.tv_sec % ARES_TIMEOUT_TABLE_SIZE]));

    /* Keep track of queries bucketed by server, so we can process server
     * errors quickly.
     */
    ares__remove_from_list(&(query->queries_to_server));
    ares__insert_in_list(&(query->queries_to_server), &(server->queries_to_server));
}
...
static int configure_socket(ares_socket_t s, int family, ares_channel channel) {
    ...
    if (fcntl(s, F_SETFD, FD_CLOEXEC) == -1) return -1;
#endif

    /* Set the socket's send and receive buffer sizes. */
    if ((channel->socket_send_buffer_size > 0) &&
        setsockopt(s, SOL_SOCKET, SO_SNDBUF, (void*)&channel->socket_send_buffer_size,
                   sizeof(channel->socket_send_buffer_size)) == -1)
        return -1;

    if ((channel->socket_receive_buffer_size > 0) &&
        setsockopt(s, SOL_SOCKET, SO_RCVBUF, (void*)&channel->socket_receive_buffer_size,
    ...
    if (family == AF_INET) {
        if (channel->local_ip4) {
            ...
            if (bind(s, &local.sa, sizeof(local.sa4)) < 0) return -1;
        }
        ...
        if (memcmp(channel->local_ip6, &ares_in6addr_any, sizeof(channel->local_ip6)) != 0) {
            ...
            if (bind(s, &local.sa, sizeof(local.sa6)) < 0) return -1;
        }
    }
...
static int open_tcp_socket(ares_channel channel, struct server_state* server) {
    ...
    switch (server->addr.family) {
        case AF_INET:
            ...
            memcpy(&saddr.sa4.sin_addr, &server->addr.addrV4, sizeof(server->addr.addrV4));
            break;
        case AF_INET6:
            sa = (void*)&saddr.sa6;
            ...
            memcpy(&saddr.sa6.sin6_addr, &server->addr.addrV6, sizeof(server->addr.addrV6));
            break;
        default:
            return -1;
    }
    ...
    if (channel->sock_create_cb) {
        int err = channel->sock_create_cb(s, SOCK_STREAM, channel->sock_create_cb_data);
        if (err < 0) {
            sclose(s);
            return err;
        }
...
static int open_udp_socket(ares_channel channel, struct server_state* server) {
    ...
    if (channel->sock_create_cb) {
        int err = channel->sock_create_cb(s, SOCK_DGRAM, channel->sock_create_cb_data);
        if (err < 0) {
            sclose(s);
...
static int same_questions(const unsigned char* qbuf, int qlen, const unsigned char* abuf,
                          int alen) {
    struct {
    ...
    if (sa->sa_family == aa->family) {
        switch (aa->family) {
            case AF_INET:
                ...
                if (memcmp(addr1, addr2, sizeof(aa->addrV4)) == 0) return 1; /* match */
                break;
            case AF_INET6:
                addr1 = &aa->addrV6;
                addr2 = &((struct sockaddr_in6*)sa)->sin6_addr;
                if (memcmp(addr1, addr2, sizeof(aa->addrV6)) == 0) return 1; /* match */
                break;
            default:
                break;
        }
...
static const char* opcodes[] = {"QUERY",     "IQUERY",    "STATUS",    "(reserved)",
                                "NOTIFY",    "(unknown)", "(unknown)", "(unknown)",
                                "(unknown)", "UPDATEA",   "UPDATED",   "UPDATEDA",
                                "UPDATEM",   "UPDATEMA",  "ZONEINIT",  "ZONEREF"};

static const char* rcodes[] = {"NOERROR",   "FORMERR",   "SERVFAIL",  "NXDOMAIN",
                               "NOTIMP",    "REFUSED",   "(unknown)", "(unknown)",
                               "(unknown)", "(unknown)", "(unknown)", "(unknown)",
                               "(unknown)", "(unknown)", "(unknown)", "NOCHANGE"};

const unsigned char* display_question(const unsigned char* aptr, const unsigned char* abuf,
                                      int alen) {
    char* name;
...
const unsigned char* display_rr_naptr(const unsigned char* aptr, const unsigned char* abuf,
                                      int alen, struct records_naptr* node,
...
const unsigned char* display_rr(const unsigned char* aptr, const unsigned char* abuf, int alen) {
    const unsigned char* p;
    int type, dnsclass, ttl, dlen, status;
    long len;
    char addr[46];
    ...
    switch (type) {
        ...
        case T_SOA:
            ...
            printf("\t\t\t\t\t\t( %lu %lu %lu %lu %lu )", (unsigned long)DNS__32BIT(p),
                   (unsigned long)DNS__32BIT(p + 4), (unsigned long)DNS__32BIT(p + 8),
                   (unsigned long)DNS__32BIT(p + 12), (unsigned long)DNS__32BIT(p + 16));
            break;
...
void callback_naptr(void* arg, int status, int timeouts, unsigned char* abuf, int alen,
                    struct records_naptr** result) {
    char* name = (char*)arg;
...
static void end_query(ares_channel channel, struct query* query, int status, unsigned char* abuf,
    ...
    if (!(channel->flags & ARES_FLAG_STAYOPEN) && ares__is_list_empty(&(channel->all_queries))) {
        for (i = 0; i < channel->nservers; i++) ares__close_sockets(channel, &channel->servers[i]);
    }
}
...
int aes_getrecords(const char* hostname, const char* service, const struct query_type* hints,
                   struct records_naptr** result) {
        ...
        if (inet_pton(AF_INET, service, &srvr->addr.addr4) > 0)
            ...
            switch (hostent->h_addrtype) {
                ...
                case AF_INET6:
                    ...
                    break;
                default:
                    ALOGD("adig: server %s unsupported address family.\n", optarg);
                    destroy_addr_list(servers);
                    return 0;
            }
        }
...
static void callback(void* arg, int status, int timeouts, unsigned char* abuf, int alen) {
    ...
    if (name) printf("Answer for query %s:\n", name);
...
int result_list_isempty(struct records_naptr** head) { return *head == NULL; }

void append_result_list(struct records_naptr** head, struct records_naptr* node) {
...
void append_addr_list(struct ares_addr_node** head, struct ares_addr_node* node) {
