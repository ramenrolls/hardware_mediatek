...
static int inet_net_pton_ipv4(const char* src, unsigned char* dst, size_t size) {
    ...
    if (ch == '0' && (src[0] == 'x' || src[0] == 'X') && ISASCII(src[1]) && ISXDIGIT(src[1])) {
        ...
        for (;;) {
            ...
            *dst++ = (unsigned char)tmp;
            ...
            if (ch != '.') goto enoent;
            ch = *src++;
            if (!ISASCII(ch) || !ISDIGIT(ch)) goto enoent;
        }
    } else
        goto enoent;
    ...
    if (bits == -1) {
        ...
        if (bits < ((dst - odst) * 8)) bits = aresx_sztosi(dst - odst) * 8;
        /*
...
static int inet_net_pton_ipv6(const char* src, unsigned char* dst, size_t size) {
    ...
    while ((ch = *src++) != '\0') {
        ...
        if (ch == '/' && getbits(src, &bits) > 0) break;
        goto enoent;
    }
    if (saw_xdigit) {
        if (tp + NS_INT16SZ > endp) goto enoent;
        *tp++ = (unsigned char)((val >> 8) & 0xff);
        *tp++ = (unsigned char)(val & 0xff);
    }
    if (bits == -1) bits = 128;
...
int ares_inet_pton(int af, const char* src, void* dst) {
    ...
    if (result == -1 && ERRNO == ENOENT) return 0;
    return (result > -1 ? 1 : -1);
}
#endif
