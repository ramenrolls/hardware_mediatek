...
int main(int argc, char** argv) {
    ...
    while ((ch = ares_getopt(argc, argv, "dvh?")) != -1) switch (ch) {
            case 'd':
#ifdef WATT32
                dbug_init();
#endif
    ...
    for (; *argv; argv++) {
        ...
        sprintf(buf, nerd_fmt, (unsigned int)(addr.s_addr >> 24),
                (unsigned int)((addr.s_addr >> 16) & 255), (unsigned int)((addr.s_addr >> 8) & 255),
...
static void callback(void* arg, int status, int timeouts, struct hostent* host) {
    ...
    TRACE(("\nFound address %s, name %s\n",
           ares_inet_ntop(AF_INET, (const char*)host->h_addr, buf, sizeof(buf)), host->h_name));
...
static const struct search_list* list_lookup(int number, const struct search_list* list, int num) {
    while (num > 0 && list->long_name) {
        if (list->country_number == number) return (list);
        num--;
        list++;
    }
    return (NULL);
...
static void find_country_from_cname(const char* cname, struct in_addr addr) {
    ...
    country = list_lookup(cnumber, country_list, sizeof(country_list) / sizeof(country_list[0]));
    if (!country)
        printf("Name for country-number %d not found.\n", cnumber);
    else {
        if (ver_1) {
            if ((country->short_name[0] != ccode_A2[0]) ||
                (country->short_name[1] != ccode_A2[1]) || (country->short_name[2] != ccode_A2[2]))
