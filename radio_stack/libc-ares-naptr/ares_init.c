...
static int config_lookup(ares_channel channel, const char* str, const char* bindch,
                         const char* filech);
...
#define ARES_CONFIG_CHECK(x)                                                                 \
    (x->lookups && x->nsort > -1 && x->nservers > -1 && x->ndomains > -1 && x->ndots > -1 && \
     x->timeout > -1 && x->tries > -1)

int ares_init(ares_channel* channelptr) { return ares_init_options(channelptr, NULL, 0); }
...
int ares_init_options(ares_channel* channelptr, struct ares_options* options, int optmask) {
    ...
    if (env) curl_memdebug(env);
    env = getenv("CARES_MEMLIMIT");
    if (env) {
        char* endptr;
        long num = strtol(env, &endptr, 10);
        if ((endptr != env) && (endptr == env + strlen(env)) && (num > 0)) curl_memlimit(num);
    }
#endif
    ...
    if (status != ARES_SUCCESS) {
        ...
        if (channel->servers) free(channel->servers);
        if (channel->domains) {
            for (i = 0; i < channel->ndomains; i++) free(channel->domains[i]);
            free(channel->domains);
        }
        if (channel->sortlist) free(channel->sortlist);
        if (channel->lookups) free(channel->lookups);
        free(channel);
        return status;
    }

    /* Trim to one server if ARES_FLAG_PRIMARY is set. */
    if ((channel->flags & ARES_FLAG_PRIMARY) && channel->nservers > 1) channel->nservers = 1;
...
int ares_save_options(ares_channel channel, struct ares_options* options, int* optmask) {
    ...
    if (!ARES_CONFIG_CHECK(channel)) return ARES_ENODATA;
    ...
    (*optmask) = (ARES_OPT_FLAGS | ARES_OPT_TRIES | ARES_OPT_NDOTS | ARES_OPT_UDP_PORT |
                  ARES_OPT_TCP_PORT | ARES_OPT_SOCK_STATE_CB | ARES_OPT_SERVERS | ARES_OPT_DOMAINS |
                  ARES_OPT_LOOKUPS | ARES_OPT_SORTLIST | ARES_OPT_TIMEOUTMS) |
    ...
    if (channel->nservers) {
        for (i = 0; i < channel->nservers; i++) {
            if (channel->servers[i].addr.family == AF_INET) ipv4_nservers++;
        }
        if (ipv4_nservers) {
            options->servers = malloc(ipv4_nservers * sizeof(struct in_addr));
            if (!options->servers) return ARES_ENOMEM;
    ...
    if (channel->ndomains) {
        ...
        if (!options->domains) return ARES_ENOMEM;

        for (i = 0; i < channel->ndomains; i++) {
            options->ndomains = i;
            options->domains[i] = strdup(channel->domains[i]);
            if (!options->domains[i]) return ARES_ENOMEM;
        }
    }
    ...
    if (channel->lookups) {
        ...
        if (!options->lookups && channel->lookups) return ARES_ENOMEM;
    }
    ...
    if (channel->nsort) {
        ...
        for (i = 0; i < channel->nsort; i++) options->sortlist[i] = channel->sortlist[i];
    }
...
static int init_by_options(ares_channel channel, const struct ares_options* options, int optmask) {
    ...
    /* Easy stuff. */
    if ((optmask & ARES_OPT_FLAGS) && channel->flags == -1) channel->flags = options->flags;
    if ((optmask & ARES_OPT_TIMEOUTMS) && channel->timeout == -1)
        channel->timeout = options->timeout;
    else if ((optmask & ARES_OPT_TIMEOUT) && channel->timeout == -1)
        channel->timeout = options->timeout * 1000;
    if ((optmask & ARES_OPT_TRIES) && channel->tries == -1) channel->tries = options->tries;
    if ((optmask & ARES_OPT_NDOTS) && channel->ndots == -1) channel->ndots = options->ndots;
    if ((optmask & ARES_OPT_ROTATE) && channel->rotate == -1) channel->rotate = 1;
    ...
    if ((optmask & ARES_OPT_SOCK_SNDBUF) && channel->socket_send_buffer_size == -1)
        channel->socket_send_buffer_size = options->socket_send_buffer_size;
    if ((optmask & ARES_OPT_SOCK_RCVBUF) && channel->socket_receive_buffer_size == -1)
    ...
    if ((optmask & ARES_OPT_DOMAINS) && channel->ndomains == -1) {
        ...
        if (options->ndomains > 0) {
            ...
            for (i = 0; i < options->ndomains; i++) {
                ...
                if (!channel->domains[i]) return ARES_ENOMEM;
            }
        }
    ...
    /* copy sortlist */
    if ((optmask & ARES_OPT_SORTLIST) && (channel->nsort == -1) && (options->nsort > 0)) {
        channel->sortlist = malloc(options->nsort * sizeof(struct apattern));
        if (!channel->sortlist) return ARES_ENOMEM;
        for (i = 0; i < options->nsort; i++) channel->sortlist[i] = options->sortlist[i];
...
static int get_res_nt(HKEY hKey, const char* subkey, char** obuf) {
    ...
    if ((result != ERROR_SUCCESS && result != ERROR_MORE_DATA) || !size) return 0;
    *obuf = malloc(size + 1);
    if (!*obuf) return 0;
...
static int get_res_interfaces_nt(HKEY hKey, const char* subkey, char** obuf) {
    ...
    while (RegEnumKeyEx(hKey, idx++, enumbuf, &enum_size, 0, NULL, NULL, NULL) !=
           ERROR_NO_MORE_ITEMS) {
        int rc;

        enum_size = 39;
        if (RegOpenKeyEx(hKey, enumbuf, 0, KEY_QUERY_VALUE, &hVal) != ERROR_SUCCESS) continue;
...
static int get_iphlpapi_dns_info(char* ret_buf, size_t ret_size) {
    ...
    if (ares_fpGetAdaptersAddresses != ZERO_NULL) {
        ...
        if (pFirstEntry) free(pFirstEntry);
        if (ret > ret_buf) ret[-1] = '\0';
        return count;
    } else {
        FIXED_INFO *fi, *newfi;
        ...
        if ((res != ERROR_BUFFER_OVERFLOW) && (res != ERROR_SUCCESS)) goto quit;
        ...
        res = (*ares_fpGetNetworkParams)(fi, &size);
        if (res != ERROR_SUCCESS) goto quit;
        ...
        if (strlen(fi->DnsServerList.IpAddress.String) > 0 &&
            inet_addr(fi->DnsServerList.IpAddress.String) != INADDR_NONE && left > ipv4_size) {
...
DhcpNameServer
   */
    ...
    if (get_iphlpapi_dns_info(buf, sizeof(buf)) > 0) {
        ...
        if (status == ARES_SUCCESS) goto okay;
    }
    ...
    if (platform == WIN_NT) {
        ...
        if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, WIN_NS_9X, 0, KEY_READ, &mykey) == ERROR_SUCCESS) {
            if ((result = RegQueryValueEx(mykey, NAMESERVER, NULL, &data_type, NULL, &bytes)) ==
                        ERROR_SUCCESS ||
                result == ERROR_MORE_DATA) {
                if (bytes) {
                    line = malloc(bytes + 1);
                    if (RegQueryValueEx(mykey, NAMESERVER, NULL, &data_type, (unsigned char*)line,
                                        &bytes) == ERROR_SUCCESS) {
                        status = config_nameserver(&servers, &nservers, line);
                    }
                    free(line);
    ...
    if (line) {
        ...
        if (!resolvers) return ARES_ENOMEM;
    ...
    for (i = 0; def_nameservers[i]; i++)
        ;
    if (i == 0) return ARES_SUCCESS; /* use localhost DNS server */

    nservers = i;
    servers = calloc(i, sizeof(struct server_state));
    if (!servers) return ARES_ENOMEM;
...
#else
    {
        ...
        if (ARES_CONFIG_CHECK(channel)) return ARES_SUCCESS;
...
static int init_by_defaults(ares_channel channel) {
    ...
    if (channel->rotate == -1) channel->rotate = 0;
...
#define toolong(x) (x == -1) && (SOCKERRNO == EINVAL)
    ...
    if (!channel->lookups) {
        ...
        if (!channel->lookups) rc = ARES_ENOMEM;
    }
...
error:
    if (rc) {
        ...
        if (channel->domains && channel->domains[0]) free(channel->domains[0]);
        if (channel->domains) free(channel->domains);
        if (channel->lookups) free(channel->lookups);
    }
...
static int config_nameserver(struct server_state** servers, int* nservers, char* str) {
    ...
    for (p = str; p;) {
        ...
        while (*p && !ISSPACE(*p) && (*p != ',')) p++;
        if (*p) /* Null terminate this address. */
            *p++ = '\0';
        else
        ...
        if (!newserv) return ARES_ENOMEM;

        /* Store address data. */
        newserv[*nservers].addr.family = host.family;
        if (host.family == AF_INET)
            memcpy(&newserv[*nservers].addr.addrV4, &host.addrV4, sizeof(host.addrV4));
        else
            memcpy(&newserv[*nservers].addr.addrV6, &host.addrV6, sizeof(host.addrV6));
...
static int config_sortlist(struct apattern** sortlist, int* nsort, const char* str) {
    ...
    while (*str && *str != ';') {
        ...
        if ((bits = ares_inet_net_pton(AF_INET6, ipbufpfx[0] ? ipbufpfx : ipbuf, &pat.addrV6,
                                       sizeof(pat.addrV6))) > 0) {
            pat.type = PATTERN_CIDR;
            pat.mask.bits = (unsigned short)bits;
            pat.family = AF_INET6;
            if (!sortlist_alloc(sortlist, nsort, &pat)) return ARES_ENOMEM;
        } else if (ipbufpfx[0] && (bits = ares_inet_net_pton(AF_INET, ipbufpfx, &pat.addrV4,
                                                             sizeof(pat.addrV4))) > 0) {
            pat.type = PATTERN_CIDR;
            pat.mask.bits = (unsigned short)bits;
            pat.family = AF_INET;
        ...
        /* See if it is just a regular IP */
            ...
            if (ipbufpfx[0]) {
                memcpy(ipbuf, str, q - str);
                ipbuf[q - str] = '\0';
                if (ip_addr(ipbuf, q - str, &pat.mask.addr4) != 0) natural_mask(&pat);
            } else
                natural_mask(&pat);
            pat.family = AF_INET;
...
static int set_search(ares_channel channel, const char* str) {
    ...
    if (channel->ndomains != -1) {
        ...
        for (n = 0; n < channel->ndomains; n++) free(channel->domains[n]);
        free(channel->domains);
        channel->domains = NULL;
        channel->ndomains = -1;
    }
    ...
    while (*p) {
        ...
        if (!channel->domains[n]) return ARES_ENOMEM;
...
static int set_options(ares_channel channel, const char* str) {
    ...
    while (*p) {
        ...
        if (val && channel->ndots == -1) channel->ndots = aresx_sltosi(strtol(val, NULL, 10));
        val = try_option(p, q, "retrans:");
        if (val && channel->timeout == -1) channel->timeout = aresx_sltosi(strtol(val, NULL, 10));
        val = try_option(p, q, "retry:");
        if (val && channel->tries == -1) channel->tries = aresx_sltosi(strtol(val, NULL, 10));
        val = try_option(p, q, "rotate");
        if (val && channel->rotate == -1) channel->rotate = 1;
        p = q;
        while (ISSPACE(*p)) p++;
    }
...
static char* try_config(char* s, const char* opt, char scc) {
    ...
    *++q = '\0';
    ...
    if ((len = strlen(opt)) == 0) /* empty option */
        return NULL;

    if (strncmp(p, opt, len) != 0) /* line and option do not match */
        return NULL;

    /* skip over given option name */
    p += len;

    if (!*p) /* no option value */
        return NULL;
...
static int ip_addr(const char* ipbuf, ssize_t len, struct in_addr* addr) {
    ...
    if (addr->s_addr == INADDR_NONE && strcmp(ipbuf, "255.255.255.255") != 0) return -1;
    return 0;
}
...
static void randomize_key(unsigned char* key, int key_data_len) {
    ...
    if (!randomized) {
        for (; counter < key_data_len; counter++) key[counter] = (unsigned char)(rand() % 256);
    }
}
...
void ares_set_local_dev(ares_channel channel, const char* local_dev_name) {
    strncpy(channel->local_dev_name, local_dev_name, sizeof(channel->local_dev_name));
