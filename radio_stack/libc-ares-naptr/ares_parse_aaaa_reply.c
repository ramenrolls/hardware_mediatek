...
int ares_parse_aaaa_reply(const unsigned char* abuf, int alen, struct hostent** host,
    ...
    for (i = 0; i < (int)ancount; i++) {
        ...
        if (rr_class == C_IN && rr_type == T_AAAA && rr_len == sizeof(struct ares_in6_addr) &&
        ...
        if (rr_class == C_IN && rr_type == T_CNAME) {
            ...
            status = ares__expand_name_for_response(aptr, abuf, alen, &rr_data, &len);
            if (status != ARES_SUCCESS) break;
            free(hostname);
            hostname = rr_data;

            /* Take the min of the TTLs we see in the CNAME chain. */
            if (cname_ttl > rr_ttl) cname_ttl = rr_ttl;
        } else
            free(rr_name);
    ...
    if (status == ARES_SUCCESS) {
        ...
        if (naddrttls) {
            ...
            for (i = 0; i < n; i++) {
                ...
                if (addrttls[i].ttl > cname_ttl) addrttls[i].ttl = cname_ttl;
            }
            *naddrttls = n;
        }
        if (aliases) aliases[naliases] = NULL;
        if (host) {
            ...
            if (hostent) {
                ...
                if (hostent->h_addr_list) {
                    ...
                    for (i = 0; i < naddrs; i++) hostent->h_addr_list[i] = (char*)&addrs[i];
    ...
    if (aliases) {
        for (i = 0; i < naliases; i++) free(aliases[i]);
        free(aliases);
    }
    free(addrs);
    free(hostname);
