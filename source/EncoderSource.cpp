...
void EncoderSource::Rotator::createBufferProducerNotify(sp<AMessage>& Producer,
                                                        sp<AMessage>& Consumer) {
...
void EncoderSource::Rotator::rotate() {
    ...
    if (mFakeRotate) {
        while (!mInputBufferQueue.empty() && !mOutputBufferQueue.empty()) {
            ...
            if ((mbuf->meta_data().findInt32(kKeyViLTEToken, &token) && token >= 0) ||
                (mbuf->meta_data().findInt32(kKeyViLTELongTimeNoData, &longtime) &&
                 longtime == 1)) {
                VT_LOGV("[ID=%d][profile:token %d]fakeRotate longtime %d size %zu",
                        mMultiInstanceID, token, longtime, mbuf->range_length());
            }

            CHECK(mbuf->meta_data().findInt64(kKeyTime, &timeUs));

            if (abuf->size() == sizeof(VideoNativeMetadata)) {
                memcpy(abuf->data(), (const uint8_t*)mbuf->data() + mbuf->range_offset(),
                       mbuf->range_length());
            } else {
    ...
    while (!mInputBufferQueue.empty() && !mOutputBufferQueue.empty()) {
        ...
        // memcpy(&rotInfo,rotInfoP,sizeof(RotateInfo));//timing issue? ENS  change while rotate
        // use?
        ...
        if (mbuf->meta_data().findInt32(kKeyViLTELongTimeNoData, &longtime) && longtime == 1) {
            VT_LOGV("[ID=%d][Rotator][profile:token %d]long time not data from camera",
                    mMultiInstanceID, token);
        }
        ...
        if (data != NULL) {
            int64_t useTimeMs =
                    rotateBuffer(data, (uint8_t*)abuf->data(), &mRotateInfo, &outFillLen);
            if (useTimeMs < 0) {
                VT_LOGE("[ID=%d][Rotator][profile:token %d]rotateBuffer buffer timeUs %lld us use "
                        "%lld ms",
                        mMultiInstanceID, token, (long long)timeUs, (long long)useTimeMs);
            }

            VT_LOGD("[ID=%d][Rotator][profile:token %d]rotateBuffer buffer timeUs %lld us use %lld "
                    "ms abuf len %d longtime flag:%d",
                    mMultiInstanceID, token, (long long)timeUs, (long long)useTimeMs, outFillLen,
                    longtime);
    ...
    switch (msg->what()) {
        ...
        case kWhatInputBffer: {
            ...
            if (!mRunning && mbuf != NULL) {
                VT_LOGI("[ID=%d][Rotator]got a buf from puller %p after stop", mMultiInstanceID,
                        mbuf);
                mbuf->release();
                break;
            }
            ...
            if (mbuf != NULL) {
                ...
                CHECK(mbuf->meta_data().findInt32(kKeyViLTEToken, &token));
                CHECK(mbuf->meta_data().findInt64(kKeyTime, &timeUs));
                VT_LOGV("[ID=%d][Rotator][profile:token %d][buffer]got a buf from puller %p size "
                        "%zu timeUs %lld us",
        ...
        case kWhatOutputBffer: {
            ...
            VT_LOGV("[ID=%d][Rotator][buffer]got a buf from encoder %p @mRunning %d mStop %d",
                    mMultiInstanceID, buffer.get(), mRunning, mStop);

            if (!mStop) {
                mOutputBufferQueue.push_back(
                        buffer);  // output can be save when not workinkg, after start working
                                  // ,directly trigger by input buffer
            }
            ...
            if (mRunning) {
                ...
                // VT_LOGD("[ID=%d][buffer]got a buf from encoder %p @working %d
                // state",abuf.get(),mRunning);
            }
...
status_t EncoderSource::Rotator::postSynchronouslyAndReturnError(const sp<AMessage>& msg) {
...
EncoderSource::Puller::Puller(int32_t multiId, const sp<MediaSource>& source,
...
status_t EncoderSource::Puller::postSynchronouslyAndReturnError(const sp<AMessage>& msg) {
    sp<AMessage> response;
    status_t err = msg->postAndAwaitResponse(&response);
...
void EncoderSource::Puller::stop() {
    ...
    VT_LOGI("[ID=%d][Puller] Don't call vtcamerasource stop, source will handle it when ~",
            mMultiInstanceID);
...
void EncoderSource::Puller::setConfig(const sp<AMessage>& params) {
    ...
    if (params->findInt32("frame-rate", &fps) && fps > 0 && mTargetFrameRate != fps) {
        VT_LOGD("[ID=%d][Puller][DFR]change fps from %d to %d  ", mMultiInstanceID,
...
bool EncoderSource::Puller::framerateEvaluation(int64_t curFrmTimeUs) {
    ...
    if (mFrameDeltaTimeUs.size() < kMinItems) {
        VT_LOGV("[ID=%d][DFR]mFrameDeltaTimeUs.size() %zu", mMultiInstanceID,
                mFrameDeltaTimeUs.size());
        return false;
    }
...
bool EncoderSource::Puller::dropFrameAsAdaptive(int64_t curFrmTimeUs) {
    ...
    if (property_get("vendor.vt.src.framerate", value, NULL)) {
        ...
        if (mTargetFrameRate != framerate) {
            VT_LOGD("[ID=%d][Puller]change framerate from %d to %d", mMultiInstanceID,
    ...
    if (mTargetFrameRate == 30) {
        VT_LOGV("[ID=%d][Puller]no drop for mTargetFrameRate %d", mMultiInstanceID,
                mTargetFrameRate);
        return false;
    }
...
void EncoderSource::Puller::onMessageReceived(const sp<AMessage>& msg) {
    ...
    switch (msg->what()) {
        ...
        case kWhatPull: {
            ...
            if (mPaused) {
                if (err == OK && mbuf != NULL) {  // stop after pause resulted mbuf == NULL
                    VT_LOGD("[ID=%d][Puller]relase a buffer %p to camera while pause",
                            mMultiInstanceID, mbuf);
                    mbuf->release();
                    mbuf = NULL;
                }
            ...
            if (err != OK || mbuf == NULL) {
                ...
                resetFrameDropper();
                resetFrameEvaluator();
                msg->post(100000);  // switch camera will return EOS and NULL buffer, switch camera
                                    // may need 500ms,so we sleep larger time
                break;
            } else {
                if (mbuf != NULL) {  // if stop cmd enter, this will return NULL
                    ...
                    VT_LOGV("[ID=%d][Puller][profile:token %d]pull a buffer %p + %lld ms from "
                            "camera",
                            mMultiInstanceID, mCount, mbuf, (long long)timeUs / 1000);

                    if (dropFrameAsAdaptive(timeUs)) {
                        VT_LOGD("[ID=%d][Puller][profile:token %d]Relase a buffer %p + %lld ms to "
                                "camera as adaptive",
                                mMultiInstanceID, mCount, mbuf, (long long)timeUs / 1000);
                        mbuf->release();
                        mbuf = NULL;
        ...
        case kWhatPause: {
            sp<AReplyToken> replyID;
            mPaused = true;
            //-->2015 UT phase[add reasone]:after pause, will switch source, make sure the kWhatPull
            // not working,else will msg loop alot and get no buffer
            //-->2015-11-25[remove reason]:[should not reset mPullGeneration, as hide me-->pause
            // source,the cameraSource will generate buffercount buffer with the old timestamp after
            // pause after resume, the old timestamp buffer will first come back, which we don't
            // want
...
EncoderSource::EncoderSource(const sp<ALooper>& looper, const sp<AMessage>& outputFormat,
...
size_t EncoderSource::getInputBufferCount() const { return mEncoderInputBuffers.size(); }
...
status_t EncoderSource::start() {
    ...
    // 1)any module dirived base from AHandler, this->id() will generated after it is
    // registerHandler in this phase, the EncoderSource is not registerHandler by Source.cpp,so id()
    // is 0 by default so we can not new the rotateNotify for rotator, or else the msg can not find
    // its handler , failed
    ...
    mRotator->createBufferProducerNotify(notifyPullerOutBufferReadyToRotator,
                                         mNofityEncoderInputReadyToRotator);
    mPuller->setNotify(notifyPullerOutBufferReadyToRotator);

    mRotatorLooper->start(false /* runOnCallingThread */, false /* canCallJava */, PRIORITY_AUDIO);

    mPullerLooper->start(false /* runOnCallingThread */, false /* canCallJava */, PRIORITY_AUDIO);
...
status_t EncoderSource::initEncoder() {
    ...
    mEncoder = MediaCodec::CreateByType(mCodecLooper, outputMIME.c_str(), true /* encoder */);
    ...
    VT_LOGV("[ID=%d][EncS]output format is '%s'", mMultiInstanceID,
            mOutputFormat->debugString(0).c_str());

    status_t err = mEncoder->configure(mOutputFormat, NULL /* nativeWindow */, NULL /* crypto */,
...
void EncoderSource::releaseABuffer(sp<MediaCodecBuffer>& accessUnit) {
    ...
    if (accessUnit->meta()->findObject("mediaBufferHolder", &holder)) {
        mediaBuf = (holder != nullptr)
                           ? static_cast<MediaBufferHolder*>(holder.get())->mediaBuffer()
                           : nullptr;
    }
    if (mediaBuf != NULL) {
        ...
        VT_LOGE("why have not mediabuffer holder in ABuffer mEncoderInputBufferStoreInMeta %d ",
                mEncoderInputBufferStoreInMeta);
    }
...
status_t EncoderSource::postSynchronouslyAndReturnError(const sp<AMessage>& msg) {
    sp<AMessage> response;
    status_t err = msg->postAndAwaitResponse(&response);
        ...
        if (!(flags & MediaCodec::BUFFER_FLAG_EOS)) {
            ...
            if (!(flags & MediaCodec::BUFFER_FLAG_CODECCONFIG)) {
                if (mIsVideo) {
                    ...
                    VT_LOGV("[ID=%d][EncS][profile:token %d]out from encoder[video] time %" PRId64
                            " us (%.2f secs)",
...
void EncoderSource::onMessageReceived(const sp<AMessage>& msg) {
    ...
    switch (msg->what()) {
        ...
        case kWhatEncoderActivity: {
            ...
            VT_LOGV("[ID=%d][EncS]kWhatEncoderActivity  in %d out %d", mMultiInstanceID, numInput,
                    numOutput);
        ...
        case kWhatStop: {
            VT_LOGI("[ID=%d][EncS]kWhatStop encoder (%s) stopping", mMultiInstanceID,
            ...
            if (mStopping) {
                ...
                VT_LOGI("[ID=%d][EncS]encoder (%s) stopping @ mStopping", mMultiInstanceID,
                        mIsVideo ? "video" : "audio");
                break;
            }
            ...
            mStopping = true;
            releaseEncoder();
            VT_LOGI("[ID=%d][EncS]kWhatStop encoder (%s) stopped", mMultiInstanceID,
                    mIsVideo ? "video" : "audio");
        ...
        case kWhatResume: {
            ...
            if (mStopping) {
                ...
                VT_LOGI("[ID=%d][EncS]EncoderSource (%s) resuming", mMultiInstanceID,
                        mIsVideo ? "video" : "audio");
                // requestIDRFrame();
                requestFullIDRFrame();  // change to full IDR, because we do not know whether the
                                        // downlink decoder reset or not
                CHECK(mPuller.get() != NULL);
...
status_t EncoderSource::feedEncoderInputBuffersAfterRotate() {
    ...
    while (!mInputABufferQueue.empty()) {
        ...
        VT_LOGI("[ID=%d][EncS][profile:token %d]queue Encoder InputBuffer :timeUs %lld us longtime "
                "flag:%d",
