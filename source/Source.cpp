...
Source::Source(int32_t multiId, uint32_t simID, uint32_t operatorID)
    ...
    if (property_get("persist.vendor.vt.vilte_rci", value, NULL)) {  // resolution change interval
        ...
        VT_LOGI("[ID=%d]mResolutionChangeIntervalS %d", mMultiInstanceID,
                mResolutionChangeIntervalS);
    }
...
status_t Source::setDeviceRoateDegree(int32_t degree) {
    ...
    if (reset) {
        resetRotateDegree();
        refineWhByRotationDegree(&(mVideoConfigSettings.width), &(mVideoConfigSettings.height),
                                 mRotateDegree);
        setSourceResolutionDegree();
    }
...
status_t Source::setCurrentCameraInfo(int32_t facing, int32_t degree, int32_t hal) {
    ...
    VT_LOGI("[ID=%d]in state %x facing %d -> %d ,degree %d -> %d,HAL %d->%d", mMultiInstanceID,
            mState, mChooseCameraFacing, facing, mChooseCameraDegree, degree, mChooseCameraHal,
            hal);
...
status_t Source::disableCVO(bool disable) {
    Mutex::Autolock autoLock(mLock);
    VT_LOGI("[ID=%d]in state %x facing %d degree %d,mRotateTarget%d,mRotateMethod %d",
            mMultiInstanceID, mState, mChooseCameraFacing, mChooseCameraDegree, mRotateTarget,
            mRotateMethod);
...
status_t Source::Start() {
    Mutex::Autolock autoLock(mLock);  // avoid multicall
    VT_LOGI("[ID=%d][%p]Start in state 0x%x  nowUs =%lld us,++++ ", mMultiInstanceID, this, mState,
            (long long)ALooper::GetNowUs());
    ...
    if (mChooseCameraFacing < 0 || mChooseCameraDegree < 0 || mDeviceDegree < 0 ||
        mRotateDegree < 0) {
...
status_t Source::Pause() {
    ...
    if (mState & PAUSED || mState & STOPPED) {
        VT_LOGW("[ID=%d]still in state paused or stoped, why pause again? do nothing",
                mMultiInstanceID);
        return OK;
    }
...
status_t Source::setAvpfParamters(const sp<AMessage>& params) {
    VT_LOGI("[ID=%d][%p]Start in state 0x%x params:%s", mMultiInstanceID, this, mState,
            params->debugString(0).c_str());
    Mutex::Autolock autoLock(mLock);  // avoid multicall
...
status_t Source::adjustEncBitRate(int32_t expect_bitrate) {
    VT_LOGI("[ID=%d]Start in state 0x%x expect_bitrate:%d", mMultiInstanceID, mState,
            expect_bitrate);
    Mutex::Autolock autoLock(mLock);  // avoid multicall
    ...
    switch (msg->what()) {
        ...
        case kWhatSourceNotify: {
            ...
            if (what == EncoderSource::kWhatAccessUnit) {
                ...
                VT_LOGD("[ID=%d][profile:token %d]source got buffer size  %zu timeUs %lld "
                        "csdBuffer %d, I frame %d",
                        mMultiInstanceID, token, buffer->size(), (long long)timeUs, csdBuffer,
                        Iframe);
                ...
                /*
                    0: Front-facing camera, facing the user. If camera direction is unknown then
                   this value is used. 1: Back-facing camera, facing away from the user.

                */
                if ((mChooseCameraFacing < 0) || (mChooseCameraFacing > 1)) {
                    VT_LOGE("[ID=%d]error mChooseCameraFacing %d", mMultiInstanceID,
                            mChooseCameraFacing);
        ...
        case kWhatCodecAvpfFeedBack: {
            VT_LOGI("[ID=%d]kWhatCodecAvpfFeedBack: %s", mMultiInstanceID,
        ...
        /*
                case kWhatClearBufferQueue:
                {
                    ...
                    {
                        VT_LOGD("[ID=%d][source]relase a buffer %p to camera while
           stop",mMultiInstanceID,mbuf); mbuf->release(); mbuf = NULL;
                    }
                    msg->post(33000);
                    break;
                }*/
...
status_t Source::stop_l() {
    ...
    if (mState & STOPPED) {
        VT_LOGD("[ID=%d]has beed stopped, not stop again, but we will disconnect camera",
                mMultiInstanceID);
        return OK;
    }
...
status_t Source::startEncoderSource_l() {
    ...
    if (err == OK) {
        mCameraSource->setMaxAcquiredBufferCount(
                mEncoderSource->getInputBufferCount());  // valid after init
        refineWhByRotationDegree(&(mVideoConfigSettings.width), &(mVideoConfigSettings.height),
                                 mRotateDegree);
        setSourceResolutionDegree();
...
status_t Source::setupVideoEncoder(sp<MediaSource> cameraSource) {
    ...
    if (cameraSource.get() != NULL) {
        ...
        if (mCameraSource.get() &&
            mCameraSource->metaDataStoredInVideoBuffers() == kMetadataBufferTypeANWBuffer) {
        ...
        VT_LOGD("[ID=%d]cameraSource != NULL,Real Camera Recording:width=%d, height=%d, "
                "encodeWidth %d,encodeHeight %d, color-format=%x,",
                mMultiInstanceID, camera_out_width, camera_out_height, mVideoConfigSettings.width,
                mVideoConfigSettings.height, mColorFormat);
    } else {
    ...
    VT_LOGD("[ID=%d]setupVideoEncoder:  mVideoEncoder %d mVideoConfigSettings.width=%d, "
            "mVideoConfigSettings.height=%d \n",
            mMultiInstanceID, mVideoEncoder, mVideoConfigSettings.width,
            mVideoConfigSettings.height);

    VT_LOGD("[ID=%d]setupVideoEncoder bitrate=%d, frame-rate=%d,\n i-frame-interval=%d",
            mMultiInstanceID, mVideoConfigSettings.bitrate, mVideoConfigSettings.framerate,
            mVideoConfigSettings.IFrameIntervalSec);
    VT_LOGD("[ID=%d]setupVideoEncoder profile=%d, level=%d setViLTEMode", mMultiInstanceID,
            mVideoConfigSettings.profile, mVideoConfigSettings.level);

    mEncoderOutputFormat->setInt32("profile", convertToOMXProfile(mVideoConfigSettings.mimetype,
                                                                  mVideoConfigSettings.profile));
    mEncoderOutputFormat->setInt32(
            "level", convertToOMXLevel(mVideoConfigSettings.mimetype, mVideoConfigSettings.profile,
    ...
    if (mCameraSource.get() &&
        ...
        mEncoderOutputFormat->setInt32("android._input-metadata-buffer-type",
    ...
    mEncoderSource = new EncoderSource(mEncoderLooper, mEncoderOutputFormat, cameraSource, notify,
                                       mMultiInstanceID);
    ...
    if (mEncoderSource.get() == NULL) {
        ...
        // we should not stop vtcamerasource, as stop will disconnect consumer, the bufferqueue can
        // not be used
        return UNKNOWN_ERROR;
    }

    mEncoderLooper->registerHandler(mEncoderSource);
    mEncoderLooper->start(false /* runOnCallingThread */, false /* canCallJava */, PRIORITY_AUDIO);
    // ever adaptive reolution before,resoution change->pause->resume->tell the adaptive resolution
...
int32_t Source::adjustEncBySignal() {
    ...
    VT_LOGI("OPTest.RTP TestMode=%d RTPMap=0x%x 0x1=%d interfaceType=%d", TestMode, RTPMap,
            (RTPMap & 0x1), mVideoConfigSettings.interfaceType);
    ...
    if (revisedBitrate > 0 && revisedBitrate < mVideoConfigSettings.bitrate) {
        VT_LOGI("change bitrate from %d to %d by signal", mVideoConfigSettings.bitrate,
                revisedBitrate);
...
status_t Source::postAsynAndWaitReturnError(const sp<AMessage>& msg) {
...
int32_t Source::compareSettingChange(video_enc_fmt* settings) {
    ...
    if (strcasecmp(mVideoConfigSettings.mimetype, settings->mimetype)) {
        VT_LOGD("[ID=%d]mime change from %s to %s ", mMultiInstanceID,
                mVideoConfigSettings.mimetype, settings->mimetype);
    ...
    if ((mVideoConfigSettings.bitrate != settings->bitrate) && settings->bitrate > 0){
        VT_LOGD("[ID=%d]bitrate change from %d to %d
    ",mMultiInstanceID,mVideoConfigSettings.bitrate,settings->bitrate); changeType |=
    Update_Bitrate;
    }
    if ((mVideoConfigSettings.framerate!= settings->framerate) && settings->framerate > 0){
        VT_LOGD("[ID=%d]framerate change from %d to %d
    ",mMultiInstanceID,mVideoConfigSettings.framerate,settings->framerate); changeType |=
    Update_Fps;
    }
    if( settings->width > 0 && settings->height > 0){
        ...
        || mVideoConfigSettings.height != settings->height){
            ...
            //if(mVideoConfigSettings.width < settings->width || mVideoConfigSettings.height <
    settings->height){
                //changeType |= Update_Resolution_Up;
            //}else{
                //changeType |= Update_Resolution_Down;
                changeType |= Update_Resolution;//if resolution changed by SDP, always reset camera
    and encoder
            //}
        }
    }
    ...
    if ((mVideoConfigSettings.IFrameIntervalSec != settings->IFrameIntervalSec)
        ...
        VT_LOGD("[ID=%d]IFrameIntervalSec change from %d to %d
    ",mMultiInstanceID,mVideoConfigSettings.IFrameIntervalSec,settings->IFrameIntervalSec);
    ...
    if ((mVideoConfigSettings.profile != settings->profile && settings->profile > 0) ||
        ...
        VT_LOGD("[ID=%d]profile change from %d to %d ", mMultiInstanceID,
                mVideoConfigSettings.profile, settings->profile);
    ...
    {
        VT_LOGD("[ID=%d]profile Level not change, so WH,fps,Iinterval,we trust before ",
                mMultiInstanceID);
    }
    ...
    if ((mVideoConfigSettings.bitrate != settings->bitrate) && settings->bitrate > 0) {
        VT_LOGD("[ID=%d]bitrate change from %d to %d ", mMultiInstanceID,
                mVideoConfigSettings.bitrate, settings->bitrate);
        changeType |= Update_Bitrate;
    }
...
status_t Source::checkSaveSourceConfig(video_enc_fmt* settings) {
    ...
    memcpy(&mVideoConfigSettings, settings,
           sizeof(video_enc_fmt));  // copy will cause the internal WidthAdapt is overwitten
    ...
    VT_LOGD("[ID=%d]encoder format %d  width=%d,  height=%d \n", mMultiInstanceID, mVideoEncoder,
            mVideoConfigSettings.width, mVideoConfigSettings.height);

    VT_LOGD("[ID=%d]bitrate=%d, frameRate=%d,  i-frame-interval=%d,", mMultiInstanceID,
            mVideoConfigSettings.bitrate, mVideoConfigSettings.framerate,
            mVideoConfigSettings.IFrameIntervalSec);
    ...
    bool retV =
            (getMediaProfile(
                     mOperatorID, ((mVideoEncoder == VIDEO_ENCODER_H264) ? VIDEO_H264 : VIDEO_HEVC),
    ...
    if (mVideoConfigSettings.bitrate <= 0) {
        ...
        VT_LOGD("[ID=%d]reset bitreta to %d by codec setting ", mMultiInstanceID,
                mVideoConfigSettings.bitrate);
    }

    // if(mVideoConfigSettings.framerate <= 0){
    mVideoConfigSettings.framerate = mediaProfile.fps;
    VT_LOGD("[ID=%d]reset framerate to %d by codec setting ", mMultiInstanceID,
            mVideoConfigSettings.framerate);
    //}
    // if(mVideoConfigSettings.width <= 0) {
    mVideoConfigSettings.width = mediaProfile.width;
    VT_LOGD("[ID=%d]reset width to %d by codec setting ", mMultiInstanceID,
            mVideoConfigSettings.width);
    //}
    // if(mVideoConfigSettings.height <= 0) {
    mVideoConfigSettings.height = mediaProfile.height;
    VT_LOGD("[ID=%d]reset height to %d by codec setting ", mMultiInstanceID,
            mVideoConfigSettings.height);
    //}
    // if(mVideoConfigSettings.IFrameIntervalSec <= 0){
    mVideoConfigSettings.IFrameIntervalSec = mediaProfile.Iinterval;
    VT_LOGD("[ID=%d]reset IFrameIntervalSec to %d by codec setting ", mMultiInstanceID,
            mVideoConfigSettings.IFrameIntervalSec);
    //}
    // if init with  WH [0,0]
    refineWhByRotationDegree(&(mVideoConfigSettings.width), &(mVideoConfigSettings.height),
    ...
    if (mState & STARTED) {  // video call is start now
        ...
        if ((changeType & Update_Resolution_Up) ||
            ((changeType & Update_Resolution) &&
             (mResChangeAlgorithm ==
              Encode_CAMERA_RESET))) {  // if size change to bigger than original setting
            notifyResolutionDegree(mVideoConfigSettings.width, mVideoConfigSettings.height, 0);
...
void Source::notifyResolutionDegree(int32_t width, int32_t height, int32_t degree) {
    ...
    notify->post();
    VT_LOGD("[ID=%d][%p]W %d H %d degree %d", mMultiInstanceID, this, MAX(width, height),
            MIN(width, height), degree);
}
...
status_t Source::setSourceResolutionDegree() {
    ...
    status_t err = mCameraSource->setResolutionDegree(tempSrcW, tempSrcH, tempTarW, tempTarH,
                                                      mRotateDegree, mRotateMethod);
...
status_t Source::resetRotateDegree() {
    ...
    VT_LOGI("[ID=%d]mChooseCameraDegree %d,Facing %d  ,mDeviceDegree %d ,mRotateDegree "
            "%d,mRotateTarget %d",
            mMultiInstanceID, mChooseCameraDegree, mChooseCameraFacing, mDeviceDegree,
            mRotateDegree, mRotateTarget);
    return OK;
}
int32_t Source::evaluateBitrateAdaptive(int32_t expBitrate) {
    ...
    if (mCameraSource.get() &&
        mCameraSource->metaDataStoredInVideoBuffers() == kMetadataBufferTypeANWBuffer) {
        VT_LOGD("[ID=%d]bitrate:  %d ==> %d", mMultiInstanceID, mVideoConfigSettings.bitrate,
                expBitrate);
    ...
    */
    ...
    VT_LOGD("[ID=%d]bitrate:  %d ==> %d", mMultiInstanceID, mVideoConfigSettings.bitrate,
            expBitrate);
    bool retV =
            (getAdaptativeFrameRateInfo(
                     mOperatorID, ((mVideoEncoder == VIDEO_ENCODER_H264) ? VIDEO_H264 : VIDEO_HEVC),
                     mVideoConfigSettings.profile, mVideoConfigSettings.level, expBitrate,
                     &tarBitrate, &tarFramerate, curW, curH, &tarW, &tarH) == 0);

    VT_LOGD("[ID=%d]bitrate  %d ==> %d,fps from 30 ==> %d,W*H [%d,%d][%d,%d]==> W*H "
            "[%d,%d],Algorithm %d",
            mMultiInstanceID, mVideoConfigSettings.bitrate, tarBitrate, tarFramerate,
            mVideoConfigSettings.width, mVideoConfigSettings.height,
    ...
    if (mResolutionChangeTimeUs > 0) {
        enableChangeCheck =
                ((mTempTimeUs - mResolutionChangeTimeUs) >= mResolutionChangeIntervalS * 1000000ll);
    } else {
    ...
    if (tarW > 0 && tarH > 0 && enableChangeCheck) {
        if ((curW != tarW) && (curH != tarH)) {  // only support same ratio change
            if (mVideoConfigSettings.width < (int32_t)tarW ||
                mVideoConfigSettings.height < (int32_t)tarH) {
                VT_LOGE("[ID=%d]error change Update_Resolution_Up,skip!!!", mMultiInstanceID);
                // changeType |=Update_Resolution_Up;
                // mVideoConfigSettings.width  = tarW;
                // mVideoConfigSettings.height  = tarH;//as we must reset camera and encoder, so
                // reset the origin value WH
            } else {
    ...
    VT_LOGD("[ID=%d]W*H [%d,%d][%d,%d] changeType %x,enableChangeCheck %d,mResolutionChangeTimeMs "
            "%lld ",
            mMultiInstanceID, mVideoConfigSettings.width, mVideoConfigSettings.height,
            mVideoConfigSettings.widthAdapt, mVideoConfigSettings.heightAdapt, changeType,
            enableChangeCheck, mResolutionChangeTimeUs / 1000ll);
...
status_t Source::setCodecParameters(int32_t changeType) {
    ...
    if (mEncoderSource.get() == NULL) {
        VT_LOGW("[ID=%d]encoder is not availble [%x:pause/home/hold/not start],next setup will "
                "follow this setting!!!",
                mMultiInstanceID, mState);
        return -1;
    }
    if (mCameraSource.get() &&
        mCameraSource->metaDataStoredInVideoBuffers() == kMetadataBufferTypeANWBuffer) {
        if ((changeType & (Update_Bitrate | Update_RotationDegree)) == 0)
            return OK;  // only allowa bitrate and degree
    } else {
    ...
    if (changeType & (Update_Resolution) && mVideoConfigSettings.widthAdapt > 0 &&
