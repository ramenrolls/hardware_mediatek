...
status_t ImsMediaSender::SetSourceConfig(video_codec_fmtp_t* video_codec_param, int32_t type) {
    ...
    if (video_codec_param->format == VIDEO_H264) {
        ...
        memcpy((char*)sprop_parameter_sets, (char*)(h264_encoder_param->sprop_parameter_sets),
        ...
        sp<ABuffer> buf = MakeAVCCodecSpecificData(sprop_parameter_sets, &profile, &level, &width,
                                                   &height, &sarWidth, &sarHeight);
        ...
        VT_LOGD("[ID=%d]source codec config is  profile %d,level %d", mMultiInstanceID, profile,
                level);
        ...
        if (updateConfig) {
            ...
            if (mConfig->profile != profile) {
                VT_LOGD("[ID=%d]profile change from %d to %d ", mMultiInstanceID, mConfig->profile,
                        profile);
            }

            if (mConfig->level != level) {
                VT_LOGD("[ID=%d]level change from %d to %d ", mMultiInstanceID, mConfig->level,
                        level);
            }

            if (mConfig->maxBitrate != (int32_t)(h264_encoder_param->max_br * 1000)) {
                VT_LOGD("[ID=%d]maxBitrate change from %d to %d ", mMultiInstanceID,
                        mConfig->maxBitrate, h264_encoder_param->max_br * 1000);
            }

            if (mConfig->bitrate != (int32_t)(h264_encoder_param->video_b_as * 1000)) {
                VT_LOGD("[ID=%d]maxBitrate change from %d to %d ", mMultiInstanceID,
                        mConfig->bitrate, h264_encoder_param->video_b_as * 1000);
            }
            ...
            if (mConfig->width != width) {
                VT_LOGD("[ID=%d]width change from %d to %d ", mMultiInstanceID, mConfig->width,
                        width);
            }

            if (mConfig->height != height) {
                VT_LOGD("[ID=%d]height change from %d to %d ", mMultiInstanceID, mConfig->height,
                        height);
            }

            if (!strcasecmp(mConfig->mimetype, MEDIA_MIMETYPE_VIDEO_AVC)) {
                VT_LOGD("[ID=%d]mimetype change from %s to VIDEO_H264 ", mMultiInstanceID,
                        mConfig->mimetype);
            }
        }
        ...
        mConfig->maxBitrate =
                h264_encoder_param->max_br *
                1000;  // video bitrate in units of 1000 bits per second for the VCL HRD
        ...
        int32_t offset = 0;
        memcpy((char*)(sprop_vps_sps_pps + offset), (char*)(hevc_encoder_param->sprop_vps),
               sprop_vps_size);
        offset += sprop_vps_size;
        sprop_vps_sps_pps[offset] = ',';
        offset += 1;

        memcpy((char*)(sprop_vps_sps_pps + offset), (char*)(hevc_encoder_param->sprop_sps),
               sprop_sps_size);
        offset += sprop_sps_size;
        sprop_vps_sps_pps[offset] = ',';
        offset += 1;

        memcpy((char*)(sprop_vps_sps_pps + offset), (char*)(hevc_encoder_param->sprop_pps),
               sprop_pps_size);
        ...
        if (updateConfig) {
            ...
            if (mConfig->maxBitrate != (int32_t)(hevc_encoder_param->max_br * 1000)) {
                VT_LOGD("[ID=%d]maxBitrate change from %d to %d ", mMultiInstanceID,
                        mConfig->maxBitrate, hevc_encoder_param->max_br * 1000);
            }

            if (mConfig->bitrate != (int32_t)(hevc_encoder_param->video_b_as * 1000)) {
                VT_LOGD("[ID=%d]maxBitrate change from %d to %d ", mMultiInstanceID,
                        mConfig->bitrate, hevc_encoder_param->video_b_as * 1000);
            }
            ...
            if (!strcasecmp(mConfig->mimetype, MEDIA_MIMETYPE_VIDEO_HEVC)) {
                VT_LOGD("[ID=%d]mimetype change from %s to VIDEO_HEVC ", mMultiInstanceID,
                        mConfig->mimetype);
            }
        }
