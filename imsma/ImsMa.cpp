...
status_t ImsMa::MediaSession::Init(uint32_t simID, uint32_t operatorID) {
    ...
    mMsLooper->start(false /* runOnCallingThread */, false /* canCallJava */, PRIORITY_AUDIO);
    ...
    mMrLooper->start(false /* runOnCallingThread */, false /* canCallJava */, PRIORITY_AUDIO);
    ...
    mMediaSender->init(mRtpC, simID,
                       operatorID);  // let mediasender to queue source au to rtp module
...
void ImsMa::MediaSession::onMessageReceived(const sp<AMessage>& msg) {
    switch (msg->what()) {
        case kWhatMANotify: {
            ...
            switch (reason) {
                ...
                case kWhatInitSource: {
                    ...
                    err = mMediaSender->SetSourceConfig((video_codec_fmtp_t*)param,
                                                        type);  // params can not match
                ...
                case kWhatInitSink: {
                    ...
                    err = mMediaReceiver->SetSinkConfig(
                            (video_codec_fmtp_t*)param);  // params can not match
                ...
                case kWhatUpdateSourceConfig: {
                    ...
                    err = mMediaSender->UpdateSourceConfig((video_codec_fmtp_t*)param,
                                                           type);  // params can not match
                ...
                case kWhatUpdateSinkConfig: {
                    ...
                    err = mMediaReceiver->UpdateSinkConfig(
                            (video_codec_fmtp_t*)param);  // params can not match
                ...
                case kWhatStartRtp: {
                    ...
                    msg->findInt32("streamsIsAlive",
                                   &streamsIsAlive);  // not check this, only special add this key
                ...
                case kWhatSetRecordParameter: {
                    ...
                    // TODO:: convert
                    //                    err = mMediaReceiver->SetRecordParameters(quality,
                    //                    (char*)path);
                ...
                case kWhatStartRecord: {
                    ...
                    //                  static_cast<record_mode_t>(recordMe);
                    //                    record_mode_t recordMode;
        ...
        case kWhatImsMediaReceiverNotify: {
            int32_t reason;
            CHECK(msg->findInt32("reason", &reason));
            VT_LOGI("[ID=%d]kWhatImsMediaReceiverNotify: %s", mMultiInstanceID,
                    ...
                    /*
                            case ImsMediaReceiver::kWhatAvpfFeedBack: {
                                ...
                                if(params->findInt32("avpf-sli",&avpf_type)) {
                                    sp<ABuffer> buffer;
                                    CHECK(params->findBuffer("sli", &buffer) && buffer.get() !=
                       NULL); sp<AMessage> errInfo = new AMessage; errInfo->setInt32("err",
                       IRTPController::DecErr_SliceLossIndication); errInfo->setBuffer("sli",
                       buffer);    //buffer from decoder
                                ...
                                if(params->findInt32("pli",&avpf_type)) {
                                    ...
                                    errInfo->setInt32("err",
                       IRTPController::DecErr_PictureLossIndication);
                                    mRtpC->feedBackDecoderError(errInfo,IMSMA_RTP_VIDEO);
                                ...
                                if(params->findInt32("fir",&avpf_type)) {
                                    sp<AMessage> errInfo = new AMessage;
                                    errInfo->setInt32("err",
                       IRTPController::DecErr_FullIntraRequest);
                                    mRtpC->feedBackDecoderError(errInfo,IMSMA_RTP_VIDEO);
                                }
        ...
        case kWhatImsMediaSenderNotify: {
            int32_t reason;
            CHECK(msg->findInt32("reason", &reason));
            VT_LOGI("[ID=%d]kWhatImsMediaSenderNotify: %s", mMultiInstanceID,
                    msg->debugString(0).c_str());
                ...
                case ImsMediaSender::kWhatError: {
                    ...
                    if (ImsMediaSender::Error_Bitrate_Drop_Down_Fail == err) {
                        if (mNotifyDowngrade == false || mAlreadyNotifyDowngrade == true) {
                            ALOGD("don't notify DropBitrate mNotifyDowngrade=%d "
                                  "mAlreadyNotifyDowngrade=%d",
        ...
        case kWhatIRTPControllerNotify: {
            ...
            VT_LOGI("[ID=%d]kWhatIRTPControllerNotify: %s", mMultiInstanceID,
                    msg->debugString(0).c_str());
            ...
            switch (reason) {
                ...
                case IRTPController::kWhatDropCall: {
                    ...
                    if (needNotifyDropCall() == true) {
                        ...
                        ALOGD("RTPController notify kWhatDropCall to VTS no need for op=%d",
                              mOperatorID);
                    }
...
void ImsMa::setLowPowerMode(bool enable) { configLowPowerMode(CONFIG_GED_VILTE_MODE, enable); }
...
status_t ImsMa::setSensorParameters(sensor_info_vilte* sensor, int32_t sensorCnt) {
    ...
    VT_LOGD("[ID=%d]we have %d sensor mSensorInfos = %p", sMultiInstanceID, sensorCnt,
            mSensorInfos);
    ...
    for (int32_t i = 1; i <= sensorCnt; i++) {
        ...
        VT_LOGI("tempLocal: %p,index %d W:%d, H:%d, D:%d,F:%d,hal %d", tempLocal, tempLocal->index,
                tempLocal->max_width, tempLocal->max_height, tempLocal->degree, tempLocal->facing,
                tempLocal->hal);
    ...
    VT_LOGD("[ID=%d]max w %d,max h %d, prefer %d", sMultiInstanceID,
            mSensorResolution->sensor_max_width, mSensorResolution->sensor_max_height,
            mSensorResolution->prefer_WH_ratio);
    return OK;
}

int32_t ImsMa::getCodecCapability(video_codec_fmtp_t** codeccap, int32_t* prefer_WHRatio,
                                  uint32_t operatorID, VT_Quality quality) {
    ...
    *codeccap = mGetCodecCapPtr;
    ...
    VT_LOGV("[ID=%d][exit]get capNumbers %d mGetCodecCapPtr %p", sMultiInstanceID, capNumbers,
            mGetCodecCapPtr);
    return capNumbers;
}

void ImsMa::getCodecBitrateTable(uint32_t* tableCnt, video_media_bitrate_t** bitrateTable,
                                 uint32_t operatorID) {
    ...
    VT_LOGD("[ID=%d]tableCnt %d,mGetCodecBitrateTablePtr%p", sMultiInstanceID, count,
            mGetCodecBitrateTablePtr);

    *bitrateTable = mGetCodecBitrateTablePtr;
    *tableCnt = count;

    // printCodecBitrateTable(count,mGetCodecBitrateTablePtr);
    VT_LOGD("[ID=%d][exit]count %d=mGetCodecBitrateTablePtr %p======", sMultiInstanceID, count,
            mGetCodecBitrateTablePtr);
}

void ImsMa::getCodecParameterSets(video_format_t informat,    // in
                                  uint32_t* levelCapNumbers,  // out
                                  video_codec_level_fmtp_t** codeclevelcap, uint32_t operatorID,
                                  VT_Quality quality) {
    // int32_t retVal;
    ...
    // get count
    getCodecLevelParameterSets(operatorID, quality, mSensorResolution, informat, &Numbers, NULL);
    ...
    if (Numbers > 0) {
        ...
        getCodecLevelParameterSets(operatorID, quality, mSensorResolution, informat, &Numbers,
                                   mGetVideoCodecLevelFmtpPtr);
    }

    *levelCapNumbers = Numbers;
    *codeclevelcap = mGetVideoCodecLevelFmtpPtr;
    VT_LOGD("[ID=%d][exit]===Numbers %d ===mGetVideoCodecLevelFmtpPtr %p=", sMultiInstanceID,
            Numbers, mGetVideoCodecLevelFmtpPtr);
}
...
ImsMa::ImsMa(uint32_t simID, uint32_t operatorID)
    : mSimID(simID), mOperatorID(operatorID), mFlags(0), mQuality(-1), mMultiInstanceID(-1) {
    sMultiInstanceID++;
    mMultiInstanceID = sMultiInstanceID;
    VT_LOGD("[ID=%d]create simID %d,mOperatorID %d++++++++++ ", mMultiInstanceID, mSimID,
            mOperatorID);
...
ImsMa::~ImsMa() {
    METHOD_WITH_ID_THIS(mMultiInstanceID, "enter", this);
    VT_LOGI("[ID=%d]delete:flags %x,%p %p---------------", mMultiInstanceID, mFlags,
            mGetCodecCapPtr, mGetCodecBitrateTablePtr);
    ...
    if(mGetCodecBitrateTablePtr != NULL) {
        VT_LOGI("[ID=%d]free mGetCodecBitrateTablePtr
    %p",mMultiInstanceID,mGetCodecBitrateTablePtr); free(mGetCodecBitrateTablePtr);
        mGetCodecBitrateTablePtr = NULL;
    }*/
...
void ImsMa::PreConnect(int fd, char* ifname, uint32_t network_id) {
    if (strlen(ifname) != 0) {
        ...
        VT_LOGI("set ifname: size=%zu sockfd=%d str=%s:%s\n", strlen(ifname), fd, ifname,
                ifr.ifr_name);

        if (setsockopt(fd, SOL_SOCKET, SO_BINDTODEVICE, &ifr, sizeof(ifr)) < 0) {
            VT_LOGE("%s: socket SO_BINDTODEVICE failed %s(%d)", __FUNCTION__, strerror(errno),
                    errno);
        }
...
int ImsMa::ConnectSock(int fd, uint8_t type, uint16_t port, uint8_t* address) {
    ...
    if (connect(fd, (struct sockaddr*)&remote_addr, addr_len) < 0) {
        // SOCKETWRAPPER_ASSERT(0, "%s: socket connect failed %s(%d)", __FUNCTION__,
        // strerror(errno), errno); tester maybe hot plug sim card, so we set ERROR flag instead of
        // trigger exception
...
int ImsMa::bindSock(uint8_t type, uint16_t port, uint8_t* address) {
    ...
    if (setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, &n, sizeof(int)) < 0) {
        VT_LOGI("[ID=%d]socket setsockopt SO_REUSEADDR failed %s(%d)", mMultiInstanceID,
    ...
    if (setsockopt(sockfd, SOL_SOCKET, SO_REUSEPORT, &m, sizeof(int)) < 0) {
        VT_LOGI("[ID=%d] socket setsockopt SO_REUSEPORT failed %s(%d)", mMultiInstanceID,
                strerror(errno), errno);
    }
    ...
    if (setsockopt(sockfd, SOL_SOCKET, SO_RCVBUF, &q, sizeof(int)) < 0) {
        VT_LOGE("[ID=%d] socket setsockopt SO_RCVBUF failed %s(%d)", mMultiInstanceID,
                strerror(errno), errno);
    } else {
...
status_t ImsMa::makeMyselfSocket(rtp_rtcp_config_t* config) {
    VT_LOGI("[ID=%d]\t makeMyselfSocket:local rtp port:%d rtcp port:%d", mMultiInstanceID,
            config->network_info.local_rtp_port, config->network_info.local_rtcp_port);
    ...
    if (mRTPfd == -1) {
        mRTPfd = bindSock(config->network_info.local_addr_type, config->network_info.local_rtp_port,
                          config->network_info.local_rtp_address);
        if (mRTPfd > 0) {
            PreConnect(mRTPfd, config->network_info.ifname, config->network_info.network_id);
            if (ConnectSock(mRTPfd, config->network_info.remote_addr_type,
                            config->network_info.remote_rtp_port,
                            config->network_info.remote_rtp_address) < 0) {
                mRTPfd = -1;
            }
        }
    }
    ...
    if (mRTCPfd == -1) {
        mRTCPfd =
                bindSock(config->network_info.local_addr_type, config->network_info.local_rtcp_port,
                         config->network_info.local_rtcp_address);
        if (mRTCPfd > 0) {
            PreConnect(mRTCPfd, config->network_info.ifname, config->network_info.network_id);
            if (ConnectSock(mRTCPfd, config->network_info.remote_addr_type,
                            config->network_info.remote_rtcp_port,
                            config->network_info.remote_rtcp_address) < 0) {
                mRTCPfd = -1;
            }
        }
    }
    ...
    VT_LOGI("[ID=%d]makeMyselfSocket: fd0=%d fd1=%d", mMultiInstanceID,
            config->network_info.socket_fds[0], config->network_info.socket_fds[1]);
...
status_t ImsMa::InitMediaConfig(media_config_t* config) {
    ...
    if (errorCsd) {
        ...
        memset(h264_decoder_param->sprop_parameter_sets, 0,
               sizeof(VOLTE_MAX_SDP_PARAMETER_SET_LENGTH));
        memset(h264_decoder_param->sprop_level_parameter_sets, 0,
               sizeof(VOLTE_MAX_SDP_PARAMETER_SET_LENGTH));
    }
...
status_t ImsMa::CheckSetBR(media_config_t* config) {
    ...
    VT_LOGD("[ID=%d]CheckSetBR baseinfo format=%d, profile=%d, level=%d", mMultiInstanceID,
            config->codec_param.format, profile, level);

    CHECK(getMediaProfile(mOperatorID, config->codec_param.format, profile, level, &mediaProfile) ==
          0);
    ...
    VT_LOGD("[ID=%d]CheckSetBR wantTo set uplink MBR=%d AS=%f preconfig=%d wantTo=%f",
            mMultiInstanceID, config->rtp_rtcp_cap.network_info.MBR_UL, as, mediaProfile.bitrate,
            wantTo);
...
status_t ImsMa::UpdateMediaConfig(media_config_t* config) {
    ...
    if (config->codec_param.format == VIDEO_H264) {
        VT_LOGD("[ID=%d]mFlags %x VIDEO_H264 MBR_UL=%d  video_b_as=%d", mMultiInstanceID, mFlags,
                config->rtp_rtcp_cap.network_info.MBR_UL,
                config->codec_param.codec_fmtp.h264_codec_fmtp.video_b_as);
    } else if (config->codec_param.format == VIDEO_HEVC) {
        VT_LOGD("[ID=%d]mFlags %x VIDEO_HEVC MBR_UL=%d  video_b_as=%d", mMultiInstanceID, mFlags,
                config->rtp_rtcp_cap.network_info.MBR_UL,
    ...
    if (config->rtp_rtcp_cap.network_info.MBR_UL > 0) {
        ...
        if ((config->rtp_rtcp_cap.network_info.MBR_UL !=
             mInitMediaConfig.rtp_rtcp_cap.network_info.MBR_UL) &&
    ...
    if ((config->codec_param.format == VIDEO_H264 &&
         config->codec_param.codec_fmtp.h264_codec_fmtp.video_b_as > 0) ||
        (config->codec_param.format == VIDEO_HEVC &&
         config->codec_param.codec_fmtp.hevc_codec_fmtp.video_b_as > 0)) {
        if ((config->rtp_rtcp_cap.network_info.local_rtp_port !=
             mInitRtpRtcpCap.network_info.local_rtp_port) ||
            (config->rtp_rtcp_cap.network_info.local_rtcp_port !=
             mInitRtpRtcpCap.network_info.local_rtcp_port) ||
            (config->rtp_rtcp_cap.network_info.remote_rtp_port !=
             mInitRtpRtcpCap.network_info.remote_rtp_port) ||
            (config->rtp_rtcp_cap.network_info.remote_rtcp_port !=
             mInitRtpRtcpCap.network_info.remote_rtcp_port)) {
            VT_LOGD("port change reopen socket fd local rtp(%d:%d) rtcp(%d:%d)  remote rtp(%d:%d) "
                    "rtcp(%d:%d)",
                    mInitRtpRtcpCap.network_info.local_rtp_port,
                    config->rtp_rtcp_cap.network_info.local_rtp_port,
                    mInitRtpRtcpCap.network_info.local_rtcp_port,
                    config->rtp_rtcp_cap.network_info.local_rtcp_port,
                    mInitRtpRtcpCap.network_info.remote_rtp_port,
                    config->rtp_rtcp_cap.network_info.remote_rtp_port,
...
status_t ImsMa::Start(ma_datapath_t type) {
    ...
    if (type & MA_SINK) {
        if (mFlags & downlink_started) {
            ...
            if (retVal != OK) {
                VT_LOGE("[ID=%d]kWhatStartRtp IMSMA_RTP_DOWNLINK error %d ", mMultiInstanceID,
                        retVal);
                return retVal;
            }
    ...
    if (type & MA_SOURCE) {
        if (mFlags & uplink_started) {
            ...
            if (retVal != OK) {
                VT_LOGE("[ID=%d]kWhatStartRtp IMSMA_RTP_UPLINK error %d ", mMultiInstanceID,
                        retVal);
                return retVal;
            }
...
status_t ImsMa::Pause(ma_datapath_t type, imsma_pause_resume_params_t* params) {
    ...
    if (params != NULL) {
        ...
        VT_LOGD("[ID=%d]mFlags 0x%x;type %d ,params %p, mode %d,blank = %d", mMultiInstanceID,
                mFlags, type, params, params->mode, params->normal_pause_resume_extra_bitControl);
        ...
        if (params->mode == MA_PAUSE_RESUME_HOLD) {
            ...
            return SetTurnOffVideo_l(type, MA_TURN_OFF_VIDEO_ENABLE,
                                     params->turnOffVideo.direction);
...
status_t ImsMa::Resume(ma_datapath_t type, imsma_pause_resume_params_t* params) {
    ...
    if (params != NULL) {
        VT_LOGD("[ID=%d]mFlags 0x%x;type %d ,params %p, mode %d,[direction] %d,[hold direction] %d",
                ...
                params->turnOffVideo.direction, params->hold.direction);
        ...
        if (params->mode == MA_PAUSE_RESUME_HOLD) {
            ...
            if ((mFlags & uplink_turn_off_video) && (type & MA_SOURCE)) {
                ...
                VT_LOGD("[ID=%d]uplink hold disable ,but should keep turn off video source",
                        mMultiInstanceID);
                retVal = SetTurnOffVideo_l(type, MA_TURN_OFF_VIDEO_ENABLE,
                                           MA_TURN_OFF_VIDEO_BY_LOCAL);
            }

            if ((mFlags & downlink_turn_off_video) && (type & MA_SINK)) {
                modifyFlags(downlink_turn_off_video, CLEAR);
                VT_LOGD("[ID=%d]downlink hold disable ,but should keep TURN_OFF_VIDEO sink",
                        mMultiInstanceID);
                retVal = SetTurnOffVideo_l(type, MA_TURN_OFF_VIDEO_ENABLE,
                                           MA_TURN_OFF_VIDEO_BY_PEER);
            }
            ...
            if (((mFlags & uplink_held) || (mFlags & uplink_hold)) && (type & MA_SOURCE)) {
                VT_LOGD("un-turn off downlink video while downlink holding,clear turn off flags "
                        "and notify source");
                modifyFlags(uplink_turn_off_video, CLEAR);
            ...
            if ((/*(mFlags & down_held) ||*/ (mFlags & downlink_hold)) && (type & MA_SINK)) {
                VT_LOGD("un-turn off uplink video while uplink holding");  // donwlink sink do
                                                                           // nothing, should call
                                                                           // following to notify
                                                                           // rtp resume
            }

            return SetTurnOffVideo_l(type, MA_TURN_OFF_VIDEO_DISABLE,
                                     params->turnOffVideo.direction);
        } else if (params->mode == MA_PAUSE_RESUME_NORMAL) {
...
status_t ImsMa::getBufferQueueProducer(sp<IGraphicBufferProducer>* outBufferProducer) {
    ...
    if (outBufferProducer == NULL) CHECK(false);
...
status_t ImsMa::setCurrentCameraId(uint32_t index) {
    ...
    for (int32_t i = 0; i < mSensorCnt; i++) {
        VT_LOGI("index %d W:%d, H:%d, D:%d,F:%d,HAL %d", temp->index, temp->max_width,
                temp->max_height, temp->degree, temp->facing, temp->hal);
...
// private interface
status_t ImsMa::SetHold_l(ma_datapath_t type, imsma_hold_mode_t mode,
                          imsma_hold_direction_t direction) {
    VT_LOGD("[ID=%d]mFlags %x;type %d,mode %d direction %d", mMultiInstanceID, mFlags, type, mode,
            direction);
    // check state
    status_t retVal = OK;
        ...
        if (mode == MA_HOLD_ENABLE) {  // enable
            ...
            if (type & MA_SOURCE) {
                if (mFlags & uplink_held) {
                    ...
                    VT_LOGW("[ID=%d]held source while source holding, do nothing",
                            mMultiInstanceID);
                } else {
                    ...
                    if (retVal != OK) {
                        VT_LOGE("[ID=%d]MA_HOLD_BY_PEER kWhatTellRtpPeerPause  error %d",
                                mMultiInstanceID, retVal);
                        return retVal;
                    }
                    ...
                    if (retVal != OK) {
                        VT_LOGE("[ID=%d]kWhatStopRtp IMSMA_RTP_UPLINK error %d", mMultiInstanceID,
                                retVal);
                        return retVal;
                    }
                }
            ...
            if (type & MA_SOURCE) {
                if (mFlags & uplink_hold) {
                    VT_LOGW("[ID=%d]un-held source while source holding, just clear held flag and "
                            "notify rtp peer resume",
                            mMultiInstanceID);
                    mHandleMsg->setInt32("reason", MediaSession::kWhatTellRtpPeerResume);
                    retVal = PostAndAwaitResponse(&response);

                    if (retVal != OK) {
                        VT_LOGE("[ID=%d]MA_HOLD_BY_PEER kWhatTellRtpPeerResume  error %d",
                                mMultiInstanceID, retVal);
                        return retVal;
                    }
                } else {
                    ...
                    if (retVal != OK) {
                        VT_LOGE("[ID=%d]kWhatResumeRtp IMSMA_RTP_UPLINK error %d", mMultiInstanceID,
                                retVal);
                        return retVal;
                    }
        ...
        if (mode == MA_HOLD_ENABLE) {  // enable
            ...
            if (type & MA_SOURCE) {
                if (mFlags & uplink_hold) {
                    ...
                    VT_LOGW("[ID=%d]hold source while source helding, do nothing",
                            mMultiInstanceID);
            ...
            if (type & MA_SINK) {
                if (mFlags & downlink_hold) {
                    ...
                    if (retVal != OK) {
                        VT_LOGE("[ID=%d]kWhatStopRtp IMSMA_RTP_DOWNLINK error %d", mMultiInstanceID,
                                retVal);
                        return retVal;
                    }
            ...
            if (type & MA_SOURCE) {
                ...
                VT_LOGD("[ID=%d]uplink_turn_off_video 0x%x ", mMultiInstanceID,
                        mFlags & uplink_turn_off_video);
            }
            ...
            if (type & MA_SINK) {
                ...
                if (retVal != OK) {
                    VT_LOGE("[ID=%d]kWhatResumeRtp IMSMA_RTP_DOWNLINK  error %d", mMultiInstanceID,
                            retVal);
                    return retVal;
                }
            ...
            if (type & MA_SOURCE) {
                if (mFlags & uplink_held) {
                    VT_LOGW("[ID=%d]un-hold source while source helding, clear uplink_hold flag ",
                            mMultiInstanceID);
...
status_t ImsMa::SetTurnOffVideo_l(ma_datapath_t type, imsma_turn_off_video_mode_t mode,
...
status_t ImsMa::Pause_l(ma_datapath_t type, int32_t bitCtrlCmd) {
    VT_LOGD("[ID=%d]mFlags %x;type %d, bitCtrlCmd = 0x%x", mMultiInstanceID, mFlags, type,
            bitCtrlCmd);
    // check state
...
status_t ImsMa::Resume_l(ma_datapath_t type, int32_t bitCtrlCmd) {
    VT_LOGD("[ID=%d]mFlags %x;type %d,bitCtrlCmd[no use now] 0x%x", mMultiInstanceID, mFlags, type,
            bitCtrlCmd);
    // check state
...
status_t ImsMa::Stop_l(ma_datapath_t type, int32_t bitCtrlCmd) {
    VT_LOGD("[ID=%d]mFlags 0x%x ,type %d,bitCtrlCmd 0x%x", mMultiInstanceID, mFlags, type,
            bitCtrlCmd);
    // check state
