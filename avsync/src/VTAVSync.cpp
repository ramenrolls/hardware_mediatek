...
void VTAVSync::getDLAudioTimeStamp(int64_t* pAudioTime) {
    ...
    *pAudioTime = mAV.nsNTP + ((cur_st - mAV.st) / 1000) + ((AVSYNC_CAL_TIME)*1000);
    VTAVSYNC_LOGD("getDLAudioTimeStamp, NTPus=%llx, pre=%llx, cur=%llx, DLAudioTime=%llx",
                  mAV.nsNTP, mAV.st, cur_st, *pAudioTime);
    return;
}
...
void VTAVSync::HandleDLAVSync(unsigned char* buffer) {
    ...
    switch (av_msg->type) {
        case AV_DL_AUDIO_NTP: {
            mAV.st = systemTime();
            /*Transfer to ns*/
            VTAVSYNC_LOGW("av_msg->integer=%x, av_msg->fraction=%x", av_msg->integer,
                          av_msg->fraction);
...
int VTAVSync::openFD(void) {
    ...
    int retry = 0;
    while (g_fd_avsync < 0) {
        VTAVSYNC_LOGE("open dev failed: retry=%d, g_fd_avsync=%d, errno=%d, %s", retry, g_fd_avsync,
...
void VTAVSync::checkFD(void) {
    ...
    while (1) {
        ...
        if (ret < 0) {
            VTAVSYNC_LOGE("ccci query md status error: fd=%d, %d, %s", g_fd_avsync, errno,
                          strerror(errno));
            break;
...
void VTAVSync::avInit(void) {
    ...
    int retry = 0;
    while (ret < 0) {
        VTAVSYNC_LOGE("write avInit failed: retry=%d, ret=%d, errno=%d, %s", retry, ret, errno,
                      strerror(errno));
        retry++;
        sleep(1);
        mAV.st = systemTime();
    ...
    VTAVSYNC_LOGI("avInit, st %llx, int=%x, frac=%x", mAV.st, av_msg.integer, av_msg.fraction);
}
        ...
        for (int i = 0; i < g_poll_cnt; i++) {
            if (g_poll_fds[i].revents & POLLIN) {
                if (g_poll_fds[i].fd == g_fd_avsync) {
                    ...
                    VTAVSYNC_LOGW(
                            "[AVSYNC] read avsync data: len=%d, dump %x %x %x %x. %x %x %x %x.  %x "
                            "%x %x %x. %x %x %x %x.",
