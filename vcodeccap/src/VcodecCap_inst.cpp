...
video_media_profile_t* getMediaProfileEntry(MediaProfileList_t* pMediaProfileList_t,
    ...
    for (Vector<video_media_profile_t*>::iterator it = pMediaProfile->begin();
        ...
        if (pMPEntry->profile == profile && pMPEntry->level == level) {
            ...
            pMPEntry->bitrate = pvideo_property_t->bitrate != 0 ? pvideo_property_t->bitrate
                                                                : pMPEntry->bitrate;
            pMPEntry->fps = pvideo_property_t->fps != 0 ? pvideo_property_t->fps : pMPEntry->fps;
            pMPEntry->Iinterval = pvideo_property_t->Iinterval != 0 ? pvideo_property_t->Iinterval
                                                                    : pMPEntry->Iinterval;

            ALOGI("[getMediaProfileEntry][%d] out:(%s, %s, %dx%d, %d fps, %d kbps, i period: %d)",
                  pMPEntry->format, toString(pMPEntry->profile), toString(pMPEntry->level),
                  pMPEntry->width, pMPEntry->height, pMPEntry->fps, pMPEntry->bitrate / 1000,
                  pMPEntry->Iinterval);
            return pMPEntry;
        }
    }
    ALOGI("[getMediaProfileEntry] Can't get mediaprofle entry(%d, %s, %s)", format,
          toString(profile), toString(level));
    return NULL;
}
...
static void createH264DefaultMediaProfileXml(int opid) {
    ...
    if (fp == NULL) {
        ...
        for (i = 0; i < count; i++) {
            ...
            ALOGI("count %d  format %d profile %d  level %d\n", i, profiles->format,
                  profiles->profile, profiles->level);
            fprintf(fp,
                    " <VideoProfile name=\"h264\" profile=\"%d\" level=\"%d\" width=\"%d\" "
                    "height=\"%d\" framerate=\"%d\" Iinterval=\"%d\" minBitRate=\"%d\" "
                    "maxBitRate=\"%d\" />\n",
                    profiles->profile, profiles->level, profiles->width, profiles->height,
                    profiles->fps, profiles->Iinterval, profiles->minbitrate, profiles->bitrate);
        }
    ...
    ALOGI("createH264DefaultMediaProfileXML() default profile table(op%d), count = %d", opid,
          count);
}
...
static void createQualityMediaProfile(int format, MediaProfileList_t* pMediaProfileList_t) {
    ...
    for (Vector<QualityInfo_t>::iterator it = QualityList.begin(); it != QualityList.end(); ++it) {
        pMPEntry =
                getMediaProfileEntry(pMediaProfileList_t, (*it).format, (*it).profile, (*it).level);
        if (pMPEntry)
            pMediaProfileList_t->mQualityMediaProfile[format].add((*it).quality, *pMPEntry);
    }
}
...
static void createH264QualtiyMediaProfile(uint32_t opID, MediaProfileList_t* pMediaProfileList_t) {
    ...
    KeyedVector<video_quality_t, video_media_profile_t>* prQMP =
            &(pMediaProfileList_t->mQualityMediaProfile[VIDEO_H264]);
    prQMP->clear();
    ...
    if (nQualityCount > 0) {
        ...
        for (int i = 0; i < 4; i++) {
            prQMP->add(qualityInfo[i].quality,
                       *getMediaProfileEntry(pMediaProfileList_t, qualityInfo[i].format,
                                             qualityInfo[i].profile, qualityInfo[i].level));
        }
...
static void createHEVCQualtiyMediaProfile(uint32_t opID, MediaProfileList_t* pMediaProfileList_t) {
    ...
    KeyedVector<video_quality_t, video_media_profile_t>* prQMP =
            &(pMediaProfileList_t->mQualityMediaProfile[VIDEO_HEVC]);
    prQMP->clear();
    ...
    if (nQualityCount > 0) {
        ...
        pMediaProfileList_t->mDefault_quality = VIDEO_QUALITY_HIGH;
        // rVTMP.add(VIDEO_QUALITY_FINE,   *getMediaProfileEntry(VIDEO_HEVC, VIDEO_PROFILE_MAIN,
        // VIDEO_LEVEL_3)); rVTMP.add(VIDEO_QUALITY_HIGH,   *getMediaProfileEntry(VIDEO_HEVC,
        // VIDEO_PROFILE_MAIN, VIDEO_LEVEL_2_1)); rVTMP.add(VIDEO_QUALITY_MEDIUM,
        // *getMediaProfileEntry(VIDEO_HEVC, VIDEO_PROFILE_MAIN, VIDEO_LEVEL_2));
        // rVTMP.add(VIDEO_QUALITY_LOW,    *getMediaProfileEntry(VIDEO_HEVC, VIDEO_PROFILE_MAIN,
        // VIDEO_LEVEL_1)); setFormat(VIDEO_HEVC);
    }
}
...
MediaProfileList_t* getMediaProfileListInst(uint32_t opID) {
    ...
    if (pMediaProfileList_t == NULL) {
        ...
        if (pvideo_property_t->format != VIDEO_HEVC && pvideo_property_t->format != VIDEO_H264) {
            ALOGE("getMediaProfileListInst: Media profile not available(op%d), format: %d", opID,
                  pvideo_property_t->format);
        }
    }
