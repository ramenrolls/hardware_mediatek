...
static video_level_t VIDEO_LEVEL[2][26] = {
        ...
        {// Encoder
         VIDEO_LEVEL_UNKNOWN, VIDEO_LEVEL_0,       VIDEO_LEVEL_1,       VIDEO_LEVEL_1b,
         VIDEO_LEVEL_1_1,     VIDEO_LEVEL_1_2,     VIDEO_LEVEL_1_3,     VIDEO_LEVEL_2,
         VIDEO_LEVEL_2_1,     VIDEO_LEVEL_2_2,     VIDEO_LEVEL_3,       VIDEO_LEVEL_3_1,
         VIDEO_LEVEL_3_2,     VIDEO_LEVEL_4,       VIDEO_LEVEL_4_1,     VIDEO_LEVEL_4_2,
         VIDEO_LEVEL_5,       VIDEO_LEVEL_5_1,     VIDEO_LEVEL_UNKNOWN, VIDEO_LEVEL_UNKNOWN,
         VIDEO_LEVEL_UNKNOWN, VIDEO_LEVEL_UNKNOWN, VIDEO_LEVEL_UNKNOWN, VIDEO_LEVEL_UNKNOWN,
         VIDEO_LEVEL_UNKNOWN, VIDEO_LEVEL_UNKNOWN}};

static sensor_level_info_t sensorLevel[14] = {
        {VIDEO_LEVEL_1_1, 176, 144}, {VIDEO_LEVEL_1_2, 320, 240},   {VIDEO_LEVEL_1_3, 352, 288},
        {VIDEO_LEVEL_2_1, 352, 480}, {VIDEO_LEVEL_2_2, 352, 480},   {VIDEO_LEVEL_3, 640, 480},
        {VIDEO_LEVEL_3, 720, 480},   {VIDEO_LEVEL_3_1, 1280, 720},  {VIDEO_LEVEL_3_2, 1280, 720},
        {VIDEO_LEVEL_4, 1920, 1080}, {VIDEO_LEVEL_4_1, 1920, 1080}, {VIDEO_LEVEL_4_2, 1920, 1080},
        {VIDEO_LEVEL_5, 2560, 1920}, {VIDEO_LEVEL_5_1, 4096, 2048}};
...
static video_media_profile_t* getMediaProfileEntryBySIM(uint32_t opID, video_format_t format,
...
static int32_t encodeBase64(const void* _data, size_t size, uint8_t* out) {
    int32_t pos = 0;
    const uint8_t* data = (const uint8_t*)_data;
...
static void getMorePropertyValue(video_level_t level,
                                 vcodec_level_property_value_t* pvcodec_level_property_value_t) {
...
static video_level_t getSensorLevel(uint32_t max_width, uint32_t max_height) {
    ...
    for (; i < count; i++) {
        if (max_width * max_height >= sensorLevel[i].width * sensorLevel[i].height) {
            // ALOGI("%d, %d, %d, %d, %d", max_width, max_height, sensorLevel[i].width,
            // sensorLevel[i].height, sensorLevel[i].level);
...
static int32_t genH264ParameterSets(VENC_DRV_H264_VIDEO_PROFILE_T eProfile,
                                    VENC_DRV_VIDEO_LEVEL_T eLevel, uint32_t width, uint32_t height,
                                    uint32_t prefer_WH_ratio, uint32_t bitrate,
                                    uint32_t* profile_level_id, uint8_t* pParameterSets) {
    int32_t iError = 0;
    ...
    ALOGI("[genParameterSets] genH264PS, eProfile(%d), eLevel(%d), width(%d), height(%d), "
          "prefer_WH_ratio(%d), bitrate(%d)",
          eProfile, eLevel, width, height, prefer_WH_ratio, bitrate);

    do {
        sSPSBitStreamBuf.rBSAddr.u4PA = sSPSBitStreamBuf.rBSAddr.u4VA =
                sSPSBitStreamBuf.u4BSStartVA = (VAL_ULONG_T)spsBuffer;
        ...
        sPPSBitStreamBuf.rBSAddr.u4PA = sPPSBitStreamBuf.rBSAddr.u4VA =
                sPPSBitStreamBuf.u4BSStartVA = (VAL_ULONG_T)ppsBuffer;
        sPPSBitStreamBuf.rBSAddr.u4Size = sPPSBitStreamBuf.u4BSSize = BUFFER_SIZE;

        CHECK_IF_BREAK(!H264_EncodePPS_SW(eProfile, &sPPSBitStreamBuf, &u4PPSSize), iError,
                       "cannot encode PPS header");
        // debug
        ...
        *profile_level_id = (pSPS[1] << 16) + (pSPS[2] << 8) + pSPS[3];
        ...
        int32_t pos = encodeBase64((unsigned char*)(sSPSBitStreamBuf.rBSAddr.u4VA + 4),
                                   u4SPSSize - 4, pParameterSets);
        ...
        pParameterSets[pos++] = ',';
        pos += encodeBase64((unsigned char*)(sPPSBitStreamBuf.rBSAddr.u4VA + 4), u4PPSSize - 4,
                            pParameterSets + pos);
...
static int32_t getH264CodecParam(video_profile_t profile, video_level_t level,
                                 video_level_t max_level, uint32_t width, uint32_t height,
                                 uint32_t prefer_WH_ratio, uint32_t bitrate,
    ...
    ALOGI("[getH264CodecParam] in:profile=%d(%s), level=%d(%s), max_level=%d(%s)", profile,
          toString((video_profile_t)profile), level, toString((video_level_t)level), max_level,
          toString((video_level_t)max_level));
    ...
    ALOGI("[getH264CodecParam] out:profile_level_id=0x%x, max_recv_level=%d, \
          ...
          ph264_codec_fmtp->profile_level_id, ph264_codec_fmtp->max_recv_level,
          ph264_codec_fmtp->redundant_pic_cap, ph264_codec_fmtp->sprop_parameter_sets,
          ph264_codec_fmtp->sprop_level_parameter_sets, pVideo_codec_fmtp_t->width,
          pVideo_codec_fmtp_t->height);
#ifdef _DEBUG_
...
int32_t searchStartCode(int32_t startpos, VAL_ULONG_T u4BSStartVA, VAL_ULONG_T u4BSSize) {
    ...
    for (; pos < u4BSSize - 5; pos++) {
        if (pBSStartVA[pos] == 0x00 && pBSStartVA[pos + 1] == 0x00 && pBSStartVA[pos + 2] == 0x00 &&
            pBSStartVA[pos + 3] == 0x01) {
            // VDO_LOGI("%d,%d,%d,%d", pBSStartVA[pos], pBSStartVA[pos + 1], pBSStartVA[pos + 2],
            // pBSStartVA[pos + 3]);
            return pos;
        }
    }
...
void removePreventionByte(uint8_t* profile_tier_level, uint8_t size) {
    int32_t pos = 0;
    for (; pos < size - 2;) {
        if (profile_tier_level[pos] == 0x00 && profile_tier_level[pos + 1] == 0x00 &&
...
static int32_t genHEVCParameterSets(VENC_DRV_HEVC_VIDEO_PROFILE_T eProfile,
    ...
    ALOGI("[genParameterSets] genHEVCPS, eProfile(%d), eLevel(%d), width(%d), height(%d), "
          "prefer_WH_ratio(%d), bitrate(%d)",
          eProfile, eLevel, width, height, prefer_WH_ratio, bitrate);

    do {
        // encode VPS
        sVPSBitStreamBuf.rBSAddr.u4PA = sVPSBitStreamBuf.rBSAddr.u4VA =
                sVPSBitStreamBuf.u4BSStartVA = (VAL_ULONG_T)vpsBuffer;
        sVPSBitStreamBuf.rBSAddr.u4Size = sVPSBitStreamBuf.u4BSSize = BUFFER_SIZE;
        CHECK_IF_BREAK(!HEVC_EncodeVPS_SW(eLevel, &sVPSBitStreamBuf, &u4VPSSize), iError,
                       "cannot encode VPS header");
        // debug
        ...
        sSPSBitStreamBuf.rBSAddr.u4PA = sSPSBitStreamBuf.rBSAddr.u4VA =
                sSPSBitStreamBuf.u4BSStartVA = (VAL_ULONG_T)spsBuffer;
        sSPSBitStreamBuf.rBSAddr.u4Size = sSPSBitStreamBuf.u4BSSize = BUFFER_SIZE;
        CHECK_IF_BREAK(!HEVC_EncodeSPS_SW(eLevel, width, height, prefer_WH_ratio, &sSPSBitStreamBuf,
                                          &u4SPSSize),
                       iError, "cannot encode SPS header");
        // debug
        printBinary((unsigned char*)sSPSBitStreamBuf.rBSAddr.u4VA, u4SPSSize);
        ...
        sPPSBitStreamBuf.rBSAddr.u4PA = sPPSBitStreamBuf.rBSAddr.u4VA =
                sPPSBitStreamBuf.u4BSStartVA = (VAL_ULONG_T)ppsBuffer;
        sPPSBitStreamBuf.rBSAddr.u4Size = sPPSBitStreamBuf.u4BSSize = BUFFER_SIZE;
        CHECK_IF_BREAK(!HEVC_EncodePPS_SW(&sPPSBitStreamBuf, &u4PPSSize), iError,
                       "cannot encode PPS header");
        // debug
        printBinary((unsigned char*)sPPSBitStreamBuf.rBSAddr.u4VA, u4PPSSize);

        {
            ...
            if ((VOLTE_MAX_SDP_PARAMETER_SET_LENGTH - 10) < u4Profile_tier_level_size ||
                0 >= u4Profile_tier_level_size) {
                ALOGE("[ERROR] u4Profile_tier_level_size error: %d\n", u4Profile_tier_level_size);
                break;
            } else {
                memcpy(profile_tier_level,
                       ((unsigned char*)sVPSBitStreamBuf.rBSAddr.u4VA) + 4 + 2 + 4,
                       u4Profile_tier_level_size);  // start code + nal type + 4 bytes
            }
            // VPS
            u4VPSSize = encodeBase64((unsigned char*)(sVPSBitStreamBuf.rBSAddr.u4VA + 4),
                                     u4VPSSize - 4, phevc_codec_fmtp->sprop_vps);
            phevc_codec_fmtp->sprop_vps[u4VPSSize] = '\0';

            // SPS
            u4SPSSize = encodeBase64((unsigned char*)(sSPSBitStreamBuf.rBSAddr.u4VA + 4),
                                     u4SPSSize - 4, phevc_codec_fmtp->sprop_sps);
            phevc_codec_fmtp->sprop_sps[u4SPSSize] = '\0';

            // PPS
            u4PPSSize = encodeBase64((unsigned char*)(sPPSBitStreamBuf.rBSAddr.u4VA + 4),
                                     u4PPSSize - 4, phevc_codec_fmtp->sprop_pps);
            phevc_codec_fmtp->sprop_pps[u4PPSSize] = '\0';

            ALOGI("%s\n%s\n%s\n", phevc_codec_fmtp->sprop_vps, phevc_codec_fmtp->sprop_sps,
                  phevc_codec_fmtp->sprop_pps);
            ...
            memcpy((unsigned char*)phevc_codec_fmtp->interop_constraints, &(profile_tier_level[5]),
                   6);
...
static int32_t getHEVCCodecParam(video_profile_t profile, video_level_t level,
                                 video_level_t max_level, uint32_t width, uint32_t height,
                                 uint32_t prefer_WH_ratio, uint32_t bitrate,
                                 video_codec_fmtp_t* pVideo_codec_fmtp_t) {
    ...
    genHEVCParameterSets(eProfile, eLevel, width, height, prefer_WH_ratio, bitrate,
                         phevc_codec_fmtp);
...
static int32_t getVCodecParam(video_format_t format, video_profile_t profile, video_level_t level,
                              video_level_t max_level, int fps, uint32_t width, uint32_t height,
                              uint32_t prefer_WH_ratio, uint32_t bitrate,
                              video_codec_fmtp_t* pVideo_codec_fmtp_t) {
    ...
    if (format == VIDEO_H264) {
        return getH264CodecParam(profile, level, max_level, width, height, prefer_WH_ratio, bitrate,
                                 pVideo_codec_fmtp_t);
    } else if (format == VIDEO_HEVC) {
        return getHEVCCodecParam(profile, level, max_level, width, height, prefer_WH_ratio, bitrate,
                                 pVideo_codec_fmtp_t);
    }
    return 0;
}
...
static bool isVEncCapSupported(video_format_t format, uint32_t profile_level_id,
                               sensor_resolution_t* psensor_resolution_t, venc_cap_t* pvenc_cap_t) {
    ...
    if (format == VIDEO_H264) {
        ...
        video_level_t sensor_level = getSensorLevel(psensor_resolution_t->sensor_max_width,
                                                    psensor_resolution_t->sensor_max_height);

        if (!(profile & pvenc_cap_t->profile)) {
            isSupported = false;
        }
        if (level > sensor_level || level > pvenc_cap_t->max_level) {
            isSupported = false;
        }
    }
...
static int32_t getDefaultCapability(vdec_cap_t* pvdec_cap_t, venc_cap_t* pvenc_cap_t,
                                    video_default_cap_t* pvideo_default_cap_t) {
    video_profile_t profile = (pvdec_cap_t->profile < pvenc_cap_t->profile) ? pvdec_cap_t->profile
                                                                            : pvenc_cap_t->profile;
    pvideo_default_cap_t->profile = getMaxProfile(profile);
    pvideo_default_cap_t->level = (pvenc_cap_t->max_level < pvdec_cap_t->max_level)
                                          ? pvenc_cap_t->max_level
                                          : pvdec_cap_t->max_level;
...
static void copyCodecLevelftmp(video_format_t format, video_codec_fmtp_t* codeccap,
    ...
    if (format == VIDEO_H264) {
        h264_codec_level_fmtp_t* ph264_codec_level_fmtp =
                &codeclevelcap->codec_level_fmtp.h264_codec_level_fmtp;
        ...
        snprintf((char*)ph264_codec_level_fmtp->sprop_parameter_sets,
                 sizeof(ph264_codec_level_fmtp->sprop_parameter_sets), "%s",
                 (char*)ph264_codec_fmtp->sprop_parameter_sets);
    } else if (format == VIDEO_HEVC) {
        hevc_codec_level_fmtp_t* phevc_codec_level_fmtp_t =
                &codeclevelcap->codec_level_fmtp.hevc_codec_level_fmtp;
        ...
        memcpy(phevc_codec_level_fmtp_t->interop_constraints, phevc_codec_fmtp->interop_constraints,
               6);
        ...
        snprintf((char*)phevc_codec_level_fmtp_t->sprop_vps,
                 sizeof(phevc_codec_level_fmtp_t->sprop_vps), "%s",
                 (char*)phevc_codec_fmtp->sprop_vps);
        snprintf((char*)phevc_codec_level_fmtp_t->sprop_sps,
                 sizeof(phevc_codec_level_fmtp_t->sprop_sps), "%s",
                 (char*)phevc_codec_fmtp->sprop_vps);
        snprintf((char*)phevc_codec_level_fmtp_t->sprop_pps,
                 sizeof(phevc_codec_level_fmtp_t->sprop_pps), "%s",
                 (char*)phevc_codec_fmtp->sprop_vps);
        snprintf((char*)phevc_codec_level_fmtp_t->sprop_sei,
                 sizeof(phevc_codec_level_fmtp_t->sprop_sei), "%s",
                 (char*)phevc_codec_fmtp->sprop_vps);
    }
}
...
static int32_t getDefaultMediaProfileBySensorLevel(
        uint32_t opID, video_level_t sensor_level,
...
/* Interface */
/**************************************************************************************/
int32_t getMediaProfile(video_format_t format, uint32_t profile, uint32_t level,
                        video_media_profile_t* pmedia_profile) {
    return getMediaProfile(0, format, profile, level, pmedia_profile);
}

int32_t getCodecCapabiltiy(uint32_t quality, sensor_resolution_t* resolution, uint32_t* capNumbers,
...
int32_t getAdaptativeFrameRateInfo(video_format_t format, uint32_t profile, uint32_t level,
                                   uint32_t expected_bitrate, uint32_t* target_bitrate,
                                   uint32_t* target_framerate) {
    return getAdaptativeFrameRateInfo(0, format, profile, level, expected_bitrate, target_bitrate,
                                      target_framerate);
}

int32_t getCodecLevelParameterSets(uint32_t quality, sensor_resolution_t* resolution,
                                   video_format_t informat, uint32_t* levelCapNumbers,
                                   video_codec_level_fmtp_t* codeclevelcap) {
    return getCodecLevelParameterSets(0, quality, resolution, informat, levelCapNumbers,
                                      codeclevelcap);
}

int32_t getMediaProfile(uint32_t opID, video_format_t format, uint32_t profile, uint32_t level,
                        video_media_profile_t* pmedia_profile) {
    video_media_profile_t* pMPEntry = getMediaProfileEntryBySIM(
            opID, format, getProfile(format, profile), getLevel(format, level));
    if (pMPEntry) {
...
int32_t getMediaProfileByQuality(uint32_t opID, video_quality_t quality, uint32_t* pprofile_count,
                                 video_media_profile_t* pmedia_profile) {
    MediaProfileList_t* pMediaProfileList_t = getMediaProfileListInst(opID);
    ...
    *pprofile_count = 0;
    ...
    for (Vector<video_format_t>::iterator it = pMediaProfileList_t->mVideoFormatList.begin();
         it != pMediaProfileList_t->mVideoFormatList.end(); ++it) {
        if (pMediaProfileList_t->mQualityMediaProfile[(*it)].indexOfKey(quality) >= 0) {
            if (pmedia_profile) {
                pmedia_profile[*pprofile_count] =
                        pMediaProfileList_t->mQualityMediaProfile[(*it)].valueFor(quality);
            }
            *pprofile_count += 1;
        }
    }
...
int32_t getCodecCapabiltiy(uint32_t opID, uint32_t quality, sensor_resolution_t* resolution,
                           uint32_t* capNumbers, video_codec_fmtp_t* codeccap) {
    Vector<video_media_profile_t> targetMediaProfile;
    video_level_t sensor_level =
            getSensorLevel(resolution->sensor_max_width, resolution->sensor_max_height);
    ...
    *capNumbers = 0;
    ...
    for (Vector<video_media_profile_t>::iterator it = targetMediaProfile.begin();
         it != targetMediaProfile.end(); ++it) {
        if (codeccap) {
            video_property_t* pvideo_property_t = getVideoProperty();
            video_profile_t profile = pvideo_property_t->profile != VIDEO_PROFILE_UNKNOWN
                                              ? pvideo_property_t->profile
                                              : (*it).profile;
            video_level_t level = pvideo_property_t->level != VIDEO_LEVEL_UNKNOWN
                                          ? pvideo_property_t->level
                                          : (*it).level;
            uint32_t fps = pvideo_property_t->fps != 0 ? pvideo_property_t->fps : (*it).fps;

            getVCodecParam((*it).format, profile, level, level, fps, (*it).width, (*it).height,
                           resolution->prefer_WH_ratio, (*it).bitrate, &codeccap[*capNumbers]);
    ...
    for (Vector<video_format_t>::iterator it = pMediaProfileList_t->mVideoFormatList.begin();
        ...
        *pcount += pMediaProfile->size();
        if (pvideo_media_bitrate) {
            for (Vector<video_media_profile_t*>::iterator it2 = pMediaProfile->begin();
                 it2 != pMediaProfile->end(); ++it2) {
                video_media_profile_t* pMPEntry = (*it2);
                pvideo_media_bitrate[u4Index].format = pMPEntry->format;
                pvideo_media_bitrate[u4Index].profile =
                        getProfile(pMPEntry->format, pMPEntry->profile);
...
int32_t getAdaptativeFrameRateInfo(uint32_t opID, video_format_t format, uint32_t profile,
                                   uint32_t level, uint32_t expected_bitrate,
    ...
    for (Vector<video_media_profile_t*>::iterator it = pMediaProfile->begin();
         it != pMediaProfile->end(); ++it) {
        video_media_profile_t* pMPEntry = (*it);
        if (profile == getProfile(pMPEntry->format, pMPEntry->profile) &&
            level == getLevel(pMPEntry->format, pMPEntry->level)) {
    ...
    if (format == VIDEO_H264)
        getH264AdaptativeEncodeInfo(expected_bitrate, target_bitrate, minbitrate,
                                    initial_frame_rate, target_framerate);
    else if (format == VIDEO_HEVC)
        getHEVCAdaptativeEncodeInfo(expected_bitrate, target_bitrate, minbitrate,
                                    initial_frame_rate, target_framerate);
    else
...
int32_t getAdaptativeFrameRateInfo(uint32_t opID, video_format_t format, uint32_t profile,
                                   uint32_t level, uint32_t expected_bitrate,
                                   uint32_t* target_bitrate, uint32_t* target_framerate,
                                   uint32_t width, uint32_t height, uint32_t* target_width,
    ...
    *target_height = 0;
    ...
    if (AdaptiveResolution == 0 || format != VIDEO_H264)
        return getAdaptativeFrameRateInfo(opID, format, profile, level, expected_bitrate,
                                          target_bitrate, target_framerate);
    else {
        MediaProfileList_t* pMediaProfileList_t = getMediaProfileListInst(opID);
        Vector<video_media_profile_t*>* pMediaProfile = &pMediaProfileList_t->mMediaProfile[format];
        uint32_t resolution = Resolution_OTHERS;

        ALOGD("[getAdaptativeFrameRateInfo] expected_bitrate: %d, resolution: %d, %d",
              expected_bitrate, width, height);
        ...
        for (Vector<video_media_profile_t*>::iterator it = pMediaProfile->begin();
            ...
            if (profile == getProfile(pMPEntry->format, pMPEntry->profile) &&
                ...
                ALOGD("[getAdaptativeFrameRateInfo] init resolution: %d, %d, %d", pMPEntry->width,
                      pMPEntry->height, resolution);

                break;
            }
        }
        getH264AdaptativeEncodeInfo(expected_bitrate, target_bitrate, minbitrate,
                                    initial_frame_rate, target_framerate, AdaptiveResolution,
                                    resolution, width, height, target_width, target_height);
    }

    ALOGD("[getAdaptativeFrameRateInfo] target framerate: %d, resolution: %d, %d",
          *target_framerate, *target_width, *target_height);

    return 0;
}

int32_t getCodecLevelParameterSets(uint32_t opID, uint32_t quality, sensor_resolution_t* resolution,
                                   video_format_t informat, uint32_t* levelCapNumbers,
                                   video_codec_level_fmtp_t* codeclevelcap) {
    MediaProfileList_t* pMediaProfileList_t = getMediaProfileListInst(opID);
    Vector<video_media_profile_t> targetMediaProfile;
    ...
    if (quality == VIDEO_QUALITY_DEFAULT) {
        ...
        if (pMediaProfileList_t->mQualityMediaProfile[informat].indexOfKey(target_quality) >= 0) {
            video_media_profile_t rvideo_media_profile =
                    pMediaProfileList_t->mQualityMediaProfile[informat].valueFor(target_quality);
            ...
            ALOGD("[getCodecLevelParameterSets] quality: %d, vprofile:%s, vlevel:%s",
                  target_quality, toString(vprofile), toString(vlevel));
        }
    } else {
        video_quality_t target_quality = (video_quality_t)quality;
    ...
    *levelCapNumbers = 0;
    ...
    for (Vector<video_media_profile_t>::iterator it = targetMediaProfile.begin();
        ...
        if (codeclevelcap) {
            ...
            getVCodecParam((*it).format, profile, level, level, fps, (*it).width, (*it).height,
                           resolution->prefer_WH_ratio, (*it).bitrate, pVideo_codec_fmtp_t);
...
struct CodecObserver : public BnOMXObserver {
    ...
    void setNotificationMessage(const sp<AMessage>& msg) { mNotify = msg; }
        ...
        for (std::list<omx_message>::const_iterator it = messages.cbegin(); it != messages.cend();
            ...
            switch (omx_msg.type) {
                ...
                case omx_message::FRAME_RENDERED: {
                    msg->setInt64("media_time_us", omx_msg.u.render_data.timestamp);
                    msg->setInt64("system_nano", omx_msg.u.render_data.nanoTime);
                    break;
                }
...
static int32_t getVEncCapability_1(video_format_t format, venc_cap_t* pvenc_cap_t) {
    // const char* name = "OMX.MTK.VIDEO.ENCODER.AVC"; // "OMX.MTK.VIDEO.ENCODER.HEVC"
    // "OMX.MTK.VIDEO.DECODER.HEVC"  "OMX.MTK.VIDEO.DECODER.AVC" const char* mime = "video/avc"; //
    // mime = "video/hevc"; const char *role = "video_encoder.avc"; // "video_decoder.avc"
    // "video_encoder.hevc"  "video_decoder.hevc"
    ...
    for (OMX_U32 index = 0; index <= kMaxIndicesToCheck; ++index) {
        ...
        status_t err = omxNode->getParameter(OMX_IndexParamVideoProfileLevelQuerySupported, &param,
                                             sizeof(param));
        if (err != OK) {
            break;
        }
...
int32_t configLowPowerMode(low_power_mode_t lpMode, int32_t enable) { return 0; }

uint32_t getAdaptiveDropFrameCapabiltiy(video_format_t format) { return 0; }
