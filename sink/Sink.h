 ...
 *@ parameters: other record parameters which are not codec related ,such as
 *rotation/max-length/duration...
 */
struct record_config {
    ...
    sp<AMessage> params;  //"latitude","longtitude","degree";"max-size";"max-duration"...
                          //'channel-count' 'audio-bitrate' 'video-bitrate' 'sample-rate'
};
...
struct Sink : public AHandler {
    ...
    /**@ start sink module, parameter describe how much tracks are enabled and the enabled track
    info; for video: mime/width/height/bitrate/framerate/profile/level/rtp-time-scale rate for
    audio: mime/samplerate/channel count/stereo/bitrate/rtp-time-scale rate
    ...
    *@ accessUnit: AU with meta info
      meta info:
        setInt64("ntp-time",  ntpTime);//must put 64 bit NTP  ,if no ntp, put 0 in this meta to tell
    sink no ntp to use setInt32("timeUs",  1); //rtp time change to timeUs setInt32("damage",  1);
    //damage buffer for video
    */
  ...
  private:
    ...
    struct TrackInfo {
        ...
        bool TrackNTPIsValid() { return mTrackNTPIsValid; }

        bool TrackFirstAUIsValid() { return mFirstAccessUnitIsValid; }
        int64_t TrackNTPBase() { return mFirstFrame.ntp; }
        int32_t TrackSysTimeBaseUs() { return mFirstFrame.systimeUs; }
    };
    ...
    bool findAndParseCsdNAL(sp<ABuffer>& accessUnit, bool withStartCode, int32_t* profile,
                            int32_t* level, int32_t* width, int32_t* height, int32_t* sarWidth,
                            int32_t* sarHeight);
    bool findAndParseCsdNALHEVC(sp<ABuffer>& accessUnit, bool withStartCode, int32_t* width,
                                int32_t* height);
