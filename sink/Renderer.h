...
struct Renderer : public AHandler {
    ...
    enum {
        kWhatError = 0,
        kWhatOutPCM,  // if need renderer pcm, the pcm should be post to sink to let recorder to
                      // record
     ...
     */
    Renderer(int32_t multiId, const sp<IGraphicBufferProducer>& bufferProducer,
  ...
  private:
    ...
    void queueOutputBuffer(int32_t isVideo, size_t index, int64_t timeUs,
    ...
    status_t getAudioPlayOutTimeUs(int64_t videotime, int64_t* audioTime,
                                   int64_t* audioNTP);  // post sync msg to sink to get the auditime
