...
EncoderContext::EncoderContext(const sp<AMessage>& notify, const sp<ALooper>& codecLooper,
...
EncoderContext::~EncoderContext() { CHECK(mEncoder == NULL); }
...
size_t EncoderContext::getInputBufferCount() const { return mEncoderInputBuffers.size(); }

sp<AMessage> EncoderContext::getOutputFormat() const { return mOutputFormat; }

bool EncoderContext::needToManuallyPrependSPSPPS() const { return mNeedToManuallyPrependSPSPPS; }
...
status_t EncoderContext::initEncoder() {
    ...
    if (mIsVideo) {
        ...
        err = mEncoder->configure(tmp, NULL /* nativeWindow */, NULL /* crypto */,
    ...
    if (err != OK) {
        ...
        err = mEncoder->configure(mOutputFormat, NULL /* nativeWindow */, NULL /* crypto */,
                                  MediaCodec::CONFIGURE_FLAG_ENCODE);
    }
...
void EncoderContext::onMessageReceived(const sp<AMessage>& msg) {
    switch (msg->what()) {
        case kWhatAccessUnitIn: {
            VT_LOGV("mEncoder->queueInputBuffer mIsFakeEncoder %d  video %d   ", mIsFakeEncoder,
                    mIsVideo);

            if (!mIsFakeEncoder && mEncoder == NULL) {
                VT_LOGE(" '%s'  encoder got au after encoder shutdown.",
                ...
                VT_LOGD("[%s] time %lld us (%.2f secs)", mIsVideo ? "video" : "audio",
                        (long long)timeUs, timeUs / 1E6);
                notify->post();
    ...
    while (!mInputBufferQueue.empty()) {
        ...
        if (mPartialAudioAU != NULL) {
            size_t bytesMissingForFullAU =
                    kNumAUsPerPESPacket * kFramesPerAU * kFrameSize - mPartialAudioAU->size() + 4;
            ...
            memcpy(mPartialAudioAU->data() + mPartialAudioAU->size(), buffer->data(), copy);
        ...
        while (buffer->size() > 0) {
            ...
            ptr[3] = (kQuantizationWordLength << 6) | (kAudioSamplingFrequency << 3) |
...
status_t EncoderContext::feedEncoderInputBuffers() {
    ...
    VT_LOGV("feedEncoderInputBuffers + mInputBufferQueue size %zu mAvailEncoderInputIndices size "
            "%zu",
    ...
    while (!mInputBufferQueue.empty() && !mAvailEncoderInputIndices.empty()) {
        ...
        if (buffer != NULL) {
            ...
            memcpy(mEncoderInputBuffers.itemAt(bufferIndex)->data(), buffer->data(),
                   buffer->size());
            ...
            if (buffer->meta()->findPointer("mediaBuffer", (void**)&mediaBuffer) &&
                mediaBuffer != NULL) {
                mEncoderInputBuffers.itemAt(bufferIndex)
                        ->meta()
        ...
        status_t err = mEncoder->queueInputBuffer(
                bufferIndex, 0, (buffer == NULL) ? 0 : buffer->size(), timeUs, flags);
...
status_t EncoderContext::doMoreWork() {
    ...
    for (;;) {
        ...
        if (flags & MediaCodec::BUFFER_FLAG_EOS) {
            ...
            memcpy(buffer->data(), mEncoderOutputBuffers.itemAt(bufferIndex)->base() + offset,
                   size);
...
void EncoderContext::requestIDRFrame() { (new AMessage(kWhatRequestIDRFrame, this))->post(); }

int32_t EncoderContext::getVideoBitrate() const { return mPrevVideoBitrate; }
