...
struct Renderer::DecoderContext : public AHandler {
    ...
    status_t init(const sp<AMessage>& format, const sp<Surface>& surface);
  ...
  private:
    ...
    void queueOutputBuffer(size_t index, int64_t timeUs, const sp<MediaCodecBuffer>& buffer);
...
//  mLastBandwidthCalcTimeUs(-1),
{
    ...
    /*
        ...
        VT_LOGI("OPTest.MA TestMode=%d MAMap=0x%x 0x2=%d mPushBlank=%d", TestMode, MAMap, (MAMap &
       0x2), mPushBlank);*/
}
...
status_t Renderer::DecoderContext::init(const sp<AMessage>& format, const sp<Surface>& surface) {
    CHECK(mDecoder == NULL);
    ATRACE_CALL();
    AString mime;
    ...
    mDecoderLooper->start(false /* runOnCallingThread */, false /* canCallJava */, PRIORITY_AUDIO);
    ...
    status_t err = mDecoder->configure(format, surface == NULL ? NULL : surface, NULL /* crypto */,
                                       0 /* flags */);
...
status_t Renderer::DecoderContext::renderOutputBufferAndRelease(size_t index) {
    ATRACE_CALL();
...
void Renderer::DecoderContext::queueDecoderInputBuffers() {
    if (mDecoder == NULL) {
        VT_LOGD("[ID=%d]decoder is NULL now, should not queueDecoderInputBuffers",
                mMultiInstanceID);
        return;
    }
    ...
    while (!mAccessUnits.empty() && !mDecoderInputBuffersAvailable.empty()) {
        size_t index = *mDecoderInputBuffersAvailable.begin();

        mDecoderInputBuffersAvailable.erase(mDecoderInputBuffersAvailable.begin());

        sp<ABuffer> srcBuffer = *mAccessUnits.begin();
        mAccessUnits.erase(mAccessUnits.begin());

        const sp<MediaCodecBuffer>& dstBuffer = mDecoderInputBuffers.itemAt(index);
        ...
        */
        ...
        VT_LOGV("[ID=%d]queueDecoderInputBuffers token %d timeUs %" PRId64 " ----",
                mMultiInstanceID, token, timeUs);

        mTimeUsTokenMaps.add(timeUs, token);

        ATRACE_INT("queueInputBuffer", index);

        status_t err = mDecoder->queueInputBuffer(index, 0 /* offset */, nal_length, timeUs,
                                                  0 /* flags */);

        if (err != OK) {
            VT_LOGE("[ID=%d]err == %d delete peer surface before stop sink ", mMultiInstanceID,
                    err);
            return;
        }
...
void Renderer::DecoderContext::onMessageReceived(const sp<AMessage>& msg) {
    switch (msg->what()) {
        VT_LOGV("[ID=%d]DecoderContext mIsVideo %d what %d ", mMultiInstanceID, mIsVideo,
                msg->what());
            ...
            case kWhatCheckDecoderBW:
               {
                ...
                VT_LOGV("DecoderContext mIsVideo %d  lowBW %d BW %d", mIsVideo, decoderIsLowBW
            ,decBW); if(decoderIsLowBW){
...
void Renderer::DecoderContext::onDecoderNotify() {
    ...
    for (;;) {
        ...
        if (err == OK) {
            ...
            VT_LOGE("[ID=%d]err == %d delete peer surface before stop sink@dequeueInputBuffer ",
                    mMultiInstanceID, err);
            notifyError(err);
            // TRESPASS();
            return;
        }
    ...
    for (;;) {
        ...
        int64_t timeUs = -1;
        uint32_t flags = 0;
        status_t err = mDecoder->dequeueOutputBuffer(&index, &offset, &size, &timeUs, &flags);

        if (err == OK) {
            ATRACE_INT("dequeueOutputBuffer", index);
            VT_LOGV("[ID=%d][from codec]dequeueOutputBuffer:timeUs  %" PRId64 " ", mMultiInstanceID,
                    timeUs);
            queueOutputBuffer(index, timeUs, mDecoderOutputBuffers.itemAt(index));
            ...
            if (res != OK) {
                VT_LOGE("[ID=%d]Failed to get output format for after INFO event (err=%d)",
                        mMultiInstanceID, res);
                CHECK(res == OK);
            }
            ...
            VT_LOGE("[ID=%d]err == %d delete peer surface before stop sink@dequeueOutputBuffer ",
                    mMultiInstanceID, err);
            // TRESPASS();
...
void Renderer::DecoderContext::queueOutputBuffer(size_t index, int64_t timeUs,
...
Renderer::AudioRenderer::AudioRenderer(const sp<DecoderContext>& DecoderContext,
                                       sp<AMessage> notify)
    : mDecoderContext(DecoderContext),
      ...
      mNumFramesWritten(0),
      mLatencyUs(-1) {
    mAudioTrack = new AudioTrack(AUDIO_STREAM_DEFAULT, 48000.0f, AUDIO_FORMAT_PCM,
                                 AUDIO_CHANNEL_OUT_STEREO, (int)0 /* frameCount */);
...
void Renderer::AudioRenderer::queueInputBuffer(size_t index, int64_t timeUs,
                                               const sp<MediaCodecBuffer>& buffer) {
    BufferInfo info;
    info.mIndex = index;
    info.mTimeUs = timeUs;
...
void Renderer::AudioRenderer::onMessageReceived(const sp<AMessage>& msg) {
    switch (msg->what()) {
        case kWhatPushAudio: {
            onPushAudio();
            break;
...
void Renderer::AudioRenderer::schedulePushIfNecessary() {
    VT_LOGV("AudioRenderer:mPushPending %d,mInputBuffers.size %zu", mPushPending,
            mInputBuffers.size());
    ...
    if (mAudioTrack->getSampleRate() != 0) {
        ...
        int64_t delayUs = msecsPerFrame * numFramesPendingPlayout * 1000ll;
...
void Renderer::AudioRenderer::dequeueTrackAccessUnit() { mStartFeedBackPCM = true; }
void Renderer::AudioRenderer::onPushAudio() {
    ...
    while (!mInputBuffers.empty()) {
        ...
        ssize_t n = writeNonBlocking(info.mBuffer->data(), info.mBuffer->size());

        if (n < (ssize_t)info.mBuffer->size()) {
            CHECK_GE(n, 0);

            info.mBuffer->setRange(info.mBuffer->offset() + n, info.mBuffer->size() - n);
            break;
        }
        ...
        if (mStartFeedBackPCM) {
            ...
            memcpy(buffer->data(), info.mBuffer->data(), (info.mBuffer->size()));
...
ssize_t Renderer::AudioRenderer::writeNonBlocking(const uint8_t* data, size_t size) {
    uint32_t numFramesPlayed;
...
Renderer::Renderer(int32_t multiId, const sp<IGraphicBufferProducer>& bufferProducer,
    ...
    VT_LOGI("OPTest.MA TestMode=%d MAMap=0x%x 0x1=%d mEnableAvsync=%d,mPushBlank=%d", TestMode,
            MAMap, (MAMap & 0x1), mEnableAvsync, mPushBlank);
}
...
Renderer::~Renderer() {
    ...
    if (mPushBlank) {
        ...
        if (err != OK) {
            VT_LOGW("[push blank]native_window_api_connect returned an error: %s (%d)",
                    strerror(-err), err);
        }
    ...
    if (mSurface != NULL) {
        ...
        if (err != OK) {
            VT_LOGW("[just handle,not error]native_window_api_disconnect returned an error: %s "
                    "(%d)",
                    strerror(-err), err);
        }
...
status_t Renderer::stop(int32_t pushBlank) {
    ...
    VT_LOGI("[ID=%d] stop-mPushBlank %d+++ mNumFrames=%d", mMultiInstanceID, mPushBlank,
            mNumFrames);
        ...
        case kWhatScanDisplayStatus: {
            ...
            if (mNumFrames > lastNum && lastShow == 0) {
                ...
                if (loop >= PAUSE_DELAY_LOOP) {
                    ...
                    loop++;  // only add here
                    VT_LOGD("[wait to pause]mNumFrames %d,nowShow %d ~~lastNum %d  lastShow %d  "
                            "loop %d",
                            mNumFrames, nowShow, lastNum, lastShow,
                            loop);  // may define different error code in future
                }
                ...
                loop = 0;
                VT_LOGD("[keep pause]mNumFrames %d,nowShow %d ~~lastNum %d  lastShow %d  loop %d",
                        mNumFrames, nowShow, lastNum, lastShow,
                        loop);  // may define different error code in future
            }

            if (nowShow != lastShow) {  // change show state
                VT_LOGD("[change]mNumFrames %d,nowShow %d !=lastNum %d  lastShow %d  loop %d",
                        mNumFrames, nowShow, lastNum, lastShow,
                        loop);  // may define different error code in future
                sp<AMessage> msg = mNotify->dup();
...
status_t Renderer::internalSetFormat(int32_t isVideo, const sp<AMessage>& format) {
    CHECK_LT(isVideo, 2);
    status_t err = OK;

    VT_LOGI("[ID=%d]SetFormat: '%s' format is %s", mMultiInstanceID, isVideo ? "video" : "audio",
...
void Renderer::queueAccessUnit(int32_t isVideo, const sp<ABuffer>& accessUnit) {
    ATRACE_CALL();
...
void Renderer::onDecoderNotify(const sp<AMessage>& msg) {
    ...
    switch (what) {
        ...
        default:
            VT_LOGE("[ID=%d] '%s'  is %s", mMultiInstanceID, isVideo ? "video" : "audio",
                    msg->debugString(0).c_str());
            TRESPASS();
    }
}

void Renderer::queueOutputBuffer(int32_t isVideo, size_t index, int64_t timeUs,
                                 const sp<MediaCodecBuffer>& buffer) {
    ATRACE_CALL();
    VT_LOGV("Renderer::queueOutputBuffer:isVideo %d index %zu  timeUs %lld us", isVideo, index,
            (long long)timeUs);
    ...
    if (!mFirstVideoTimeBaseValid) {
        ...
        VT_LOGI("[ID=%d]1th video frame is out :mFirstVideoMediaTimeUs %" PRId64
                " us,mFirstVideoComeSysTimeUs %" PRId64 " us",
...
status_t Renderer::getAudioPlayOutTimeUs(int64_t videotime, int64_t* audioTime, int64_t* audioNTP) {
    ...
    VT_LOGV("[ID=%d]Renderer::getAudioPlayOutTimeUs+ : videotime %" PRId64 " us", mMultiInstanceID,
            videotime);
    status_t err = OK;
    ...
    if (mAudioRenderer.get() != NULL) {
        ...
        *audioTime = mAudioRenderer->getAudioPlayOutTimeUs();
        VT_LOGV("[ID=%d]Renderer::getAudioPlayOutTimeUs from audioRender-  audioTime %" PRId64
                " us",
                mMultiInstanceID, *audioTime);
        ...
        *audioNTP = *audioTime;  // record audio ntp for trace log
        ...
        if (mNTPIsValid && mVideoTimeUsNTPMaps.size() != 0) {
            if (mVideoTimeUsNTPMaps.indexOfKey(videotime) < 0) {
                VT_LOGE("[ID=%d]may be NTP is invalid or  lost for videoTimeUs %" PRId64 " us",
                        mMultiInstanceID, videotime);
            } else {
        ...
        if (*audioTime == 0 || videoNTP == 0 ||
            llabs(*audioTime - videoNTP) > AVSYNC_THRESHOLD) {  // audio ntp is not valid, free run
            *audioTime = videotime;
        }
    }
...
void Renderer::onRenderVideo() {
    ...
    while (!mVideoOutputBuffers.empty()) {
        ...
        if (avSyncType) {
            VT_LOGV("[ID=%d]getAudioPlayOutTimeUs+: videoTimeUs=%" PRId64 " ms", mMultiInstanceID,
                    videoTimeUs / 1000);
            err = getAudioPlayOutTimeUs(videoTimeUs, &audioTimeUs,
                                        &audioNTP);  // have the valid ref time
            VT_LOGV("[ID=%d]getAudioPlayOutTimeUs-: audioTimeUs=%" PRId64 " ms", mMultiInstanceID,
                    audioTimeUs / 1000);
        }
        ...
        if (err == OK && avSyncType) {  // modem from 0 mean valid
            ...
            VT_LOGV("[ID=%d][AVSync@AudioTimeStamp]:videoTimeUs %" PRId64 " ,audioTimeUs %" PRId64
                    ",delayUs %" PRId64 "",
            ...
            // VT_LOGV("[ID=%d][AVSync@systime]:videoTimeUs %" PRId64 " ,shouldUs %" PRId64 ",nowUs
            // %" PRId64 ",delayUs %" PRId64 "",
        ...
        if (delayUs > 20000ll) {  // ealry  more than 10m
            ...
            if (mLastVideoHow != VIDEO_DELAY) {
                VT_LOGD("[ID=%d][AVSync][ms]vNTP %" PRId64 ",aNTP %" PRId64 ", aNTPO %" PRId64
                        ",video early %" PRId64 " ms,show %d ",
                        mMultiInstanceID, videoTimeUs / 1000, audioTimeUs / 1000, audioNTP / 1000,
                        delayUs / 1000, mCurrentVideoHow);
            }
            delayUs = delayUs / 2;
        ...
        switch (mCurrentVideoHow) {
            case VIDEO_SHOW: {
                ...
                mNumFramesDrop = 0;
                mContinuouslyDropCnt = 0;
                err = mDecoderContext[mVideoTrackIndex]->renderOutputBufferAndRelease(
                        info.mIndex /*maybe add timeNs*/);
        ...
        // CHECK_EQ(err, (status_t)OK);
        if (err != OK) {  //
            VT_LOGE("[ID=%d]err == %d delete peer surface before stop sink@releaseRender",
                    mMultiInstanceID, err);
            notifyError(err);
        ...
        if (mCurrentVideoHow == VIDEO_DELAY) {
            ...
            mVideoTimeUsNTPMaps.removeItem(
                    videoTimeUs);  // maybe should not use this, just record the 1th valid timestamp
        }
    }
...
status_t Renderer::resetSurfaceTransformDegree(int32_t degree, bool force) {
    ...
    VT_LOGI("[ID=%d]mRotateInside %d ,degree change from %d to %d", mMultiInstanceID, mRotateInside,
    ...
    err = native_window_set_buffers_transform(static_cast<ANativeWindow*>(mSurface.get()),
                                              transform);

    if (err != 0) {
        VT_LOGE("[ID=%d]native_window_set_buffers_transform failed: %s (%d)", mMultiInstanceID,
                strerror(-err), -err);
    }
