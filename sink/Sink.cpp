...
Sink::Sink(int32_t multiId, uint32_t flags, uint32_t simID, uint32_t operatorID)
    ...
    VT_LOGI("[ID=%d]create sink mDump %d,mSimID %d,mOperatorID %d ,mSupportSliceNAL %d",
            mMultiInstanceID, mDump, mSimID, mOperatorID, mSupportSliceNAL);
}
...
void Sink::SetSurface(const sp<IGraphicBufferProducer>& bufferProducer) {
    ...
    if (mSurface.get() == NULL) {
        ...
        if (bufferProducer.get() != NULL) {
            ...
            mState |= SURFACE_SET;
            VT_LOGI("[ID=%d]SetSurface:pre %p, now %p ,mState %x", mMultiInstanceID, mSurface.get(),
                    bufferProducer.get(), mState);
...
status_t Sink::postAsynAndWaitReturnError(const sp<AMessage>& msg) {
...
status_t Sink::setSinkCongfig(audio_dec_fmt* audioFormat, video_dec_fmt* videoFormat) {
    ...
    if (mState & INITED) {
        VT_LOGI("[ID=%d][%p]update config setting again,should handle setting changes",
                mMultiInstanceID, this);
    ...
    VT_LOGD("[ID=%d]mVideoWidth=%d, mVideoHeight=%d", mMultiInstanceID, mVideoConfigSettings.width,
            mVideoConfigSettings.height);

    VT_LOGD("[ID=%d]bitrate=%d, frame-rate=%d", mMultiInstanceID, mVideoConfigSettings.bitrate,
            mVideoConfigSettings.framerate);
    VT_LOGD("[ID=%d]profile=%d, level=%d", mMultiInstanceID, mVideoConfigSettings.profile,
...
status_t Sink::start(bool startVideo, bool startAudio) {
    ...
    mRenderLooper->start(false /* runOnCallingThread */, false /* canCallJava */, PRIORITY_AUDIO);
    ...
    if (startVideo) {
        ...
        /*
                //only for avpf==>
                sp<AMessage> avpfNotify = new AMessage(kWhatCodecAvpfFeedBack,this);    //only for
           avpf feedback trackInfo->mTrackMeta->setMessage("avpf-notify",avpfNotify);
                //<====avfp
        */
...
status_t Sink::initRecorder(record_config* RecCfg) {
    ...
    VT_LOGI("[ID=%d]useFd %d,*path %s,mode %d,outf %d  ae %d , ve %d", mMultiInstanceID,
            RecCfg->useFd, RecCfg->path, RecCfg->mode, RecCfg->outf, RecCfg->ae, RecCfg->ve);
    ...
    if (mRecordCfg->mode == RECORD_DLVIDEO_MIXADUIO) {
        CHECK(mRecorder->setVideoSource(VIDEO_ENCODER_H264,
                                        (mTracks.valueFor(VIDEO_DL))->mTrackMeta) == OK);
        sp<AMessage> as = NULL;
        /*
        ...
        */
        ...
        VT_LOGE("[ID=%d][error]not supprot this record mode %d", mMultiInstanceID,
                mRecordCfg->mode);
    }
    ...
    if (!mRecordCfg->useFd) {
        ...
        CHECK(mRecorder->setOutputFile(mRecordCfg->fd, mRecordCfg->offset, mRecordCfg->length) ==
              OK);
    }
...
status_t Sink::startRecord() {
    ...
    if (mRecorderLooper.get() == NULL) {
        ...
        mRecorderLooper->start(false /* runOnCallingThread */, false /* canCallJava */,
                               PRIORITY_AUDIO);
    }
...
{
    VT_LOGI("[ID=%d]Start in state %x
params:%s",mMultiInstanceID,mState,params->debugString(0).c_str());
...
status_t Sink::initLooper_l() {
    ...
    if (mLooper.get() == NULL) {
        ...
        mLooper->start(false /* runOnCallingThread */, false /* canCallJava */, PRIORITY_AUDIO);
    }
...
status_t Sink::stop_l(int32_t pushBlank) {
    VT_LOGD("[ID=%d]stop modules mRecorder %p,mRenderer %p, reset degree[%d]", mMultiInstanceID,
            mRecorder.get(), mRenderer.get(), mRotationDegree);
    status_t err = OK;
...
void Sink::updateTrackInfo(TrackIndex trackIndex, sp<ABuffer>& accessUnit) {
    ...
    VT_LOGV("[ID=%d]IN:trackIndex %d frame %d NowUs %" PRId64 ",timeUs %" PRId64 ",ntpTime %" PRId64
            "",
            mMultiInstanceID, trackIndex, info->mFrameCount, nowUs, info->mCurrentFrame.timeUs,
            info->mCurrentFrame.ntp);
    ...
    if (!info->mFirstAccessUnitIsValid) {  // 1th frame re-calc
        ...
        if (ntpTime > 0 && !info->mTrackNTPIsValid) {
            VT_LOGD("[ID=%d]ntp is turn to valid from %d frame, mFirstValidNTP=%" PRId64 "",
                    mMultiInstanceID, info->mFrameCount, ntpTime);
        } else if (ntpTime == 0 && info->mTrackNTPIsValid) {
            VT_LOGD("[ID=%d]ntp is turn to invalid from %d frame", mMultiInstanceID,
                    info->mFrameCount);
        }
        ...
        /*if(info->mCurrentFrame.ntp == 0 && info->mTrackNTPIsValid ){//ntp is invalid, reset to
        used rtp time info->mTrackNTPIsValid = false; VT_LOGI("[ID=%d]trackIndex=%d,ntp is turn to
        invalid from %d frame ",mMultiInstanceID,trackIndex,info->mFrameCount);
        }

        if(info->mTrackNTPIsValid){//nto is valid before
            if(ntpTime > 0 && npt_delta >= 0){ //ntp keep right and increase right,ALPS03457456:some
        UE rtp time is set error,cause ntp increase,so use ntp direclty
                //directly use ntp time
                info->mCurrentFrame.refineTimeUs  = ntpTime;
            }else if(npt_delta >= 0){ // this time ntp is not right, but ntp increase right,basicly
        can not happen to here refine_delta =  npt_delta; }else if(timeUs_delta > 0){ // ntp
        increase error, but timeUs increase right VT_LOGE("[ID=%d]input ntp is error from last %"
        PRId64 " to  now %" PRId64 " @ frame %d ",
                        mMultiInstanceID,info->mLastFrame.ntp,info->mCurrentFrame.ntp
        ,info->mFrameCount); refine_delta =  timeUs_delta; } else { //both ntp and timeUs increase
        error, only use systime refine_delta = sysTime_delta;
            }
            if(refine_delta >= 0){
            ...
            if(ntpTime > 0){//ntp begin valid now,   first time npt valid
                ...
                VT_LOGD("[ID=%d]trackIndex=%d,ntp is turn to valid from %d frame, mFirstValidNTP=%"
        PRId64 "" ,mMultiInstanceID,trackIndex,info->mFrameCount,ntpTime);
    ...
    VT_LOGD("[ID=%d]trackIndex %d frame %d NowUs %" PRId64 " ms,timeUs %" PRId64
            " ms,ntpTime %" PRId64 " ms,refineTimeUs %" PRId64
            " ms ,damagedAu %d,lostfull %d,isIDR %d",
            mMultiInstanceID, trackIndex, info->mFrameCount, nowUs / 1000,
            info->mCurrentFrame.timeUs / 1000, info->mCurrentFrame.ntp / 1000,
            info->mCurrentFrame.refineTimeUs / 1000, damagedAu, lostfull, isIDR);
}
...
bool Sink::waitForNextNal(sp<ABuffer>& accessUnit) {
    ...
    if (mVideoDLQueue.empty()) {
        ...
        if (timeUs != LastTimeUs) {
            VT_LOGV("[ID=%d]timeUs %" PRId64 ",LastTimeUs %" PRId64 "", mMultiInstanceID, timeUs,
                    LastTimeUs);
            timeUsChange = true;
            goto FLUSHNOW;
        }
...
FLUSHNOW:
    ...
    VT_LOGV("[ID=%d]mVideoDLQueue %zu totalSize %d,markEOF %d,timeUsChange%d", mMultiInstanceID,
            mVideoDLQueue.size(), totalSize, markEOF, timeUsChange);
...
void Sink::dispatchInputAccessUnit(TrackIndex trackIndex, sp<ABuffer>& accessUnit) {
    ...
    if (isVideo) {
        ...
        if (accessUnit->meta()->findInt32("ccw_rotation", &degree)) {
            ...
            if (deviceRoateNow)
                VT_LOGD("[ID=%d]buffer with rotation degree = %d <- mRotationDegree = %d ",
                        mMultiInstanceID, degree, mRotationDegree);
        ...
        if (accessUnit->meta()->findInt32("flip", &flip)) {
            if (mFlip != flip)
                VT_LOGW("[ID=%d][not support]buffer with flip = %d mFlip = %d", mMultiInstanceID,
                        flip, mFlip);
        ...
        if (accessUnit->meta()->findInt32("camera_facing", &facing)) {
            ...
            if (deviceRoateNow)
                VT_LOGW("[ID=%d] buffer with facing = %d mFacing = %d degree = %d",
                        mMultiInstanceID, facing, mFacing, degree);
        }
        ...
        if (!trackInfo->mCsdParsed) {
            ...
            bool result =
                    isH264 ? findAndParseCsdNAL(accessUnit, multiNALWithStartCode, &profile, &level,
                                                &width, &height, &sarWidth, &sarHeight)
                           : (isHEVC ? findAndParseCsdNALHEVC(accessUnit, multiNALWithStartCode,
                                                              &width, &height)
                                     : false);

            if (result) {
                VT_LOGD("[ID=%d]track %d find csd buffer from %dth frame", mMultiInstanceID,
                        trackIndex, trackInfo->mFrameCount);
                mVideoWidth = width;
                mVideoHeight = height;
        ...
        if (!mNotifyClientSize || deviceRoateNow) {
            ...
            if (mRotateInside) {
                VT_LOGD("[ID=%d]notifyResolutionDegree change rotate way, old   w:%d, h:%d, d:%d ",
                        mMultiInstanceID, w, h, d);
                ...
                d = 0;
                VT_LOGD("[ID=%d]notifyResolutionDegree  change rotate way, after w:%d, h:%d, d:%d ",
                        mMultiInstanceID, w, h, d);
            }
...
bool Sink::convertToMeta(bool isVideo, sp<AMessage> meta) {
    if (isVideo) {
        ...
        /*
        meta->setInt32("profile",mVideoConfigSettings.profile);// 66/31 type.acodec not need
        profiel/level params in config meta->setInt32("level", mVideoConfigSettings.level); // even
        need, acodec need OMX type 1/256 type profile/level
        */
        ...
        if (width <= 0 || height <= 0) {
            ...
            int32_t err = getMediaProfile(
                    mOperatorID,
                    ((!strcasecmp(mVideoConfigSettings.mimetype, MEDIA_MIMETYPE_VIDEO_AVC))
                             ? VIDEO_H264
                     : (!strcasecmp(mVideoConfigSettings.mimetype, MEDIA_MIMETYPE_VIDEO_HEVC))
                             ? VIDEO_HEVC
                             : VIDEO_FORMAT_NUM),
            ...
            VT_LOGD("[ID=%d]no valid W/H in init , use fake W %d,H %d to pass codec check",
                    mMultiInstanceID, mediaProfile.width, mediaProfile.height);
        }
        ...
        if (csd.get() != NULL && isH264) {  // sps
            ...
            bool result = findAndParseCsdNAL(csd, true, &profile, &level, &width, &height,
                                             &sarWidth, &sarHeight);
            ...
            */
            ...
            while (getNextNALUnit(&data, &size, &nalStart, &nalSize, true) == OK) {
                if ((nalStart[0] & 0x1f) == 7) {
                    ...
                    memcpy(seqParamSet->data(), nalStart - 4,
                           nalSize + 4);  // send to codec with start code
                ...
                if ((nalStart[0] & 0x1f) == 8) {
                    ...
                    memcpy(picParamSet->data(), nalStart - 4,
                           nalSize + 4);  // send to codec with start code
            ...
            while (getNextNALUnit(&data, &size, &nalStart, &nalSize, true) == OK) {
                if ((nalStart[0] & 0x1f) == 7) {
                     seqParamSet = new ABuffer(nalSize + 4);
                     memcpy(seqParamSet->data(), nalStart - 4, nalSize + 4);//send to codec with
            start code meta->setBuffer("csd-0",seqParamSet); findBoth++;
                }
                if ((nalStart[0] & 0x1f) == 8) {
                     picParamSet = new ABuffer(nalSize + 4);
                     memcpy(picParamSet->data(), nalStart - 4, nalSize + 4);//send to codec with
            start code meta->setBuffer("csd-1",picParamSet); findBoth++;
                }
                if(findBoth == 2){
                    break;
                }
            ...
            VT_LOGD("[ID=%d]no csd buffer init start phase, it will be in the 1th frame:profile %d "
                    "level %d",
...
bool Sink::findAndParseCsdNAL(sp<ABuffer>& accessUnit, bool withStartCode, int32_t* profile,
                              int32_t* level, int32_t* width, int32_t* height, int32_t* sarWidth,
                              int32_t* sarHeight) {
    *width = -1;
    *height = -1;
    *sarWidth = -1;
    ...
    *level = spsInf.level;
    VT_LOGD("[ID=%d]W %d H %d P %d L %d", mMultiInstanceID, spsInf.width, spsInf.height,
            spsInf.profile, spsInf.level);
...
bool Sink::findAndParseCsdNALHEVC(sp<ABuffer>& accessUnit, bool withStartCode, int32_t* width,
                                  int32_t* height) {
    ...
    switch (msg->what()) {
        ...
        case kWhatFeedRender: {
            if (mState & RENDERING) {
                ...
                VT_LOGE("[ID=%d]post kWhatFeedRender in error mState =%d", mMultiInstanceID,
                        mState);
            }
        ...
        case kWhatFeedRecorder: {
            if (mState & RECORDING) {
                ...
                VT_LOGE("[ID=%d]kWhatFeedRecorder  isVideo %d in  mState =%d", mMultiInstanceID,
                        isVideo, mState);
                mRecorder->queueAccessUnit(isVideo, accessUnit);

            } else {
                VT_LOGE("[ID=%d]post kWhatFeedRecorder in error mState =%d", mMultiInstanceID,
                        mState);
            }
        ...
        case kWhatRenderNotify: {
            ...
            notify->post();
                ...
                if (mRotateInside) {
                    ...
                    degree = 0;
                    VT_LOGD("[ID=%d] notifyResolutionDegree change rotate way, after w:%d, h:%d, "
                            "d:%d ",
                ...
                VT_LOGD("[ID=%d]notifyResolutionDegree kWhatVideoSizeChanged,W %d H %d degree %d ",
                        mMultiInstanceID, width, height, degree);
        ...
        case kWhatRecorderNotify: {
            ...
            if (what == Recorder::kWhatREC_ReachMaxDuration) {
                sp<AMessage> notify = mNotify->dup();
                notify->setInt32(
                        "what", Recorder::kWhatREC_ReachMaxDuration);  /// may add some error detail
                notify->post();
            } else if (what == Recorder::kWhatREC_ReachMaxFileSize) {
                sp<AMessage> notify = mNotify->dup();
                notify->setInt32(
                        "what", Recorder::kWhatREC_ReachMaxFileSize);  /// may add some error detail
                notify->post();
            } else if (what == Recorder::kWhatREC_ReachNoStorageSpace) {
                sp<AMessage> notify = mNotify->dup();
                notify->setInt32(
                        "what",
                        Recorder::kWhatREC_ReachNoStorageSpace);  /// may add some error detail
                notify->post();
            } else if (what == Recorder::kWhatREC_RequsetIFrame) {
                sp<AMessage> notify = mNotify->dup();
                notify->setInt32("what",
                                 Recorder::kWhatREC_RequsetIFrame);  /// may add some error detail
                notify->post();
            } else if (what == Recorder::kWhatREC_WriteComplete) {
                sp<AMessage> notify = mNotify->dup();
                notify->setInt32("what",
                                 Recorder::kWhatREC_WriteComplete);  /// may add some error detail
                notify->post();
...
int32_t Sink::compareSettingChange(audio_dec_fmt* audioFormat, video_dec_fmt* videoFormat) {
    ...
    if (strcasecmp(mVideoConfigSettings.mimetype, videoFormat->mimetype)) {
        VT_LOGD("[ID=%d]mime change from %s to %s ", mMultiInstanceID,
                mVideoConfigSettings.mimetype, videoFormat->mimetype);
        changeType |= Update_CodecType;
    }

    if (mVideoConfigSettings.bitrate != videoFormat->bitrate) {
        VT_LOGD("[ID=%d]bitrate change from %d to %d ", mMultiInstanceID,
                mVideoConfigSettings.bitrate, videoFormat->bitrate);
        changeType |= Update_Bitrate;
    }

    if (mVideoConfigSettings.framerate != videoFormat->framerate) {
        VT_LOGD("[ID=%d]framerate change from %d to %d ", mMultiInstanceID,
                mVideoConfigSettings.framerate, videoFormat->framerate);
    ...
    if (mVideoConfigSettings.width != videoFormat->width ||
        mVideoConfigSettings.height != videoFormat->height) {
        VT_LOGD("[ID=%d]width change from %d to %d ", mMultiInstanceID, mVideoConfigSettings.width,
                videoFormat->width);
        VT_LOGD("[ID=%d]height change from %d to %d ", mMultiInstanceID,
                mVideoConfigSettings.height, videoFormat->height);
    ...
    if (mVideoConfigSettings.profile != videoFormat->profile ||
        mVideoConfigSettings.level != videoFormat->level) {
        VT_LOGD("[ID=%d]profile change from %d to %d ", mMultiInstanceID,
                mVideoConfigSettings.profile, videoFormat->profile);
        VT_LOGD("[ID=%d]level change from %d to %d ", mMultiInstanceID, mVideoConfigSettings.level,
                videoFormat->level);
