...
struct ImsSignal : public AHandler {
    ...
    ImsSignal() {
        ...
        ALOGI("[VT][ImsSignal]create ImsSignal  mTimedMsSignalValidCount =3,mTimedGapThresholdMs "
              "=3s");
        char value[PROPERTY_VALUE_MAX];

        if (property_get("persist.vendor.vt.Signal", value, NULL)) {
            mTimedMsSignalValidCount = atoi(value);
            ALOGI("[VT][ImsSignal]reset ImsSignal  mTimedMsSignalValidCount "
                  "=%d,mTimedGapThresholdMs  = %lld s",
    ...
    bool feedSignal(uint32_t simID, int32_t* itsSignal) {
        ...
        if (RSRP > -44) {
            rsrpIconLevel =
                    Signal_STRENGTH_NONE_OR_UNKNOWN;  // too good to believe or invalid value set
        } else if (RSRP >= -85) {
        ...
        *itsSignal = nowSignal;
        ...
        ALOGI("[VT][ImsSignal][feedSignal]simID=%d,RSRP=%d,SINR=%d,nowSignal=%s ", simID, RSRP,
              SINR, signalToString(nowSignal));
        return true;
    }
    // return worse case
    bool getSignal(uint32_t simID, int32_t* longSignal, int32_t* shortSignal) {
        ...
        *shortSignal = Signal_STRENGTH_NONE_OR_UNKNOWN;
        ...
        if (tempTimedMsSignal.size() == 0) {
            ...
            *shortSignal = (tempTimedMsSignal.editValueAt(tempTimedMsSignal.size() - 1));
            ALOGI("[VT][ImsSignal]getSignal simID %d,size %zu < %d", simID,
                  tempTimedMsSignal.size(), mTimedMsSignalValidCount);
            ALOGI("[VT][ImsSignal]getSignal simID %d,longSignal [%d][%s]  shortSignal [%d][%s] "
                  "stable %d",
                  simID, *longSignal, signalToString(*longSignal), *shortSignal,
                  signalToString(*shortSignal), stable);
            return false;
        }
        ...
        *shortSignal = (tempTimedMsSignal.editValueAt(tempTimedMsSignal.size() - 1));
        ...
        if ((time2 - time1) > mTimedGapThresholdMs) {
            ALOGE("[VT][ImsSignal]getSignal simID %d,GapMs %lld s ", simID,
                  (time2 - time1) / 1000ll);
        }

        ALOGI("[VT][ImsSignal]getSignal simID %d,longSignal [%d][%s]  shortSignal [%d][%s] stable "
              "%d",
              simID, *longSignal, signalToString(*longSignal), *shortSignal,
              signalToString(*shortSignal), stable);
        return stable;
  ...
  protected:
    ...
    virtual ~ImsSignal() { ALOGI("[VT][~ImsSignal]"); }
