...
sp<ImsSignal> getDefaultImsSignal() {
    ...
    if (gDefaultImsSignal != NULL) {
        ...
        ALOGI("[VT][ImsSignal]aleady getDefaultImsSignal %p,gDefaultImsSignal %d",
              gDefaultImsSignal.get(), gDefaultImsSignalCnt);
...

    {
        if (gDefaultImsSignal == NULL) {
            ...
            gDefaultImsSignalCnt++;
            ALOGI("[VT][ImsSignal]1th getDefaultImsSignal %p gDefaultImsSignalCnt %d",
                  gDefaultImsSignal.get(), gDefaultImsSignalCnt);
        }
    }
...
void deleteDefaultImsSignal()  // if no delete it ,we always
{
    ...
    gDefaultImsSignalCnt--;
    ALOGI("[deleteDefaultImsSignal]gDefaultImsSignal %p gDefaultImsSignalCnt %d",
          gDefaultImsSignal.get(), gDefaultImsSignalCnt);
