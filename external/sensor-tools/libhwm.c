...
int gsensor_get_static_calibration(struct caliData* caliDat) {
    ...
    HWMLOGD("[RD] %9.4f %9.4f %9.4f => %5d %5d %5d\n", caliDat->data[0], caliDat->data[1],
            caliDat->data[2], cali.x, cali.y, cali.z);
...
int alsps_read_nvram(HwmData* dat) {
    ...
    F_ID FileID = NVM_GetFileDesc(file_lid, &rec_size, &rec_num, ISREAD);
    int res, fd = FileID.iFileDesc;
    NVRAM_HWMON_PS_STRUCT hwmonPS = {{0, 0, 0}};
    ...
    if (res < 0) {
        ...
        HWMLOGD("nvram read: %d, %d, %d \n", hwmonPS.ps_cali[0], hwmonPS.ps_cali[1],
                hwmonPS.ps_cali[2]);
...
int alsps_write_nvram(HwmData* dat) {
    ...
    return !bRet;
#else
    HWMLOGD("alsps nvram write: %d, %d, %d \n", dat->ps_cali, dat->ps_threshold_high,
            dat->ps_threshold_low);
#endif
    return 0;
}
...
int calculate_psensor_unit_value(void) {
    ...
    HWMLOGD("calculatr before ps_min_smallest:%d, ps_min_largest:%d", ps_min_smallest,
            ps_min_largest);
    for (i = 0; i < 10; i++) {
        if ((err = alsps_read(ps_fd, &dat)) < 0) {
            HWMLOGE("read als_ps fail (%s)", strerror(errno));
        ...
        if (ps_min_smallest > dat.ps_data)
            ...
            ps_min_largest = dat.ps_data;
        HWMLOGD("calculatr after ps_min_smallest:%d, ps_min_largest:%d", ps_min_smallest,
                ps_min_largest);
...
int get_psensor_min_value(void) {
    ...
    for (i = 0; i < 20; i++) {
        ...
        if (ps_min < dat.ps_data) ps_min = dat.ps_data;
    }
...
int get_psensor_max_value(void) {
    ...
    for (i = 0; i < 20; i++) {
        ...
        if (ps_max > dat.ps_data) ps_max = dat.ps_data;
    }
