...
int write_data_to_driver(char* data, size_t length) {
    ...
    if (ret < 0) LOGD("write data to driver failed reason is %s \n", strerror(errno));
...
int write_wificfg(char* file_name, char* prefix) {
    ...
    if (pcfg) {
        ...
        if (ret < 0) LOGD("set cfg to driver error \n");
        free(pcfg);
    }
end:
    fclose(fd);
...
int file_event_hander(int inot_fd, int wd, struct inotify_event* event, char* path,
                      char* file_name) {
    char file_path[512];
...
void* wlan_files_monitor(void* pdata) {
    ...
    (void)pdata;
    while (access(WIFI_LOADER_DEV, F_OK) < 0) sleep(1);
#if 0
    ...
    while (access(nvram_filename, R_OK) < 0) sleep(1);
    ...
    while (running) {
        ...
        while (readbytes > readlen) {
            ...
            if (event->wd == nvram_wd) {
                nvram_wd = file_event_hander(inot_fd, nvram_wd, event, WIFI_NVRAM_PATH,
                                             nvram_filename + strlen(WIFI_NVRAM_PATH) + 1);
