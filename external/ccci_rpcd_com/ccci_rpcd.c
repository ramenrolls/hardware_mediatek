...
static int RPC_GetPackInfo(RPC_PACKET_INFO* pPackInfo, unsigned char* pData) {
    ...
    if (PackNum > RPC_MAX_ARG_NUM) return false;
    ...
    if (Index > RPC_MAX_BUF_SIZE) return false;
 ...
 */
void* RPC_PreparePktEx(unsigned char* pData, unsigned int data_to_send, CCCI_BUFF_T* ccci_src,
...
static bool RPC_WriteToMD(int DeviceFd, int BufIndex, RPC_PACKET_INFO* pPacketSrc,
    ...
    int data_to_send = 0;
    if (!stream_support) {
        pRpcBuf = (rpc_stream_buffer_t*)((char*)g_RpcInfo.pRpcBuf +
                                         (RPC_MAX_BUF_SIZE + sizeof(rpc_stream_msg_t)) * BufIndex);
    } else {
        buffer_slot = (STREAM_DATA*)((char*)g_RpcInfo.pRpcBuf +
                                     (RPC_MAX_BUF_SIZE + sizeof(STREAM_DATA)) * BufIndex);
    ...
    *((unsigned int*)pData) = PacketNum;
    ...
    if (!stream_support) {
        ...
        if (data_len <= MAX_RPC_PKT_BYTE) {
            ...
            while ((unsigned int)(data_sent + sizeof(CCCI_BUFF_T) + sizeof(unsigned int)) <
                   DataLength) {
                ...
                // not cosindered as payload, so not counted in MAX_RPC_PKT_BYTE
...
static int tc1_srv_check(int ops_id, RPC_PACKET_INFO* packet_info, int* packet_num, int buf[]) {
    if (tc1_lib_ready) return tc1_rpc_srv_entry(ops_id, packet_info, packet_num, buf);
...
void signal_treatment(int param) {
     ...
     * set property ctl.stop will send SIGKILL to ccci_rpcd(check service_stop_or_reset() in
     * init.c), but SIGKILL is not catchable. kill pid will send SIGTERM to ccci_rpcd, we can catch
     * this signal, but the process is just terminated, and no time for us to check exit_signal in
     * main(). per system team's comment, kernel will free all resource (memory get from malloc,
     * etc.), so we do NOT need to take care of these.
     */
...
int main(int argc, char* argv[]) {
    ...
    if (!stream_support) {
        g_RpcInfo.pRpcBuf = mmap(NULL, sizeof(rpc_stream_buffer_t), PROT_READ | PROT_WRITE,
    ...
    retry:
        if (!stream_support) {
            ...
            pRpcBuf = (rpc_stream_buffer_t*)((char*)g_RpcInfo.pRpcBuf +
                                             (RPC_MAX_BUF_SIZE + sizeof(rpc_stream_buffer_t)) *
                                                     ReqBufIndex);
        } else {
            while (1) {
                ...
                RetVal = read(DeviceFd, pkt_buff,
                              (MAX_RPC_PKT_BYTE + sizeof(CCCI_BUFF_T) + sizeof(unsigned int)));
                if (RetVal <= 0) {
                ...
                buffer_slot =
                        (STREAM_DATA*)((char*)g_RpcInfo.pRpcBuf +
                                       (RPC_MAX_BUF_SIZE + sizeof(STREAM_DATA)) * ReqBufIndex);
                ...
                if (!CCCI_RPC_PEER_REQ_SEND_AGAIN(ccci_h)) {
                    ...
                    if (g_RpcInfo.rpc_buff_state[ReqBufIndex] == RPC_BUFF_IDLE) {
                        ...
                        unsigned int length =
                                ccci_h->data[1] - sizeof(CCCI_BUFF_T) - sizeof(unsigned int);
                        memcpy(p_rpc_buff + g_RpcInfo.rpc_buff_offset[ReqBufIndex],
                               stream->payload.buffer, length); /* CCCI_HEADER + RPC_OP_ID */
                        g_RpcInfo.rpc_buff_offset[ReqBufIndex] += length;
                    } else {
                ...
                if (g_RpcInfo.rpc_buff_state[ReqBufIndex] == RPC_BUFF_IDLE) break;
                RPC_WAKE_UNLOCK();
            }
            pRpcBuf = &buffer_slot->payload;
        }
    ...
    close(DeviceFd);
    free(PackInfo);
    if (stream_support) free(g_RpcInfo.pRpcBuf);
    dlclose(tc1_support_lib);
    return 0;
}
