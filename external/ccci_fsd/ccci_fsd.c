...
static char FsRootDir_MD1[FS_MAX_DIR_NUM][36] = {
        FS_ROOT_DIR_MD1, FS_ROOT_DIR1_MD1, FS_ROOT_DIR2_MD1, FS_ROOT_DIR_DSP,
        FS_ROOT_COMMON,  FS_ROOT_MDLPM,    FS_ROOT_DIR3_MD1, FS_ROOT_DIR4_MD1};
static char FsRootDir_MD2[FS_MAX_DIR_NUM][36] = {FS_ROOT_DIR_MD2, FS_ROOT_DIR1_MD2,
                                                 FS_ROOT_DIR2_MD2, FS_ROOT_DIR_DSP, FS_ROOT_COMMON};
static char FsRootDir_MD3[FS_MAX_DIR_NUM][36] = {
        FS_ROOT_DIR_MD3, FS_ROOT_DIR1_MD3, FS_ROOT_DIR2_MD3, FS_ROOT_DIR_DSP, FS_ROOT_DIR3_MD3};
static char FsRootDir_MD5[FS_MAX_DIR_NUM][36] = {FS_ROOT_DIR_MD5, FS_ROOT_DIR1_MD5,
                                                 FS_ROOT_DIR2_MD5, FS_ROOT_DIR_DSP, FS_ROOT_COMMON};
static char* MdRootDir[FS_MAX_DIR_NUM] = {MD_ROOT_DIR,  MD_ROOT_DIR1, MD_ROOT_DIR2, MD_ROOT_DIR_DSP,
                                          MD_ROOT_DIR3, MD_ROOT_DIR4, MD_ROOT_DIR5, MD_ROOT_DIR6};
...
static int FS_IsReadOnly(const char* file) {
    ...
    Mode = StatBuf.st_mode;
    if ((Mode & (S_IRUSR | S_IRGRP | S_IROTH)) && !(Mode & (S_IWUSR | S_IWGRP | S_IWOTH)) &&
...
static int FS_GetFileDetail(const char* file, FS_FileDetail* FileDetail) {
    ...
    LOGE("FS_GetFileDetail: mtime: %llu, atime=%llu, ctime=%llu\n", FileDetail->LastModifiedTime,
         FileDetail->LastAccessTime, FileDetail->LastStatusChangeTime);
...
static int FS_GetFATAttr(const char* file) {
    ...
    if (ioctl(fd, FAT_IOCTL_GET_ATTRIBUTES, &attr)) goto err;
...
int PatiMatch(const char* pattern, const char* string) {
    switch (pattern[0]) {
        ...
        default:
            return (toupper(pattern[0]) == toupper(string[0])) &&
                   PatiMatch(pattern + 1, string + 1);
    }
}
...
static void FS_EntryLinuxToDos(FS_DOSDirEntry* DosFileInfo, const char* file) {
    ...
    if (AccessTime != NULL) {
        ...
        // Year: 7 bit in DOS DateTime stucture and year in DOS is from 1980 but year in Linux is
        // from 1900
        *(__packed int*)(&(DosFileInfo->CreateDateTime)) =
                (((AccessTime->tm_sec & 0x1F)) | ((AccessTime->tm_min & 0x3F) << 5) |
                 ((AccessTime->tm_hour & 0x1F) << 11) | ((AccessTime->tm_mday & 0x1F) << 16) |
    ...
    if (ModifyTime != NULL) {
        ...
        // Year: 7 bit in DOS DateTime stucture and year in DOS is from 1980 but year in Linux is
        // from 1900
        *(__packed int*)(&(DosFileInfo->DateTime)) =
                (((ModifyTime->tm_sec & 0x1F)) | ((ModifyTime->tm_min & 0x3F) << 5) |
                 ((ModifyTime->tm_hour & 0x1F) << 11) | ((ModifyTime->tm_mday & 0x1F) << 16) |
    ...
    if (Index > FS_MAX_BUF_SIZE) {
        ...
        for (i = 0; i < PackNum; i++) {
            ...
            LOGE("GetPackInfo: packet[%d].Length=0x%08X, packet[%d].pData=0x%08X, cnt=0x%08X \n", i,
                 pPackInfo[i].Length, i, *((unsigned int*)pPackInfo[i].pData), cnt);
        }

        for (i = 0; i < 7; i++)
            LOGE("0x%08X, 0x%08X, 0x%08X, 0x%08X\n", *((unsigned int*)(pData + 16 * i)),
                 *((unsigned int*)(pData + 16 * i + 4)), *((unsigned int*)(pData + 16 * i + 8)),
                 *((unsigned int*)(pData + 16 * i + 12)));
 ...
 */
void* FS_PreparePktEx(char* pData, unsigned int data_to_send, CCCI_BUFF_T* ccci_src,
                      unsigned int op_id, unsigned int again) {
    char* bak_ptr = NULL;
    ...
    // move pointer forward to fill in CCCI header, this will replace orignal data there, so we
    // backup them first
    ...
    //      stream->header.data[0], stream->header.data[1], stream->header.channel,
    //      stream->header.reserved, stream->payload.OperateID);
...
static bool FS_WriteToMD(int DeviceFd, int BufIndex, FS_PACKET_INFO* pPacketSrc, int PacketNum) {
    ...
    int data_to_send = 0;
    int local_errno = 0;
    STREAM_DATA* buffer_slot = (STREAM_DATA*)((char*)g_FsInfo.pFsBuf +
                                              (FS_MAX_BUF_SIZE + sizeof(STREAM_DATA)) * BufIndex);
    ...
    *((unsigned int*)pData) = PacketNum;
    ...
    for (i = 0; i < PacketNum; i++) {
        ...
        *((unsigned int*)pData) = pPacketSrc[i].Length;
        ...
        if (pData != pPacketSrc[i].pData) memcpy(pData, pPacketSrc[i].pData, pPacketSrc[i].Length);
        pData += AlignLength;
    }
        ...
        RETRY_WRITE:
            ...
            if (ret != DataLength) {
                ...
                LOGE("Failed to write only one FS packet(%d)!! (%d/%d)\n", DataLength, ret,
                     local_errno);
                if (local_errno == EINTR) goto RETRY_WRITE;
                return bRet;
            }
            ...
            while ((int)(data_sent + sizeof(CCCI_BUFF_T) + sizeof(unsigned int)) < DataLength) {
                ...
                // Add CCCI header and operation ID size to packet size, be aware of that OP_ID is
                // not cosindered as payload, so not counted in MAX_FS_PKT_BYTE
                ...
                if (ret != pkt_size) {
                    ...
                    LOGD("Write %d bytes to slot %d, CCCI_H(0x%X)(0x%X)(0x%X)(0x%X)\n", ret,
                         BufIndex, dst_ccci_h->data[0], dst_ccci_h->data[1], dst_ccci_h->channel,
                         dst_ccci_h->reserved);
                }
...
static unsigned int FS_GetFreeHandle() {
    ...
    for (i = 1; i < FS_FILE_MAX; i++) {
        if (g_FsInfo.hFileHandle[i].fInUse == false) return i;
    }
    return 0xFFFFFFFF;
}
...
static inline int is_protect_path(char* ConvFileName) {
    ...
    if (ret) ret = strncmp(ConvFileName, MD_ROOT_DIR2, len);
...
static void CCCI_Dump_FS_Path(const char* FsFileName) {
    ...
    if (need_trigger_ne) *(unsigned int volatile*)0 = 0;
}
...
static int FS_CCCI_Open(const wchar_t* FileName, unsigned int Flag) {
    ...
    for (i = 0; i < FS_MAX_DIR_NUM; i++) {
        if (strncmp(ConvFileName, MdRootDir[i], strlen(MdRootDir[i])) == 0) {
            if ((strlen(ConvFileName + strlen(MdRootDir[i])) + strlen(FsRootDir[i]) + 1) >
                PATH_MAX) {
            ...
            if (strncmp(ConvFileName, MD_ROOT_DIR_DSP, strlen(MD_ROOT_DIR_DSP)) == 0) {
                ...
                if (access(FsFileName, F_OK) != 0) {
                    LOGE("DSP image %s not exist, try CIP path %s\n", FsFileName,
                         FS_ROOT_DIR_DSP_CIP);
                    FsFileNameCount =
                            sprintf(FsFileName, "%s/dsp_%s.bin", FS_ROOT_DIR_DSP_CIP, md_post_fix);
                }
                LOGD("open DSP image %s\n", FsFileName);
            } else {
                FsFileNameCount = snprintf(FsFileName, sizeof(FsFileName), "%s%s", FsRootDir[i],
                                           (ConvFileName + strlen(MdRootDir[i])));
            }
            break;
        }
    }
    ...
    if (Flag & FS_CREATE) LinuxFlag |= O_CREAT | O_RDWR;

    if (Flag & FS_CREATE_ALWAYS) LinuxFlag |= O_CREAT | O_RDWR | O_TRUNC;

    // Notice here
    // if(Flag & FS_COMMITTED)
    //	LinuxFlag |= O_SYNC;

    if (Flag & FS_NONBLOCK_MODE) LinuxFlag |= O_NONBLOCK;
    ...
    if (Fd == INVALID_HANDLE_VALUE) {
        ...
        g_FsInfo.hFileHandle[HandleIndex].pFsFileName =
                malloc(sizeof(char) * (FsFileNameCount + 1));
        ...
        memcpy(g_FsInfo.hFileHandle[HandleIndex].pFsFileName, FsFileName,
               sizeof(char) * (FsFileNameCount + 1));
        ret = HandleIndex;
    }
...
_Exit:
    ...
    if (len >= 8) {
        ...
        LOGD("O: [%02X%02X%02X%02X%02X%02X%02X%02X](0x%08X): (%d %d) \n",
             (ConvFileName[len - 1] - 32), (ConvFileName[len - 2] - 32),
             (ConvFileName[len - 3] - 32), (ConvFileName[len - 4] - 32),
             (ConvFileName[len - 5] - 32), (ConvFileName[len - 6] - 32),
             (ConvFileName[len - 7] - 32), (ConvFileName[len - 8] - 32), Flag, Fd, ret);
    } else {
        LOGD("O: len<8(%d)\n", len);
    }
    return ret;
...
static int FS_CCCI_Close(int HandleIndex) {
    ...
    if (g_FsInfo.hFileHandle[HandleIndex].fSearch) {
        ...
        if (g_FsInfo.hFileHandle[HandleIndex].Flag &
            (FS_CREATE | FS_CREATE_ALWAYS | FS_COMMITTED)) {
            LOGD("C:fsync+ %d, %d, FLAG:0x%x\n", HandleIndex, ret,
                 g_FsInfo.hFileHandle[HandleIndex].Flag);
...
Exit:
    len = strlen(ConvFileName);
    if (len >= 8) {
        LOGD("CD: [%02X%02X%02X%02X%02X%02X%02X%02X]: %d \n", (ConvFileName[len - 1] - 32),
             (ConvFileName[len - 2] - 32), (ConvFileName[len - 3] - 32),
             (ConvFileName[len - 4] - 32), (ConvFileName[len - 5] - 32),
             (ConvFileName[len - 6] - 32), (ConvFileName[len - 7] - 32),
             (ConvFileName[len - 8] - 32), ret);
    } else {
        LOGD("CD: len<8(%d)\n", len);
    }
    return ret;
...
Exit:
    len = strlen(ConvFileName);
    if (len >= 8) {
        if (is_protect_path(ConvFileName)) LOGW("MD: %s, ret %d\n", ConvFileName, ret);
...
static int FS_CCCI_GetFileSize(int HandleIndex, unsigned int* pFileSize) {
    ...
    if (fstat(Fd, &StatBuf) == -1) {
        ...
        *pFileSize = (unsigned int)StatBuf.st_size;
        ...
        // g_FsInfo.hFileHandle[HandleIndex].pFsFileName, 	(unsigned int)StatBuf.st_size);
        ret = FS_NO_ERROR;
    }
...
static int FS_CCCI_Rename(const wchar_t* FileName, const wchar_t* NewFileName) {
    ...
    for (i = 0; i < FS_MAX_DIR_NUM; i++) {
        if (strncmp(NewConvFileName, MdRootDir[i], strlen(MdRootDir[i])) == 0) {
            if ((strlen(NewConvFileName + strlen(MdRootDir[i])) + strlen(FsRootDir[i]) + 1) >
                PATH_MAX) {
                LOGE("Rename: [error]file path too long: %s \n", NewConvFileName);
                ret = FS_PATH_OVER_LEN_ERROR;
                goto Exit;
            ...
            // Convert the file name
            FsFileNameCount = snprintf(FsNewFileName, sizeof(FsNewFileName), "%s%s", FsRootDir[i],
                                       (NewConvFileName + strlen(MdRootDir[i])));
            break;
        }
    }
...
Exit:
    ...
    if ((len1 >= 8) && (len2 >= 8)) {
        LOGD("R: [%02X%02X%02X%02X%02X%02X%02X%02X] -> [%02X%02X%02X%02X%02X%02X%02X%02X]: %d \n",
             (ConvFileName[len1 - 1] - 32), (ConvFileName[len1 - 2] - 32),
             (ConvFileName[len1 - 3] - 32), (ConvFileName[len1 - 4] - 32),
             (ConvFileName[len1 - 5] - 32), (ConvFileName[len1 - 6] - 32),
             (ConvFileName[len1 - 7] - 32), (ConvFileName[len1 - 8] - 32),
             (NewConvFileName[len2 - 1] - 32), (NewConvFileName[len2 - 2] - 32),
             (NewConvFileName[len2 - 3] - 32), (NewConvFileName[len2 - 4] - 32),
             (NewConvFileName[len2 - 5] - 32), (NewConvFileName[len2 - 6] - 32),
             (NewConvFileName[len2 - 7] - 32), (NewConvFileName[len2 - 8] - 32), ret);
    } else {
...
Exit:
    len = strlen(ConvFileName);
    if (len >= 8) {
        if (is_protect_path(ConvFileName)) LOGW("D: %s, ret %d\n", ConvFileName, ret);
...
static int FS_RecursiveSearch(const char* FsDirName, unsigned int Flag,
                              unsigned int* pClusterCount) {
    DIR* pDir;
    ...
    while ((pDirent = readdir(pDir)) != NULL) {
        ...
        if ((StatBuf.st_mode & S_IFDIR) && strcmp(pDirent->d_name, ".") != 0 &&
...
static int FS_CCCI_GetFolderSize(const wchar_t* FullPath, unsigned int Flag) {
    ...
    for (i = 0; i < FS_MAX_DIR_NUM; i++) {
        if (strncmp(ConvFullPath, MdRootDir[i], strlen(MdRootDir[i])) == 0) {
            if ((strlen(ConvFullPath + strlen(MdRootDir[i])) + strlen(FsRootDir[i]) + 1) >
                PATH_MAX) {
            ...
            // Convert the file name
            FsFileNameCount = snprintf(FsFileName, sizeof(FsFileName), "%s%s", FsRootDir[i],
                                       (ConvFullPath + strlen(MdRootDir[i])));
            break;
        }
    }
    ...
    ClusterCount = 0;
    FileCount = FS_RecursiveSearch(FsFileName, FS_FILE_TYPE | FS_DIR_TYPE | FS_RECURSIVE_TYPE,
                                   &ClusterCount);

    if (FileCount < 0) {
        local_errno = errno;
        LOGE("GetFolderSize: [error]fail recursive search file %s: %d, error=%d\n", FsFileName,
             FileCount, local_errno);
        ret = FileCount;
        goto Exit;
    }
...
Exit:
    ...
    if (len >= 8) {
        LOGD("FS: [%02X%02X%02X%02X%02X%02X%02X%02X](0x%08X): %d \n", (ConvFullPath[len - 1] - 32),
             (ConvFullPath[len - 2] - 32), (ConvFullPath[len - 3] - 32),
             (ConvFullPath[len - 4] - 32), (ConvFullPath[len - 5] - 32),
             (ConvFullPath[len - 6] - 32), (ConvFullPath[len - 7] - 32),
             (ConvFullPath[len - 8] - 32), Flag, ret);
    } else {
        LOGD("FS: len<8(%d)\n", len);
    }
    return ret;
...
Exit:
    ...
    if (len >= 8) {
        LOGD("GetFileDetail: [%02X%02X%02X%02X%02X%02X%02X%02X]: %d \n",
             ...
             (ConvFileName[len - 3] - 32), (ConvFileName[len - 4] - 32),
             (ConvFileName[len - 5] - 32), (ConvFileName[len - 6] - 32),
             (ConvFileName[len - 7] - 32), (ConvFileName[len - 8] - 32), ret);
    } else {
...
Exit:
    len = strlen(ConvFileName);
    if (len >= 8) {
        if (is_protect_path(ConvFileName)) LOGW("X: %s, ret %d\n", ConvFileName, ret);
        LOGD("X: [%02X%02X%02X%02X%02X%02X%02X%02X](0x%08X): %d \n", (ConvFileName[len - 1] - 32),
             (ConvFileName[len - 2] - 32), (ConvFileName[len - 3] - 32),
             (ConvFileName[len - 4] - 32), (ConvFileName[len - 5] - 32),
             (ConvFileName[len - 6] - 32), (ConvFileName[len - 7] - 32),
             (ConvFileName[len - 8] - 32), Flag, ret);
    } else {
        LOGD("X: len < 8(%d)\n", len);
    }
    return ret;
...
static int FS_CCCI_Move(const wchar_t* SrcFullPath, const wchar_t* DstFullPath, unsigned int Flag) {
    ...
    for (i = 0; i < FS_MAX_DIR_NUM; i++) {
        if (strncmp(DstConvFullPath, MdRootDir[i], strlen(MdRootDir[i])) == 0) {
            if ((strlen(DstConvFullPath + strlen(MdRootDir[i])) + strlen(FsRootDir[i]) + 1) >
                PATH_MAX) {
                LOGE("Move: [error]file path too long: %s \n", DstConvFullPath);
                ret = FS_PATH_OVER_LEN_ERROR;
                goto Exit;
            ...
            // Convert the file name
            FsFileNameCount = snprintf(FsNewFileName, sizeof(FsNewFileName), "%s%s", FsRootDir[i],
                                       (DstConvFullPath + strlen(MdRootDir[i])));
            break;
        }
    }
...
Exit:
    ...
    if ((len1 >= 8) && (len2 >= 8)) {
        if (is_protect_path(ConvFullPath)) LOGW("D: %s, ret %d\n", ConvFullPath, ret);
        LOGD("M: [%02X%02X%02X%02X%02X%02X%02X%02X] -> [%02X%02X%02X%02X%02X%02X%02X%02X](0x%08X): "
             "%d \n",
             (ConvFullPath[len1 - 1] - 32), (ConvFullPath[len1 - 2] - 32),
             (ConvFullPath[len1 - 3] - 32), (ConvFullPath[len1 - 4] - 32),
             (ConvFullPath[len1 - 5] - 32), (ConvFullPath[len1 - 6] - 32),
             (ConvFullPath[len1 - 7] - 32), (ConvFullPath[len1 - 8] - 32),
             (DstConvFullPath[len2 - 1] - 32), (DstConvFullPath[len2 - 2] - 32),
             (DstConvFullPath[len2 - 3] - 32), (DstConvFullPath[len2 - 4] - 32),
             (DstConvFullPath[len2 - 5] - 32), (DstConvFullPath[len2 - 6] - 32),
             (DstConvFullPath[len2 - 7] - 32), (DstConvFullPath[len2 - 8] - 32), Flag, ret);
    } else {
...
static int FS_CCCI_FindFirst(const wchar_t* PatternName, char Attr, char AttrMask,
                             FS_DOSDirEntry* pFileInfo, wchar_t* FileName,
    ...
    *pMaxLength = 0;
    ...
    for (i = 0; i < FS_MAX_DIR_NUM; i++) {
        if (strncmp(ConvPatternName, MdRootDir[i], strlen(MdRootDir[i])) == 0) {
            if ((strlen(ConvPatternName + strlen(MdRootDir[i])) + strlen(FsRootDir[i]) + 1) >
                PATH_MAX) {
            ...
            // Convert the file name
            FsFileNameCount = snprintf(FsPatternName, sizeof(FsPatternName), "%s%s", FsRootDir[i],
                                       (ConvPatternName + strlen(MdRootDir[i])));
            break;
        }
    }
    ...
    if (bFound == true) {
        ...
        g_FsInfo.hFileHandle[HandleIndex].pFsFileName =
                (char*)malloc(sizeof(char) * (strlen(FsPatternName) + 1));
        if (g_FsInfo.hFileHandle[HandleIndex].pFsFileName == NULL) {
            LOGE("FindFirst: [error]fail alloc memory for FileName \n");
            ret = FS_ERROR_RESERVED;
            goto Exit;
        }
        memcpy(g_FsInfo.hFileHandle[HandleIndex].pFsFileName, FsPatternName,
               sizeof(char) * (strlen(FsPatternName) + 1));

        g_FsInfo.hFileHandle[HandleIndex].pFsSearchPattern =
                (char*)malloc(sizeof(char) * (strlen(FsFileName) + 1));
        if (g_FsInfo.hFileHandle[HandleIndex].pFsSearchPattern == NULL) {
            LOGE("FindFirst: [error]fail alloc memory for SearchPattern \n");
            ret = FS_ERROR_RESERVED;
            goto Exit;
        }
        memcpy(g_FsInfo.hFileHandle[HandleIndex].pFsSearchPattern, FsFileName,
               sizeof(char) * (strlen(FsFileName) + 1));
    ...
    if (pDirent == NULL) {
        ...
        if (len1 >= 8) {
            LOGD("FF: [%02X%02X%02X%02X%02X%02X%02X%02X](%d %d %d): [null] %d \n",
                 (FsDirName[len1 - 1] - 32), (FsDirName[len1 - 2] - 32), (FsDirName[len1 - 3] - 32),
                 (FsDirName[len1 - 4] - 32), (FsDirName[len1 - 5] - 32), (FsDirName[len1 - 6] - 32),
                 (FsDirName[len1 - 7] - 32), (FsDirName[len1 - 8] - 32), Attr, AttrMask,
                 *pMaxLength, ret);
        } else {
            LOGD("FF: len1 < 8(%d)\n", len1);
        }
        ...
        if ((len2 >= 8) && (len1 >= 8)) {
            LOGD("FF: [%02X%02X%02X%02X%02X%02X%02X%02X](%d %d %d): "
                 "[%02X%02X%02X%02X%02X%02X%02X%02X] %d \n",
                 (FsDirName[len1 - 1] - 32), (FsDirName[len1 - 2] - 32), (FsDirName[len1 - 3] - 32),
                 (FsDirName[len1 - 4] - 32), (FsDirName[len1 - 5] - 32), (FsDirName[len1 - 6] - 32),
                 (FsDirName[len1 - 7] - 32), (FsDirName[len1 - 8] - 32), Attr, AttrMask,
                 *pMaxLength, (pDirent->d_name[len2 - 1] - 32), (pDirent->d_name[len2 - 2] - 32),
                 (pDirent->d_name[len2 - 3] - 32), (pDirent->d_name[len2 - 4] - 32),
                 (pDirent->d_name[len2 - 5] - 32), (pDirent->d_name[len2 - 6] - 32),
                 (pDirent->d_name[len2 - 7] - 32), (pDirent->d_name[len2 - 8] - 32), ret);
        } else {
...
static int FS_CCCI_FindNext(int HandleIndex, FS_DOSDirEntry* pFileInfo, wchar_t* FileName,
                            unsigned int* pMaxLength) {
    static char FsPatternName[PATH_MAX] = {0};
    bool bFound = false;
    DIR* pDir;
    ...
    if (pDirent == NULL) {
        ...
        if (len >= 8) {
            LOGD("FN: %d %d: [%02X%02X%02X%02X%02X%02X%02X%02X] %d \n", HandleIndex, *pMaxLength,
                 (pDirent->d_name[len - 1] - 32), (pDirent->d_name[len - 2] - 32),
                 (pDirent->d_name[len - 3] - 32), (pDirent->d_name[len - 4] - 32),
                 (pDirent->d_name[len - 5] - 32), (pDirent->d_name[len - 6] - 32),
                 (pDirent->d_name[len - 7] - 32), (pDirent->d_name[len - 8] - 32), ret);
        } else {
            LOGD("FN: stren < 8(%d)\n", len);
        }
    }
...
static int FS_CCCI_FindClose(int HandleIndex) {
    ...
    LOGD("FindClose:%s: (%d, %d)\n", g_FsInfo.hFileHandle[HandleIndex].pFsFileName, HandleIndex,
         g_FsInfo.hFileHandle[HandleIndex].hFile);
...
static int FS_CCCI_GetDrive(unsigned int Type, unsigned int Serial, unsigned int AltMask) {
    ...
    if (((Type & (FS_DRIVE_I_SYSTEM | FS_DRIVE_V_NORMAL | FS_DRIVE_V_REMOVABLE)) == 0) ||
        ...
        ((Type == FS_DRIVE_V_REMOVABLE) && (Serial > 1)) || (Serial < 1) ||
...
static void FS_CCCI_ShutDown() { FS_CCCI_CloseAll(); }
...
static int FS_CCCI_GetDiskInfo(FS_DiskInfo* pDiskInfo) {
    ...
    LOGD("I: %s: %d, %d, %d\n", FsRootDir[0], pDiskInfo->SectorsPerCluster,
         pDiskInfo->TotalClusters, pDiskInfo->FreeClusters);
...
static int FS_CCCI_Restore(const wchar_t* FileName) {
    ...
    for (i = 0; i < FS_MAX_DIR_NUM; i++) {
        if (strncmp(ConvFileName, MdRootDir[i], strlen(MdRootDir[i])) == 0) {
            ...
            // Convert the file name
            snprintf(FsFileName, sizeof(FsFileName), "%s%s", FsRootDir[i],
                     (ConvFileName + strlen(MdRootDir[i])));
            break;
        }
    }
...
static int FS_CCCI_CMPT_Read(wchar_t* name, nvram_fs_para_cmpt_t* in_para) {
    ...
    if (in_para->opid_map & FS_CMPT_OPEN) {
        ...
        if (slower_100ms.profiling) {
            ...
            /*LOGD("profile: debug: elapsed_in_ms: %ld %ld %ld %ld, %d", time_start.tv_sec,
               time_start.tv_nsec, time_end.tv_sec, time_end.tv_nsec, elapsed_in_ms);*/
        ...
        if (in_para->opid_map & FS_CMPT_READ) {
            ret[3] = FS_CCCI_Read(HandleIndex, in_para->data_ptr, in_para->length,
                                  &in_para->act_read);
            if (ret[3] < 0) {
...
void FS_Deinit(void) { FS_CCCI_ShutDown(); }
...
static void ccci_wait_md_status_to_HS1() {
    ...
    while (1) {
        ...
        if (count > 0) {
            ...
            if (status_buf.event_type == MD_STA_EV_HS1) break;
...
int main(int argc, char* argv[]) {
    ...
    if (!stream_support) {
        g_FsInfo.pFsBuf = mmap(NULL, sizeof(fs_stream_buffer_t) * 5, PROT_READ | PROT_WRITE,
                               MAP_SHARED, DeviceFd, 0);
    } else {
        // FS_MAX_BUF_SIZE : ccci_h + opid + argc + MAX_ARG * arg.len + MAX_ARG * arg.align + 16KB
        // data + align(128B)
    ...
    if (signal(SIGHUP, signal_treatment) == SIG_ERR) LOGE("can't catch SIGHUP\n");
    if (signal(SIGPIPE, signal_treatment) == SIG_ERR) LOGE("can't catch SIGPIPE\n");
    if (signal(SIGKILL, signal_treatment) == SIG_ERR) LOGE("can't catch SIGKILL\n");
    if (signal(SIGINT, signal_treatment) == SIG_ERR) LOGE("can't catch SIGINT\n");
    if (signal(SIGUSR1, signal_treatment) == SIG_ERR) LOGE("can't catch SIGUSR1\n");
    if (signal(SIGUSR2, signal_treatment) == SIG_ERR) LOGE("can't catch SIGUSR2\n");
    if (signal(SIGTERM, signal_treatment) == SIG_ERR) LOGE("can't catch SIGTERM\n");
    if (signal(SIGALRM, signal_treatment) == SIG_ERR) LOGE("can't catch SIGALRM\n");
    ...
    retry:
        if (!stream_support) {
            ...
            pFsBuf = (FS_BUF*)((char*)g_FsInfo.pFsBuf +
                               (FS_MAX_BUF_SIZE + sizeof(FS_BUF)) * ReqBufIndex);
        } else {
            while (1) {
                memset(pkt_buff, 0, MAX_FS_BUF_BYTE);
                // add an extra integer as MD consider OP_ID as not part of the "payload"
                RetVal = read(DeviceFd, pkt_buff,
                              (MAX_FS_PKT_BYTE + sizeof(CCCI_BUFF_T) + sizeof(unsigned int)));
                if (RetVal <= 0) {
                ...
                LOGD("Read %d bytes from slot %d, CCCI_H(0x%X)(0x%X)(0x%X)(0x%X)", RetVal,
                     ReqBufIndex, ccci_h->data[0], ccci_h->data[1], ccci_h->channel,
                     ccci_h->reserved);
                if (((ccci_h->channel & 0xFFFF) != 0x0e) || (ReqBufIndex < 0) ||
                    (ReqBufIndex >= FS_REQ_BUFFER_MUN) ||
                    (ccci_h->data[1] > (sizeof(STREAM_DATA) + FS_MAX_BUF_SIZE))) {
                    LOGE("Main: [error]packet data check fail: ch =0x%x, fs_buf_idx=%d, data[1] = "
                         "0x%x, 0x%x \n",
                         ccci_h->channel, ReqBufIndex, ccci_h->data[1],
                         (sizeof(STREAM_DATA) + FS_MAX_BUF_SIZE));
                    RetVal = FS_PARAM_ERROR;
                    PackInfo[PacketNum].Length = sizeof(unsigned int);
                    PackInfo[PacketNum++].pData = (void*)&RetVal;
                    goto _Next;
                }
                buffer_slot = (STREAM_DATA*)((char*)g_FsInfo.pFsBuf +
                                             (FS_MAX_BUF_SIZE + sizeof(STREAM_DATA)) * ReqBufIndex);
                ...
                if (!CCCI_FS_PEER_REQ_SEND_AGAIN(ccci_h)) {
                    ...
                    if (g_FsInfo.fs_buff_state[ReqBufIndex] == FS_BUFF_IDLE) {
                        ...
                        unsigned int length =
                                ccci_h->data[1] - sizeof(CCCI_BUFF_T) - sizeof(unsigned int);
                        memcpy(p_fs_buff + g_FsInfo.fs_buff_offset[ReqBufIndex],
                               stream->payload.Buffer, length); /* CCCI_HEADER + FS_OP_ID */
                        g_FsInfo.fs_buff_offset[ReqBufIndex] += length;
                    } else {
                ...
                if (g_FsInfo.fs_buff_state[ReqBufIndex] == FS_BUFF_IDLE) break;
                FS_WAKE_UNLOCK();
            };
            pFsBuf = &buffer_slot->payload;
        }
        ...
        switch (pFsBuf->OperateID) {
            ...
            case FS_CCCI_OP_CMPT_READ: {
                ...
                in_cmpt_para.data_ptr =
                        pFsBuf->Buffer + (1 + 1 + 2 + 1 + 1 + 1 + 1 + 1) * sizeof(int);
            ...
            case FS_CCCI_OP_CMPT_WRITE: {
                ...
                FS_ConvWcsToCs(FileName, ConvFileName);
                // FS_MD2APPath(FileName, ConvFileName, FsFileName, PATH_MAX, &FsFileNameCount);
                LOGD("CMPTW File name:%s opid_map:0x%08x Flag:0x%08x Offset:%d Whence:%d Length:%d "
                     "file_handle:%d",
            ...
            case FS_CCCI_OP_READ: {
                ...
                /*memory corrupt issue: there are only two parameters for read from modem CCCI, so
                 * PackInfo[2] is invalid*/
                // void* pBuffer = PackInfo[2].pData;
            ...
            case FS_CCCI_OP_FINDFIRST: {
                ...
                RetVal = FS_CCCI_FindFirst(PatternName, Attr, AttrMask, &DosDirEntry, FileNameTemp,
                                           &MaxLength);
                PackInfo[PacketNum].Length = sizeof(int);
                PackInfo[PacketNum++].pData = (void*)&RetVal;
                PackInfo[PacketNum].Length = sizeof(FS_DOSDirEntry);
                PackInfo[PacketNum++].pData = &DosDirEntry;
                PackInfo[PacketNum].Length =
                        (MaxLength > 0) ? ((MaxLength + 1) * 2) : 0;  // wide character
    ...
    close(DeviceFd);
    free(PackInfo);
    if (stream_support) free(g_FsInfo.pFsBuf);
    return 0;
}
