...
// FS_Move, FS_Count, FS_GetFolderSize, FS_XDelete, FS_XFindReset (Sorting) Parameter and Flag
// Passing
...
// FS_Seek Parameter
typedef enum { FS_FILE_BEGIN, FS_FILE_CURRENT, FS_FILE_END } FS_SEEK_POS_ENUM;

// Find Series Return Value
typedef enum { FS_NOT_MATCH, FS_LFN_MATCH, FS_SFN_MATCH } FS_FIND_ENUM;
...
#define CCCI_FS_PEER_REQ_SEND_AGAIN(_p) \
    (((((CCCI_BUFF_T*)(_p))->data[0] & CCCI_FS_REQ_SEND_AGAIN) != 0) ? 1 : 0)
...
typedef struct _nvram_fs_para_cmpt {
    unsigned int opid_map;   // Indicate which operation shal be taken: open, get file size, seek,
                             // read, close
...
typedef struct _nvram_fs_para_cmptw {
    ...
    unsigned int act_write_size;  // FS_WRITE [out] parameter (get from AP side when after write the
                                  // file)
