...
int get_rsc_protol_value(int md_id, char* buf, int len) {
    ...
    if (md_id == MD_SYS1)
        retpropget = property_get("ro.vendor.mtk_md1_support", value, NULL);

    else if (md_id == MD_SYS3)
        retpropget = property_get("ro.vendor.mtk_md3_support", value, NULL);

    if (retpropget) snprintf(buf, len, "%s", value);
...
int start_service_verified(const char* service_name, const char* service_status_name,
                           int waitmsec) {
    int succeeded = -1;
    char value[PROPERTY_VALUE_MAX] = {'\0'};
...
int rild_generation(void) {
    ...
    if ((retpropget > 0) && (strcmp(value, "c6m_1rild") == 0)) ril_gen = 1;
...
int wait_for_property(const char* name, const char* desired_value, int waitmsec) {
    ...
    do {
        ...
        if (retpropget > 0) {
            ...
            // return 1; // when phone boot up, service's property is just not created yet, no need
            // to stop
        } else {
...
static int wait_decrypt_done(void) {
    ...
    /* AOSP adds a new encrypting flow, encrypting with password:
          ...
          6. After finish encrypting, the device will show the UI to let the end use decrypt the
       device (The property, vold.decrypt, is 'trigger_restart_min_framework')
    */
    ...
    while (true) {
            ...
            if (toCheckIsEncryptWithPasswd == true && isEncryptWithPasswd == false) {
                ...
                if (info.uptime - becomeEncryptedTimestamp > 10) {
                    if (!strcmp(decrypt_state, "trigger_restart_min_framework")) {
                        ...
                        CCCI_LOGD(
                                "Password is NOT default because vold.decrypt is still "
        ...
        } else if (!strcmp(crypto_state,
                           "unencrypted")) { /* fake encrypting in meta/factory mode */
            break;
        }
...
OTHER_CHECK:
    ...
    if (first_boot) ccci_ccb_init_users(md_id);
}
