...
static int need_silent_reboot =
        0; /* This varialbe will set to 1 when modem exception at boot ready state */
...
static int store_nvram_ef_data(int fid, int recsize, void* pdata) {
    ...
    CCCI_LOGD("store_nvram_ef_data fid:%d, size:%d, pdata:0x%x\n", fid, recsize,
              ((pdata == NULL) ? 0 : ((unsigned int)pdata)));
    ...
    if (pold_data != NULL)
        CCCI_LOGD("store_nvram_ef_data ret:%d, data1:%d, data2:0x%x\n", fid,
                  (!ret) ? (*pold_data) : 0, (!ret) ? (*(pold_data + 1)) : 0);
    else
    ...
    if (rec_size != write(nvram_ef_fid.iFileDesc, pdata, rec_size)) {
        CCCI_LOGE(
                "store_nvram_ef_data: Fail to write nvram file!! fid:%d, errno:0x%x, rec_size:%d, "
                "recsize:%d\n",
        ...
        if (pold_data) {
            ...
            CCCI_LOGE(
                    "store_nvram_ef_data: recovery data!! fid:%d, errno:0x%x, rec_size:%d, "
                    "recsize:%d\n",
                    fid, errno, rec_size, recsize);
        }
    }
    ...
    switch (message) {
        ...
        case CCCI_MD_MSG_EXCEPTION:
            ...
            if (mdlogger_cnt == 0) {
                ...
                start_service_verified(mdlogger_name, mdlogger_service_status,
        ...
        case CCCI_MD_MSG_RESET:
            ...
            if (!is_meta_mode()) {
                ...
                if (mdlogger_cnt == 1) {
                    ...
                    // stop_service_verified(mdlogger_name, mdlogger_service_status,
                    // MAX_WAIT_FOR_PROPERTY);
                    property_set("ctl.stop", mdlogger_name);
                    wp[wpcount].prop_name = mdlogger_service_status;
                    wp[wpcount].disr_value = "stopped";
...
static void wait_service_ready_to_reset_state_handler(int message) {
    ...
    switch (message) {
        case CCCI_MD_MSG_READY_TO_RESET:
            ...
            if (ret != 0) CCCI_LOGD("Triggert modem reset ioctl fail, after retry\n");
            //}
            /* After all services stoped, modem reset flow will trigger */
            break;
...
#define CIP_MD2_SBP_FILE "CIP_MD2_SBP"
    ...
    if (md_id == MD_SYS1) {
        ...
        snprintf(md_cip_sbp_file, sizeof(md_cip_sbp_file), "%s%s", md_img_cip_folder,
                 CIP_MD2_SBP_FILE);
    } else {
 ...
 *      if MTK_MD_SBP_CUSTOM_VALUE_=0, it means need SBP process flow, but not care ProjectConfig
 * value. CIP_MD_SBP: under /custom/etc/firmware/, It is Hexadecimai number string, ex: 0x3 Rules:
 *    wwop project use CIP_MD_SBP file, the number transform from CIP_MD_SBP MUST NOT be 0
 *        MTK_MD_SBP_CUSTOM_VALUE_ SHOULD be defined as 0x0(or 0) in ProjectConfig.mk of wwop
 * project in non-wwop project: MTK_MD_SBP_CUSTOM_VALUE_ Defined: the MTK_MD_SBP_CUSTOM_VALUE_
 * should be transfer to md MTK_MD_SBP_CUSTOM_VALUE_ Undefined: the value in MD_SBP should be
 * transfer to md The sb_code needn't transfer to md from the second boot up time, if the sb_code is
 * not changed from the first boot.
 */
static int get_md_sbp_code(int md_id) {
    ...
    if (usp_sbp_value && usp_sbp_value != sbpc) sbpc = usp_sbp_value;
    ...
    if (stored_sbp_code > 0) sbpc = stored_sbp_code;

    if (sbp_default > 0) sbpc = sbp_default;

    if (cip_sbp_value > 0) sbpc = cip_sbp_value;
...
static void cal_md_starttime(CCCI_BUFF_T buff) {
    ...
    if (buff.data[1] == CCCI_MD_MSG_BOOT_UP) {
        ...
        time(&time_HS2);
        if (snprintf(time_val, sizeof(time_val), "%lds %s", time_HS2 - time_HS1,
                     ctime(&time_HS2)) >= PROPERTY_VALUE_MAX) {
...
static int get_mdlog_boot_mode(int md_id) {
    ...
    switch (md_id) {
        case 0:
            ...
            if (fd < 0) fd = open(MDLOGGER_FILE_PATH, O_RDONLY);
            break;
        case 1:
...
static int get_md_dbg_dump_flag(int md_id) {
    ...
    if (0 != strcmp(buf, "none")) ret = strtoul(buf, NULL, 16);
    return ret;
}
...
int main_v1(int input_md_id, int sub_ver) {
    ...
    CCCI_LOGI("[%s][%s][%s][%s]\n", muxd_service_status, rild_service_status,
              mdlogger_service_status, fsd_service_status);
