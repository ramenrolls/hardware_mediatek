...
static int md_env_val_get(const char name[], char buf[], int size) {
    ...
    CCCI_LOGD("md env r\n");
    if (ccci_md_env_cfg_folder_init() < 0) return -1;
...
#define MD_CAP_MASK                                                                  \
    (MD_CAP_GSM | MD_CAP_TDS_CDMA | MD_CAP_WCDMA | MD_CAP_TDD_LTE | MD_CAP_FDD_LTE | \
     MD_CAP_CDMA2000)
#define MD_CAP_BIT_NUM (6)

static unsigned int get_capability_bit(char cap_str[]) {
    if (cap_str == NULL) return 0;
    ...
    if ((strcmp(cap_str, "W") == 0) || (strcmp(cap_str, "w") == 0)) return MD_CAP_WCDMA;
    if ((strcmp(cap_str, "C") == 0) || (strcmp(cap_str, "c") == 0)) return MD_CAP_CDMA2000;
    if ((strcmp(cap_str, "T") == 0) || (strcmp(cap_str, "t") == 0)) return MD_CAP_TDS_CDMA;
    if ((strcmp(cap_str, "G") == 0) || (strcmp(cap_str, "g") == 0)) return MD_CAP_GSM;
...
static unsigned int legacy_rat_map[] = {
        ...
        (MD_CAP_FDD_LTE | MD_CAP_TDD_LTE | MD_CAP_WCDMA | MD_CAP_CDMA2000 | MD_CAP_TDS_CDMA |
         MD_CAP_GSM),                                                   /* ulwctg */
...
static unsigned int legacy_md_support_id_to_rat(int md_support_id) {
    ...
    if ((md_support_id & MD_CAP_ENHANCE) == MD_CAP_ENHANCE) return md_support_id;
...
int get_stored_modem_type_val(int md_id) {
    ...
    if (md_id != 0) return 0; /* Only support modem 1 */
