...
static int get_partition_path(void) {
    ...
    if (nvram_emmc_support() || nvram_ufs_support()) {
        ...
        if (rec == nullptr) {
            ...
            snprintf(g_NVRAM_BACKUP_DEVICE, sizeof(g_NVRAM_BACKUP_DEVICE), "%s",
        ...
        if (rec == nullptr) {
            ...
            snprintf(g_NVRAM_NVDATA_DEVICE, sizeof(g_NVRAM_NVDATA_DEVICE), "%s",
                     rec->blk_device.c_str());
        }
        ...
        if (rec == nullptr) {
            ...
            snprintf(g_NVRAM_PROINFO_DEVICE, sizeof(g_NVRAM_PROINFO_DEVICE), "%s",
                     rec->blk_device.c_str());
        }
    } else {
        memset(g_LOG_DEVICE, 0, sizeof(g_LOG_DEVICE));
...

{
    ...
    if (nvram_platform_layout_version == 0)
        BinRegionBlockTotalNum =
                info.size / iBlockSize - nvram_platform_log_block - nvram_platform_resv_block;
    ...
    iResult = lseek(fd, pos * iBlockSize + 3 * sizeof(unsigned int) + iMapFileSize, SEEK_SET);
...
int NVM_Init(void) {
    ...
    NVRAM_LOG(
            "libfile nvram_platform_log_block "
            "=%d,nvram_platform_resv_block=%d,nvram_platform_DM_block=%d "
            "nvram_platform_layout_version##=%d,nvram_platform_header_offset=%d,g_i4CFG_File_Count="
            "%d\n",
...
int NVM_GetLIDByName(char* filename) {
    ...
    if (gFileNameArray == NULL) {
        ...
        for (i = 0; i < g_i4CFG_File_Count; i++) {
            ...
            snprintf(gFileNameArray[i].cFileName, sizeof(g_akCFG_File[i].cFileName), "%s",
                     g_akCFG_File[i].cFileName);
        ...
        for (i = g_i4CFG_File_Count; i < g_i4CFG_File_Count + g_i4CFG_File_Custom_Count; i++) {
            // strcpy(gFileNameArray[i].cFileName,
            // g_akCFG_File_Custom[i-iCustomBeginLID].cFileName);
            snprintf(gFileNameArray[i].cFileName, sizeof(gFileNameArray[i].cFileName), "%s",
                     g_akCFG_File_Custom[i - iCustomBeginLID].cFileName);
            gFileNameArray[i].iLID = i;
...
bool NVM_GenerateFileVer(bool CPY_File_To_NVM) {
    ...
    if (iFileDesc == -1) {
        ...
        iFileDesc = open_file_with_dirs(g_akCFG_File[iFileVerInfoLID].cFileName, O_CREAT | O_RDWR,
                                        S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH);
    }
    ...
    for (iFilNO = iCustomBeginLID; iFilNO < (g_i4CFG_File_Count + g_i4CFG_File_Custom_Count);
         iFilNO++) {
        int i;
        ...
        for (i = iStrLen; i >= 0; i--) {
            if (g_akCFG_File_Custom[iFilNO - iCustomBeginLID].cFileName[i] == '/') {
                strncpy(tempstr, g_akCFG_File_Custom[iFilNO - iCustomBeginLID].cFileName + i + 1,
                        sizeof(tempstr) - 1);
...
bool NVM_UpdateFileVerNo(int file_lid, VerInfoUpdateFlag UpdateFlag) {
    ...
    if (UpdateFlag == VerUpdate) {
        ...
        for (iIndex = iStrLen; iIndex >= 0; iIndex--) {
            if (pCfgFileTable[iRealFileLid].cFileName[iIndex] == '/') {
                // strcpy( tempstr1, pCfgFileTable[iRealFileLid].cFileName + iIndex + 1 );//get the
                // target name
                snprintf(tempstr1, sizeof(tempstr1), "%s",
                         pCfgFileTable[iRealFileLid].cFileName + iIndex + 1);
                break;
            }
        }
        ...
        for (iIndex = 0; iIndex < ilooptime; iIndex++) {
            ...
            if (iResult == -1) {
                NVRAM_LOG("pread failed with errno=%d\n", errno);
                iResult = 0;
            }
            tempstr[iResult] = '\0';
    ...
    if (UpdateFlag == VerUpdate) {
                ...
                for (i = iStrLen; i >= 0; i--) {
                    if (g_akCFG_File[file_lid].cFileName[i] == '/') {
                        ...
                        snprintf(tempstr, sizeof(tempstr), "%s",
                                 g_akCFG_File[file_lid].cFileName + i + 1);
                        break;
                    }
                }
            ...
            if (bIsNewVerInfo) {
                ...
                for (i = iStrLen; i >= 0; i--) {
                    if (g_akCFG_File_Custom[file_lid - iCustomBeginLID].cFileName[i] == '/') {
                        ...
                        snprintf(tempstr, sizeof(tempstr), "%s",
                                 g_akCFG_File_Custom[file_lid - iCustomBeginLID].cFileName + i + 1);
                        break;
                    }
                }
                pwrite(iFileDesc, tempstr, FILENAMELENGTH, offset);
        ...
        if (!bIsNewVerInfo) {
            ...
            iResult = pwrite(iFileDesc, buffer2, iFileSize - FILEVERLENGTH - FILENAMELENGTH, 0);
                ...
                if (bIsNewVerInfo) {
                    ...
                    for (i = iStrLen; i >= 0; i--) {
                        if (g_akCFG_File_Custom[file_lid - iCustomBeginLID].cFileName[i] == '/') {
                            // strcpy(tempstr,g_akCFG_File_Custom[file_lid-iCustomBeginLID].cFileName+i+1);
                            snprintf(tempstr, sizeof(tempstr), "%s",
                                     g_akCFG_File_Custom[file_lid - iCustomBeginLID].cFileName + i +
                                             1);
                            break;
                        }
                    }
...
static unsigned int NVM_ComputeCheckSum(void) {
    ...
    NVBAK_LOG("Starting NVM_ComputeCheckSum (pid %d) on %s", getpid(),
              ctime(&start));  ////add for log
    int iFileDesc_file;
    ...
    // NVBAK_LOG("Start_while NVM_ComputeCheckSum (pid %d) on
    // %s",getpid(),ctime(&start_while));//add
    for (i = 0; i < tmplooptime; i++) {
        ...
        // NVBAK_LOG("Start_while: i is %d, NVM_ComputeCheckSum (pid %d) on
        // %s",i,getpid(),ctime(&start_while));// add for log
        iResult = read(iFileDesc_file, tempBuf, jloop_num * iLength);
        // NVBAK_LOG("Start_while: iResult is %d, NVM_ComputeCheckSum (pid %d) on
        // %s",iResult,getpid(),ctime(&start_while));//add for log
        if (iResult != jloop_num * iLength) {
            NVBAK_LOG("NVM_ComputeCheckSum cannot read checksum data\n");
            close(iFileDesc_file);
            return 0;
        }
        // NVBAK_LOG("Start_while:before_while NVM_ComputeCheckSum (pid %d) on
        // %s",getpid(),ctime(&start_while));//add for log
...
static bool NVM_CompareCheckSum(unsigned int ulCheckSum1, unsigned int ulCheckSum2) {
    if (ulCheckSum1 != ulCheckSum2) return false;
    return true;
}
bool NVM_RestoreFromFiles_OneFile(int eBackupType, int file_lid, const char* filename,
                                  bool* find_flag) {
    ...
    int fhs = sizeof(unsigned int) + 2 * sizeof(unsigned int) + sizeof(File_Title_Header1);
    ...
    } else if (nvram_md5_support() && !nvram_evdo_support() && !nvram_ecci_c2k_support()) {
        NVRAM_LOG("NVM_RestoreFromFiles_OneFile: File_Title_Header2 set\n");
        fhs = sizeof(unsigned int) + 2 * sizeof(unsigned int) + sizeof(File_Title_Header2) -
              2 * sizeof(short int);  // In order to align, minus 2*sizeof(short int)
    } else if (!nvram_md5_support() && nvram_evdo_support() && !nvram_ecci_c2k_support()) {
        NVRAM_LOG("NVM_RestoreFromFiles_OneFile: File_Title_Header3 set\n");
        fhs = sizeof(unsigned int) + 2 * sizeof(unsigned int) + sizeof(File_Title_Header3) -
              4 * sizeof(short int);  // In order to align, minus 4*sizeof(short int)
    } else if (!nvram_md5_support() && !nvram_evdo_support() && !nvram_ecci_c2k_support()) {
        NVRAM_LOG("NVM_RestoreFromFiles_OneFile: File_Title_Header4 set\n");
        fhs = sizeof(unsigned int) + 2 * sizeof(unsigned int) + sizeof(File_Title_Header4) -
              4 * sizeof(short int);
    } else {
        NVRAM_LOG("NVM_RestoreFromFiles_OneFile: File_Title_Header5 set\n");
        fhs = sizeof(unsigned int) + 2 * sizeof(unsigned int) + sizeof(File_Title_Header5);
    }
    ...
    if (INVALID_HANDLE_VALUE == iFileDesc_file) {
        ...
        free(FileInfo);
        if (iFileDesc_map > 0) close(iFileDesc_map);
        return false;
    }
    ...
    if (nvram_md5_support() && nvram_evdo_support() && !nvram_ecci_c2k_support())
        ...
        iSize = (int)read(
                iFileDesc_map, &FileTitleInfo2,
                sizeof(File_Title_Header2) -
                        2 * sizeof(short int));  // In order to align, minus 2*sizeof(short int)

    else if (!nvram_md5_support() && nvram_evdo_support() && !nvram_ecci_c2k_support())
        iSize = (int)read(
                iFileDesc_map, &FileTitleInfo3,
                sizeof(File_Title_Header3) -
                        4 * sizeof(short int));  // In order to align, minus 4*sizeof(short int)
    ...
    switch (eBackupType) {
        ...
        case MDBOOT:
            if (nvram_md5_support() && nvram_evdo_support() && !nvram_ecci_c2k_support()) {
                ...
                iFileTitleOffset =
                        fhs + (FileTitleInfo1.iApBootNum + FileTitleInfo1.iApCleanNum) * fis;
            } else if (nvram_md5_support() && !nvram_evdo_support() && !nvram_ecci_c2k_support()) {
                NVBAK_LOG("NVM_RestoreFromFiles_OneFile MDBOOT start !");
                iFileNum = FileTitleInfo2.iMdBootNum;
                iFileTitleOffset =
                        fhs + (FileTitleInfo2.iApBootNum + FileTitleInfo2.iApCleanNum) * fis;
            } else if (!nvram_md5_support() && nvram_evdo_support() && !nvram_ecci_c2k_support()) {
                NVBAK_LOG("NVM_RestoreFromFiles_OneFile MDBOOT start !");
                iFileNum = FileTitleInfo3.iMdBootNum;
                iFileTitleOffset =
                        fhs + (FileTitleInfo3.iApBootNum + FileTitleInfo3.iApCleanNum) * fis;
            } else if (!nvram_md5_support() && !nvram_evdo_support() && !nvram_ecci_c2k_support()) {
                NVBAK_LOG("NVM_RestoreFromFiles_OneFile MDBOOT start !");
                iFileNum = FileTitleInfo4.iMdBootNum;
                iFileTitleOffset =
                        fhs + (FileTitleInfo4.iApBootNum + FileTitleInfo4.iApCleanNum) * fis;
            } else {
                NVBAK_LOG("NVM_RestoreFromFiles_OneFile MDBOOT start !");
                iFileNum = FileTitleInfo5.iMdBootNum;
                iFileTitleOffset =
                        fhs + (FileTitleInfo5.iApBootNum + FileTitleInfo5.iApCleanNum) * fis;
            }
        ...
        case MD2BOOT:
            ...
            if (nvram_platform_header_offset == 0) {
                if (nvram_md5_support() && nvram_evdo_support() && !nvram_ecci_c2k_support()) {
                    ...
                    iFileTitleOffset =
                            fhs + (FileTitleInfo1.iApBootNum + FileTitleInfo1.iApCleanNum +
                                   FileTitleInfo1.iMdBootNum + FileTitleInfo1.iMdCleanNum +
                                   FileTitleInfo1.iMdImpntNum + FileTitleInfo1.iMdCoreNum +
                                   FileTitleInfo1.iMdDataNum) *
                                          fis;
                    ...
                    iFileTitleOffset =
                            fhs + (FileTitleInfo2.iApBootNum + FileTitleInfo2.iApCleanNum +
                                   FileTitleInfo2.iMdBootNum + FileTitleInfo2.iMdCleanNum +
                                   FileTitleInfo2.iMdImpntNum + FileTitleInfo2.iMdCoreNum +
                                   FileTitleInfo2.iMdDataNum) *
                                          fis;
                } else if (!nvram_md5_support() && nvram_evdo_support() &&
                           !nvram_ecci_c2k_support()) {
                    NVBAK_LOG("FileOp_RestoreFromFiles MD2BOOT start !");
                    ...
                    iFileTitleOffset =
                            fhs + (FileTitleInfo3.iApBootNum + FileTitleInfo3.iApCleanNum +
                                   FileTitleInfo3.iMdBootNum + FileTitleInfo3.iMdCleanNum +
                                   FileTitleInfo3.iMdImpntNum + FileTitleInfo3.iMdCoreNum +
                                   FileTitleInfo3.iMdDataNum) *
                                          fis;
                }
                    ...
                    iFileTitleOffset =
                            fhs + (FileTitleInfo4.iApBootNum + FileTitleInfo4.iApCleanNum +
                                   FileTitleInfo4.iMdBootNum + FileTitleInfo4.iMdCleanNum +
                                   FileTitleInfo4.iMdImpntNum + FileTitleInfo4.iMdCoreNum +
                                   FileTitleInfo4.iMdDataNum) *
                                          fis;
                } else {
                    NVBAK_LOG("FileOp_RestoreFromFiles MD2BOOT start !");
                    iFileNum = FileTitleInfo5.iMd2BootNum;
                    iFileTitleOffset =
                            fhs + (FileTitleInfo5.iApBootNum + FileTitleInfo5.iApCleanNum +
                                   FileTitleInfo5.iMdBootNum + FileTitleInfo5.iMdCleanNum +
                                   FileTitleInfo5.iMdImpntNum + FileTitleInfo5.iMdCoreNum +
                                   FileTitleInfo5.iMdDataNum) *
                                          fis;
                }
            }
            break;
        ...
        case MD5IMP:
                ...
                if (nvram_evdo_support()) {
                    ...
                    iFileTitleOffset =
                            fhs + (FileTitleInfo2.iApBootNum + FileTitleInfo2.iApCleanNum +
                                   ...
                                   FileTitleInfo2.iMdDataNum + FileTitleInfo2.iMd2BootNum +
                                   FileTitleInfo2.iMd2CleanNum + FileTitleInfo2.iMd2ImpntNum +
                                   FileTitleInfo2.iMd5BootNum + FileTitleInfo2.iMd5CleanNum) *
                                          fis;
                }
            } else if (nvram_platform_header_offset == 0 && nvram_md5_support() &&
                       nvram_ecci_c2k_support()) {
            ...
            break;
        case VIA:
            if (nvram_platform_header_offset == 0 && nvram_md5_support() && nvram_evdo_support() &&
                !nvram_ecci_c2k_support()) {
                ...
                iFileTitleOffset =
                        fhs + (FileTitleInfo3.iApBootNum + FileTitleInfo3.iApCleanNum +
                               ...
                               FileTitleInfo3.iMdImpntNum + FileTitleInfo3.iMdCoreNum +
                               FileTitleInfo3.iMdDataNum + FileTitleInfo3.iMd2BootNum +
                               FileTitleInfo3.iMd2CleanNum + FileTitleInfo3.iMd2ImpntNum) *
                                      fis;
    ...
    while (iFileNum > 0 && iFileNum < MAX_FILENUM) {
        ...
        if ((filename == NULL) && (file_lid >= 0) && (pCfgFielTable != NULL)) {
            ...
            if (strcmp(FileInfo->cFileName + 11, filename + 7) !=
                0) {  // compare string after /mnt/vendor/nvdata and /nvdata
                --iFileNum;
                continue;
            }
...
bool NVM_CheckData_OneFile(int file_lid, const char* filename) {
    ...
    for (eBackupType = APBOOT; eBackupType < ALL; eBackupType++) {
        if (iFileMask[eBackupType] == 1) continue;
        if (find_flag == 0)
...
bool NVM_RestoreFromBinRegion_OneFile(int file_lid, const char* filename) {
    ...
    unsigned int iMapFileSize, iDatFileSize, iBlockSize, iPartitionSize, iAPBackupFileNum,
            iWriteSize;
    ...
    if ((iMapFileSize + 3 * sizeof(unsigned int)) % iWriteSize != 0)
        iMemSize = (((iMapFileSize + 3 * sizeof(unsigned int)) / iWriteSize) + 1) * iWriteSize;
    else
        iMemSize = iMapFileSize +
                   3 * sizeof(unsigned int);  // mapfile size, datfile size, cleanboot fla
    ...
    free(tempBuffer);
    tempBuffer = NULL;
    tempBuffer = (char*)malloc(iBlockSize * iReserveBlockCount);  // for 128KB and 256KB
    if (tempBuffer == NULL) {
        NVBAK_LOG("[NVRAM]:memory malloc error\r\n");
        close(fd);
        return false;
    ...
    iDatFileSize = *((unsigned int*)(tempBuffer + 4));
    iAPBackupFileNum = *((unsigned int*)(tempBuffer + 3 * sizeof(unsigned int) + iMapFileSize));
    iMDBackupFileNum = *((unsigned short*)(tempBuffer + 3 * sizeof(unsigned int) + iMapFileSize +
                                           sizeof(unsigned int)));
    NVBAK_LOG("[NVRAM]: map file:%d,dat file:%d\n", iMapFileSize, iDatFileSize);
    /* NVBAK_LOG("[NVRAM]: AP backup file num:%d,MD backup file
    num:%d\n",iAPBackupFileNum,iMDBackupFileNum); if(iAPBackupFileNum != 0 ){ iFileDesc_backupflag =
    open(g_pcNVM_BackFlag , O_TRUNC|O_CREAT|O_RDWR, S_IRUSR|S_IWUSR| S_IRGRP |S_IWGRP);
    ...
    iFileDesc_file = open(g_pcNVM_AllFile, O_TRUNC | O_CREAT | O_RDWR,
                          S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP);
    iFileDesc_map =
            open(g_pcNVM_AllMap, O_TRUNC | O_CREAT | O_RDWR, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP);

    if (INVALID_HANDLE_VALUE == iFileDesc_file) {
        NVBAK_LOG("[NVRAM]: cannot open file data\n");
        if (iFileDesc_map != INVALID_HANDLE_VALUE) close(iFileDesc_map);
        close(fd);
        free(tempBuffer);
        free(tempBitmap1);
        free(tempBitmap2);
    ...
    int iFreeBlockNum = 0;
    for (i = pos + 1 + iReservePos; i < iBlockNum; i++) {  // heli if > 126KB need pos + 2
        if (tempBitmap1[i] == 0) {
            pos = i;
            break;
        }
    ...
    if (i == iBlockNum) {
        ...
        for (; i < iBlockNum; i++) {
            if (tempBitmap1[i] == 0) iFreeBlockNum++;
        }
    ...
    for (i = 0; i < iReadTime; i++) {
        ...
        iAlreadyRead++;
        if (iAlreadyRead == iReadTime) break;
...
int NVM_CheckFileSize(int iRealFileLid, const TCFG_FILE* pCfgFielTable) {
    ...
    switch (pCfgFielTable[iRealFileLid].stDefualType) {
        ...
        case SIGNLE_DEFUALT_REC:
            if (((long long)pCfgFielTable[iRealFileLid].i4RecNum) *
        ...
        case MULTIPLE_DEFUALT_REC:  // when the type of default value is MULTIPLE_DEFUALT_REC, we
                                    // use the defined default value of one record to generate file
            if (((long long)pCfgFielTable[iRealFileLid].i4RecNum) *
    ...
    for (j = strlen(pCfgFielTable[iRealFileLid].cFileName) - 1; j >= 0; j--)
        if (pCfgFielTable[iRealFileLid].cFileName[j] == '/') {
            ...
            snprintf(filename, sizeof(filename), "%s",
                     (char*)(pCfgFielTable[iRealFileLid].cFileName + j + 1));
...
bool NVM_ResetFileToDefault(int file_lid) {
    ...
    switch (pCfgFielTable[iRealFileLid].stDefualType) {
        case DEFAULT_ZERO:
        case DEFAULT_FF:  // when the type of default value is DEFAULT_ZERO/DEFAULT_FF, we don't
                          // care the default vlaue
            iFileDesc = open(pCfgFielTable[iRealFileLid].cFileName, O_CREAT | O_TRUNC | O_RDWR,
                             S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP);

            if (iFileDesc == -1) {
                iFileDesc =
                        open_file_with_dirs(pCfgFielTable[iRealFileLid].cFileName, O_CREAT | O_RDWR,
                                            S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP);
        ...
        case SIGNLE_DEFUALT_REC:  // when the type of default value is SIGNLE_DEFUALT_REC, we use
                                  // the defined default vlaue to generate file
            iFileDesc = open(pCfgFielTable[iRealFileLid].cFileName, O_CREAT | O_TRUNC | O_RDWR,
                             S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP);
...
bool NVM_CmpFileVerNo(int file_lid) {
    ...
    // get the file informatin table. if the file lid is version file, just generate it directly by
    // call GenerateFileVer
    pCfgFielTable = NVM_GetCfgFileTable(file_lid);
    if (pCfgFielTable == NULL) {
        NVRAM_LOG("NVM_GetCfgFileTable Fail!!!\n");
        return false;
    ...
    for (i = iStrLen; i >= 0; i--) {
        if (pCfgFielTable[iRealFileLid].cFileName[i] == '/') {
            ...
            snprintf(tempstr1, sizeof(tempstr1), "%s",
                     pCfgFielTable[iRealFileLid].cFileName + i + 1);
            break;
        }
    }
    if (i < 0) {
    ...
    if (iResult == -1) {
        NVRAM_LOG("read failed with errno=%d\n", errno);
        iResult = 0;
    }
    tempstr[iResult] = '\0';
    if (0 == strcmp(tempstr, strVerInfo)) {
    ...
    if (bIsNewVerInfo) {
        ...
        if (0 == strcmp(tempstr, tempstr1)) {
            if (-1 == lseek(iFileDesc,
                            (file_lid + 1) * (FILEVERLENGTH + FILENAMELENGTH) + FILENAMELENGTH,
                            SEEK_SET)) {
                NVRAM_LOG("Error NVM_CmpFileVerNo lseek version file fail: %s\n",
                          g_akCFG_File[iFileVerInfoLID].cFileName);
                close(iFileDesc);
            ...
            for (i = 0; i < ilooptime; i++) {
                ...
                if (0 == strcmp(tempstr, tempstr1)) {
                    memset(tempstr, 0, FILENAMELENGTH);
                    if (-1 == lseek(iFileDesc,
                                    i * (FILEVERLENGTH + FILENAMELENGTH) + FILENAMELENGTH,
                                    SEEK_SET)) {
                        NVRAM_LOG("Error NVM_CmpFileVerNo lseek version file fail: %s\n",
                                  g_akCFG_File[iFileVerInfoLID].cFileName);
                        close(iFileDesc);
                    ...
                    read(iFileDesc, cFbyte, FILEVERLENGTH);
                    NVRAM_LOG(
                            "FILEVER size change, Load File Version: %s, NvRam File Version: %s\n",
    ...
    for (i = 0; i < 4; i++) {
        if (pCfgFielTable[iRealFileLid].cFileVer[i] != cFbyte[i]) {
            NVRAM_LOG("Error NVM_CmpFileVerNo is not same %d, %d ,%d\n", iRealFileLid,
                      pCfgFielTable[iRealFileLid].cFileVer[i], cFbyte[i]);
            return false;
        }
    }
...
int NVM_DataVerConvert(int file_lid) {
    ...
    if (bIsNewVerInfo) {
        if (((file_lid + 1 + 1) * (FILEVERLENGTH + FILENAMELENGTH)) > st.st_size) {
            NVRAM_LOG(
                    "NVM_DataVerConvert:the file lid is greater than the max file lid, reset it\n");
            if (!NVM_ResetFileToDefault(file_lid)) {
    ...
    */
    if (pCfgFielTable[iRealFileLid].NVM_DataConvertFunc != NULL) {
        iResult = pCfgFielTable[iRealFileLid].NVM_DataConvertFunc(iCurrentFileVerNO, iNewFileVerNO,
                                                                  pSrcMem, pDstMem);
        if (iResult != 1) {
            NVRAM_LOG("Error NVM_DataVerConvert fail \n");
            free(pSrcMem);
...
static bool NVM_CheckFileNum(unsigned int iOldCommonFileNum, unsigned int iOldCustomFileNum) {
    ...
    if ((iOldCommonFileNum == 0) || (iOldCustomFileNum == 0)) {
        NVRAM_LOG(
                "File Num is zero, maybe this nvram data is from 32mp,48mpv0 or SD update\n");  // support for 32mp and 48mpv0
        return true;
    }
...
bool NVM_DataVerConvertAll(unsigned int iOldCommonFileNum, unsigned int iOldCustomFileNum) {
    ...
    if (bWrongFileVer == true) return false;
    return true;
}
/********************************************************************************
//FUNCTION:
...
bool NVM_CheckFileID(int file_lid, int rec_id) {
    ...
    if ((rec_id > pCfgFielTable[iRealFileLid].i4RecNum) || (file_lid >= g_i4MaxNvRamLid)) {
        return false;
    }
...
F_INFO NVM_ReadFileVerInfo(int file_lid) {
    ...
    if (NVM_CheckVerFile(true)) {
        ...
        if (!NVM_CmpFileVerNo(file_lid)) {
            ...
            if (pCfgFielTable[iRealFileLid].bDataProcessingType == DataReset) {  // Reset Data
                ...
                close(iFileDesc);
                if (!NVM_ResetFileToDefault(file_lid)) return file_info;
            } else {
    ...
    memcpy(file_info.cFileName, pCfgFielTable[iRealFileLid].cFileName, FILENAMELENGTH);
...
F_ID NVM_GetFileDesc(int file_lid, int* pRecSize, int* pRecNum, bool IsRead) {
    ...
    NVRAM_LOG("%s: Open %s,LID:%d\n", __func__, pCfgFielTable[iRealFileLid].cFileName, file_lid);
    ...
    if (iFileDesc == -1) {  // if file doesn't exist
        ...
        NVRAM_LOG("Create the dir path of %s\n", pCfgFielTable[iRealFileLid].cFileName);
        iFileDesc = open_file_with_dirs(pCfgFielTable[iRealFileLid].cFileName, O_CREAT | O_RDWR,
                                        S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP);  // create the file
        if (iFileDesc == -1) {
        ...
        if (!NVM_CmpFileVerNo(file_lid)) {
            ...
            if (pCfgFielTable[iRealFileLid].bDataProcessingType == DataReset) {  // Reset Data
                ...
                close(iFileDesc);
                if (!NVM_ResetFileToDefault(file_lid)) return FileID;
            } else {
                NVRAM_LOG("NVM_ReadFileVerInfo NVM_CheckFileInfo Fail: Convert!!! \n");
                // only for test
                iResult = NVM_DataVerConvert(file_lid);
...
Solve_Special_Lid:
    ...
    if (g_new_nvram_lid[index_in_list].start_address % info.erasesize != 0 ||
        g_new_nvram_lid[index_in_list].start_address + g_new_nvram_lid[index_in_list].size >
                (long)info.size) {
    ...
    if (*pRecSize % info.writesize != 0) {
        NVRAM_LOG(
                "Please make sure size for special lid in new nvram partition should alignment "
                "%d\n",
                info.writesize);
        return FileID;
    }
...
// add by min
bool NVM_HistoryLog_add_file(const char* fix_dir, const char* pre_fix_file,
                             nvram_journal_handle_t* journal) {
    DIR* dir;
    struct dirent* entry;
    struct stat st;
    ...
    while (1) {
        ...
        if (strncmp(entry->d_name, pre_fix_file, strlen(pre_fix_file)) == 0) {
            if (num >= 3) break;  // total 3 barcode file
            ...
            if (120 == read_size) {
                memcpy(journal->imei, entry->d_name,
                       MAX_IMEI_LENGTH);  // add by min, copy IMEI_FLAG to joural
                memcpy(journal->imei + strlen(entry->d_name), buffer,
                       size);  // add by min, copy imei to joural
            } else if (66 == read_size) {
                memcpy(journal->barcode + num * (size + strlen(entry->d_name)), entry->d_name,
                       strlen(entry->d_name));  // add by min, copy BARCODE_FLAG to joural
                memcpy(journal->barcode + num * (size + strlen(entry->d_name)) +
                               strlen(entry->d_name),
                       buffer, size);  // add by min, copy barcode to joural
            } else {
...
bool NVM_HistoryLog_Time(unsigned int level, const char* func, unsigned int line, const char* log,
    ...
    if (level < DEFAULT_LOG_LEVEL) return true;
    ...
    start_address = (loff_t)((loff_t)info.size -
                             ((loff_t)nvram_platform_log_block * (loff_t)info.erasesize));
    offset = (loff_t)((loff_t)info.writesize *
                      ((loff_t)sequence %
                       ((loff_t)writepages_per_block * ((loff_t)nvram_platform_log_block - 1))));
...
bool NVM_HistoryLog(unsigned int level, const char* func, unsigned int line, const char* log) {
    ...
    start_address = info.size - ((loff_t)nvram_platform_log_block * (loff_t)info.erasesize);
    offset = (loff_t)((loff_t)info.writesize *
                      ((loff_t)sequence %
                       ((loff_t)writepages_per_block * (loff_t)(nvram_platform_log_block - 1))));
...
unsigned int NVM_MiscGetSeqNum(void) {
    ...
    offset = (loff_t)((loff_t)info.size -
                      (loff_t)info.erasesize * (loff_t)nvram_misc_log_block_offset);
...
bool NVM_MiscLog(unsigned int level, const char* func, unsigned int line, const char* log) {
    ...
    start_address = info.size - (loff_t)info.erasesize * (loff_t)nvram_misc_log_block_offset;
    // sequence number always in first write unit
    offset = (loff_t)((loff_t)info.writesize *
                      ((loff_t)sequence % (loff_t)writepages_per_block + 1));
...
bool Check_UpdateStatus(void) {
    ...
    if (-1 == val) {
        ...
        //------KK->L/KK.AOSP->L
        if ((info.st_gid != AID_SYSTEM) ||
            (ret >= 0 && statbuf.st_uid != AID_RADIO && statbuf.st_uid != AID_SYSTEM &&
             statbuf.st_uid != AID_ROOT) ||
            (ret >= 0 && statbuf.st_gid != AID_RADIO && statbuf.st_gid != AID_SYSTEM)) {
        ...
        if (vre >= 0 && stb.st_gid != AID_BLUETOOTH) {
            ...
            if (chmod("/mnt/vendor/nvdata/APCFG", 0771) ||
                ...
                NVRAM_LOG(
                        "Fail to change BT folder permission when upgrade for "
                        "/mnt/vendor/nvdata\n");
                return false;
            }
        }
