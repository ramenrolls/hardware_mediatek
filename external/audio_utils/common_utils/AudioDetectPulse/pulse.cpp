...
static const char* TagString[] = {"CaptureDataProvider",
                                  "FastCapture",
                                  "NormalCapture",
                                  "AudioRecord",
                                  "AudioTrack",
...
const char* Tag2String(const int TagNum) { return TagString[TagNum]; }

void detectPulse_(const int TagNum, const int pulseLevel, short* ptr, const size_t desiredFrames,
    ...
    for (int i = 0; i < precision; i++) {
        ...
        nsecs_t time = frameTimeUs - (int64_t)((desiredFrames - baseFrames - count) * 1000000 /
                                               (int64_t)sampleRate);
        if (sum >= pulseLevel) {
            ALOGD("TagNum %d - %s, sum %d, time %1.3f, detect pulse", TagNum, Tag2String(TagNum),
                  sum, (double)time * 1e-3);
        } else {
            ALOGD("TagNum %d - %s, sum %d, time %1.3f", TagNum, Tag2String(TagNum), sum,
                  (double)time * 1e-3);
        }
...
void detectPulse(const int TagNum, const int pulseLevel, const int dump, void* ptr,
    ...
    //             __FUNCTION__, TagNum, pulseLevel, (int)ptr, format, (int)desiredFrames,
    //             channels);
    ...
    if (desiredFrames > MAX_FRAMECOUNT || desiredFrames <= 0) {
        ALOGE("%s, %s, frames(%d) is not support!!", __FUNCTION__, Tag2String(TagNum),
              (int)desiredFrames);
        return;
    }
    if (ptr == NULL) {
    ...
    memcpy_by_audio_format(SaveBuffer[TagNum], AUDIO_FORMAT_PCM_16_BIT, ptr, format,
                           desiredFrames * channels);

    if (dump) {
        // dump pcm
        char fileName[80] = {0};
        sprintf(fileName, "%s.%s.pcm", "/sdcard/mtklog/audio_dump/detectPulse_16bit_Tag",
                Tag2String(TagNum));
        dumpPCMData(fileName, SaveBuffer[TagNum],
                    desiredFrames * channels * audio_bytes_per_sample(AUDIO_FORMAT_PCM_16_BIT));
    }

    detectPulse_(TagNum, pulseLevel, SaveBuffer[TagNum], desiredFrames, channels, sampleRate,
                 precision);
}

#else
void detectPulse(__attribute__((unused)) const int TagNum,
                 __attribute__((unused)) const int pulseLevel,
                 __attribute__((unused)) const int dump, __attribute__((unused)) void* ptr,
                 __attribute__((unused)) const size_t desiredFrames,
                 __attribute__((unused)) const audio_format_t format,
                 __attribute__((unused)) const int channels,
                 __attribute__((unused)) const int sampleRate,
