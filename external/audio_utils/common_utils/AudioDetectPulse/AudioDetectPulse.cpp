...
void AudioDetectPulse::doDetectPulse(const int TagNum, const int pulseLevel, const int dump,
                                     void* ptr, const size_t desiredBufferSize,
                                     const audio_format_t format, const int channels,
    ...
    if (getDetectPulse()) {
        ...
        size_t desiredFrames =
                desiredBufferSize / channels / ((format == AUDIO_FORMAT_PCM_16_BIT) ? 2 : 4);

        if (mPrecision < 1 || mPrecision > (int)desiredFrames) {
            ALOGW("%s, precision (%d) is not support! Use default value (%d)", __FUNCTION__,
                  mPrecision, defaultPrecision);
            mPrecision = defaultPrecision;
        }
        detectPulse(TagNum, pulseLevel, dump, ptr, desiredFrames, format, channels, sampleRate,
                    mPrecision);
    }
}
