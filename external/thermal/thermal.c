...
static char IFC_NAME[IFC_NUM][17] = {"wlan0", "ap0", "p2p0"};

#if defined(MD_UL_DR_THROTTLE)
static char MD_IFC_NAME[MD_IFC_NUM][17] = {"ccemni0", "ccemni1", "ccemni2", "ccmni"};
#endif

static char IFC_PATH[IFC_NUM][50] = {WLAN_IFC_PATH, AP_IFC_PATH, P2P_IFC_PATH};

#if defined(MD_UL_DR_THROTTLE)
static char MD_IFC_PATH[MD_IFC_NUM][50] = {CCEMNI0_IFC_PATH, CCEMNI1_IFC_PATH, CCEMNI2_IFC_PATH,
                                           CCMNI_IFC_PATH};
#endif
...
int RilRPC_send(int v1, int v2) {
    ...
    if (sendRpcRequest) ret = sendRpcRequest(v1, v2);
...
static void exe_cmd(int wifi_ifc, int level) {
    if (0 == access(THROTTLE_SCRIPT_PATH, R_OK | X_OK) && wifi_ifc >= 0) {
        ...
        sprintf(cmd, "%s %s %d %d", THROTTLE_SCRIPT_PATH, IFC_NAME[wifi_ifc],
                level * ONE_MBITS_PER_SEC, level * ONE_MBITS_PER_SEC);
...
static void set_wifi_throttle(int level) {
    ...
    for (i = 0; i < IFC_NUM; i++) {
        ...
        if (0 == access(IFC_PATH[i], R_OK)) {
            ...
            close(fd);
            if (!strncmp(buf, "up", 2)) {
                // ifc_set_throttle(IFC_NAME[i], level * ONE_MBITS_PER_SEC, level *
                // ONE_MBITS_PER_SEC); mark for CTS fail ifc_set_throttle(IFC_NAME[i], level,
                // level);    //[star] unit: Kbytes
...
#endif
static void set_md_dul_throttle(
        int level)  // level 1~10, 1: unlimit for 1s and zero for 9s, ..., 10: unlimit
{
...
static int RildConnect(void) {
    ...
    if (sock < 0) TM_DBG_LOG("RildConnect %s\n", strerror(errno));
        ...
        if (0 < ret) {
            if (strncmp("ERROR", (const char*)strParm, 5) != 0) {
                    ...
                    if (tok == 2) {
                        ...
                        if (mdinfoex_idx > -1) {
                            ...
                            if (fd >= 0) {
                                ...
                                ret = write(fd, mdinfo_string,
                                            sizeof(char) * strlen(mdinfo_string));
                                if (ret <= 0) {
                                    TM_DBG_LOG("Fail to write %s to %s %x\n", mdinfo_string,
                                               "/proc/driver/thermal/mdm_mdinfoex", ret);
                                } else {
                                    TM_DBG_LOG("Success to write %s to %s\n", mdinfo_string,
                                               "/proc/driver/thermal/mdm_mdinfoex");
                                }
                                close(fd);
                            }
                        }
                ...
                if (opcode == -1) {
                    ...
                    int tok;
                    tok = sscanf((const char*)strParm, "%d,%d,%d,%d,%d,%d", &md, &temp, &tx,
                                 &opval0, &opval1, &opval2);
                    if (tok == 2)  // tx not exist
                    {
                        TM_DBG_LOG(
                                "[%s] tok=%d md=%d temp=%d tx=%d opval0=%d opval1=%d opval2=%d\n",
                                strParm, tok, md, temp, tx, opval0, opval1, opval2);
                        tok = sscanf((const char*)strParm, "%d,%d,,%d,%d,%d", &md, &temp, &opval0,
                                     &opval1, &opval2);
                    }
                    ...
                    // write to proc
                    {
                        ...
                        char mdinfo_string[SOCKET_RECV_LEN] = {0};
                        if (fd >= 0) {
                            sprintf(mdinfo_string, "%d,%d,%d,%d", slotId, md, temp,
                                    ((tok > 2) ? tx : -127));
                            ret = write(fd, mdinfo_string, sizeof(char) * strlen(mdinfo_string));
                            if (ret <= 0) {
                                TM_DBG_LOG("Fail to write %s to %s %x\n", mdinfo_string,
                                           "/proc/driver/thermal/mdm_mdinfo", ret);
                            } else {
                                TM_DBG_LOG("Success to write %s to %s\n", mdinfo_string,
                                           "/proc/driver/thermal/mdm_mdinfo");
                            }
                            close(fd);
                        }
                    }
...
static int connectToRild(int* solt_id) {
    ...
    int sock = -1;
    sock = socket_local_client("rild-oem", ANDROID_SOCKET_NAMESPACE_RESERVED, SOCK_STREAM);
    if (sock < 0) TM_DBG_LOG("connectToRild %s\n", strerror(errno));
#if 0
    ...
    while (1) {
        // read thresholds
        {
            if (0 == access("/proc/driver/thermal/mdm_mdinfoex_thre", R_OK)) {
                ...
                if (fd < 0) {
                    ...
                    close(fd);
                    if (3 == sscanf(buf, "%d,%d,%d,", &new_thresholds[1], &new_thresholds[0],
                                    &new_thresholds[2]))
                        TM_DBG_LOG("new thresholds: %d,%d,%d\n", new_thresholds[0],
                                   new_thresholds[1], new_thresholds[2]);
                    else
                        TM_DBG_LOG("%s\n", buf);
                }
            }
        ...
        for (i = 0; i < 3; i++) {
            if (new_thresholds[i] == thresholds[i])
                ...
                if (0 > recvMdThermalInfo(socket, 0, 0)) TM_DBG_LOG("set threshold failed %d\n", i);
...
// mark for CTS fail ifc_set_throttle(IFC_NAME[i], thro * ONE_MBITS_PER_SEC, thro *
// ONE_MBITS_PER_SEC);
#ifdef NEVER
                exe_cmd(i, thro);
#endif
