...
static const char* g_pcNVM_ModomBackNum =
        "/mnt/vendor/nvdata/md/NVRAM/IMPORTNT/MP0D_000";  ////The file will be updated after
                                                          /// modifying MD important data
...
static void fix_legacy_nvram_path_in_binregion(char* path, int path_size) {
    ...
    if (legacy_path_index != -1) {
        snprintf(path_suffix, sizeof(path_suffix), "%s",
                 path + strlen(legacy_nvram_path_prefix[legacy_path_index]));
...
readbackupdone:
    ...
    if ((iMapFileSize + 3 * sizeof(unsigned int)) >
        (iBlockSize - sizeof(unsigned int) - sizeof(unsigned short))) {
    ...
    if (bSetFlag) {
        *((unsigned int*)(tempBuffer + 3 * sizeof(unsigned int) + iMapFileSize)) = iBackupFileNum;
        *((unsigned short*)(tempBuffer + 3 * sizeof(unsigned int) + iMapFileSize +
                            sizeof(unsigned int))) = iMDBackupNum;
    } else {
        *((unsigned int*)(tempBuffer + 3 * sizeof(unsigned int) + iMapFileSize)) = 0xFFFFFFFF;
        *((unsigned short*)(tempBuffer + 3 * sizeof(unsigned int) + iMapFileSize +
    ...
    iBackupFileNum1 = *((unsigned int*)(tempBuffer + 3 * sizeof(unsigned int) + iMapFileSize));
    iMDBackupNum1 = *((unsigned short*)(tempBuffer + 3 * sizeof(unsigned int) + iMapFileSize +
                                        sizeof(unsigned int)));
...
bool FileOp_CmpBackupFileNum() {
    ...
    if (iFileDesc == -1) {
        ...
        if (iMDBackupFileNum == 0) ++open_error;
        close(iFileDesc);
    }
    if (open_error >= 2) {
        NVBAK_LOG("[NVRAM]:BackupFileNum Flag and ModemBackupNum not exist!\n");
...
recover:
    if (NVM_GetBackupFileNum(&iSavedAPBackupFileNum, &iSavedMDBackupFileNum)) {
        ...
        if (!NVM_ComputeBackflagCheckSum(iFileDesc)) {
            NVBAK_LOG(
                    "[NVRAM]:FileOp_CmpBackupFileNum:Backflag compute check sum "
                    "failed(ignore)\r\n");
            unlink(g_pcNVM_BackFlag);
        }
        close(iFileDesc);
...
bool FileOp_CreateBinRegionBadBlockBitMap() {
    ...
    */
    ...
    BinRegionBlockTotalNum =
            iPartitionSize / iBlockSize - nvram_platform_log_block - nvram_platform_resv_block;
    if (nvram_platform_layout_version == 0)
        BinRegionBlockTotalNum =
                info.size / iBlockSize - nvram_platform_log_block - nvram_platform_resv_block;
    else if (nvram_platform_layout_version == 1)
...
bool FileOp_CheckFileExist() {
    ...
    if (g_Check_File_Count == 0) return true;
    for (i = 0; i < g_Check_File_Count; i++) {
        if (stat(aCheckExistList[i].cCheckFileName, &st) == 0) {
            gFileExistMark[aCheckExistList[i].iCheckFileIndex / 32 + 1] |=
                    (0x1U << (aCheckExistList[i].iCheckFileIndex -
                              (aCheckExistList[i].iCheckFileIndex / 32) * 32));
        } else {
            NVBAK_LOG("FileOp_CheckFileExist stat %s fail: %s", aCheckExistList[i].cCheckFileName,
                      (char*)strerror(errno));
            gFileExistMark[aCheckExistList[i].iCheckFileIndex / 32 + 1] &=
                    ~(0x1U << (aCheckExistList[i].iCheckFileIndex -
                               (aCheckExistList[i].iCheckFileIndex / 32) * 32));
    ...
    start_address = info.size -
                    ((loff_t)nvram_platform_log_block + (loff_t)nvram_check_exist_block_offset) *
...
bool FileOp_CreatMdFileList(const char* SrcDirName) {
    ...
    filelistfd = open(filelistPathName, O_TRUNC | O_CREAT | O_WRONLY,
...
bool checkNvramReady(void) {
    ...
    while (read_nvram_ready_retry < MAX_RETRY_COUNT) {
        ...
        if (strcmp(nvram_init_val, "Ready") == 0 || strcmp(nvram_init_val, "Pre_Ready") == 0) {
            ...
            NVRAM_LOG(
                    "%s(), property_get(\"vendor.service.nvram_init\") = %s, "
                    "read_nvram_ready_retry = %d",
...
bool FileOp_BackupToBinRegion_All() {
    ...
    if (nvram_misc_log_feature) NVRAM_MISC_LOG(1, "Enter BackupToBinRegion_all");
    ...
    if (nvram_check_exist_feature) FileOp_CheckFileExist();
    ...
    if ((iMapFileSize + 3 * sizeof(unsigned int)) % iWriteSize != 0)
        ...
        iMemSize = iMapFileSize +
                   3 * sizeof(unsigned int);  // mapfile size, datfile size, cleanboot flag
    // iMemSize=iBlockSize;
    ...
    if ((3 * NvRamBlockNum * sizeof(char) + sizeof(unsigned int) + iMemSize) <
        iBlockSize * iReserveBlockCount) {
        memcpy(BitMapBuffer + 3 * NvRamBlockNum * sizeof(char) + sizeof(unsigned int), tempBuffer,
               iMemSize);
    } else {
        NVBAK_LOG("[NVRAM]block too small,do not save Mapfile in last block \r\n");
    }
    if (lseek(fd, (j - iReservePos) * iBlockSize, SEEK_SET) < 0) {
        NVBAK_LOG("[NVRAM]:seek to %d error %s\n", (j - iReservePos) * iBlockSize,
                  (char*)strerror(errno));
        close(fd);
        free(tempBuffer);
        free(BitMapBuffer);
    ...
    if (i == j) {
        ...
        for (i; i < j; i++) {
            if (bBadBlockBitMap[i] == 0) iFreeBlockNum++;
        }
    ...
    for (i = 0; i < iWriteTime; i++) {
        ...
        iAlreadyWrite++;
        if (iAlreadyWrite == iWriteTime) break;
    ...
    if (nvram_misc_log_feature) NVRAM_MISC_LOG(1, "Leave BackupToBinRegion_all");
...
bool FileOp_BackupToBinRegion_All_Exx(unsigned char* time_value) {
    time_t before;
    time_t after;
    unsigned char* leave_time = {
            '\0'};  // can't get end time of FileOp_BackupToBinRegion_All_Ex,use NULL.
    bool bRet = true;
    struct stat st;
    char* tempBuffer = NULL;
    char* BitMapBuffer = NULL;
...
bool FileOp_BackupToBinRegion_All_Ex(int value) {
    ...
    if (nvram_misc_log_feature) NVRAM_MISC_LOG(1, cLog);
...
bool FileOp_RestoreFromBinRegion(bool bCleanBoot) {
    ...
    NVBAK_LOG(
            ...
            "nvram_platform_layout_version##=%d\n",
    ...
    while (iBlockNum > 0) {
        ...
        if (lseek(fd, (iBlockNum - iReservePos) * iBlockSize, SEEK_SET) < 0) {
            NVBAK_LOG("[NVRAM]:seek to %d error %s\n", (iBlockNum - iReservePos) * iBlockSize,
                      (char*)strerror(errno));
            free(tempBuffer);
            free(tempBitmap1);
            free(tempBitmap2);
 ...
 * For, MOTA update, data patition will not be ereased; then, ccci_fsd loss the capability to
 * read/write md nvram. EE will occur. So, we modify md nvram files's attribute fisrt here
 */
...
bool FileOp_RecoveryData() {
    ...
    Recovery:
        ...
        if (nvram_misc_log_feature) NVRAM_MISC_LOG(1, "Enter clean boot, Restore From BinRegion");
        ...
        if (nvram_misc_log_feature) NVRAM_MISC_LOG(1, "Leave clean boot, Restore From BinRegion");
...
bool FileOp_RestoreFromFiles(int eBackupType) {
    ...
    int fhs = sizeof(unsigned int) + 2 * sizeof(unsigned int) +
              sizeof(File_Title_Header1);  // In order to align, minus sizeof(short int)
...
static unsigned int FileOp_ComputeCheckSum(void) {
    ...
    NVBAK_LOG("Starting FileOp_ComputeCheckSum (pid %d) on %s", getpid(),
    ...
    // NVBAK_LOG("Start_while FileOp_ComputeCheckSum (pid %d) on
    ...
    for (i = 0; i < tmplooptime; i++) {
        ...
        // NVBAK_LOG("Start_while: i is %d, FileOp_ComputeCheckSum (pid %d) on
        // %s",i,getpid(),ctime(&start_while));// add for log
        iResult = read(iFileDesc_file, tempBuf, jloop_num * iLength);
        // NVBAK_LOG("Start_while: iResult is %d, FileOp_ComputeCheckSum (pid %d) on
        ...
        // NVBAK_LOG("Start_while:before_while FileOp_ComputeCheckSum (pid %d) on
    ...
    NVBAK_LOG("Ending FileOp_ComputeCheckSum (pid %d) on %s", getpid(), ctime(&end));  // add for
                                                                                       // log
    return ulCheckSum;
}
#if 0
...
bool FileOp_IsSyncWithBinRegion() {
    ...
    NVBAK_LOG("[NVRAM]: %s, checksum(in nvdata)=0x%X, checksum(in binregion)=0x%X \n", __FUNCTION__,
              stBackupFileInfo.ulCheckSum, ulCheckSum);
...
bool FileOp_BackupDataToFiles(int* iFileMask, bool bWorkForBinRegion) {
    ...
    for (eBackupType = APBOOT; eBackupType < ALL; eBackupType++) {
        if (iMask[eBackupType] == 1) continue;
        ...
        if (nvram_platform_header_offset != 0 && !nvram_ecci_c2k_support()) {
            if (nvram_md5_support() && nvram_evdo_support()) {
                *((short int*)&(FileTitleInfo1.iFileBufLen) - nvram_platform_header_offset) +=
                        FileSize;
                NVBAK_LOG("iFileBufLen is (%d) !", *((short int*)&(FileTitleInfo1.iFileBufLen) -
                                                     nvram_platform_header_offset));
            } else if (nvram_md5_support() && !nvram_evdo_support()) {
                *((short int*)&(FileTitleInfo2.iFileBufLen) - nvram_platform_header_offset) +=
                        FileSize;
                NVBAK_LOG("iFileBufLen is (%d) !", *((short int*)&(FileTitleInfo2.iFileBufLen) -
                                                     nvram_platform_header_offset));
            } else if (!nvram_md5_support() && nvram_evdo_support()) {
                *((short int*)&(FileTitleInfo3.iFileBufLen) - nvram_platform_header_offset) +=
                        FileSize;
                NVBAK_LOG("iFileBufLen is (%d) !", *((short int*)&(FileTitleInfo3.iFileBufLen) -
                                                     nvram_platform_header_offset));
            } else {
                *((short int*)&(FileTitleInfo4.iFileBufLen) - nvram_platform_header_offset) +=
                        FileSize;
                NVBAK_LOG("iFileBufLen is (%d) !", *((short int*)&(FileTitleInfo4.iFileBufLen) -
                                                     nvram_platform_header_offset));
            }
        }
    ...
    iResult = lseek(iFileDesc_map, sizeof(unsigned int) + 2 * sizeof(unsigned int), SEEK_SET);
    if (!nvram_ecci_c2k_support()) {
        if (nvram_platform_header_offset == 0 && nvram_md5_support() && nvram_evdo_support())
            ...
            read(iFileDesc_map, &FileTitleInfo4, sizeof(FileTitleInfo4) - 4 * sizeof(short int));
        else if (nvram_platform_header_offset != 0 && nvram_md5_support() && nvram_evdo_support())
            ...
            read(iFileDesc_map, &FileTitleInfo4,
                 sizeof(FileTitleInfo4) - 4 * sizeof(short int) - nvram_platform_header_offset * 2);
    } else
...
bool FileOp_BackupData_Special(char* buffer, int count, int mode) {
    ...
    if (iNum != count) {
        NVBAK_LOG(
                "BackupData_Special: the num of real string doesn't match with the value of count "
                "!");
        return false;
    }
    for (i = 0; i < iNum; i++) {
    ...
    NVBAK_LOG("iMask:%d,%d,%d,%d,%d,%d,%d\n", iMask[0], iMask[1], iMask[2], iMask[3], iMask[4],
              iMask[5], iMask[6]);
...
bool FileOp_RestoreAll_NvRam(void) {
    ...
    if (bRet == true) {
        ...
        if (bRet != true) NVBAK_LOG("[FileOp_RestoreAll_NvRam] convert data fail !");
    }
    return bRet;
}
...
bool FileOp_BackupToBinRegionForDM() {
    ...
    if (dir == NULL) {
        NVBAK_LOG("FileOp_BackupData open dir: %s ,this dir not exist!", strDMFileFolderPath);
        close(iFileDesc_map);
        close(iFileDesc_file);
        return false;
    }
    ...
    iResult = read(
            iFileDesc_map,
            tempBuf + 3 * sizeof(int) + (BinRegionBlockTotalNum - NvRamBlockNum) * sizeof(char),
            iMapFileSize);
    ...
    iResult = read(iFileDesc_file,
                   tempBuf + 3 * sizeof(int) +
                           (BinRegionBlockTotalNum - NvRamBlockNum) * sizeof(char) + iMapFileSize,
                   iDatFileSize);
    ...
    iWriteSize = 0;
    if ((int)(3 * sizeof(int) + (BinRegionBlockTotalNum - NvRamBlockNum) * sizeof(char) +
              iMapFileSize + iDatFileSize) > iBlockSize) {
        int offset = 0;
        for (pos = NvRamBlockNum; pos < BinRegionBlockTotalNum; pos++) {
            if (bDMBitmap[pos - NvRamBlockNum] == 0) {
                ...
                if (iWriteSize >= (int)(3 * sizeof(int) +
                                        (BinRegionBlockTotalNum - NvRamBlockNum) * sizeof(char) +
                                        iMapFileSize + iDatFileSize))
                    break;
            } else
...
bool FileOp_RestoreFromBinRegionForDM() {
    ...
    int iMapFileSize, iDatFileSize, iBlockSize, i, iDMBackupFlag, iFileNum, iFileSize;
    ...
    tempBuffer = (char*)malloc((BinRegionBlockTotalNum - NvRamBlockNum) * iBlockSize);
    if (tempBuffer == NULL) {
        NVBAK_LOG("[NVRAM]:memory malloc error\r\n");
        close(fd);
        free(bDMBitmap);
    ...
    for (i = NvRamBlockNum; i < BinRegionBlockTotalNum; i++) {
        ...
        if ((int)(iMapFileSize + iDatFileSize + 3 * sizeof(int) +
            ...
            for (j = i - NvRamBlockNum + 1; j < BinRegionBlockTotalNum - NvRamBlockNum; j++) {
                if (bDMBitmap[j] == 0) {
                    ...
                    if (iReadSize >=
                        (int)(iMapFileSize + iDatFileSize + 3 * sizeof(int) +
                              (BinRegionBlockTotalNum - NvRamBlockNum) * sizeof(char))) {
    ...
    iResult = write(
            iFileDesc_map,
            tempBuffer + 3 * sizeof(int) + (BinRegionBlockTotalNum - NvRamBlockNum) * sizeof(char),
            iMapFileSize);
    if (iResult != iMapFileSize) {
    ...
    while (iFileNum > 0 && iFileNum <= INT_MAX) {
        memcpy(acSrcPathName, tempBuffer + sizeof(int) + i * (MAX_NAMESIZE + sizeof(int)),
               MAX_NAMESIZE);
        NVBAK_LOG("dm file:%s\n", acSrcPathName);
        memcpy(&iFileSize,
               tempBuffer + sizeof(int) + i * (MAX_NAMESIZE + sizeof(int)) + MAX_NAMESIZE,
               sizeof(int));
...
bool FileOp_RestoreFromBinRegion_ToFile() {
    ...
    open(g_pcNVM_BackFlag , O_TRUNC|O_CREAT|O_RDWR|O_SYNC, S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP);
        if(INVALID_HANDLE_VALUE == iFileDesc_backupflag)
        {
            NVBAK_LOG("[NVRAM]: cannot open backup file num data\n");
            close(iFileDesc_backupflag);
