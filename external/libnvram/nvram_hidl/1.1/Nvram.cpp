...
Return<void> Nvram::readFileByName(const hidl_string& filename, uint32_t size,
                                   readFileByName_cb _hidl_cb) {
    // TODO implement
    ...
    if (nvramstr == NULL || size == 0 || cstr == NULL) {
        ...
        if (nvramstr != NULL) free(nvramstr);
    ...
    NVRAM_LOG(
            "nvramstr buff[0]%x, buff[1]%x, buff[2]%x, buff[3]%x, buff[4]%x, buff[5]%x, buff[6]%x, "
            "buff[7]%x, buff[8]%x \n",
...
Return<int8_t> Nvram::writeFileByNamevec(const hidl_string& filename, uint32_t size,
    ...
    if (cstr_data == NULL || size == 0 || cstr_filename == NULL) {
        ...
        if (cstr_filename != NULL) delete[] cstr_filename;
        return int8_t{};
    }
...
Return<void> Nvram::writeFileBylid(int32_t lid, uint16_t para, const hidl_vec<uint8_t>& data,
                                   writeFileBylid_cb _hidl_cb) {
    // TODO implement
    char* cstr_data = new char[data.size() + 1];
    if (cstr_data == NULL) {
        NVRAM_LOG("cstr_data==NULL\n");
...
Return<bool> Nvram::BackupData_Special(const hidl_vec<uint8_t>& data, uint32_t count,
                                       uint32_t mode) {
    char* cstr_data = new char[data.size() + 1];
    if (cstr_data == NULL) {
        NVRAM_LOG("cstr_data==NULL\n");
...
INvram* HIDL_FETCH_INvram(const char* /* name */) { return new Nvram(); }
