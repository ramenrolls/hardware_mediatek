...
static NVRAM_USER_DATA g_akNUData[NU_MAX] = {{"Battery", 0, NVRAM_BATTERY},
...
int main(void) {
    ...
    if (nvram_new_partition_support()) {
        ...
        init_callback();
        if (callback_for_nvram_daemon != NULL) ret_callback = callback_for_nvram_daemon();
    ...
    if (0 == property_set("vendor.service.nvram_init", "Ready")) {
        ...
        if (!Init_AllThreads_In_Nvram()) NVRAM_LOG("init all threads fail\n");
    } else
