...
static unsigned long long get_wp_grp_size(char* wp_grp_size_path) {
    ...
    memset(buffer, 0, 256);
    if (fgets(buffer, 256, fp)) size = strtoul(buffer, NULL, 0) / 512;
    fclose(fp);
    return size;
}
...
int emmc_block_read(struct otp* otp_device, char* buffer, unsigned long long blk_offset,
...
static int emmc_block_write(struct otp* otp_device, char* buffer, unsigned long long blk_offset,
                            unsigned long long blknr) {
    const char* path = OTP_BLOCK_PATH;
    int fd = 0;
    int ret = 0;
...
static int emmc_set_usr_wp(unsigned char wp) {
    ...
    idata.arg = (MMC_SWITCH_MODE_SET_BITS << 24) | (EXT_CSD_USR_WP << 16) | (wp << 8) |
...
static int emmc_clr_usr_wp() {
    ...
    memset(&idata, 0, sizeof(idata));
    idata.opcode = MMC_SWITCH;
    idata.arg = (MMC_SWITCH_MODE_CLEAR_BITS << 24) | (EXT_CSD_USR_WP << 16) |
...
int emmc_otp_get_status(struct otp* otp_device, unsigned long long start_block,
    ...
    *type = wp_type[0];
    ...
    if (*type == WP_TEMPORARY) *type = WP_PERMANENT;
#endif
    return 0;
}
...
int emmc_storage_init(struct otp* otp_device) {
    ...
    OTP_LOG(TAG "Device info: block device path: %s. sysfs start path: %s, %d\n",
            otp_device->otp_blk_path, otp_start_path, PATH_MAX);
    ...
    OTP_LOG(TAG "wp_grp_size = 0x%llx, otp_start = 0x%llx, otp_size = 0x%llx\n",
            otp_device->wp_grp_size, otp_device->otp_start, otp_device->otp_size);
...
out:
    ...
    if (otp_start_path) free(otp_start_path);
    if (otp_size_path) free(otp_size_path);
    return ret;
}
