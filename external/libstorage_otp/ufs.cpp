...
int ufs_block_read(struct otp* otp_device, char* buffer, unsigned long long start_blk,
...
static int ufs_block_write(struct otp* otp_device, char* buffer, unsigned long long start_blk,
                           unsigned long long blknr) {
    const char* path = OTP_PARTITION_BLOCK_PATH;
    int fd;
    int ret;
...
int ufs_storage_init(struct otp* otp_device) {
    ...
    OTP_LOG(TAG "Device info: block device path: %s. sysfs start path: %s\n",
            otp_device->otp_blk_path, otp_start_path);
