...
#define ubrd_warn_log(...) __ubrd_log_buf_print(LOG_ID_MAIN, ANDROID_LOG_WARN, LOG_TAG, __VA_ARGS__)
#define ubrd_info_log(...) __ubrd_log_buf_print(LOG_ID_MAIN, ANDROID_LOG_INFO, LOG_TAG, __VA_ARGS__)
#else
#ifdef LOCAL_DEBUG
#define ubrd_debug_log(format, ...) ALOGD(format, ##__VA_ARGS__)
#else
#define ubrd_debug_log(format, ...) \
    do {                            \
    } while (0)
...
#define ubrd_error_log(format, ...) ALOGE(format, ##__VA_ARGS__)
#define ubrd_warn_log(format, ...) ALOGW(format, ##__VA_ARGS__)
#define ubrd_info_log(format, ...) ALOGI(format, ##__VA_ARGS__)
#endif
...
typedef enum { UBRD_HISTORICAL_TABLE = 0, UBRD_RING_BUFFER = 1 } UBRD_RINGBUF_MODE;
