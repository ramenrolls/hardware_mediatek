...
static int malloc_func_rehook(void) {
    ...
    real_malloc_usable_size =
            (MallocDebugMallocUsableSize)dlsym(malloc_libc_handle, "malloc_usable_size");
...
static inline void init_slim_guard_free_poison(void* mem, int type) {
    if (type == MALLOC_TYPE) {
        ...
        if (hdr_size > POISON_FREE_LEN) hdr_size = POISON_FREE_LEN;
...
#define BUF_SIZE 512
    ...
    if (type == MALLOC_TYPE) {
        ...
        *Pbase = hdr_memalign->base;
        ...
        if (type == MALLOC_TYPE) {
            ...
            ptr = (char*)hdr_memalign;
            len = snprintf(buf, BUF_SIZE, "[ERROR] %p front guard overwritten, head ptr:%p", mem,
                           hdr_memalign);
            for (i = 0; i < sizeof(mtk_hdr_memalign_rear); i++) {
                if (i % sizeof(size_t) == 0) {
                    len += snprintf(buf + len, BUF_SIZE - len, "\n%p", ptr + i);
    ...
    if (rear_check_fail) {
        ...
        if (type == MALLOC_TYPE) {
            ptr = (char*)hdr_malloc;
            len = snprintf(buf, BUF_SIZE, "[ERROR]malloc rear guard overwritten, hdr_malloc:%p",
                           hdr_malloc);
            for (i = 0; i < sizeof(mtk_hdr_malloc); i++) {
                if (i % sizeof(size_t) == 0) {
                    len += snprintf(buf + len, BUF_SIZE - len, "\n%p", ptr + i);
        ...
        if (type == MEMALIGN_TYPE) {
            ptr = (char*)hdr_memalign;
            len = snprintf(buf, BUF_SIZE, "[ERROR]memalign rear guard overwritten, hdr_memalign:%p",
                           hdr_memalign);
            for (i = 0; i < sizeof(mtk_hdr_memalign_rear); i++) {
                if (i % sizeof(size_t) == 0) {
                    len += snprintf(buf + len, BUF_SIZE - len, "\n%p", ptr + i);
...
#endif
EXIT:
    if (recovery) clean_exit();
}

static inline void free_remove_info(ChunkHashEntry* entry) {
    int recovery = 0;

    if (!entry || debug15_mspace_full) return;
    ...
    if (move_to_historical(entry, backtrace, numEntries) < 0) recovery = 1;
    ...
    if (malloc_debug15_enabled) {
        ...
        if (!oldMem) {
            ...
            debug_log("%s: oldMem %p front_guard [%zx] copy_size:%zu\n", __FUNCTION__, oldMem,
                      *(size_t*)hdr->front_guard_malloc, copy_size);
...
static inline void* mtk_guard_memalign(size_t alignment, size_t bytes) {
    ...
    size_t size = alignment + bytes + sizeof(mtk_hdr_memalign_rear) + sizeof(mtk_ftr_t) +
                  2 * sizeof(void*);
    if (size < bytes) {  // Overflow.
        return NULL;
    }
...
DEBUG15_EXPORT
void malloc_disable() {
    if (real_malloc_disable) real_malloc_disable();
    multi_mutex_lock();
}
...
void malloc_enable() {
    multi_mutex_unlock();
    if (real_malloc_enable) real_malloc_enable();
}
...
static void init_debug_config() {
    if (debug15_config) {
        ...
        gDebugConfig.mHistoricalBufferSize =
                ((debug15_config & HISTORICAL_BUFFER_SIZE_MASK) >> 12) *
                HISTORICAL_BUFFER_SIZE_UNIT;
        gDebugConfig.mMaxBtDepth = ((debug15_config & MAX_BT_DEPTH_MASK) >> 24) * BT_DEPTH_UNIT;

        gDebugConfig.mBtMethod = (debug15_config & UNWIND_BT_MASK) >> 29;
    ...
    if (gDebugConfig.mMaxBtDepth == 0) gDebugConfig.mMaxBtDepth = DEFAULT_MAX_BACKTRACE_DEPTH;
    // when using FP,
...
static void mtk_malloc_debug_prepare(void) {
...
#ifdef MTK_USE_RESERVED_EXT_MEM
    if (gDebugMspaceBase == NULL) return;

    if (gDebugConfig.mDebugMspaceSource == INTERNAL_MEM) return;
    ...
    if (gMspaceBuffer == MAP_FAILED) {
        if (strstr(progname, "/system/bin/logd")) {
            snprintf(malloc_init_error_msg, sizeof(malloc_init_error_msg),
                     "parent: Internal mem MAP_FAILED, errno = %d\n", errno);
        } else {
...
#ifdef MTK_USE_RESERVED_EXT_MEM
    if (gDebugMspaceBase == NULL) goto UNLOCK;

    if (gDebugConfig.mDebugMspaceSource == INTERNAL_MEM) goto UNLOCK;
...
static void mtk_malloc_debug_child(void) {
    ...
    if (fd < 0) {
        if (strstr(progname, "/system/bin/logd")) {
            snprintf(malloc_init_error_msg, sizeof(malloc_init_error_msg), "%s cannot open %s",
                     __FUNCTION__, EXM_DEV);
        } else {
        ...
        gDebugMspaceBase =
                mmap(old_mmap_addr, gDebugConfig.mDebugMspaceSize,
...
void malloc_debug_reg_atfork(void) {
    if (malloc_debug15_enabled) {
        if (pthread_atfork(mtk_malloc_debug_prepare, mtk_malloc_debug_parent,
...
void malloc_debug_initialize(void) {
    ...
    if (!strcmp(progname, "sh") || !strcmp(progname, "/system/bin/sh") ||
        strstr(progname, "/system/bin/logcat") || strstr(progname, "/system/bin/installd") ||
    ...
    if (mtk_malloc_debug_set_config(progname)) {
        if (malloc_init_flag) {
            snprintf(malloc_init_error_msg, sizeof(malloc_init_error_msg),
                     "mtk_malloc_debug_set_config fails\n");
        } else {
    ...
    if (!init_recorder()) {
        ...
        debug_log("%s: using malloc %d, debug15_config %x\n", progname, malloc_debug_level,
                  debug15_config);
    } else {
        if (malloc_init_flag) {
            snprintf(malloc_init_error_msg, sizeof(malloc_init_error_msg),
                     "malloc_debug_initialize fails\n");
        } else {
