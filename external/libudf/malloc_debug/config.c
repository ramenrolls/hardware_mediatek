...
#define CONFIG_TABLE_SIZE \
    ((CONFIG_MAP_SIZE - sizeof(struct DebugConfigTable)) / sizeof(struct DebugProcessConfig))
...
anon:
    ...
    goto out;
#else
    gDebugConfigTable = mmap(NULL, CONFIG_MAP_SIZE, PROT_READ | PROT_WRITE | PROT_MALLOCFROMBIONIC,
                             MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    if (gDebugConfigTable == MAP_FAILED) {
        gDebugConfigTable = NULL;
        return -1;
    }
    prctl(PR_SET_VMA, PR_SET_VMA_ANON_NAME, gDebugConfigTable, CONFIG_MAP_SIZE, ASHMEM_CONFIG_NAME);
    gDebugConfigTable->count = 1;
    return 0;
#endif
}
...
static int get_config(const char* name, DebugConfig* config) {
    ...
    if (gDebugConfigTable == NULL) return 0;
...
static int set_config(char* name, DebugConfig* config) {
    ...
    if (i == CONFIG_TABLE_SIZE) return -1;
...
static uint32_t config_to_uint(DebugConfig* cs) {
    ...
    config |= ((cs->mHistoricalBufferSize / HISTORICAL_BUFFER_SIZE_UNIT) << 12) &
              HISTORICAL_BUFFER_SIZE_MASK;
...
static void dump_configs(void) {
    ...
    if (gDebugConfigTable == NULL) return;

    assert(gDebugConfigTable->count);

    for (i = 1; i < gDebugConfigTable->count; i++)
        info_log("%08X %s", config_to_uint(&gDebugConfigTable->configs[i].config),
                 gDebugConfigTable->configs[i].name);

    if (gDebugConfigTable->configs[0].name[0])
        info_log("%08X %s", config_to_uint(&gDebugConfigTable->configs[0].config),
                 gDebugConfigTable->configs[0].name);
}
...
static int add_process(int argc, char** argv) {
    ...
    if (i == 1) set_default_config(&config);
#endif
...
static int add_all_processes(int argc, char** argv) {
    ...
    if (ret) set_default_config(&config);

    return 1 + ret;
}

static int block_process(int argc, char** argv) {
    DebugConfig config = {.mDebugMspaceSize = (uint32_t)-1};
    int i;
...
static int add_config_file(int argc, char** argv) {
    ...
    if (process_config_file(argv[1])) return -1;
