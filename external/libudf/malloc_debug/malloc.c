...
#define CHUNK_ERROR_DETECTION(E, ADDR_1, ADDR_2)                                          \
    if (__builtin_expect(!(E), 0)) {                                                      \
        snprintf(mspace_corruption_msg, sizeof(mspace_corruption_msg),                    \
                 "FUNCTION %s Line %d addr_1 0x%zx addr_2 0x%zx", __FUNCTION__, __LINE__, \
                 (size_t)ADDR_1, (size_t)ADDR_2);                                         \
        ABORT;                                                                            \
    }
#else
...
static void* tmalloc_large(mstate m, size_t nb) {
    ...
    if (v != 0 && rsize < (size_t)(m->dvsize - nb)) {
        if (RTCHECK(ok_address(m, v))) { /* split */
            ...
            CHUNK_ERROR_DETECTION(chunksize(v) == rsize + nb, v,
                                  r);  // assert(chunksize(v) == rsize + nb);
    ...
    if (!PREACTION(ms)) {
        ...
        if (bytes <= MAX_SMALL_REQUEST) {
            ...
            if ((smallbits & 0x3U) != 0) { /* Remainderless fit to a smallbin. */
                ...
                p = b->fd;
                CHUNK_ERROR_DETECTION(chunksize(p) == small_index2size(idx), ms,
                                      p);  // assert(chunksize(p) == small_index2size(idx));
                ...
                if (smallbits != 0) { /* Use chunk in next nonempty smallbin */
                    ...
                    b = smallbin_at(ms, i);
                    p = b->fd;
                    CHUNK_ERROR_DETECTION(chunksize(p) == small_index2size(i), ms,
                                          p);  // assert(chunksize(p) == small_index2size(i));
