...
static int malloc_get_main_thread_stack(pid_t pid) {
    ...
    if (line[0] != '\0') {
        ...
        malloc_main_thread_stack_size = 8 * 1024 * 1024;  // 8MB for simple handling
        debug_log(
                "main_thread_stack_start from /proc/stat %d:0x%zx, main_thread_stack_size:0x%zx\n",
...
size_t get_backtrace_fp(void* fp, intptr_t* addrs, size_t max_entries) {
    ...
    (void)fp;
    if (malloc_get_stack(&pthread_stack_start, &pthread_stack_end)) return 0;
...
size_t get_backtrace_fp(void* fp, intptr_t* addrs, size_t max_entries) {
     ...
     */
    while (i < max_entries && (size_t)fp >= pthread_stack_end &&
           (size_t)fp <= (pthread_stack_start - 4)) {
        /*
