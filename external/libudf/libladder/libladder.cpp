...
static bool UnwindThread(pid_t tid, std::string* str) {
    ...
    if (tgkill(pid, tid, THREAD_SIGNAL) != 0) {
        ...
        if (errno == ESRCH) {
            ALOGE("errno == ESRCH: %s,tid:%d", strerror(errno),
                  tid);  // BACKTRACE_UNWIND_ERROR_THREAD_DOESNT_EXIST;
        } else {
        ...
        if (ucontext == nullptr) {
            ...
            pthread_mutex_unlock(&g_sigaction_mutex);
#ifdef DEBUG_LIBLADDER_BACKTRACE_TIMEOUT
            ALOGE("tid:%d, time:%lu.%ld-%lu.%ld-%lu.%ld-%lu.%ld-%lu.%ld", tid, t[0].tv_sec,
                  t[0].tv_nsec, t[1].tv_sec, t[1].tv_nsec, t[2].tv_sec, t[2].tv_nsec, t[3].tv_sec,
                  t[3].tv_nsec, t[4].tv_sec, t[4].tv_nsec, t[5].tv_sec, t[5].tv_nsec);
#endif
            return false;
        }
    ...
    ALOGE("tid:%d, time:%lu.%ld-%lu.%ld-%lu.%ld-%lu.%ld-%lu.%ld-%lu.%ld-%lu.%ld-%lu.%ld", tid,
          t[0].tv_sec, t[0].tv_nsec, t[1].tv_sec, t[1].tv_nsec, t[2].tv_sec, t[2].tv_nsec,
          t[3].tv_sec, t[3].tv_nsec, t[4].tv_sec, t[4].tv_nsec, t[5].tv_sec, t[5].tv_nsec,
...
bool UnwindCurProcessBT(std::string* strBacktrace) {
    ...
    if (pthread_mutex_trylock(&g_process_unwind_mutex)) return false;
