...
static int ubrd_get_main_thread_stack(pid_t pid) {
    ...
    while (1) {
        ...
        if (c == '\n') {
            ...
            if ((i > 21 + 7) && (strncmp((const char*)&line[i - 7], "[stack]", 7) == 0)) {
                ...
                ubrd_main_thread_stack_size =
                        8 * 1024 * 1024;  // bypass RLIMIT check for simple handling
                ubrd_debug_log("main_thread_stack_start:%p, main_thread_stack_size:%p\n",
                               (void*)ubrd_main_thread_stack_start,
                               (void*)ubrd_main_thread_stack_size);
                close_fptr(fd);
                dlclose(lib);
                return 0;
            } else {
    ...
    if (line[0] != '\0') {
        ...
        ubrd_debug_log(
                "main_thread_stack_start from /proc/self/stat:%p, main_thread_stack_size:%p\n",
...
#if defined(__LP64__)
static inline size_t internal_get_backtrace_fp(uintptr_t* addrs, size_t max_entries,
...
size_t ubrd_get_backtrace_fp(void* fp, uintptr_t* addrs, size_t max_entries) {
    ...
    (void)fp;
    if (ubrd_get_stack(&pthread_stack_start, &pthread_stack_end)) return 0;
...
size_t ubrd_get_backtrace_fp(void* fp, uintptr_t* addrs, size_t max_entries) {
    ...
    while (i < max_entries && (size_t)fp > pthread_stack_end && (size_t)fp < pthread_stack_start) {
        ...
        if ((lr > pthread_stack_end) && (lr < (pthread_stack_start - 4))) {  // CLANG
            ...
            if ((size_t)((intptr_t*)fp + 1) > (pthread_stack_start - 4)) break;
...
void get_java_backtrace(u4* backtrace, u4 numEntries) {
    if (dvmGetThreadStack != NULL) dvmGetThreadStack(backtrace, numEntries);
}
