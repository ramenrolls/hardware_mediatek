...
__attribute__((section(".init_array"))) typeof(udf_init_array)* __udf_init_array = udf_init_array;
...
static int MMAPDebug_compareFunc(PUBRD_EntryInfo data1, PUBRD_EntryInfo data2) {
    if (data1->mAddr == data2->mAddr && data1->mBytes == data2->mBytes)
        return 1;
    else
        return 0;
}
...
static inline void MMAPDebug_btrace_record(void* addr, size_t bytes) {
...
#ifdef ENABLE_MMAP_BT_NAME
    PUBRD_EntryInfo pEntryInfo = (PUBRD_EntryInfo)mspaceAllocate(
            g_MMAPDebug_PUBRD,
            sizeof(UBRD_EntryInfo) +
                    (pBtEntry->numEntries * (sizeof(size_t) * 2 + 3) + 1 + 3) * sizeof(char));
#else
    PUBRD_EntryInfo pEntryInfo =
            (PUBRD_EntryInfo)mspaceAllocate(g_MMAPDebug_PUBRD, sizeof(UBRD_EntryInfo));
#endif
    if (pEntryInfo == NULL) {
    ...
    if (pEntryInfo->mExtraInfo) {
        ...
        *next++ = 't';
        ...
        for (i = 1; i < numEntries; i++) next = int2Str(backtrace[i], next);
    ...
    //
    if (recordToHashTable(g_MMAPDebug_PUBRD, (size_t)addr /*hash key*/, pEntryInfo,
