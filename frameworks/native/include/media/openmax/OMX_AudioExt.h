...
#define OMX_AUDIO_AACToolAndroidSSBR                                                         \
    (OMX_AUDIO_AACToolVendor << 0) /**< SSBR: MPEG-4 Single-rate (downsampled) Spectral Band \
                                      Replication tool allowed or active */
#define OMX_AUDIO_AACToolAndroidDSBR \
    (                                \
            OMX_AUDIO_AACToolVendor  \
            << 1) /**< DSBR: MPEG-4 Dual-rate Spectral Band Replication tool allowed or active */
...
typedef struct OMX_AUDIO_PARAM_ANDROID_AACPRESENTATIONTYPE {
    OMX_U32 nSize;              /**< size of the structure in bytes */
    OMX_VERSIONTYPE nVersion;   /**< OMX specification version information */
    OMX_S32 nMaxOutputChannels; /**< Maximum channel count to be output, -1 if unspecified, 0 if
                                   downmixing disabled */
    ...
    OMX_S32 nHeavyCompression;     /**< 0 for light compression, 1 for heavy compression, -1 if
                                      unspecified */
    OMX_S32 nTargetReferenceLevel; /**< Target reference level, between 0 and 127, -1 if unspecified
                                    */
    OMX_S32 nEncodedTargetLevel;   /**< Target reference level assumed at the encoder, between 0 and
                                      127, -1 if unspecified */
    OMX_S32 nPCMLimiterEnable;     /**< Signal level limiting, 0 for disable, 1 for enable, -1 if
                                      unspecified */
