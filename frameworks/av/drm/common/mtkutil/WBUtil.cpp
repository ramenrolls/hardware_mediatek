    ...
    while (true) {
        ...
        if (permission != DrmDef::PERMISSION_NONE_INDEX) {
            ...
            if ((wb.p[wb.offset - 1] & 0x40))  // e.g. is <display>
            {
                ...
                if (!gotoNextEndTag(wb)) {
                    ALOGE("Not found %s-end-tag, offset:[%d]",
                          DrmDef::getPermissionStr(permission).string(), wb.offset);
                    return false;
                }
            } else {
                ALOGV("<%s/> happened, offset:[%d]", DrmDef::getPermissionStr(permission).string(),
                      wb.offset);
            }
            ...
            if (!DrmUtil::isEntryValid(&ro->rights.entryPtr[permission])) {
                ...
                ALOGW("remove invalid entry for [%s].",
                      DrmDef::getPermissionStr(permission).string());
            }
        }
    }
...
bool WBUtil::parseConstraint(WB& wb, int permission, RO* ro) {
    ...
    char ch = 0;
    if (peekNextStartTag(wb, ch) &&
        (ch & 0x3f) == DrmDef::DRM_WB_CONSTRAINT_TAG)  // constraint-start-tag
    {
        ALOGV("2. [%d]", wb.offset);
        if (!gotoNextStartTag(wb)) {
        ...
        while (true) {
            ...
            if (peekNextStartTag(wb, ch))  // have start-tag below constraint-start-tag
            {
                if ((ch & 0x3f) == DrmDef::DRM_WB_COUNT_TAG)  // count-start-tag
                {
                    ...
                    ro->rights.entryPtr[permission].used_count =
                            ro->rights.entryPtr[permission].total_count;
                ...
                } else if ((ch & 0x3f) == DrmDef::DRM_WB_INTERVAL_TAG)  // interval-start-tag
                {
                    ...
                    if (!StrUtil::intervalStrToTick(&wb.p[wb.offset + 1],
                                                    ro->rights.entryPtr[permission].interval)) {
                ...
                } else if ((ch & 0x3f) == DrmDef::DRM_WB_DATETIME_TAG)  // datetime-start-tag
                {
                    ...
                    if (peekNextStartTag(wb, ch) && (ch & 0x3f) == DrmDef::DRM_WB_START_TAG) {
                        ...
                        if (wb.p[wb.offset] ==
                            0x01) {  // assume start date_time valid if not specify start date_time
                            ALOGV("date-time start time not specify, assign default start time");
                            // M: @}
                        } else if (!StrUtil::datetimeStrToTick(
                                           &wb.p[wb.offset + 1],
                                           ro->rights.entryPtr[permission].start_time)) {
                    ...
                    if (peekNextStartTag(wb, ch) && (ch & 0x3f) == DrmDef::DRM_WB_END_TAG) {
                        ...
                        if (wb.p[wb.offset] ==
                            0x01) {  // assume end date-time valid if not specify end date-time
                            ALOGV("date-time end time not specify, assign default end time");
                            // M: @}
                        } else if (!StrUtil::datetimeStrToTick(
                                           &wb.p[wb.offset + 1],
                                           ro->rights.entryPtr[permission].end_time)) {
                            ALOGE("date-time string:[%s] is not valid.", &wb.p[wb.offset + 1]);
                            goto ERROR;
                        }
                    ...
                    if (!bStartTag && !bEndTag) {
                        ALOGD("datetime: both start-tag and end-tag not found, assume this "
                              "constraint has no meaning and ignore");
                        ...
                        goto ERROR;
                    } else if (ro->rights.entryPtr[permission].start_time >=
                               ro->rights.entryPtr[permission].end_time) {
...
bool WBUtil::gotoNextStartTag(WB& wb) { return gotoNextTag(wb, true); }

bool WBUtil::gotoNextEndTag(WB& wb) { return gotoNextTag(wb, false); }
