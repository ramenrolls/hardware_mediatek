...
int CryptoHelper::cipherKeyLen(CIPHER cipher) { return getCipher(cipher)->key_len; }

int CryptoHelper::cipherIvLen(CIPHER cipher) { return getCipher(cipher)->iv_len; }

int CryptoHelper::cipherBlkLen(CIPHER cipher) { return getCipher(cipher)->block_size; }

// return 1 for success with {len_out} bytes of data placed in {buf_out};
// return 0 for failure and no data result is available.
int CryptoHelper::doCryption(BYTE* buf_in, int len_in, BYTE* buf_out, int& len_out, BYTE* iv,
                             bool is_final /* =false */) {
    // if for RC4 cryption, IV may be NULL
    if (len_in <= 0 || buf_in == NULL || buf_out == NULL) {
        ALOGE("doCryption() : error parameter: len_in:[%d], buf_in:[%p], buf_out:[%p]", len_in,
              buf_in, buf_out);
        return 0;
    }
    ALOGV("doCryption() : len_in:[%d], buf_in:[%p], buf_out:[%p], is_final:[%d]", len_in, buf_in,
          buf_out, (int)is_final);
...
int CryptoHelper::getKeyLen(void) { return EVP_CIPHER_CTX_key_length(&m_ctx); }

int CryptoHelper::getIvLen(void) { return EVP_CIPHER_CTX_iv_length(&m_ctx); }

int CryptoHelper::getBlkSize(void) { return EVP_CIPHER_CTX_block_size(&m_ctx); }

int CryptoHelper::desiredOutBufSize(int len_in) { return calc_cipher_length(len_in); }
