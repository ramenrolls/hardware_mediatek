...
Cta5NormalFile::Cta5NormalFile(int fd, String8 key) : Cta5File(fd, key) {
...
Cta5NormalFile::Cta5NormalFile(String8 mimeType, String8 cid, String8 dcfFlHeaders,
...
bool Cta5NormalFile::encrypt(int fd_in, int fd_out,
    ...
    if (result != 16) {
        ALOGE("[ERROR]encrypt: failed to write first iv to cipher header.reason=[%s]",
              strerror(errno));
        return false;
    }
    cipher_size += result;
    ...
    if (-1 == lseek(fd_in, data_offset, SEEK_SET)) {
        ALOGE("[ERROR]encrypt: failed to seek to the start of content!reason=[%s]",
              strerror(errno));
        return false;
    }
    ...
    notify(infoListener, callBackStr);
    ALOGD("encrypt done:data_size=[%d], cipher_size=[%d],cnt_total=[%d],", data_size,
          (int)cipher_size, (int)cnt_total);
...
bool Cta5NormalFile::decrypt(int fd_out, const Vector<DrmCtaUtil::Listener>* infoListener) {
    ...
    while (readSize < contentLength) {
        memset(buffer, 0, sizeof(buffer));
        int size = (contentLength - readSize) > DECRYPT_BUF_LEN ? DECRYPT_BUF_LEN
                                                                : (contentLength - readSize);
        int readLength = Cta5NormalFile::readCipherFile(cipher_fd, buffer, size, resultSize,
    ...
    callBackStr.append(DrmDef::CTA5_DONE);
    notify(infoListener, callBackStr);
    ALOGD("decrypt done: read size=[%d], resultSize=[%d],contentL=[%d]", readSize, resultSize,
          contentLength);
    return true;
}

ssize_t Cta5NormalFile::readCipherFile(int fd, void* buffer, int numBytes, int offset,
                                       int cipher_content_offset, int cipher_content_size) {
    ...
    if (NULL != buffer && numBytes > -1 && offset > -1) {
        ...
        bytesRead = CipherFileUtil::CipherFileRead(
                fd, buffer, numBytes, offset, cipher_content_offset, cipher_content_size, cr_hlp);
...
int64_t Cta5NormalFile::pread(void* buf, uint64_t size, off64_t offset) {
    ...
    bytesRead = Cta5NormalFile::readCipherFile(cipher_fd, buf, size, offset, (int)headerLen,
                                               (int)contentLength);
    ALOGV("pread, size=[%ld], offset=[%ld],byteRead=[%d]", (long)size, (long)offset,
          (int)bytesRead);
