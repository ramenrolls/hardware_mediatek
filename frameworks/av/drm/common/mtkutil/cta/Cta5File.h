...
const static unsigned char IV[CTA_KEY_LEN] = {0xFE, 0xAE, 0xBC, 0xC6, 0x16, 0x02, 0xB5, 0xBE,
                                              0xFF, 0x3E, 0xB6, 0x16, 0x19, 0xA2, 0xB9, 0x3E};
...
struct KeyInfo {
  public:
    const static uint32_t KEYINFO_SIZE =
            CTA_KEY_LEN + CTA_KEY_LEN + CTA_KEY_LEN + sizeof(uint64_t) + sizeof(uint32_t);
...
class Cta5FileHeader {
  ...
  public:
    ...
    const static uint32_t CTA5_FILE_HEADER_LEN =
            ...
            + sizeof(uint64_t) + sizeof(uint64_t) + CTA_KEY_LEN + CTA_KEY_LEN;
...
class Cta5File {
  ...
  public:
    ...
    Cta5File(String8 mimeType, uint64_t datatLen, String8 key, bool needPartialEncrypt);
  ...
  public:
     ...
     */
    virtual bool encrypt(int fd_in, int fd_out,
                         const Vector<DrmCtaUtil::Listener>* infoListener) = 0;
    /**
