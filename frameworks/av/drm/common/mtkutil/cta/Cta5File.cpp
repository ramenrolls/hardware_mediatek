 ...
 */
bool Cta5FileUtil::hmac128Md5Digest(const uint8_t* key, const uint8_t* msg, size_t len,
                                    uint8_t* out) {
    HMAC_CTX ctx;
...
bool Cta5FileUtil::aes128cbcEncrypt(const uint8_t* in, uint8_t* out, size_t len, const uint8_t* key,
                                    uint8_t* iv) {
...
bool Cta5FileUtil::aes128cbcDecrypt(const uint8_t* in, uint8_t* out, size_t len, const uint8_t* key,
                                    uint8_t* iv) {
    if (in == NULL || out == NULL || key == NULL) {
        ALOGE("bad parameters\n");
        return false;
...
KeyInfo::KeyInfo(String8 keyInfo) { setKeyInfo(keyInfo.string()); }
...
bool KeyInfo::deriveKeyInfo(String8 key) {
    ...
    /*ALOGD("mDerivedKey:0x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x",
               mDerivedKey[0], mDerivedKey[1], mDerivedKey[2],mDerivedKey[3], mDerivedKey[4],
       mDerivedKey[5], mDerivedKey[6], mDerivedKey[7], mDerivedKey[8], mDerivedKey[9],
               mDerivedKey[10], mDerivedKey[11], mDerivedKey[12], mDerivedKey[13], mDerivedKey[14],
       mDerivedKey[15]);*/
    // Generate hash
    ret = Cta5FileUtil::md5Digest((const uint8_t*)mDerivedKey, (size_t)sizeof(mDerivedKey), mHash);
    if (ret == false) {
    ...
    /*ALOGD("mHash:0x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x",
            mHash[0], mHash[1],mHash[2], mHash[3], mHash[4], mHash[5], mHash[6], mHash[7], mHash[8],
       mHash[9], mHash[10], mHash[11], mHash[12], mHash[13], mHash[14], mHash[15]);*/
    ...
    /*ALOGD("mContentKey:0x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x",
            mContentKey[0], mContentKey[1],mContentKey[2], mContentKey[3], mContentKey[4],
       mContentKey[5], mContentKey[6], mContentKey[7], mContentKey[8], mContentKey[9],
            mContentKey[10], mContentKey[11], mContentKey[12], mContentKey[13], mContentKey[14],
       mContentKey[15]);*/
...
bool KeyInfo::restoreKeyInfo(String8 key) {
    ...
    // generate derivedKey form salt and key
    /* ALOGD("mSalt:0x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x",
                 mSalt[0], mSalt[1], mSalt[2], mSalt[3], mSalt[4], mSalt[5], mSalt[6], mSalt[7],
       mSalt[8], mSalt[9], mSalt[10], mSalt[11], mSalt[12], mSalt[13], mSalt[14], mSalt[15]);*/
    bool ret = Cta5FileUtil::hmac128Md5Digest((const uint8_t*)mSalt, (const uint8_t*)key.string(),
                                              (size_t)key.length(), mDerivedKey);
    // Iterate 2000 times to make the key hard to hack
    ...
    ret = Cta5FileUtil::md5Digest((const uint8_t*)mDerivedKey, (size_t)sizeof(mDerivedKey), hash);
    /*ALOGD("hash:0x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x", hash[0],
       hash[1], hash[2],hash[3], hash[4], hash[5], hash[6], hash[7], hash[8], hash[9], hash[10],
    ...
    if (memcmp(hash, mHash, sizeof(mHash)) != 0) {
        ...
        ALOGE("mHash:0x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x", mHash[0],
              mHash[1], mHash[2], mHash[3], mHash[4], mHash[5], mHash[6], mHash[7], mHash[8],
              mHash[9], mHash[10], mHash[11], mHash[12], mHash[13], mHash[14], mHash[15]);
        ALOGE("hash:0x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x", hash[0],
    ...
    /*ALOGE("mContentKey:0x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x",
       mContentKey[0], mContentKey[1], mContentKey[2],mContentKey[3], mContentKey[4],
       mContentKey[5], mContentKey[6], mContentKey[7], mContentKey[8], mContentKey[9],
       mContentKey[10], mContentKey[11], mContentKey[12], mContentKey[13], mContentKey[14],
       mContentKey[15]);*/
...
bool KeyInfo::isKeyValid(String8 key) {
    ...
    bool ret = Cta5FileUtil::hmac128Md5Digest((const uint8_t*)mSalt, (const uint8_t*)key.string(),
                                              (size_t)key.length(), tmp_derivedKey);
    // Iterate 2000 times to make the key hard to hack
    for (int i = 0; i < CTA_KDF_NUM; ++i) {
    ...
    // Generate hash
    uint8_t hash[CTA_KEY_LEN] = {0};
    ret = Cta5FileUtil::md5Digest((const uint8_t*)tmp_derivedKey, sizeof(tmp_derivedKey), hash);
    /*ALOGD("tmp_derivedKey:0x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x",
            tmp_derivedKey[0], tmp_derivedKey[1], tmp_derivedKey[2], tmp_derivedKey[3],
    tmp_derivedKey[4], tmp_derivedKey[5], tmp_derivedKey[6], tmp_derivedKey[7], tmp_derivedKey[8],
    tmp_derivedKey[9], tmp_derivedKey[10], tmp_derivedKey[11], tmp_derivedKey[12],
    tmp_derivedKey[13], tmp_derivedKey[14], tmp_derivedKey[15]);
    ALOGD("mDerivedKey:0x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x",
                mDerivedKey[0], mDerivedKey[1], mDerivedKey[2], mDerivedKey[3], mDerivedKey[4],
    mDerivedKey[5], mDerivedKey[6], mDerivedKey[7], mDerivedKey[8], mDerivedKey[9], mDerivedKey[10],
    mDerivedKey[11], mDerivedKey[12], mDerivedKey[13], mDerivedKey[14], mDerivedKey[15]); dump();*/
    if (ret == false) {
...
uint8_t* KeyInfo::getContentKey() { return (uint8_t*)mContentKey; }

uint64_t KeyInfo::getCount() { return mCount; }
...
uint64_t PartialEncryptHeader::getClearHeaderOffset() { return mClearHeaderOffset; }

uint64_t PartialEncryptHeader::getClearHeaderLen() { return mClearHeaderLen; }

uint64_t PartialEncryptHeader::getCipherHeaderOffset() { return mCipherHeaderOffset; }

uint64_t PartialEncryptHeader::getCipherHeaderLen() { return mCipherHeaderLen; }
...
const uint8_t Cta5FileHeader::HMAC_KEY[MD5_DIGEST_LENGTH] = {0x01, 0xFE, 0x9A, 0x50, 0x1C, 0x63,
                                                             0xA0, 0x2A, 0x21, 0x5B, 0x49, 0x31,
                                                             0xAA, 0xB5, 0xB6, 0x80};
...
void Cta5FileHeader::_init(String8 mimeType, uint64_t dataLen, String8 key,
...
uint64_t Cta5FileHeader::getContentPos() { return CTA5_FILE_HEADER_LEN; }

uint64_t Cta5FileHeader::getContentLen() { return mTotalLen - CTA5_FILE_HEADER_LEN; }

// If the CTA5 file is partial encrypted, then return true
bool Cta5FileHeader::isPartialEncryptedFile() { return mIsPartialEncrypted; }

uint64_t Cta5FileHeader::getClearHeaderOffset() {
    return mPartialEncryptHeader.getClearHeaderOffset();
}

uint64_t Cta5FileHeader::getClearHeaderLen() { return mPartialEncryptHeader.getClearHeaderLen(); }

uint64_t Cta5FileHeader::getCipherHeaderOffset() {
    return mPartialEncryptHeader.getCipherHeaderOffset();
}

uint64_t Cta5FileHeader::getCipherHeaderLen() { return mPartialEncryptHeader.getCipherHeaderLen(); }
...
uint64_t Cta5FileHeader::getCtaHeaderLen() { return CTA5_FILE_HEADER_LEN; }
...
uint8_t* Cta5FileHeader::getContentKey() { return mCurrentKeyInfo->getContentKey(); }

bool Cta5FileHeader::isKeyValid(String8 key) { return mCurrentKeyInfo->isKeyValid(key); }

KeyInfo* Cta5FileHeader::getCurrentKeyInfo() { return mCurrentKeyInfo; }

// return keyinfo array's address
KeyInfo* Cta5FileHeader::getKeyInofs() { return mKeyInfo; }
...
//////////////////
// CTA5 file format
Cta5File::Cta5File(int fd, String8 key) : mCta5FileHeader(fd, key) {
    mFd = fd;
    mNeedCancel = false;
    mCipherFd = -1;
    mClearFd = -1;
...
Cta5File::Cta5File(String8 mimeType, String8 cid __attribute__((unused)),
                   String8 dcfFlHeaders __attribute__((unused)), uint64_t datatLen, String8 key)
...
// If the CTA5 file is partial encrypted, then return true
bool Cta5File::isPartialEncryptedFile() { return mCta5FileHeader.isPartialEncryptedFile(); }

uint64_t Cta5File::getClearHeaderOffset() { return mCta5FileHeader.getClearHeaderOffset(); }

uint64_t Cta5File::getClearHeaderLen() { return mCta5FileHeader.getClearHeaderLen(); }

uint64_t Cta5File::getCipherHeaderOffset() { return mCta5FileHeader.getCipherHeaderOffset(); }

uint64_t Cta5File::getCipherHeaderLen() { return mCta5FileHeader.getCipherHeaderLen(); }

bool Cta5File::setClearHeaderOffset(uint64_t offset) {
    return mCta5FileHeader.setClearHeaderOffset(offset);
}

bool Cta5File::setClearHeaderLen(uint64_t len) { return mCta5FileHeader.setClearHeaderLen(len); }

bool Cta5File::setCipherHeaderOffset(uint64_t offset) {
    return mCta5FileHeader.setCipherHeaderOffset(offset);
}

bool Cta5File::setCipherHeaderLen(uint64_t len) { return mCta5FileHeader.setCipherHeaderLen(len); }
...
String8 Cta5File::getMimeType() { return mCta5FileHeader.getMimeType(); }

String8 Cta5File::getOriginalMimeType() { return mCta5FileHeader.getOriginalMimeType(); }
...
uint8_t* Cta5File::getIv() { return mIv; }
// Get the cta5 file's filedescriptor
int Cta5File::getFd() { return mFd; }

// When you construct a cta5 file header and want to dump it to file
// Please call this api to get header data
String8 Cta5File::getCta5FileHeader() { return mCta5FileHeader.getCtaHeader(); }

String8 Cta5File::calculateFileSig(int fd) { return this->mCta5FileHeader.calculateFileSig(fd); }

/**
 * Now it's same as getCta5FileHeader
 */
String8 Cta5File::getHeader() { return mCta5FileHeader.getCtaHeader(); }

// get the position of real content, the content is encrypted
uint64_t Cta5File::getContentPos() { return mCta5FileHeader.getContentPos(); }

uint64_t Cta5File::getContentLen() { return mCta5FileHeader.getContentLen(); }
...
int Cta5File::getClearFd() { return mClearFd; }

int Cta5File::getCipherFd() { return mCipherFd; }

void Cta5File::setClearFd(int fd) { mClearFd = fd; }

void Cta5File::setCipherFd(int fd) { mCipherFd = fd; }

uint8_t* Cta5File::getContentKey() { return mCta5FileHeader.getContentKey(); }

void Cta5File::dump() { this->mCta5FileHeader.dump(); }

int64_t Cta5File::pread(void* buf __attribute__((unused)), uint64_t size __attribute__((unused)),
