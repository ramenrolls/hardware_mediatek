...
static bool getAPEInfo(int fd, off_t* inout_pos, ape_parser_ctx_t* ape_ctx, bool parseall) {
    unsigned int i;
    ...
    while (1) {
        ...
        if (memcmp(sync, "MAC ", 4) == 0) {
            ...
            *inout_pos += 3;
            ...
            size_t len = ((sync[6] & 0x7f) << 21) | ((sync[7] & 0x7f) << 14) |
                         ((sync[8] & 0x7f) << 7) | (sync[9] & 0x7f);
    ...
    *inout_pos += 4;
    ...
    if (DrmCtaMultiMediaUtil::readAt(fd, *inout_pos, &ape_ctx->fileversion,
                                     sizeof(ape_ctx->fileversion)) < 0) {
        goto GetApeInfo_Exit;
    }

    if ((ape_ctx->fileversion > 4200) || (ape_ctx->fileversion < 3940)) {
    ...
    *inout_pos += 2;
    ...
    if (ape_ctx->fileversion >= 3980) {
        if (DrmCtaMultiMediaUtil::readAt(fd, *inout_pos, &ape_ctx->padding1,
                                         sizeof(ape_ctx->padding1)) < 0) {
            goto GetApeInfo_Exit;
        }

        *inout_pos += 2;

        if (DrmCtaMultiMediaUtil::readAt(fd, *inout_pos, &ape_ctx->descriptorlength,
                                         sizeof(ape_ctx->descriptorlength)) < 0) {
            goto GetApeInfo_Exit;
        }

        *inout_pos += 4;

        if (DrmCtaMultiMediaUtil::readAt(fd, *inout_pos, &ape_ctx->headerlength,
                                         sizeof(ape_ctx->headerlength)) < 0) {
            goto GetApeInfo_Exit;
        }

        *inout_pos += 4;

        if (DrmCtaMultiMediaUtil::readAt(fd, *inout_pos, &ape_ctx->seektablelength,
                                         sizeof(ape_ctx->seektablelength)) < 0) {
            goto GetApeInfo_Exit;
        }

        *inout_pos += 4;

        if (DrmCtaMultiMediaUtil::readAt(fd, *inout_pos, &ape_ctx->wavheaderlength,
                                         sizeof(ape_ctx->wavheaderlength)) < 0) {
            goto GetApeInfo_Exit;
        }

        *inout_pos += 4;

        if (DrmCtaMultiMediaUtil::readAt(fd, *inout_pos, &ape_ctx->audiodatalength,
                                         sizeof(ape_ctx->audiodatalength)) < 0) {
            goto GetApeInfo_Exit;
        }

        *inout_pos += 4;

        if (DrmCtaMultiMediaUtil::readAt(fd, *inout_pos, &ape_ctx->audiodatalength_high,
                                         sizeof(ape_ctx->audiodatalength_high)) < 0) {
            goto GetApeInfo_Exit;
        }

        *inout_pos += 4;

        if (DrmCtaMultiMediaUtil::readAt(fd, *inout_pos, &ape_ctx->wavtaillength,
                                         sizeof(ape_ctx->wavtaillength)) < 0) {
            goto GetApeInfo_Exit;
        }
        ...
        *inout_pos += 16;
        ...
        /* Read header data */
        if (DrmCtaMultiMediaUtil::readAt(fd, *inout_pos, &ape_ctx->compressiontype,
                                         sizeof(ape_ctx->compressiontype)) < 0) {
            goto GetApeInfo_Exit;
        }

        if (ape_ctx->compressiontype > APE_MAX_COMPRESS) {
            ALOGE("[ERROR][CTA5]getAPEInfo(Line%d): unsupported compressiontype = %u", __LINE__,
        ...
        *inout_pos += 2;
        ...
        if (DrmCtaMultiMediaUtil::readAt(fd, *inout_pos, &ape_ctx->formatflags,
                                         sizeof(ape_ctx->formatflags)) < 0) {
            goto GetApeInfo_Exit;
        }

        *inout_pos += 2;

        if (DrmCtaMultiMediaUtil::readAt(fd, *inout_pos, &ape_ctx->blocksperframe,
                                         sizeof(ape_ctx->blocksperframe)) < 0) {
            goto GetApeInfo_Exit;
        }

        *inout_pos += 4;

        if (DrmCtaMultiMediaUtil::readAt(fd, *inout_pos, &ape_ctx->finalframeblocks,
                                         sizeof(ape_ctx->finalframeblocks)) < 0) {
            goto GetApeInfo_Exit;
        }

        *inout_pos += 4;

        if (DrmCtaMultiMediaUtil::readAt(fd, *inout_pos, &ape_ctx->totalframes,
                                         sizeof(ape_ctx->totalframes)) < 0) {
            goto GetApeInfo_Exit;
        }
        ...
        *inout_pos += 2;
        ...
        if (DrmCtaMultiMediaUtil::readAt(fd, *inout_pos, &ape_ctx->channels,
                                         sizeof(ape_ctx->channels)) < 0) {
            goto GetApeInfo_Exit;
        }

        *inout_pos += 2;

        if (DrmCtaMultiMediaUtil::readAt(fd, *inout_pos, &ape_ctx->samplerate,
                                         sizeof(ape_ctx->samplerate)) < 0) {
            goto GetApeInfo_Exit;
        }

        if (ape_ctx->blocksperframe <= 0 || ape_ctx->totalframes <= 0 || ape_ctx->bps <= 0 ||
            ape_ctx->seektablelength <= 0 || ape_ctx->samplerate <= 0 ||
            ape_ctx->samplerate > 192000) {
            ALOGD("getAPEInfo header error: blocksperframe %x,totalframes %x, bps "
                  "%x,seektablelength %x, samplerate %x ",
                  ape_ctx->blocksperframe, ape_ctx->totalframes, ape_ctx->bps,
                  ape_ctx->seektablelength, ape_ctx->samplerate);
            goto GetApeInfo_Exit;
        }
        ...
        *inout_pos += 4;
        ...
        } else if ((ape_ctx->fileversion >= 3900) ||
                   (ape_ctx->fileversion >= 3800 && ape_ctx->compressiontype >= APE_MAX_COMPRESS)) {
    ...
    ape_ctx->firstframe = ape_ctx->junklength + ape_ctx->descriptorlength + ape_ctx->headerlength +
                          ape_ctx->seektablelength + ape_ctx->wavheaderlength;
    ape_ctx->seektablefilepos =
            ape_ctx->junklength + ape_ctx->descriptorlength + ape_ctx->headerlength;

    *inout_pos = ape_ctx->firstframe;

    ALOGV("getAPEInfo header info: offset %d, ape_ctx->junklength %x,ape_ctx->firstframe %x, "
          "ape_ctx->totalsamples %x,ape_ctx->fileversion %x, ape_ctx->padding1 %x ",
          file_offset, ape_ctx->junklength, ape_ctx->firstframe, ape_ctx->totalsamples,
          ape_ctx->fileversion, ape_ctx->padding1);

    ALOGV("ape_ctx->descriptorlength %x,ape_ctx->headerlength %x,ape_ctx->seektablelength "
          "%x,ape_ctx->wavheaderlength %x,ape_ctx->audiodatalength %x ",
          ape_ctx->descriptorlength, ape_ctx->headerlength, ape_ctx->seektablelength,
          ape_ctx->wavheaderlength, ape_ctx->audiodatalength);

    ALOGV("ape_ctx->audiodatalength_high %x,ape_ctx->wavtaillength %x,ape_ctx->compressiontype %x, "
          "ape_ctx->formatflags %x,ape_ctx->blocksperframe %x",
          ape_ctx->audiodatalength_high, ape_ctx->wavtaillength, ape_ctx->compressiontype,
          ape_ctx->formatflags, ape_ctx->blocksperframe);

    ALOGV("ape_ctx->finalframeblocks %x,ape_ctx->totalframes %x,ape_ctx->bps %x,ape_ctx->channels "
          "%x,ape_ctx->samplerate %x",
          ape_ctx->finalframeblocks, ape_ctx->totalframes, ape_ctx->bps, ape_ctx->channels,
          ape_ctx->samplerate);
...
Cta5APEFile::Cta5APEFile(String8 mimeType, String8 cid, String8 dcfFlHeaders, uint64_t datatLen,
                         String8 key)
