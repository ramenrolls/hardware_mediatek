...
Cta5CommonMultimediaFile::Cta5CommonMultimediaFile(String8 mimeType, String8 cid,
                                                   String8 dcfFlHeaders, uint64_t datatLen,
                                                   String8 key)
...
bool Cta5CommonMultimediaFile::encrypt(int fd_in, int fd_out __attribute__((unused)),
    ...
    for (Vector<Header*>::iterator iter = mHeaders.begin(); iter != mHeaders.end(); ++iter) {
        Header* header = (*iter);
        if (header->clear_header_offset != -1 && header->clear_header_size != -1) {
            // do ecnrypt header
            ...
            if (!encryptHeader(fd_in, header->clear_header_offset, header->clear_header_size,
    ...
    if (-1 == lseek64(fd_in, 0, SEEK_END)) {
        ALOGE("[ERROR][CTA5]encrypt - failed to seek to the end of file, reason[%s]",
              strerror(errno));
        return false;
    }
    ...
    notifyProgress(total_header_size, progress, fd_in, DrmDef::CTA5_MULTI_MEDIA_ENCRYPT_DONE,
                   infoListener);
...
bool Cta5CommonMultimediaFile::decrypt(int fd_out __attribute__((unused)),
    ...
    notifyProgress(total_header_size, progress, getFd(), DrmDef::CTA5_MULTI_MEDIA_DECRYPT_DONE,
                   infoListener);
...
int64_t Cta5CommonMultimediaFile::pread(void* buf, uint64_t size, off64_t offset) {
    ...
    for (Vector<Header*>::iterator iter = mHeaders.begin(); iter != mHeaders.end(); ++iter) {
        ...
        if (header->clear_header_offset != -1 && header->clear_header_size != -1) {
            ...
            ++count;
            // beacuse the mHeaders is sorted by offset, we only need to check offset whether is
            // less than clear header bounds
    ...
    if (position == 0LL || found_position == false) {
        // ALOGD("not found position, directly read clear buffer, offset[%lld], size[%lld]", offset,
        // size);
    ...
    while (curr_offset < offset + off_size && position <= cipherBounds.size()) {
        ...
        if (position % 2 == 1) {  // if  odd, first block is clear buffer
            ...
            ssize_t cipherLength =
                    cipherPread(fd, (uint8_t*)buf + buffer_offset, readLength,
                                curr_offset - header->clear_header_offset,
                                header->cipher_header_offset, header->cipher_header_size);
            if (cipherLength == -1) {
...
uint64_t Cta5CommonMultimediaFile::getContentPos() { return 0LL; }
...
bool Cta5CommonMultimediaFile::encryptHeader(int fd, off64_t header_offset, off64_t header_size,
    ...
    if (fd < 0 || header_offset < 0 || header_size < 0) {
        ALOGE("encryptHeader: error parameter: fd[%d], header_offset[%ld], header_size[%ld]", fd,
              (long)header_offset, (long)header_size);
        return false;
    }
    ALOGD("encryptHeader: fd[%d], header_offset[%ld], header_size[%ld]", fd, (long)header_offset,
          (long)header_size);
    ...
    if (-1 == originalEndOffset) {
        ALOGE("[ERROR][CTA5]encryptHeader: failed to seek to the end of content, reason[%s]",
              strerror(errno));
        return false;
    }
    cipher_header_offset = originalEndOffset;
    ALOGD("encryptHeader: originalEndOffset[%ld], cipher_header_offset[%ld]",
          (long)originalEndOffset, (long)cipher_header_offset);
    ...
    if (-1 == lseek64(fd, header_offset, SEEK_SET)) {
        ALOGE("[ERROR][CTA5]encryptHeader: failed to seek to the start of content, reason[%s]",
              strerror(errno));
        return false;
    }
    ...
    while (cnt_total < header_size && !mNeedCancel) {
        ...
        cnt = read(fd, buf_in,
                   ((header_size - cnt_total) > BLK_LEN) ? BLK_LEN : (header_size - cnt_total));
        cnt_total += cnt;
        // update progress
        progress += cnt;
        ...
        if (-1 == temp_offset) {
            ALOGE("[ERROR][CTA5]encryptHeader: failed to get current offset, reason[%s]",
                  strerror(errno));
            goto ERROR;
        }

        if (1 != cr_hlp.doCryption(buf_in, (int)cnt, buf_out, len_out, iv_arr,
                                   ((int)cnt_total >= header_size))) {
        ...
        if (-1 == lseek64(fd, temp_offset, SEEK_SET)) {
            ALOGE("[ERROR][CTA5]encryptHeader: failed to seek to read offset, reason[%s]",
                  strerror(errno));
            goto ERROR;
        }
    ...
    // output
    ALOGD("encryptHeader: header offset[%ld], header size[%ld], cipher header offset[%ld], cipher "
          "header size[%ld]",
          (long)header_offset, (long)header_size, (long)cipher_header_offset,
          (long)cipher_header_size);
...
bool Cta5CommonMultimediaFile::decryptHeader(off64_t header_offset, off64_t header_size,
    ...
    // check parameters
    if (fd < 0 || header_offset < 0 || header_size < 0 || cipher_header_offset < 0 ||
        cipher_header_size < 0) {
        ALOGE("[ERROR][CTA5]decryptHeader: error parameters: fd[%d], header_offset[%ld], "
              "header_size[%ld], cipher_header_offset[%ld], cipher_header_size[%ld]",
              fd, (long)header_offset, (long)header_size, (long)cipher_header_offset,
              (long)cipher_header_size);
        return false;
    }
    ALOGD("decryptHeader: parameters: fd[%d], header_offset[%ld], header_size[%ld], "
          "cipher_header_offset[%ld], cipher_header_size[%ld]",
          fd, (long)header_offset, (long)header_size, (long)cipher_header_offset,
          (long)cipher_header_size);
    ...
    while (readSize <= cipher_header_size && !mNeedCancel) {
        ...
        ssize_t size = (cipher_header_size - readSize) > DECRYPT_BUF_LEN
                               ? DECRYPT_BUF_LEN
                               : (cipher_header_size - readSize);
        ALOGD("decryptHeader: preare to decrypt[%d]", (int)size);
        ssize_t readLength =
                cipherPread(fd, buffer, size, resultSize, cipher_header_offset, cipher_header_size);
        if (readLength <= 0) {
        ...
        if (-1 == lseek64(fd, header_offset + resultSize, SEEK_SET)) {
            ALOGE("[ERROR][CTA5]decryptHeader: failed to seek to header offset, reason[%s]",
                  strerror(errno));
            return false;
        }
        ...
        if (result != readLength) {
            ALOGE("[ERROR][CTA5]decryptHeader: failed to write data to output file. "
                  "readLength[%d], result[%d]",
                  (int)readLength, (int)result);
            return false;
        }
    ...
    if (resultSize <= 0) {
        ALOGE("[ERROR][CTA5]decryptHeader: failed to read decrypted data. resultSize[%ld]",
              (long)resultSize);
        return false;
    }
...
void Cta5CommonMultimediaFile::notifyProgress(off64_t total, off64_t progress, int fd,
                                              String8 result,
                                              const Vector<DrmCtaUtil::Listener>* infoListener) {
...
bool Cta5CommonMultimediaFile::recoverClearData(int fd, off64_t original_end_offset) {
    ...
    if (-1 == ftruncate64(fd, original_end_offset)) {
        ALOGE("[ERROR][CTA5]recover: failed to recover, offset[%ld], reason[%s]",
              (long)original_end_offset, strerror(errno));
        return false;
    }
    return true;
}
...
bool Cta5CommonMultimediaFile::recoverCipherData(int fd, off64_t header_offset, off64_t header_size,
    ...
    return overwriteClearHeader(fd, header_offset, header_size, cipher_header_offset,
                                cipher_header_size);
}

// for data protection, use cipher to overwrite clear header
bool Cta5CommonMultimediaFile::overwriteClearHeader(int fd, off64_t header_offset,
                                                    off64_t header_size,
                                                    off64_t cipher_header_offset,
                                                    off64_t cipher_header_size
    ...
    while (readSize <= header_size) {
        bzero(buffer, sizeof(buffer));
        ssize_t size = (header_size - readSize) > DECRYPT_BUF_LEN ? DECRYPT_BUF_LEN
                                                                  : (header_size - readSize);
        ...
        if (-1 == cipher_offset) {
            ALOGE("[ERROR][CTA5]overwriteClearHeader: get currnt cipher read offset fail, "
                  "reason[%s]",
                  strerror(errno));
            return false;
        }
        ...
        if (-1 == lseek64(fd, header_offset + resultSize, SEEK_SET)) {
            ALOGE("[ERROR][CTA5]overwriteClearHeader: failed to seek to header offset, reason[%s]",
                  strerror(errno));
            return false;
        }
        ...
        ssize_t result = write(fd, buffer, readLength);
        if (result != readLength) {
            ALOGE("[ERROR][CTA5]overwriteClearHeader: failed to write data to output file. "
                  "readLength[%d], result[%d]",
                  (int)readLength, (int)result);
            return false;
        }
...
String8 Cta5CommonMultimediaFile::getCtaMultimediaHeader() {
    ...
    ALOGD("getCtaMultimediaHeader - count: 0x%02x, 0x%02x, 0x%02x, 0x%02x", ((uint8_t*)&count)[0],
          ((uint8_t*)&count)[1], ((uint8_t*)&count)[2], ((uint8_t*)&count)[3]);
...
ssize_t Cta5CommonMultimediaFile::cipherPread(int fd, void* buffer, ssize_t numBytes,
                                              off64_t offset, off64_t cipher_header_offset,
                                              off64_t cipher_header_size) {
    ...
    if (NULL != buffer && numBytes > -1 && offset > -1) {
        ...
        bytesRead = CipherFileUtil::CipherFileRead(
                fd, buffer, numBytes, offset, cipher_header_offset, cipher_header_size, cr_hlp);
...
bool Cta5CommonMultimediaFile::changeKey(String8 oldKey, String8 newKey) {
    ALOGD("Cta5CommonMultimediaFile changeKey oldkey = %s, newKey = %s", oldKey.string(),
          newKey.string());
