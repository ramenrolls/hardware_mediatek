...
static uint16_t U16_LE_AT(const uint8_t* ptr) { return ptr[1] << 8 | ptr[0]; }
...
Cta5WAVFile::Cta5WAVFile(String8 mimeType, String8 cid, String8 dcfFlHeaders, uint64_t datatLen,
                         String8 key)
...
bool Cta5WAVFile::parseHeaders(int fd) {
    ALOGD("parseHeaders()");
    uint8_t header[12];
    if (DrmCtaMultiMediaUtil::readAt(fd, 0, header, sizeof(header)) < (ssize_t)sizeof(header)) {
        return false;
    }
    ...
    while (remainingSize >= 8) {
        ...
        if (!memcmp(chunkHeader, "fmt ", 4)) {
            ...
            if (mWaveFormat != WAVE_FORMAT_PCM && mWaveFormat != WAVE_FORMAT_ALAW &&
                mWaveFormat != WAVE_FORMAT_MULAW
#ifdef AUDIO_ADPCM_SUPPORT
                && mWaveFormat != WAVE_FORMAT_MSADPCM && mWaveFormat != WAVE_FORMAT_DVI_IMAADCPM
#endif
            ...
            if (mWaveFormat == WAVE_FORMAT_PCM || mWaveFormat == WAVE_FORMAT_EXTENSIBLE) {
...
#ifdef AUDIO_ADPCM_SUPPORT
            else if (mWaveFormat == WAVE_FORMAT_MSADPCM ||
                ...
                if (mBitsPerSample != 0) return false;
            } else {
                if (!(mWaveFormat == WAVE_FORMAT_MULAW || mWaveFormat == WAVE_FORMAT_ALAW)) {
                    return false;
                }
                if (mBitsPerSample != 8) {
                    return false;
            ...
            if (mWaveFormat == WAVE_FORMAT_MSADPCM || mWaveFormat == WAVE_FORMAT_DVI_IMAADCPM) {
                ...
                if (NULL == mpExtraData) {
                    ...
                    if (DrmCtaMultiMediaUtil::readAt(fd, offset + 18, mpExtraData, mExtraDataSize) <
                        mExtraDataSize) {
                        return false;
                    }
                }
            ...
            ALOGD("fact chunk mChannelCount is %d, mSamplesNumberPerChannel is %d, "
                  "mSamplesPerBlock is %d",
