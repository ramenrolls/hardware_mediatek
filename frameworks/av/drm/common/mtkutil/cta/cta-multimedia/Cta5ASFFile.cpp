...
static const uint8_t ASF_HEADER_OBJECT_ID[16] = {0x30, 0x26, 0xB2, 0x75, 0x8E, 0x66, 0xCF, 0x11,
...
Cta5ASFFile::Cta5ASFFile(String8 mimeType, String8 cid, String8 dcfFlHeaders, uint64_t datatLen,
                         String8 key)
...
bool Cta5ASFFile::parseHeaders(int fd) {
    ...
    if (DrmCtaMultiMediaUtil::readAt(fd, 16, (uint8_t*)&header_size, sizeof(header_size)) !=
        sizeof(header_size)) {
        ALOGE("[ERROR][CTA5]failed to read header at 0x%08x", 16);
        return false;
    }
    ALOGD("parseASFHeader - header_offset[%ld], header_size[%ld]", (long)header_offset,
          (long)header_size);
