...
#define SEC_1900_TO_1970 \
    0x83aa7e80U  // seconds between 1900/1/1 - 1970/1/1
...
const char* NTP_SERVER_ADDR[NTP_SERVER_CNT] = {
        DrmSntpServer::NTP_SERVER_1, DrmSntpServer::NTP_SERVER_2, DrmSntpServer::NTP_SERVER_3,
        DrmSntpServer::NTP_SERVER_4, DrmSntpServer::NTP_SERVER_5};

long32 send_packet(int fd_socket) {
    uint32 data[12];
    memset(data, 0, sizeof(data));

    data[0] = htonl((LI << 30) | (VN << 27) | (MODE << 24) | (STRATUM << 16) | (POLL << 8) |
                    (PREC & 0xff));
    data[1] = htonl(1 << 16);
    data[2] = htonl(1 << 16);
...
long32 get_time_diff(uint32* data, long32 ref, long32 ori) {
    ...
    long32 diff = (long32)(((long64)recv_time.coarse - (ori + SEC_1900_TO_1970) + tran_time.coarse -
                            (ref + SEC_1900_TO_1970)) /
                           2);
#ifdef _DRM_ST_SNTP_DEBUG
...
void print_curr_time(TimeValue tv __attribute__((unused))) {
    ...
    printf("current time: [%d-%d-%d] [%d:%d.%d] \n", s_time.tm_year + 1900, s_time.tm_mon + 1,
           s_time.tm_mday, s_time.tm_hour, s_time.tm_min, s_time.tm_sec);
#endif
}
