...
int CipherFileUtil::CipherFileRead(int fd, void* buf, int byte_num, int offset, int data_offset,
                                   int data_size, CryptoHelper& hlp) {
    if (fd < 0 || NULL == buf || byte_num < 0 || offset < 0 || data_offset < 0 || data_size < 0 ||
        (data_size % DrmDef::DRM_DCF_BLK_LENGTH) != 0) {
        ALOGE("CipherFileRead: error parameter: file_d[%d], buf[%p], byte_num[%d], offset[%d], "
              "data_offset[%d], data_size[%d]",
              fd, buf, byte_num, offset, data_offset, data_size);
        return -1;
    }
    if (sDebug)
        ALOGV("CipherFileRead: file_d[%d], buf[%p], byte_num[%d], offset[%d], data_offset[%d], "
              "data_size[%d]",
              fd, buf, byte_num, offset, data_offset, data_size);
    ...
    if (-1 == lseek(fd, data_offset + blk_begin * DrmDef::DRM_DCF_BLK_LENGTH, SEEK_SET)) {
        ALOGE("CipherFileRead: failed to seek to [%d+%d*%d]!", data_offset, blk_begin,
              DrmDef::DRM_DCF_BLK_LENGTH);
        return -1;
    }
    ...
    if (-1 ==
        lseek(fd, data_offset + DrmDef::DRM_DCF_IV_LENGTH + blk_begin * DrmDef::DRM_DCF_BLK_LENGTH,
              SEEK_SET)) {
        ALOGE("CipherFileRead: failed to seek to [%d+%d+%d*%d]!", data_offset,
              DrmDef::DRM_DCF_IV_LENGTH, blk_begin, DrmDef::DRM_DCF_BLK_LENGTH);
        delete[] temp;
        return -1;
    }
    ...
    if (1 == ret)  // decryption success
    {
        if (length > delta_begin) {
            ...
            memcpy(buf, out + delta_begin,
                   (size_t)bytesRead);  // the {buf} should be large enough!!
...
int CipherFileUtil::CipherFileRead(char* data, void* buf, int byte_num, int offset, int data_offset,
                                   int data_size, CryptoHelper& hlp) {
    ALOGD("CipherFileRead: enter data[%p] data_offset[%d] data_size[%d]", data, data_offset,
          data_size);
    if (data == NULL || NULL == buf || byte_num < 0 || offset < 0 || data_offset < 0 ||
        data_size < 0 || (data_size % DrmDef::DRM_DCF_BLK_LENGTH) != 0) {
        ALOGE("CipherFileRead: error parameter: data[%p], buf[%p], byte_num[%d], offset[%d], "
              "data_offset[%d], data_size[%d]",
              data, buf, byte_num, offset, data_offset, data_size);
        return -1;
    }
    ALOGD("CipherFileRead: data[%p], buf[%p], byte_num[%d], offset[%d], data_offset[%d], "
          "data_size[%d]",
          data, buf, byte_num, offset, data_offset, data_size);
    ...
    ALOGD("iv bytes: %x %x %x %x %x %x", curIndex[0], curIndex[1], curIndex[2], curIndex[3],
          curIndex[4], curIndex[5]);
    ...
    {
        ALOGE("CipherFileRead: failed to seek to [%d+%d*%d]!", data_offset, blk_begin,
    DrmDef::DRM_DCF_BLK_LENGTH); return -1;
    }
    if (DrmDef::DRM_DCF_IV_LENGTH != read(fd, iv, sizeof(iv)))
    {
        ALOGE("CipherFileRead: failed to read [%d] bytes iv data!", DrmDef::DRM_DCF_BLK_LENGTH);
    ...
    curIndex = (data + data_offset + DrmDef::DRM_DCF_IV_LENGTH +
                blk_begin * DrmDef::DRM_DCF_BLK_LENGTH);

    ALOGD("CipherFileRead: curIndex [%p]", curIndex);
    if (curIndex > (data + data_offset + data_size) ||
        (curIndex + byte_cnt) > (data + data_offset + data_size)) {
        ALOGD("CipherFileRead: error, out of data boundary, curIndex[%p] > %p, byte_cnt[%d]",
              curIndex, (data + data_offset + data_size), byte_cnt);
        delete[] temp;
        return -1;
    }
...
// deprecated
int CipherFileUtil::CipherFileDump(String8 file_name, int fd, int data_offset, int data_size,
                                   CryptoHelper& hlp) {
...
int CipherFileUtil::CipherFileInstall(FILE* origin, int data_offset, int data_size, FILE* out,
    ...
    if (NULL == origin || NULL == out || NULL == iv || data_offset < 0 || data_size < 0) {
        ALOGE("CipherFileInstall: error parameter: origin[%p], out[%p], iv[%p], data_offset[%d], "
              "data_size[%d]",
              origin, out, iv, data_offset, data_size);
        return -1;
    }
    if (sDebug)
        ALOGV("CipherFileInstall: origin[%p], out[%p], iv[%p], data_offset[%d], data_size[%d]",
              origin, out, iv, data_offset, data_size);
    ...
    while ((int)cnt_total < data_size) {
        ...
        cnt = fread(buf_in, sizeof(unsigned char),
                    ((data_size - cnt_total) > BLK_LEN) ? BLK_LEN : (data_size - cnt_total),
                    origin);
        cnt_total += cnt;

        if (1 != hlp.doCryption(buf_in, (int)cnt, buf_out, len_out, iv_arr,
                                ((int)cnt_total >= data_size))) {
