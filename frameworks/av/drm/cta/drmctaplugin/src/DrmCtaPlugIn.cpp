...
extern "C" IDrmEngine* create() { return new DrmCtaPlugIn(); }
...
status_t DrmCtaPlugIn::onSetOnInfoListener(int uniqueId,
    ...
    DrmCtaUtil::Listener newListener =
            DrmCtaUtil::Listener(const_cast<IDrmEngine::OnInfoListener*>(infoListener), uniqueId);
    bool found = false;
...
bool DrmCtaPlugIn::onCanHandle(int uniqueId, const String8& path) {
    if (DrmUtil::sDDebug)
        ALOGD("onCanHandle() uniqueId: [%d], file path: %s ", uniqueId, path.string());
    int fd_in = open(path.string(), O_RDONLY);
    if (-1 == fd_in) {
        if (DrmUtil::sDDebug)
            ALOGE("[ERROR]onCanHandle() : failed to open file, reason [%s]", strerror(errno));
        return false;
    }
...
status_t DrmCtaPlugIn::onSaveRights(int uniqueId __attribute__((unused)),
                                    ...
                                    const String8& rightsPath __attribute__((unused)),
                                    const String8& contentPath __attribute__((unused))) {
    return DRM_NO_ERROR;
}
    ...
    if (NULL != drmInfoRequest) {
        ...
        if (DrmRequestType::TYPE_SET_DRM_INFO == type) {
            ...
            if (actionResult == true) {
                ...
                String8 errorMsg =
                        DrmCtaUtil::getErrorCallbackMsg(String8("error"), DrmDef::CTA5_ERROR);
            ...
            if (DrmRequestType::ACTION_CTA5_GETTOKEN == action) {
                ...
                result = isTokenValid(drmInfoRequest) ? DrmRequestType::RESULT_SUCCESS
                                                      : DrmRequestType::RESULT_FAILURE;
            } else if (DrmRequestType::ACTION_CTA5_CLEARTOKEN == action) {
                result = clearToken(drmInfoRequest) ? DrmRequestType::RESULT_SUCCESS
                                                    : DrmRequestType::RESULT_FAILURE;
            }
        }
        ...
        // ALOGD("DrmCtaPlugIn hongen::onAcquireDrmInfo : mime type=[%s]",
...
String8 DrmCtaPlugIn::onGetOriginalMimeType(int uniqueId, const String8& path, int fd) {
    if (DrmUtil::sDDebug)
        ALOGD("onGetOriginalMimeType() rui [%d],fd = %d, path = %s", uniqueId, fd, path.string());
    String8 mime;
    /*if (mKey.isEmpty())
    {
...
int DrmCtaPlugIn::onGetDrmObjectType(int uniqueId __attribute__((unused)),
                                     const String8& path __attribute__((unused)),
...
status_t DrmCtaPlugIn::onSetPlaybackStatus(int uniqueId __attribute__((unused)),
                                           sp<DecryptHandle>& decryptHandle __attribute__((unused)),
                                           int playbackStatus __attribute__((unused)),
                                           int64_t position __attribute__((unused))) {
    // if (DrmUtil::sDDebug) ALOGD("DrmCtaPlugIn::onSetPlaybackStatus playbackStatus =
    // %d,position=%lld", playbackStatus,position);
    return DRM_NO_ERROR;
}

bool DrmCtaPlugIn::onValidateAction(int uniqueId __attribute__((unused)),
                                    const String8& path __attribute__((unused)),
                                    int action __attribute__((unused)),
...
status_t DrmCtaPlugIn::onRemoveRights(int uniqueId __attribute__((unused)),
                                      const String8& path __attribute__((unused))) {
    return DRM_NO_ERROR;
}
...
status_t DrmCtaPlugIn::onOpenConvertSession(int uniqueId __attribute__((unused)),
                                            int convertId __attribute__((unused))) {
    return DRM_NO_ERROR;
}

DrmConvertedStatus* DrmCtaPlugIn::onConvertData(int uniqueId __attribute__((unused)),
                                                int convertId __attribute__((unused)),
...
DrmConvertedStatus* DrmCtaPlugIn::onCloseConvertSession(int uniqueId __attribute__((unused)),
                                                        int convertId __attribute__((unused))) {
    return NULL;
}
...
status_t DrmCtaPlugIn::onOpenDecryptSession(int uniqueId, sp<DecryptHandle>& decryptHandle, int fd,
    ...
    if (DrmUtil::sDDebug)
        ALOGD("onOpenDecryptSession() [%d], fd = %d, offset = %ld, length = %ld", uniqueId, fd,
    ...
    if (decryptHandle) {
        if (fd < 0) {
            if (DrmUtil::sDDebug)
                ALOGE("[ERROR]onOpenDecryptSession() : invalid input file descriptor [%d]", fd);
            return DRM_ERROR_CANNOT_HANDLE;
        }
        ...
        if (!m_decryptSessionMap.isCreated(decryptHandle->decryptId)) {
            ...
            if (-1 == fileDesc) {
                if (DrmUtil::sDDebug)
                    ALOGE("[ERROR]Failed to duplicate the file descriptor, reason [%s]",
                          strerror(errno));
                return DRM_ERROR_CANNOT_HANDLE;
            }
        } else {
            if (DrmUtil::sDDebug)
                ALOGE("[ERROR]onOpenDecryptSession() : decrypt session map error.");
            return DRM_ERROR_CANNOT_HANDLE;
        }
        ...
        if (processName != "com.mediatek.dataprotection") {
            ...
            if (index < 0 || index >= mTokenCheckedMap.size()) {
                if (DrmUtil::sDDebug)
                    ALOGD("onOpenDecryptSession check token don't find path:%s, convert path and "
                          "try again",
                          filePath.string());
                if (0 ==
                    strncasecmp(filePath.string(), mntMediaPath.string(), mntMediaPath.length())) {
                    filePath = String8("file:///storage") +
                               String8(filePath.string() + mntMediaPath.length());
                    index = mTokenCheckedMap.indexOfKey(filePath);
                } else if (0 == strncasecmp(filePath.string(), dataMediaPath.string(),
                                            dataMediaPath.length())) {
                    filePath = String8("file:///storage/emulated") +
                               String8(filePath.string() + dataMediaPath.length());
                    index = mTokenCheckedMap.indexOfKey(filePath);
                }
            }
            if (index < 0 || index >= mTokenCheckedMap.size()) {
                if (DrmUtil::sDDebug)
                    ALOGE("onOpenDecryptSession check token don't find path:%s", filePath.string());
                return DRM_ERROR_CANNOT_HANDLE;
            }
        }
        ...
        //        decryptHandle->decryptInfo->decryptBufferLength = totalSize -
        //        pCta5File->getHeader().length();
        ...
        if (DrmUtil::sDDebug)
            ALOGE("[ERROR]onOpenDecryptSession() : invalid decryptHandle [%p]",
                  decryptHandle.get());
    }
...
status_t DrmCtaPlugIn::onCloseDecryptSession(int uniqueId __attribute__((unused)),
                                             sp<DecryptHandle>& decryptHandle) {
    if (decryptHandle) {
...
status_t DrmCtaPlugIn::onDecrypt(int uniqueId __attribute__((unused)),
                                 sp<DecryptHandle>& decryptHandle __attribute__((unused)),
                                 int decryptUnitId __attribute__((unused)),
                                 const DrmBuffer* encBuffer __attribute__((unused)),
                                 DrmBuffer** decBuffer __attribute__((unused)),
                                 DrmBuffer* IV __attribute__((unused))) {
    return DRM_NO_ERROR;
}
...
ssize_t DrmCtaPlugIn::onPread(int uniqueId __attribute__((unused)),
                              sp<DecryptHandle>& decryptHandle, void* buffer, ssize_t numBytes,
                              off64_t offset) {
...
bool DrmCtaPlugIn::encrypt(const DrmInfoRequest* drmInfoRequest) {
    ...
    // ALOGD( "clear_fd_str : %s, cipher_fd_str : %s", clear_fd_str.string(),
    // cipher_fd_str.string()); ALOGD("clear_fd : %d, cipher_fd : %d", clear_fd, cipher_fd); content
    // length

    if (-1 == lseek(clear_fd, 0, SEEK_SET)) {
        if (DrmUtil::sDDebug)
            ALOGE("[ERROR]encrypt() : failed to seek set clear_fd, reason [%s]", strerror(errno));
        return false;
    }
    int contentLength = lseek(clear_fd, 0, SEEK_END);
    if (-1 == contentLength) {
        if (DrmUtil::sDDebug)
            ALOGE("[ERROR]encrypt() : failed to seek end clear_fd, reason [%s]", strerror(errno));
        return false;
    }
    ...
    if (-1 == dupClearFd) {
        if (DrmUtil::sDDebug)
            ALOGE("[ERROR]encrypt() : failed to dup clear_fd, reason [%s]", strerror(errno));
        return false;
    }
    int dupCipherFd = dup(cipher_fd);
    if (-1 == dupCipherFd) {
        if (DrmUtil::sDDebug)
            ALOGE("[ERROR]encrypt() : failed to dup cipher_fd, reason [%s]", strerror(errno));
        close(dupClearFd);
        return false;
    }
    ...
    Cta5File* pCta5File =
            Cta5FileFactory::createCta5File(file_mime, contentLength, mKey, isDcfFile);
    if (pCta5File == NULL) {
...
bool DrmCtaPlugIn::decryptInfR(const DrmInfoRequest* drmInfoRequest) {
    ...
    if (dupClearFd == -1) {
        if (DrmUtil::sDDebug)
            ALOGE("[ERROR]decrypt - dup clear_fd failed, because %s", strerror(errno));
        return false;
    }
    int dupCipherFd = dup(cipher_fd);
    if (dupCipherFd == -1) {
        if (DrmUtil::sDDebug)
            ALOGE("[ERROR]decrypt - dup cipher_fd failed, because %s", strerror(errno));
        close(dupClearFd);
        return false;
    }
...
bool DrmCtaPlugIn::cancel(const DrmInfoRequest* drmInfoRequest) {
    ...
    if (canceled == false) {
        ...
        for (Vector<Cta5File*>::iterator iter = mAvailEncryptFiles.begin();
            ...
            Cta5File* PCta5File = (*iter);
            // ALOGD( "hongen cancel encrypt, orin path:%s",
            // Cta5FileUtil::getPathFromFd(PCta5File->getClearFd()).string());
...
bool DrmCtaPlugIn::isCta5File(const DrmInfoRequest* drmInfoRequest) {
    ...
    if (path_in != -1) {
        ...
        if (DrmUtil::sDDebug)
            ALOGW("[Warning] open file[%s] failed, reason: %s", path_str.string(), strerror(errno));
    }
    return result;
}
...
bool DrmCtaPlugIn::createEncryptThread() {
    ...
    if (ret) {
        ...
        if (DrmUtil::sDDebug)
            ALOGE("[ERROR]createEncryptThread() : createEncryptThread error =[%d]", ret);
        return false;
    }
...
bool DrmCtaPlugIn::createDecryptThread() {
    ...
    if (ret) {
        ...
        if (DrmUtil::sDDebug)
            ALOGE("[ERROR]createDecryptThread() : createDecryptThread error =[%d]", ret);
        return false;
    }
...
bool DrmCtaPlugIn::isTokenValid(const DrmInfoRequest* drmInfoRequest) {
    ...
    if (isTrustedClient == false) {
        if (DrmUtil::sDDebug)
            ALOGE("[ERROR]isTokenValid - Not a trusted client: %s", processName.string());
        return false;
    }
    if (mKey.isEmpty()) {
...
bool DrmCtaPlugIn::clearToken(const DrmInfoRequest* drmInfoRequest) {
    ...
    if (isTrustedClient == false) {
        if (DrmUtil::sDDebug)
            ALOGE("[ERROR]clearToken - Not a trusted client: %s", processName.string());
        return false;
    }
    if (mKey.isEmpty()) {
