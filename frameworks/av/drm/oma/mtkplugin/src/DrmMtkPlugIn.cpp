...
extern "C" IDrmEngine* create() { return new DrmMtkPlugIn(); }
...
DrmMtkPlugIn::DrmMtkPlugIn() : DrmEngineBase() {
...
void parseConctraints(Entry* entryPtr, DrmConstraints* drmConstraints) {
    ...
    if (DrmUtil::sDDebug)
        ALOGD("parseConctraints() : total_count: %ld, used_count: %ld", entry.total_count,
              entry.used_count);
    // Since count type rights is at the last item
    // So there is no need to get count info from drmConstraints
    long max_repeat_count =
            str2long(drmConstraints->get(DrmConstraints::MAX_REPEAT_COUNT).string());
    long used_count =
            str2long(drmConstraints->get(DrmConstraints::REMAINING_REPEAT_COUNT).string());
    ...
    if (max_repeat_count > 0 && used_count > 0) {
        ...
        drmConstraints->put(&(DrmConstraints::REMAINING_REPEAT_COUNT),
                            StrUtil::toString(used_count));
    }
    ...
    if (DrmUtil::sDDebug)
        ALOGD("parseConctraints() : start_time %ld, start_intv %ld", entry.start_time,
              entry.start_intv);
    ...
    if (entry.start_time != min || entry.start_intv != 0) {
        ...
        if (DrmUtil::sDDebug)
            ALOGD("parseConctraints() : put start_time: %ld = %ld - %ld", start - offset, start,
                  offset);
    ...
    if (DrmUtil::sDDebug)
        ALOGD("parseConctraints() : end_time %ld, end_intv %ld", entry.end_time, entry.end_intv);
    ...
    if (entry.end_time != max || entry.end_intv != 0) {
        ...
        if (DrmUtil::sDDebug)
            ALOGD("parseConctraints() : put end_time: %ld = %ld - %ld", end - offset, end, offset);
...
DrmConstraints* DrmMtkPlugIn::onGetConstraints(int uniqueId, const String8* path, int action) {
...
DrmMetadata* DrmMtkPlugIn::onGetMetadata(int uniqueId, const String8* path) {
    ...
    if (m_bFwdLockOnly && method != StrUtil::toString(DrmDef::METHOD_FL)) {
        if (DrmUtil::sDDebug)
            ALOGD("onGetMetadata() : Forward-lock-only is set, and it's not FL type");
        delete drmMetadata;
        return NULL;
    }
...
bool DrmMtkPlugIn::onCanHandle(int uniqueId, const String8& path) {
    ...
    extString.toLower();
    return (extString == DrmDef::EXT_DRM_CONTENT || extString == DrmDef::EXT_DRM_MESSAGE ||
            extString == DrmDef::EXT_RIGHTS_XML || extString == DrmDef::EXT_RIGHTS_WBXML);
}

DrmInfoStatus* DrmMtkPlugIn::onProcessDrmInfo(int uniqueId, const DrmInfo* drmInfo) {
    ...
    if (DrmRequestType::TYPE_GET_DRM_INFO == drmInfo->getInfoType()) {
        ...
        if (actionType == ACTION_GET_CONTENT_ID) {
            ...
            if (DrmUtil::sDDebug)
                ALOGD("DrmMtkPlugIn::onProcessDrmInfo - get content-id: %s", cid.string());
            ...
            drmInfoStatus = new DrmInfoStatus(DrmInfoStatus::STATUS_OK, drmInfo->getInfoType(),
                                              valueBuffer, drmInfo->getMimeType());
        }
    }
...
status_t DrmMtkPlugIn::onSaveRights(int uniqueId, const DrmRights& drmRights,
    ...
    if (DrmUtil::sDDebug)
        ALOGD("SaveRights() [%d] r[%s] c[%s]", uniqueId, rightsPath.string(), contentPath.string());
    // "Forward Lock Only"
    if (m_bFwdLockOnly) {
        if (DrmUtil::sDDebug)
            ALOGD("onSaveRights() : Forward-lock-only is set, and don't save any rights.");
        return DRM_NO_ERROR;
    }
    ...
    if (DrmUtil::checkExistence(roPath)) {
        ...
        if (DrmUtil::sDDebug)
            ALOGD("onSaveRights() : the old RO file: %s existed, merge it.", roPath.string());
        ...
        if (DrmUtil::sDDebug)
            ALOGD("onSaveRights() : the old RO file: %s didn't exist, save it.", roPath.string());
...
DrmInfo* DrmMtkPlugIn::onAcquireDrmInfo(int uniqueId, const DrmInfoRequest* drmInfoRequest) {
    ...
    if (NULL != drmInfoRequest) {
        ...
        if (DrmUtil::sDDebug)
            ALOGD("DrmMtkPlugIn::onAcquireDrmInfo : execute action %s", action.string());

        // "SET_DRM_INFO" is used when it requires plugin to make some opeartion with given
        // information
        if (DrmRequestType::TYPE_SET_DRM_INFO == type) {
            String8 result;  // the result of the operation
            if (DrmRequestType::ACTION_INSTALL_DRM_MSG ==
                action)  // should remove after download check in.
            {
                // shall be an valid path
                String8 dmPath = drmInfoRequest->get(DrmRequestType::KEY_DATA);
                result = DrmMtkUtil::installDrmMsg(dmPath) ? DrmRequestType::RESULT_SUCCESS
            ...
            if (ACTION_INSTALL_DRM_MSG_BY_FD == action) {
                if (DrmUtil::sDDebug)
                    ALOGD("DrmMtkPlugIn::onAcquireDrmInfo() install drm msg by fd");
                ...
                if (DrmUtil::sDDebug)
                    ALOGD("dm_fd : %s, dcf_fd : %s", dm_fd_str.string(), dcf_fd_str.string());
                if (DrmUtil::sDDebug) ALOGD("dm_fd : %d, dcf_fd : %d", dm_fd, dcf_fd);
                result = DrmMtkUtil::installDrmMsg(dm_fd, dcf_fd) ? DrmRequestType::RESULT_SUCCESS
                                                                  : DrmRequestType::RESULT_FAILURE;
            }
            ...
            if (DrmRequestType::ACTION_CONSUME_RIGHTS == action) {
                ...
                result = (DRM_NO_ERROR == res) ? DrmRequestType::RESULT_SUCCESS
                                               : DrmRequestType::RESULT_FAILURE;
            }
            ...
            if (DrmRequestType::ACTION_SHOW_DRM_DIALOG_IF_NEED == action) {
                ...
                if (-1 == lseek(fd, 0, SEEK_SET)) {
                    ...
                    if (data != MAP_FAILED) {
                        ...
                        if (DrmUtil::sDDebug)
                            ALOGD("onAcquireDrmInfo can't map shared fd with error: %s",
                                  strerror(errno));
                    }
                } else {
                ...
                if (parseResult == 0) {
                    // Check rights status, when status is invalid, all can not open decryption
                    // success except decode image just for bound(height and width).
                    ...
                    String8 rightsIssuer =
                            pMetaData->get(String8(DrmMetaKey::META_KEY_RIGHTS_ISSUER));
                    ...
                    if (status != RightsStatus::RIGHTS_VALID) {
                        ...
                        ALOGD("onOpenDecryptSession() : start drm dialog for filepath: %s",
                              path.string());
                        startDrmDialogService(String16(cid), String16(rightsIssuer),
                                              String16(method), status, String16(path));
                        if (DrmUtil::sDDebug)
                            ALOGD("onOpenDecryptSession() : dialog for file: %s", cid.string());
                    }
                }
                delete pMetaData;
                ...
                if (DrmUtil::sDDebug)
                    ALOGD("DrmMtkPlugIn::onAcquireDrmInfo : mark %s as consume in app client for "
                          "%s",
                ...
                if (DrmUtil::sDDebug)
                    ALOGD("DrmMtkPlugIn::onAcquireDrmInfo : update clock with offset %ld", offset);

                if (offset == INVALID_OFFSET) {
                    if (DrmUtil::sDDebug)
                        ALOGD("DrmMtkPlugIn::onAcquireDrmInfo - invalid offset, do not update "
                              "clock");
                ...
                if (DrmUtil::sDDebug)
                    ALOGD("DrmMtkPlugIn::onAcquireDrmInfo : check rightsStatus[%d] by fd[%d]",
                          status, fd);
            }

            if (DrmRequestType::ACTION_UPDATE_OFFSET == action) {
                int res = SecureTimer::instance().updateOffset();
                result = (SecureTimer::RESULT_OK == res) ? DrmRequestType::RESULT_SUCCESS
                                                         : DrmRequestType::RESULT_FAILURE;
            }
            ...
            if (DrmRequestType::ACTION_SAVE_DEVICE_ID == action) {
                ...
                if (DrmUtil::sDDebug)
                    ALOGD("DrmMtkPlugIn::onAcquireDrmInfo : save device id %s", idValue.string());
                int res = DrmUtil::saveId(idValue);
                result = (0 == res) ? DrmRequestType::RESULT_SUCCESS
                                    : DrmRequestType::RESULT_FAILURE;
            }

            if (DrmUtil::sDDebug)
                ALOGD("DrmMtkPlugIn::onAcquireDrmInfo : result of SET_DRM_INFO operation %s",
                      result.string());
        ...
        // "GET_DRM_INFO" is used when that it need to get information for further use
            ...
            if (DrmRequestType::ACTION_CHECK_SECURE_TIME == action) {
                ...
                if (DrmUtil::sDDebug)
                    ALOGD("DrmMtkPlugIn::onAcquireDrmInfo : check secure timer status %s",
                          result.string());
            }
            ...
            if (DrmRequestType::ACTION_LOAD_DEVICE_ID == action) {
                ...
                if (DrmUtil::sDDebug)
                    ALOGD("DrmMtkPlugIn::onAcquireDrmInfo : load device id %s", result.string());
            }

            if (DrmUtil::sDDebug)
                ALOGD("DrmMtkPlugIn::onAcquireDrmInfo : result of GET_DRM_INFO operation %s",
                      result.string());
            // the result value is passed back to caller side.
            int length = result.length();
            char* data = new char[length];
...
int DrmMtkPlugIn::onGetDrmObjectType(int uniqueId, const String8& path, const String8& mimeType) {
    if (DrmUtil::sDDebug) ALOGD("onGetDrmObjectType() [%d]", uniqueId);

    if (mimeType.length() > 0) {
        if (mimeType == DrmDef::MIME_RIGHTS_XML || mimeType == DrmDef::MIME_RIGHTS_WBXML) {
            return DrmObjectType::RIGHTS_OBJECT;
        } else if (mimeType == DrmDef::MIME_DRM_CONTENT || mimeType == DrmDef::MIME_DRM_MESSAGE) {
    ...
    if (path.length() > 0 && onCanHandle(uniqueId, path)) {
        ...
        if (ext == DrmDef::EXT_RIGHTS_XML || ext == DrmDef::EXT_RIGHTS_WBXML) {
            return DrmObjectType::RIGHTS_OBJECT;
        } else if (ext == DrmDef::EXT_DRM_CONTENT || ext == DrmDef::EXT_DRM_MESSAGE) {
            return DrmObjectType::CONTENT;
        }
    }
...
int DrmMtkPlugIn::onCheckRightsStatus(int uniqueId, const String8& path, int action) {
...
status_t DrmMtkPlugIn::onConsumeRights(int uniqueId, sp<DecryptHandle>& decryptHandle, int action,
                                       bool reserve) {
    if (DrmUtil::sDDebug) ALOGD("onConsumeRights() [%d]", uniqueId);
    // "Forward Lock Only"
    if (m_bFwdLockOnly) {
        if (DrmUtil::sDDebug)
            ALOGD("onConsumeRights() : Forward-lock-only is set, and don't consume any rights.");
        return DRM_NO_ERROR;
    }
    ...
    if (!reserve)  // if shall not reserve
    {
        ...
        if (NAME_NOT_FOUND != decryptHandle->extendedData.indexOfKey(String8("clientProcName")) &&
            ...
            if (DrmMtkUtil::isDrmConsumeInAppClient(procName) && !DrmMtkUtil::isNeedConsume(cid)) {
                if (DrmUtil::sDDebug)
                    ALOGD("onConsumeRights: don't consume rights this time for [%s], control by "
                          "itself",
                          procName.string());
                return DRM_NO_ERROR;
            }
...
status_t DrmMtkPlugIn::onSetPlaybackStatus(int uniqueId, sp<DecryptHandle>& decryptHandle,
                                           int playbackStatus, int64_t position) {
    if (DrmUtil::sDDebug)
        ALOGD("DrmMtkPlugIn::onSetPlaybackStatus() : %d, playbackStatus : %d", uniqueId,
              playbackStatus);
...
bool DrmMtkPlugIn::onValidateAction(int uniqueId, const String8& path, int action,
...
DrmConvertedStatus* DrmMtkPlugIn::onCloseConvertSession(int uniqueId, int convertId) {
...
status_t DrmMtkPlugIn::onOpenDecryptSession(int uniqueId, sp<DecryptHandle>& decryptHandle, int fd,
                                            off64_t offset, off64_t length) {
    if (DrmUtil::sDDebug)
        ALOGD("onOpenDecryptSession() [%d], Start with file descryptor", uniqueId);
    ...
    if (-1 == lseek(fd, 0, SEEK_SET)) {
        ...
        if (DrmUtil::sDDebug)
            ALOGD("onOpenDecryptSession() : using shared memory fd mapped memory: %p", data);
        DrmBuffer drmBuffer(data, length);
        status_t result =
                openDecryptSessionWithBuffer(uniqueId, decryptHandle, drmBuffer, offset, String8());
        return result;
    }
    // @}
    ...
    if (decryptHandle) {
        ...
        if (m_bFwdLockOnly && method != StrUtil::toString(DrmDef::METHOD_FL)) {
            if (DrmUtil::sDDebug)
                ALOGD("onOpenDecryptSession() : Forward-lock-only is set, and it's not FL type");
            delete pMetaData;
            return DRM_ERROR_CANNOT_HANDLE;
        }
        ...
        if (DrmUtil::sDDebug)
            ALOGD("onOpenDecryptSession() : status=%d, method=%s, offset=%lld", status,
                  method.string(), offset);
        if (status != RightsStatus::RIGHTS_VALID) {
            if (DrmUtil::sDDebug) ALOGD("onOpenDecryptSession() : invalid right for drm file.");
            // Check decode action when invalid, get action from offset parameter with flag
            // 0xfffffff, if ACTION_JUST_DECODE_BOUND return decrypt session to let app can decode
            // bound
            int decodeAction = int(offset - 0xfffffff);
            if (decodeAction == ACTION_JUST_DECODE_BOUND) {
                if (1 == strncasecmp(mimeType.string(), "image/", 6)) {
                    if (DrmUtil::sDDebug)
                        ALOGD("onOpenDecryptSession: only support just decode bound case when "
                              "rights invalid for image");
                    return DRM_ERROR_CANNOT_HANDLE;
                }
            } else {
                return DRM_ERROR_CANNOT_HANDLE;
...
status_t DrmMtkPlugIn::onOpenDecryptSession(int uniqueId, sp<DecryptHandle>& decryptHandle,
                                            const char* uri) {
    ...
    if (decryptHandle && NULL != uri && strlen(uri) > sizeof(fileTag)) {
        ...
        if (m_bFwdLockOnly) {
            ...
            if (method != StrUtil::toString(DrmDef::METHOD_FL)) {
                if (DrmUtil::sDDebug)
                    ALOGD("onOpenDecryptSession() : Forward-lock-only is set, and it's not FL "
                          "type");
                delete pMetaData;
                close(fd);
                return DRM_ERROR_CANNOT_HANDLE;
        ...
        decryptHandle->mimeType = pMetaData->get(String8(DrmMetaKey::META_KEY_MIME));
        decryptHandle->decryptApiType = DecryptApiType::CONTAINER_BASED;
        ...
        if (!m_decryptSessionMap.isCreated(decryptHandle->decryptId)) {
            ...
            fileDesc = fd;
            if (DrmUtil::sDDebug)
                ALOGD("onOpenDecryptSession() : the file descriptor [%d] stored in session", fd);
        } else {
            ALOGE("onOpenDecryptSession() : decrypt session map error.");
            close(fd);
            return DRM_ERROR_CANNOT_HANDLE;
...
status_t DrmMtkPlugIn::openDecryptSessionWithBuffer(int uniqueId, sp<DecryptHandle>& decryptHandle,
                                                    const DrmBuffer& buffer, const off64_t offset,
                                                    const String8& mimeType) {
    if (DrmUtil::sDDebug)
        ALOGD("onOpenDecryptSession() [%d], Start with buffer length [%d]", uniqueId,
              buffer.length);
    status_t result = DRM_ERROR_CANNOT_HANDLE;
    ...
    if (decryptHandle && NULL != buffer.data && buffer.length > 0) {
        ...
        if (m_bFwdLockOnly && method != StrUtil::toString(DrmDef::METHOD_FL)) {
            if (DrmUtil::sDDebug)
                ALOGD("onOpenDecryptSession(buffer) : Forward-lock-only is set, and it's not FL "
                      "type");
            delete pMetaData;
            return DRM_ERROR_CANNOT_HANDLE;
        }
        ...
        if (status != RightsStatus::RIGHTS_VALID) {
            ...
            if (decodeAction == ACTION_JUST_DECODE_BOUND) {
                if (1 == strncasecmp(mimeType.string(), "image/", 6)) {
                    if (DrmUtil::sDDebug)
                        ALOGD("onOpenDecryptSession: only support image for just decode bound when "
                              "rights invalid");
                    return DRM_ERROR_CANNOT_HANDLE;
                }
            } else {
...
status_t DrmMtkPlugIn::onCloseDecryptSession(int uniqueId, sp<DecryptHandle>& decryptHandle) {
...
status_t DrmMtkPlugIn::onInitializeDecryptUnit(int uniqueId, sp<DecryptHandle>& decryptHandle,
                                               int decryptUnitId, const DrmBuffer* headerInfo) {
    if (DrmUtil::sDDebug) ALOGD("DrmMtkPlugIn::onInitializeDecryptUnit() : %d", uniqueId);
    return DRM_NO_ERROR;
}

status_t DrmMtkPlugIn::onDecrypt(int uniqueId, sp<DecryptHandle>& decryptHandle, int decryptUnitId,
...
status_t DrmMtkPlugIn::onFinalizeDecryptUnit(int uniqueId, sp<DecryptHandle>& decryptHandle,
                                             int decryptUnitId) {
...
ssize_t DrmMtkPlugIn::onPread(int uniqueId, sp<DecryptHandle>& decryptHandle, void* buffer,
                              ssize_t numBytes, off64_t offset) {
    if (!decryptHandle || buffer == NULL || numBytes < 0 || offset < 0) {
        ALOGE("onPread[%d] with invalid parameter: numBytes[%d], offset[%ld]", uniqueId, numBytes,
              offset);
        return -1;
    }
    DecryptSession* pDecryptSession = m_decryptSessionMap.getValue(decryptHandle->decryptId);
    ...
    if (pDecryptSession->m_data != NULL) {  // share memoty data
        bytesRead = CipherFileUtil::CipherFileRead((char*)(pDecryptSession->m_data), buffer,
                                                   numBytes, offset, pDecryptSession->m_dataOffset,
                                                   pDecryptSession->m_dataLength, cr_hlp);
    } else if (pDecryptSession->m_fileDesc > -1) {  // fd data
        bytesRead = CipherFileUtil::CipherFileRead(pDecryptSession->m_fileDesc, buffer, numBytes,
                                                   offset, pDecryptSession->m_dataOffset,
                                                   pDecryptSession->m_dataLength, cr_hlp);
