...
static const char* RO_VERSION_01_0 =
        "VER_01_0";                  // the version string tag, placed at the beginning of RO file
...
void rc4_decrypt_buffer(unsigned char* toBuffer, unsigned char* fromBuffer, int size,
                        String8& deviceId);
void rc4_encrypt_buffer(unsigned char* toBuffer, unsigned char* fromBuffer, int size,
                        String8& deviceId);
...
int getRightsPosition() { return 0; }
...
void dump_entry(Entry* pEntry) {
    ...
    // get String8 for constraints <----
    printf("        next[%s] type[%s] %s\n", StrUtil::toString(pEntry->next), typeStr.string(),
           conStr.string());
}
...
bool restoreRights(FILE* fp, RO* ro, String8& deviceId) {
    ...
    unsigned char* output =
            (0 == version) ? (unsigned char*)&old_rights : (unsigned char*)&ro->rights;
...
ByteBuffer getDrmKey(String8& deviceId) {
    ...
    RC4(&k, (unsigned long)imei.length() + 1, (unsigned char*)imei.string(),
    ...
    for (int i = DrmDef::PERMISSION_PLAY_INDEX; i <= DrmDef::PERMISSION_PRINT_INDEX; i++) {
        ...
        if (ro->rights.num[i] != 0) {
            ...
            while (entryIndex != 0xFF) {
                ...
                ALOGD("restoreEntry: restore entry #[%d] from position [%ld]", entryIndex, pos);
                ...
                rc4_decrypt_buffer((unsigned char*)entry, encryptedEntryBuffer,
                                   sizeof(encryptedEntryBuffer), deviceId);
