...
int decrypt(const char* pathname, int fd_out, const char* id) {
    ...
    while (readSize < totalSize) {
        ...
        memset(buffer, 0, sizeof(buffer));
        int size =
                (totalSize - readSize) > DECRYPT_BUF_LEN ? DECRYPT_BUF_LEN : (totalSize - readSize);
        ...
        readLength = CipherFileUtil::CipherFileRead(fd, buffer, size, offset, totalSize - dataLenI,
                                                    dataLenI, cr_hlp);
        if (readLength < 0) {
 ...
 * return 0 indicates fail
 */
static jint mediatek_drm_RecoveryOmaDrm_recoveryOmaDrmFile(JNIEnv* env, jobject thiz,
                                                           jstring jdevideId, jstring jpath,
                                                           jobject fileDescriptorOutput) {
    ALOGD("recoveryOmaDrmFile Enter fileDescriptorOutput = %p", fileDescriptorOutput);

    int fdOut = (fileDescriptorOutput == NULL)
                        ? -1
                        : jniGetFDFromFileDescriptor(env, fileDescriptorOutput);
...
static int registerNativeMethods(JNIEnv* env) {
    int result = -1;
    /* look up the class */
    return jniRegisterNativeMethods(env, "com/mediatek/ap15/drm/RecoveryOmaDrmFile", nativeMethods,
                                    NELEM(nativeMethods));
