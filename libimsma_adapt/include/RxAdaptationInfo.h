...
class RxAdaptationInfo {
  public:
    RxAdaptationInfo();
    bool updateStatisticInfo(int32_t iRtpOH, int32_t iRtpSize, uint32_t uiNetSize,
                             uint32_t lostcount, int32_t iRtpTime);
    void calculateArrivalJitter(uint32_t uiRtpTimeStamp, int64_t iPacketRecvTimeUs,
                                uint32_t ClockRate);
    int processSenderInfo(uint8_t* data, uint64_t* ntpTime, uint32_t* rtpTime);
    int addReceiverReportBlock(uint32_t mID, uint8_t* data, uint32_t HighestSeqNumber,
                               uint32_t FirstPacketSeqNum, uint32_t ClockRate, bool* isTrigger);
    ...
    bool GetdebugInfo(bool needNotify, int32_t* uiEncBitRate, uint32_t lostcount,
                      uint32_t IDamageCount, RTPDebugNotifyInfo* NotifyInfo, int* count,
                      bool* isTrigger, int Operator);
    ...
    // scattered info
    void setReceivePacketFlag(void) { mReceivePacket = true; };
    ...
    int32_t getFrameCount(void) { return mNumBuffersReceived; };
  ...
  private:
    ...
    int32_t m_diffBR_LowWater;  // low water for increase encodig bit rate,need can be adjust by
                                // property
