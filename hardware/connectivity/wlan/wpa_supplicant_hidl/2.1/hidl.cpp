...
void mtk_wpas_hidl_init(struct wpa_global* global) {
    ...
    if (!hidl_manager) return;
    hidl_manager->registerHidlService(global);
}

void mtk_wpas_hidl_deinit() { HidlManager::destroyInstance(); }

int mtk_wpas_hidl_register_interface(struct wpa_supplicant* wpa_s) {
    if (!wpa_s) return 1;

    wpa_printf(MSG_DEBUG, "Registering interface to mtk hidl control: %s", wpa_s->ifname);

    HidlManager* hidl_manager = HidlManager::getInstance();
    if (!hidl_manager) return 1;
...
int mtk_wpas_hidl_unregister_interface(struct wpa_supplicant* wpa_s) {
    ...
    wpa_printf(MSG_DEBUG, "Deregistering interface from mtk hidl control: %s", wpa_s->ifname);
...
int mtk_wpas_hidl_register_network(struct wpa_supplicant* wpa_s, struct wpa_ssid* ssid) {
    if (!wpa_s || !ssid) return 1;

    wpa_printf(MSG_DEBUG, "Registering network to mtk hidl control: %d", ssid->id);
...
int mtk_wpas_hidl_unregister_network(struct wpa_supplicant* wpa_s, struct wpa_ssid* ssid) {
    if (!wpa_s || !ssid) return 1;

    wpa_printf(MSG_DEBUG, "Deregistering network from mtk hidl control: %d", ssid->id);
...
void mtk_wpas_hidl_notify_wnm_ess_disassoc_imminent_notice(struct wpa_supplicant* wpa_s,
                                                           u32 pmf_enabled, u32 reauth_delay,
                                                           const char* url) {
    if (!wpa_s || !url) return;

    wpa_printf(MSG_DEBUG, "Notifying WNM ESS disassoc imminent notice rx to hidl control: %s", url);

    HidlManager* hidl_manager = HidlManager::getInstance();
    if (!hidl_manager) return;

    hidl_manager->notifyWNMRxEssDisassocImminentNotice(wpa_s, pmf_enabled, reauth_delay, url);
}
