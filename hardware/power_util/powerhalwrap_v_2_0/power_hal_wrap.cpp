    ...
    if (gMtkPowerHalExists && (gMtkPowerHal == nullptr && gPowerHal == nullptr)) {
        ...
        if (gMtkPowerHal != nullptr) {
            ...
            android::hardware::Return<bool> linked =
                    gMtkPowerHal->linkToDeath(mtkPowerHalDeathRecipient, /*cookie*/ 0);
            if (!linked.isOk()) {
            ...
            mtkPowerHalDeathRecipient = new PowerDeathRecipient();
            android::hardware::Return<bool> linked =
                    gPowerHal->linkToDeath(mtkPowerHalDeathRecipient, /*cookie*/ 0);
            if (!linked.isOk()) {
                ALOGE("Transaction error in linking to PowerHAL death: %s",
                      linked.description().c_str());
            } else if (!linked) {
...
extern "C" int PowerHal_Wrap_mtkPowerHint(uint32_t hint, int32_t data) {
...
extern "C" int PowerHal_Wrap_mtkCusPowerHint(uint32_t hint, int32_t data) {
    std::lock_guard<std::mutex> lock(gMtkPowerHalMutex);
    if (getMtkPowerHal() && gMtkPowerHal != nullptr) {
        ALOGD("%s, hint:%d, data:%d", __FUNCTION__, hint, data);
...
extern "C" int PowerHal_Wrap_querySysInfo(uint32_t cmd, int32_t param) {
    int data = 0;
...
extern "C" int PowerHal_Wrap_notifyAppState(const char* packname, const char* actname, uint32_t pid,
...
extern "C" int PowerHal_Wrap_scnReg(void) {
    int handle = -1;
...
extern "C" int PowerHal_Wrap_scnConfig(int32_t hdl, int32_t cmd, int32_t param1, int32_t param2,
                                       int32_t param3, int32_t param4) {
    std::lock_guard<std::mutex> lock(gMtkPowerHalMutex);
    if (getMtkPowerHal() && gPowerHal != nullptr) {
        ALOGI("%s, hdl:%d, cmd:%d, param1:%d, param2:%d", __FUNCTION__, hdl, cmd, param1, param2);
        Return<void> ret = gPowerHal->scnConfig_2_1(
                hdl, (enum vendor::mediatek::hardware::power::V2_1::MtkPowerCmd)cmd, param1, param2,
                param3, param4);
        processReturn(ret, __FUNCTION__);
    }
...
extern "C" int PowerHal_Wrap_scnUnreg(int32_t hdl) {
    std::lock_guard<std::mutex> lock(gMtkPowerHalMutex);
    if (getMtkPowerHal() && gPowerHal != nullptr) {
...
extern "C" int PowerHal_Wrap_scnEnable(int32_t hdl, int32_t timeout) {
    std::lock_guard<std::mutex> lock(gMtkPowerHalMutex);
    if (getMtkPowerHal() && gPowerHal != nullptr) {
...
extern "C" int PowerHal_Wrap_scnDisable(int32_t hdl) {
    std::lock_guard<std::mutex> lock(gMtkPowerHalMutex);
    if (getMtkPowerHal() && gPowerHal != nullptr) {
        ALOGI("%s, hdl:%d", __FUNCTION__, hdl);
...
extern "C" int PowerHal_Wrap_scnUltraCfg(int32_t hdl, int32_t ultracmd, int32_t param1,
...
extern "C" int PowerHal_TouchBoost(int32_t timeout) {
    std::lock_guard<std::mutex> lock(gMtkPowerHalMutex);
    if (getMtkPowerHal() && gMtkPowerHal != nullptr) {
        ALOGD("%s", __FUNCTION__);
...
extern "C" int PowerHal_Wrap_setSysInfo(int32_t type, const char* data) {
    int result = 0;
