...
static bool getMtkPerfHal() {
    if (check_meta_mode() != 0) return false;
    ...
    if (gMtkPerfHalExists && gMtkPerfHal == nullptr) {
        ...
        if (gMtkPerfHal != nullptr) {
            ...
            android::hardware::Return<bool> linked =
                    gMtkPerfHal->linkToDeath(mtkPerfHalDeathRecipient, /*cookie*/ 0);
            if (!linked.isOk()) {
...
extern "C" int perf_lock_acq(int hdl, int duration, int list[], int numArgs) {
...
extern "C" int perf_lock_rel(int hdl) {
    int my_tid = (int)gettid();
