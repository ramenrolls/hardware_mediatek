...
static int add_new_pack(int type, tPackNode* curr, int set)  // set:1 => write to file
{
    ...
    if (type < 0 || type > APK_GAME) return -1;
...
int smart_table_init_flag(void) { return nTableIsReady; }
...
int smart_check_pack_existed(int type, const char* pack, int uid, int fromUid) {
    ...
    for (i = 0; i < num; i++) {
        if (!strncmp(pList->list[i].pack_name, pack, strlen(pack))) {
            ...
            return i;
        } else if (type == APK_BENCHMARK && pList->list[i].uid > 0 &&
                   pList->list[i].uid ==
                           fromUid) {  // launch from a benchmark => also enter benchmark mode
