...
void checkConTable(void) {
    ALOGI("Cmd name, Cmd ID, Entry, default value, current value, compare, max value, min value, "
          "isValid, normal value, sport value");
    for (int idx = 0; idx < FIELD_SIZE; idx++) {
        if (tConTable[idx].cmdName.length() == 0) continue;
        ALOGI("%s, %d, %s, %d, %d, %s, %d, %d %d %d %d", tConTable[idx].cmdName.c_str(),
              tConTable[idx].cmdID, tConTable[idx].entry.c_str(), tConTable[idx].defaultVal,
              tConTable[idx].curVal, tConTable[idx].comp.c_str(), tConTable[idx].maxVal,
              tConTable[idx].minVal, tConTable[idx].isValid, tConTable[idx].normalVal,
              tConTable[idx].sportVal);
    }
}
...
void xmlparser_start(void* userData, const char* name, const char* arg[]) {
    ...
    if (*xmlaction == XMLPARSE_GET_ACTIVITYNUM) {
    ...
    } else if (*xmlaction == XMLPARSE_GET_ACTIVITYLIST) {
        if (!strcmp(name, "PerfService")) {
            xmlPerService_flag = 1;
        }
        ...
        if ((xmlPerService_flag == 1) && !strcmp(name, "Package")) {
            ...
            len = strlen(arg[1]);
            if (len >= PACK_NAME_MAX) len = PACK_NAME_MAX - 1;
        ...
        if ((xmlPerService_flag == 1) && !strcmp(name, "Activity")) {
            if (!strcmp(arg[1], "Common")) {
                ...
                len = strlen(arg[1]);
                if (len >= CLASS_NAME_MAX) len = CLASS_NAME_MAX - 1;
        ...
        if ((xmlPerService_flag == 1) && !strncmp(name, "PERF_RES", 8)) {
            ...
            for (i = 0; arg[i] != 0; i += 2) {
                if (i == 0) ptXmlActList[nXmlCmdIndex].param1 = atoi(arg[1]);
                if (i == 2) ptXmlActList[nXmlCmdIndex].param2 = atoi(arg[3]);
                if (i == 4) ptXmlActList[nXmlCmdIndex].param3 = atoi(arg[5]);
                if (i == 6) ptXmlActList[nXmlCmdIndex].param4 = atoi(arg[7]);
            }
            ALOGD("[xmlparser_start] XMLPARSE_GET_ACTIVITYLIST CmdIndex:%d cmd:%s pack:%s, "
                  "activity:%s p1:%d p2:%d p3:%d p4:%d",
                  nXmlCmdIndex, ptXmlActList[nXmlCmdIndex].cmd, ptXmlActList[nXmlCmdIndex].packName,
                  ptXmlActList[nXmlCmdIndex].actName, ptXmlActList[nXmlCmdIndex].param1,
                  ptXmlActList[nXmlCmdIndex].param2, ptXmlActList[nXmlCmdIndex].param3,
...
void updateScnListfromXML(tScnNode* pPackList) {
    ...
    for (i = 0; i < lines; i++) act_name[i] = (char*)malloc(CLASS_NAME_MAX * sizeof(char));
    ...
    for (j = 0; j < nXmlCmdNum; j++) {
        if (strncmp(ptXmlActList[j].cmd, "PERF_RES", 8) == 0) {
            ...
            if (i == num) {  // new pack
                ...
                // pPackList[i].scn_valid    = (mode == PERF_MODE_NORMAL) ? 1 : 0;  // only normal
                // mode app are default valid
...
int get_activity_totalnum(void) {
    ...
    for (j = 0; j < nXmlCmdNum; j++) {
        if (strncmp(ptXmlActList[j].cmd, "CMD_SET", 7) == 0) {
            ...
            if (!strncmp(ptXmlActList[j].cmd, "[SPORTS]", 8)) continue;
...
void perfxml_read_cmddata(XMLElement* elmtScenario, int scn) {
    ...
    while (dataelmt) {
        ...
        ALOGD("[updateCusScnTable] cmd:%s, scn:%d, param_1:%d", str, scn, param_1);
        if (strlen(str) < 64) set_str_cpy(cmd, str, 64);
...
int updateCusScnTable(const char* path) {
    ...
    if (errXml != XML_SUCCESS) {
        ALOGE("%s: Unable to powerhal CusScnTable config file '%s'. Error: %s", __FUNCTION__, path,
              XMLDocument::ErrorIDToName(errXml));
        return 0;
    } else {
    ...
    while (elmtScenario != NULL) {
        if (!strcmp("MTKPOWER_HINT_PROCESS_CREATE", (elmtScenario->Attribute("powerhint")))) {
            ...
            perfxml_read_cmddata(elmtScenario, scn);
            elmtScenario = elmtScenario->NextSiblingElement();
            //} else if(!strcmp("MTKPOWER_HINT_FRAME_UPDATE",
            //(elmtScenario->Attribute("powerhint")))) {
            ...
            perfxml_read_cmddata(elmtScenario, scn);
            elmtScenario = elmtScenario->NextSiblingElement();
        } else if (!strcmp("MTKPOWER_HINT_GALLERY_STEREO_BOOST",
                           (elmtScenario->Attribute("powerhint")))) {
            ...
            perfxml_read_cmddata(elmtScenario, scn);
            elmtScenario = elmtScenario->NextSiblingElement();
        } else if (!strcmp("MTKPOWER_HINT_WHITELIST_LAUNCH",
                           (elmtScenario->Attribute("powerhint")))) {
            scn = MTKPOWER_HINT_WHITELIST_LAUNCH;
            perfxml_read_cmddata(elmtScenario, scn);
            elmtScenario = elmtScenario->NextSiblingElement();
        } else if (!strcmp("MTKPOWER_HINT_WIPHY_SPEED_DL",
                           (elmtScenario->Attribute("powerhint")))) {
...
int loadConTable(const char* file_name) {
    ...
    if (errXml != XML_SUCCESS) {
        ALOGE("%s: Unable to powerhal ConTable config file '%s'. Error: %s", __FUNCTION__,
              file_name, XMLDocument::ErrorIDToName(errXml));
        return 0;
    } else {
    ...
    while (elmtCMD) {
        ...
        if (elmtValid != NULL) {
            ...
            if (tConTable[idx].ignore == 1) {
                tConTable[idx].isValid = 0;
                ALOGI("[loadConTable][%d] ignore:%d isValid:%d ", idx, tConTable[idx].ignore,
                      tConTable[idx].isValid);
            }
        } else {
        ...
        if (tConTable[idx].prefix.length() != 0) {
            ...
            if (found != std::string::npos) tConTable[idx].prefix.replace(found, 1, " ");
        ...
        ALOGI("[loadConTable] cmd:%s, path:%s, normal:%d, default:%d",
              tConTable[idx].cmdName.c_str(), tConTable[idx].entry.c_str(),
              tConTable[idx].normalVal, tConTable[idx].defaultVal);
...
int perfservice_xmlparse_reload_whitelist() {
    ...
    if (app_file_path == NULL) return -1;
    ...
    if (nXmlCmdNum > 0) {
        ALOGI("[re-init] nXmlPackNum:%d nXmlActivityNum:%d nXmlCmdNum:%d", nXmlPackNum, nXmlActNum,
              nXmlCmdNum);
...
int perfservice_xmlparse_init() {
    ...
    if (app_file_path == NULL) return 0;
    ...
    if (nXmlCmdNum > 0) {
        ALOGI("[init] nXmlPackNum:%d nXmlActivityNum:%d nXmlCmdNum:%d ", nXmlPackNum, nXmlActNum,
              nXmlCmdNum);
        if ((ptXmlActList = (xml_activity*)malloc(sizeof(xml_activity) * (nXmlCmdNum))) == NULL) {
            ALOGE("Can't allocate memory");
            return 0;
