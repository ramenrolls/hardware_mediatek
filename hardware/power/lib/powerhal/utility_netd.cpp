...
extern "C" int netd_set_priority_uid(int uid, void* scn) {
    char cmd[128];
    memset(cmd, 0, 128);
...
extern "C" int netd_clear_priority_uid(int uid, void* scn) {
    char cmd[128];
    memset(cmd, 0, 128);
...
int SetDupPacketMultiLink(const char* linkInfo) {
    ...
    if (deleteAllDupPackerLink() < 0) return -1;
    ...
    /* check count */
    if (count > MAX_NETD_IP_FILTER_COUNT) return -1;
