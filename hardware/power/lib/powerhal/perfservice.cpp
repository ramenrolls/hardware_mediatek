...
int init() {
    ...
    if (!nIsReady) {
        ...
        if (gtDrvInfo.sysfs == 0 &&
            gtDrvInfo.dvfs == 0)  // /sys/devices/system/cpu/possible is not existed
            return 0;
        ...
        if (nPackNum >= 0) {
            if ((ptScnList = (tScnNode*)malloc(sizeof(tScnNode) * (SCN_APP_RUN_BASE + nPackNum))) ==
                NULL) {
        ...
        if (strstr(prop_content, "Android/aosp") != NULL) perfScnEnable(MTKPOWER_HINT_TEST_MODE);
    }
    return 1;
}
...
void setClusterCores(int scenario, int clusterNum, int totalCore, int* pCoreTbl, int* pMaxCoreTbl) {
    ...
    if (gtDrvInfo.acao) return;

    ALOGV("[setClusterCores] scn:%d, total:%d, cores:%d, %d", scenario, totalCore, pCoreTbl[0],
          pCoreTbl[1]);
    ...
    if (gtDrvInfo.perfmgrCpu) {
        ...
        for (i = 0; i < clusterNum; i++) {
            coreToSet = (pCoreTbl[i] <= 0 || pCoreTbl[i] > ptClusterTbl[i].cpuNum) ? PPM_IGNORE
                                                                                   : pCoreTbl[i];
            maxToSet = (pMaxCoreTbl[i] < 0 || pMaxCoreTbl[i] >= ptClusterTbl[i].cpuNum)
                               ? PPM_IGNORE
                               : pMaxCoreTbl[i];
        ...
        maxToSet = (pMaxCoreTbl[0] < 0 || pMaxCoreTbl[0] >= ptClusterTbl[0].cpuNum)
                           ? ptClusterTbl[0].cpuNum
                           : pMaxCoreTbl[0];
        if (gtDrvInfo.hmp) {
            coreToSetBig =
                    (pCoreTbl[1] <= 0 || pCoreTbl[1] > ptClusterTbl[1].cpuNum) ? 0 : pCoreTbl[1];
            maxToSetBig = (pMaxCoreTbl[1] < 0 || pMaxCoreTbl[1] >= ptClusterTbl[1].cpuNum)
                                  ? ptClusterTbl[1].cpuNum
                                  : pMaxCoreTbl[1];
...
void setClusterHardFreq(int scenario, int clusterNum, int* pFreqTbl, int* pMaxFreqTbl) {
    ...
    ALOGV("[setClusterHardFreq] scn:%d, freq:%d, %d , %d, %d", scenario, pFreqTbl[0],
          pMaxFreqTbl[0], pFreqTbl[1], pMaxFreqTbl[1]);
    if (gtDrvInfo.hard_user_limit) {
        str[0] = '\0';
        for (i = 0; i < clusterNum; i++) {
            freqToSet = (pFreqTbl[i] <= 0) ? PPM_IGNORE : pFreqTbl[i];
            maxToSet = (pMaxFreqTbl[i] <= 0 || pMaxFreqTbl[i] >= ptClusterTbl[i].freqMax)
                               ? PPM_IGNORE
                               : pMaxFreqTbl[i];
...
void setClusterFreq(int scenario, int clusterNum, int* pFreqTbl, int* pMaxFreqTbl) {
    ...
    if (gtDrvInfo.perfmgrCpu) {
        str[0] = '\0';
        for (i = 0; i < clusterNum; i++) {
            // freqToSet = (pFreqTbl[i] <= 0) ? PPM_IGNORE : (pFreqTbl[i] > ptClusterTbl[i].freqMax)
            // ? ptClusterTbl[i].freqMax : pFreqTbl[i];
            freqToSet = (pFreqTbl[i] <= 0) ? PPM_IGNORE : pFreqTbl[i];
            maxToSet = (pMaxFreqTbl[i] <= 0 || pMaxFreqTbl[i] >= ptClusterTbl[i].freqMax)
                               ? PPM_IGNORE
                               : pMaxFreqTbl[i];
        ...
        freqToSet = (pFreqTbl[0] <= 0)
                            ? 0
                            : ((pFreqTbl[0] > ptClusterTbl[0].freqMax) ? ptClusterTbl[0].freqMax
                                                                       : pFreqTbl[0]);
        maxToSet = (pMaxFreqTbl[0] <= 0 || pMaxFreqTbl[0] >= ptClusterTbl[0].freqMax)
                           ? 0
                           : pMaxFreqTbl[0];
        if (gtDrvInfo.dvfsHevc) {
            if (gtDrvInfo.hmp) {
                freqToSetBig = (pFreqTbl[1] <= 0) ? 0
                                                  : ((pFreqTbl[1] > ptClusterTbl[1].freqMax)
                                                             ? ptClusterTbl[1].freqMax
                                                             : pFreqTbl[1]);
                maxToSetBig = (pMaxFreqTbl[1] <= 0 || pMaxFreqTbl[1] >= ptClusterTbl[1].freqMax)
                                      ? 0
                                      : pMaxFreqTbl[1];
...
void checkDrvSupport(tDrvInfo* ptDrvInfo) {
    ...
    ALOGI("checkDrvSupport - perfmgr:%d, ppm:%d, ppmAll:%d, acao:%d, hmp:%d, hps:%d, hevc:%d, "
          "fliper:%d, sysfs:%d, dvfs:%d, turbo:%d, fstb:%d",
          ptDrvInfo->perfmgrLegacy, ptDrvInfo->ppmSupport, ptDrvInfo->ppmAll, ptDrvInfo->acao,
          ptDrvInfo->hmp, ptDrvInfo->hps, ptDrvInfo->dvfsHevc, ptDrvInfo->fliper, ptDrvInfo->sysfs,
          ptDrvInfo->dvfs, ptDrvInfo->turbo, ptDrvInfo->fstb);
}

void getCputopoInfo(int isHmpSupport, int* pnCpuNum, int* pnClusterNum,
                    tClusterInfo** pptClusterTbl) {
    int i, j;
    ...
    *pnClusterNum = (cputopoClusterNum > 0 && isHmpSupport == 0)
                            ? 1
                            : cputopoClusterNum;  // temp solution for D3
    ALOGI("getCputopoInfo - cpuNum:%d, cluster:%d, cputopoCluster:%d", *pnCpuNum, *pnClusterNum,
          cputopoClusterNum);
    ...
    *pptClusterTbl = (tClusterInfo*)malloc(sizeof(tClusterInfo) * (*pnClusterNum));
    ...
    for (i = 0; i < *pnClusterNum; i++) {
        ...
        if (gtDrvInfo.ppmSupport)
            get_ppm_cpu_freq_info(i, &((*pptClusterTbl)[i].freqMax),
                                  &((*pptClusterTbl)[i].freqCount),
                                  &((*pptClusterTbl)[i].pFreqTbl));
        else
            get_cpu_freq_info(cpu_index[i], &((*pptClusterTbl)[i].freqMax),
                              &((*pptClusterTbl)[i].freqCount), &((*pptClusterTbl)[i].pFreqTbl));
        (*pptClusterTbl)[i].freqMinNow = (*pptClusterTbl)[i].freqHardMinNow = 0;
        (*pptClusterTbl)[i].freqMaxNow = (*pptClusterTbl)[i].freqHardMaxNow =
                (*pptClusterTbl)[i].freqMax;
        ALOGI("[cluster %d]: cpu:%d, first:%d, freq count:%d, max_freq:%d", i,
              (*pptClusterTbl)[i].cpuNum, (*pptClusterTbl)[i].cpuFirstIndex,
              (*pptClusterTbl)[i].freqCount, (*pptClusterTbl)[i].freqMax);
...
inline int checkSuccess(int scenario) {
    return (((scenario < MTKPOWER_HINT_NUM || scenario >= MTKPOWER_HINT_NUM + REG_SCN_MAX) &&
             (scenario > 0)) &&
            ptScnList[scenario].scn_valid) ||
           (scenario >= nUserScnBase && scenario < nUserScnBase + REG_SCN_MAX);
}

int perfScnEnable(int scenario) {
    int needUpdateCores = 0, needUpdateCoresMax = 0, needUpdateFreq = 0, needUpdateFreqMax = 0,
        i = 0;
    ...
    if (checkSuccess(scenario)) {
        if (STATE_ON == ptScnList[scenario].scn_state) return 0;
        ALOGD("[perfScnEnable] scn:%d", scenario);

        ptScnList[scenario].scn_state = STATE_ON;

        ALOGV("[perfScnEnable] scn:%d, scn_cores_now:%d, scn_core_total:%d", scenario,
        ...
        for (i = 0; i < nClusterNum; i++) {
            ...
            // ALOGV("[perfScnEnable] scn:%d, i:%d, cpuMaxNow:%d, scn_core_max:%d", scenario, i,
            // ptClusterTbl[i].cpuMaxNow, ptScnList[scenario].scn_core_max[i]);
            if ((ptClusterTbl[i].cpuMaxNow > ptScnList[scenario].scn_core_max[i] ||
                 ptClusterTbl[i].cpuMaxNow == PPM_IGNORE)) {
                ptClusterTbl[i].cpuMaxNow = ptScnList[scenario].scn_core_max[i];
                needUpdateCoresMax = 1;
            }

            ALOGV("[perfScnEnable] scn:%d, i:%d, freqMinNow:%d, scn_freq_min:%d", scenario, i,
            ...
            ALOGV("[perfScnEnable] scn:%d, i:%d, freqMaxNow:%d, scn_freq_max:%d", scenario, i,
            ...
            if (ptClusterTbl[i].freqMaxNow <
                scn_freq_min[i]) {  // if max < min => align max with min
                ptClusterTbl[i].freqMaxNow = scn_freq_min[i];
                needUpdateFreqMax = 1;
            }
            scn_freq_max[i] = ptClusterTbl[i].freqMaxNow;

            ALOGV("[perfScnEnable] scn:%d, i:%d, freqHardMinNow:%d, scn_freq_hard_min:%d", scenario,
                  i, ptClusterTbl[i].freqHardMinNow, ptScnList[scenario].scn_freq_hard_min[i]);
            ...
            ALOGV("[perfScnEnable] scn:%d, i:%d, freqHardMaxNow:%d, scn_freq_hard_max:%d", scenario,
                  i, ptClusterTbl[i].freqHardMaxNow, ptScnList[scenario].scn_freq_hard_max[i]);
            ...
            if (ptClusterTbl[i].freqHardMaxNow <
                scn_freq_hard_min[i]) {  // if max < min => align ceiling with floor
        ...
        if ((ptScnList[scenario].scn_gpu_freq != -1) ||
            (ptScnList[scenario].scn_gpu_freq_max != -1)) {
            ...
            if (scn_gpu_freq_max_now > scn_gpu_freq_now) scn_gpu_freq_max_now = scn_gpu_freq_now;
        ...
        for (i = nClusterNum - 1; i >= 0; i--) {
            coreToSet = (scn_core_min[i] <= 0 || scn_core_min[i] > ptClusterTbl[i].cpuNum ||
                         totalCore <= 0)
                                ? PPM_IGNORE
                                : ((scn_core_min[i] > totalCore) ? totalCore : scn_core_min[i]);
            if (coreToSet >= 0) totalCore -= coreToSet;
            actual_core_min[i] = coreToSet;

            if (ptClusterTbl[i].cpuMaxNow <
                actual_core_min[i]) {  // min priority is higher than max
        ...
        */
        for (int idx = 0; idx < FIELD_SIZE; idx++) {
            if (tConTable[idx].entry.length() == 0) break;

            ALOGV("[perfScnEnable] scn:%d, cmd:%x, isValid:%d, cur:%d, param:%d", scenario,
                  tConTable[idx].cmdID, tConTable[idx].isValid, tConTable[idx].curVal,
                  ptScnList[scenario].scn_param[idx]);

            if (tConTable[idx].isValid == -1) continue;
            ...
            if (tConTable[idx].comp.compare(LESS) == 0) {
                if (ptScnList[scenario].scn_param[idx] < tConTable[idx].curVal) {
                    ...
                    if (tConTable[idx].prefix.length() == 0)
                        set_value(tConTable[idx].entry.c_str(), ptScnList[scenario].scn_param[idx]);
                    else {
                        char inBuf[64];
                        // ALOGD("[perfScnEnable] cmd:%x, (less) prefix:%s; %d",
                        // tConTable[idx].cmdID, tConTable[idx].prefix.c_str(),
                        // ptScnList[scenario].scn_param[idx]);
                        snprintf(inBuf, 64, "%s%d", tConTable[idx].prefix.c_str(),
                                 ptScnList[scenario].scn_param[idx]);
                        set_value(tConTable[idx].entry.c_str(), inBuf);
                        ALOGI("[perfScnEnable] (less) %s set +prefix:%s;",
                              tConTable[idx].entry.c_str(), inBuf);
                    }
                }
            } else {
                if (ptScnList[scenario].scn_param[idx] > tConTable[idx].curVal) {
                    ...
                    if (tConTable[idx].prefix.length() == 0)
                        ...
                        char inBuf[64];
                        // ALOGD("[perfScnEnable] cmd:%x, (more) prefix:%s; %d",
                        // tConTable[idx].cmdID, tConTable[idx].prefix.c_str(),
                        // ptScnList[scenario].scn_param[idx]);
                        snprintf(inBuf, 64, "%s%d", tConTable[idx].prefix.c_str(),
                                 ptScnList[scenario].scn_param[idx]);
                        set_value(tConTable[idx].entry.c_str(), inBuf);
                        ALOGI("[perfScnEnable] (more) %s set +prefix:%s;",
                              tConTable[idx].entry.c_str(), inBuf);
                    }
                }
            }
        ...
        for (int idx = 0; idx < gRscCtlTblLen; idx++) {
            if (RscCfgTbl[idx].set_func == NULL || RscCfgTbl[idx].unset_func == NULL) break;

            ALOGV("[perfScnEnable] RscCfgTbl[%d] scn:%d, cmd:%x, isValid:%d, cur:%d, param:%d "
                  "resetVal:%d",
                  idx, scenario, RscCfgTbl[idx].cmdID, gRscCtlTbl[idx].isValid,
                  gRscCtlTbl[idx].curVal, ptScnList[scenario].scn_rsc[idx],
                  gRscCtlTbl[idx].resetVal);

            if (gRscCtlTbl[idx].isValid != 1) continue;

            if (RscCfgTbl[idx].comp == SMALLEST) {
                if (ptScnList[scenario].scn_rsc[idx] < gRscCtlTbl[idx].curVal) {
                    gRscCtlTbl[idx].curVal = ptScnList[scenario].scn_rsc[idx];
                    result = RscCfgTbl[idx].set_func(ptScnList[scenario].scn_rsc[idx],
                                                     (void*)&ptScnList[scenario]);
                    ALOGV("[perfScnEnable] RscCfgTbl SMALLEST cur:%d, ret:%d",
                          gRscCtlTbl[idx].curVal, result);
                }
                ...
                if (ptScnList[scenario].scn_rsc[idx] > gRscCtlTbl[idx].curVal) {
                    ...
                    result = RscCfgTbl[idx].set_func(ptScnList[scenario].scn_rsc[idx],
                                                     (void*)&ptScnList[scenario]);
                    ALOGV("[perfScnEnable] RscCfgTbl BIGGEST cur:%d, ret:%d",
                          gRscCtlTbl[idx].curVal, result);
                }
            } else {
                if (ptScnList[scenario].scn_rsc[idx] != gRscCtlTbl[idx].resetVal) {
                    ...
                    result = RscCfgTbl[idx].set_func(ptScnList[scenario].scn_rsc[idx],
                                                     (void*)&ptScnList[scenario]);
                    ALOGV("[perfScnEnable] RscCfgTbl ONESHOT cur:%d, ret:%d",
                          gRscCtlTbl[idx].curVal, result);
                }
            }
        }
...
int perfScnDisable(int scenario) { return perfScnUpdate(scenario, 0); }
...
int perfScnUpdate(int scenario, int force_update) {
    ...
    int needUpdate = 0;
    int coresToSet[CLUSTER_MAX], actual_core_min[CLUSTER_MAX], maxCoresToSet[CLUSTER_MAX],
            lastCore[CLUSTER_MAX];
    int freqToSet[CLUSTER_MAX], lastFreq[CLUSTER_MAX], maxFreqToSet[CLUSTER_MAX], lastGpuFreq,
            lastGpuMaxFreq;
    ...
    if (checkSuccess(scenario)) {
        if (STATE_OFF == ptScnList[scenario].scn_state) return 0;
        ...
        // check core
        // ALOGV("[perfScnUpdate] scenario:%d, scn_cores_now:%d, scn_core_total:%d", scenario,
        // scn_cores_now, ptScnList[scenario].scn_core_total);
        ...
        if (scn_cores_now <= ptScnList[scenario].scn_core_total || force_update) {
            ...
            if (scn_cores_now != totalCoresToSet) {
                ALOGV("[perfScnUpdate] scn_cores_now:%d, totalCoresToSet:%d", scn_cores_now,
                      totalCoresToSet);
        ...
        for (i = 0; i < nClusterNum; i++) {
            ...
            if (ptClusterTbl[i].cpuMinNow <= ptScnList[scenario].scn_core_min[i] || force_update) {
                ...
                if (coresToSet[i] != ptClusterTbl[i].cpuMinNow) {
                    ALOGV("[perfScnUpdate] i:%d, cpuMinNow:%d, coresToSet:%d", i,
            ...
            ALOGV("[perfScnUpdate] scn:%d, i:%d, cpuMaxNow:%d, scn_core_max:%d", scenario, i,
                  ptClusterTbl[i].cpuMaxNow, ptScnList[scenario].scn_core_max[i]);
            if (ptClusterTbl[i].cpuMaxNow >= ptScnList[scenario].scn_core_max[i] ||
                ptClusterTbl[i].cpuMaxNow == lastCore[i] || force_update) {
        ...
        for (i = nClusterNum - 1; i >= 0; i--) {
            coreToSet =
                    (coresToSet[i] <= 0 || coresToSet[i] > ptClusterTbl[i].cpuNum || totalCore <= 0)
                            ? PPM_IGNORE
                            : ((coresToSet[i] > totalCore) ? totalCore : coresToSet[i]);
            if (coreToSet >= 0) totalCore -= coreToSet;
            actual_core_min[i] = coreToSet;
        ...
        for (i = 0; i < nClusterNum; i++) {
            ...
            /* CPU freq ceiling */
            ALOGV("[perfScnUpdate] scn:%d, i:%d, last_min:%d, global_max:%d, max:%d", scenario, i,
                  lastFreq[i], ptClusterTbl[i].freqMaxNow, ptScnList[scenario].scn_freq_max[i]);
            if (ptClusterTbl[i].freqMaxNow >= ptScnList[scenario].scn_freq_max[i] ||
                ptClusterTbl[i].freqMaxNow == lastFreq[i] ||
            ...
            ALOGV("[perfScnUpdate] scn:%d, i:%d, last_hard_min:%d, global_hard_max:%d, max:%d",
                  scenario, i, lastHardFreq[i], ptClusterTbl[i].freqHardMaxNow,
                  ptScnList[scenario].scn_freq_max[i]);
            if (ptClusterTbl[i].freqHardMaxNow >= ptScnList[scenario].scn_freq_hard_max[i] ||
                ptClusterTbl[i].freqHardMaxNow == lastHardFreq[i] ||
                force_update) {  // perfservice might ignore someone's setting before
                maxHardFreqToSet[i] = FREQ_MAX;
                for (j = 0; j < SCN_APP_RUN_BASE + nPackNum; j++) {
                    if (ptScnList[j].scn_state == STATE_ON)
                        maxHardFreqToSet[i] =
                                min(maxHardFreqToSet[i], ptScnList[j].scn_freq_hard_max[i]);
                }
        ...
        if ((ptScnList[scenario].scn_gpu_freq != -1) ||
            ...
            ALOGD("[perfScnUpdate] gpu_freq:%d, gpu_max:%d, scn_gpu_freq:%d, scn_gpu_max:%d, "
                  "gpuFreqToSet:%d, gpuFreqMaxToSet:%d",
                  scn_gpu_freq_now, scn_gpu_freq_max_now, ptScnList[scenario].scn_gpu_freq,
                  ptScnList[scenario].scn_gpu_freq_max, gpuFreqToSet, gpuFreqMaxToSet);
            ...
            if (baseNeedTraverSal) scn_gpu_freq_now = gpuFreqToSet;

            if (scn_gpu_freq_now != lastGpuFreq || scn_gpu_freq_max_now != lastGpuMaxFreq)
                setGpuFreq(scenario, scn_gpu_freq_now);

            if (upBaseNeedTraverSal)
                scn_gpu_freq_max_now = ((gpuFreqMaxToSet > scn_gpu_freq_now) ? (scn_gpu_freq_now)
                                                                             : (gpuFreqMaxToSet));
            else
                scn_gpu_freq_max_now =
                        ((scn_gpu_freq_max_now > scn_gpu_freq_now) ? (scn_gpu_freq_now)
                                                                   : (scn_gpu_freq_max_now));
        ...
        for (int idx = 0; idx < FIELD_SIZE; idx++) {
            ...
            ALOGV("[perfScnUpdate] scn:%d, cmd:%x, reset:%d, default:%d, cur:%d, param:%d",
                  scenario, tConTable[idx].cmdID, tConTable[idx].resetVal,
                  tConTable[idx].defaultVal, tConTable[idx].curVal,
                  ptScnList[scenario].scn_param[idx]);
            if (tConTable[idx].comp.compare(LESS) == 0) {
                if (force_update || (ptScnList[scenario].scn_param[idx] < tConTable[idx].resetVal &&
                    ...
                    if (tConTable[idx].curVal != numofCurr) {
                        ...
                        if (tConTable[idx].prefix.length() == 0)
                            ...
                            set_value(tConTable[idx].entry.c_str(), inBuf);
                            ALOGI("[perfScnUpdate] (less) %s set +prefix:%s;",
                                  tConTable[idx].entry.c_str(), inBuf);
                        }
                    }
                }
                ...
                if (force_update || (ptScnList[scenario].scn_param[idx] > tConTable[idx].resetVal &&
                    ...
                    if (tConTable[idx].curVal != numofCurr) {
                        ...
                        if (tConTable[idx].prefix.length() == 0)
                            ...
                            snprintf(inBuf, 64, "%s%d", tConTable[idx].prefix.c_str(), numToSet);
                            set_value(tConTable[idx].entry.c_str(), inBuf);
                            ALOGI("[perfScnUpdate] (more) %s set +prefix:%s;",
                                  tConTable[idx].entry.c_str(), inBuf);
                        }
                    }
                }
        ...
        /*Rescontable*/
        for (int idx = 0; idx < gRscCtlTblLen; idx++) {
            ALOGV("[perfScnUpdate] RscCfgTbl[%d] scn:%d, cmd:%x, param:%d", idx, scenario,
                  RscCfgTbl[idx].cmdID, ptScnList[scenario].scn_rsc[idx]);

            if (RscCfgTbl[idx].set_func == NULL || RscCfgTbl[idx].unset_func == NULL) break;

            if (gRscCtlTbl[idx].isValid != 1) continue;

            ALOGV("[perfScnUpdate] RscCtlTbl[%d] scn:%d, cmd:%x, reset:%d, default:%d, cur:%d, "
                  "param:%d",
                  idx, scenario, RscCfgTbl[idx].cmdID, gRscCtlTbl[idx].resetVal,
                  RscCfgTbl[idx].defaultVal, gRscCtlTbl[idx].curVal,
                  ptScnList[scenario].scn_rsc[idx]);
                ...
                if (force_update || (ptScnList[scenario].scn_rsc[idx] < gRscCtlTbl[idx].resetVal &&
                    ...
                    if (gRscCtlTbl[idx].curVal != numofCurr || RscCfgTbl[idx].force_update == 1) {
                        if (gRscCtlTbl[idx].curVal == gRscCtlTbl[idx].resetVal) {
                            ...
                            result = RscCfgTbl[idx].unset_func(numToSet,
                                                               (void*)&ptScnList[scenario]);
                        } else {
                    ...
                    ALOGV("[perfScnUpdate] RscCfgTbl SMALLEST numToSet:%d, ret:%d", numToSet,
                          result);
                }
            } else if (RscCfgTbl[idx].comp == BIGGEST) {
                if (force_update || (ptScnList[scenario].scn_rsc[idx] > gRscCtlTbl[idx].resetVal &&
                    ...
                    ALOGV("[perfScnUpdate] RscCfgTbl BIGGEST numToSet:%d, ret:%d", numToSet,
                          result);
                }
            } else {
                if (force_update) {
                    // unset original value
                    ALOGD("[perfScnUpdate] idx:%d, reset:%d, prev:%d, rsc:%d", idx,
                          gRscCtlTbl[idx].resetVal, ptScnList[scenario].scn_prev_rsc[idx],
                          ptScnList[scenario].scn_rsc[idx]);
                    if (ptScnList[scenario].scn_prev_rsc[idx] != gRscCtlTbl[idx].resetVal &&
                        ...
                        result = RscCfgTbl[idx].unset_func(ptScnList[scenario].scn_prev_rsc[idx],
                                                           (void*)&ptScnList[scenario]);
                        ALOGV("[perfScnUpdate] RscCfgTbl ONESHOT unset_func:%d, ret:%d",
                              ptScnList[scenario].scn_prev_rsc[idx], result);
                    }

                    // set new value
                    if ((ptScnList[scenario].scn_rsc[idx] != gRscCtlTbl[idx].resetVal &&
                         ptScnList[scenario].scn_rsc[idx] !=
                                 ptScnList[scenario].scn_prev_rsc[idx]) ||
                        RscCfgTbl[idx].force_update == 1) {
                        result = RscCfgTbl[idx].set_func(ptScnList[scenario].scn_rsc[idx],
                                                         (void*)&ptScnList[scenario]);
                        ALOGV("[perfScnUpdate] RscCfgTbl ONESHOT cur:%d, ret:%d",
                              gRscCtlTbl[idx].curVal, result);
                    }
                } else if (ptScnList[scenario].scn_rsc[idx] != gRscCtlTbl[idx].resetVal) {
                    result = RscCfgTbl[idx].unset_func(ptScnList[scenario].scn_rsc[idx],
                                                       (void*)&ptScnList[scenario]);
...
int perfScnDumpAll(void) {
    ...
    for (i = 0; i < nUserScnBase; i++) {
        if (ptScnList[i].scn_state == STATE_ON) {
            ...
            for (j = 0; j < nClusterNum; j++) {
                ALOGD("            - cpu:%d, freq:%d", ptScnList[i].scn_core_min[j],
                      ptScnList[i].scn_freq_min[j]);
            }
        }
    }
    ...
    for (i = nUserScnBase; i < nUserScnBase + REG_SCN_MAX; i++) {
        if (ptScnList[i].scn_state == STATE_ON) {
            ALOGI("perfDumpAll (user register)- handle:%d, pid:%d, tid:%d, comm:%s",
                  ptScnList[i].handle_idx, ptScnList[i].pid, ptScnList[i].tid, ptScnList[i].comm);
            for (j = 0; j < nClusterNum; j++) {
                ALOGD("            - cpu:%d, freq:%d", ptScnList[i].scn_core_min[j],
                      ptScnList[i].scn_freq_min[j]);
            }
    ...
    for (i = SCN_APP_RUN_BASE; i < SCN_APP_RUN_BASE + nPackNum; i++) {
        if (ptScnList[i].scn_state == STATE_ON) {
            ALOGI("perfDumpAll (app list)- type:%d, pack_name:%s, act_name:%s",
                  ptScnList[i].scn_type, ptScnList[i].pack_name, ptScnList[i].act_name);
            for (j = 0; j < nClusterNum; j++) {
                ALOGD("            - cpu:%d, freq:%d", ptScnList[i].scn_core_min[j],
                      ptScnList[i].scn_freq_min[j]);
            }
...
int cmdSetting(int icmd, char* scmd, tScnNode* scenario, int param_1) {
    ...
    if (((icmd == -1) && !strcmp(scmd, "PERF_RES_GPU_FREQ_MIN")) || icmd == PERF_RES_GPU_FREQ_MIN) {
        ...
        scenario->scn_core_min[0] = (param_1 <= ptClusterTbl[0].cpuNum)
                                            ? ((param_1 < 0) ? 0 : param_1)
                                            : ptClusterTbl[0].cpuNum;

        scenario->scn_core_total = 0;
        for (i = 0; i < nClusterNum; i++) {
            if (scenario->scn_core_min[i] >= 0 &&
        ...
        if (nClusterNum > 1) {
            scenario->scn_core_min[1] = (param_1 <= ptClusterTbl[1].cpuNum)
                                                ? ((param_1 < 0) ? 0 : param_1)
                                                : ptClusterTbl[1].cpuNum;
        ...
        scenario->scn_core_max[0] = (param_1 >= 0) ? param_1 : ptClusterTbl[0].cpuNum;
    } else if (((icmd == -1) && !strcmp(scmd, "PERF_RES_CPUCORE_MAX_CLUSTER_1")) ||
               icmd == PERF_RES_CPUCORE_MAX_CLUSTER_1) {
        if (nClusterNum > 1) {
            scenario->scn_core_max[1] = (param_1 >= 0) ? param_1 : ptClusterTbl[1].cpuNum;
        }
        ...
        for (i = 0; i < nClusterNum; i++) {
            // ALOGI("cmdSetting - cmd:%x, i:%d, cpu:%d, freq:%d", icmd, i, ptClusterTbl[i].cpuNum,
            // ptClusterTbl[i].freqMax);
            scenario->scn_core_min[i] = ptClusterTbl[i].cpuNum;
            scenario->scn_core_total += scenario->scn_core_min[i];
        ...
        if (param_1 == 1) {
            ...
            if (smart_check_pack_existed(APK_BENCHMARK, scenario->pack_name, foreground_uid,
                                         last_from_uid) == -1) {
                smart_add_benchmark();
            }
        }
    } else if (((icmd == -1) &&
                !strcmp(scmd, "PERF_RES_POWERHAL_WHITELIST_APP_LAUNCH_TIME_COLD")) ||
               icmd == PERF_RES_POWERHAL_WHITELIST_APP_LAUNCH_TIME_COLD) {
        scenario->launch_time_cold = (param_1 >= 0) ? param_1 : -1;
    } else if (((icmd == -1) &&
                !strcmp(scmd, "PERF_RES_POWERHAL_WHITELIST_APP_LAUNCH_TIME_WARM")) ||
    ...
    for (int idx = 0; idx < FIELD_SIZE; idx++) {
        if (((icmd == -1) && !strcmp(scmd, tConTable[idx].cmdName.c_str())) ||
            ...
            if (param_1 >= tConTable[idx].minVal && param_1 <= tConTable[idx].maxVal)
                ...
                ALOGE("input parameter exceed reasonable range %d %d %d %d", tConTable[idx].cmdID,
                      param_1, tConTable[idx].maxVal, tConTable[idx].minVal);
    ...
    for (int idx = 0; idx < gRscCtlTblLen; idx++) {
        if (((icmd == -1) && !strcmp(scmd, RscCfgTbl[idx].cmdName.c_str())) ||
            ...
            ALOGD("cmdSetting RscCfgTbl cmdID:%x param_1:%d ,maxVal:%d ,minVal:%d, force:%d",
                  RscCfgTbl[idx].cmdID, param_1, RscCfgTbl[idx].maxVal, RscCfgTbl[idx].minVal,
                  RscCfgTbl[idx].force_update);
            if (param_1 >= RscCfgTbl[idx].minVal && param_1 <= RscCfgTbl[idx].maxVal) {
                scenario->scn_rsc[idx] = param_1;
            } else
                ALOGE("input parameter exceed reasonable range %d %d %d %d", RscCfgTbl[idx].cmdID,
                      param_1, RscCfgTbl[idx].maxVal, RscCfgTbl[idx].minVal);
    ...
    if (ret == -1) ALOGI("cmdSetting - unknown cmd:%x, scmd:%s ,param_1:%d", icmd, scmd, param_1);
...
int LegacyCmdSetting(int icmd, tScnNode* scenario, int param_1, int param_2, int param_3,
                     int param_4) {
    int i = 0, ret = 0;
    ...
    if (icmd == (int)MtkPowerCmd::CMD_SET_GPU_FREQ_MIN) {
        ...
        if (param_1 >= nClusterNum || param_1 < 0) return -1;
        scenario->scn_core_min[param_1] = (param_2 <= ptClusterTbl[param_1].cpuNum)
                                                  ? ((param_2 < 0) ? 0 : param_2)
                                                  : ptClusterTbl[param_1].cpuNum;
        ...
        if (param_1 >= nClusterNum || param_1 < 0) return -1;
        scenario->scn_core_max[param_1] = (param_2 >= 0) ? param_2 : ptClusterTbl[param_1].cpuNum;
    } else if (icmd == (int)MtkPowerCmd::CMD_SET_CLUSTER_CPU_FREQ_MIN) {
        if (param_1 >= nClusterNum || param_1 < 0) return -1;
        scenario->scn_freq_min[param_1] = (param_2 >= ptClusterTbl[param_1].freqMax)
                                                  ? ptClusterTbl[param_1].freqMax
                                                  : param_2;
    } else if (icmd == (int)MtkPowerCmd::CMD_SET_CLUSTER_CPU_FREQ_MAX) {
        if (param_1 >= nClusterNum || param_1 < 0) return -1;
        scenario->scn_freq_max[param_1] = (param_2 >= ptClusterTbl[param_1].freqMax)
                                                  ? ptClusterTbl[param_1].freqMax
                                                  : param_2;
        ...
        for (i = 0; i < nClusterNum; i++) {
            // ALOGI("cmdSetting - cmd:%d, i:%d, cpu:%d, freq:%d", icmd, i, ptClusterTbl[i].cpuNum,
            // ptClusterTbl[i].freqMax);
            scenario->scn_core_min[i] = ptClusterTbl[i].cpuNum;
            scenario->scn_core_total += scenario->scn_core_min[i];
            scenario->scn_freq_min[i] = ptClusterTbl[i].freqMax;
    ...
    for (int idx = 0; idx < FIELD_SIZE; idx++) {
        if (icmd == tConTable[idx].legacyCmdID) {
            if (param_1 >= tConTable[idx].minVal && param_1 <= tConTable[idx].maxVal)
                ...
                ALOGI("input parameter exceed reasonable range %d %d %d %d",
                      tConTable[idx].legacyCmdID, param_1, tConTable[idx].maxVal,
    ...
    if (ret == -1)
        ALOGI("LegacyCmdSetting - unknown cmd:%d, p1:%d, p2:%d, p3:%d, p4:%d", icmd, param_1,
...
void switchNormalAndSportMode(int mode) {
    for (int idx = 0; idx < FIELD_SIZE; idx++) {
        if (tConTable[idx].cmdName.length() == 0) continue;

        if (tConTable[idx].isValid == -1) continue;

        if (tConTable[idx].sportVal == CFG_TBL_INVALID_VALUE) continue;

        if (tConTable[idx].normalVal == tConTable[idx].sportVal) continue;

        ALOGV("[switchNormalAndSportMode] mode:%d, idx:%d, id:%x, default:%d, normal:%d, "
              "sports:%d, reset:%d, cur:%d",
        ...
        if (tConTable[idx].prefix.length() == 0)
            ...
            set_value(tConTable[idx].entry.c_str(), inBuf);
            ALOGD("[switchNormalAndSportMode] (less) %s set +prefix:%s;",
                  tConTable[idx].entry.c_str(), inBuf);
        }
    }
}
...
int switchPowerMode(int mode)  // call by perfNotifyUserStatus
{
    ...
    if (mode < 0 || mode > PERF_MODE_SPORTS) return 0;
...
int switchSportsMode(int reason, int bEnable)  // for smart detection
{
    ...
    if (nCurrPowerMode == PERF_MODE_SPORTS) {
        ...
        perfScnDisable(
                MTKPOWER_HINT_SPORTS);  // only enable if app is running and it's benchmark mdoe
    }
...
int gameNotifyDisplayType(int type) {
    ...
    if (type == DISPLAY_TYPE_GAME) {
        ...
        if (nFbcSupport) fbcNotifyTouch(0);

        perfScnEnable(MTKPOWER_HINT_GAMING);

        ALOGE("foreground_uid %d last_from_uid %d", foreground_uid, last_from_uid);
        if (smart_check_pack_existed(APK_GAME, foreground_pack, foreground_uid, last_from_uid) ==
            -1) {
            smart_add_game();
        }
    } else {
        if (type == DISPLAY_TYPE_NO_TOUCH_BOOST) perfScnDisable(MTKPOWER_HINT_APP_TOUCH);
...
int getForegroundInfo(char** pPackName, int* pPid, int* pUid) {
    if (pPackName != NULL) *pPackName = foreground_pack;

    if (pPid != NULL) *pPid = foreground_pid;

    if (pUid != NULL) *pUid = foreground_uid;
...
static int allocNewHandleNum(void) {
    ...
    if (user_handle_now == 0) user_handle_now++;
...
extern "C" int perfBoostEnable(int scenario) {
    ...
    if (scenario < MTKPOWER_HINT_NUM) perfScnEnable(scenario);
    ...
    switch (scenario) {
        ...
        case MTKPOWER_HINT_APP_TOUCH:
            if (nFbcSupport) fbcNotifyTouch(1);
...
extern "C" int perfBoostDisable(int scenario) {
    Mutex::Autolock lock(sMutex);
    if (!nIsReady)
        if (!init()) return 0;

    if (scenario < MTKPOWER_HINT_NUM) perfScnDisable(scenario);
...
extern "C" int perfNotifyAppState(const char* packName, const char* actName, int state, int pid,
                                  int uid) {
    int i, ret = 0;
    ...
    ALOGV("[perfNotifyAppState] pack:%s, com:%s, state:%d, pid:%d, uid:%d", packName, actName,
          state, pid, uid);
    ...
    if (state == STATE_DEAD && pid != foreground_pid) return 0;
    ...
    if (state == STATE_RESUMED && strcmp(foreground_pack, packName)) {
        ...
        ALOGI("[perfNotifyAppState] foreground:%s, pid:%d, uid:%d", foreground_pack, foreground_pid,
              uid);
        if (smart_check_pack_existed(APK_BENCHMARK, foreground_pack, foreground_uid,
                                     last_from_uid) != -1) {
    ...
    if (state == STATE_RESUMED && strcmp(foreground_act, actName)) {
        ...
        for (i = 0; i < nPackNum; i++) {
            ...
            if (strncmp(pPackList[i].act_name, "Common", 6) != 0) {
                if (!strcmp(pPackList[i].pack_name, packName) &&
                    !strcmp(pPackList[i].act_name, actName)) {
                    Act_Match = 1;
                    ...
                    if (fg_launch_time_cold > 0 || fg_launch_time_warm > 0) {
                        ...
                        ALOGI("[perfNotifyAppState] Activity policy match launch time=%d, %d!!!",
        ...
        if (Act_Match != 1 && Common_index != -1) {
            ALOGI("[perfNotifyAppState] launch cold:%d, warm:%d",
                  pPackList[Common_index].launch_time_cold,
                  pPackList[Common_index].launch_time_warm);
        ...
        } else if (Act_Match == 1 && Common_index != -1) {
...
extern "C" int perfUserScnEnable(int handle) {
    int idx = -1;
    ...
    if (idx < nUserScnBase || idx >= nUserScnBase + REG_SCN_MAX || idx < 0) return -1;
...
extern "C" int perfUserScnDisable(int handle) {
    int idx = -1;
    ...
    idx = findHandleToIndex(handle);
    if (idx < nUserScnBase || idx >= nUserScnBase + REG_SCN_MAX) return -1;

    perfScnDisable(idx);
    return 0;
}

extern "C" int perfUserScnResetAll(void) {
    int i;
    ...
    for (i = nUserScnBase; i < nUserScnBase + REG_SCN_MAX; i++) {
        if (ptScnList[i].scn_type != -1 && ptScnList[i].scn_state == STATE_ON) perfScnDisable(i);
        resetScenario(i, 1);
    }
    return 0;
}

extern "C" int perfUserScnDisableAll(void) {
    int i;
    struct stat stat_buf;
    int exist;
    char proc_path[128];
    ...
    for (i = 0; i < SCN_APP_RUN_BASE + nPackNum; i++) {
        if (ptScnList[i].scn_type != -1) {
            if (ptScnList[i].scn_state == STATE_ON &&
                ptScnList[i].screen_off_action != MTKPOWER_SCREEN_OFF_ENABLE) {
                ALOGI("perfUserScnDisableAll, h:%d, s:%d, a:%d", i, ptScnList[i].scn_state,
                      ptScnList[i].screen_off_action);
                perfScnDisable(i);
            ...
            if (i >= nUserScnBase && i < SCN_APP_RUN_BASE) {
                if (ptScnList[i].scn_type == SCN_USER_HINT ||
                    ...
                    if (!exist) {
                        ALOGI("perfUserScnDisableAll, hdl:%d, pid:%d, comm:%s, is not existed", i,
                              ptScnList[i].pid, ptScnList[i].comm);
                        perfScnDisable(i);
                        resetScenario(i, 1);
                    }
                }
...
extern "C" int perfUserScnRestoreAll(void) {
    int i;
    ...
    for (i = 0; i < SCN_APP_RUN_BASE + nPackNum; i++) {
        if (ptScnList[i].scn_type != -1 && ptScnList[i].scn_state == STATE_WAIT_RESTORE) {
            ALOGI("perfUserScnRestoreAll, h:%d, s:%d, a:%d", i, ptScnList[i].scn_state,
                  ptScnList[i].screen_off_action);
            perfScnEnable(i);
        }
    }
...
extern "C" int perfUserScnCheckAll(void) {
    int i;
    struct stat stat_buf;
    int exist;
    char proc_path[128];
...
extern "C" int perfSetSysInfo(int type, const char* data) {
    int ret = 0;
    ...
    switch (type) {
        ...
        case SETSYS_SPORTS_APK:
            if (smart_check_pack_existed(APK_BENCHMARK, data, -1, -1) ==
                -1) {  // fake "uid" and "from uid"
                smart_add_specific_benchmark(data);
            }
            break;

        case SETSYS_FOREGROUND_SPORTS:
            if (smart_check_pack_existed(APK_BENCHMARK, foreground_pack, -1, -1) ==
                -1) {  // fake "uid" and "from uid"
        ...
        case SETSYS_PREDICT_INFO:
            ...
            if (str != NULL) {
                int uid = -1;
                pack_name = str;
                if ((str = strtok_r(NULL, " ", &saveptr)) != NULL) uid = atoi(str);
...
extern "C" int perfUserGetCapability(int cmd, int id) {
    int value = -1, idx = -1;
    nsecs_t now = 0;
    int interval = -1;
    ...
    switch (cmd) {
        ...
        case MTKPOWER_CMD_GET_CLUSTER_CPU_NUM:
            if (id >= 0 && id <= nClusterNum) value = ptClusterTbl[id].cpuNum;
            break;

        case MTKPOWER_CMD_GET_CLUSTER_CPU_FREQ_MIN:
            if (id >= 0 && id <= nClusterNum) value = ptClusterTbl[id].pFreqTbl[0];
            break;

        case MTKPOWER_CMD_GET_CLUSTER_CPU_FREQ_MAX:
            if (id >= 0 && id <= nClusterNum) value = ptClusterTbl[id].freqMax;
            break;
        ...
        case MTKPOWER_CMD_GET_FOREGROUND_TYPE:
            if (smart_check_pack_existed(APK_GAME, foreground_pack, foreground_uid,
                                         last_from_uid) != -1)
                value = APK_GAME;
            break;
        ...
        case MTKPOWER_CMD_GET_POWER_HINT_HOLD_TIME:
            if (checkSuccess(id)) value = (ptScnList[id].hint_hold_time * 1000);
            break;

        case MTKPOWER_CMD_GET_POWER_HINT_EXT_HINT:
            if (checkSuccess(id)) value = ptScnList[id].ext_hint;
            break;
        ...
        case MTKPOWER_CMD_GET_TIME_TO_LAST_TOUCH:
            ...
            if (interval > 100000 || interval < 0)
                ...
                value = interval;
            ALOGD("perfUserGetCapability - now:%ld, value:%ld interval:%d", now, last_touch_time,
                  interval);
            break;

        case MTKPOWER_CMD_GET_POWER_HINT_EXT_HINT_HOLD_TIME:
            if (checkSuccess(id)) value = (ptScnList[id].ext_hint_hold_time * 1000);
            break;
...
extern "C" int perfCusUserRegScn(void) {
...
extern "C" int perfUserRegScn(int pid, int tid) {
    int i, handle = -1;
    char filepath[64] = "\0";
...
extern "C" int perfUserRegScnConfig(int handle, int cmd, int param_1, int param_2, int param_3,
                                    int param_4) {
    int idx = -1;
    Mutex::Autolock lock(sMutex);
    if (!nIsReady)
        if (!init()) return 0;
    ALOGD("perfUserRegScnConfig - handle:%d, cmd:%d, p1:%d, p2:%d, p3:%d, p4:%d", handle, cmd,
          param_1, param_2, param_3, param_4);

    idx = findHandleToIndex(handle);
    if ((idx < nUserScnBase || idx >= nUserScnBase + REG_SCN_MAX) &&
        param_4 != 1)  // param_4 == 1 => modify pre-defined scn
        return -1;
...
extern "C" int perfUserUnregScn(int handle) {
    int idx = -1;
...
extern "C" int perfSetUidInfo(int uid, int fromUid) {
    Mutex::Autolock lock(sMutex);
    if (!nIsReady)
        if (!init()) return 0;

    if (foreground_uid == uid && last_from_uid == fromUid) return -1;
...
extern "C" int perfLockAcq(int* list, int handle, int size, int pid, int tid, int duration) {
    int i, idx = -1;
    int exist;
    ...
    if (size % 2 == 0) {
        for (i = 0; i < size; i += 2) ALOGD("data:0x%08x, %d", list[i], list[i + 1]);
    }
    ...
    if (handle <= 0) goto regiter_handle;
...
regiter_handle:
    ...
    if (hdl_enabled == 0) {
        ...
        if (ptScnList[idx].lock_rsc_size != size ||
            ...
            if (ptScnList[idx].lock_rsc_size != size) {
                ptScnList[idx].lock_rsc_size = size;
                if ((ptScnList[idx].lock_rsc_list = (int*)realloc(ptScnList[idx].lock_rsc_list,
    ...
    ALOGI("perfLockAcq hdl:%d pid:%d duration:%d pid_exist:%d => new_hdl:%d", handle, pid, duration,
          exist, new_handle);
    if (hdl_enabled == 1) {
        if (rsc_modified == 1 || force_update == 1) perfScnUpdate(idx, 1);  // update setting
    } else {
        perfScnEnable(idx);
    }
...
extern "C" int perfLockRel(int handle) {
    int idx = -1;
    ...
    if (ptScnList[idx].scn_state == STATE_ON) return -1;
...
extern "C" int perfLibpowerInit(void) {
    Mutex::Autolock lock(sMutex);
...
int loadRscTable(int power_on_init) {
    ...
    for (i = 0; i < gRscCtlTblLen; i++) {
        ALOGI("[loadRscTable] RscCfgTbl[%d] cmdName:%s cmdID:%x, param:%d, defaultVal:%d comp:%d "
              "maxVal:%d",
              i, RscCfgTbl[i].cmdName.c_str(), RscCfgTbl[i].cmdID, RscCfgTbl[i].defaultVal,
              RscCfgTbl[i].comp, RscCfgTbl[i].maxVal, RscCfgTbl[i].minVal);

        /*reset all Rsctable*/
        if (RscCfgTbl[i].init_func != NULL) ret = RscCfgTbl[i].init_func(power_on_init);
        ...
        ALOGD("[loadRscTable] gRscCtlTbl[%d] resetVal:%d curVal:%d isValid:%d log:%d", i,
              gRscCtlTbl[i].resetVal, gRscCtlTbl[i].curVal, gRscCtlTbl[i].isValid,
              gRscCtlTbl[i].log);
    }
...
int whilelist_reload(void) {
    ...
    if ((ptScnList = (tScnNode*)realloc(
                 ptScnList, sizeof(tScnNode) * (SCN_APP_RUN_BASE + nPackNum))) == NULL) {
        ALOGE("Can't allocate memory");
        return -1;
    }
