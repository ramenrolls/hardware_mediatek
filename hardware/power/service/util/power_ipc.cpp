...
static const int _CntofConfigHandlerMapper =
        sizeof(_ConfigHandlerMapper) / sizeof(struct tConfigHanderMap);
...
static const int _CntofConfigHandlerMarshallMapper =
        sizeof(_ConfigHandlerMarshallMapper) / sizeof(struct tConfigHanderMarshallMap);
...
static int _PS_Cfg_Hdler_Mar_MEMORY(tPS_CMD* pCmd, char* pBuffer, int BufferLen, int* pUsedLen) {
    ...
    *pUsedLen = 0;
    ...
    // TWPCDBGP("SCN T L: %x %x %x %x\n", *(pBuffer), *(pBuffer + 1), *(pBuffer + 2), *(pBuffer +
    // 3)); TWPCDBGP("SCN V: %x %x %x %x\n", *(pBuffer + 4), *(pBuffer + 5), *(pBuffer + 6),
    // *(pBuffer + 7));
    ...
    while (vHandledLen < pJob->ContentLen) {
        ...
        for (vI = 0; vI < _CntofConfigHandlerMapper; vI++) {
            if (_ConfigHandlerMapper[vI].Idx == vCfgType) {
                vRet = _ConfigHandlerMapper[vI].pHandler(vpCfg, pJob->ContentLen - vHandledLen,
                                                         vpCmd, &vUsedLen);
...
SecondRound:
    ...
    if (pJob->RecvLen < PS_IPC_COM_HEADER_LEN) {
        ...
        vRet = recv(PSDc_fd, pJob->pContent + (pJob->RecvLen - PS_IPC_COM_HEADER_LEN), vWantRecvLen,
                    0);
    } else {
        // should not happen
...
int powerd_ipc_sendcmd(tPS_IPC_SendJob* pJob, int PSDc_fd) {
    if (send(PSDc_fd, pJob->pData, pJob->DataLen, MSG_NOSIGNAL) < 0) return -1;
...
int powerd_cmd_marshall(tPS_IPC_SendJob* pJob, tPS_CMD* pCmd) {
    ...
    if (vBufLen > 0) vpBuf = (char*)malloc(vBufLen);

    if (vpBuf == NULL) return -1;
    ...
    // 1. Predict buffer len
    for (vI = 0; vI < _CntofConfigHandlerMarshallMapper; vI++) {
        vRet = _ConfigHandlerMarshallMapper[vI].pMarshallHandler(pCmd, vpCfgBuf, vCfgBufLen,
                                                                 &vCfgUsedLen);
...
int powerd_ipc_resetRecvJob(tPS_IPC_RecvJob* pJob) {
    if (pJob->pContent) free(pJob->pContent);
