...
static void _Powerd_handleIPC(_tGlobeContext_* pCtxt, _tTCPCliContext_* pCliCtxt, int isRead,
                              int isWrite) {
    int vRet;
    int vHandle;
...
//   int r_anti_clone = drvb_f0();
    ...
    while (1) {
        ...
        // TWPCDBGP("vMaxfd %d, ovtv sec %lu usec %lu, vtv sec %lu usec %lu, vRet %d", vMaxfd,
        // ovtv.tv_sec, ovtv.tv_usec, vtv.tv_sec, vtv.tv_usec, vRet);
...
static int _slave_main(int argc, char** argv, void (*listen_cb)(void)) {
    ...
    vTCPServFD =
            socket_local_server(_TCP_BIND_ADDR_, ANDROID_SOCKET_NAMESPACE_ABSTRACT, SOCK_STREAM);
    ...
    if (listen(vTCPServFD, 1024) < 0) goto exit;
