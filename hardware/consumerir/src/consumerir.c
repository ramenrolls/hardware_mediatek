...
//          Mode
//////////////////////////////////////////////////////////
static int signals_generate(int type, int carrier_freq, const int pattern[], int pattern_len,
    ...
    if (type == 1 || type == 0) {
        ...
        if (irtx_hal_log_enabled()) {
            for (i = 0; i < (pattern_len >> 3); i++) {
                int ii = (i << 3);
                ALOGD("temp[%d] - %d, %d, %d, %d, %d, %d, %d, %d\n", ii, temp[ii + 0], temp[ii + 1],
                      temp[ii + 2], temp[ii + 3], temp[ii + 4], temp[ii + 5], temp[ii + 6],
                      temp[ii + 7]);
            }
            i = (i << 3);
            for (; i < pattern_len; i++) ALOGD("%d, ", temp[i]);
        }
    } else if (type != 0) {
    ...
    buffer_len =
            ceil(total_tick / (float)32) + 1;  // number of integers, one bit for one micro-seconds
    ...
    if (type == 0) {
        ...
        for (i = 0; i < pattern_len; i++) {
            ...
            for (j = 0; j < send_signal; j++) {
                if (current_level) {
                    if ((cycle_counter * whole_cycle <= j) &&
                        (j < (cycle_counter * whole_cycle + duty_cycle)) && j <= normal_range) {
                        *(wave_buffer + int_ptr) |= (1 << bit_ptr);
                    } else {
                        *(wave_buffer + int_ptr) &= (~(1 << bit_ptr));
                    }
 ...
 *-------------------------------------------------------------------*/
static int consumerir_transmit(struct consumerir_device* dev __unused, int carrier_freq,
    ...
    if (ret == -1)
        if (irtx_hal_log_enabled()) ALOGD("IRTX_IOC_SET_CARRIER_FREQ %d\n", errno);

    duty_cycle = (1 << 16) | 3;
    ret = ioctl(fd, IRTX_IOC_SET_DUTY_CYCLE, &duty_cycle);
    if (ret == -1)
        if (irtx_hal_log_enabled()) ALOGD("IRTX_IOC_SET_DUTY_CYCLE %d\n", errno);

    // Generate Signals
    ret = signals_generate(solution_type, carrier_freq, pattern, pattern_len, &wave_buffer,
                           &buffer_len);
    if (ret < 0) {
    ...
    if (irtx_hal_log_enabled()) {
        ...
        for (i = 0; i < (buffer_len >> 3); i++) {
            ...
            ALOGD("&wave_buffer[%d]:%p - 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x\n", ii,
                  &(wave_buffer[ii]), wave_buffer[ii + 0], wave_buffer[ii + 1], wave_buffer[ii + 2],
                  wave_buffer[ii + 3], wave_buffer[ii + 4], wave_buffer[ii + 5],
                  wave_buffer[ii + 6], wave_buffer[ii + 7]);
        }
        i = (i << 3);
        for (; i < buffer_len; i++) {
...
static int consumerir_get_carrier_freqs(struct consumerir_device* dev __unused, size_t len,
...
/*-------------------------------------------------------------------
 ...
 |  Returns:  The status code of this execution.
 *-------------------------------------------------------------------*/
static int consumerir_open(const hw_module_t* module, const char* name, hw_device_t** device) {
