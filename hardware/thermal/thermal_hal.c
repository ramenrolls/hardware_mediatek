...
static const char* CPU_ALL_LABEL[] = {"CPU0", "CPU1", "CPU2", "CPU3", "CPU4",
...
static ssize_t mtk_get_temperatures(thermal_module_t* module, temperature_t* list, size_t size) {
    ...
    if (file == NULL) {
        ALOGW("thermal_hal: %s : failed to open file %s: %s", __func__, TSCPU_PATH,
              strerror(-errno));
    } else {
        if (fscanf(file, "%f", &temp) > 0) {
            ...
            ALOGW("thermal_hal: %s: failed to fscanf %s: %s", __func__, TSCPU_PATH,
                  strerror(-errno));
    ...
    if (file == NULL) {
        ALOGW("thermal_hal: %s: failed to open directory %s: %s", __func__, TSGPU_PATH,
              strerror(-errno));
    } else {
        if (fscanf(file, "%f", &temp) > 0) {
            temp_gpu = temp;
            ...
            ALOGW("thermal_hal: %s: failed to fscanf %s: %s", __func__, TSGPU_PATH,
                  strerror(-errno));
    ...
    if (file == NULL) {
        ALOGW("thermal_hal: %s: failed to open directory %s: %s", __func__, TSBAT_PATH,
              strerror(-errno));
    } else {
        if (fscanf(file, "%f", &temp) > 0) {
            temp_bat = temp;
            ...
            ALOGW("thermal_hal: %s: failed to fscanf %s: %s", __func__, TSBAT_PATH,
                  strerror(-errno));
    ...
    if (file == NULL) {
        ALOGW("thermal_hal: %s: failed to open directory %s: %s", __func__, TSSKIN_PATH,
              strerror(-errno));
    } else {
        if (fscanf(file, "%f", &temp) > 0) {
            temp_tskin = temp;
            ...
            ALOGW("thermal_hal: %s: failed to fscanf %s: %s", __func__, TSSKIN_PATH,
                  strerror(-errno));
    ...
    TH_DLOG(ANDROID_LOG_INFO, "%s: cpu = %f, gpu = %f, bat = %f, tskin = %f", __func__, temp_cpu,
            temp_gpu, temp_bat, temp_tskin);
...
static ssize_t get_cpu_usages(thermal_module_t* module, cpu_usage_t* list) {
    ...
    if (((max_core_num + 1) > cpu_array) || ((max_core_num + 1) <= 0)) {
        ALOGW("thermal_hal: %s: max_core_num = %d, cpu_array = %d", __func__, max_core_num,
              cpu_array);
        return -1;
    }
    ...
    for (i = 0; i < max_core_num; i++) {
        list[i] = (cpu_usage_t){.name = CPU_ALL_LABEL[i], .active = 0, .total = 0, .is_online = 0};
    }
    ...
    while ((read = getline(&line, &len, file)) != -1) {
        ...
        if (vals != 5) {
            ALOGW("thermal_hal: %s: failed to read CPU information from file: %s", __func__,
                  strerror(errno));
            fclose(file);
    ...
    TH_DLOG(ANDROID_LOG_INFO, "%s end loop, size %d, cpu_num = %d, max_core_num = %d", __func__,
            size, cpu_num, max_core_num);
