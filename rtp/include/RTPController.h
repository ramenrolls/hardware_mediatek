...
class RTPController : public IRTPController {
  public:
    ...
    virtual status_t setConfigParams(rtp_rtcp_config_t* pRTPNegotiatedParams,
                                     uint8_t uiMediaType = IMSMA_RTP_VIDEO);
...
#ifdef UT_PCAP_TEST
    virtual sp<SocketWrapper> getVideoRTPSocketWrapper() { return mVideoRTPSocketWrapper; }
#endif
  ...
  private:
    ...
    bool mDowngradeEnable;   // we don't trigger downgrade when TMO audio early-media (no date
                             // receive)
    ...
    // status_t onPause(uint8_t rtpPath = IMSMA_RTP_UP_DOWN_LINK,uint8_t trackIndex =
    // IMSMA_RTP_VIDEO); status_t onResume(uint8_t rtpPath,uint8_t trackIndex); status_t
    // onHoldOn(int32_t isHoldOnEnable);

    int32_t findSocketFd(uint16_t port);

    status_t queueRTCPPacket(sp<ABuffer>& packet, uint8_t trackIndex = IMSMA_RTP_VIDEO);
    // status_t processReportBlocks(uint8_t RC,uint32_t length,uint8_t trackIndex =
    // IMSMA_RTP_VIDEO);
