...
class IRTPController : virtual public RefBase {
  public:
    ...
    enum rtp_notification_reason {
        ...
        kWhatTimeOut =
                'tiou',  // when no packet received during timeout,with trackindex number as param

        kWhatByeReceived = 'rBye',  // receive bye rtcp from remote side

        // AVPF notify
        // kWhatNeedRefreshPoint = 'refP',//if receive FIR,PLI and the packet lost info in Generic
        // NAKC is too serious                           // need notify this mess
        ...
        kWhatBandWithChanged = 'badw',  // when serveral continuous sending operation consum too
                                        // much time and too many packet delayed
    ...
    /**
     *@ Description: static function of get RTP module capability, RTP may support serveral media
     *type
     *@                    and for each media type may be support serverl packetization mode,rtcp
     *fds
     *@
     *@ Parameters:
     ...
     *@        [in]uiMediaType: the media type of which caller wish to get capability, default value
     *is Video
     *@ Return:
     *@    status_t, OK or UNKNOWN_ERROR
     */
    static status_t getCapability(rtp_rtcp_capability_t** pRTPCapArray, uint8_t* pNumOfCap,
                                  uint8_t uiMediaType = IMSMA_RTP_VIDEO, uint32_t operatorID = 0);

    /**
     *@ Description: another static function of get RTP module capability, RTP may support serveral
     *media type
     *@                    and for each media type may be support serverl packetization mode,rtcp
     *fds
     *@                caller can choose either one getCapability
     *@ Parameters:
     *@        [in/out]pRTPCapArray: pointer pointing to array of pointers which need allocated by
     *caller
     ...
     */
    static uint8_t getCapability(rtp_rtcp_capability_t** pRTPCapArray, uint8_t uiMaxNumOfCap,
                                 uint8_t uiMediaType = IMSMA_RTP_VIDEO, uint32_t operatorID = 0);
     ...
     */
    static List<rtp_rtcp_capability_t*> getCapability(uint8_t uiMediaType = IMSMA_RTP_VIDEO,
     ...
     *@        [in]uiMediaType: the media type of which caller wish to set params, default value is
     *Video
     *@ Return:
     *@    status_t type, OK indicate successful, otherwise error type will return
     */
    virtual status_t setConfigParams(rtp_rtcp_config_t* pRTPNegotiatedParams,
                                     uint8_t uiMediaType = IMSMA_RTP_VIDEO) = 0;
     ...
     *@    status_t type, OK indicate successful, otherwise error type will return
     */
    virtual status_t updateConfigParams(rtp_rtcp_config_t* pRTPNegotiatedParams,
                                        uint8_t uiMediaType = IMSMA_RTP_VIDEO) = 0;
     ...
     *@        [in]uiMediaType: the media type of which observer wish to listen, default value is
     *Video
     *@ Return:
     *@        status_t type, OK indicate successful, otherwise error type will return
     */
    virtual status_t setAccuNotify(const sp<AMessage>& accessUnitNotify,
                                   uint8_t uiMediaType = IMSMA_RTP_VIDEO) = 0;
     ...
     *@
     *@ Parameters:
     *@        [in]rtpPath: path need to add.IMSMA_RTP_UPLINK for uplink path; IMSMA_RTP_DOWNLINK
     *for downlink path
     ...
     *@        status_t type, OK indicate successful, otherwise error type will return
     */
    virtual status_t addStream(uint8_t rtpPath = IMSMA_RTP_UP_DOWN_LINK,
                               uint8_t trackIndex = IMSMA_RTP_VIDEO) = 0;

    /**
     *@ Description: remove stream from RTP Controller
     *@
     *@ Parameters:
     *@        [in]rtpPath: path need to remove.IMSMA_RTP_UPLINK for uplink path; IMSMA_RTP_DOWNLINK
     *for downlink path
     *@            default is IMSMA_RTP_UP_DOWN_LINK which means both uplink and downlink path
     ...
     *@        status_t type, OK indicate successful, otherwise error type will return
     */
    virtual status_t removeStream(uint8_t rtpPath = IMSMA_RTP_UP_DOWN_LINK,
                                  uint8_t trackIndex = IMSMA_RTP_VIDEO) = 0;
     ...
     *@
     *@ Parameters:
     *@        [in]rtpPath: path need to start.IMSMA_RTP_UPLINK for uplink path; IMSMA_RTP_DOWNLINK
     *for downlink path
     *@            default is IMSMA_RTP_UP_DOWN_LINK which means both uplink and downlink path
     ...
     *@        status_t type, OK indicate successful, otherwise error type will return
     */
    virtual status_t start(uint8_t rtpPath = IMSMA_RTP_UP_DOWN_LINK,
                           uint8_t trackIndex = IMSMA_RTP_VIDEO) = 0;
     ...
     *@
     *@ Parameters:
     *@        [in]rtpPath: path need to pause.IMSMA_RTP_UPLINK for uplink path; IMSMA_RTP_DOWNLINK
     *for downlink path
     *@            default is IMSMA_RTP_UP_DOWN_LINK which means both uplink and downlink path
     ...
     *@        status_t type, OK indicate successful, otherwise error type will return
     */
    // virtual status_t pause(uint8_t rtpPath = IMSMA_RTP_UP_DOWN_LINK, uint8_t trackIndex =
    // IMSMA_RTP_VIDEO ) = 0;
     ...
     *@
     *@ Parameters:
     *@        [in]rtpPath: path need to resume.IMSMA_RTP_UPLINK for uplink path; IMSMA_RTP_DOWNLINK
     *for downlink path
     *@            default is IMSMA_RTP_UP_DOWN_LINK which means both uplink and downlink path
     ...
     *@        status_t type, OK indicate successful, otherwise error type will return
     */
    // virtual status_t resume(uint8_t rtpPath = IMSMA_RTP_UP_DOWN_LINK, uint8_t trackIndex =
    // IMSMA_RTP_VIDEO ) = 0;
     ...
     *@
     *@ Parameters:
     *@        [in]rtpPath: path need to stop.IMSMA_RTP_UPLINK for uplink path; IMSMA_RTP_DOWNLINK
     *for downlink path
     *@            default is IMSMA_RTP_UP_DOWN_LINK which means both uplink and downlink path
     ...
     *@        status_t type, OK indicate successful, otherwise error type will return
     */
    virtual status_t stop(uint8_t rtpPath = IMSMA_RTP_UP_DOWN_LINK,
                          uint8_t trackIndex = IMSMA_RTP_VIDEO) = 0;
     ...
     *@        status_t type, OK indicate successful, otherwise error type will return
     */
    virtual status_t queueAccessUnit(const sp<ABuffer>& buffer,
                                     uint8_t trackIndex = IMSMA_RTP_VIDEO) = 0;
     ...
     *@                 RTP controller will contain orientation info to each rtp packet after
     *receive thie message
     *@                 But the suggested way is that the caller contain the orientation related to
     *a buffer
     ...
     *@        status_t type, OK indicate successful, otherwise error type will return
     */
    virtual status_t setVideoOrientation(uint8_t rotation, uint8_t camera_facing = 0,
                                         uint8_t flip = 0) = 0;
    ...
    ********************************************************************/
     ...
     *@                 error info like sli,pli,fir, which need inform to peer encoder to enhance
     *error resilience
     *@
     *@ Parameters:
     *@        [in]errInfo:errInfo
     ...
     *@        void
     */
    virtual void feedBackDecoderError(sp<AMessage> errInfo,
                                      uint8_t trackIndex = IMSMA_RTP_VIDEO) = 0;
