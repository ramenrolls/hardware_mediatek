...
status_t IRTPController::getCapability(rtp_rtcp_capability_t** pRTPCapArray, uint8_t* pNumOfCap,
                                       uint8_t uiMediaType, uint32_t operatorID) {
    ALOGD("%s,capArray %p,pNumOfCap %p,media type 0x%x,", __FUNCTION__, pRTPCapArray, pNumOfCap,
          uiMediaType);
    ...
    if (uiMediaType == IMSMA_RTP_VIDEO) {
        if (!sVideoCapParamsFirstItem) {
            ...
            memset(rtp_cap[0].rtp_ext_map[0].extension_uri, 0,
                   sizeof(rtp_cap[0].rtp_ext_map[0].extension_uri));
            // ToDo: sizof char* will return sizeof string? or 4
            // why can't sizeof(extension_uri)
            memcpy(rtp_cap[0].rtp_ext_map[0].extension_uri, extension_uri,
                   26);  // sizeof(extension_uri));
            ...
            ALOGD("\t media(0x%x),mime(0x%x),profile(0x%x),sampleRate(%d)", rtp_cap[0].media_type,
                  rtp_cap[0].mime_Type, rtp_cap[0].rtp_profile, rtp_cap[0].sample_rate);

            ALOGD("\t ext_header_num:%d", rtp_cap[0].rtp_header_extension_num);

            for (uint8_t i = 0; i < rtp_cap[0].rtp_header_extension_num; i++) {
                ALOGD("\t ext_header[%d]:id(%d),uri(%s)", i, rtp_cap[0].rtp_ext_map[i].extension_id,
                      rtp_cap[0].rtp_ext_map[i].extension_uri);
            }

            ALOGD("\t packetize_mode(%d),rtcp_rsize(%d)", rtp_cap[0].packetize_mode,
                  rtp_cap[0].rtcp_reduce_size);
            ...
            */
            ...
            memset(rtp_cap[0].rtcp_fb_type[0].rtcp_fb_sub_param, 0,
                   sizeof(rtp_cap[0].rtcp_fb_type[0].rtcp_fb_sub_param));
            ...
            memset(rtp_cap[0].rtcp_fb_type[1].rtcp_fb_sub_param, 0,
                   sizeof(rtp_cap[0].rtcp_fb_type[1].rtcp_fb_sub_param));
            ...
            */
            ...
            memset(rtp_cap[0].rtcp_fb_type[2].rtcp_fb_sub_param, 0,
                   sizeof(rtp_cap[0].rtcp_fb_type[2].rtcp_fb_sub_param));

            rtp_cap[0].rtcp_fb_type[3].rtcp_fb_id = IMSMA_CCM;
            rtp_cap[0].rtcp_fb_type[3].rtcp_fb_param = IMSMA_FIR;
            memset(rtp_cap[0].rtcp_fb_type[3].rtcp_fb_sub_param, 0,
                   sizeof(rtp_cap[0].rtcp_fb_type[3].rtcp_fb_sub_param));
            ALOGI("%s,3-id:%d,3-param:%d,sub:%s", __FUNCTION__,
                  rtp_cap[0].rtcp_fb_type[3].rtcp_fb_id, rtp_cap[0].rtcp_fb_type[3].rtcp_fb_param,
                  rtp_cap[0].rtcp_fb_type[3].rtcp_fb_sub_param);

            ///*not set trr-int
            rtp_cap[0].rtcp_fb_type[4].rtcp_fb_id = IMSMA_TRR_INT;
            rtp_cap[0].rtcp_fb_type[4].rtcp_fb_param = 1000;
            memset(rtp_cap[0].rtcp_fb_type[4].rtcp_fb_sub_param, 0,
                   sizeof(rtp_cap[0].rtcp_fb_type[4].rtcp_fb_sub_param));
            //*/

            rtp_cap[0].rtcp_fb_type[5].rtcp_fb_id = IMSMA_CCM;
            rtp_cap[0].rtcp_fb_type[5].rtcp_fb_param = IMSMA_TMMBR;
            memset(rtp_cap[0].rtcp_fb_type[5].rtcp_fb_sub_param, 0,
                   sizeof(rtp_cap[0].rtcp_fb_type[5].rtcp_fb_sub_param));

            ALOGD("\t fb_num(%d)", rtp_cap[0].rtcp_fb_param_num);

            for (uint8_t j = 0; j < rtp_cap[0].rtcp_fb_param_num; j++) {
                ALOGD("\t Feedback[%d]:fb_id(%d),fb_param(%d),fb_sub_param(%s)", j,
                      (rtp_cap[0].rtcp_fb_type[j]).rtcp_fb_id,
                      (rtp_cap[0].rtcp_fb_type[j]).rtcp_fb_param,
            ...
            ALOGD("%s, op id change %d-->%d reset rtp_profile %d", __FUNCTION__, opID, operatorID,
                  sVideoCapParamsFirstItem[0].rtp_profile);
        }

        *pRTPCapArray = sVideoCapParamsFirstItem;
        *pNumOfCap = sVideoCapParamsNum;
        opID = operatorID;
        ALOGI("%s,cap array first item address:%p, cap num:%d rtp_profile(%d)", __FUNCTION__,
              sVideoCapParamsFirstItem, sVideoCapParamsNum,
              sVideoCapParamsFirstItem[0].rtp_profile);
        return OK;
...
uint8_t IRTPController::getCapability(rtp_rtcp_capability_t** pRTPCapArray, uint8_t uiMaxNumOfCap,
                                      uint8_t uiMediaType, uint32_t operatorID) {
    ALOGD("%s,capArray %p,uiMaxNumOfCap %d,media type 0x%x,", __FUNCTION__, pRTPCapArray,
          uiMaxNumOfCap, uiMediaType);
    // ToDo: if only assign value to pRTPCapArray, but not use,will build error
    // so we print the value of it to pass build
    ...
    if (uiMediaType == IMSMA_RTP_VIDEO) {
        if (!sVideoCapParams) {
            ...
            // rtp_cap->rtp_ext_map[0].extension_uri = "urn:3gpp:video-orientation";
            const char* extension_uri = "urn:3gpp:video-orientation";
            memset(rtp_cap->rtp_ext_map[0].extension_uri, 0,
                   sizeof(rtp_cap->rtp_ext_map[0].extension_uri));
            // ToDo: sizof char* will return sizeof string? or 4
            // why can't sizeof(extension_uri)
            memcpy(rtp_cap->rtp_ext_map[0].extension_uri, extension_uri,
                   26);  // sizeof(extension_uri));
            ...
            ALOGD("\t media(0x%x),mime(0x%x),profile(0x%x),sampleRate(%d)", rtp_cap->media_type,
                  rtp_cap->mime_Type, rtp_cap->rtp_profile, rtp_cap->sample_rate);

            ALOGD("\t ext_header_num:%d", rtp_cap->rtp_header_extension_num);

            for (uint8_t i = 0; i < rtp_cap->rtp_header_extension_num; i++) {
                ALOGD("\t ext_header[%d]:id(%d),uri(%s)", i, rtp_cap->rtp_ext_map[i].extension_id,
                      rtp_cap->rtp_ext_map[i].extension_uri);
            }

            ALOGD("\t packetize_mode(%d),rtcp_rsize(%d)", rtp_cap->packetize_mode,
                  rtp_cap->rtcp_reduce_size);
            ...
            */
            ...
            memset(rtp_cap->rtcp_fb_type[0].rtcp_fb_sub_param, 0,
                   sizeof(rtp_cap->rtcp_fb_type[0].rtcp_fb_sub_param));
            ...
            memset(rtp_cap->rtcp_fb_type[1].rtcp_fb_sub_param, 0,
                   sizeof(rtp_cap->rtcp_fb_type[1].rtcp_fb_sub_param));
            ...
            */
            ...
            memset(rtp_cap->rtcp_fb_type[2].rtcp_fb_sub_param, 0,
                   sizeof(rtp_cap->rtcp_fb_type[2].rtcp_fb_sub_param));

            rtp_cap->rtcp_fb_type[3].rtcp_fb_id = IMSMA_CCM;
            rtp_cap->rtcp_fb_type[3].rtcp_fb_param = IMSMA_FIR;
            memset(rtp_cap->rtcp_fb_type[3].rtcp_fb_sub_param, 0,
                   sizeof(rtp_cap->rtcp_fb_type[3].rtcp_fb_sub_param));
            ALOGI("%s,3-id:%d,3-param:%d,sub:%s", __FUNCTION__, rtp_cap->rtcp_fb_type[3].rtcp_fb_id,
                  rtp_cap->rtcp_fb_type[3].rtcp_fb_param,
                  rtp_cap->rtcp_fb_type[3].rtcp_fb_sub_param);

            ///*not set trr-int
            rtp_cap->rtcp_fb_type[4].rtcp_fb_id = IMSMA_TRR_INT;
            rtp_cap->rtcp_fb_type[4].rtcp_fb_param = 1000;
            memset(rtp_cap->rtcp_fb_type[4].rtcp_fb_sub_param, 0,
                   sizeof(rtp_cap->rtcp_fb_type[4].rtcp_fb_sub_param));
            //*/

            rtp_cap->rtcp_fb_type[5].rtcp_fb_id = IMSMA_CCM;
            rtp_cap->rtcp_fb_type[5].rtcp_fb_param = IMSMA_TMMBR;
            memset(rtp_cap->rtcp_fb_type[5].rtcp_fb_sub_param, 0,
                   sizeof(rtp_cap->rtcp_fb_type[5].rtcp_fb_sub_param));

            ALOGD("\t fb_num(%d)", rtp_cap->rtcp_fb_param_num);

            for (uint8_t j = 0; j < rtp_cap->rtcp_fb_param_num; j++) {
                ALOGD("\t Feedback[%d]:fb_id(%d),fb_param(%d),fb_sub_param(%s)", j,
                      (rtp_cap->rtcp_fb_type[j]).rtcp_fb_id,
                      (rtp_cap->rtcp_fb_type[j]).rtcp_fb_param,
...
List<rtp_rtcp_capability_t*> IRTPController::getCapability(uint8_t uiMediaType,
                                                           uint32_t operatorID) {
    ...
    if (uiMediaType == IMSMA_RTP_VIDEO) {
        if (sVideoCapParamsList.empty()) {
            ...
            *rtp_cap2 = *rtp_cap;
            ...
            for(uint8_t i = 0; i < rtp_cap->rtp_header_extension_num; i++){
                ALOGD("\t
            ext_header[%d]:id(%d),uri(%s)",i,rtp_cap->rtp_ext_map[i].extension_id,rtp_cap->rtp_ext_map[i].extension_uri);
            }

            ALOGD("\t b=AS(%d),packetize_mode(%d),b=RS(%" PRId64 "),b=RR(%" PRId64
            "),rtcp_rsize(%d)",\
...
IRTPController::IRTPController() : RefBase() {}
