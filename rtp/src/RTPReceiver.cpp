...
RTPReceiver::~RTPReceiver() { ALOGI("%s", __FUNCTION__); }
status_t RTPReceiver::addStream(rtp_rtcp_config_t* pRTPNegotiatedParams,
                                sp<SocketWrapper> socketWrapper, sp<AMessage> accuNotify,
                                int32_t trackIndex) {
...
status_t RTPReceiver::updateConfigParams(rtp_rtcp_config_t* pRTPNegotiatedParams,
                                         int32_t trackIndex) {
    ALOGD("%s,track %d save negotiated done params %p", __FUNCTION__, trackIndex,
          pRTPNegotiatedParams);
...
status_t RTPReceiver::processSenderInfo(const sp<ABuffer>& buffer, uint32_t uSSRC,
                                        uint8_t trackIndex) {
    ...
    switch (msg->what()) {
        ...
        */
        ...
        case kWhatTrackNotify: {
            int32_t trackIndex = IMSMA_RTP_VIDEO;
            msg->findInt32("trackIndex", &trackIndex);
            ALOGV("kWhatTrackNotify,trackIndex(%d),trackInfos num(%zu)", trackIndex,
                  mpTrackInfos.size());
            ...
            if (what == RTPSource::kWhatTimeUpdate) {
                ...
                // int64_t ntpTimeUs = (ntpTime >> 32)*1E6 + (double)(ntpTime & 0xffffffff) * 1E6 /
                // (1ll << 32); int64_t ntpTimeUsTemp = (int64_t)(((ntpTime >> 32) +
                // (double)(ntpTime & 0xffffffff) / (1ll << 32))*1E6);

                ALOGI("%s,kWhatTimeUpdate track(%d)(rtpTime:%u,ntpTime:%" PRId64 "us", __FUNCTION__,
                      trackIndex, rtpTime, ntpTimeUs);  //,ntpTimeUsTemp);
                ...
                if (pTrack->mTimeMapped) {
                    ...
                    if (rtpTime < pTrack->mRtpTimeAnchor) {
                        if ((pTrack->mRtpTimeAnchor - rtpTime) > 0x40000000) {
                            ...
                            ALOGI("%s,kWhatTimeUpdate,rtp time overflow times( %d )( %d -> %d )",
                                  __FUNCTION__, pTrack->mSRRtpTimeCycles, pTrack->mRtpTimeAnchor,
                                  rtpTime);
                        }
                    }
                }

                pTrack->mExtenedRtpTimeAnchor =
                        (int64_t)rtpTime + kMaxUint32bitValue * (int64_t)pTrack->mSRRtpTimeCycles;
                ...
                ALOGI("kWhatTimeUpdate:track(%d),sys_time(%" PRId64 "us)", trackIndex,
                      pTrack->mLastNTPTimeUpdateUs);

                ALOGI("kWhatTimeUpdate:mRtpTimeAnchor:%u tick,mExtenedRtpTimeAnchor:%" PRId64
                      " tick,mNtpTimeAnchor:%" PRId64 "us",
                      pTrack->mRtpTimeAnchor, pTrack->mExtenedRtpTimeAnchor,
                      pTrack->mNtpTimeAnchor);
                ...
                //     we can evaluate the decoding speed according the reponses and the accumulated
                //     accu queue
                // 3GVT no queue--maybe different with 4G VT, 3GVT is circuit switch with fix
                // bandwidth but 4GVT is packet switch without guarantee network but the other hand:
                // VT is real time application... so first we didn't queue the accu
                sp<ABuffer> accu;

                if ((trackIndex == IMSMA_RTP_VIDEO) && msg->findBuffer("access-unit", &accu) &&
                    accu.get()) {
                    ...
                    ALOGV("EarliestPacket_token=%d latestPacekt_token=%d damagedAu=%d", early_seq,
                          end_seq, damagedAu);

                    if (pTrack->mLastDamageFlag == false && pTrack->mLastEndSeq != -1 &&
                        (pTrack->mLastEndSeq + 1 != early_seq)) {
                    ...
                    iExtenedRtpTime = (int64_t)rtp_time +
                                      kMaxUint32bitValue * (int64_t)pTrack->mRtpTimeCycles;
                    // int64_t mediaTimeUs = (int64_t)(rtp_time - (*i)->mFirstAccuRtpTime) *
                    // 1000000l / timeScale;
                    int64_t mediaTimeUs = (iExtenedRtpTime - (int64_t)pTrack->mFirstAccuRtpTime) *
                                          1000000l / timeScale;

                    accu_meta->setInt64("timeUs", mediaTimeUs);  // mediaTime calcuated from rtp
                                                                 // time
                    ...
                    if (!(*i)->mTimeMapped) {
                        ALOGV("video track notify, rtp<->ntp mapping not established  accu "
                              "timeUs(rtpTime) (%" PRId64 "us),rtp(%u tick),extRtp(%" PRId64
                              "tick),cycle(%d)",
                              mediaTimeUs, rtp_time, iExtenedRtpTime, pTrack->mRtpTimeCycles);
                        accu_meta->setInt64("ntp-time", 0);  // ToDo: check whether need revise to
                                                             // -1

                    } else {
                        ALOGV("rtp<->ntp mapping has established");
                        int64_t ntp_time = 0;
                        ntp_time = (iExtenedRtpTime - pTrack->mExtenedRtpTimeAnchor) * 1000000l /
                                           timeScale +
                                   pTrack->mNtpTimeAnchor;
                        accu_meta->setInt64("ntp-time", ntp_time);
                        ALOGV("video track notify, timeUs(ntpTime) (%" PRId64
                              "us) accu timeUs(rtpTime) (%" PRId64
                              "us),rtp(%u tick),extRtp(%" PRId64 "tick),cycle(%d)",
                              ntp_time, mediaTimeUs, rtp_time, iExtenedRtpTime,
                              pTrack->mRtpTimeCycles);
                    }
                ...
                for (int32_t i = 0; i < LostCount; i++) {
                    ...
                    for (List<uint32_t>::iterator j = (pTrack->mLostPacketSeqNumList).begin();
                         j != (pTrack->mLostPacketSeqNumList).end();) {
                ...
                if (list_size >= 8) {
                    ...
                    for (List<uint32_t>::iterator i = (pTrack->mLostPacketSeqNumList).begin();
                         i != (pTrack->mLostPacketSeqNumList).end(); i++) {
...
status_t RTPReceiver::onAddStream(rtp_rtcp_config_t* pRTPNegotiatedParams,
                                  sp<SocketWrapper> socketWrapper, sp<AMessage> accuNotify,
                                  int32_t trackIndex) {
...
status_t RTPReceiver::copyConfigParams(rtp_rtcp_config_t* srcParams, rtp_rtcp_config_t* dstParams) {
    ...
    for (uint32_t j = 0; j < srcParams->rtp_header_extension_num; j++) {
        ...
        memcpy(((dstParams->rtp_ext_map)[j]).extension_uri,
               ((srcParams->rtp_ext_map)[j]).extension_uri,
    ...
    for (uint8_t i = 0; i < dstParams->rtcp_fb_param_num; i++) {
        ...
        memcpy((dstParams->rtcp_fb_type)[i].rtcp_fb_sub_param,
               srcParams->rtcp_fb_type[i].rtcp_fb_sub_param,
    ...
    memcpy((dstParams->network_info).remote_rtp_address,
           (srcParams->network_info).remote_rtp_address,
           sizeof((srcParams->network_info).remote_rtp_address));
    memcpy((dstParams->network_info).remote_rtcp_address,
           (srcParams->network_info).remote_rtcp_address,
    ...
    memcpy((dstParams->network_info).local_rtcp_address,
           (srcParams->network_info).local_rtcp_address,
...
status_t RTPReceiver::onUpdateConfigParams(rtp_rtcp_config_t* pRTPNegotiatedParams,
                                           int32_t trackIndex) {
...
status_t RTPReceiver::changeSSRC(sp<ABuffer>& packet, int32_t trackIndex, uint32_t newSsrc) {
    ...
    if (!mVideoOtherSSRCQueue.empty() && (mSsrcInOtherQueue != newSsrc)) {
        ALOGD("%s,3th mSsrcInOtherQueue 0x%x, newssrc(%d)", __FUNCTION__, mSsrcInOtherQueue,
              newSsrc);
        mVideoOtherSSRCQueue.clear();
    }
...
status_t RTPReceiver::queueRTPPacket(sp<ABuffer>& packet, int32_t trackIndex, uint32_t* newSsrc) {
    ...
    *newSsrc = 0;
    ...
    if (seqN - mLastSeqN != 1) {
        ALOGD("%s,seq not serial track(%d),seqN(%d),mLastSeqN(%d)", __FUNCTION__, trackIndex, seqN,
              mLastSeqN);
    }
    ...
    if (data[0] & 0x10) {
        ...
        if (size < payloadOffset + 4 + extensionLength) {
            ALOGE("%s,extensionLength %zu %zu %zu", __FUNCTION__, extensionLength, payloadOffset,
                  size);
            return -1;
        }
        ...
        if ((extensionData[0] == 0xBE) && (extensionData[1] == 0xDE)) {
            ...
            while (offset < extensionLength) {
                ...
                // avoid peer set wrong direction capability, we will not filter cvo info as
                // direction for(uint32_t j = 0; j < pTrack->mConfigParam.rtp_header_extension_num;
                // j++){
    ...
    if (pt == 73) {
        ALOGW("playload type is 73(Reserved for RTCP conflict avoidance), not supported, ignore "
              "it");
        return OK;
    }
    ...
    ALOGV("%s -- ,track(%d),seqN(%d),consume time =%" PRId64 "Us", __FUNCTION__, trackIndex, seqN,
          time_endUs - time_startUs);

    return OK;
}

status_t RTPReceiver::onProcessSenderInfo(const sp<ABuffer>& buffer, uint32_t uSSRC,
                                          uint8_t trackIndex) {
