...
RTPController::RTPController(uint32_t simID, uint32_t operatorID) {
    ...
    */
    ...
    if (mDumpRTCPPacket > 0) {
        ...
        mRecvRTCPFd = open(recv_rtcp_filename, O_CREAT | O_LARGEFILE | O_TRUNC | O_RDWR,
                           S_IRUSR | S_IWUSR);
        ALOGD("open %s,recv_rtcpFd(%d)", recv_rtcp_filename, mRecvRTCPFd);

        const char* send_rtcp_filename = "/sdcard/send_rtcp_data.raw";
        mSendRTCPFd = open(send_rtcp_filename, O_CREAT | O_LARGEFILE | O_TRUNC | O_RDWR,
                           S_IRUSR | S_IWUSR);
...
status_t RTPController::setConfigParams(rtp_rtcp_config_t* pRTPNegotiatedParams,
                                        uint8_t uiMediaType) {
    ...
    ALOGV("[setConfigParams],fd0:%d,fd1:%d", (pRTPNegotiatedParams->network_info).socket_fds[0],
          (pRTPNegotiatedParams->network_info).socket_fds[1]);

    ALOGV("[setConfigParams],fd2:%d,fd3:%d", (pRTPNegotiatedParams->network_info).socket_fds[2],
          (pRTPNegotiatedParams->network_info).socket_fds[3]);
...
status_t RTPController::updateConfigParams(rtp_rtcp_config_t* pRTPNegotiatedParams,
                                           uint8_t uiMediaType) {
    ALOGV("%s", __FUNCTION__);
...
static uint16_t u16at(const uint8_t* data) { return data[0] << 8 | data[1]; }

static uint32_t u32at(const uint8_t* data) { return u16at(data) << 16 | u16at(&data[2]); }
...
void RTPController::onMessageReceived(const sp<AMessage>& msg) {
    ...
    case kWhatHoldOn: {
...
#ifdef DEBUG_ENABLE_ADAPTATION
                    ALOGD("RTPSender::kWhatAdjustEncBitRate,enable_adaptation=%d",
                          enable_adaptation);
        ...
        case kWhatReceiverNotify: {
            ...
            if (what == RTPReceiver::kWhatStartRR) {
                ...
                ALOGD("RTPReceiver::kWhatSendTMMBR,isReduce=%d,tmmbr_fci=%p", isReduce, data);
#ifdef DEBUG_ENABLE_ADAPTATION
                ...
                if (trackIndex == IMSMA_RTP_VIDEO) {
                    ...
                    ALOGD("RTPReceiver::kWhatUpdateSSRC,set mDowngradeEnable true %d",
                          mDowngradeEnable);
                }
                ...
                if (mHandoverState > 0) {
                    ALOGD("RTPReceiver::kWhatDropCall mHandoverState=%d we can't drop call",
                          mHandoverState);
                } else if (mDowngradeEnable == false)
                    ALOGD("RTPReceiver::kWhatDropCall mDowngradeEnable=%d we can't drop call",
                          mDowngradeEnable);
                else {
                    sp<AMessage> notify = mNotify->dup();
                ...
                if (mRecLatency == true && mLastEncBitrate != 0 && uiEncBitRate > mLastEncBitrate) {
                    ALOGW("we can't adjust bitrate mRecLatency=%d new bitrate %d > %d", mRecLatency,
                          uiEncBitRate, mLastEncBitrate);
                    return;
                }
        ...
        case kWhatRTCPTimer: {
            ...
            if (mReceiveRTCPPacket == false && (++mNoRTCPCount > mRTCPTimerInterval) &&
                mNoRTPFlag) {
                if (mOperatorID != 8) {
                    ...
                    ALOGE("RTCP Timer timeout %d s, mOperatorID=%d we don't drop call",
                          mRTCPTimerInterval, mOperatorID);
                }
    ...
    case kWhatSendSLI: {
        ...
        case kWhatSetCVOinfo: {
            ...
            if (mVideoRTPSender.get()) {
                mVideoRTPSender->setVideoOrientation((uint8_t)rotation, (uint8_t)camera_facing,
                                                     (uint8_t)flip);
            }
...
status_t RTPController::onSetConfigParams(rtp_rtcp_config_t* pRTPNegotiatedParams,
                                          uint8_t uiMediaType) {
    if (uiMediaType == IMSMA_RTP_VIDEO) {
        ...
        ALOGD("\t ext num:%d,max allow ext num:%d", mVideoConfigParam.rtp_header_extension_num,
              max_allow_ext_num);
        ...
        for (uint32_t j = 0; j < mVideoConfigParam.rtp_header_extension_num; j++) {
            mVideoConfigParam.rtp_ext_map[j].extension_id =
                    (pRTPNegotiatedParams->rtp_ext_map[j]).extension_id;
            mVideoConfigParam.rtp_ext_map[j].direction =
                    (pRTPNegotiatedParams->rtp_ext_map[j]).direction;
            memcpy(mVideoConfigParam.rtp_ext_map[j].extension_uri,
                   (pRTPNegotiatedParams->rtp_ext_map[j]).extension_uri,
                   sizeof((pRTPNegotiatedParams->rtp_ext_map[j]).extension_uri));

            ALOGD("\t ext id:%d,direction:%d,uri:%s", mVideoConfigParam.rtp_ext_map[j].extension_id,
                  mVideoConfigParam.rtp_ext_map[j].direction,
                  mVideoConfigParam.rtp_ext_map[j].extension_uri);
        }
        ...
        ALOGD("\t media(0x%x),mime(0x%x),PT(%d),profile(0x%x),sampleRate(%d)",
              ...
              mVideoConfigParam.rtp_payload_type, mVideoConfigParam.rtp_profile,
              mVideoConfigParam.sample_rate);
        ...
        ALOGD("\t b=AS(%d),packetize_mode(%d),b=RS(%" PRId64 "),b=RR(%" PRId64 "),rtcp_rsize(%d)",
              ...
              mVideoConfigParam.rtcp_sender_bandwidth, mVideoConfigParam.rtcp_receiver_bandwidth,
              mVideoConfigParam.rtcp_reduce_size);
        ...
        for (uint8_t i = 0; i < mVideoConfigParam.rtcp_fb_param_num; i++) {
            mVideoConfigParam.rtcp_fb_type[i].rtcp_fb_id =
                    pRTPNegotiatedParams->rtcp_fb_type[i].rtcp_fb_id;
            mVideoConfigParam.rtcp_fb_type[i].rtcp_fb_param =
                    pRTPNegotiatedParams->rtcp_fb_type[i].rtcp_fb_param;

            memcpy(mVideoConfigParam.rtcp_fb_type[i].rtcp_fb_sub_param,
                   pRTPNegotiatedParams->rtcp_fb_type[i].rtcp_fb_sub_param,
                   sizeof(pRTPNegotiatedParams->rtcp_fb_type[i].rtcp_fb_sub_param));

            ALOGV("\t Feedback[%d]:fb_id(%d),fb_param(%d),fb_sub_param(%s)", i,
                  mVideoConfigParam.rtcp_fb_type[i].rtcp_fb_id,
                  mVideoConfigParam.rtcp_fb_type[i].rtcp_fb_param,
            ...
            if (mVideoConfigParam.rtcp_fb_type[i].rtcp_fb_id == IMSMA_CCM &&
                mVideoConfigParam.rtcp_fb_type[i].rtcp_fb_param == IMSMA_TMMBR) {
                bSupportTMMBR = true;
            }
        }
        ...
        ALOGI("\t m_Trr_interval=%" PRId64 "us  SupportTMMBR=%d : %d", m_Trr_interval,
              bSupportTMMBR, mSupportTMMBR);

        mVideoConfigParam.network_info.ebi = (pRTPNegotiatedParams->network_info).ebi;
        mVideoConfigParam.network_info.interface_type =
                (pRTPNegotiatedParams->network_info).interface_type;
        mVideoConfigParam.network_info.dscp = (pRTPNegotiatedParams->network_info).dscp;
        mVideoConfigParam.network_info.soc_priority =
                (pRTPNegotiatedParams->network_info).soc_priority;

        mVideoConfigParam.network_info.remote_rtp_port =
                (pRTPNegotiatedParams->network_info).remote_rtp_port;
        mVideoConfigParam.network_info.remote_rtcp_port =
                (pRTPNegotiatedParams->network_info).remote_rtcp_port;
        mVideoConfigParam.network_info.local_rtp_port =
                (pRTPNegotiatedParams->network_info).local_rtp_port;
        mVideoConfigParam.network_info.local_rtcp_port =
                (pRTPNegotiatedParams->network_info).local_rtcp_port;
        ...
        memcpy(mVideoConfigParam.network_info.remote_rtp_address,
               (pRTPNegotiatedParams->network_info).remote_rtp_address,
               sizeof((pRTPNegotiatedParams->network_info).remote_rtp_address));
        memcpy(mVideoConfigParam.network_info.remote_rtcp_address,
               (pRTPNegotiatedParams->network_info).remote_rtcp_address,
               sizeof((pRTPNegotiatedParams->network_info).remote_rtcp_address));

        memcpy(mVideoConfigParam.network_info.local_rtp_address,
               (pRTPNegotiatedParams->network_info).local_rtp_address,
               sizeof((pRTPNegotiatedParams->network_info).local_rtp_address));

        memcpy(mVideoConfigParam.network_info.local_rtcp_address,
               (pRTPNegotiatedParams->network_info).local_rtcp_address,
               sizeof((pRTPNegotiatedParams->network_info).local_rtcp_address));

        mVideoConfigParam.network_info.remote_addr_type =
                (pRTPNegotiatedParams->network_info).remote_addr_type;
        mVideoConfigParam.network_info.local_addr_type =
                (pRTPNegotiatedParams->network_info).local_addr_type;

        ALOGI("\t NetworkInfo:remote address type:%x",
              mVideoConfigParam.network_info.remote_addr_type);
        ...
        if (mVideoConfigParam.network_info.remote_addr_type == ViLTE_IPv4) {
            ALOGI("\t NetworkInfo:remote rtp address IPv4:%d.%d.%d.%d",
                  mVideoConfigParam.network_info.remote_rtp_address[0],
                  mVideoConfigParam.network_info.remote_rtp_address[1],
                  mVideoConfigParam.network_info.remote_rtp_address[2],
                  mVideoConfigParam.network_info.remote_rtp_address[3]);

            ALOGI("\t NetworkInfo:remote rtcp address IPv4:%d.%d.%d.%d",
                  mVideoConfigParam.network_info.remote_rtcp_address[0],
                  mVideoConfigParam.network_info.remote_rtcp_address[1],
                  mVideoConfigParam.network_info.remote_rtcp_address[2],
                  mVideoConfigParam.network_info.remote_rtcp_address[3]);
            ...
            for (uint8_t i = 0; i < 8; i++) {
                char temp_addr[8];
                sprintf(temp_addr, "%x:",
                        mVideoConfigParam.network_info.remote_rtp_address[i * 2] << 8 |
                                mVideoConfigParam.network_info.remote_rtp_address[i * 2 + 1]);
        ...
        if (mVideoConfigParam.network_info.local_addr_type == ViLTE_IPv4) {
            ALOGI("\t NetworkInfo:local rtp address IPv4:%d.%d.%d.%d",
                  mVideoConfigParam.network_info.local_rtp_address[0],
                  mVideoConfigParam.network_info.local_rtp_address[1],
                  mVideoConfigParam.network_info.local_rtp_address[2],
                  mVideoConfigParam.network_info.local_rtp_address[3]);

            ALOGI("\t NetworkInfo:local rtcp address IPv4:%d.%d.%d.%d",
                  mVideoConfigParam.network_info.local_rtcp_address[0],
                  mVideoConfigParam.network_info.local_rtcp_address[1],
                  mVideoConfigParam.network_info.local_rtcp_address[2],
                  mVideoConfigParam.network_info.local_rtcp_address[3]);
            ...
            for (uint8_t i = 0; i < 8; i++) {
                char temp_addr[8];
                sprintf(temp_addr, "%x:",
                        mVideoConfigParam.network_info.local_rtp_address[i * 2] << 8 |
                                mVideoConfigParam.network_info.local_rtp_address[i * 2 + 1]);
                strncat(rtp_addr_string, temp_addr, 8);
            }
            ...
            for (uint8_t i = 0; i < 8; i++) {
                char temp_addr[8];
                sprintf(temp_addr, "%x:",
                        mVideoConfigParam.network_info.local_rtcp_address[i * 2] << 8 |
                                mVideoConfigParam.network_info.local_rtcp_address[i * 2 + 1]);
                strncat(rtcp_addr_string, temp_addr, 8);
            }
        ...
        strncpy(mVideoConfigParam.network_info.ifname, (pRTPNegotiatedParams->network_info).ifname,
                16);
        mVideoConfigParam.network_info.ifname[15] = '\0';
        mVideoConfigParam.network_info.uid = (pRTPNegotiatedParams->network_info).uid;
        mVideoConfigParam.network_info.rtp_direction =
                (pRTPNegotiatedParams->network_info).rtp_direction;
        ...
        ALOGI("\t NetworkInfo:network_id[%d], uid=%d, rtp_direction[%d]  ifname size[%zu][%s]",
              ...
              mVideoConfigParam.network_info.rtp_direction,
              strlen(mVideoConfigParam.network_info.ifname), mVideoConfigParam.network_info.ifname);

        memcpy(mVideoConfigParam.network_info.socket_fds,
               (pRTPNegotiatedParams->network_info).socket_fds,
               sizeof((pRTPNegotiatedParams->network_info).socket_fds));

        // reduce log
        for (uint8_t ifd = 0; ifd < 2 /*VILTE_BIND_FD_NUMBER*/; ifd++) {
            ALOGI("\t NetworkInfo:socket_fds[%d] = %d", ifd,
                  (mVideoConfigParam.network_info.socket_fds)[ifd]);
        }
...
status_t RTPController::onUpdateConfigParams(rtp_rtcp_config_t* pRTPNegotiatedParams,
                                             uint8_t uiMediaType) {
    if (uiMediaType == IMSMA_RTP_VIDEO) {
        ...
        if (mVideoConfigParam.media_type != pRTPNegotiatedParams->media_type ||
            mVideoConfigParam.mime_Type != pRTPNegotiatedParams->mime_Type) {
            ALOGE("%s, not support update mime(%d,%d->%d,%d)", __FUNCTION__,
                  mVideoConfigParam.media_type, mVideoConfigParam.mime_Type,
        ...
        for (uint32_t j = 0; j < mVideoConfigParam.rtp_header_extension_num; j++) {
            ...
            ALOGD("\t ext id:%d,uri:%s", mVideoConfigParam.rtp_ext_map[j].extension_id,
                  mVideoConfigParam.rtp_ext_map[j].extension_uri);
        }

        // need handle special case for rr=0, rs=0
        if (mVideoConfigParam.rtcp_sender_bandwidth == 0 &&
            mVideoConfigParam.rtcp_receiver_bandwidth == 0) {
            if (pRTPNegotiatedParams->rtcp_sender_bandwidth != 0 ||
                pRTPNegotiatedParams->rtcp_receiver_bandwidth != 0) {
        ...
        ALOGI("\t m_Trr_interval=%" PRId64 "us, bSupportTMMBR = %d : %d", m_Trr_interval,
              bSupportTMMBR, mSupportTMMBR);
        ...
        if (mVideoConfigParam.network_info.remote_rtp_port !=
            (pRTPNegotiatedParams->network_info).remote_rtp_port) {
            ALOGI("\t NetworkInfo remote port change:before port:[rtp:%d][rtcp:%d],now "
                  "port:[rtp:%d][rtcp:%d]",
                  mVideoConfigParam.network_info.remote_rtp_port,
                  mVideoConfigParam.network_info.remote_rtcp_port,
                  (pRTPNegotiatedParams->network_info).remote_rtp_port,
        ...
        if (mVideoConfigParam.network_info.local_rtp_port !=
            (pRTPNegotiatedParams->network_info).local_rtp_port) {
            ALOGI("\t NetworkInfo local port change:before port:[rtp:%d][rtcp:%d],now "
                  "port:[rtp:%d][rtcp:%d]",
                  mVideoConfigParam.network_info.local_rtp_port,
                  mVideoConfigParam.network_info.local_rtcp_port,
                  (pRTPNegotiatedParams->network_info).local_rtp_port,
        ...
        if (isPortChange == true || isTagChange == true) {
            ...
            memcpy(param_rtcp.local_address, mVideoConfigParam.network_info.local_rtcp_address,
                   sizeof(mVideoConfigParam.network_info.local_rtcp_address));
            param_rtcp.local_port = mVideoConfigParam.network_info.local_rtcp_port;
            param_rtcp.tag = mVideoConfigParam.network_info.tag;

            memcpy(param_rtcp.peer_address, mVideoConfigParam.network_info.remote_rtcp_address,
                   sizeof(mVideoConfigParam.network_info.remote_rtcp_address));
            ...
            memcpy(param_rtp.local_address, mVideoConfigParam.network_info.local_rtp_address,
                   sizeof(mVideoConfigParam.network_info.local_rtp_address));
            param_rtp.local_port = mVideoConfigParam.network_info.local_rtp_port;
            param_rtp.tag = mVideoConfigParam.network_info.tag;

            memcpy(param_rtp.peer_address, mVideoConfigParam.network_info.remote_rtp_address,
                   sizeof(mVideoConfigParam.network_info.remote_rtp_address));
            ...
            if (uiLastMBR_DL < uiNewMBR_DL) {
                ...
                */
                ...
                if (uiNewMBR_DL < mVideoConfigParam.rtp_packet_bandwidth) {
                    ALOGI("\t NetworkInfo:uiLastMBR_DL=%dkbps,uiNewMBR_DL=%dkbps "
                          "rtp_packet_bandwidth=%d kWhatUpdateVB",
                    ...
                    if (!mSupportTMMBR) {
                        ...
                        ALOGI("\t NetworkInfo:bSupportTMMBR[%d:%d] not notify UPFATE",
                              bSupportTMMBR, mSupportTMMBR);
                    }
                }
                ...
                if(!bSupportTMMBR){
                    ...
                    //but we don't think it's necessary, we should trigger sip update as soon as
                possible
...
status_t RTPController::onAddStream(int32_t rtpPath, int32_t trackIndex) {
    ...
    if (trackIndex & IMSMA_RTP_VIDEO) {
        if (!mVideoRTCPSocketWrapper.get()) {
            ...
            ALOGI("%s,findSocketFd,rtcp local port:%d,fd:%d", __FUNCTION__,
                  mVideoConfigParam.network_info.local_rtcp_port, socket_fd);
            ...
            memcpy(param.local_address, mVideoConfigParam.network_info.local_rtcp_address,
                   sizeof(mVideoConfigParam.network_info.local_rtcp_address));
            param.local_port = mVideoConfigParam.network_info.local_rtcp_port;
            param.tag = mVideoConfigParam.network_info.tag;

            memcpy(param.peer_address, mVideoConfigParam.network_info.remote_rtcp_address,
                   sizeof(mVideoConfigParam.network_info.remote_rtcp_address));
        ...
        if (!mVideoRTPSocketWrapper.get()) {
            ...
            ALOGI("%s,findSocketFd,rtp local port:%d,fd:%d", __FUNCTION__,
                  mVideoConfigParam.network_info.local_rtp_port, socket_fd);
            ...
            ALOGI("%s,isBlock=%d,sendBufferSize:%d,receiveBufferSize:%d", __FUNCTION__,
                  param.isBlock, param.sendBufferSize, param.receiveBufferSize);
            param.dscp = mVideoConfigParam.network_info.dscp;
            param.priority = mVideoConfigParam.network_info.soc_priority;
            param.network_id = mVideoConfigParam.network_info.network_id;
            strncpy(param.ifname, mVideoConfigParam.network_info.ifname, 16);
            param.ifname[15] = '\0';
            param.uid = mVideoConfigParam.network_info.uid;
            memcpy(param.local_address, mVideoConfigParam.network_info.local_rtp_address,
                   sizeof(mVideoConfigParam.network_info.local_rtp_address));
            param.local_port = mVideoConfigParam.network_info.local_rtp_port;
            param.tag = mVideoConfigParam.network_info.tag;

            memcpy(param.peer_address, mVideoConfigParam.network_info.remote_rtp_address,
                   sizeof(mVideoConfigParam.network_info.remote_rtp_address));
        ...
        if ((rtpPath & IMSMA_RTP_UPLINK) && !(mVideoAddedPath & IMSMA_RTP_UPLINK)) {
            ...
            mVideoRTPSender =
                    new RTPSender(mSSRC, mVideoRTPSocketWrapper, notify, mSimID, mOperatorID);
        ...
        if ((rtpPath & IMSMA_RTP_DOWNLINK) && !(mVideoAddedPath & IMSMA_RTP_DOWNLINK)) {
            ...
            mRTPReceiver->addStream(&mVideoConfigParam, mVideoRTPSocketWrapper,
                                    mVideoAccessUnitNotify, IMSMA_RTP_VIDEO);
...
status_t RTPController::onRemoveStream(int32_t rtpPath, int32_t trackIndex) {
    ...
    /*
        ...
        if(mAddedTrack == 0){
            ...
            //so the message handling in onMessagereceive will be complete
            //before unregisterhandler can return
            //so caller can release RTPController safely even there are messages receving from other
       thread if(mLooper.get()){ ALOGI("stop RTPController looper"); mLooper->stop();
                mLooper->unregisterHandler(mReflector->id());
                mLooper = NULL;
            }
        }
...
status_t RTPController::onStart(int32_t rtpPath, int32_t trackIndex) {
    ...
    ALOGI("%s,active track:0x%x, video active path:0x%x", __FUNCTION__, mActiveTrack,
          mVideoActivePath);
...
status_t RTPController::onResume(uint8_t rtpPath,uint8_t trackIndex){
    ...
    if(trackIndex & IMSMA_RTP_VIDEO){
        ...
        startSendRTCPPacket(0,trackIndex); // peer will need the new RTCP info with SR to sync
multi-track as soon as possible mStartedRTCPSchedule = true;
    }
    return OK;
}
*/
...
int32_t RTPController::findSocketFd(uint16_t port) {
    ...
    for (uint8_t i = 0; i < fd_num; i++) {
        ...
        if (fd != -1) {
            ...
            int res = getsockname(fd, (struct sockaddr*)&sock_Addr, (socklen_t*)&sock_AddrLen);
...
status_t RTPController::queueRTCPPacket(sp<ABuffer>& packet, uint8_t trackIndex) {
    ...
    ALOGI("%s,rtcp_size = %zu,new m_avg_rtcp_size = %d", __FUNCTION__, rtcp_size, m_avg_rtcp_size);
    ...
    while (offset < packet->size()) {
        ...
        if (packet->size() - offset < length) {
            ALOGE("%s,invalid rtcp packet(size:%zu,offset:%u,length:%u)", __FUNCTION__,
    ...
    if (offset != packet->size()) {
        ALOGE("%s,invalid rtcp packet,offset(%u),packet size(%zu)", __FUNCTION__, offset,
              packet->size());
    }
...
void RTPController::stopRtcpTimer() { mRTCPTimer = false; }
...
int64_t RTPController::calculateNextInterval() {
    ...
    t_rr = (int64_t)8 * (m_avg_rtcp_size + IMSMA_IP_IPSec_UDP_HEADER) * 1000000ll /
           rtcp_bandwidth;  // us
        ...
        if (mNextScheduleRTCPinfo.mRTCPMode == REGULAR_MODE) {
            if (!mNextScheduleRTCPinfo.mFeedBackFlag) {
                ...
                if ((m_Trr_interval > 0) && (tc - m_Trr_last < m_Trr_interval)) {
                    ...
                    m_AllowEarlyRtcp = true;
                    ALOGD("%s,regular,no fb,discard by trr-int(c:%" PRId64 "us,last:%" PRId64
                          "us,trr-int:%" PRId64 "us)",
...
status_t RTPController::makeAndsendRTCP(uint8_t trackIndex) {
    if (trackIndex & IMSMA_RTP_VIDEO) {
        ...
        m_avg_rtcp_size = rtcp_size / 16 + 15 * m_avg_rtcp_size / 16;
        ALOGI("%s,rtcp_size = %zu,new m_avg_rtcp_size = %d", __FUNCTION__, rtcp_size,
              m_avg_rtcp_size);
        ...
        if (write_size < 0) {
            ...
            ALOGE("%s,writeSock write partial data:%d/%zu", __FUNCTION__, write_size,
                  buffer->size());
        }
...
void RTPController::checkAndAddFB(uint32_t fb_type, int64_t max_delayUs) {
    ...
    if (mNextScheduleRTCPinfo.mFeedBackFlag != 0) {
        ALOGI("%s,combine this FB(0x%x) with the next RTCP(already contain FB)", __FUNCTION__,
              fb_type);
        ...
        if (m_AllowEarlyRtcp) {
            ...
            */
            ...
            ALOGI("%s,allow early rtcp,keep original mRtcp_tp:%" PRId64 "", __FUNCTION__, mRTCP_tp);
...
void RTPController::onSendGenericNack(sp<ABuffer> buffer) {
    ...
    for (uint8_t i = 0; i < mVideoConfigParam.rtcp_fb_param_num; i++) {
        ...
        if ((fb_id == IMSMA_NACK) &&
            ((fb_param == IMSMA_GENERIC_NACK) || (fb_param == IMSMA_NONE))) {
...
bool RTPController::scanTMMBR(sp<ABuffer> rtcp_packet) {
    ...
    while (offset < rtcp_packet->size()) {
        ...
        if (rtcp_packet->size() - offset < length) {
            ALOGE("%s,invalid rtcp packet(size:%zu,offset:%u,length:%u)", __FUNCTION__,
                  rtcp_packet->size(), offset, length);
            return false;
        }
