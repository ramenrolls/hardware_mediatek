...
RTPSender::RTPSender(uint32_t ssrc, sp<SocketWrapper> spRTPSocketWrapper, sp<AMessage> notify,
                     uint32_t simID, uint32_t operatorID) {
        ...
        case kWhatSendRTPPacket: {
            ...
            if (rtp_generation == mRTPGeneration) {
                while (!mRTPPacketQueue.empty()) {
                    ...
                    if (write_size < 0) {
                        ALOGW("kWhatSendRTPPacket,writeSock fail err(%d)(%s)", write_size,
                              strerror(-write_size));

                        if ((write_size == -EAGAIN) || (write_size == -EWOULDBLOCK) ||
                            (write_size == -EPERM)) {
                            ...
                            break;
                            // mSendRTPEventPending = true;
                        } else if ((write_size == -ECONNREFUSED) || (write_size == -ENETUNREACH) ||
                                   (write_size == -EINVAL)) {
                            ALOGW("ignore write error:%d,mLastRTPSeqNum:%u,mRTPSentCount:%u",
                                  write_size, mAdaInfo->getLastRTPSeqNum(),
                                  mAdaInfo->getRTPSentCount());
                        } else {
                            CHECK(write_size > 0);
                            break;
                        }
                    } else if ((uint32_t)write_size != (rtpPacket->size())) {
                        ALOGE("kWhatSendRTPPacket,writeSock write partial data:%d/%zu", write_size,
                              rtpPacket->size());
                    }
                    ...
                    ALOGV("kWhatSendRTPPacket,dequeue rtp packet(seqNum:%d) from "
                          "mRTPPacketQueue(%zu)",
                          mAdaInfo->getLastRTPSeqNum(), mRTPPacketQueue.size());

                    size_t frame_boundary = 0;

                    if ((rtp_meta->findSize("frame_boundary", &frame_boundary)) &&
                        (frame_boundary > 0)) {
                        int32_t token = 0;
                        rtp_meta->findInt32("token", &token);
                ...
                ALOGD("kWhatSendRTPPacket,dequeue rtp packet(seqNum:%d-%d)", firstSeq,
                      mAdaInfo->getLastRTPSeqNum());
            } else {
...
status_t RTPSender::onSetConfigParams(rtp_rtcp_config_t* pRTPNegotiatedParams) {
    ...
    ALOGD("\t ext num:%d,max allow ext num:%d", mConfigParam.rtp_header_extension_num,
          max_allow_ext_num);
    ...
    for (uint32_t j = 0; j < mConfigParam.rtp_header_extension_num; j++) {
        mConfigParam.rtp_ext_map[j].extension_id =
        ...
        memcpy(mConfigParam.rtp_ext_map[j].extension_uri,
    ...
    ALOGD("\t media(0x%x),mime(0x%x),PT(%d),profile(0x%x),sampleRate(%d)", mConfigParam.media_type,
          mConfigParam.mime_Type, mConfigParam.rtp_payload_type, mConfigParam.rtp_profile,
          mConfigParam.sample_rate);
    ...
    ALOGD("\t b=AS(%d),packetize_mode(%d),b=RS(%" PRId64 "),b=RR(%" PRId64 "),rtcp_rsize(%d)",
          ...
          mConfigParam.rtcp_sender_bandwidth, mConfigParam.rtcp_receiver_bandwidth,
          mConfigParam.rtcp_reduce_size);

    ALOGD("\t fb_num(%d) m_extmap_CVO_supported=%d", mConfigParam.rtcp_fb_param_num,
          m_extmap_CVO_supported);
...
status_t RTPSender::onQueueAccessUnit(const sp<ABuffer>& accessUnit) {
    ...
    if (mConfigParam.media_type == IMSMA_RTP_VIDEO &&
        mConfigParam.mime_Type == IMSMA_RTP_VIDEO_H264) {
    ...
    else if (mConfigParam.media_type == IMSMA_RTP_VIDEO &&
             mConfigParam.mime_Type == IMSMA_RTP_VIDEO_HEVC) {
...
status_t RTPSender::queueSingleNALUnitPacket(const sp<ABuffer> accessUnit) {
    ...
    ALOGD("%s,accu num(%d),accu size(%zu),timeUs(%" PRId64
          " us),rtpTime(%u),rotation(%d),face(%d),flip(%d)",
          __FUNCTION__, mAccuReceived, size, timeUs, rtpTime, ccw_rotation, camera_facing, flip);

    // only single NAL in one RTP packet support
    while (getNextNALUnit(&data, &size, &nalStart, &nalSize, true /* startCodeFollows */) == OK) {
        // ToDo:
        // check NAL type
...
status_t RTPSender::queueFUorSTAPPacket(const sp<ABuffer> accessUnit) {
    ...
    while (getNextNALUnit(&data, &size, &nalStart, &nalSize, true /* startCodeFollows */) == OK) {
        ...
        if (!out.get()) {
            ...
            //  to avoid the receiver know this is the last rtp pakcet until receiving the next rtp
            //  header
        ...
        if (outBytesUsed + bytesNeeded > out->capacity()) {
            ...
            if (isFirstNAL && outBytesUsed + nalSize <= out->capacity()) {
            ...
            if (!isFirstNAL) {
                ...
                if (size <= 0 && emitSingleNALPacket) {
                    ALOGD("%s,last packet of the frame,total packets(%d)", __FUNCTION__,
                          packetCount);
                    rtp_meta->setSize("frame_boundary", 1);
    ...
    if (out.get()) {
        ...
        rtp_meta->setSize("payload_size", payload_size);
        ALOGV("%s,no more nal,queue last packet(offset(%zu),size(%zu))", __FUNCTION__,
              out->offset(), out->size());
        // revise offset to rtp header
        int32_t rtpOffset = 0;
        rtp_meta->findInt32("rtpOffet", &rtpOffset);
    ...
    while (getNextNALUnit(&data, &size, &nalStart, &nalSize, true /* startCodeFollows */) == OK) {
        ...
        if (outBytesUsed + bytesNeeded > out->capacity()) {
            ...
            if (!isFirstNAL) {
                ...
                if (size <= 0 && emitSingleNALPacket) {
                    ALOGV("%s,last packet of the frame,total packets(%d)", __FUNCTION__,
                          packetCount);
                    rtp_meta->setSize("frame_boundary", 1);
...
status_t RTPSender::queueFragmentNALUnitPacket(const sp<ABuffer> accessUnit, size_t remain_size,
                                               const uint8_t* nalStart, const size_t nalSize,
                                               sp<ABuffer>& out, int32_t* packetCount) {
...
status_t RTPSender::queueFragmentNALUnitPacket_HEVC(const sp<ABuffer> accessUnit,
                                                    size_t remain_size, const uint8_t* nalStart,
                                                    const size_t nalSize, sp<ABuffer>& out,
                                                    int32_t* packetCount) {
...
void RTPSender::queueRTPPacket(sp<ABuffer> rtpPacket) {
    ...
    if ((frame_start || frame_boundary) && mRTPPacketQueue.size() > 10) {
        ALOGD("%s,rtp queue size(%zu),frame_start =%zu,frame_boundary=%zu", __FUNCTION__,
              mRTPPacketQueue.size(), frame_start, frame_boundary);
        uint32_t target = 0;
...
status_t RTPSender::addRTPFixHeader(sp<ABuffer> rtpPacket) {
    ...
    // and should consider:if hold on back,the timeUs will change,but the rtpTime whether need
    // change whether need re-map(rtp->ntp) after hold on back whether the inital rtp time need new
    // rand()
    ...
    ALOGV("%s,before setRange,offset(%zu),size(%zu)", __FUNCTION__, rtpPacket->offset(),
          rtpPacket->size());
    rtpPacket->setRange(rtpPacket->offset(), rtpPacket->size() + 12);
    ALOGV("%s,after setRange,offset(%zu),size(%zu)", __FUNCTION__, rtpPacket->offset(),
          rtpPacket->size());
...
status_t RTPSender::addRTPExtHeader(sp<ABuffer> rtpPacket) {
    ...
    if (rtp_ext_header_len == 0) {
        ...
        uint16_t rtp_ext_element_len =
                (rtp_ext_with_padding - 4) /
                4;  // subtract header of 4 bytes and change to in 32-bit words
...
status_t RTPSender::onProcessReportBlock(const sp<ABuffer>& packet) {
    ...
    if (mConfigParam.network_info.interface_type) {
        ...
        ALOGD("%s, not over LTE %d, not expand", __FUNCTION__,
              mConfigParam.network_info.interface_type);
    } else {
...
void RTPSender::onProcessTMMBR(const sp<ABuffer> tmmbr_fci) {
    ...
    if ((TestMode && (RTPMap & 0x1)) || mConfigParam.network_info.interface_type) {
        sigInfo.canExpand = false;
        ALOGD("%s, not over LTE %d, or test mode 0x1=%d:%d not expand", __FUNCTION__,
              mConfigParam.network_info.interface_type, TestMode, (RTPMap & 0x1));
    } else {
        sigInfo.canExpand = ifRiseBitrateBaseSignal(&sigInfo.ratio);
    }
...
uint32_t RTPSender::requestRTPQueueDuration() {
    ...
    ALOGD("%s,rtp queue size(%zu) durationUs(%d)", __FUNCTION__, mRTPPacketQueue.size(),
          durationUs);
...
bool RTPSender::ifRiseBitrateBaseSignal(int32_t* ratio) {
    *ratio = 100;
    ...
    if (stable) {
        if (sigLong != ImsSignal::Signal_STRENGTH_NONE_OR_UNKNOWN && sigLong > mLastReduceSignal &&
