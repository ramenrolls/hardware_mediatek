...
static uint16_t getRtpSN(const sp<ABuffer>& buf) { return u16at(&((buf->data())[2])); }
...
SocketWrapper::~SocketWrapper() {
    ...
    SOCKETWRAPPER_LOGI("%s:dataUsage fd=%d mSendDataUasage=%lld, mrecvDataUasage=%lld",
                       __FUNCTION__, mParam.sockfd, (long long)mSendDataUasage,
                       (long long)mrecvDataUasage);
...
int SocketWrapper::setParam(Sock_param_t param) {
    ...
    if (mParam.peer_port != 0) {
        ...
        if (mParam.peer_port != param.peer_port) {
            SOCKETWRAPPER_LOGI("%s: peer port from %d to %d and put address", __FUNCTION__,
        ...
        if (mParam.tag != param.tag) {
            ...
            SOCKETWRAPPER_LOGI("%s: RegisterSocketUserTag fd[%d] uid[%d] tag[0x%x]", __FUNCTION__,
                               mParam.sockfd, mParam.uid, mParam.tag);

            int res = qtaguid_untagSocket(mParam.sockfd);
            if (res != 0) {
                SOCKETWRAPPER_LOGE("Failed untagging socket %d (My UID=%d)", mParam.sockfd,
                                   geteuid());
            }

            int res1 = qtaguid_tagSocket(mParam.sockfd, mParam.tag, mParam.uid);
            if (res1 != 0) {
                SOCKETWRAPPER_LOGE("Failed tagging socket %d for uid %d (My UID=%d)", mParam.sockfd,
                                   mParam.uid, geteuid());
            }
        }
    ...
    if (rx_cb == NULL) {
        ...
        if (m_bStarted) {
            ...
            if (ret == 1) {
                ...
                SOCKETWRAPPER_LOGI("%s:wakeup fail, ret=%d, %s(%d)", __FUNCTION__, ret,
...
int SocketWrapper::writeSock(const sp<ABuffer>& buffer) {
    ...
    switch (mParam.protocol_version) {
        ...
        case VoLTE_Event_IPv6:
            ...
            memcpy((char*)&(addr6_ptr->sin6_addr), mParam.peer_address,
                   sizeof(addr6_ptr->sin6_addr));
    ...
    ret = sendto(fd, buffer->data(), buffer->size(), MSG_NOSIGNAL, (struct sockaddr*)&remote_addr,
                 addr_len);
#endif
    ...
    if (ret < 0) {
        mWriteFail++;
        error_no = errno;
        SOCKETWRAPPER_LOGE("%s: socket write failed %s(%d)", __FUNCTION__, strerror(error_no),
                           error_no);

        if (!(error_no == EMSGSIZE || error_no == EAGAIN || error_no == EWOULDBLOCK ||
              error_no == EPERM || error_no == ECONNREFUSED || error_no == ENETUNREACH ||
              error_no == EINVAL)) {
...
int SocketWrapper::dumpAddr(struct sockaddr* addr_ptr) {
    ...
    switch (addr_ptr->sa_family) {
        case AF_INET:
            inet_ntop(AF_INET, &(((struct sockaddr_in*)addr_ptr)->sin_addr), addr_str,
                      sizeof(addr_str));
            port = ntohs(((struct sockaddr_in*)addr_ptr)->sin_port);
            break;
        case AF_INET6:
            inet_ntop(AF_INET6, &(((struct sockaddr_in6*)addr_ptr)->sin6_addr), addr_str,
                      sizeof(addr_str));
...
int SocketWrapper::readSock(int fd, const sp<ABuffer>& buffer) {
    ...
    if (ret < 0) {
        ...
        SOCKETWRAPPER_ASSERT(((unsigned int)ret <= buffer->size()),
                             "ret(%d) > buffer_size(%d), MAX_MTU_SIZE(%d)", ret,
                             (int)buffer->size(), MAX_MTU_SIZE);
        return -1;
    }
...
int SocketWrapper::createSock(void) {
    ...
    if (mParam.protocol_version == VoLTE_Event_IPv4) {
        ...
        SOCKETWRAPPER_ASSERT(0, "%s: uknow protocol_version %d", __FUNCTION__,
                             mParam.protocol_version);
    }

    if (sockfd < 0) {
        SOCKETWRAPPER_ASSERT(0, "%s: socket create failed %s(%d)", __FUNCTION__, strerror(errno),
                             errno);
    }

    /* bind socket */
    if (bind(sockfd, (struct sockaddr*)&local_addr, addr_len) < 0) {
        SOCKETWRAPPER_ASSERT(0, "%s: socket bind failed %s(%d)", __FUNCTION__, strerror(errno),
                             errno);
    }
...
void SocketWrapper::setUdpConnect(void) {
    ...
    if (connect(mParam.sockfd, (struct sockaddr*)&remote_addr, addr_len) < 0) {
        ...
        // trigger exception
        SOCKETWRAPPER_LOGE("%s: socket connect failed %s(%d)", __FUNCTION__, strerror(errno),
                           errno);
        mError = true;
    }
...
int SocketWrapper::setSock(void) {
    SOCKETWRAPPER_LOGI("%s:enter, sockfd=%d, protocol=%d, port=%d", __FUNCTION__, mParam.sockfd,
                       mParam.protocol_version, mParam.peer_port);
    ...
    */
    ...
    switch (mParam.protocol_version) {
        case VoLTE_Event_IPv4:
            ...
            if (setsockopt(mParam.sockfd, IPPROTO_IP, IP_TOS, &optv4, sizeof(optv4)) < 0) {
                SOCKETWRAPPER_ASSERT(0, "%s: socket setsockopt dscp failed %s(%d)", __FUNCTION__,
                                     strerror(errno), errno);
            }
    ...
    if (setsockopt(mParam.sockfd, SOL_SOCKET, SO_PRIORITY, &(mParam.priority), sizeof(int)) < 0) {
        SOCKETWRAPPER_ASSERT(0, "%s: socket setsockopt pirority failed %s(%d)", __FUNCTION__,
                             strerror(errno), errno);
    }
    ...
    if (mParam.isBlock == false) {
        ...
        if (res < 0) {
            error_no = errno;
            SOCKETWRAPPER_LOGE("%s: socket set to non-blocking failed %s(%d)", __FUNCTION__,
    ...
    if (mParam.sendBufferSize != 0) {
        if (setsockopt(mParam.sockfd, SOL_SOCKET, SO_SNDBUF, &(mParam.sendBufferSize),
                       sizeof(uint32_t)) == -1) {
            SOCKETWRAPPER_ASSERT(0, "%s: set socket sendbuf size (%d) fail %s(%d)", __FUNCTION__,
                                 mParam.sendBufferSize, strerror(errno), errno);
        } else {
    ...
    if (mParam.receiveBufferSize != 0) {
        if (setsockopt(mParam.sockfd, SOL_SOCKET, SO_RCVBUF, &(mParam.receiveBufferSize),
                       sizeof(uint32_t)) == -1) {
            SOCKETWRAPPER_ASSERT(0, "%s: set socket recvbuf size (%d) fail %s(%d)", __FUNCTION__,
                                 mParam.receiveBufferSize, strerror(errno), errno);
        } else {
...
void* SocketWrapper::receiveThread(void* pParam) {
    ...
    while ((pSelf->m_bStarted) && (pSelf->mError == false)) {
        ...
        if (last_time.tv_sec == 0) {
            ...
            SOCKETWRAPPER_LOGI(
                    "IMSSOCK DEBUG fd(%d) writeCount(%u) writeFail(%u) writeSize(%lld) "
                    "receiveCount(%u) receiveSize(%lld)",
        ...
        // diff_time = (end_time.tv_sec-
        // begin_time.tv_sec)*1000+(end_time.tv_usec-begin_time.tv_usec)/1000;
